...
quality: high
full_path: ByteCTF逆向解析.full.md
meta_path: ByteCTF逆向解析.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: ByteCTF 逆向解析 - babyapk。ByteCTF babyapk 逆向 — Flutter APK blutter 解析 + IDA 找 m3N4B5V6 + Frida hook app.so + z3 解 4 轮 8 变量非线性约束 + 32e750c8... + 4 个 - 位置穷举 + byte_18E46 表。。关键路径：Flutter APK, jadx 无法看, 用...
category: reverse
subcategory: reverse
tools_used:
- IDA
...
contest_type: open
wp_url: https://www.ctfiot.com/208437.html
reasoning_chain:
- 触发点：拿到 babyapk APK，jadx 反编译全是 R8/Flutter 桩 → 假设：Flutter 应用逻辑在 Dart 编译产物里
- 动作：用 blutter 工具解析 libapp.so → 观察：拿到 main.dart 与 simple.dart 源码，定位 m3N4B5V6() 是校验入口
- 假设：加密函数是 Rust FFI 通过 cross-ref 反追到 app.so 里的 native 实现 → 动作：IDA 搜 'Simple'/'m3N4B5V6' 上溯交叉引用
- 观察：找到 4 轮 8 变量非线性表达式加密函数 → 下一步：用 z3 构造 16 条 BitVec 约束
- 假设：flag 长度存在长度检测不过 '正常 hook 没反应' → 动作：动态 HOOK + 修改长度观察 → 观察：{} 内是 36 位总长 0x2d = 45
- 动作：写 z3 脚本 v58[j*8+i] ∈ [0,0x10FFFF]，对每轮 8 表达式等于 cmp 数组 → 观察：s.check()=sat，得字符数组解
- 假设：z3 解的字符串都是可显字符 → 动作：直接十六进制拼接 → 观察：得到 32e750c8fb214562af22973fb5176b9c
- 假设：flag 含 4 个 '-'，长度 32 = 36 - 4 → 动作：枚举 4 个 '-' 位置 C(28,4)=20475 种 → 观察：byte_18E46 表匹配出正确位置
failed_attempts:
- 试图 Frida hook 各种 native 函数 → 失败：很多函数无触发，猜测有 length check
- 试图 jadx 直接看 Dart 逻辑 → 失败：Flutter app 反编译出来全是 stub
- 试图不看 m3N4B5V6 直接找 native 函数对应加密 → 失败：native 函数太多难以定位
key_observations:
- Flutter APK 逆向核心是 blutter 解析 libapp.so（jadx 无法搞定 Dart）
- Rust FFI 加密函数通过 IDA cross-ref 反追是常用手法（搜包名/函数特征）
- z3 解非线性约束 + 短解集穷举 = 混合策略必杀技
- 长度检测通过 Frida 动态试错（不断改输入长度观察 hook 触发）
- 可显字符约束（c(i) ∈ printables）是 z3 解唯一性兜底
prerequisites:
- blutter 工具使用（解析 Flutter libapp.so）
- IDA 交叉引用分析（找 FFI 加密函数）
- Frida 动态 hook（拦截 native 函数返回值）
- Z3 约束求解器 + BitVec 建模
- Python/Crypto 脚本编写
---
# ByteCTF逆向解析
