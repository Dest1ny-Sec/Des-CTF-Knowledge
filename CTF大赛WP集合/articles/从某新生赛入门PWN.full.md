...
quality: high
full_path: 从某新生赛入门PWN.full.md
meta_path: 从某新生赛入门PWN.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: '从某新生赛入门PWN。bad_c0de 某新生赛 9 题 PWN：encrypto 还原 + 32/64 位 ROP + fmtstr + 字符 shellcode + seccomp+orw。。关键路径：题 1 ezcmp: 自实现 enccrypt (rand+buff+bua+buffff XOR), srand(v0=1) 固定种子, GDB 0x4014b4 断点读 buff → ...'
category: pwn
...
contest_type: open
wp_url: https://www.ctfiot.com/80255.html
reasoning_chain:
- '题 1 ezcmp: 自实现 enccrypt (rand+buff+bua+buffff XOR) → 触发点：加密函数'
- 假设：srand(v0=1) 固定种子 → 动作：GDB 0x4014b4 断点读 buff
- 动作：发密文 → strncmp 通过 → system('/bin/sh')
- '题 2 ezr0p: 32 位栈溢出 0x20+call_system(0x08048562)+0x0804A080 bss 写 /bin/sh'
- '题 3 ezrop64: pop_rdi+binsh+ret+system → 泄 libc'
- '题 4 ezfmt: 6 个 %p 找到 name 偏移 → %7$s....+p64(0x4040a0) 读 name'
- '题 5 shellcoder: 字符限制 ''0''~''z'' 内构造 xor 自解码 syscall read'
- xor dword ptr[rsi+0x33],ecx 改写 rip + asm(shellcraft.sh()) 拼接
- '题 6 shellcode: mprotect RWX + 0x108 溢出 + ret=0x4040a0'
- '题 7 int 溢出: gets 覆盖 number!=0 触发 cat flag'
- '题 8 arrayRE: 24 数字 0-9 爆破 decode(a1,a2)=(35*(a1-48)+18*(a2-48)+2)%10'
- '题 9 intorw: seccomp 限制 open/read/write + puts_got 泄 libc → ORW open+read+puts'
failed_attempts:
- 试图不解密 enc 直接读 flag → 失败：buff 是密文
- 试图解 srand(0) 重新加密 → 失败：v0=1 不是 0
- 试图 mprotect 后直接跳 shellcode → 失败：地址空间没开 RWX
key_observations:
- encrypto 自实现加密直接 GDB 断点读密文 (srand(1) 固定种子)
- 字符限制 shellcode 用 xor [rsi+0x33], ecx 自解码 syscall
- intorw seccomp 用 libc pop_rsi/pop_rdx_r12 gadget
- 32 位 call system + /bin/sh 顺序 vs ret system 顺序差 4 字节
prerequisites:
- GDB/pwndbg 断点与内存读取
- ROP gadget 构造（pop_rdi/pop_rsi/pop_rdx）
- shellcode 字符限制 xor 自解码
- seccomp-tools dump 沙箱规则
---
# 从某新生赛入门PWN
