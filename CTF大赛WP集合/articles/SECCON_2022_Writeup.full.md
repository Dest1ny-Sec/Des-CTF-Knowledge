...
quality: high
full_path: SECCON_2022_Writeup.full.md
meta_path: SECCON_2022_Writeup.meta.md
images_removed: true
images_removed_count: 0
schema_version: v3.0.0-P0
summary: 'SECCON 2022 Writeup。SECCON 2022 koncha (scanf 栈溢出 + one_gadget) + babyfile (FILE 结构 _IO_read_base/vtable 偏移 trick 任意改 → _IO_finish 触发 system(binsh))。。关键路径：题目 1 koncha: scanf 栈溢出 + libc 地址泄露 → name[...'
category: pwn
subcategory: rop
time_required: long
difficulty_score: 4
...
contest_type: open
wp_url: https://www.ctfiot.com/74039.html
reasoning_chain:
- 题目 1 koncha scanf("%[^\n]s") name[0x30]+country[0x20] → 触发：栈溢出绕过 \n 截断
- printf("Nice to meet you, %s!\n", name) → 泄栈地址 → cyclic(88) 测溢出长度
- cyclic(88) + p64(libc_base+one_gadget 0xe3b01) → 跳 one_gadget 弹 shell
- '题目 2 babyfile menu: 1=Flush 2=Trick 0=Exit → 触发：FILE 结构 trick(offset, value)'
- _IO_FILE _flags + _IO_read_ptr/end/base + _IO_write_ptr + _chain 字段任意改
- 改 _mode=0x1 + vtable=0xa8 → 触发 _IO_doallocbuf + 分配 buffer
- 改 vtable=0x60 → 触发 _IO_underflow 设置 read_base → getheap 16 次爆破 leak
- trick(_fileno, 0x04) + trick(vtable, 0x50) → 触发 _IO_finish
- finish → puts(fp->_IO_read_ptr) 调 system → binsh 8 字节写入 free_space
failed_attempts:
- 题目 1 试图改 return address → 失败：canary 阻挡需先 leak
- 题目 2 试图覆盖 __free_hook → 失败：glibc 2.34+ 已移除
- 试图用 puts 触发 _IO_finish → 失败：vtable 必须对齐 0x50 才命中
key_observations:
- scanf("%[^\n]s") 格式串保留换行前内容（区别 %s）
- FILE 结构 vtable 偏移可触发任意 _IO_xxx 函数
- trick(offset, value) 任意改 FILE 字段是 FILE 攻击经典模式
- one_gadget 是栈溢出终极 sink 但需满足 [rsp+0x70]==NULL 等约束
prerequisites:
- scanf 格式串语义（%[^\n]s vs %s）
- FILE 结构布局 + _IO_xxx 函数指针表
- pwntools / one_gadget / cyclic 工具链
- Glibc heap + FSOP 原理
---
# SECCON 2022 Writeup
