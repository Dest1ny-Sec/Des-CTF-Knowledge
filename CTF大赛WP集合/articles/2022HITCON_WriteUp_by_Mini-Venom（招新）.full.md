...
- '0x222000'
attack_chain:
- 'Web RCE: 字符级 cookie 增量爆破恢复 server 端 NodeJS 代码'
- 'NodeJS payload: global.a1=''child''; a1=a1+''_process''; b=require(a1); c1=''cat /flag-1e''+suffix; b.exec(c1)'
- ThreadPool(10) 并发 16 字符 payload 字符串每 1 字符顺序爆破
- 'exploit 字符 1: ''global.a1 = ''child'' 字符串服务端 echo 回去'
- 'exploit 字符 2: 拼 ''a1 = a1 + ''_process'''' 等等'
- 'Reverse: Windows 驱动 IRP switch ByteOffset 0x222000..0x222070 调用 sub_1400014D0'
- sub_1400014D0(N) 把数据写到 byte_140013190+N 偏移
- 0x222000 + N*0x10 触发不同全局变量初始化
...
quality: high
full_path: 2022HITCON_WriteUp_by_Mini-Venom（招新）.full.md
meta_path: 2022HITCON_WriteUp_by_Mini-Venom（招新）.meta.md
images_removed: true
images_removed_count: 1
schema_version: v3.0.0-P0
summary: '2022 HITCON WriteUp by Mini-Venom (招新)。HITCON 字符级增量 cookie 爆破 + NodeJS child_process RCE + Windows 驱动 IOCTL。关键路径：Web RCE: 字符级 cookie 增量爆破恢复 server 端 NodeJS 代码 → NodeJS payload: global.a1=''child''; a...'
category: web
subcategory: ssrf
subcategories:
- ssrf
...
contest_type: open
wp_url: https://www.ctfiot.com/81488.html
wp_author: Mini-Venom
reasoning_chain:
- '[触发点] 服务端返回 cookie 字段按字符增量 → 假设：可逐字符爆破服务端代码 → [动作] ThreadPool(10) 并发 16 字符 / [观察] 拿到 NodeJS 代码 / [下一步] 拼 payload'
- '[触发点] 拼 global.a1=''child''; a1=a1+''_process''; b=require(a1); c1=''cat /flag-1e''+suffix → 假设：child_process.execSync 路径 / [动作] 提交 / [观察] 拿到 flag-1e... / [下一步] 继续拼 -32 等'
- '[触发点] 驱动 ByteOffset 0x222000..0x222070 步长 0x10 → 假设：dispatch IOCTL 表 / [动作] IDA 反汇编 / [观察] sub_1400014D0(N) → byte_140013190+N / [下一步] 拼全局变量'
failed_attempts:
- 试图直接拿完整 cookie → 失败：服务端只 echo 已 commit 部分
- 试图读 IRP_MJ_DEVICE_CONTROL 表 → 失败：偏移是 0x10 步长不是 0x8
key_observations:
- 增量 cookie 响应可逐字符爆破服务端代码
- NodeJS child_process.execSync 路径 = 'child'+'_process'
- Windows 驱动 IOCTL 0x222000 是 32 字节间隔 dispatch
prerequisites:
- NodeJS child_process 加载机制
- HTTP cookie 增量处理特性
- Windows驱动 IRP/IOCTL 模型
- IDA 反汇编驱动
---
