...
- easy_rgb
attack_chain:
- 'hard_php: PHP 全角字符算术 ０/０ = NAN, [''''==''$''] 取第一位, 增量运算还原字母, 拼函数调用'
- NKCTF=$$_[__]($$_[_]) → system(shell_exec) → echo `cat /flag`>3.txt
- 'easy_php: GET/POST 传 NS[CTF.go] + cmd, PDF 内嵌 JPG SHA-1 触发'
- 'baby_php: 反序列化 Welcome → Happy → Hell0 → system($_POST[1])'
- 'Webpagetest: getallheaders() 注入 (XRay 漏洞)'
- 'easy_pms: 禅道 pms 已知 CVE (https://github.com/webraybtl/zentaopms_poc)'
- 'ez_baby_apk: jadx 找加壳, frida dump dex'
- 'easy_bmp: BMP 文件头修复, 像素 stego'
- 'blue: 蓝牙 pcap 流量分析'
- '三体: 数学建模 + 文字'
- 'first spam of rabbit year: 找春节相关字符串'
- 'easy_rgb: 像素 RGB 通道 stego'
...
quality: high
full_path: 2023NKCTF-WriteUp_By_EDISEC.full.md
meta_path: 2023NKCTF-WriteUp_By_EDISEC.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: '2023 NKCTF WriteUp By EDISEC。NKCTF 2023：PHP 全角字符 + PDF-RCE + ZenTao CVE + APK + BMP stego。关键路径：hard_php: PHP 全角字符算术 ０/０ = NAN, [''''==''$''] 取第一位, 增量运算还原字母, 拼函数调用 → NKCTF=$$_[__]($$_[_]) → system(shell...'
category: web
subcategory: rce
subcategories:
- rce
...
contest_type: open
wp_url: https://www.ctfiot.com/105824.html
reasoning_chain:
- 'hard_php: $_=(０/０)._; 全角字符 + NAN → 触发点：全角 ''0'' / ''0'' = NAN 字符串'
- 假设：NAN 字符串取第一位 = 'N' → 动作：$_=$_[''=='$'] 取第 0 个字符 → $_++ 增量到字母
- 观察：构造出 'system' 字符串 → $$_[__]($$_[_]) 调用
- 动作：$_=shell_exec, $__=cat /flag → 提交 → 观察：访问 3.txt 拿 flag
- 'easy_php: get NS[CTF.go=a 触发 thinkphp 路由 → PDF 内嵌 JPG + SHA-1 is dead! 触发 PDF RCE'
- 假设：PDF 内嵌 JPG SHA-1 碰撞可执行任意代码 → 动作：构造 SHAttered PDF → 观察：RCE
- 'ZenTao PMS CVE: 禅道 pms 已知 CVE → 假设：未授权访问 + 文件上传'
- 动作：搜索 CVE-2022-XXX → 观察：拿 shell
- 'BMP stego: BMP 像素位藏密 → 假设：异或像素还原 → 动作：遍历像素 → 观察：flag'
failed_attempts:
- 试图用 ASCII 字符 0/0 → 失败：必须全角字符触发 NAN
- 试图用普通 PHP 标签写文件 → 失败：黑名单拦截
- 试图手工解 BMP LSB → 失败：必须按 RGB 通道重组
key_observations:
- PHP 0/0=NAN + 全角字符是字符构造经典
- PDF 内嵌 JPG 触发 SHA-1 碰撞 (SHAttered) 是 0day 利用
- 禅道 pms CVE 是国内常见 CMS 漏洞
- BMP 像素异或还原是 stego 经典
prerequisites:
- PHP 全角字符与 NAN 字符串处理
- SHAttered SHA-1 碰撞 PDF 原理
- 禅道 / ThinkPHP 框架 CVE 漏洞库
- BMP 文件结构 + PIL 像素遍历
---
