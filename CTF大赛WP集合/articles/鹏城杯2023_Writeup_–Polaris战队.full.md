...
quality: high
full_path: 鹏城杯2023_Writeup_–Polaris战队.full.md
meta_path: 鹏城杯2023_Writeup_–Polaris战队.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: '鹏城杯2023 Writeup –Polaris战队。鹏城杯2023 Polaris战队WP,PWN4题(6502模拟器+silent栈迁移+babyheap FSOP+咖啡机)+RE2题(Rust猜数字+RC4 PE节区)+WEB3题(反序列化+glob爆破+tera SSTI)。。关键路径：6502 CPU模拟器: get_mem/write_mem+IZX寻址模式,LDX/LDA/ST...'
category: pwn
...
contest_type: open
wp_url: https://www.ctfiot.com/143115.html
reasoning_chain:
- '6502 题 IDA 看 get_mem/write_mem + IZX 寻址 → 触发点: 自定义 CPU 模拟器 → 假设: 用 LDX/LDA/STA/ADC 组合写 shellcode'
- '动作: 拼字节码 → LDX #$f2; STX $00; LDX #$de; STX $01; LDX #$00; LDA ($00,X); ... 等'
- '观察: payload 写入 shellcode 触发 get_mem/write_mem → /bin/sh 拿到'
- 'silent 题: read 0x100 溢出 buf[64] → 假设栈迁移到 bss → 动作: cyclic(64) + flat([0x603000-0x100, ...]) 重入 _start'
- '下一步: 爆破 write (1/4096 成功率) → libc_addr - 0x110020 → 汇编 shellcode open/read/write flag'
- 'babyheap 题: readn 触发 off-by-one → 假设: chunk overlap → 动作: 0x408 + 0x4f8 构造 overlap'
- '观察: tcache 劫持 stdout 泄露栈 → ROP 栈劫持 → 完成'
- 'Auto_Coffee_machine 题: change_default edit 越界 → re(4421) 密码 bypass 后 ed(idx, -0x20, ...) 改 stdout → __free_hook → system'
- 'RE-安全编程: Rust 猜数字 → 改执行流程到解密函数 → flag'
- 'Bad PE: RC4 th3k3y! 解密节区 → IDA 看出 XOR → flag'
- 'web1 PHP 反序列化链: Hacker->__get->finish->hacker() → 动作: 构造 H:Hacker 链'
- web2 glob 爆破 backdoor_*.php 32 字符 → ?title[]=.php + data[]=<?php system("cat /f*") 双参数覆盖
failed_attempts:
- '6502 试图用现成 shellcode → 失败: 字节码必须自己拼 LDX/LDA/ADC/STA 指令组合'
- 'silent 试图 ROPgadget 拼链 → 失败: write 不可靠, 必须爆破'
- 'babyheap 试图 fastbin attack → 失败: glibc 2.38 已用 tcache'
- 'web2 试图文件包含 → 失败: 必须 glob:// 爆破 32 字符后门'
key_observations:
- 6502/ARM 异构 CPU 模拟器 PWN 题要识别寻址模式 (IZX) + 指令集
- 栈迁移爆破 write 是 2023 新热点, 1/4096 成功率靠统计
- Rust 逆向需要从 unsafe 块开始跟, 不要纠结语法
- tera 模板 {%%} 块标签是绕表达式注入的常见手法
- glob:// 协议爆后门文件名是 glob 模糊匹配 + 字典枚举的组合
prerequisites:
- 6502 CPU 指令集 (LDX/LDA/STA/ADC/IZX)
- glibc heap (off-by-one / tcache / chunk overlap)
- PHP 反序列化魔术方法链
- Tera 模板引擎 / glob 协议
---
