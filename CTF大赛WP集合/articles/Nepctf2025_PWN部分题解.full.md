...
lesson: '|'
quality: medium
full_path: Nepctf2025_PWN部分题解.full.md
meta_path: Nepctf2025_PWN部分题解.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: 'Nepctf 2025 PWN 部分题解 (race condition + format string)。Nepctf 2025 PWN: 多线程 race condition 绕过 flag 字符串过滤 + printf(name) format string 泄栈地址。。经验：|'
category: pwn
...
contest_type: open
wp_url: https://www.ctfiot.com/272018.html
reasoning_chain:
- input_my_name → 触发点：printf(name) format string
- 假设：栈地址泄漏 → 动作：%p.%p.%p.%p → 观察：拿到栈地址
- input_filename 检测 strstr(file,'flag') → 触发点：flag 字符串过滤
- 假设：race condition 绕过 → 动作：多线程并发请求
- work 函数 md5_1_ + md5_2_ → 假设：file 写入触发 race window
- 动作：开 2 线程：线程 A 持续写入 '/time'，线程 B 读 flag → 观察：race 命中
- 动作：printf(name) 输出栈地址 + format string → 下一步：构造 leak payload
- 观察：成功读 flag → 完成
failed_attempts:
- 试图单线程绕过 strstr → 失败：检查同步无 race window
- 不 race 直接 read → 失败：flag 字符串拦截
- 只用 format string 不 race → 失败：file 写入不触发
key_observations:
- strstr 检测 + race condition 是 PWN 经典组合
- printf(name) format string 必须用 %p 泄栈
- 多线程并发是 CTF race 通用思路
- TOCTOU 漏洞在文件写入/读取间夹缝利用
- fork+execve('/bin/ls') 可作为目录枚举通道
prerequisites:
- PWN race condition 原理
- format string 泄漏栈地址
- 多线程同步原语
- TOCTOU 攻击模型
---
