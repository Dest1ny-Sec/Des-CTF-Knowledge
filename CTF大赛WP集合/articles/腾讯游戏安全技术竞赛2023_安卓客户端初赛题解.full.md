...
quality: high
full_path: 腾讯游戏安全技术竞赛2023_安卓客户端初赛题解.full.md
meta_path: 腾讯游戏安全技术竞赛2023_安卓客户端初赛题解.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: 腾讯游戏安全技术竞赛2023 安卓客户端初赛题解。腾讯游戏安全技术竞赛2023安卓客户端初赛题解,frida源码bypass+IL2CPPDumper恢复符号+patch金币自增+四层加密(XTEA+VM+混淆去除+BlackObfuscator dex)注册机。。关键路径：修改frida源码:re.frida_server字符串+port 27042→其他+so特征字符串 → 用修改版fr...
category: reverse
subcategory: reverse
tools_used:
- Frida
...
contest_type: open
wp_url: https://www.ctfiot.com/116213.html
reasoning_chain:
- 触发点：frida 默认被检测 → 假设：改 frida 源码过反调试
- 动作：改 server.vala 中 re.frida_server 字符串 + linux-host-session.vala 中 so 特征字符串 + 27042 端口
- 观察：游戏启动不被检测 → 下一步：frida hook dlopen
- 动作：frida hook dlopen 当 libil2cpp.so 加载后 dump 解密 so → 观察：拿到解密后的 libil2cpp.so
- 动作：IL2CPPDumper 恢复 IDA 符号 → 观察：看到 CollectCoin 等
- 假设：金币增加函数 → 动作：frida patch CollectCoin 改为每次 +1000 → 观察：金币暴涨
- 触发点：四层加密（XTEA + VM + 混淆 + BlackObfuscator）→ 假设：分别拆解
- 动作：frida hook g_sec2023_p_array 函数 → Unicon 模拟执行 + IDAPython 修改汇编 → 观察：去混淆
- 动作：第三处加密用 frida 提走加密结果比对 → 第四处 hook Sec2023.Encrypt 找到实现
failed_attempts:
- 默认 frida hook → 失败：被反调试检测
- JADX 反编译 dex → 失败：BlackObfuscator 混淆
- 静态 IDA 还原 libil2cpp.so → 失败：未 dump 时加密
key_observations:
- frida 源码修改过反调试：改 re.frida_server + so 特征字符串 + 端口
- IL2CPPDumper 恢复 libil2cpp.so 符号是 Unity 安卓逆向标配
- frida hook dlopen 在加载后 dump 是绕过加密壳的有效方法
- 四层加密（XTEA/VM/混淆/BlackObfuscator）需分别拆解：frida hook + Unicon 模拟 + frida 提结果
prerequisites:
- frida 源码编译
- Unity IL2CPP 逆向
- Unicorn 模拟执行
- BlackObfuscator 字符串还原
---
