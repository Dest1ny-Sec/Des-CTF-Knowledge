...
- DJ64 loader
- comcom64 stubless
- STFLG1_NO32PL|SHM_EXCL|SHM_NEW_NS
- VNC上傳hex+decoder
- /usr/bin/cat SUID
attack_chain:
...
quality: high
full_path: ACTF2026_WriteUp_By_F1ux.full.md
meta_path: ACTF2026_WriteUp_By_F1ux.meta.md
images_removed: true
images_removed_count: 4
schema_version: v3.0.0-P0
summary: ACTF 2026 WriteUp By F1ux。ACTF 2026 — 5 道 Crypto (inverse_pow + jail help('secret') + 低指 RSA 广播 + QuadraticField+decimal sqrt + TenSEAL CKKS 同态 rank) + Misc Farthest2026 DOSEMU2 DJ64 loader + SUID ...
category: crypto
subcategory: rsa
tools_used:
- C
...
contest_type: open
wp_url: https://www.ctfiot.com/307824.html
reasoning_chain:
- 'Crypto1 inverse_pow: pow(x,3)=2^384+y → 触发点：cube root 还原'
- 动作：C 写 128-bit 定点扫描 + Decimal 复核 → 观察：候选值精确还原
- 'Crypto2 jeval: eval() 沙箱缺 helper_builtins 禁用 → 假设：help() 可读模块源'
- 动作：help('secret') → 观察：pydoc DATA 区打印 secret.py 内 FLAG
- Crypto3 低指 RSA 广播：e=3 多模广播 → 触发点：中国剩余定理
- 动作：CRT 求 m^3 mod n1n2n3 → 观察：直接开立方根
- 'Crypto4 QuadraticField: K=Q(sqrt(-p)) + max_order OK + Oq 虚二次域 → 触发点：虚数域 DLP'
- 假设：输出 [A>>360, B, decimal_part_of_sqrt(p*q^2)] 可逆 → 动作：反推 45 字节 token
- Crypto5 TenSEAL CKKS：Base 128 元素加密 → 服务端按 sorted(Base).index 校验
- 假设：无私钥但有 galois 可同态 rank → 动作：构造加法+密文乘法+旋转 → round(answer[i])==rank 完成
- Misc Farthest2026 DOSEMU2：dos 不能读 /flag → 触发点：SUID + DJ64 loader
- 动作：R.ELF + DJ64 stub + int 0e6h + dj64init_once fork+execve /usr/bin/cat /flag 重定向 OUT.TXT
failed_attempts:
- 试图解 QuadraticField 整 DLP → 失败：max_order 长度爆炸走约化
- 试图纯 Python TenSEAL 解密 → 失败：无 secret_key 仅能同态计算
key_observations:
- pydoc help() 是 Python 沙箱逃逸经典非预期 (DATA section 打印模块源)
- TenSEAL CKKS 即使无私钥也能加法+密文乘密文+galois 旋转做排序
- DOSEMU2 DJ64 loader 提供 host exec 桥，SUID cat 是 host fs 读权限
- STFLG1_NO32PL|SHM_EXCL|SHM_NEW_NS 组合是关键 flags
- 虚二次域 DLP 走 max_order + reduced_form + isqrt decimal 拼接
prerequisites:
- Python 沙箱逃逸（builtins/pydoc/help）
- TenSEAL CKKS 同态 API（无 secret_key 用法）
- DOSEMU2 + DJ64 loader 内部结构
---
