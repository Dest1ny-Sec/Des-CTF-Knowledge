...
quality: high
full_path: 第三届黄河流域公安院校网络空间安全技能挑战赛官方Writeup.full.md
meta_path: 第三届黄河流域公安院校网络空间安全技能挑战赛官方Writeup.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: 第三届黄河流域公安院校网络空间安全技能挑战赛官方Writeup。第三届黄河流域公安院校挑战赛官方WP：.user.ini包含+CSP绕过XSS+JSP文件包含+路径穿越。经验：.user.ini可作为LFI目标；max_input_vars=1000触发Warning致header失败可绕过CS...
category: web
...
contest_type: open
wp_url: https://www.ctfiot.com/249193.html
reasoning_chain:
- 奶龙题提示 '.user.ini 未过滤' → 触发点：PHP 自动加载 .user.ini
- 假设：auto_prepend_file=/var/log/nginx/access.log → UA 头写马 → 触发点：nginx 日志含 PHP 代码
- 动作：UA 头写入 <?php system('id');?> → include 日志 → 执行命令 → 写 webshell
- 外国山海经题 CSP 限制 → 触发点：max_input_vars=1000 触发 Warning
- 假设：1000+ 参数触发 PHP Warning → header() 失败 → CSP 头被丢弃 → 假设：可注入 XSS
- 假设：构造 <svg onload=alert()> 绕过 CSP → 观察：admin bot 触发
- Message Board 题 welcome.jsp 注释泄露 isValidUser 逻辑 → 触发点：源码审计
- 假设：用户名密码 _ 连接存储到 /var/lib/jetty/webapps/root/username_password.txt → 注册 m1xian_a 密码空 → 覆盖 m1xian 密码
- 假设：/fileload 文件上传 + 路径穿越到 WEB-INF → 动作：../%u002e/WEB-INF/lib 上传 xml
- 假设：Jetty Configure XML 支持 BCEL 字节码加载 → 动作：构造 $$BCEL$$$l$8b$I... bcel 字节码
- 假设：jetty scan 线程会解析新 xml → 动作：观察 → 反弹 shell → Capabilities 提权 → SetUID.c/setuid(0) → 读 /fllag
failed_attempts:
- 直接 CSP 头绕不过 → 失败：必须 max_input_vars=1000 触发 PHP Warning
- mypdf invite=-3.3e99999999999999 直接绕过 admin → 失败：注册步骤需先 m1xian_a 替换
- fileupload 单纯上传 .xml → 失败：必须路径穿越到上级 webapps 目录
key_observations:
- .user.ini + auto_prepend_file 是 PHP LFI 经典入口，配合 nginx access.log UA 写马即可 RCE
- max_input_vars=1000 触发 PHP Warning 导致 header 失败可绕过 CSP
- Jetty Configure XML 支持 BCEL 字节码加载是 Java 反序列化另类入口
- Capabilities 提权（java setuid capability）需写 SetUID.c + SetUID.java + gcc 编译 .so
- 源码审计注释泄露逻辑是 CTF Web 重要突破口
prerequisites:
- PHP .user.ini + auto_prepend_file 原理
- CSP 头与 PHP Warning header() 失败的交互
- Jetty Configure XML + BCEL 字节码加载
- Linux Capabilities 提权（setuid capability）
---
