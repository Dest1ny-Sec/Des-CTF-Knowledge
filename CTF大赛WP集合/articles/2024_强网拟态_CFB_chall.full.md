...
quality: high
full_path: 2024_强网拟态_CFB_chall.full.md
meta_path: 2024_强网拟态_CFB_chall.meta.md
images_removed: true
images_removed_count: 7
schema_version: v3.0.0-P0
summary: 2024 强网拟态 CFB_chall。AES-CFB 8-bit 单字节爆破 + reset 重置 iv/key 凑运气。。经验：AES CFB 8-bit 模式每个密文字节是独立流密码块，改 1 字节只影响后续；revenge 用不同 error ...
category: crypto
...
contest_type: open
wp_url: https://www.ctfiot.com/211724.html
reasoning_chain:
- 触发点：注册用户 admim + a*15+11123456 → 假设：token 16 字节密文 → 动作：register 接口注册
- 观察：返回 token → 假设：AES-CFB 8-bit 加密 → 动作：分析加密代码
- 触发点：CFB 8-bit 模式 → 假设：每个密文字节独立流密码块 → 动作：改第 5 字节 m→n 让 n 后面解密出 \x00
- 观察：token 第 5 字节 m→n → 假设：第 6 字节爆破 256 次让后面 admin 隔离 → 动作：脚本循环
- 触发点：第 6 字节爆破 → 假设：第 7 字节爆破 256 次让第 23 字节变 \x00 拆出 123456 → 动作：split('\x00') 切
- 观察：split 出 admin 和 123456 → 假设：reset 重置 iv/key 继续尝试 → 动作：reset endpoint
- 触发点：总尝试 1/512 概率 → 假设：循环 reset 直到成功 → 动作：while True 反复爆破
- 观察：登录 admin/123456 成功 → 假设：拿到 flag → 动作：response flag
failed_attempts:
- 试图不解 CFB 8-bit → 失败：必须理解每字节独立流密码
- 试图不 reset 直接猜 → 失败：单次爆破概率低
- 试图用 username=admin 爆破 → 失败：必须分块隔离
- 试图不解密 token → 失败：必须伪造 token 撞 admin 凭证
key_observations:
- AES CFB 8-bit 模式每个密文字节是独立流密码块，改 1 字节只影响后续
- revenge 用不同 error 文案做 oracle 攻击
- reset 重置 iv/key 凑运气 1/512 概率是 CTF web 密码学经典 trick
- split('\x00') 切字符串多余段归 _ 是 Python 解密常见手法
- token = encrypt('{username}\x00{password}\x01\x02\x03', key) 是格式模板
prerequisites:
- AES CFB 8-bit 模式工作原理
- Python split 与 bytes 操作
- Flask 路由 + reset endpoint 触发
- 概率爆破 1/512 期望循环
---
# 2024 强网拟态 CFB_chall
