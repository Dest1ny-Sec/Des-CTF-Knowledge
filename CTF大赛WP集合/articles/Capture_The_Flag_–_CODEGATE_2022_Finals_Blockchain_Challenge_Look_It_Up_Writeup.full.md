...
quality: high
full_path: Capture_The_Flag_–_CODEGATE_2022_Finals_Blockchain_Challenge_Look_It_Up_Writeup.full.md
meta_path: Capture_The_Flag_–_CODEGATE_2022_Finals_Blockchain_Challenge_Look_It_Up_Writeup.meta.md
images_removed: true
images_removed_count: 0
schema_version: v3.0.0-P0
summary: Capture The Flag – CODEGATE 2022 Finals Blockchain Challenge Look It Up Writeup。CODEGATE 2022 Finals区块链题 Look It Up，3层proof验证。关键路径：nc启动私有链+uuid+RPC+private key → Setup合约challenge=new Challenge() → ...
category: web
...
contest_type: open
wp_url: https://www.ctfiot.com/95239.html
reasoning_chain:
- nc 3.34.81.192 31337 输入 ticket → 返回 uuid+RPC+private key+setup 合约 → 触发点：Paradigm CTF 私有链部署框架
- challenge 合约含 flag 但 query() 要求 msg.value >= 10000 ether → 假设：必须绕过金额或伪造 solved 状态
- 读 Setup 合约 isSolved() 调用 challenge.solved() → 假设：必须让 solved==true，路径是 declareSolved()
- declareSolved() 要求 solved1/2/3 全 true → 每个 solved 由 challenge1/2/3 触发 → 假设：必须构造合法 proof
- challenge 流程 = sanity_check(isPowerOf2(n+1)+长度边界) + final_check(LHS==RHS + f与t无交集) → 假设：n=1 最小化输入
- final_check 是 LHS=RHS 模 p 等式 + 防 f 与 t 重叠 → 假设：取 beta=gamma=0 简化等式 → 动作：Sage 解 n=1 多项式
failed_attempts:
- 试图直接 call query() 拿 flag → 失败：msg.value < 10000 ether 触发 require 回滚
- 试图改 setup 合约的 storage → 失败：私有链每次启动 challenge 重新部署，无法保留修改
key_observations:
- Paradigm CTF 框架是部署 Paradigm CTF 系列区块链题的标准化基础设施（uuid+RPC+private key）
- declareSolved() 必须三个子条件全为 true，单独 bypass 任一子条件无效
- n=1 时 LHS/RHS 是 (1+beta)^2 * ((gamma*(1+beta)+beta*t1+t0)*(gamma+f)) 等简单模等式
- isPowerOf2 校验 n+1 是 2 的幂 → 最小 n=1 满足（n+1=2）
prerequisites:
- Solidity 0.8 基础语法（合约、require、view 函数）
- Paradigm CTF 部署框架使用（foundry+nc+uuid）
- 有限域模运算（mulmod / 加减模运算）
- SageMath 多项式求解或 Z3 约束求解
---
