...
- IDA
attack_chain:
- file 命令识别 ELF 64-bit LSB 静态链接 stripped
- '字符串定位: 0x47c093 ''wrong!'' 0x47c09a ''right!'' 0x47c0a1 ''password:'
- 查 signal/sigaction 函数引用 → 注册信号处理器
- signal handler 里藏 flag 校验逻辑（保护主函数不让单步）
- '反反调试: 用 ptrace 接管 / 改 signal handler 跳过'
- 静态链接 + stripped → 手工定位关键函数（搜索 0x400a70 等候选）
- OLLVM 混淆（控制流平坦化 + 虚假控制流）→ 用 D-810 反混淆插件
- '还原加密函数: 输入 password → state machine → 比较 right! 决定输出'
...
quality: high
full_path: SigMachine_CTF逆向题深度解析-从零到一的完整解题之旅.full.md
meta_path: SigMachine_CTF逆向题深度解析-从零到一的完整解题之旅.meta.md
images_removed: true
images_removed_count: 0
schema_version: v3.0.0-P0
summary: 'SigMachine CTF 逆向题深度解析。ELF+signal handler+OLLVM 三合一逆向入门。关键路径：file 命令识别 ELF 64-bit LSB 静态链接 stripped → 字符串定位: 0x47c093 ''wrong!'' 0x47c09a ''right!'' 0x47c0a1 ''password: → 查 signal/sigaction 函数引用 → 注册信号...'
category: reverse
subcategory: reverse
subcategories:
- reverse
...
contest_type: open
wp_url: https://www.ctfiot.com/283400.html
reasoning_chain:
- file sigmachine → ELF 64-bit statically linked stripped → 触发点：无符号逆向
- strings 定位 0x47c093 'wrong!' 0x47c09a 'right!' 0x47c0a1 'password:'
- 查 sigaction/SIGUSR1/SIGSEGV 引用 → 假设：signal handler 藏 flag 校验
- 假设：signal handler 反调试 → 动作：ptrace 接管或绕过
- static stripped → 手工定位关键函数 (搜 0x400a70 等候选地址)
- OLLVM 控制流平坦化 + 虚假控制流 → 假设：需 D-810 IDA 插件反混淆
- D-810 还原 state machine → 假设：state machine 即 password 加密逻辑
- 输入 password → state machine 校验 → 比较 right!/wrong! → flag
failed_attempts:
- 试图用 GDB 单步调试 → 失败：signal handler 反调试改变执行流
- 试图静态读 main 函数 → 失败：OLLVM flatten 后 4000+ 行 switch 不可读
key_observations:
- Linux signal handler 是反调试常用手段 (SIGUSR1/SIGSEGV/SIGTRAP)
- OLLVM 控制流平坦化 + 虚假控制流需 D-810 或 OLLVM-Unflattener 反混淆
- statically linked stripped ELF 需手工搜候选函数地址 (0x400a70 等)
- 字符串定位 'right!'/'wrong!'/'password:' 是逆向 flag 校验的快速入口
prerequisites:
- ELF 文件结构与 file 命令识别
- Linux signal/sigaction 机制
- IDA Pro + D-810 OLLVM 反混淆插件
- GDB / pwndbg 调试
---
