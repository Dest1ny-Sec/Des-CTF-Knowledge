...
quality: high
full_path: 2023_安洵杯SYCTF_writeup_by_Arr3stY0u.full.md
meta_path: 2023_安洵杯SYCTF_writeup_by_Arr3stY0u.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: 2023 安洵杯 SYCTF WriteUp by Arr3stY0u。综合 9 题：数独贪吃蛇符号执行 FSOP RSA-Coppersmith 全面覆盖。。关键路径：Sudoku 数独游戏用 pwntools 连远程 + 回溯算法 5 秒内解 9x9 → telnetlib 收数独 + 算解 + WASD 模拟键盘操作提交 → librosa 音频 2*audio1 - audio2 =...
category: misc
...
contest_type: open
wp_url: https://www.ctfiot.com/120089.html
wp_author: Arr3stY0u
reasoning_chain:
- '数独题: 9x9 数独回溯求解 → 触发点：sudoku 网格 + 10000ms 时限'
- 假设：Python 回溯算法 solve_sudoku → 动作：pwntools remote → recv 数独 → solve → send
- 观察：拿到 flag
- '贪吃蛇 RC4: 移动序列加密 → 假设：srand + ARC4 解密'
- 动作：解 RC4 还原 WASD 序列 → 观察：flag
- 'FSOP 符号执行: FSOP (File Stream Oriented Programming) → 假设：libc 2.35 House of Apple 2'
- 动作：构造 fake IO_FILE → 触发 _IO_wfile_overflow → ROP
- '观察：RSA-Coppersmith: p 已知部分高位 → small_roots 恢复 → flag'
failed_attempts:
- 试图用 z3 解数独 → 失败：时延过高
- 试图手工解贪吃蛇 → 失败：路径太长
- 试图 House of Banana → 失败：环境是 libc 2.35
key_observations:
- 数独用回溯算法是基础
- 贪吃蛇 RC4 用 srand + ARC4 解密
- claripy BVS 做符号执行是逆向通用手法
- libc 2.35 House of Apple 2 是 IO 攻击标配
prerequisites:
- 数独回溯算法
- Python pwntools / telnetlib 远程交互
- libc 2.35 House of Apple 2 (FSOP)
- RSA Coppersmith + Sage
---
