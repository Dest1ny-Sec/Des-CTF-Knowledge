...
- v6 溢出覆盖返回地址
- seccomp 沙箱手写 ORW
- shellcraft.open+read+write pwntools
- mov rdi
- 0; mov rsi
- rbp; pop rax; pop rcx; pop rdx; syscall
- heap_base 0x910-0x9a0 写 shellcode
attack_chain:
- 50 个堆块 add 后越界覆盖 v6
- v6 是返回地址 → 覆盖为堆地址
...
quality: high
full_path: 2024鹏程杯_writeup_by_Mini-Venom.full.md
meta_path: 2024鹏程杯_writeup_by_Mini-Venom.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: 2024 鹏程杯 writeup by Mini-Venom（cool_book 沙箱 ORW）。cool_book 50 块后 v6 越界覆盖返回地址到堆地址 + 堆上写 ORW shellcode，seccomp 禁 execve → 走手写 open/read/write。。关键路径：50 个堆块 add 后越界覆盖 v6 → v6 是返回地址 → 覆盖为堆地址 → 堆地址处写 she...
category: pwn
subcategory: rop
tools_used:
- ROP
...
contest_type: open
wp_url: https://www.ctfiot.com/214840.html
wp_author: Mini-Venom
reasoning_chain:
- '[触发点] cool_book 菜单 pwn + add 50 次后 v6 越界 → 假设：off-by-N 覆盖返回地址到堆地址 → [动作] for i in range(46): add(i, b''\x90''*0x1) → [观察] addr=heap_base-0x100 → [下一步] 堆上写 ORW shellcode'
- '[触发点] seccomp 禁 execve → 假设：必须 ORW（open/read/write）三连 → [动作] shellcraft.open(''/flag'')+shellcraft.read(3,heap_base-0x100,0x30)+shellcraft.write(1,heap_base-0x100,0x30) → [观察] 直接拼 shellcode'
- '[触发点] exit(3) 触发栈溢出到返回地址（堆地址）→ 假设：执行堆上 ORW shellcode → [动作] sleep(2); s(b''\x90''*0x40+asm(sc)) → [观察] 拿到 flag'
- '[触发点] LOOKUP web 题 JacksonToSignedObject + 二次反序列化 + 内存马 → 假设：没 ban SignedObject → [动作] BadAttributeValueExpException + POJONode(templates) + SignedObject 包一层 → [观察] Spring Boot Controller 内存马注入成功'
- '[触发点] Jackson 二次反序列化用 javassist 移除 writeReplace → 假设：TemplatesImpl load bytecode → [动作] createTemplatesImpl(SpringBootMemoryShellOfController.class) → [观察] 类加载触发静态块 → [下一步] Controller 注入'
failed_attempts:
- 试图直接 execve shellcode → 失败：seccomp 禁用
- 试图找现成 libc one_gadget → 失败：版本不匹配
- 试图用 ysoserial 工具 → 失败：必须二次反序列化触发链
key_observations:
- 开了 seccomp 的 pwn 题，syscall 只留 read/write/open 等几条 → 必须在堆上写 ORW shellcode，pwntools `shellcraft.open/read/write` 是最简方案
- add 50 次后 v6 越界是经典菜单题 off-by-N 套路
- JacksonToSignedObject 二次反序列化 + javassist 移除 writeReplace + Spring Boot 内存马
- pwntools shellcraft.open/read/write 拼 ORW 比手写汇编方便
prerequisites:
- seccomp 沙箱限制 + ORW shellcode
- pwntools shellcraft 模块
- Jackson 反序列化 gadget 链
- Spring Boot Controller 内存马
---
