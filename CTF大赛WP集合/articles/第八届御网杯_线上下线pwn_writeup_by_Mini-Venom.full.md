...
quality: high
full_path: 第八届御网杯_线上下线pwn_writeup_by_Mini-Venom.full.md
meta_path: 第八届御网杯_线上下线pwn_writeup_by_Mini-Venom.meta.md
images_removed: true
images_removed_count: 2
schema_version: v3.0.0-P0
summary: '第八届御网杯 线上下线 pwn Writeup by Mini-Venom。御网杯 3 题 PWN：SROP rax=0x15 sigreturn 调 execve + RWX 栈 fmt 泄 + aarch64 fmt 改 X30 寄存器。。关键路径：asm (SROP): 控制 rax=0x15 触发 sigreturn → frame.rax=constants.SYS_execve,...'
category: pwn
...
contest_type: open
wp_url: https://www.ctfiot.com/213477.html
wp_author: Mini-Venom
reasoning_chain:
- 触发点：asm 题无 system 函数只有 /bin/sh 字符串 → 假设：需要 syscall 调 execve
- 观察：题目给出 ret2syscall gadget 但不能直接拼 → 假设：需 sigreturn 触发 rax=0xf
- 动作：写 SigreturnFrame 设 rax=constants.SYS_execve rdi=0x40200A rsi=0 rdx=0 rip=syscall → 观察：调用 syscall 触发 execve(/bin/sh)
- ret 题：触发点栈上有 rwx → 假设：fmt 泄栈地址再写 shellcode
- 动作：printf %p 泄 rsp 偏移 → 观察：得到可写栈地址
- 动作：栈溢出覆盖 ret 到 shellcode → 观察：执行 /bin/sh
- aarch64 fmt 改 X30：触发点 fmt %n 写指针 → 假设：fmt 控制 pc 跳板
- 动作：fmt 多参数 %c %hn 写 X30 到 gadget → 观察：劫持控制流到 execve
failed_attempts:
- 试图找 system@plt → 失败：题目刻意删除 system
- ret 题试图 brute force 栈地址 → 失败：栈 ASLR 不可爆破
- aarch64 直接覆盖栈上 LR → 失败：栈偏移太大难精确定位
key_observations:
- SROP 是无 system 时的 execve 标配：SigreturnFrame 设寄存器 + ret 到 syscall
- 栈 rwx 段才能写 shellcode，否则只能 ROP
- aarch64 fmt 字符串 %n 写指针可控制 X30（LR）实现 ROP
- fmt %hn 写两字节是构造大地址的常用组合
prerequisites:
- SROP 原理（sigreturn + sigcontext frame 构造）
- x86_64 syscall 编号（execve=0x3b, sigreturn=0xf）
- aarch64 寄存器约定（X0-X30）
- 格式化字符串 %n %hn %hhn 写任意地址
---
