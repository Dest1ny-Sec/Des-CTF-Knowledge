...
- SROP
- mprotect栈执行
- mybash_structural_confusion
- ret2dlresolve
- RSA
- RSA_repeated_squaring
- Polaris战队
attack_chain:
...
quality: high
full_path: 2023上海磐石行动-WP_-Polaris战队.full.md
meta_path: 2023上海磐石行动-WP_-Polaris战队.meta.md
images_removed: true
images_removed_count: 6
schema_version: v3.0.0-P0
summary: '2023 上海磐石行动 WP - Polaris 战队。磐石行动 4 道 pwn：keybox 负数绕 + ssql 0x291+one_gadget + Hp SROP + mybash ucontext_t。。关键路径：keybox: 负数数组下标绕过 s[v5]=v4 (v5=-9223372036854775796) → edit 负索引 (-0x398) 覆盖 v9 指针，调用 e...'
category: misc
...
contest_type: open
wp_url: https://www.ctfiot.com/119177.html
wp_author: one
reasoning_chain:
- 触发点：keybox 主函数 s[v5]=v4 但 v5 未校验 → 假设数组负下标绕过
- 动作：first key = -9223372036854775796 →观察：s[12]=1 进入菜单
- 动作：edit负索引 -0x398 覆盖 v9 指针 → 观察：v9 指向 end 函数 → 调用 exit 时执行 end
- actor add(0x18)+edit(-0x398,8,p64(0x401765)[:7]) → 假设栈 ROP → 动作：构造 ROP 链
- actor ssql column off-by-one + 0x291 fake size → 假设链表 hijack → 动作：tcache 改 fake
- 动作：libc-2.35 _IO_list_all + 0x52290 one_gadget → 观察：拿到 shell
- actor Hp HTTP 自定义解析器 + off-by-one + 8 字节部分写 → 假设 fake IO_FILE → SROP
- actor mybash realloc 调整 chunk_size + strncpy 类型混淆 → 假设 ucontext_t SROP 改 rip/rsp
failed_attempts:
- 试图直接 ROP keybox → 失败：必须绕过 v5 数组下标校验
- 试图常规 fastbin attack ssql → 失败：需 0x291 fake size 走 libc-2.35
key_observations:
- 负数数组下标可绕过 s[v5]=v4 边界，是 int 溢出经典
- off-by-one + 0x291 fake size 是菜单题经典结构
- ucontext_t SROP 在 realloc 场景里可控制 rip/rsp
- SROP pop rax=10 + mprotect(rdi=stack) 可写 shellcode
prerequisites:
- C 语言 int 溢出与负数组下标
- SROP（Sigreturn-Oriented Programming）原理
- glibc heap off-by-one 利用链
- ucontext_t 结构与栈布局
---
