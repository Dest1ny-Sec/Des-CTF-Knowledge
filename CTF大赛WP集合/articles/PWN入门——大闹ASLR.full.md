...
quality: high
full_path: PWN入门——大闹ASLR.full.md
meta_path: PWN入门——大闹ASLR.meta.md
images_removed: true
images_removed_count: 8
schema_version: v3.0.0-P0
summary: PWN入门 - 大闹 ASLR。从 Linux VFS/proc 虚文件系统、sysctl 接口逐层下钻到 load_elf_binary 内核 ELF 加载器，详解 ASLR 0/1/2 三档随机化原理。。关键路径：三档 ASLR 控制：0 全关 / 1 部分 / 2 全开 (堆栈 mmap libc) → echo xxx | sudo tee -a /proc/sys/kernel/r...
category: pwn
...
contest_type: open
wp_url: https://www.ctfiot.com/194007.html
wp_author: userns
reasoning_chain:
- 触发点:栈地址每次运行都不同(7ffd1b667000 vs 7ffd7024c000) → 假设:ASLR 开了
- ASLR 三档:0 全关 / 1 部分(堆栈 mmap libc) / 2 全开 → 假设:由 /proc/sys/kernel/randomize_va_space 控制
- 动作:echo xxx | sudo tee -a /proc/sys/kernel/randomize_va_space → 观察:随机化开关可被改写
- proc 虚文件系统通过 file_system_type + super_block 注册 → 假设:proc 是 VFS 实例
- sysctl kern_table.procname='randomize_va_space' + proc_dointvec 处理写 → 假设:sysctl 与 proc 联动
- load_elf_binary:personality & ADDR_NO_RANDOMIZE + PF_RANDOMIZE → 假设:加载期一次性随机化
- arch_mmap_rnd:get_random_long() & ((1<<rndbits)-1) << PAGE_SHIFT → 观察:mmap 32 位随机 + 12 位页对齐
- 绕过思路:稳定泄露一个地址 = 减偏移得 libc 基址 → 全地址可控
failed_attempts:
- 试图运行期改 ASLR → 失败:randomize_va_space 仅加载期生效
- 试图只泄栈地址 → 失败:ASLR 同步随机化栈/堆/libc,需先算页对齐
key_observations:
- ASLR 是加载期一次性行为,运行期不可改变
- mmap 32 bit 随机 + 12 bit 页对齐,栈 22 bit 随机
- 只要能稳定泄露一个地址 = 全部可控(因为相对偏移固定)
prerequisites:
- Linux 内核 VFS/proc/sysctl 框架
- load_elf_binary ELF 加载流程
- ASLR 熵计算与页对齐机制
---
# PWN入门——大闹ASLR
