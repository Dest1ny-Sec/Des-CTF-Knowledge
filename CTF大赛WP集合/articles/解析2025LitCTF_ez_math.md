...
```
# 正确性验证（可选）defmat_mul(M, N, mod): return[ [(M[0][0]*N[0][0] + M[0][1]*N[1][0]) % mod, (M[0][0]*N[0][1] + M[0][1]*N[1][1]) % mod], [(M[1][0]*N[0][0] + M[1][1]*N[1][0]) % mod, (M[1][0]*N[0][1] + M[1][1]*N[1][1]) % mod], ]defmat_pow(M, e, mod): R = [[1,0],[0,1]] A = [[M[0][0], M[0][1]],[M[1][0], M[1][1]]] whilee >0: ife &1: R = mat_mul(R, A, mod) A = mat_mul(A, A, mod) e >>=1 returnR# 验证 A^e = BA = [[A11, A12],[A21, A22]]B_recomputed = mat_pow(A, e, p)assertB_recomputed[0][0] == B11andB_recomputed[0][1] == B12assertB_recomputed[1][0] == B21andB_recomputed[1][1] == B22
```
