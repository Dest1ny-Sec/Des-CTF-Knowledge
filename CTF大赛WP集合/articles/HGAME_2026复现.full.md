...
quality: high
full_path: HGAME_2026复现.full.md
meta_path: HGAME_2026复现.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: 'HGAME 2026复现。HGAME 2026复现：short整数边界绕过+canary泄露+shellcode。关键路径：week1-adrift: 主循环菜单0=way/1=delete/2=show/3=change/4=exit → v6[0] 是 short int 16位 → -32768 → 0x8000 取反+1 = 0xFFFB → 等价+5。经验：short int -3...'
category: pwn
...
contest_type: open
wp_url: https://www.ctfiot.com/301877.html
reasoning_chain:
- week1-adrift 主菜单 0=way/1=delete/2=show/3=change/4=exit → 触发点：v6[0] 是 short int 16 位
- 假设：-32768 短整数边界异常 → 动作：0x8000 取反+1 = 0xFFFB = +5
- 观察：case2 show(-32768) 实际打印 +5 索引 → 假设：能泄漏 canary
- case3 修改 canary 绕过 case4 检查 → 假设：canary 比对可过
- case0 0x410 字节栈溢出 → 触发点：写入 shellcode 空间充足
- 动作：0x3e8+2 字节 padding + shellcode + return_addr
- 'shellcode: pop r11; mov rax ''/bin/sh''; push rax; push rsp; pop rdi; xor eax,59; xor rdx,rdx; syscall'
- 观察：execve('/bin/sh') 触发 → 完成
- week2 diary keeper → 触发点：glibc 2.35 patchelf + PROTECT_PTR 指针保护
- off by null 合并 ABC → 假设：触发合并后读写 B 内容
failed_attempts:
- 试图用 -1 越界 read → 失败：实际是 -32768 = 0x8000
- 直接改 canary 不看边界检查 → 失败：case4 canary 比对在退出时
- 不 patchelf 直接堆攻击 → 失败：libc 版本不匹配
key_observations:
- short int -32768 = 0xFFFB = +5 是 16 位补码关键
- canary 泄漏+修改绕过是 stack overflow 通用手法
- 0x410 字节栈溢出足够装 shellcode+ROP
- shellcode execve 通用模板（pop r11/mov rax/syscall）
- PROTECT_PTR 保护可用低 12 位异或绕过
prerequisites:
- C 语言 short int 边界理解
- canary 泄漏与栈溢出组合
- shellcode 编写基础
- patchelf + glibc 2.35 堆管理
---
# HGAME 2026复现
