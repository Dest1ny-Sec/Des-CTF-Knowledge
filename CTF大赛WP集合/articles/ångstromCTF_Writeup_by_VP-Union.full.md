...
quality: high
full_path: ångstromCTF_Writeup_by_VP-Union.full.md
meta_path: ångstromCTF_Writeup_by_VP-Union.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: ångstromCTF Writeup by VP-Union。VP-Union ångstromCTF 2023 第二份 WP，crypto 比重更大，含 RSA 二方程解、homomorphic oracle、伪 set 交为空 trick。。经验：RSA 二方程 (p-2)(q-1) + (p-1)(q-2) + n 可解 p q；homomorphic RSA 选...
category: crypto
subcategory: rsa
tools_used:
- Z3
...
contest_type: open
wp_url: https://www.ctfiot.com/122441.html
wp_author: VP-Union
reasoning_chain:
- 触发点：7 道题 tag 含 'caesar-shift' / 'RSA-Leak-pmqm' / 'RSA-homomorphic' / 'fake_psi' / 'one_encoding' / 'jinja2-SSTI' / 'BEAM-bytecode'，覆盖 crypto+web+reverse → 动作：按 tag 分组逐题突破
- puzzle 题：前 3 字符来自 5×caesar / 后 3 字符交叉 → 触发点：客户端可控 → 假设：response 显示'前 3 字母 of ... 与 后 3 字母 of ...' → 动作：循环 256 次取字符组合
- 观察：可直接 read 3+3 字符拼接 6 字母子串 → 下一步：累积 5 段拼出 flag 头
- rsa-leak-pmqm：给 (p-2)(q-1) 和 (p-1)(q-2) 两个方程 + n → 触发点：经典双方程解 p/q → 假设：z3/Sage 联立可解
- 动作：构建 SymPy 方程组 {pq=n, (p-2)(q-1)=a, (p-1)(q-2)=b} → 观察：p/q 解唯一 → 算 phi 还原 RSA
- homomorphic RSA：oracle 解密 Y=c·X^e mod n → 假设：选 X prime 让 gcd(X,n)=1 → 动作：循环 X 试探 → 拿到 m·X mod n
- 下一步：枚举 i 找 n*i + m·X 中包含 actf 的密文段 → 动作：分解得到 m
- fake_psi + one_encoding/zero_encoding：one_encoding(x)=x 高 65 位 / zero_encoding(y)=y 低 64 位 → 触发点：交为空需要 x 高位 + y 低位完全互斥
- 动作：x=1<<64+1（最高位 =1）、y=1<<64（最高位=0）→ 观察：one ∩ zero = ∅ → z3 验证
failed_attempts:
- 试图用普通 RSA d=e^-1 mod phi 解 homomorphic 题 → 失败：oracle 只解 m*X 而非 m，需配合 X 恢复
- 试图对 fake_psi 直接加集合运算 → 失败：集合交非空，必须用 one_encoding/zero_encoding 互斥编码
key_observations:
- RSA 双方程 (p-2)(q-1)+(p-1)(q-2)+n 是 CTF 经典出题套路，用 SymPy/Sage 联立而非暴力因式分解
- RSA 同态 oracle 攻击：选 X 与 n 互素，Y=c·X^e 经 oracle 解得 m·X，再除回 m
- fake set 互斥编码 = one_encoding(x)∩zero_encoding(y)=∅，本质是位错位分配
- Puzzle 题客户端可控响应是拼合 flag 子串的最快通道
prerequisites:
- SymPy/Sage 符号代数（解方程组 / 求 phi）
- RSA 基础数论（phi / 同态性质 / 欧拉定理）
- pwntools 交互式脚本（recv / sendline / recvuntil）
- 格式化字符串漏洞利用（%25$p / %hhn 任意地址写）
---
