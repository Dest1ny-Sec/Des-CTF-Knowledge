...
quality: high
full_path: 操纵预言机(ApplePool_WP).full.md
meta_path: 操纵预言机(ApplePool_WP).meta.md
images_removed: true
images_removed_count: 3
schema_version: v3.0.0-P0
summary: 操纵预言机(ApplePool WP) - DeFi UniswapV2预言机价格操纵。操纵预言机(ApplePool):DeFi UniswapV2预言机价格操纵套利token3耗尽isSolved。。经验：DeFi预言机操纵经典攻击:UniswapV2的getReserves()或balanceOf()直接作为价格源易被大量...
category: web
...
contest_type: open
wp_url: https://www.ctfiot.com/107182.html
reasoning_chain:
- 题目是 DeFi ApplePool → 触发点：合约 check 部署 4 个 AppleToken + UniswapV2Factory + 2 个 pair + AppleRewardPool
- 假设：AppleRewardPool 用 UniswapV2 预言机拿价格 → 触发点：rate() = amount0 * 1e18 / amount1 读 pair1 储备
- 假设：大量 swap token1 能操纵 pair1 储备 → 动作：UniswapV2 swap 大量 token1 兑换 token0
- 观察：pair1 reserves 改变 → rate() 输出被人为抬高 → 假设：claim/deposit/withdraw 用错误价格套利
- 动作：用操纵后预言机低价 claim → 高价 deposit → withdraw 套利 token3
- 观察：token3 被抽空 → 假设：isSolved() = token3.balanceOf(check) == 0
- 假设：用 safemath 防整数溢出 → 动作：payload 不溢出边界 → 观察：解出
failed_attempts:
- 试图直接转 token3 → 失败：token3 仅 AppleRewardPool 持有
- 试图不操纵预言机直接 deposit → 失败：rate() 返回真实价格无法套利
- 试图用 addPool 直接灌满 → 失败：Ownable 限定 owner = check 合约
key_observations:
- DeFi 预言机操纵经典攻击：UniswapV2 的 getReserves() 或 balanceOf() 直接作为价格源易被大量 swap 操纵
- AppleRewardPool 用 token3 作为奖励，token3 耗尽 isSolved
- use safemath 防整数溢出是 Solidity 0.5.x 标配
- 通过 addPool/deposit/withdraw 逻辑利用预言机错误价格套利
- Ownable 限定 owner = check 合约，普通用户无法直接 setApplePertime
prerequisites:
- Solidity 0.5.x 智能合约基础
- UniswapV2 getReserves / balanceOf 预言机原理
- DeFi 价格操纵攻击模型
