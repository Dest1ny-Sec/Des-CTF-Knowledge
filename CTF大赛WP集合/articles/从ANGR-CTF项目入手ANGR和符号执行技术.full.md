...
quality: high
full_path: 从ANGR-CTF项目入手ANGR和符号执行技术.full.md
meta_path: 从ANGR-CTF项目入手ANGR和符号执行技术.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: 从ANGR-CTF项目入手ANGR和符号执行技术。angr-CTF 15 关系统训练，从 find/avoid 路径搜索到 SimProcedure 替换库函数避爆。。关键路径：加载 angr Project 设定 base_addr 与 auto_load_libs=False → factory.entry_state 创建符号化初始状态 → claripy.BVS 创建符号向量绑定寄存...
category: misc
...
contest_type: open
wp_url: https://www.ctfiot.com/285534.html
reasoning_chain:
- angr-CTF 15 关训练 → 触发点：syssec labs angr 系列入门
- '00_angr_find: Project(''file'',auto_load_libs=False) + factory.entry_state() → simgr.explore(find=target)'
- 假设：find 接受地址/状态函数 → 动作：so_state.posix.dumps(0) 提输入
- '01_angr_avoid: explore(find=target, avoid=un_target) 排除陷阱地址'
- '02_angr_find_condition: 改用 is_succ/is_fail 状态函数 → b''Good Job.'' in posix.dumps(1)'
- '03_angr_symbolic_registers: claripy.BVS(''passwd0'',32) → init_state.regs.eax = passwd0'
- '04-05_angr_symbolic_stack/memory: stack_push(BVS) + memory.store(addr, BVS)'
- '06_angr_symbolic_dynamic_memory: fake_chunk0=0x12340 + BVS 模拟 malloc 指针'
- '07_angr_symbolic_scanf: blank_state + symbolic 模式读 scanf'
- '08-09_angr_constraints: 约束限制 BVS 取值范围（start/end charset）'
- '10_angr_hook: project.hook(addr, hook_func) 替换目标函数直接返回'
- '11-12_angr_simprocedure: SimProcedure 类 + project.hook_symbol 替换 scanf/check_equals'
failed_attempts:
- 试图直接走默认 explore 不设 find → 失败：路径爆炸
- 试图不用 hook 替换 scanf → 失败：路径数爆炸
- 试图手动合并 path group → 失败：veritesting 已自动做
key_observations:
- SimProcedure 替代 check_equals + veritesting 是处理 2^16 路径爆炸的标准姿势
- hook_symbol 比 hook(addr) 健壮，符号化 stdin 写入 SimFile 即可虚拟文件读取
- auto_load_libs=False 提速明显，否则 load libc 拖延 10x
- find/avoid 函数比地址更灵活，可结合 stdout 内容判断
prerequisites:
- angr 符号执行框架基础（Project/factory/simgr）
- claripy 符号表达式 API（BVS/BV）
- Z3 求解器概念
- 二进制 reverse 基础
---
