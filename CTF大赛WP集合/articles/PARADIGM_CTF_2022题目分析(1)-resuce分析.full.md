...
lesson: '|'
quality: high
full_path: PARADIGM_CTF_2022题目分析(1)-resuce分析.full.md
meta_path: PARADIGM_CTF_2022题目分析(1)-resuce分析.meta.md
images_removed: true
images_removed_count: 8
schema_version: v3.0.0-P0
summary: 'PARADIGM CTF 2022 题目分析 rescue (Uniswap V2 流动性)。PARADIGM CTF 2022 rescue: MasterChefHelper.sol + Uniswap V2 addLiquidity 按比例添加, 凑足 DAI 比例转走 10 WETH。。经验：|'
category: web
...
contest_type: open
wp_url: https://www.ctfiot.com/80208.html
reasoning_chain:
- 触发点:setup 合约创建时把 10 WETH 转入 mcHelper,isSolved 检查 mcHelper WETH 余额 → 假设:必须把 10 WETH 抽走
- MasterChefHelper.swapTokenForPoolToken 把一半 tokenIn 换成 pair.token0,另一半换 pair.token1,按比例 addLiquidity → 假设:必须先凑齐 DAI/WETH 比例
- 动作:poolId=2 对应 DAI/WETH pair,账户先 swap 20 ETH → 20 ETH 等值 DAI + 调用费 USDC → 观察:账户持有 USDC
- 动作:把 20 ETH 等值 DAI 转给 mcHelper,凑足合约内 DAI 与 10 WETH 等比例 → 观察:此时合约 DAI/WETH 满足 Uniswap pair 比例
- 调用 swapTokenForPoolToken(poolId=2, tokenIn=USDC, amountIn=20 ETH等值, minAmountOut=0) → 假设:合约把 10 WETH + 等比 DAI 一起 addLiquidity
- 动作:返回值作为 LP 留在攻击合约,合约 WETH 余额清零 → 观察:isSolved 返回 true
failed_attempts:
- 试图不转入 DAI 直接调 swapTokenForPoolToken → 失败:Uniswap 按比例 addLiquidity,WETH 仍留在合约
- 试图只转入 5 ETH 等值 DAI → 失败:比例不足,WETH 无法按对应份额配对
key_observations:
- Uniswap V2 addLiquidity 按当前储备比例计算份额,只要合约内两种代币比例恰好等于 pair 比例,即可一次性把全部余额入池
- DeFi 流动性题的本质是凑比例,而不是凑数量
- Poc 关键是先 swap 出中间代币再用中间代币换 pair 中第二种代币
prerequisites:
- Uniswap V2 Router / Pair 合约接口(addLiquidity/swapExactTokensForTokens)
- Solidity 攻击合约编写与部署
- DeFi 流动性池定价公式(x*y=k)
---
