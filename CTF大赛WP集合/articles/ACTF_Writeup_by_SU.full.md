...
tags:
- Shellshock BASH_FUNC_env
- cgi-bin/hello
- WebSocket prototype pollution
- JSON.stringify劫持
- hook onanimationstart
- XSS webhook exfil
- TLS-poison client-hello
- vsFTPd 3.0.3蜜罐
- MySQL LOAD DATA LOCAL
- file:// 协议
- JSON.parse hook exfil
attack_chain:
...
quality: high
full_path: ACTF_Writeup_by_SU.full.md
meta_path: ACTF_Writeup_by_SU.meta.md
images_removed: true
images_removed_count: 9
schema_version: v3.0.0-P0
summary: 'ACTF Writeup by SU。ACTF by SU — Shellshock BASH_FUNC_env + WebSocket prototype pollution XSS + JSON.stringify/parse hook exfil + TLS-poison + vsFTPd 蜜罐 + MySQL LOAD DATA LOCAL。。关键路径：Shellshock: BAS...'
category: web
...
contest_type: open
wp_url: https://www.ctfiot.com/46685.html
wp_author: SU
reasoning_chain:
- 'Shellshock: BASH_FUNC_env%%=(None, ''() { cat /flag; exit; }'') → 触发点：BASH 环境变量函数注入'
- 假设：cgi-bin/hello 用 bash 解析 → 动作：requests.post files=payload
- 观察：r.text 内有 /flag 内容 → XSS 部分
- WebSocket XSS：sendmsg type='image' + onanimationstart=eval(atob(...)) + animation='1s App-logo-spin'
- '假设：animation: keyframe 可触发 onanimationstart → 动作：注入 hook 代码'
- JSON.stringify hook：备份原函数 + 调用时改 {api:'getflag'} → 假设：前端会请求这个 API
- JSON.parse hook：备份原函数 + send(str) → webhook exfil → 触发点：cookie 偷
- 'TLS-poison: client-hello SNI 后 32 字节注入 → 假设：可绕过前置代理'
- vsFTPd 3.0.3 蜜罐：监听 2048 + USER/PASS + CWD secret_path → 触发点：PASV 模式引 mysql
- 动作：227 Passive Mode (127,0,0.1,43,192) → 假设：mysql 端口在 11200
- MySQL LOAD DATA LOCAL：触发 mysql client 读 /flag → 假设：服务端会执行查询
- 观察：file:// 协议读取敏感文件 → 完成
failed_attempts:
- 试图用 <img src=x onerror=...> → 失败：CSP nonce 限制
- 试图 <iframe> → 失败：frame-src 'none'
- 试图 <script src=https://unpkg.com/...> → 失败：nonce 替代不被允许
key_observations:
- BASH_FUNC_env%% 函数名导出自 2014 Shellshock 漏洞至今仍是 CGI 风险
- JSON.stringify/parse 钩子是 Web 前端 exfil cookie/flag 经典手法
- TLS-poison 在 SNI 后 32 字节注入可绕过部分 7 层 IDS
- vsFTPd PASV 模式可引到非 FTP 服务做 MySQL 攻击
- MySQL LOAD DATA LOCAL 是任意文件读客户端侧漏洞 2018 后被默认禁
prerequisites:
- Shellshock (CVE-2014-6271) 与 BASH_FUNC_env 攻击
- WebSocket XSS + animation 事件触发链
- vsFTPd PASV 模式 + MySQL 客户端 file:// 读取
---
# ACTF Writeup by SU
