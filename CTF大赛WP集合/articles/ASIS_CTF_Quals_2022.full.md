...
- BinomialExpansion
- enc % pubkey^2 // pubkey
- LatticeHashCollision
- chaffymasking
- fixed_p DLP
- CRT Joye-Libert
- EC离散对数smallfactor
- EC Pohlig-Hellman+CRT
- ratio nearby_rational
- LSB_attack
- openssl_pbkdf2
- Xor256爆破
attack_chain:
...
quality: high
full_path: ASIS_CTF_Quals_2022.full.md
meta_path: ASIS_CTF_Quals_2022.meta.md
images_removed: true
images_removed_count: 0
schema_version: v3.0.0-P0
summary: ASIS CTF Quals 2022。ASIS CTF Quals 2022 — 8 道 Crypto 集锦：Binomial 展开 + Lattice 哈希碰撞 + Fixed p DLP + Joye-Libert CRT 还原 + 嵌套 EC Pohlig-Hellman + nearby_rational 求 phi + LSB discard 收窄 + openssl pbkdf...
category: crypto
subcategory: rsa
time_required: long
difficulty_score: 4
...
contest_type: open
wp_url: https://www.ctfiot.com/67889.html
wp_author: your
reasoning_chain:
- 'Binomial: enc % pubkey^2 // pubkey → 触发点：模运算拆 (a+b)^2'
- 假设：(a+b)^2 拆 a²+2ab+b² 但 mod n² 后只剩 a² → 动作：long_to_bytes(enc%n²//n)
- 观察：flag ASIS{8!N0miaL_3XpAn5iOn_Us4G3_1N_cRyp7o_9rApHy!} → Lattice hash collision
- 'chaffymasking: 1024 bit salt 但同 salt 同 masked_flag → 触发点：固定输出'
- 动作：直接 bytes.fromhex(masked_flag.hex()) → 观察：flag 不需要解 salt
- 'Fixed p DLP: p 已知 → 假设 x = log(1-p, p) → 动作：send (1-p)'
- 观察：服务端返 ASIS{fiX3d_pOIn7s_f0r_d!5Cret3_l0g4riThmS!} → Joye-Libert CRT
- 'Joye-Libert: n>>268 拆 a+b + a*b → GF(2^134) 算 (F(c)^a / F(s)^a).log() → 假设：solve([a+b==sm, a*b==pd])'
- 'Nested EC: y1=0, y2=i 控制曲线 → Pohlig-Hellman small factor + CRT 拼 → 触发点：小因子分解'
- 'Nearby rational: (y-x)/n 2^64 逼近 → 假设：可反推 phi'
- 'LSB discard: 40 字符 8 个候选 → 假设：逐位 discard 收窄到 1 个'
- XOR 256 字节爆破 wormrep.klr.enc1 → openssl aes-256-cbc -pbkdf2 -a -d -salt 解密
failed_attempts:
- 试图直接解 RSA → 失败：enc 是 (a+b)^2 不是 RSA
- 试图解 chaffymasking → 失败：服务端固定 masked_flag 不需真 salt
- 试图 EC brute-force 离散对数 → 失败：order 太大走 Pohlig-Hellman
key_observations:
- enc % n² // n 是 Binomial 展开拆解 (a+b)² 的标志
- chaffymasking XOR 漏洞 = 服务端固定输出时不需要 salt
- Joye-Libert 是教科书方案，a+b / a*b 二元方程解 a,b
- Pohlig-Hellman + CRT 拼 small factor 是 EC DLP 标准法
- nearby_rational(max_denominator=2^64) 是连分数在 phi 恢复的应用
prerequisites:
- Sage 数学工具（QuadraticField, GF, EllipticCurve）
- Pohlig-Hellman 算法 + CRT
- 连分数 (continued fraction) + nearby_rational
---
# ASIS CTF Quals 2022
