...
quality: high
full_path: Realworld_CTF_之_hso-groupie_Writeup.full.md
meta_path: Realworld_CTF_之_hso-groupie_Writeup.meta.md
images_removed: true
images_removed_count: 5
schema_version: v3.0.0-P0
summary: Realworld CTF 之 hso-groupie Writeup (xpdf 4.03 JBIG2 RCE)。Realworld CTF 之 hso-groupie：xpdf 4.03 JBIG2Stream 整数溢出 + 堆风水覆盖 GList 后备存储 + 图灵完备 (全加器 + 5 种 bit 操作) + ROP/Shellcode 完整 exp PDF。。关键路径：Projec...
category: misc
...
contest_type: open
wp_url: https://www.ctfiot.com/25951.html
reasoning_chain:
- 题目 xpdf 4.03 + flag in /run/secrets/flag → 触发点：Clone-and-Pwn 找 CVE
- 搜索 Project Zero NSO iMessage 0-click → 关键思路：JBIG2 整数溢出 + 图灵完备运算
- 看 JBIG2Stream::readTextRegionSeg → numSyms += getSize() 整数溢出 → 假设：syms[] 堆溢出
- gmallocn(numSyms, sizeof(JBIG2Bitmap*)) → 分配过小 + 写超出 → 覆盖 GList 后备指针
- 观察：堆风水覆盖 pageBitmap.Width/Height → 假设：pageBitmap.expand 越界读写
- pageBitmap.combine 5 种位运算 (与/或/异或/替换) → 假设：图灵完备可构造全加器
- 全加器 → ROP gadget → 栈上 shellcode → PDF FlateDecode+JBIG2Decode 包装 → sploit.pdf 触发
failed_attempts:
- 试图直接堆喷弹 shellcode → 失败：pie + canary 阻挡，需先 leak
- 试图用普通 PDF 漏洞 → 失败：必须用 JBIG2 流触发 pageBitmap
- 试图直接构造 shellcode PDF → 失败：堆风水难，需 ROP 中转
key_observations:
- JBIG2 算术编码 + pageBitmap 与/或/异或/替换 5 种运算可构建图灵完备计算机
- 整数溢出 + 堆风水是 Project Zero 0-click 攻击核心思路
- Clone-and-Pwn 模式需看最近 CVE 修复记录
- 全加器是图灵完备计算的最简构造单元
- GList 后备指针覆盖是 GLib 应用通用堆攻击面
prerequisites:
- xpdf / poppler PDF 渲染库架构
- JBIG2 算术编码标准（fcd14492）
- Project Zero NSO iMessage 漏洞论文
- 图灵完备计算基础（全加器/位运算）
---
