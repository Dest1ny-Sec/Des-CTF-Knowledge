...
quality: high
full_path: RCTF_2024_Writeup_–Polaris战队.full.md
meta_path: RCTF_2024_Writeup_–Polaris战队.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: 'RCTF 2024 Writeup - Polaris 战队。RCTF 2024 多题大杂烩：Crypto DSA 私钥恢复 BKZ + RSA LLL 64字符 XCTF 背包 + PWN 多线程 race condition + dwebp CVE-2023-4863 + Web LDAP/JNDI + love_key SQL 注入。。关键路径：Crypto SignSystem: 1...'
category: pwn
subcategory: rop
tools_used:
- C
...
contest_type: open
wp_url: https://www.ctfiot.com/184335.html
reasoning_chain:
- 多题大杂烩 (Crypto/PWN/Misc/Web) → 触发点：DSA 19 次签名 + LWE-HNP 是核心
- SignSystem：每签 (r,s) = ((g^k mod p) mod q, (H+x*r)/k mod q) → 假设：19 次签名可 BKZ 恢复私钥
- 动作：构造格 A=[R[i]*s0*inv(r0*S[i],q)] + BKZ block_size=30 → 观察：找短向量
- 高 152 bit 未知 → 动作：穷举 256*4=1024 → 验证 sign(pub,d,"get flag",k0) → 拿到 flag
- PWN 五一国际劳动节：多线程菜单 + race condition → 整数溢出 money → MagicCastle SwitchHandle(1337) → system
- dwebp：CVE-2023-4863 libwebp 堆溢出 + mistymntncop craft.c PoC → add(0x43a0)+edit(0x20)+delete 触发
- Web what_is_love：love_key SQL 注入 jasper' || love_key REGEXP '^R'# → 倒序 64 字符爆破
- Web openYourEyes：Spring LDAP search JNDI → c_lookup → c_processJunction_nns → 远程反序列化
failed_attempts:
- 试图直接解 DSA 单签 → 失败：必须多签 + 短向量
- 试图覆盖 canary 全 256 字节 → 失败：canary 单字节爆破更快
- 试图用 fastjson 反序列化 → 失败：必须走 Hessian/LDAP
key_observations:
- DSA 19 次签名 + 高 8 bit 未知可用 LLL/BKZ 短向量恢复私钥
- CVE-2023-4863 libwebp 堆溢出可绕过 dwebp 沙盒
- Spring LDAP search JNDI 是 2024+ 主流反序列化入口
- parseInt('non-numeric')=NaN 是 JS 环境变量绕过
- love_key REGEXP '^X' 倒序逐字符爆破是盲注 SQL 标准手法
prerequisites:
- DSA 签名 + 格基约化 (LLL/BKZ)
- libwebp heap 漏洞原理
- Spring LDAP JNDI 注入链
- SQL REGEXP 倒序爆破
---
