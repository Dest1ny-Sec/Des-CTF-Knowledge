...
quality: high
full_path: 从Wiz_Cluster_Games_挑战赛漫谈K8s集群安全.full.md
meta_path: 从Wiz_Cluster_Games_挑战赛漫谈K8s集群安全.meta.md
images_removed: true
images_removed_count: 5
schema_version: v3.0.0-P0
summary: '从Wiz Cluster Games挑战赛漫谈K8s集群安全。Wiz EKS 5 关云原生攻击链：RBAC 读 secret → imagePullSecrets 拉镜像 → IMDS 提 AWS 凭证 → ECR 历史泄密 → IRSA AssumeRoleWithWebIdentity 读 S3。。关键路径：Challenge 1: RBAC 仅给 secrets get/list → ...'
category: web
subcategory: logic
...
contest_type: open
wp_url: https://www.ctfiot.com/151534.html
reasoning_chain:
- 'Challenge 1: RBAC 给 secrets get/list → 触发点：能读 k8s secret'
- kubectl get secret log-rotate -o json → base64 解码 'flag' 字段
- 'Challenge 2: pods list/get + secrets get → 假设：imagePullSecrets 含镜像仓库密码'
- kubectl get pod database-pod-2c9b3a4e -o yaml → 找 imagePullSecrets=registry-pull-secrets-780bab1d
- 假设：.dockerconfigjson base64 → crane auth login index.docker.io 用 eksclustergames 登录
- 动作：crane pull eksclustergames/base_ext_image /tmp/image.tar → tar tvf 看 flag.txt
- 'Challenge 3: ECR 镜像 401 → IMDS 169.254.169.254/latest/meta-data/iam/security-credentials 拿 ASIA 凭证'
- 假设：ECR 需 AWS 凭证 → aws ecr get-login-password docker login 拉 central_repo-aaf4a7c
- 动作：docker history --no-trunc 翻 RUN 层泄 ARTIFACTORY_TOKEN
- 'Challenge 4: SA 无权 → IMDS 拿 NodeInstanceRole → aws eks get-token → kubectl --token 读 secrets'
- 'Challenge 5: SA debug-sa IRSA 绑 challengeEksS3Role → kubectl create token debug-sa'
- 假设：IRSA AssumeRoleWithWebIdentity 拿 STS → 动作：aws sts assume-role-with-webidentity
failed_attempts:
- 试图直接 kubectl exec 进 pod 拿 flag → 失败：没 pods/exec 权限
- 试图用 token 登 crane → 失败：crane 不认 OIDC token
- 试图 docker pull ECR 镜像 → 失败：401 Unauthorized
key_observations:
- K8s 攻击面按 RBAC→Secrets→IMDS→IRSA 层层升级
- aws sts get-caller-identity 是验证当前凭证的快捷命令
- service-account-token + OIDC 是 K8s 1.24+ 标准姿势
- docker history --no-trunc 翻镜像层是泄密 token 的高招
prerequisites:
- Kubernetes 基础（kubectl get/describe/yaml）
- AWS IMDS 凭证提取（169.254.169.254）
- IRSA (IAM Roles for Service Accounts) AssumeRoleWithWebIdentity
- Docker 镜像拉取与 history 层分析
---
