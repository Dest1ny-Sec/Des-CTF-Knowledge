...
quality: high
full_path: 【WP】2022数字中国创新大赛车联网安全赛初赛.full.md
meta_path: 【WP】2022数字中国创新大赛车联网安全赛初赛.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: 【WP】2022数字中国创新大赛车联网安全赛初赛。2022 数字中国车联网初赛 6 题 WP，CANopen 协议 SDO 读对象解析 + 仲裁 ID 表查询。。经验：CANopen 协议常用对象：0x1000 设备类型/0x1008 设备名/0x1009 硬件版本/0x100A 软件版...
category: misc
...
contest_type: open
wp_url: https://www.ctfiot.com/41033.html
reasoning_chain:
- 打开 maybeEasy.pcapng → strings 提字符串 → 看到 CANopen 字串 → 触发点：CANopen 协议
- 假设：CANopen 协议 → 找 4 个标准 SDO 读请求 40 00 10 00 / 40 08 10 00 / 40 09 10 00 / 40 0A 10 00
- 主从响应 CS=43（多字节）/41（字符串） → 动作：直接解析 hex
- 设备类型 41f7 + 设备名 CANopenDemoLinux-9f1b + 硬件版本 f957da98 + 软件版本 b0459595fd93
- CANopenDemoLinux 专属他裁 ID 表 0x2120 子 ID 表 → 动作：拼出 flag
failed_attempts:
- 试图直接看 CAN 帧内容 → 需 SDO 协议知识
- 试图拿 1000-100A 之外对象 → 不是标准 SDO
- 试图从 OBD-II 入口 → 是 CANopen 不是 OBD
key_observations:
- CANopen 协议标准对象：0x1000 设备类型/0x1008 设备名/0x1009 硬件版本/0x100A 软件版本
- SDO 读 CS=40（请求）+ 43（多字节响应）+ 41（字符串响应）
- strings 提取协议字符串是 pcap 分析第一步
- CANopenDemoLinux 是开源 CANopen 协议栈
prerequisites:
- CAN 总线 + CANopen 协议
- SDO/PDO 服务类型
- strings 命令提取 pcap 字符串
- Wireshark CAN 解码
---
