...
quality: high
full_path: 【二等奖方案】数据湖流批一体性能优化「TT」团队思路.full.md
meta_path: 【二等奖方案】数据湖流批一体性能优化「TT」团队思路.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: CCF BDCI 2022 二等奖 - 数据湖流批一体性能优化 TT。汤振东+樊秋轩双人在LakeSoul源码加自定义版本号使乱序upsert并行可行,数据湖性能优化拿二等奖。。经验：LakeSoul等数据湖默认按系统时间戳决定合并顺序,无法乱序并行;改源码增userDefinedTimestamp使并...
category: misc
...
contest_type: open
wp_url: https://www.ctfiot.com/135378.html
reasoning_chain:
- 赛题：数据湖流批一体乱序 upsert → 触发点：LakeSoul 默认按系统时间戳合并
- 假设：默认行为无法乱序并行 → 动作：改 LakeSoul 源码加 userDefinedTimestamp 字段
- 假设：并行 + 乱序 + 补数场景都需要 userDefinedTimestamp → 动作：全局替换默认时间戳为业务时间
- 11 个 parquet 用 Future 并行 upsert → 假设：异步可加速 → 动作：scala.concurrent.Future.traverse
- 触发点：0 号大文件写入慢 → 假设：repartition 可分散 → 动作：repartition=8
- LakeSoul MOR 表读时合并 → 触发点：COW 写时合并对小写频繁场景太慢 → 假设：MOR 更合适
- 动作：注册 version=999 读最新元数据 → 观察：读性能比 COW 高 3 倍
- MergeOperator 注册求和/非空/最后值 → 触发点：默认合并逻辑只取最新 → 假设：自定义合并语义
- 动作：org.apache.spark.sql.lakesoul.operators.MergeOperator 注册自定义类型 → 观察：字段级合并正确
- snappy 压缩 parquet 输出 → 触发点：默认 uncompressed → 假设：snappy 平衡压缩率与速度
- 最终：自定义 Upsert 版本号 + Future 并行 + LakeSoul MOR + 读时合并 → 二等奖
failed_attempts:
- 试图用默认 LakeSoul 系统时间戳 → 失败：乱序 upsert 后数据丢失
- 试图用 COW 表 (写时合并) → 失败：小写频繁场景写入太慢
- 试图不用 snappy → 失败：输出 parquet 体积大、读慢
key_observations:
- LakeSoul/Iceberg/Hudi 默认按系统时间戳无法乱序并行，必须改源码加 userDefinedTimestamp
- repartition 数应根据数据量+核数判断，小文件直接写入避免 shuffle
- MergeOperator 注册支持字段级合并策略 (求和/非空/最后值) 是高级用法
- COW (Copy on Write) vs MOR (Merge on Read) 选择对性能影响巨大 (写频繁选 MOR)
- Spark Future 并行 upsert 比 RDD partitionBy 写快 2 倍
prerequisites:
- LakeSoul/Iceberg/Hudi 数据湖架构
- Spark Scala API + Future 并发
- Parquet 列存 + Snappy 压缩
- 源码修改 (Scala/Java) 加自定义字段
---
