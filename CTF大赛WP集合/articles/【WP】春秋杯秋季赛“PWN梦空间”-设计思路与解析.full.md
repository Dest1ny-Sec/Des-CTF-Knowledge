...
quality: high
full_path: 【WP】春秋杯秋季赛“PWN梦空间”-设计思路与解析.full.md
meta_path: 【WP】春秋杯秋季赛“PWN梦空间”-设计思路与解析.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: 【WP】春秋杯秋季赛 "PWN 梦空间" 设计思路与解析。春秋杯秋季赛 PWN 梦空间 6 层梦境关卡设计稿，盗梦空间 + 神秘博士剧情融合，覆盖 ret2text/格式化字符串/UAF/house of force 多种 PWN 技巧。。经验：6 层关卡设计覆盖栈溢出 / fmt 写 .text 段 / UAF / House of Force 全 PWN 攻击...
category: pwn
...
contest_type: open
wp_url: https://www.ctfiot.com/23827.html
reasoning_chain:
- city：linux 命令使用 → 基础 shell 题 → 动作：ls / cat / env
- hotel：栈溢出 0x38 padding + p64(0x4006B6 backdoor) → ret2text 完成
- snow：一次格式化字符串 %1515c%43$n → 写 .text 段 RWX → shellcode 注入
- home：UAF 释放后引用 → 假设：tcache 攻击
- world：House of Force → 改 top chunk size=-1 → 拿 top chunk 迁到 got 区 → 观察：free.got=puts-ret gadget 泄 libc
- world：free.got=system → final：life / death 文件夹 flxxg 文件 → 拼图得 root flag
failed_attempts:
- 试图 hotel 二次溢出 → 后端栈不够
- 试图 snow 多次 fmt → 仅一次机会
- 试图 world 走 unsorted bin → 缺 leak
key_observations:
- 6 层关卡设计 = 盗梦空间剧情包装
- 栈溢出 ret2text 是 PWN 入门
- 格式化字符串写 .text 段 RWX 是绕 NX 经典手法
- House of Force 拿 top chunk 迁到 got 区 = 经典 GOT hijack
- UAF + tcache 是 glibc 2.27+ 主流攻击
prerequisites:
- 栈溢出 + ret2text
- 格式化字符串 %n 任意地址写
- glibc heap（UAF/tcache/unsorted bin）
- House of Force + pwntools
---
