...
quality: medium
full_path: 国赛babytree_赛题解析.full.md
meta_path: 国赛babytree_赛题解析.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: 国赛babytree 赛题解析。Swift babytree 逆向：key="345y" 4 字节循环移位 + r0/r1/r2/r3 异或 + 42 字节目标反推。。关键路径：Swift babytree 逆向 (swiftc -dump-ast hello.swift) → var key = "345y" (4 字节) → var data = CommandLine.argu...
category: reverse
subcategory: reverse
tools_used:
- Radare2
...
contest_type: open
wp_url: https://www.ctfiot.com/162996.html
reasoning_chain:
- '国赛 babytree Swift 逆向, swiftc -dump-ast hello.swift → 触发点: Swift 源码还原'
- '观察: check(data, key) 函数 key=''345y'' (4 字节) → 假设: 经典 4 字节循环移位加密 → 动作: IDApython patch'
- 'var b = [UInt8](data.utf8), k = [UInt8](''345y''.utf8) = [51, 52, 53, 121] → for i in 0..b.count-4: → 触发点: 加密循环'
- 'r0,r1,r2,r3 = b[i..i+3]; b[i+0] = r2 ^ ((k[0] + (r0>>4)) & 0xff); b[i+1] = r3 ^ ((k[1] + (r1>>2)) & 0xff); b[i+2] = r0 ^ k[2]; b[i+3] = r1 ^ k[3] → 假设: 可逆'
- 'k = [k[1], k[2], k[3], k[0]] 循环左移 → 假设: 反向倒序循环 + k 循环右移恢复 → 动作: r0/r1/r2/r3 恢复'
- 目标 [88,35,88,225,7,201,57,94,...,255,20,16] (42 字节) → 倒推 input data
failed_attempts:
- '试图不解 Swift 字节码直接逆向二进制 → 失败: Swift 字节码复杂, 必须先还原 Swift 源码'
- '试图不解循环移位直接异或 → 失败: key 顺序错位'
key_observations:
- Swift 编译产物可用 swiftc -dump-ast 看 AST 还原源码 (比 IDApython patch 快)
- 4 字节 key 循环移位加密是经典 pattern
- '反向算法: 倒序循环 + k 循环右移'
- r0/r1/r2/r3 偏移量 (>>4 / >>2) 是非线性, 不能线性化
prerequisites:
- Swift 字节码逆向 (swiftc -dump-ast)
- '[UInt8] 数组操作'
- 循环移位加密模式
- IDApython patch
---
