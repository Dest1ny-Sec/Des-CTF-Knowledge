...
quality: high
full_path: 第七届蓝帽杯全国大学生网络安全技能大赛初赛Writeup.full.md
meta_path: 第七届蓝帽杯全国大学生网络安全技能大赛初赛Writeup.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: 第七届蓝帽杯全国大学生网络安全技能大赛初赛Writeup。第七届蓝帽杯初赛LovePHP：my[secret.flag绕过+Serializable接口绕过__wakeup+php://filter字符oracle。经验：GET参数名中[被替换为_→用my[secret.flag绕过；Serializable接口类不再支持__wakeup
category: web
...
contest_type: open
wp_url: https://www.ctfiot.com/132309.html
reasoning_chain:
- 触发点：LovePHP 源码 `file($_GET['secret'])` + `unserialize($_GET['my_secret.flag'])` →假设：反序列化触发文件读取
- 假设：GET 参数 `[` 被替换为 `_`→动作：用 `my[secret.flag=...` 绕过→观察：参数成功接收
- 假设：`__wakeup()` 把 check 置 False 必须绕过→动作：用 Serializable 接口类→观察：`C:8:"Saferman":0:{}` 序列化格式
- 假设：`file()` 接受 php://filter 流→动作：构造 php://filter 链→观察：调用 file()
- 假设：convert.iconv.L1.UCS-4LE filter 让字符串长度 ×4 触发 PHP memory limit 500 错误→动作：构造字符 oracle
- 假设：dechunk filter 在字符串无换行 + 0-9a-f 开头时清空→动作：与上一 oracle 组合判断单字符→观察：爆破出 flag
failed_attempts:
- 试图用标准 O 类反序列化 → 失败：触发 __wakeup check=False
- 试图走 php://filter/read=string.rot13 → 失败：file() 输出不可控，无法触发错误
- 试图 base64-decode 整文件 → 失败：长度爆炸触发 limit 立即 500
key_observations:
- GET 参数 `[`→`_` 替换绕过用 `my[secret.flag` 形式
- Serializable 接口类不再支持 __wakeup()，序列化为 `C:8:"ClassName":0:{}` 格式
- convert.iconv.L1.UCS-4LE 让字符串长度 ×4，触发 memory limit 是经典 error oracle
- dechunk filter(无换行时 0-9a-f 开头清空)与 iconv oracle 组合是单字符爆破
prerequisites:
- 'PHP 反序列化基础(O: vs C: 格式差异)'
- php://filter 流用法(convert.iconv / dechunk)
- PHP 内存限制与错误触发原理
- GET 参数特殊字符替换绕过
---
