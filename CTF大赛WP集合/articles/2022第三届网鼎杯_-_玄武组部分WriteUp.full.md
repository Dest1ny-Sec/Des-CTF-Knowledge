...
- slub
attack_chain:
- 'misc999 base62 换表: CHARSET=''9876543210qwertyuiopasdfghjklzxcvbnmMNBVCXZLKJHGFDSAPOIUYTREWQ'
- decode 7dFRjPItGFkeXAALp6GMKE9Y4R4BuNtIUK1RECFlU4f3PomCzGnfemFvO → flag{cf492422-13cb-4123-8bc5-5495f0349494}
- 'crypto 二次剩余 (Goldwasser-Micali): x=quadratic_residue 50%, x^r 也是 50%; flag bit=1→x^r (QR), bit=0→r (QR 50%)'
- 13 组数据, flag 错误概率 1/2^13
- 'pwn557 内核 UAF: 分配 0x20 opcode 1024 大小, 释放 0x30 不置零, 4 步利用'
- 'Step 1: 堆喷 msg_msg 主从消息队列'
- 'Step 2: 构造 UAF, 堆喷 sk_buff 定位 victim'
- 'Step 3: 堆喷 sk_buff 伪造辅助消息, 泄露 UAF obj 地址'
- 'Step 4: 堆喷 pipe_buffer, 泄露内核基址'
- 'Step 5: 伪造 pipe_buffer, 构造 ROP, 劫持 RIP, 提权'
...
quality: high
full_path: 2022第三届网鼎杯_-_玄武组部分WriteUp.full.md
meta_path: 2022第三届网鼎杯_-_玄武组部分WriteUp.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: '2022 第三届网鼎杯 - 玄武组部分 WriteUp。网鼎杯 2022 玄武：base62 换表 + Goldwasser-Micali 二次剩余 + 内核 pwn UAF。关键路径：misc999 base62 换表: CHARSET=''9876543210qwertyuiopasdfghjklzxcvbnmMNBVCXZLKJHGFDSAPOIUYTREWQ → decode 7dFR...'
category: crypto
subcategory: oracle
subcategories:
- oracle
...
contest_type: open
wp_url: https://www.ctfiot.com/56442.html
reasoning_chain:
- 'misc999: 表 9876543210qwertyuiopasdfghjklzxcvbnmMNBVCXZLKJHGFDSAPOIUYTREWQ 密文 7dFRjPItGFkeXAALp6GMKE9Y4R4BuNtIUK1RECFlU4f3PomCzGnfemFvO → 触发点：base62 换表'
- 假设：标准 base62 是 0-9a-zA-Z，题目表是 9876543210qwertyu...MNBVCXZ → 动作：改 suminb/base62 模块 CHARSET
- 动作：base62.decodebytes(str1) → 观察：flag{cf492422-13cb-4123-8bc5-5495f0349494}
- 'crypto: x^r 是 50% 二次剩余 → 假设：flag 是 39 bit 二进制'
- 动作：18 个 256-bit prime + enc[i] = x^r 或 r → 收集 13 组二次剩余数据
- 假设：flag 设为全 1，每组数据更新 → flag bit=0 → 50% QR，flag bit=1 → 必 QR → 错误率 1/2^13
- 观察：恢复 39 bit flag
- 'pwn557: 0x20 分配 1024 + 0x30 free 不置零 → 假设：slub UAF + D3CTF2022 d3kheap 思路'
- 动作：堆喷 msg_msg 定位 victim + sk_buff 伪造辅助消息 + pipe_buffer 泄露内核基址
- 观察：ROP 劫持 RIP → 提权
failed_attempts:
- 试图用标准 base62 → 失败：换表非标准
- 试图爆破 256-bit 二次剩余 → 失败：x^r 不能爆破
- 试图在 glibc 2.x 做 1024 大小 UAF → 失败：环境是内核 slub 分配器
key_observations:
- base62 换表是 misc 入门手法（看表头异常字符）
- Goldwasser-Micali QR 加密可被多组二次剩余性质破解
- 内核 pwn UAF + msg_msg/sk_buff/pipe_buffer 堆喷是 2022 热点
- D3CTF2022 d3kheap 是经典内核 pwn 模板
prerequisites:
- base62 编码原理 + GitHub base62 模块使用
- 二次剩余（Quadratic Residue）数论
- Linux 内核 slub 分配器原理
- msg_msg / sk_buff / pipe_buffer 内核结构体
---
