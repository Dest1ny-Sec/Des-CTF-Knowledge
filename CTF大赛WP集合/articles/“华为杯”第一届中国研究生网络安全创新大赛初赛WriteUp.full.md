...
quality: high
full_path: “华为杯”第一届中国研究生网络安全创新大赛初赛WriteUp.full.md
meta_path: “华为杯”第一届中国研究生网络安全创新大赛初赛WriteUp.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: 华为杯第一届中国研究生网络安全创新大赛初赛WriteUp。华为杯 2022 研究生国赛初赛 EDI 安全战队 WP，Web HS256 伪造 + URL 对象 bypass / Crypto 四层 LCG / Re angr / Pwn 栈迁移四方向。。经验：JWT 用 public.pem 作 HS256 secret 是经典算法混淆漏洞；URL 对象 pathname 可绕...
category: web
...
contest_type: open
wp_url: https://www.ctfiot.com/75117.html
reasoning_chain:
- 'Web：app.use(expressjwt({secret: publicKey, algorithms: ["HS256","RS256"]})) → 触发点：publicKey 既作 RS256 公钥又可作 HS256 共享密钥'
- 假设：jwt.sign(payload, publicKey, {algorithm:'HS256'}) 用 public.pem 作 HMAC 密钥 → 动作：构造 isAdmin:true username:admin payload
- 观察：secret = publicKey 文件内容，签名通过 → 下一步：绕过 /api/login 之外的 regex 关键字检查
- 动作：home.pathname 用 URL 编码 %72%6f%75%74%65%73/index.%6a%73 绕关键字 + url 对象构造 file:// 协议 /app/routes/index.js → 观察：成功
- 假设：/api/upload 可上传文件 + start.sh 有 nodemon 自动重启 → 动作：上传 execSync(req.query.cmd) 的 index.js
- Crypto：四层 LCG 挑战 solve1~4 分别 1/2/3/6 元逆元解 a/N/seed → 假设：等差数列求 N 用 GCD 三个二阶差分
- 动作：solve4 = GCD(GCD(t[1]*t[3]-t[2]**2, t[2]*t[4]-t[3]**2), t[0]*t[4]-t[1]*t[3]) → 完成
- 'Re：angr 符号执行 infantvm → 动作：sm.explore(find=lambda a: b''Good job'' in a.posix.dumps(1), avoid=lambda a: b''Try again'' in a.posix.dumps(1)) → 观察：拿到 flag'
- Pwn：栈迁移到 bss 0x6010A0 + leave_ret 跳板 + 泄 puts libc + one_gadget → 动作：payload = p64(ret)*0x20 + p64(pop_rdi)+p64(puts_got)+p64(puts_plt)+p64(main) → sendafter input your name → 完成
failed_attempts:
- Web 试图直接传路由路径字符串 → 失败：regex 关键字拦截
- Crypto 试图手工解 6 元 LCG → 失败：solve4 等差数列求 N 模式没意识到
- Pwn 试图覆盖返回地址到 system → 失败：canary 未知
key_observations:
- JWT 用 public.pem 作 HS256 secret 是经典算法混淆漏洞
- URL 对象 pathname 可绕关键字正则
- LCG 等差数列求 N 用 GCD 三个二阶差分
- angr entry_state + simulation_manager 找 good/avoid 路径
- 栈迁移到 bss + leave_ret 是经典 pwn 入门
prerequisites:
- JWT 签名算法（HS256/RS256）+ jsonwebtoken 库使用
- URL 对象 / JavaScript URI 解析
- LCG 数学（差分 + GCD + 求模逆元）
- angr 符号执行基础 + pwntools ret2libc + one_gadget
---
