...
quality: medium
full_path: Hackvent_2024_–_Easy.full.md
meta_path: Hackvent_2024_–_Easy.meta.md
images_removed: true
images_removed_count: 0
schema_version: v3.0.0-P0
summary: Hackvent 2024 – Easy。Hackvent 2024 Easy：gifsicle改色+EAN-8条形码校验位。关键路径：gifsicle --change-color 234 "#000000" --change-color 235 "#ffffff" 04ab...gif > mod.gif → 解压help.zip得到9个example*.png → 162x19...
category: misc
...
contest_type: open
wp_url: https://www.ctfiot.com/218690.html
reasoning_chain:
- 触发点：04ab...gif 含透明像素 + help.zip 含 9 张 PNG (162x191) → 假设：透明像素 → EAN-8 条形码
- 动作：gifsicle --change-color 234 '#000000' --change-color 235 '#ffffff' 04ab...gif > mod.gif → 观察：透明像素变黑白
- 下一步：解压 help.zip 得到 example*.png → 假设：每张是 EAN-8 条码但缺一数字 → 动作：写 Python EAN-8 校验位算法
- 观察：odd_sum = sum(odd positions * 3) + even_sum = sum(even positions) → check_digit = (10 - total%10) % 10 → 下一步：find_missing_digit 遍历 0-9
- 下一步：所有 PNG 的 EAN-8 缺位数字组成 flag → 动作：python checksum.py ?3371333 → 观察：missing_digit = 1
- 假设：9 个 EAN-8 缺位数字 (12, 5, 20, 9, 20, 19, 14, 15, 4) → 动作：chr(0x40 + N) → 观察：'L', 'E', 'T', 'I', 'T', 'S', 'N', 'O', 'D' → 拿到 HV24{...}
failed_attempts:
- 试图直接 png2ascii + 人工读 → 失败：EAN-8 编码复杂，必须按规范解码
- 试图 zbarimg 单次扫 → 失败：缺位无法识别
key_observations:
- gifsicle --change-color 是 GIF 透明像素改色的标准工具，必须先变黑白才能识别条形码
- EAN-8 校验位算法 (odd*3 + even) % 10 是 CTF 条形码题标配
- 9 个 PNG → 9 个数字 → 9 个字符是 emoji/字母转换题型的常见结构
- chr(0x40 + N) 把数字 1-26 映射到 A-Z 是 CTF flag 解码常用映射
prerequisites:
- gifsicle 命令行改透明色
- EAN-8 / UPC-A 校验位算法
- PIL PNG 处理 + EAN-8 解码
- Python find_missing_digit 暴力枚举
---
# Hackvent 2024 – Easy
