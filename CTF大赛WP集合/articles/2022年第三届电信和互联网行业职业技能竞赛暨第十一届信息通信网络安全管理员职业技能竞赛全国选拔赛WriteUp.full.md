...
- 临时密钥重用
attack_chain:
- 'PWN: 内联库函数, 调试器找 flag (无原文代码)'
- 'Reverse: 输入反转 + XOR 0x70 → 反转后 xor 0x70 还原'
- 'MISC 1: 1 进入 calc, print(eval(line)) 直接拿 key'
- 'MISC 2: __import__(''os'').popen(''cat flag'').read()'
- 'MISC 3: binwalk jpg → zip → CRC 碰撞密码 ohhhh_you_found_me → 解压 → base64 → 图片 SilentEye → flag'
- 'WEB func.php: ddddocr 识别验证码 + 异或 SQL 注入 1''^(ascii(substr(database(),{0},1))={1})^''1, 过滤 select, 改查 code 字段'
- 'WEB EzPop: POP 链构造'
- 'WEB 超级马里奥: js 藏 flag 路径'
- 'Crypto: 爆破 X0=4/5 → 恢复 key → 解密第一段 → 作 key 解密第二段'
- 'Crypto old_rsa: 费马小定理分解 modulus = s, p,q 关系 n/s 开根, 解 rsa'
...
quality: high
full_path: 2022年第三届电信和互联网行业职业技能竞赛暨第十一届信息通信网络安全管理员职业技能竞赛全国选拔赛WriteUp.full.md
meta_path: 2022年第三届电信和互联网行业职业技能竞赛暨第十一届信息通信网络安全管理员职业技能竞赛全国选拔赛WriteUp.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: '2022 年第三届电信和互联网行业职业技能竞赛 WriteUp。2022 电信职业技能赛：xor 0x70 + binwalk + CRC 碰撞 + ddddocr SQL 异或 + RSA。关键路径：PWN: 内联库函数, 调试器找 flag (无原文代码) → Reverse: 输入反转 + XOR 0x70 → 反转后 xor 0x70 还原 → MISC 1: 1 进入 calc, ...'
category: web
...
wp_url: https://www.ctfiot.com/61661.html
reasoning_chain:
- 'PWN: 二进制内联一坨库函数看不懂 → 触发点：调试器断在 xor 位置'
- 假设：输入反转 + XOR 0x70 是加密逻辑 → 动作：gdb 断在 xor 调用处，dump buffer 内容
- 观察：buffer 是反转 flag 后 XOR 0x70 的结果 → 动作：写脚本逐字节 XOR 0x70 + 反转 → 观察：还原 flag
- 'MISC 1: Python supper_calc 输入 key 通过 eval(line) → 假设：可注入 eval 调用'
- 动作：输入 __import__('os').popen('cat flag').read() → 观察：拿到 flag
- 'MISC 3: jpg + binwalk → zip → CRC 碰撞 → SilentEye → 假设：分段爆破解压密码'
- 动作：CRC 碰撞短文件名 ohhhh_you_found_me → SilentEye LSB → 观察：得到 flag
- 'WEB func.php: 验证码 ddddocr + SQL 异或 1''^(ascii(substr(code,{0},1))={1})^''1 → 假设：绕过 select 过滤'
- 动作：循环爆破 code 字段 → 观察：得到 flag
failed_attempts:
- 试图反编译 pwn 二进制分析逻辑 → 失败：内联库函数太多
- 试图手工解 rclone obfuscate → 失败：base64 表非标准
- 试图用常见 SQL 注入 union select 爆破 → 失败：select 被过滤
key_observations:
- XOR 0x70 + 输入反转是入门 reverse 套路
- Python eval(input) 是无沙箱 RCE 经典入口
- CRC32 碰撞短文件 zip 密码是 misc 入门手法
- SQL 异或注入 1'^ascii(substr())^'1 绕过关键字过滤
- 费马小定理可分解 RSA modulus 的特殊构造
prerequisites:
- gdb 调试器基础（断点 / dump memory）
- Python eval 沙箱逃逸
- binwalk + CRC32 碰撞工具链
- ddddocr 验证码识别 + SQL 异或注入
---
