...
- frida_hook
- android_dlopen_ext
- fread_buffer_overwrite
- sofixer_dump
- function_similarity_hash
- custom_isa_z3_solve
- 14_type_classification
- 256_func_bytecode_clustering
- instruction_lifting
...
quality: high
full_path: '[SUCTF2025]_SU_APP、SU_Harmony_出题小记.full.md'
meta_path: '[SUCTF2025]_SU_APP、SU_Harmony_出题小记.meta.md'
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: '[SUCTF2025] SU_APP、SU_Harmony 出题小记。SUCTF 2025 SU_APP Android 逆向：Frida hook android_dlopen_ext+fread 改写返回 ret 0x50 + SoFixer 修复 + 自定义 256 函数 ISA 14 类型聚类 + Z3 求解 32 字节 flag。。经验：Frida hook fread/strcm...'
category: misc
...
contest_type: open
wp_url: https://www.ctfiot.com/226563.html
reasoning_chain:
- 触发点:SU_APP 是 Android Native + Frida hook 入口 → 假设:hook android_dlopen_ext → 动作:Interceptor.attach(dlopen, onEnter)
- 动作:在 suapp 路径时 NativeHook → 0xA158 randcode 收集 + 0x9FA8 重置 → 观察:跑通 randcode 收集流程
- 动作:hook fread count==8 时改写 [0x50,0x00,0x00,0x58,0x00,0x02,0x1f,0xd6] 返回 ret 0x50 → 观察:fread 返回值控制成功
- 动作:SoFixer-Windows-64 修复 .so + IDA Python 批量重命名 + md5 哈希聚类 14 类型 → 观察:type1-14 函数归类完成
- 触发点:自定义 ISA 6 字节一指令 (a,b,c,indexA,indexB,logic) → 假设:C++ type1-14 模板 → 动作:getType(opcode) → 14 types
- 动作:Z3 Python BitVec 32 求解 32 字节输入 → 观察:得到 flag 输入
failed_attempts:
- 试图不解 fread 改写 → 失败:fread 是 anti-frida 检测点
- 试图不解 md5 哈希聚类 → 失败:256 函数必须先聚类才能逆向
- 试图不解自定义 ISA → 失败:type1-14 必须符号化求逆
key_observations:
- Frida hook fread/strcmp/memcmp 在 count 固定时改写返回值是 anti-debug 通用绕过
- 自定义 ISA 逆向中 'md5 哈希聚类函数类型' 是 CTF 高效还原利器
- SoFixer-Windows-64 是 .so 修复 + IDA 静态分析标准工具
- Interceptor.attach(dlopen, onEnter) + onLeave retval 是 Frida hook 标准用法
- Z3 Python BitVec + C++ type1-14 模板是自定义指令求解模板
prerequisites:
- Frida hook + android_dlopen_ext / fread 改写
- SoFixer 修复 .so 文件
- IDA Python 脚本 + md5 哈希聚类
- Z3 Python BitVec 求解
---
