...
- IDAPython
attack_chain:
- 'Web ezphp: ?num=1_11 PHP 弱类型 + 数字下划线绕过'
- 'Misc nan''s analysis: Wireshark 导出 ftp 对象 → 图片末尾附加 shell.zip (零宽隐写)'
- 'Misc reindeer game: 简单小游戏通关 → flag{82a2acb6-9803-4936-92db-f1431d90c6d1}'
- 'Re easy_python: Python 字节码逆向 → LOAD_CONST 列表 + 位旋转 (x >> 5 | x << 3) & 0xff'
- flag = [204,141,44,...] → flag{ddbae889-2895-44df-897d-2ae30df77b61}
...
quality: medium
full_path: 2022年春秋杯冬季赛部分WriteUp.full.md
meta_path: 2022年春秋杯冬季赛部分WriteUp.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: '2022 年春秋杯冬季赛部分 WriteUp (天权信安)。春秋杯 2022 冬季赛：PHP 下划线弱类型 + 零宽隐写 + Python 字节码。关键路径：Web ezphp: ?num=1_11 PHP 弱类型 + 数字下划线绕过 → Misc nan''s analysis: Wireshark 导出 ftp 对象 → 图片末尾附加 shell.zip (零宽隐写) → Misc rei...'
category: web
...
contest_type: open
wp_url: https://www.ctfiot.com/88076.html
reasoning_chain:
- 'Web ezphp: ?num=1_11 → 触发点：PHP 弱类型 + 数字下划线绕过'
- 假设：服务端用 == 比较且 num 转 int 后等于 11 → 动作：构造 ?num=1_11 → 观察：通过验证
- 'Misc nan''s analysis: pcap 文件 + ftp 数据 → 假设：导出 ftp 对象 → 动作：wireshark File → Export Objects → FTP'
- 观察：得到图片末尾追加 shell.zip（零宽隐写）→ 假设：零宽字符藏密码
- 动作：用在线工具解零宽 → 观察：得到密码解 zip → flag
- 'Re easy_python: Python bytecode 反编译 → 触发点：LOAD_CONST 列表 + 位旋转 (x >> 5 | x << 3) & 0xff'
- 假设：密文数组逆向 → 动作：写 flag[i] = (flag[i] >> 5 | flag[i] << 3) & 0xff → 观察：flag{ddbae889-2895-44df-897d-2ae30df77b61}
failed_attempts:
- 试图 ?num=11 直接绕过 → 失败：PHP 下划线才是答案
- 试图用 file2pcap 提取 ftp → 失败：流顺序乱序
- 试图用 010editor 看 pcap → 失败：图片末尾数据隐藏在 JPG 数据段中
key_observations:
- PHP 下划线数字 1_11 是 PHP 7+ 字面量，等价于 11，绕过弱类型比较
- Wireshark File → Export Objects → FTP 是 ftp-pcap 分析标配
- Python 字节码 LOAD_CONST + 位旋转是入门逆向题型
- 零宽隐写（Zero-Width Space）藏 zip 密码是 misc 经典套路
prerequisites:
- PHP 弱类型与下划线数字字面量（PHP 7+）
- Wireshark 对象导出
- Python 字节码逆向（LOAD_CONST / BINARY_SUBSCR）
- 位运算基础（>> / << / & / |）
---
