...
- UploadFile-destruct
- Prophecy-chain
- PHPUnit-MockObject-MockTrait-generate-eval
- swoole-RCE
...
quality: high
full_path: 【WP】2022年春秋杯春季赛Web类题目解析.full.md
meta_path: 【WP】2022年春秋杯春季赛Web类题目解析.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: 【WP】2022年春秋杯春季赛 Web 类题目解析。2022 春秋杯春季赛 Web 2 题 WP，EasySwoole + Redis 反序列化 + gopher 注入 + PHPUnit Mock 链 RCE。。经验：EasySwoole 默认 SERIALIZE_PHP 配置可被 gopher 注入利用；Swoole framewor...
category: web
...
contest_type: open
wp_url: https://www.ctfiot.com/42191.html
reasoning_chain:
- 读源码：Redis 类 SERIALIZE_PHP 配置 + 爬虫 curl 无协议过滤 → 触发点：gopher 注入反序列化
- 写 gopher URL 把序列化数据塗入 Redis → 动作：程序读取缓存触发反序列化
- 选 EasySwoole\Http\Message\UploadFile → __destruct → $this->stream->close → __call
- 构造 Prophecy\Prophecy\ObjectProphecy → Doubler\LazyDouble 链
- PHPUnit\Framework\MockObject\MockTrait::generate() → eval → RCE
- Swoole system() 无回显 → 抛异常方式输出
failed_attempts:
- 试图直接 Prophecy __call 链 → 缺 Faker 依赖
- 试图 Laravel/ThinkPHP gadget → 是 EasySwoole
- 试图 system 直接回显 → Swoole 不支持
key_observations:
- EasySwoole 默认 SERIALIZE_PHP = 反序列化 RCE 关键配置
- 爬虫无协议过滤 + Redis SERIALIZE_PHP = gopher 注入 SSRF 经典链
- Prophecy + PHPUnit MockObject 是 PHP 反序列化常见 gadget 库
- Swoole framework system() 无回显需 throw Exception
prerequisites:
- PHP 反序列化 POP 链构造
- EasySwoole 框架结构
- gopher 协议 + Redis 协议
- PHPUnit MockObject generate → eval 链
---
