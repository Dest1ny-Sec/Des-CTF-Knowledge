...
quality: high
full_path: 看雪2023_KCTF年度赛_-_第四题设计思路及解析.full.md
meta_path: 看雪2023_KCTF年度赛_-_第四题设计思路及解析.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: 看雪2023 KCTF年度赛 | 第四题设计思路及解析。KCTF2023年度赛Q4《AI控制空间站》LLVM PASS PWN：VecPass.so+vector.erase() double free+glibc 2.35。经验：C++ STL vector::erase单元素删除源码是漏洞关键（destroy+copy+--finish）；LLV...
category: pwn
...
contest_type: open
wp_url: https://www.ctfiot.com/134889.html
reasoning_chain:
- 题目4天全程无一人攻破 → 触发点：LLVM PASS PWN首次出现
- 假设：VecPass.so自定义模块是核心 → 动作：opt -load ./VecPass.so -winmt exp.ll -enable-new-pm=0
- 观察：runOnFunction虚函数重写 → 下一步：定位run()入口
- 函数名大写必须为KCTF才处理 → 假设：函数名是过滤条件
- 实例化15个ChunkInfo对象push_back到vector → 假设：vector::erase是漏洞源
- ChunkInfo{size, buf}析构free(buf) → 触发点：vector::erase源码审计
- 动作：erase = copy + --finish + destroy → 假设：buf已被free则double free
- 观察：erase后buf指针未清空 → 再次delete时再free → double free触发
- glibc 2.35 tcache/fastbin + 不同内核找共同劫持链 → 远程shell
failed_attempts:
- 试图用新版opt加载VecPass → 失败：必须加-enable-new-pm=0用旧PM
- 试图覆盖函数名不是KCTF的函数 → 失败：过滤条件直接跳过
- 直接free不通过erase → 失败：漏洞是vector::erase触发，不是显式free
key_observations:
- C++ STL vector::erase单元素删除源码 = destroy+copy+--finish 是审计点
- LLVM PASS自定义模块逆向 = 找vtable定位runOnFunction
- 函数名KCTF过滤 = 出题人留的特殊标记
- double free在glibc 2.35 tcache下需找稳定跨内核利用链
- ChunkInfo析构里free(buf) + erase前未清指针 = 经典double free
prerequisites:
- LLVM PASS开发/opt命令使用
- C++ STL源码级理解（vector::erase实现）
- glibc 2.35堆利用（tcache/fastbin）
- C++析构函数行为
---
