...
quality: high
full_path: Copy_Fail_深度研究：Linux_页缓存漏洞的根因、利用与检测.full.md
meta_path: Copy_Fail_深度研究：Linux_页缓存漏洞的根因、利用与检测.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: 'Copy Fail 深度研究：Linux 页缓存漏洞的根因、利用与检测。Linux page cache写穿漏洞：AF_ALG+splice in-place污染只读文件。关键路径：漏洞点：AF_ALG + splice in-place处理导致page cache被改 → authencesn: AAD[4:8]临时写入scratch供HMAC → sendmsg发8字节AAD 4字节X填...'
category: web
...
contest_type: open
wp_url: https://www.ctfiot.com/307687.html
reasoning_chain:
- 漏洞点 Linux 内核 AF_ALG + splice in-place 处理导致 page cache 被改写 → 触发点：容器逃逸类 CVE
- 阅读 crypto/authencesn.c 源码 → scatterwalk_map_and_copy 把 AAD[4:8] 临时写入 dst 偏移 assoclen+cryptlen → 假设：写操作落到 dst SGL 链尾的 page cache page
- 2017 之前用 out-of-place（src=TX SGL, dst=RX SGL）→ 2017 之后改为 in-place（src=dst=combined RX SGL）→ 观察：chained tag pages 是 splice 引入的原始 page cache 引用
- 动作：构造 PoC — socket(AF_ALG, SOCK_SEQPACKET) → setsockopt(ALG_SET_KEY) → accept → sendmsg 发 8B AAD（4B 填+4B evil_bytes）
- 动作：os.splice(target_fd, pipe_w, offset+4) 把 /usr/bin/su 的前 offset+4 字节引入 SGL → os.splice(pipe_r, op.fileno(), offset+4) 送 AF_ALG socket
- 假设：HMAC 校验失败返回 EBADMSG 但 page cache 写入已完成 → 动作：多次 page_cache_write_4bytes 覆盖 ELF 头
- 观察：篡改的 /usr/bin/su 含 root shellcode → 执行得 root shell → 完成容器逃逸
- 验证：drop_caches 3 不驱逐 tmpfs，但 ext4 上会从磁盘重读 → 同 namespace 容器共享镜像层 page cache
failed_attempts:
- 试图 out-of-place 模式触发写穿 → 失败：2017 修复后 RX/TX SGL 分离，无法污染原 page
- 试图 splice 普通文件触发覆盖 → 失败：splice 引入的是 page cache 引用，需 AEAD in-place 才会被改写
- 试图 AF_ALG socket 自身做读写绕过 → 失败：必须 sendmsg + splice 双通道，AAD 数据来自 sendmsg 而非 splice
key_observations:
- AF_ALG + splice 引入原始 page cache page 引用 + AEAD in-place sg_chain → 写操作污染只读 page
- authenc/seqiv 模块默认 in-place 是性能优化但牺牲了 page cache 隔离性
- 同 namespace 容器共享镜像层 page cache 是 docker 默认 seccomp 不阻止的关键
- /proc/sys/vm/drop_caches 对 tmpfs 无效但对 ext4 有效，可用于鉴别 page cache vs 磁盘数据
- scatterwalk_map_and_copy 写入方向参数（最后一个 1=写）是漏洞入口
prerequisites:
- Linux 内核 page cache 原理（共享 mmap/splice 零拷贝）
- AF_ALG socket 编程（setsockopt ALG_SET_KEY/OP/AEAD_AUTHSIZE + sendmsg/splice）
- 内核 scatterlist / sg_table 数据结构与 sg_chain 链表
- Linux 容器逃逸基础（namespace/seccomp/capabilities）
- QEMU + GDB + DWARF 调试符号的内核 PoC 验证
---
