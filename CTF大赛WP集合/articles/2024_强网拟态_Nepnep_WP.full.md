...
quality: high
full_path: 2024_强网拟态_Nepnep_WP.full.md
meta_path: 2024_强网拟态_Nepnep_WP.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: 2024 强网拟态 Nepnep WP。原型链污染 + pickle 内存马 + phar:// 反序列化 + JVM-SANDBOX uninstall。。经验：Python 原型链污染必须显式允许 safe_modules=['os','builtins'] 才有效；JVM-SA...
category: web
...
contest_type: open
wp_url: https://www.ctfiot.com/211007.html
reasoning_chain:
- 触发点：ez_picker /register JSON 接收 → 假设：原型链污染 → 动作：填 username=1&password=1&__init__.__globals__.safe_modules=['os']
- 观察：safe_modules=['os','builtins'] 显式允许 → 假设：safe_names=['eval','popen'] 白名单 → 动作：构造 secret_key=111
- '触发点：secret_key 污染 → 假设：pickle reduce eval() 写内存马 → 动作：构造 pickle payload add_route(''/shell'',lambda request: os.popen(cmd).read())'
- 观察：上传 1.pkl → 假设：触发 pickle 反序列化 → 动作：访问 /shell?cmd=cat /tr3e_fl4g_1s_h3re_lol
- 触发点：capoo file_get_contents 读 showpic.php → 假设：phar:// 反序列化 → 动作：上传 .gif stub 含 phar
- 观察：phar:// + gzip 压缩 GIF89a stub → 假设：绕 PILER 检测 → 动作：gzcompress 改 .gif
- 触发点：phar:// 反序列化 __wakeup + diff → 假设：读 flag → 动作：触发 __wakeup
- 观察：diff 触发 → 假设：flag 写入 → 动作：读 /flag
- 触发点：OnlineRunner Java 不 import 类 → 假设：java.io.FileReader 列目录 → 动作：java.io.FileReader('/')
- 观察：读 app.jar → 假设：反编译 agent.jar → 动作：调用 com.alibaba.jvm.sandbox.agent.AgentLauncher uninstall
- 触发点：uninstall RASP → 假设：Runtime.exec 通杀 → 动作：exec bash -c
- 观察：拿到 shell → 下一步：cat /flag
failed_attempts:
- 试图不污染 safe_modules → 失败：必须显式 ['os','builtins']
- 试图不解 phar:// stub → 失败：PILER 检测必须 GIF89a + gzip
- 试图不解 JVM-SANDBOX uninstall → 失败：RASP 拦所有 exec
- 试图不解 __wakeup diff → 失败：phar 触发条件不够
key_observations:
- Python 原型链污染必须显式允许 safe_modules=['os','builtins'] 才有效
- JVM-SANDBOX 启动时调用 uninstall() 可彻底关掉 RASP
- phar:// + gzip 压缩 GIF89a stub 绕 PILER 是图像文件包含经典 trick
- pickle reduce eval 是 Python 内存马标配
- agent.jar 反编译定位 AgentLauncher 是 JVM 沙箱 escape 关键
prerequisites:
- Python 原型链污染（__init__.__globals__）
- pickle reduce + eval 内存马
- phar:// 反序列化 + gzip stub 绕 PILER
- JVM-SANDBOX 架构与 uninstall API
- Java Runtime.exec + RASP 绕过
---
# 2024 强网拟态 Nepnep WP
