...
quality: high
full_path: 详解第九届XCTF国际网络攻防联赛总决赛0解PWN题.full.md
meta_path: 详解第九届XCTF国际网络攻防联赛总决赛0解PWN题.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: 详解第九届XCTF国际网络攻防联赛总决赛0解PWN题。第九届XCTF总决赛0解PWN题"All-Pwn-Red-Book App"详解,std::vector push_back realloc导致旧chunk遗留+UAF,vtable边界检查防type confusion,base64+AES+RSA加密链路。。关键路径：main用std::random_device+rand()生成...
category: pwn
...
contest_type: open
wp_url: https://www.ctfiot.com/278572.html
reasoning_chain:
- std::random_device + rand() 生成 salt 写入文件 → 触发点：C++ 笔记类 PWN + salt 持久化
- 菜单 create/list/edit/show/delete + 三种 Type (A=0x18,C=0x30,B=0x40) → 假设：vtable 攻击面
- vec push_back 触发 realloc 旧 chunk 释放但 vec 内指针残留 → 触发点：UAF/double-free 入口
- vtable 边界检查 ((x<<58)|(x>>6))>3 触发 ud1 崩溃 → 假设：必须控制 vtable 落在合法区间
- 动作：构造 TypeA→TypeB→TypeA 多次 alloc/realloc 触发旧 chunk free
- TypeA_edit 用 rand()%sub_6E750(obj_) 作 val → 受 srand seed 影响 → 假设：可控 rand 输出
- '密码校验 sub_6C8A0: 长度=32, 字符 ''!''-''~'', 至少 3 类(大写/小写/数字/特殊) → 绕过用标准密码学集合'
- 内容 base64 解码后 sub_6F930(256字节) AES 解密 → 假设：篡改 AES 链控流
- sub_6C5A0 生成 key，sub_6E0F0 存储 → 动作：覆盖指针拿 key → 触发解密
failed_attempts:
- 试图直接 type confusion TypeA→TypeB → 失败：vtable boundary check ((x<<58)|(x>>6))>3 拦截
- 试图覆盖 GOT → 失败：Full RELRO
- 试图覆盖 __free_hook → 失败：glibc 2.32+ 移除
- 试图反复 create/delete 让 tcache 池溢出 → 失败：vec push_back 才是触发点，create/delete 单调
key_observations:
- std::vector push_back 触发 realloc 旧 chunk free 但 vec 指针残留是 C++ PWN 经典漏洞
- vtable boundary check 通过差值编码 ((x<<58)|(x>>6))>3 实现，对小偏移有效但可绕过
- TypeA_edit 的 rand()%X 受 srand seed 控制 → 用 LD_PRELOAD 劫持可控
- base64+AES+RSA 链是 C++ PWN 数据加密常见组合
prerequisites:
- C++ std::vector / 智能指针 / vtable 机制
- glibc tcache + realloc 行为
- AES/RSA/base64 密码学 C++ 实现
- LD_PRELOAD 动态劫持 srand
---
