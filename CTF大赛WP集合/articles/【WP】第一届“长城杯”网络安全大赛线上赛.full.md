...
tags:
- custom-VM
- INFO-breakpoint-leak
- mmap-0x200000+libc-ld-offset
- page-table-64bit-low-16-bits
- LW-SW-arbitrary-RW
- one_gadget
- linkmap-text-base
- ROP-via-got
...
quality: high
full_path: 【WP】第一届“长城杯”网络安全大赛线上赛.full.md
meta_path: 【WP】第一届“长城杯”网络安全大赛线上赛.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: 【WP】第一届"长城杯"网络安全大赛线上赛。第一届长城杯网络安全大赛 easy_vm 高级 PWN，自定义 VM + 64 位页表检查漏洞 + LW/SW 任意地址读写。。经验：VM 申请内存 > 0x200000 触发 mmap 与 libc/ld 固定偏移是泄地址经典方法；64 位页表项只检查...
category: pwn
...
contest_type: open
wp_url: https://www.ctfiot.com/19019.html
reasoning_chain:
- easy_vm 二进制 → 触发点：自定义 VM（INFO 断点可打内存地址）→ 假设：VM 申请内存触发 mmap 与 libc/ld 固定偏移
- 动作：申请 0x300000 大块 → INFO 断点打印地址 → 观察：地址落在 mmap 区，与 libc/ld 偏移固定
- 假设：拿到 ld 基址后找 linkmap 里 .text 段基址 → 动作：解析 linkmap 结构 → 观察：拿到 .text 指针
- VM 有 PAGE 指令初始化页表 → 触发点：64 位页表项检查只查低 16 位 → 假设：可绕检查填任意 64 位值
- 动作：PAGE 写入用 ld 基址当低 16 位绕过检查 → 观察：页框基址被填入 .text
- VM 提供 LW/SW 指令读写页框 → 触发点：页框基址+24 位偏移 = 任意 64 位地址 R/W
- 动作：SW 改 puts.got = one_gadget → 观察：调用 puts 时跳 one_gadget
- EXIT 触发 → 系统触发 puts → 命中 one_gadget → get shell
failed_attempts:
- 试图用 INFO 泄 libc 后直接 ret2libc → 失败：VM 内只跑 VM 字节码，没有普通 ROP 链空间
- 试图让 PAGE 填真实合法 64 位地址 → 失败：检查算法只看低 16 位，全 0 也能混过
- 试图改 .got.plt 段 → 失败：ld 段保护严格，改 .text 更稳
key_observations:
- mmap 大块 (≥0x200000) 与 libc/ld 固定偏移是泄地址经典思路
- 64 位页表检查只看低 16 位是页表完整性漏洞的常见疏忽
- LW + SW + 页框基址 + 24 位偏移 = 任意地址 R/W 是 VM 沙盒逃逸模板
- one_gadget 是单指令 RCE 终杀，比 ROP 链更短
- linkmap 结构是 .text 段基址反查的官方入口
prerequisites:
- 自定义 VM 逆向（字节码 + 指令集分析）
- glibc mmap 分配机制（与 libc/ld 偏移关系）
- 页表结构 PTE/页框 64 位解析
- one_gadget 工具使用 + LD_PAGE/LW/SW 字节码编写
---
