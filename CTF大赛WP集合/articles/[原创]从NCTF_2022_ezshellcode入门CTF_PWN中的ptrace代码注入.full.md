...
tags:
- ptrace_attach
- ptrace_scope_disabled
- shellcode_two_process
- seccomp_sandbox_bypass
- mmap_rwx_0x401000
- getregs_setregs_rip_modify
- ptrace_peekdata_pokedata
- fork_parent_child_pwn
- pwn_shellcode_codegen
...
quality: high
full_path: '[原创]从NCTF_2022_ezshellcode入门CTF_PWN中的ptrace代码注入.full.md'
meta_path: '[原创]从NCTF_2022_ezshellcode入门CTF_PWN中的ptrace代码注入.meta.md'
images_removed: true
images_removed_count: 3
schema_version: v3.0.0-P0
summary: '[原创] 从 NCTF 2022 ezshellcode 入门 CTF PWN 中的 ptrace 代码注入。NCTF 2022 ezshellcode：父子进程 ptrace 注入绕过 close(0/1/2) + seccomp 沙箱，父进程 PTRACE_ATTACH+SYSCALL+GETREGS+SETREGS rip=0x401000 控制子进程直接 shellcraft.sh(...'
category: misc
...
contest_type: open
wp_url: https://www.ctfiot.com/84178.html
reasoning_chain:
- 触发点:程序 mmap(0x401000, 0x1000) rwx + 读 shellcode + seccomp 禁 socket + close(0/1/2) → 假设:可 fork 父子进程 ptrace 注入 → 动作:fork() + ptrace(PTRACE_ATTACH, pid)
- 动作:ptrace_scope=0 + shellcode = shellcraft.ptrace(0x10,PTRACE_ATTACH)+ptrace(0x18,DETACH)+wait4+ptrace(12,GETREGS)+setregs rip=0x401000 → 观察:父子双线执行
- 触发点:close(0/1/2) 在子进程但父进程没 close → 假设:可绕过 → 动作:子进程 0x401000 跳 shellcode 父子交互
- 动作:父进程 ptrace(PTRACE_SYSCALL) + wait4 → rax==0 持续 → rax!=0 时 setregs rip=0x401000 → 观察:控制子进程 PC
- 动作:detach + shellcraft.sh() → 观察:/bin/sh 触发 → 完成
failed_attempts:
- 试图单进程 seccomp bypass → 失败:seccomp 不可绕过
- 试图不解 ptrace_scope → 失败:Linux 容器 ptrace 默认禁
- 试图单 close(0/1/2) 反弹 → 失败:无 fd 可用
key_observations:
- ptrace_scope = 0 是 Linux 容器 CTF 标配
- 父子进程 ptrace 注入跳过 sandbox 限制是 2022 流行 pwn 技巧
- PTRACE_GETREGS 改 RIP 是控制子进程 PC 指针的通用方法
- seccomp 禁 socket + close fd 不能阻止 ptrace 注入
- shellcraft.ptrace + shellcraft.sh() 是 pwntools shellcode 速写
prerequisites:
- ptrace 系统调用 (PTRACE_ATTACH/SYSCALL/GETREGS/SETREGS/DETACH)
- ptrace_scope /proc/sys/kernel/yama/ptrace_scope 配置
- 父子进程 ptrace 注入原理
- shellcraft ptrace + sh() 模板
---
