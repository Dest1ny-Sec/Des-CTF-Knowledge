...
quality: high
full_path: UKY_2025UCSC_WP.full.md
meta_path: UKY_2025UCSC_WP.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: UKY 2025 UCSC WP。UKY 战队 UCSC 赛 8 小时 4 题全解+Misc 全收，覆盖 3 路 stego、3 道 pwn、4 道 re、4 道 crypto 完整链。。经验：高校赛题的"组合技"是常态：XR4=RC4+random seed；MERGE_ECC=爆破+Smart's att...
category: misc
...
contest_type: school
wp_url: https://www.ctfiot.com/244129.html
reasoning_chain:
- 触发点：附件包含 QR + 两个压缩包 + 多题型套娃 → 假设是高校赛典型'每题套娃'结构
- 小套不是套：QR 扫出 !@#QWE123987 → 解 test.zip 得到伪加密 mushroom.zip → 010editor 抽出 PNG IDAT 块拼回 89504E47 头 → 假设：从 PNG 隐写中找 CRC 已知明文
- 动作：用分离出的 PNG 作明文 bkcrack attack 真加密 套.zip → 观察：图片随波逐流给出 hint → flag{th1s1sf14g}
- three-ucs part1：BlindWatermark-v0.0.3.jar decode -f signwithflag.png flag.png → 观察：8f02d3e7
- part2：01001100... 二进制 → ASCII 'LS4uLi4tIC0u...' → base64 → morse '-ce89-4d6b-830e-' → 假设：flag 4 段拼接
- part3：pcapng thinkbell 解压密码 → part3.txt 5d0cb5695077 → 拼接 flag{8f02d3e7-ce89-4d6b-830e-5d0cb5695077}
- Userlogin：supersecureuser 两次 fmt-string → b'%10$p' 泄 stack_base → b'%{shell}c%9$hn' 写 ret → shell 0x1262
- BoFido：name 覆盖 rand 种子为 0 → 本地跑 rand 二进制同种子三次 → 把三个 rand_num 发回 → 触发后门
- 疯狂复制：edit(-4) 改 _IO_2_1_stdout_ 走 house_of_apple2 → fake_io_file flat({0:b'  sh', ...}) → puts → _IO_wfile_overflow → system
- easy_re：data[i]^10 一行代码 → flag{d7610b86-...} / simplere：魔改 UPX 改 CTF→UPX 脱壳 + 逆 base58 变表 → flag{0ba878d9-...}
- EZ_debug：RC4 密文+key 给齐 → 动调断点拿 flag / re_ez：BFS 找 +5,+5,+5,+1,+1,-5,-5,-5 路径 → XOR 3 + 32 → md5
- XR4：RC4 decrypt + random.seed() 重放 → 还原明文 / essential-ucsc：RSA p,q 邻近 iroot → p = iroot(N,2) / MERGE_ECC：ECC Pohlig-Hellman + Smart's attack
failed_attempts:
- 试图爆破套.zip 密码 → 失败：密码藏在 mushroom.png 的随波逐流隐写数据里
- three-ucs 试图直接读 PNG 找 flag → 失败：必须 BlindWatermark 工具 + 流量包密码 thinkbell 三段拼
- 疯狂复制 直接覆盖 _IO_list_all → 失败：触发链走 stdout 才是 House of Apple 2 标配
key_observations:
- 高校赛题的'组合技'是常态：XR4=RC4+random seed；MERGE_ECC=爆破+Smart's attack；Ez_Calculate=背包+RSA+Rabin 一题三杀
- fmt-string 两次利用 %10$p 泄地址 + %9$hn 写 ret 是入门 pwn 必会链
- house_of_apple2 通过 IO_wfile_overflow → _IO_wdoallocbuf → system 是 stdout 结构体劫持的高阶版
- 魔改 UPX 改 CTF→UPX 即可用标准 UPX 脱壳，是常见 RE 出题套路
- RSA p,q 邻近用 iroot(N,2) 一行还原 p，Rabin 密码 E=2 必须 CRT 拼 4 解
prerequisites:
- pwntools 基础（fmt-string payload 构造 + ret 写）
- House of Apple 2 原理（_IO_wfile_overflow 链）
- GDB 动调 + rc4 调试经验
- RSA/Rabin/ECC 密码学基础（CRT 拼接 + Smart's attack）
- BlindWatermark / bkcrack / snow 等工具使用
---
# UKY 2025UCSC WP
