...
quality: high
full_path: 【WriteUP】VSEC_车联网安全_CTF_挑战赛（二）.full.md
meta_path: 【WriteUP】VSEC_车联网安全_CTF_挑战赛（二）.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: 【WriteUP】VSEC 车联网安全 CTF 挑战赛（二）。VSEC 车联网安全 CTF（二），ICSim 模拟 + UDS Service 0x27 SecurityAccess + MAAATH 取反 + ReadMemory 读 flag。。经验：ICSim 是开源 CAN 仿真器 zombieCraig/ICSim；UDS Service 0x22 ReadDat...
category: misc
...
contest_type: open
wp_url: https://www.ctfiot.com/194326.html
reasoning_chain:
- ICSim 模拟器 → 触发点：./icsim vcan0 -s 10000 启动仪表盘 → 动作：./controls vcan0 -s 10000 启动控制器
- 统计 CAN ID 出现次数 → 假设：门锁/速度表/转向 arbitration ID 唯一 → 动作：candump -L vcan0
- cansend vcan0 7df#0322f190 → 触发点：UDS Service 0x22 ReadDataByIdentifier → 观察：返回 VIN
- DTC Pxxxx-xx 读 → 触发点：Diagnostic Trouble Code → 动作：cansend vcan0 7df#031903... → 观察：P0xxx 故障码
- Service 0x23 ReadMemoryByAddress → 触发点：0xC3F80000 → 动作：cansend vcan0 7df#0323C3F8000000000000
- 观察：内存内容 → 假设：需 SecurityAccess Level 3 解锁 → 动作：UDS 0x27 SecurityAccess Level 3
- MAAATH seed = 0x1337 → 触发点：算法 MAAATH → 假设：按位取反 → 动作：~0x1337 = 0xECC8 → 观察：key = 0xECC8
- Level 1 key = 9102870c c43b2d1b d64217a8 837bbdbf → 动作：完成 securityaccess → 读 flag
failed_attempts:
- 试图绕过 SecurityAccess 直接 0x23 读 → 失败：被 NRC 0x33 拒绝
- 试图用 0x22 读 flag → 失败：flag 在内存区，不是 DID
- 试图用 MAAATH 直接算 Level 3 → 失败：seed 0x1337 需按位取反才是 key
key_observations:
- ICSim = zombieCraig/ICSim 仪表盘仿真器，./icsim vcan0 -s seed 启动
- UDS Service 0x22 读 VIN / 0x23 读内存 / 0x27 解锁 SecurityAccess 三件套
- MAAATH 是车厂常用 SecurityAccess 算法，seed = 0x1337 时 key = ~0x1337 = 0xECC8
- DTC (Diagnostic Trouble Code) Pxxxx-xx 是故障码读取
- CAN ID 统计可反推 arbitration ID 是车辆功能识别
prerequisites:
- ICSim 仿真器使用（./icsim + ./controls）
- UDS 协议 Service 0x22/0x23/0x27
- MAAATH SecurityAccess 算法（按位取反 / 加减 / 异或）
- can-utils candump/cansend 命令
---
