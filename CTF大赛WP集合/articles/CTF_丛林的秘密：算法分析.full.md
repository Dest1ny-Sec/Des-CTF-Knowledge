...
- wasm2c
- libgogogo.so mm0导出
- Frida hook gogogoJNI.check_key
...
quality: high
full_path: CTF_丛林的秘密：算法分析.full.md
meta_path: CTF_丛林的秘密：算法分析.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: CTF 丛林的秘密：算法分析。CTF 丛林的秘密 — 安卓 + WebAssembly 算法分析 + libgogogo.so mm0 导出 wasm 字节 + wasm2c 转 c + 32x32 矩阵 numpy.linalg.solve 还原 flag。。关键路径：安卓 APK 装 WebAssembly, 验证逻辑在 wasm 内 → libgogogo.so 导出 mm0 函数 (...
category: reverse
subcategory: reverse
time_required: long
difficulty_score: 4
...
contest_type: open
wp_url: https://www.ctfiot.com/161579.html
reasoning_chain:
- 触发点：APK 含 WebAssembly 文件 + 校验逻辑在 wasm 内 → 假设：算法保护用 wasm 不暴露字节码
- 动作：jadx 看 MainActivity 加载 .wasm → 观察：调用 gogogoJNI.sayHello + gogogoJNI.check_key
- 假设：libgogogo.so 导出函数中 mm0 实际指向 wasm 字节 → 动作：Frida hook Module.findExportByName('libgogogo.so', 'mm0').readByteArray(35000)
- 观察：导出 35000 字节正是 wasm 字节码 → 下一步：wasm2c 转 c 看 32 字符 flag 的 check_key 函数
- 动作：wasm2c hex.wasm -o web.c → gcc -c web.c → web.o → 解析 WebAssembly.instantiate() 入口 set_input_flag_len + set_input_flag + check_key
- 下一步：发现 check_key 用 32x32 矩阵做约束求解 → 动作：wasm.dump 还原 32 个变量 + 32 个方程
- 假设：32 字符 flag = 32 字节线性方程组 → 动作：numpy 构建 32x32 矩阵 A + b，scipy.linalg.solve
- 观察：得到 32 个字节值 → 动作：拼接为 string → 完成
failed_attempts:
- 试图用 wasm 反编译工具 wasm-decompile → 失败：输出有混淆，不直接出 check_key 函数逻辑
- 试图直接用 wasm2wat + wat2c → 失败：wat 文本过长，C 代码可读性反而更差
- 试图单 hook check_key 拿返回值 → 失败：check_key 是 wasm 内函数，不能直接 hook
key_observations:
- WebAssembly 是 Android 算法保护新趋势（不能直接 hook 需导出 wasm 字节）
- Frida hook 导出函数 + readByteArray 拿 wasm 字节是 wasm 类题目标准流程
- wasm2c 转 c 后用 numpy 解线性方程组是 wasm 32x32 矩阵验证的经典 trick
- 32 字符 flag + 32 变量 = 单一线性方程组解（不需要 z3 非线性 solver）
- gogogoJNI 接口在 JNI 层是 wasm 字节加载的 indirection
prerequisites:
- wasm2c/wasm-decompile 工具使用
- Frida hook Module.findExportByName + readByteArray
- numpy/scipy 线性方程组求解 (linalg.solve)
- JEB/jadx + IDA Native 层静态分析
- WebAssembly 字节码格式 (.wasm magic 0x00 0x61 0x73 0x6D)
---
# CTF 丛林的秘密：算法分析
