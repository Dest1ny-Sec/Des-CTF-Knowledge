...
quality: high
full_path: 长安战疫-WriteUp.full.md
meta_path: 长安战疫-WriteUp.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: 长安战疫-WriteUp。长安战疫2022多方向:无参RCE array_rand+array_flip+array_values+get_defined_vars+eval+SSTI 8进制Unicode转义+凯撒D减3 A→X+Java字节77,68,65=MA+Python stegosaurus +Sage线性方程组+2^20 AES-ECB爆破+线程竞争XOR 10字符flag。经...
category: web
...
contest_type: open
wp_url: https://www.ctfiot.com/22102.html
wp_author: IntelliJ
reasoning_chain:
- URL ?code=eval(...)&a=system('cat flag.php') → 触发点：无参数 RCE
- 假设：array_rand(array_flip(current(array_values(get_defined_vars())))) → 随机取一个 var → 动作：构造 payload
- 观察：a=system('cat flag.php') 被 eval → flag{9a7f25…}
- F12 发现 ?name={{2*2}}.js → 触发点：SSTI Jinja2
- 假设：黑名单拦 __globals__ → 动作：8 进制 \u005f\u005f 转义
- 观察：lipsum|attr(u005f..u0067..)|attr(u005f..u0065..)(eval) → RCE
- 加密 'FDCB[8LDQ?ZLOO?…' → 假设：D-Z 减 3，A/B/C→X/Y/Z → 动作：凯撒偏移
- 动作：Java 字节流 77 68 65 77 78 → 拼 'MAWX…XT'
- Python .pyc 文件 → 假设：stegosaurus 隐写 → 动作：./stegosaurus -x steg.pyc
- Sage 3 未知数方程 → 动作：M=Matrix(R, [[d2,d3,d4]…]) solve_left → 观察：解
- AES-ECB → 假设：2^20 key 空间可爆破 → 动作：遍历 padding 校验
- f[i]^=i + f[i]^=f[i+1] 线程竞争 → 动作：num 共享变量间隔 0.5s 启动
failed_attempts:
- 试图用 <img onerror=...> SSTI → 失败：被黑名单拦
- 试图不解密直接读 .pyc → 失败：flag 藏在隐写段
key_observations:
- 无参 RCE 经典链：eval(array_rand(array_flip(current(array_values(get_defined_vars())))))
- SSTI \u005f\u005f 8 进制 Unicode 转义是绕黑名单通用手法
- 凯撒密码 A→X B→Y C→Z 是 ROT-3 移位
- Python stegosaurus 是 .pyc 隐写首选工具
- 线程竞争 + 共享 num 变量是经典 flag 提取套路
prerequisites:
- PHP get_defined_vars/array_* 函数链
- Jinja2 SSTI + Python 内置对象链
- 凯撒密码 / Java 字节 / AES-ECB
- Sage / Z3 线性代数求解
---
# 长安战疫-WriteUp
