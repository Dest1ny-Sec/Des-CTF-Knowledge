...
quality: high
full_path: 【卫星安全系列一】HAS-beckley赛题复现.full.md
meta_path: 【卫星安全系列一】HAS-beckley赛题复现.meta.md
images_removed: true
images_removed_count: 5
schema_version: v3.0.0-P0
summary: 【卫星安全系列一】HAS-beckley 赛题复现。HackASat 2020 资格赛 beckley 题复现，KML + Skyfield 库 + Google Earth 三件套算卫星拍摄角度。。经验：HackASat 是美国国防部太空 CTF；KML 是 Google Earth 数据格式；Skyfield 是 Pyt...
category: misc
...
contest_type: open
wp_url: https://www.ctfiot.com/148373.html
reasoning_chain:
- HackASat 2020 资格赛 beckley 题 → 触发点：'Fire up Google Earth and KML tutorials'
- 动作：编译 Dockerfile → docker run -p 19020:19020 beckley → nc 172.17.0.1 19020
- 服务端返回 'Washington Monument satellite image 2020-03-26 21:53:13' → 触发点：经纬度+时间+卫星名
- 动作：读 remote.kml → 假设：含 NetworkLink/LookAt 元素 → 观察：<longitude>/<latitude>/<altitude>/<range>/<heading>/<tilt>
- 触发点：<LookAt> 缺部分值 → 假设：用 Skyfield 库计算卫星拍摄角度补全
- 动作：pip install skyfield → 构造 Earth Satellite + Topos (观察者) → 算卫星 vs 观察者位置
- altitudeMode=clampToGround → 假设：altitude=0 忽略
- 动作：用 Skyfield + KML 已有的 longitude/latitude 计算 range/heading/tilt → 观察：完整 LookAt
- 提交答案到服务端 → 触发点：成功显示 flag
failed_attempts:
- 试图不读 KML 直接猜经纬度 → 失败：Washington Monument 坐标精度不够
- 试图只算 longitude/latitude 提交 → 失败：服务端校验 heading/tilt
- 试图用 Skyfield 但不构造 Topos → 失败：range 算成无穷远
key_observations:
- HackASat 是美国国防部太空 CTF (DoD + USSF 主办)
- KML 是 Google Earth 数据格式 (含 NetworkLink/LookAt/Link 等元素)
- Skyfield 是 Python 卫星轨道计算库 (含 TLE/Satellite/Observer)
- LookAt 元素指定 longitude/latitude/altitude/range/heading/tilt 完整相机参数
- 'altitudeMode: clampToGround 表示可忽略高度 (altitude=0)'
prerequisites:
- KML 格式 + Google Earth 使用
- Skyfield Python 卫星库
- TLE (卫星轨道根数) + 轨道力学基础
- Docker + socat + nc 启动 challenge
---
