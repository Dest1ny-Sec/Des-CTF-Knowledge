...
lesson: '|'
quality: high
full_path: L3HCTF_Writeup.full.md
meta_path: L3HCTF_Writeup.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: L3HCTF 2025 Writeup (XMCVE-Polaris 战队 第 9 名)。L3HCTF 2025 XMCVE-Polaris 战队的 vul2 / vul2_revenge 两道 libc 2.35 堆题，House of Apple 2 收尾。。经验：|
category: pwn
...
contest_type: open
wp_url: https://www.ctfiot.com/262264.html
reasoning_chain:
- vul2 / vul2_revenge：libc 2.35 堆题 → 触发点：tcache + safe linking
- 假设：先控制 tcache_perthread_struct → 动作：UAF 改 entries
- 假设：用 House of Apple 2 → 动作：构造 fake_io 链
- 观察：fake_io.vtable = _IO_wfile_jumps → _IO_wfile_overflow → system
- 动作：触发 exit / FSOP → 完成
- vul2_revenge：更严约束 → 假设：需要更大写原语
- 动作：tcache poisoning → 改 free_hook 等价路径（FSOP）→ 完成
failed_attempts:
- vul2 直接 tcache poisoning → 失败：safe linking 干扰
- vul2_revenge 试 House of Botcake → 失败：libc 2.35 防御增强
- 试 __free_hook 覆盖 → 失败：2.35 已移除
key_observations:
- House of Apple 2 是 libc 2.35 FSOP 标准链
- fake_io 必须满足 _IO_wfile_jumps 的 vtable check
- safe linking 是 libc 2.32+ tcache fd 指针随机化
- vul2_revenge 强调'无 leak' 必须 FSOP / House of Apple 路径
- FSOP 是 glibc 2.34+ 收尾的必备手法
prerequisites:
- glibc 2.35 heap 数据结构 + House of Apple 2 (fake_io)
- tcache safe linking + FSOP (file stream oriented programming)
