...
quality: high
full_path: 【二等奖方案】基于TPU平台实现人群密度估计赛题「深藏blue」团队解题思路.full.md
meta_path: 【二等奖方案】基于TPU平台实现人群密度估计赛题「深藏blue」团队解题思路.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: 基于 TPU 平台实现人群密度估计 CCF BDCI 2022 二等奖方案。2022 CCF BDCI TPU 人群密度估计：4 模型网格搜索 + INT8 量化对比 + 选 VGG FP32 144×192 拿 B 榜第二。。关键路径：4 个预训练模型 CANNet/CSRNet/MCNN/VGG，FP32 vs INT8 BModel → 测试 FP32 vs INT8 在 (576,7...
category: misc
...
contest_type: open
wp_url: https://www.ctfiot.com/123438.html
reasoning_chain:
- 赛题：TPU 平台人群密度估计 4 模型对比 → 触发点：FP32 vs INT8 BModel
- CANNet 权重加载失败 → 跳过 → 假设：剩下 CSRNet/MCNN/VGG 三选一
- 动作：FP32 + INT8 在 576×768~2048×2048 输入对比 → 观察：VGG 综合分 186.74/177.64
- 假设：VGG > MCNN > CSRNet → 动作：选 VGG 作为推理 backbone
- 网格搜索超参 → 触发点：6 种模型输入 × 5 种 Batch × 5 种缩放 = 150 组
- 动作：模型输入 [112×112, 144×144, 144×192, 224×224, 288×384, 576×768] → 观察：144×192 综合最优
- Batch Size = [1, 2, 4, 8, 16] → 假设：Batch 推理耗时近线性 → 观察：Batch=1 时综合分最高
- 图片缩放 [224, 768], [288, 1024]...5 种 → 假设：大图分块推理避免下采样丢特征
- 动作：选 (288×384, 768×768) 缩放 → 观察：精度+速度平衡最优
- INT8 量化 (200 张训练图 lmdb 校准) → 假设：精度↑ → 观察：分数反而低 → 放弃
- '最终 FP32 BModel + 144×192 + Batch=1 + (288×384, 768×768) → B 榜 #2'
failed_attempts:
- 试图选 CSRNet 综合分 68.34 → 失败：比 VGG/MCNN 低太多
- 试图 INT8 量化 (lmdb 200 张校准) → 失败：精度敏感任务 INT8 反而掉分
- 试图用 Batch=4/8/16 → 失败：MAE 几乎不变但速度分降低
key_observations:
- 网格搜索 (Grid Search) 是模型选择标准方法，6×5×5 = 150 组很快跑完
- FP32 vs INT8 量化在精度敏感任务中 INT8 不一定更好 (与 ChaMd5-AI 共识)
- Batch Size 对单 batch 推理耗时近线性，B=1 是 TPU 推理最佳
- 大图分块推理 (288×384, 768×768) 避免下采样特征丢失
- VGG 在 TPU 上的综合分 (186.74) 显著高于 MCNN/CSRNet (169.76/68.34)
prerequisites:
- TPU 推理原理 (FP32/INT8 BModel)
- 网格搜索 (Grid Search) 超参调优
- 人群密度估计指标 (MAE/RMSE/NAE)
- 图片缩放与下采样权衡
---
