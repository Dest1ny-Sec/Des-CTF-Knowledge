...
quality: high
full_path: 【Hack_The_Box】Sandworm【Writeup】.full.md
meta_path: 【Hack_The_Box】Sandworm【Writeup】.meta.md
images_removed: true
images_removed_count: 0
schema_version: v3.0.0-P0
summary: 【Hack The Box】Sandworm【Writeup】。Hack The Box Sandworm 中等题，Flask Web + JWT 伪造 + RADIUS 协议深度利用链。。经验：dirsearch 找 admin/login/logout 路由；ffuf 高并发子目录爆破；Flask + PyJW...
category: web
...
wp_url: https://www.ctfiot.com/146323.html
reasoning_chain:
- nmap 扫 22/80/443 → /etc/hosts 加 ssa.htb → 触发点：Flask Web 应用
- dirsearch -u https://ssa.htb/ 跑出 /admin /login /logout /guide → 假设：Flask-login 后端
- ffuf -t 150 子目录暴破 → /guide 下发现 PDF/页面 → 动作：看页面源码
- Flask 会话 JWT 酒店 → 假设：CVE-2023-25577 长度扩展 → 动作：伪造 alg=HS256 不验签
- 拿到 RADIUS secret → 假设：可发 RADIUS UDP 1812 包 → 动作：写 Python RADIUS client
- 构造 crafted Access-Request → 提权完成 → 观察：get root
failed_attempts:
- 试图 ffuf 全站子目录 → 太慢，限定 /guide 子树
- 试图 JWT alg=none → 失败：PyJWT 默认拒绝
- 试图 RADIUS 直接连 1812 → 需 secret 才能认证
key_observations:
- CVE-2023-25577 PyJWT 长度扩展是 Flask+JWT 经典 CVE
- RADIUS UDP 1812 协议 secret 鉴权，craft packet 是核心
- dirsearch+ffuf 组合 = HTB Web 题标准扫描流
- JWT 密钥破裂/secrets 泄漏是 SSTI 后常见下一步
prerequisites:
- dirsearch/ffuf 目录扫描
- PyJWT 长度扩展攻击原理
- RADIUS 协议 UDP 报文格式
