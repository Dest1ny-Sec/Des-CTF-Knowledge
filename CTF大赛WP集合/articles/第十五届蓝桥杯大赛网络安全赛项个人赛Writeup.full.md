...
quality: high
full_path: 第十五届蓝桥杯大赛网络安全赛项个人赛Writeup.full.md
meta_path: 第十五届蓝桥杯大赛网络安全赛项个人赛Writeup.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: 第十五届蓝桥杯大赛网络安全赛项个人赛Writeup。第十五届蓝桥杯网络安全7题:robots.txt+流量包追踪+DWT水印+Arnold逆+yafu分解RSA(差206)+魔改XXTEA(rounds=(415/n)+114)+RC4(gamelab@)+UAF(tcache+fastbin+free_hook)+ECDSA同k攻击。经验：1) robots.txt:3个Disallow路...
category: web
...
contest_type: school
wp_url: https://www.ctfiot.com/176353.html
reasoning_chain:
- 爬虫协议题触发点：robots.txt 提示 → 假设：直接访问 /robots.txt
- 动作：访问 robots.txt 第三路径 → 观察：得到 flag
- packet 题触发点：流量包 → 假设：追踪流找加密 flag
- 动作：wireshark 追踪 TCP 流 → 观察：解码 flag{7d6f17a4...}
- 缺失的数据题触发点：压缩包密码 → 假设：pavilion 是密码
- 动作：套字典爆破 zip 密码 → 观察：解出 a.png
- 假设：DWT 水印 + Arnold 变换 → 动作：仿写 deArnold 函数解水印
- 魔改 XXTEA：触发点：rounds=(415/n)+114 → 假设：可爆破 n 还原
- RSA yafu 分解：触发点：n 差 206 位 → 假设：yafu 自动分解
failed_attempts:
- robots.txt 直接访问根路径 → 失败：需 /robots.txt
- packet 找明文字符串 → 失败：flag 已加密
- DWT 水印不 deArnold → 失败：水印图错位
key_observations:
- robots.txt 是 robots 协议（爬虫约定）必查文件
- DWT 水印 + Arnold 变换是图片隐写经典组合
- 魔改 XXTEA 关键：rounds 是 flag 长度函数（rounds=(415/n)+114）
- yafu 是 CTF RSA 分解标配工具（差几位时尤其有效）
prerequisites:
- robots.txt 协议
- wireshark 流量追踪
- PyWavelets DWT 变换
