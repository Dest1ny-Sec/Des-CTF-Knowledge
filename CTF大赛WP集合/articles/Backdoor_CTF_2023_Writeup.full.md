...
- jadx smali
- open_sesame MainActivity
- it4chi n4ut1lus sh4dy sl4y3r
- valid_password 7字节
- XOR循环校验
- Character.isDigit
- sl4y3r
...
quality: medium
full_path: Backdoor_CTF_2023_Writeup.full.md
meta_path: Backdoor_CTF_2023_Writeup.meta.md
images_removed: true
images_removed_count: 0
schema_version: v3.0.0-P0
summary: Backdoor CTF 2023 Writeup。Backdoor CTF 2023 — exiftool Artist 字段直接读 flag + open_sesame APK smali 分析 it4chi/n4ut1lus/sh4dy/sl4y3r/flag 函数 + valid_password 0x34 0x6c 0x31 0x62 0x61 0x62 0x61 = "4l1b...
category: misc
...
contest_type: open
wp_url: https://www.ctfiot.com/152665.html
reasoning_chain:
- '[触发点] friend.jpeg EXIF → 假设：Artist 字段藏 flag / [动作] exiftool friend.jpeg / [观察] Artist: flag{7h3_r34l_ctf_15_7h3_fr13nd5_w3_m4k3_al0ng} / [下一步] 看 open_sesame.apk'
- '[触发点] open_sesame.apk jadx 反编译 smali → 假设：valid_password 数组硬编码 / [动作] jadx 看 MainActivity.valid_password / [观察] 数组 = [0x34, 0x6c, 0x31, 0x62, 0x61, 0x62, 0x61] = "4l1baba" / [下一步] 看混淆函数'
- '[触发点] 函数名混淆 it4chi / n4ut1lus / sh4dy / sl4y3r / flag → 假设：海龟汤四元素 / [动作] 分析 smali 每个函数 / [观察] it4chi char→int[] / n4ut1lus 比较 int[] / sh4dy 保留数字 (Character.isDigit) / sl4y3r 字符串处理 / flag charAt XOR / [下一步] 登录'
- '[触发点] 登录输入 user + 密码 → 假设：触发 flag 输出 / [动作] 输入 user + 4l1baba 登录 / [观察] 拿到 flag / [下一步]'
failed_attempts:
- 试图 jadx 反编译找 flag 字符串 → 失败：flag 是运行期 XOR 生成
- 试图绕过 valid_password 直接登录 → 失败：必须输入 4l1baba
key_observations:
- JPEG EXIF Artist 字段常被用来藏 flag
- smali 读 valid_password 数组转 ASCII 是 APK 入门基础
- 函数名混淆 it4chi/n4ut1lus/sh4dy/sl4y3r 提示四元素（海龟汤）
- Character.isDigit 保留数字是常见 APK 反混淆步骤
prerequisites:
- EXIF 元数据读取（exiftool）
