...
quality: high
full_path: 看雪·众安_2021_KCTF_秋季赛_-_第五题设计思路及解析.full.md
meta_path: 看雪·众安_2021_KCTF_秋季赛_-_第五题设计思路及解析.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: 看雪·众安 2021 KCTF 秋季赛 | 第五题设计思路及解析。看雪众安KCTF2021秋Q5《拔刀相向》数据流混淆：DARTS并行+Codelets打断控制流+多项式商环RSA+Feistel。经验：数据流混淆（Dataflow flattening）+ DARTS并行codelet调度干扰控制流分析；多项式商环RSA...
category: misc
...
contest_type: open
wp_url: https://www.ctfiot.com/14096.html
reasoning_chain:
- DARTS运行时+Codelets细粒度线程 → 触发点：多线程调度打断控制流
- 假设：传统断点失效，因内存中无严格算法中间状态 → 动作：理解blockHasher fire()
- 观察：blockHasher=xorshift+0x2545F4914F6CDD1D PRF → 假设：xorshift变体可分析
- PolyRSA(GF(61)模多项式t^24+51*t^21+...+t+38) → 假设：商环RSA需Sage分解
- 动作：SageMath分解多项式得(t^11+...)*(t^13+...) → phi=(61^11-1)*(61^13-1)
- 16轮6路Feistel网络（Luby-Rackoff PRF→PRP） → 假设：可逆
- 反演：分解多项式求phi → 逆PolyRSA → 反Feistel → Username=KCTF对应SN
- 观察：SN=4E6B0C83AA2E6B6E946477B2149A8816D730 → flag
failed_attempts:
- 试图用传统断点跟Feistel → 失败：DARTS并行调度，内存无中间状态
- 试图不分解多项式直接求逆 → 失败：模多项式不可整除
- 忽略Luby-Rackoff假设 → 失败：Feistel需知道轮数才能逆
key_observations:
- DARTS Codelets并行调度是数据流混淆前沿方向
- 多项式商环RSA = 在GF(p)[x]/(f(x))上做RSA，需Sage分解多项式
- Luby-Rackoff定理：PRF经3+轮Feistel转PRP
- xorshift PRF (mul 0x2545F4914F6CDD1D) 是高质量随机源
- DARTS运行时内存中无严格中间状态=调试失效
prerequisites:
- DARTS Codelets数据流编程模型
- SageMath多项式分解（factor）
- Feistel网络结构与Luby-Rackoff定理
- xorshift PRF原理
---
