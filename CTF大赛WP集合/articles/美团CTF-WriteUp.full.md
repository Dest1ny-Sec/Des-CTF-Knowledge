...
quality: high
full_path: 美团CTF-WriteUp.full.md
meta_path: 美团CTF-WriteUp.meta.md
images_removed: true
images_removed_count: 7
schema_version: v3.0.0-P0
summary: 美团CTF-WriteUp。美团CTF WriteUp,涵盖ChaMd5 Venom招新文+PHP OOP反序列化(Users/User/Welcome/File/Logs+call_user_func+phar触发)+时间盲注+RSA低位爆破+ret2libc babyrop。。关键路径：Web PHP反序列化:abstract Users {db, verify_user, check_...
category: pwn
...
contest_type: open
wp_url: https://www.ctfiot.com/16962.html
reasoning_chain:
- 触发点：Users/User/Welcome/File/Logs 类 + abstract eval → 假设：POP 链触发
- 动作：构造 User.func=call_user_func + param=[file, 'php://filter/write=...', contents] → 观察：file_put_contents 触发
- 动作：phar:// 触发反序列化 → 假设：phar 元数据 unserialize → 观察：触发 call_user_func
- 触发点：Users abstract eval + User extends eval → 假设：实例化 User 时触发 test() → eval()
- 动作：构造 User.func='system', param=['cat /flag'] → 观察：RCE
- 时间盲注：触发点：username 与数据库有 password 比较 → 假设：sleep 注入
- 动作：username='admin' AND IF(SUBSTR(password,1,1)='a',SLEEP(5),0) → 观察：时间盲注
- call_user_func 触发点：抽象类 Users.eval 强制调用 → 假设：通过 test() 链触发
failed_attempts:
- 直接 User('system', 'ls') → 失败：eval 内要求 is_array
- phar://file.jpg 反序列化 → 失败：未触发
- 时间盲注直接 union → 失败：union 被过滤
key_observations:
- abstract class eval 是 PHP 反序列化经典 gadget
- phar:// 元数据反序列化是文件操作触发点（unlink/file_exists 都可触发）
- call_user_func + 数组参数是函数注入关键
- 时间盲注 if+sleep() 是经典 SQL 注入手法
prerequisites:
- PHP OOP 抽象类与继承
- phar:// 反序列化触发原理
- 时间盲注 SQL 注入
