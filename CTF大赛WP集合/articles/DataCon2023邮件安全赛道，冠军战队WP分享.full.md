...
quality: high
full_path: DataCon2023邮件安全赛道，冠军战队WP分享.full.md
meta_path: DataCon2023邮件安全赛道，冠军战队WP分享.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: DataCon2023邮件安全赛道，冠军战队WP分享。DataCon2023邮件安全：4类邮件分类+钓鱼特征工程+邮件炸弹攻击。关键路径：邮件分类4类：正常/垃圾/钓鱼/误报 → 训练集4277封含垃圾+钓鱼各半 → 6大主题分类：金钱补助/软件更新/科研培训/广告营销/色情/开票开户。经验：钓鱼邮件检测特征工程：关键词黑白名单+URL白名单+附件+内联图
category: misc
...
contest_type: open
wp_url: https://www.ctfiot.com/149890.html
reasoning_chain:
- 邮件 eml 文件 → 触发点：4 分类（正常/垃圾/钓鱼/误报）
- 假设：训练集 4277 封特征工程而非深度学习 → 动作：6 大主题关键词黑名单
- 金钱补助/软件更新/科研培训/广告营销/色情/开票开户 → 动作：关键词黑名单+通配符（截断/谐音/近形）
- 假设：URL 在 Tranco TOP1M 是合法 → 动作：URL 白名单过滤
- 观察：内联图片无文本 = 钓鱼 → 假设：附件合法后缀白名单
- 邮件炸弹：2 小时大量发送 → 假设：用正常邮件服务（订阅/验证码）绕过反垃圾 → 动作：寻找多个免费发件服务并行调用
failed_attempts:
- 试图用 BERT 分类器直接训 → 失败：样本量 4277 偏小，特征工程更可控
- 试图靠 SPF/DKIM 拦截炸弹 → 失败：题目要求用合法服务发
key_observations:
- 钓鱼邮件检测 = 关键词黑白名单 + URL 白名单 + 附件白名单 + 内联图片特征
- 中间截断/谐音/近形字是黑名单必须考虑的绕过
- 邮件炸弹突破思路 = 大量正常发件服务并行触发
prerequisites:
- Tranco TOP1M 域名白名单
- 邮件协议基础（SMTP/POP3/IMAP）
- 关键词特征工程（截断/谐音/近形）
- 免费邮件服务 API 滥用
---
