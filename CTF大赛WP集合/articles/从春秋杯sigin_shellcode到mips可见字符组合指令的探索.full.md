...
quality: medium
full_path: 从春秋杯sigin_shellcode到mips可见字符组合指令的探索.full.md
meta_path: 从春秋杯sigin_shellcode到mips可见字符组合指令的探索.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: '从春秋杯 sigin_shellcode 到 mips 可见字符组合指令的探索。春秋杯 sigin_shellcode：探索 MIPS 可见字符组合指令，R 型 opcode=0 不可用 + J 型只有 jalx + 没有 syscall。。关键路径：MIPS 32 寄存器 ($0-$31) + HI/LO 特殊寄存器 → 指令 32 位: R 型 (opcode=0) + I 型 (opc...'
category: pwn
...
contest_type: open
wp_url: https://www.ctfiot.com/119807.html
reasoning_chain:
- 题目 sigin_shellcode 探索 MIPS 可见字符 shellcode → 触发点：32 位指令+可见字符 [0x21,0x7e]
- MIPS 32 寄存器 $0-$31 + HI/LO 特殊寄存器 → 指令 32 位 R/I/J 型
- R 型 opcode=0 必须为 0 → 不可见字符 → 不可用
- J 型 fuzz 只剩 jalx (但需 microMIPS/MIPS16e 支持) → 大多 MIPS 不支持
- 'I 型可用: beqzl/beqz/bnezl/bnel/blezl/bgtzl/andi/slti → 但没有 syscall 指令'
- 假设：MIPS 可见字符无法构造 syscall → 异构 pwn 题设计思路
- 动作：opcode fuzz 全枚举可见字符组合 → 结论：通用 MIPS 可见字符 shellcode 不存在
failed_attempts:
- 试图用 R 型指令的可见字符组合 → 失败：opcode 必须为 0x00
- 试图用 J 型 jalx → 失败：通用 MIPS 不支持 microMIPS
- 试图构造 syscall 指令 → 失败：syscall 操作码超出可见字符范围
key_observations:
- 通用 MIPS 可见字符 shellcode 几乎不可能（无 syscall 指令）
- 异构 pwn 题设计思路：出题人探索不可解题
- R/I/J 型指令分布 + opcode 限制分析是 ISA 逆向基础
- MIPS 寄存器别名（$sp/$gp/$at）与 x86 大不相同
prerequisites:
- MIPS 指令集（R/I/J 型 opcode 分布）
- MIPS 汇编（寄存器别名）
- 可见字符 shellcode 约束构造
- microMIPS/MIPS16e 扩展指令集
---
