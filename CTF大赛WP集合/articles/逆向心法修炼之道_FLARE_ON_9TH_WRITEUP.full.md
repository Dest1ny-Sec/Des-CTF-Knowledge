...
quality: high
full_path: 逆向心法修炼之道_FLARE_ON_9TH_WRITEUP.full.md
meta_path: 逆向心法修炼之道_FLARE_ON_9TH_WRITEUP.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: 逆向心法修炼之道 FLARE ON 9TH WRITEUP。FlareOn 9th (2022)全11题WP,涵盖Flaredle硬编码/Pixel Poker坐标/Magic 8 Ball方向键/darn_mice单字节ret反推/T8满月+srand+RC4+月相计算+àla mode mixed-mode PE+pipe RPC+password MyV0ic3!。。关键路径：第1题F...
category: reverse
subcategory: reverse
time_required: long
difficulty_score: 4
...
contest_type: open
wp_url: https://www.ctfiot.com/74040.html
reasoning_chain:
- Flaredle (T1)：word.js 硬编码值对比 → 触发点：JavaScript 静态字谜
- Pixel Poker (T2)：像素坐标 x == 0x52414C46%0x2E5=95, y == 0x6E4F2D45%0x281=313
- Magic 8 Ball (T3)：输入 gimme flag pls? + L L U R U L D U L 方向键序列
- darn_mice (T4)：str[i]+v5[i]=单字节 ret (0xC3) → 反推输入 see three, C3 C3 C3 C3 C3 C3 C3! XD
- T8 srand(本小时毫秒数) + rc4(FO9+randnum) → POST flare-on.com → 触发点：弱 PRNG + 网络
- T8 攻击：枚举 srand seed (14:14:36 前后) + 遍历 65535 个 md5 + 找出 rc4 key
- T8 解密：14 组 _SYSTEMTIME + 月相值索引 alphabet 表 = flag 前缀 i_**
- à la mode (T6)：capa v4.0 识别 mixed mode + floss-floss 找 xor 解密后的 \\pipe\\FlareOn
- DllEntryPoint 定位 0x1000181A + native code xor 动态解密函数符号 + .pipe.FlareOn RPC 通信
- 预置密码 MyV0ic3! + 0x10001187 解密 flag
failed_attempts:
- 试图不识别 mixed mode 直接 IDA 反编译 à la mode → 失败：.NET 伪代码看不到 native code
- T8 不枚举时间戳直接猜种子 → 失败：毫秒级种子太精确
- darn_mice 直接看伪代码找输入 → 失败：ret 字节反推需理解加密机制
key_observations:
- FlareOn 是 Windows 逆向顶级比赛，Mandiant 出题质量高
- mixed-mode PE（.NET + native）是复杂逆向场景，capa+floss+IDA 联合分析是标配
- 满月+日期穿越是 T8 的特色（SYSTEMTIME 月相计算）
- 像素坐标取模 = 硬编码坐标是简单但巧妙的出题思路
prerequisites:
- Windows PE/.NET 程序结构（mixed mode）
- capa / floss 静态分析工具
- IDA Pro 高级用法（native code 跟踪）
- RC4 / PRNG 种子爆破
---
