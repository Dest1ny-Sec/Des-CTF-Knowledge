...
- resource
attack_chain:
- 'Q1 checkin: 调 get_flag entry 函数即得 flag'
- aptos move run --function-id 0x3dd3f092...::checkin::get_flag
- 'Q2 hello_move: 三关'
- (1) init_challenge 拿 Challenge 资源
- (2) hash 提交 [103,111,111,100] = 'good
- '(3) discrete_log: 计算 3123592912467026955 的对数'
- (4) add(2u8, 0u8) 让 res.balance < Initialize_balance
- (5) get_flag 拿 flag
- Move script 调 init_challenge（不是 entry 需 script 包装）
...
quality: high
full_path: 2022MOVEment_Aptos_writeup_by_ChaMd5.full.md
meta_path: 2022MOVEment_Aptos_writeup_by_ChaMd5.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: '2022 MOVEment Aptos writeup by ChaMd5。Aptos Move 智能合约 checkin + 三关 hello_move。关键路径：Q1 checkin: 调 get_flag entry 函数即得 flag → aptos move run --function-id 0x3dd3f092...::checkin::get_flag → Q2 hello_...'
category: web
subcategory: logic
subcategories:
- logic
...
contest_type: open
wp_url: https://www.ctfiot.com/86671.html
wp_author: ChaMd5
reasoning_chain:
- '[触发点] Q1 checkin entry get_flag 函数 → 假设：直接调 entry / [动作] aptos move run --function-id 0x3dd3f092...::checkin::get_flag / [观察] 拿到 flag / [下一步] hello_move'
- '[触发点] Q2 hello_move 三关：init_challenge → hash → discrete_log → add → get_flag / [假设] 5 步顺序触发 → [动作] init + hash([103,111,111,100])=good + discrete_log + add(2u8,0u8) → [观察] balance< init → get_flag / [下一步] Q3'
- '[触发点] Q3 需 script 包装 init_challenge → 假设：非 entry 需 script / [动作] 写 move script / [观察] flag'
failed_attempts:
- 试图直接调 init_challenge → 失败：非 entry 需 script 包装
- 试图猜 discrete_log → 失败：必须算130-bit 大数对数
key_observations:
- Move 资源 + 入口函数 + script 调 entry-less 函数
- discrete_log 在 u128 大数用 Sage 内置函数
- hash(4 字节) = 字符串前缀确认
prerequisites:
- Aptos Move 智能合约语法
- entry vs non-entry 函数区别
- Sage discrete_log 函数使用
---
