...
quality: high
full_path: 2024国城杯_writeup_by_Mini-Venom.full.md
meta_path: 2024国城杯_writeup_by_Mini-Venom.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: 2024 国城杯 writeup by Mini-Venom。UDP .264 视频 + seed 爆破乱序图 + PGP 词表 + AES-ECB。。经验：PGP 词表每个 2 字母/词对应 0x00-0xFF 字节；np.random.shuffle 行列只需 argsor...
category: misc
subcategory: stego
subcategories:
- stego
...
contest_type: open
wp_url: https://www.ctfiot.com/219184.html
wp_author: Mini-Venom
reasoning_chain:
- 触发点：password.pacpng 流量 → 假设：含 UDP 流 → 动作：wireshark 追踪 UDP 流
- 动作：hex 复制原始数据 → Cyberchef 转 → 改后缀 .264 → 动作：VLC 打开 → 观察：拿到 ZIP 密码 !t15tH3^pAs5W#RD*f0RFL@9
- 触发点：解压得乱序图 + 加密脚本 → 假设：np.random.shuffle(seed 范围) → 动作：写解码脚本遍历 seed
- 观察：seed=某值时图像还原 → DataMatrix 二维码 → 动作：在线扫码 → 拿到密文
- 触发点：密文是 PGP wordlist → 假设：PGP 词表 256 词对应 0x00-0xFF 字节 → 动作：词→字节映射
- 观察：每个词对应 2 字节 → 还原原文 → 拿到 AES key 'oursecret is D0g3xGC'
- 触发点：AES-ECB + key='oursecret is D0g3xGC' → 假设：直接解密 → 动作：pycryptodome
- 观察：拿到 flag
- 触发点：Just_F0r3n51Cs 流量包导出图 → 动作：拼图还原 → base64 → 观察：拿到 AES key
- 下一步：AES-ECB 解密得 flag
failed_attempts:
- 试图用普通种子文件还原 np.random.shuffle → 失败：必须从图像视觉特征反推 seed
- 试图 PGP wordlist 词→数字表硬编 → 失败：词表是字典 256 词对应 1 字节
- 试图 AES-ECB 不用 key 直接解 → 失败：key 是词表派生必须先还原
key_observations:
- PGP wordlist 每个词对应 0x00-0xFF 字节（256 词一字节）
- np.random.shuffle 行列只需 argsort 逆变换还原
- AES-ECB key='oursecret is D0g3xGC' 是题目提示明文（典型脑洞题）
- UDP 流 hex 改后缀 .264 是视频取证隐写套路（VLC 兼容 .h264/.264 裸流）
prerequisites:
- VLC 视频播放 + .264 裸流识别
- numpy argsort + np.random.shuffle 还原
- PGP wordlist 词表查表（256 词 1 字节）
- AES-ECB / Cyberchef 基础
---
