...
lesson: '|'
quality: high
full_path: Linux新版内核下内存取证分析附CTF题.full.md
meta_path: Linux新版内核下内存取证分析附CTF题.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: Linux 新版内核下内存取证分析附 CTF 题。Linux 5.15 新版内核的 Volatility2/3 profile 生成方法 (docker + dwarf2json) + bip39 钱包 mnemonic 解密 CTF 题。。经验：|
category: forensic
...
contest_type: open
wp_url: https://www.ctfiot.com/60876.html
reasoning_chain:
- 题目：Ubuntu 22.04 + Linux 5.15 内核内存镜像 → 触发点：Volatility profile 不支持
- 假设：必须自定义 profile → 动作：用 dwarf2json + docker 制作 profile
- 观察：dwarf2json file /usr/lib/debug/boot/vmlinux-5.15...-dbgsym → 生成 profile.zip
- 动作：vol.py --plugins=/usr/lib/python3/dist-packages/volatility/plugins/ --profile=LinuxUbuntu22045x64 -f mem.dmp imageinfo
- 观察：成功识别 → 下一步：pslist / bash_history / filescan
- 假设：题目钱包 mnemonic 在内存 → 动作：grep 关键词
- 动作：memdump -p PID -D dump/ → strings 找 bip39 单词 → 12 个 bip39 单词 → 完成 flag
failed_attempts:
- 用现成 Ubuntu 22.04 profile → 失败：内核版本不一致
- volatility3 直接用 banners → 失败：识别不到
- 试 strings 找 flag → 失败：flag 由 bip39 单词组成
key_observations:
- dwarf2json + docker 是生成 Volatility 2 Linux profile 的标准流程
- Volatility 3 用 symbols 直接识别但 Linux 支持差
- bip39 钱包 mnemonic 在内存中是明文可 grep
- Ubuntu 22.04 默认内核 5.15 是新版典型
- memdump -p PID → strings 是内存取证通用技巧
prerequisites:
- Volatility 2 / 3 区别 + dwarf2json 工具
- Linux kernel symbols 提取 + 内存镜像分析（pslist / bash / filescan）
- bip39 钱包原理
---
