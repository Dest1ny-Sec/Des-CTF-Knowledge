...
quality: high
full_path: UTCTF_·_2025_WriteUp.full.md
meta_path: UTCTF_·_2025_WriteUp.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: UTCTF 2025 WriteUp (狼组)。狼组出品的 UTCTF 2025 全 11 题解，覆盖 Web/Game/Pwn/Misc/Reverse/Crypto 全部题型，详细到脚本可直接复用。。经验："鸵鸟算法"在 CTF 里指绕过潜在复杂检查的策略；很多题目 reverse 关键不是"逆向"而是"识别它不检...
category: web
...
contest_type: open
wp_url: https://www.ctfiot.com/232193.html
reasoning_chain:
- OTP 触发点：注册 username + lookup secret 差值最小 → 假设：可爆破 32 位 flag（payload 在响应差中）
- Number Champion：lat/lon 二分定位 → geopy 训练地址 → 3000 elo → 触发点：游戏 ELO 比分系统伪造
- secbof：ret2libc 64-bit ROP 链 → read(0, bss+0x200, 0x100) → open(bss+0x200) → read→bss+0x300 → write(1, bss+0x300, 0x100) → 假设：标准 open-read-write shellcode-like 链
- Trapped 1/2：ssh 进去 flag 启 ACL 扩展 → secretuser 用户读 → 假设：sudo ACL 提权
- Streamified：25x25 bitstring 还原 QR → 假设：bit string 直接重组为黑白方块
- Ostrich：鸵鸟算法题，跳过开环循环即可得 → 观察：代码里 while/for 循环其实没用
- Retro Cookie Clicker：CE 搜索单字节找 Dozen=0x7fff → 假设：内存修改经典
- Safe Word：malloc 巨型内存初始化 + char-pick VM 解释器 + 0xC358XX6A 验证 → 假设：解释器字节码模式识别
- DCΔ：n=p² → e=3 小明文攻击 → iroot → m = c^(1/3)
- Autokey：已知 flag 头 utflag{ → 假设：可逐字符推 key → 解密
failed_attempts:
- OTP 试图跑全部 username 拼访问 → 失败：必须先注册再差值爆破 32 位 flag
- Safe Word 试图逐指令 trace 解释器 → 失败：选用的字节码 0xC358 是 EAX 模式常量，先识别 VM 类型再逆
- Ostrich 试图分析每个循环结构 → 失败：鸵鸟算法直接跳过即得，是 reverse'不检查什么'思维
key_observations:
- 鸵鸟算法在 CTF 里指绕过潜在复杂检查的策略；很多题 reverse 关键不是'逆向'而是'识别它不检查什么'
- open-read-write ROP 链是 64-bit 文件读取 shellcode-like 模板
- 25x25 bitstring 是 QR 码 21x21 起步最小版本外的扩展格式
- Safe Word VM 解释器选字节码 0xC358XX6A 是典型的 char-pick opcode 模式
- Autokey 已知明文头逐字符推 key 是维吉尼亚变种基础攻击
prerequisites:
- ROP 链构造（read/open/write syscall 序列）
- Autokey cipher 数学基础
- RSA e=3 小明文攻击（iroot）
- CheatEngine 内存搜索入门
- 游戏 ELO 评分系统伪造
