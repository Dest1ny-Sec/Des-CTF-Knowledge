...
quality: high
full_path: UIUCTF_2024_Writeup.full.md
meta_path: UIUCTF_2024_Writeup.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: 'UIUCTF 2024 Writeup - 狼组。UIUCTF 2024 狼组：MD5 binary SQL 注入 + syscall sandbox 盲注 + z3 矩阵 + OSINT 系列。。关键路径：Fare Evasion (Web): SQLite 注入 + MD5 binary 注入 → todo: convert md5 to hex string instead of la...'
category: web
...
contest_type: open
wp_url: https://www.ctfiot.com/191124.html
wp_author: SIGPwny
reasoning_chain:
- Fare Evasion → MD5 函数未 hex 编码直接返回 bytes → 触发点：md5 digest 是 16 字节二进制
- 假设：latin1 编码 MD5 字节可能含 'or' SQL 闭合字符 → 动作：枚举 5/6 字符字典
- 观察：爆破 xdax82'or'1+xd7Vpx1bwxd7.xb6 触发 SQL 注入
- Syscalls → shellcode 限制只能用 openat+mmap 不能 write → 触发点：sandbox 限制系统调用
- 假设：侧信道爆破 → 动作：mov dl, [rdi+pos]; cmp cl, char; jz loop; mov al, 60
- 观察：按位爆破 uiuctf{a53aaf9aaed1fa5906de364a1162e0833c57a0246ab9ffc}
- OSINT 系列 → 3 题 → 触发点：地铁线路 + Instagram + 经纬度坐标
- 假设：Massachusetts Ave Bridge + 经纬度 → 动作：Google Maps + Wikipedia 反查 → 观察：得出 Arlington Street / Dallas 铁路交叉点
- Summarize → 6 个 9 位数 → 假设：z3 求解线性约束 → 动作：z3.Solver + Int() → 观察：还原
- Determined → 矩阵推理 → 假设：Artin 代数引用 → 动作：手算特征方程
failed_attempts:
- 试图用 hex(md5) 字符串注入 → 失败：题目提示 md5 直接是 binary 而非 hex
- 试图用普通 write 系统调用读 flag → 失败：seccomp 沙箱禁用
key_observations:
- MD5 digest 是 16 字节 binary 不是 32 字符 hex 是关键
- seccomp 禁用 write 但允许 openat+mmap → 必须用 side-channel
- OSINT 题目本质是 Google Maps + Wikipedia 跨源关联
- z3 是 9 位数求解的轻量级工具
prerequisites:
- SQL 注入中 latin1 编码与 hex 编码差异
- seccomp 系统调用过滤绕过
- z3 求解器基础（Int / Solver）
- OSINT 工具链（Google Maps / Wikipedia / GeoNames）
---
# UIUCTF 2024 Writeup
