...
- heap-overflow
attack_chain:
- 'ezjava: 扫 /actuator/heapdump 拿 redis 密码 → jodd-http SSRF → redis SLAVEOF + MODULE LOAD 写 /tmp/exp.so 反弹'
- 'newweb: nghttp HTTP/2 → Jinja2 __class__.__base__.__subclasses__() 走 attr 取 __globals__[''__builtins__''].eval'
- 'safe_script: sessid=php://filter/convert.iconv.utf-7.utf-8 + 一句话木马 php://filter 链'
- 'Esc@pE_ASt_Reverge_d: Misc 隐写 / 还原'
- 'tomic: qsieve 分解 n → p+q；GCD(n,e) 求共享因子 p'
- 'gonote: libc-2.27 负 size 整数溢出 tcache 越界 → leak libc → __free_hook = system → /bin/sh'
...
quality: high
full_path: 2022_DSCTF决赛wp.full.md
meta_path: 2022_DSCTF决赛wp.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: '2022 DSCTF 决赛 WP。6 题综合：Spring Boot SSRF + Jinja2 SSTI + PHP iconv + RSA + libc-2.27 堆。关键路径：ezjava: 扫 /actuator/heapdump 拿 redis 密码 → jodd-http SSRF → redis SLAVEOF + MODULE LOAD 写 /tmp/exp.so 反弹 → ...'
category: web
subcategory: ssrf
subcategories:
- ssrf
...
contest_type: open
wp_url: https://www.ctfiot.com/51953.html
reasoning_chain:
- '[触发点] /actuator/heapdump 暴露 → 假设：Java Spring Boot 信息泄露 / [动作] 找 redis 密码 / [观察] 拿到 redis密码 / [下一步] jodd-http SSRF'
- '[触发点] jodd-http 库支持 SSRF → 假设：打 Redis / [动作] gopher 打 Redis SLAVEOF + MODULE LOAD / [观察] /tmp/exp.so 反弹 / [下一步] newweb'
- '[触发点] newweb nghttp HTTP/2 → 假设：Jinja2 SSTI / [动作] __class__.__base__.__subclasses__()走 attr 取 __globals__[''__builtins__''].eval / [观察] RCE / [下一步] safe_script'
- '[触发点] safe_script sessid=php://filter/convert.iconv.utf-7.utf-8 → 假设：PHP filter 链 / [动作] 写 php://filter 链 + 一句话木马 / [观察] RCE / [下一步] tomic'
- '[触发点] tomic qsieve 分解 n → 假设：RSA 多素数 / [动作] GCD(n,e) / [观察] 共享因子 p / [下一步] gonote'
- '[触发点] gonote libc-2.27 负 size 整数溢出 → 假设：tcache 越界 / [动作] leak libc → __free_hook=system / /bin/sh'
failed_attempts:
- 试图 heapdump 直接看密钥 → 失败：heapdump 是 HPROF 格式需解析
- 试图用 PHP 常规 RCE → 失败：filter 链才能绕
key_observations:
- Spring Boot heapdump + jodd-http SSRF 是 Java CTF 标配
- Jinja2 attr 链是 WAF 绕过关键
- PHP iconv filter 链可写文件
- RSA 共享素数 = GCD(n,e)
- libc-2.27 负 size 是 tcache 越界经典
prerequisites:
- Spring Boot actuator 端点识别
- Redis SLAVOO + MODULE LOAD 攻击
- Jinja2 SSTI attr 链
- PHP iconv filter 链
- RSA 共享素数攻击
- libc-2.27 heap 基础
