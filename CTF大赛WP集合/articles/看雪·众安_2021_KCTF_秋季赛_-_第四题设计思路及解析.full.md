...
quality: high
full_path: 看雪·众安_2021_KCTF_秋季赛_-_第四题设计思路及解析.full.md
meta_path: 看雪·众安_2021_KCTF_秋季赛_-_第四题设计思路及解析.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: 看雪·众安 2021 KCTF 秋季赛 第四题 偶遇棋痴。把 flag 验证逻辑用 Lambda 演算写出来序列化到 Android JNI，逆向时要还原 Lambda 语法树并独立爆破每个 4bit 输入。。关键路径：出题方用 Lambda 演算（Church Numeral + ADD/SUB/MULT/IF/XOR/ABSDIFF）编码 flag 校验逻辑 → 程序把 Lambda 表...
category: reverse
subcategory: reverse
tools_used:
- ARM
...
contest_type: open
wp_url: https://www.ctfiot.com/13578.html
reasoning_chain:
- Android JNI + Lambda Calculus + Boost serialize → 触发点：学术+实战结合题
- 假设：程序用Lambda演算（Church Numeral+ADD/SUB/MULT/IF/XOR/ABSDIFF）编码flag验证 → 动作：学Lambda演算
- 预编译Lambda表达式用Boost text_oarchive序列化到C字符串数组 → 假设：运行时反序列化得Lambda语法树
- 观察：用户输入14字符按4bit拆28个ipt插入预编译树 → sub_39614删减 → 最终树剩3节点即通过
- 假设：每个ipt对其他ipt独立 → 动作：28个独立4bit爆破
- 第一步：还原AST → z3求解28个IntVector → flag 14字符
- 或方式2：独立分支爆破 + unidbg运行 + ARM汇编转C暴力
- 观察：暴低4bit后再暴高4bit（可见字符最高位0仅3bit） → 14字符flag
failed_attempts:
- 试图不解Lambda语法树直接爆破 → 失败：函数嵌套层数太深
- 用unidbg跑原始UXTB16指令 → 失败：报错，改ARM汇编转C暴力
- 尝试还原AST但没意识到预编译 → 失败：28个ipt插入点是AST的修改点
key_observations:
- Lambda演算+序列化是"代码即数据"反模式（隐蔽）
- Church Numeral ADD/SUB/MULT/IF/XOR/ABSDIFF 实现数值/逻辑
- Boost text_oarchive/text_iarchive序列化二进制Lambda
- 用户每个4bit输入相对独立 = 可分28道独立小题
- z3 IntVector约束=28组ABSDIFF方程求和最终==0
prerequisites:
- Lambda演算基础（Church Numeral/Beta Reduction）
- Boost序列化（text_oarchive/iarchive）
- z3 IntVector建模
- Android JNI + unidbg
---
