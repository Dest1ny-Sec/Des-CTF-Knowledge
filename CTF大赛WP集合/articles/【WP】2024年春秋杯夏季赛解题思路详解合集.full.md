...
quality: high
full_path: 【WP】2024年春秋杯夏季赛解题思路详解合集.full.md
meta_path: 【WP】2024年春秋杯夏季赛解题思路详解合集.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: 【WP】2024年春秋杯夏季赛解题思路详解合集。2024 春秋杯夏季赛解题思路详解合集，ECC 离散对数 + RSA+AES HSSP + 格攻击。。经验：ECC 离散对数用 `pow(x2-x1, M-2, M)` 模逆 + c1-k*c2 还原明文；HSSP (Hidde...
category: crypto
subcategory: rsa
tools_used:
- Sage
...
contest_type: open
wp_url: https://www.ctfiot.com/196224.html
reasoning_chain:
- ezzzecc：base62+4 次 base64 解密 p（已知格式差 -11） → 动作：写脚本提取
- ECC 椭圆曲线 y²=x³+ax+b over GF(M) → add 函数 + pow(x2-x1, M-2, M) 模逆 → 假设：可以复现
- 爆破 k 找到 base + pub → 动作：c1-k*c2 还原明文
- happy2024：RSA+AES 组合 + RSA 格攻击 → 假设：HSSP Hidden Sum of Subset Products
- AES iv 矩阵混淆 + 排列爆破 proof（2^19 case 调整 SHA256 前 6 字符） → flag
failed_attempts:
- 试图标准 base64 解密 → 实际是 base62+4 次 base64
- 试图 Pollard-rho ECC 离散对数 → M 太大，转模逆
- 试图标准 RSA 分解 → 是 HSSP 格攻击
key_observations:
- ECC 离散对数用 pow(x2-x1, M-2, M) 模逆解有限域除法
- HSSP (Hidden Sum of Subset Products) 用格 (lattice) 攻击
- 排列爆破 proof 2^19 case 调整是 SHA256 前缀碰撞
- Sage 数学工具是 CTF Crypto 必备
prerequisites:
- ECC 椭圆曲线密码学（加法/标量乘/有限域）
- RSA 格攻击 + LLL 算法
- Sage 数学工具
