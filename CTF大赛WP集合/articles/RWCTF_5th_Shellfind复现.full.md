...
quality: high
full_path: RWCTF_5th_Shellfind复现.full.md
meta_path: RWCTF_5th_Shellfind复现.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: RWCTF 5th Shellfind 复现。RWCTF 5th Shellfind 复现 WP：qemu malta mips 模拟器 + tap0 网络 + ipfind UDP 62720 + base64 栈溢出 + dup2 busybox sh 回连。。关键路径：复现 RealWorld CTF 5th IoT PWN 题目 - 基于真实设备固件改编 → 拿到 .bin 固件包解...
category: pwn
subcategory: rop
tools_used:
- ROP
...
contest_type: open
wp_url: https://www.ctfiot.com/94114.html
reasoning_chain:
- 复现 RealWorld CTF 5th IoT PWN → 触发点：真实设备固件改编 + bindiff 比对
- 拿到 .bin 固件包解压获得完整文件系统 → 假设：替换被改的 ipfind 二进制
- MIPS 大端 + 无任何保护 → qemu-system-mips + malta + vmlinux-3.2.0-4-4kc-malta + tap0
- 启动 chroot squashfs-root + mount /proc + /etc/rc.d/rcS → 启动 ipfind eth0 & (qemu 用 eth0 而非 br0)
- '漏洞点：sub_400F50 base64 decode 到栈 → 栈溢出 → checksec: No RELRO + No canary + NX disabled'
- mipsrop 无输出 → 自构 ROP → 利用 UDP 协议 FIVI + cmd=2 (Shell) + base64(username:password) 触发
- sub_4013F4 校验 + sub_400F50 触发 base64 decode 栈溢出 → s_log_nothing 泄栈地址
- 改 GOT + ret2shellcode → dup2 stdin/stdout/stderr + execve /bin/busybox sh → IPFIND UDP fd 复用
failed_attempts:
- 试图直接用 br0 启动 ipfind → 失败：qemu 用 eth0
- 试图用 mipsrop 自动化 → 失败：mips 大端无输出
- 试图单命令反弹 → 失败：必须 dup2 复用 UDP fd
key_observations:
- qemu-system-mips + tap0 桥接 + chroot 启动是 IoT 复现基础
- ipfind 启动时指定 eth0 (qemu) 而非 br0 (真实设备) 才能 gdbserver
- base64 decode 写入栈是经典溢出模式
- bindiff 比对官方固件 vs 复现固件是改点定位手段
- UDP fd 复用为 stdin/stdout 是无 nc 场景唯一路径
prerequisites:
- qemu-system-mips + tap0 桥接
- MIPS ROP 自构
- chroot squashfs-root 启动
- GDB server + remote 调试
---
