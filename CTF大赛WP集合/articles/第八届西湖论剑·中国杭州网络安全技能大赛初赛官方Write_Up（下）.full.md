...
- AES-CBC+HMAC
- PWN-babytrace
- Heaven's door-32/64切换+proc-mem,Vpwn-栈溢出-edit高地址,RE-BitDance位重排
...
quality: high
full_path: 第八届西湖论剑·中国杭州网络安全技能大赛初赛官方Write_Up（下）.full.md
meta_path: 第八届西湖论剑·中国杭州网络安全技能大赛初赛官方Write_Up（下）.meta.md
images_removed: true
images_removed_count: 12
schema_version: v3.0.0-P0
summary: 第八届西湖论剑·中国杭州网络安全技能大赛初赛官方Write Up(下)。第八届西湖论剑下半:MISC糟糕的磁盘(5张img合并)→RSA+AES-CBC+HMAC+COM_UnMunge2解码UDP stream→secret.pcapng|PWN-babytrace(int3+pop rax+add rsp,0x78 ret读/flag)+Heaven's door(32/64切换+ope...
category: misc
subcategory: misc_other
time_required: long
difficulty_score: 4
code_blocks_count: 53
images_count: 12
last_verified: 2026-09-20
contest_type: open
wp_url: https://www.ctfiot.com/226338.html
reasoning_chain:
- 糟糕的磁盘：触发点：5 个 img 压缩包文件名 → 假设：解压后拼合成完整磁盘
- 动作：unzip img1-5.img → losetup 挂载 → 观察：得到合并文件系统
- CSCS：触发点：RSA 16 进制公钥 + AES-CBC 加密 UDP 流 → 假设：从公钥恢复 RSA 参数
- 动作：RSA.import_key(hex) → 提取 n/e → 观察：得到加密密钥
- COM_UnMunge2 解码：触发点：UDP 流压缩编码 → 假设：COM_UnMunge2 是 Office 的 munged 格式
- 动作：用 unmunge 工具还原 stream → 观察：得到原始 UDP 数据
- AES-CBC+HMAC：触发点：secret.pcapng 含密文+IV → 假设：用 RSA 私钥解 HMAC 密钥再解 AES
- PWN babytrace：触发点：trace 输出 syscall 序列 → 假设：strace 类工具截获 syscall
failed_attempts:
- 糟糕的磁盘直接 mount 单 img → 失败：缺 5 个拼合
- CSCS 直接用密文硬解 → 失败：缺 RSA 私钥
- AES-CBC 跳过 HMAC 验证 → 失败：完整性校验失败
key_observations:
- 取证题多文件拆分是常见模式（5 张 img 需合并）
- RSA 16 进制公钥导入用 PyCryptodome RSA.import_key(hex)
- AES-CBC + HMAC 是经典加密+认证组合
- COM_UnMunge2 是 Office COM 序列化混淆格式
prerequisites:
- Linux 磁盘取证（losetup/mount/dd）
- RSA 公钥 ASN.1 解码
- AES-CBC + HMAC-SHA256 协议
- strace ptrace 调试基础
---
...
```shell[图片已移除]http://192.168.18.32/admin/Uploads/5f5e82a45dfaabb7df5a1fe28632d10d/a9eb430817ca0ca62b44895f5235c0a2.php```
