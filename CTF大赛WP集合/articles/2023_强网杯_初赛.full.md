...
quality: high
full_path: 2023_强网杯_初赛.full.md
meta_path: 2023_强网杯_初赛.meta.md
images_removed: true
images_removed_count: 2
schema_version: v3.0.0-P0
summary: '2023 强网杯初赛。强网杯初赛 6+ 题：sage gamma 阶乘+bsgs+RSA_p^5+共享比特+复数RSA+ISIS。。关键路径：Speed up: sage gamma(2^27+1) 各位数求和 → 石头剪刀布: 摸黑试探 + 脚本固定套路 → discrete_log: p-1=2q, bsgs+padding 12 字节 hex 拆分爆破。经验：sage gamma() ...'
category: misc
...
contest_type: open
wp_url: https://www.ctfiot.com/152614.html
reasoning_chain:
- 'speed up: 2^27 阶乘各位数之和 → 触发点：大数阶乘'
- 假设：sage gamma(2^27+1) 一步算 → 动作：str(aaa) + 各位求和
- 观察：得到答案
- '石头剪刀布: 5 次随机 + ''训练'' AI → 假设：根据我方出法预测'
- 动作：每次试错 + 选能赢的出法 → 观察：拿到 flag
- 'discrete_log: flag 被填充 + 偏移 → 假设：flag 39 byte + ''flag{}'' 已知'
- 动作：去掉填充 + 开根开掉偏移 → bsgs(p, c, p) → 观察：得到 flag
- 'RSA p^5: n=p^5 标准 → 动作：sage nth_root → 观察：flag'
- '共享比特: 两个 RSA n 共享 p 高位 → 假设：gcd 找公因子'
- 动作：sympy.gcd(n1, n2) → 观察：p
- '复数 RSA: n = a+bi 复数 → 假设：复数范数 |n| 分解'
- 动作：abs(n) → sage.factor → 观察：p, q → 解 RSA → flag
failed_attempts:
- 试图直接 factorial(2^27) → 失败：内存溢出
- 试图循环求各位数 → 失败：太慢
- 试图标准 Pollard rho → 失败：共享比特需要 gcd
key_observations:
- sage gamma() 算大数阶乘是简单技巧
- p-1=2q 特殊 DLP 用 bsgs+padding 拆分爆破可破
- 共享 RSA 素数用 gcd 找公因子
- 复数 RSA 转 |n|^2 = a^2+b^2 分解
prerequisites:
- Sage gamma 函数
- BSGS 算法
- RSA 共享素数攻击
