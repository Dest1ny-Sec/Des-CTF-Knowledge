...
quality: high
full_path: 【三等奖方案】基于TPU平台实现人群密度估计赛题「innovation」团队解题思路.full.md
meta_path: 【三等奖方案】基于TPU平台实现人群密度估计赛题「innovation」团队解题思路.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: CCF BDCI 2022 三等奖 - TPU 人群密度估计 innovation 团队。台州学院innovation团队用VGG-INT8+图像增强在算能TPU上B榜第五。。经验：模型选择需兼顾精度和泛化 — A榜10张图选VGG，到A榜1201张MCNN精度严重下滑；量化迭代次数对精度影响不大(趋...
category: misc
...
contest_type: open
wp_url: https://www.ctfiot.com/123025.html
reasoning_chain:
- TPU 密度估计 → 触发点：算能 BM1684 平台 → 动作：选 VGG backbone → 观察：FP32 精度优于 MCNN
- 转 INT8 bmodel 格式 → 触发点：TPU 推理快 → 动作：算能 sail/tools 转换 → 观察：得到 bmodel 文件
- 量化校准 230 次迭代 → 触发点：INT8 精度损失 → 动作：训 230 步校准集 → 观察：精度损失 < 1%
- 测试图像增强 → 触发点：A 榜精度不够 → 动作：旋转 + 缩放至 576×768 + 锐度×1.5 + 亮度×1.5
- 观察：图像增强后 MAE -8 + RMSE -3 → score +2.5
- B 榜 1201 张泛化测试 → 动作：保持 INT8 + 图像增强组合 → 观察：B 榜第五（精度大幅下滑但稳在前 5）
- 反思：A 榜选 VGG 是因为 10 张图小场景，到 B 榜 MCNN 大规模人群场景精度严重下滑
failed_attempts:
- 试图用 MCNN 通用模型 → 失败：A 榜 10 张图小场景好，B 榜 1201 张泛化严重下滑
- 试图用更多量化迭代次数 → 失败：迭代次数对精度影响不大（已趋于平稳）
- 试图不增强图像直接推理 → 失败：score 中 NAE 项扣太多
key_observations:
- 模型选择需兼顾精度和泛化 — A 榜 10 张图选 VGG，到 B 榜 1201 张 MCNN 精度严重下滑
- 量化迭代次数对精度影响不大（趋于平稳），230 次已足够
- 图像预处理（尺寸+锐度+亮度组合）对最终 FINAL_SCORE 贡献最大
- INT8 量化 + 锐度/亮度增强是 TPU 部署最优组合
- B 榜泛化测试是真实场景，A 榜小样本易过拟合
prerequisites:
- VGG / MCNN / CSRNet / CANNet 等密度估计模型
- 算能 BM1684 TPU 工具链（量化 + bmodel）
- 图像增强（锐度/亮度/旋转/缩放）
- INT8 量化原理与校准集选取
---
