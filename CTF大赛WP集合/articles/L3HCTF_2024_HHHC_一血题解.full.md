...
lesson: '|'
quality: high
full_path: L3HCTF_2024_HHHC_一血题解.full.md
meta_path: L3HCTF_2024_HHHC_一血题解.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: L3HCTF 2024 HHHC 一血题解 (H3C MSR3610 路由器 PPPoE)。L3HCTF 2024 HHHC 一血题解 — H3C MSR3610 路由器配置中恢复 PPPoE 密码，靠 H3C 自家模拟器解 $c$3$ cipher + 重画拓扑抓 PAP 明文。。经验：|
category: misc
...
contest_type: open
wp_url: https://www.ctfiot.com/161467.html
reasoning_chain:
- 题目描述：L3HSec MSR3610 路由器配置中找 PPPoE 密码 → 触发点：路由器密码恢复
- 动作：分析配置文件 → 观察：发现 $c$3$ cipher 编码密码
- 假设：H3C $c$3$ 加密 → 动作：搜 H3C $c$3$ 解密算法
- 观察：H3C 官方有密码恢复机制 → 假设：用 H3C 模拟器
- 动作：HCL 模拟器载入同型号配置 → 假设：display pppoe-client 看明文
- 观察：模拟器显示 PAP 明文密码 → 完成
- 备用：分析 $c$3$ = 3DES + salt → 假设：固定 salt + 配置已知 → 动作：自己解密 → 完成
failed_attempts:
- 直接 Cisco IOS 解析 → 失败：H3C 是私有格式
- 试 7z 解压 .bin 备份 → 失败：H3C 配置非压缩
- 试网上 $c$3$ 通用解密 → 失败：必须用自家模拟器
key_observations:
- H3C $c$3$ 加密是 3DES + salt，靠官方模拟器解最稳
- 路由器配置题非预期解：直接搭同型号模拟器复现
- PPPoE 客户端配置中密码可走 PAP 明文
- Hillst0ne 是 L3HCTF 2021 复用题目背景
- HCL 模拟器是 H3C 设备的 GNS3 替代
prerequisites:
- H3C MSR3610 路由器基础 + PPPoE 协议（PAP / CHAP）
- HCL 模拟器使用 + 思科 / H3C show 命令集
- 密码恢复（type 7 / type 9 / $c$3$）
---
