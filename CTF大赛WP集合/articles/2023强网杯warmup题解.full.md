...
quality: high
full_path: 2023强网杯warmup题解.full.md
meta_path: 2023强网杯warmup题解.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: 2023 强网杯 warmup 题解。强网杯 warmup：off by null + House of apple2 + _IO_wfile_overflow + 栈迁移 ORW。。关键路径：漏洞点：add 函数 off by null 漏洞 → 利用链：off by null + chunk overlap + 改 tcache fd 指向 stdout → 申请到 stdout 区域写...
category: pwn
...
contest_type: open
wp_url: https://www.ctfiot.com/165317.html
wp_author: null利用
reasoning_chain:
- 触发点：add 函数 off by null 漏洞 → 假设 chunk overlap → 动作：分配 + 写入末位 0
- actor chunk overlap 改 tcache fd 指向 stdout → 假设 malloc 到 stdout 区域 → 观察：申请到 chunk
- actor 写 fake IO_FILE_plus_struct → 假设 House of apple2 _IO_wfile_overflow → 动作：触发 vtable
- 动作：vtable 设置 _IO_wfile_jumps - 0x20 → 假设 _wide_data 指向 stdout - 0x48
- 动作：_wide_data->_wide_vtable->_doallocate = leave_ret → 假设：栈迁移到 stdout + 8
- actor pop_rbp + leave_ret 二次栈迁移 → 假设 ROP 链触发 → 动作：构造 syscall open+read+write
- actor 栈迁移 ORW 完成 → 假设：高版本 off by null 需绕 bck->fd==P 检查
- actor House of apple2 用 _IO_wfile_jumps - 0x20 直接劫持 → pwncli IO_FILE_plus_struct().house_of_apple2
failed_attempts:
- 试图用 House of apple1 → 失败：apple2 可直接执行 ROP 链更高效
- 试图标准 off by null → 失败：高版本需绕 size vs prevsize + bck->fd==P 检查
key_observations:
- 高版本 off by null 需绕 bck->fd==P 检查
- House of apple2 用 _IO_wfile_jumps - 0x20 直接劫持
- 栈迁移需要 leave;ret 二次迁移
- House of apple2 比 apple1 可直接执行 ROP
prerequisites:
- glibc 2.29+ off by null 绕过
- House of apple2 利用链原理
- 栈迁移 leave;ret gadget
- ORW open/read/write 系统调用
---
# 2023强网杯warmup题解
