...
tags:
- JS bridge
- _jsbridge.add/edit/show/delete
...
quality: high
full_path: AVSS_2024_Final_Writeup.full.md
meta_path: AVSS_2024_Final_Writeup.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: AVSS 2024 Final Writeup。AVSS 2024 Final — _jsbridge add/edit/show/delete + heap 0x90 重叠 (del 偶数+add 奇数) + show 0x8 找指针 + stringToHex/u32/p32 端序 + xhr 异步 exfil。。关键路径：_jsbridge.add(i, key, size) 申请 0...
category: web
...
contest_type: open
wp_url: https://www.ctfiot.com/213863.html
reasoning_chain:
- '[触发点] _jsbridge 暴露 add/edit/show/delete 四个方法 → 假设：这是 hybrid 移动端 pwn 入口 / [假设] 0x90 是 fastbin 临界 chunk 大小可制造重叠 / [动作] 连续 add 0..48 size=0x90 填充 tcache / [观察] 申请量到位后 del 偶数下标释放 24 个 0x90 块 / [下一步] add 奇数下标回收触发 chunk 重叠'
- '[触发点] show(j, 0x8) 只读 8 字节 → 假设：可泄露其他对象指针做 leak / [假设] 重叠 chunk 中能改写 chunk header 与 fd / [动作] stringToHex 把 chunk 内容转 hex + u32/p32 端序转换 / [观察] 找到相同指针的两个 chunk 即为重叠点 / [下一步] 改相邻 chunk fd 触发 tcache 任意地址分配'
- '[触发点] stringToHex + u32 + p32 端序组合可用 → 假设：可写任意 32-bit 到重叠 fd / [假设] 0x1000 大小 add 可绕 tcache 进 unsorted bin / [动作] add(48, 0x1000) 十次构造 unsorted bin 头部 / [观察] 泄露出 libc 指针 / [下一步] 改 free_hook 走 system'
- '[触发点] xhr log 异步 exfil / [假设] 内部网络环境可用 _jsbridge 栈溢出触发 RCE / [动作] 构造 _jsbridge 栈溢出 payload 调用 execve / [观察] xhr 把 stack 内容异步上报 / [下一步] 拿到 flag'
failed_attempts:
- 试图用单次 del 整片回收 → 失败：需要奇偶下标交叉才能制造 0x90 重叠
- 试图不读 fd 直接覆盖 → 失败：tcache_count 检查在 glibc 2.35 上卡得死
key_observations:
- 0x90 fastbin 临界 chunk 大小是 heap overlap 经典尺寸
- show 部分字节（0x8）是堆指针发现的标准手法
- stringToHex + u32 + p32 端序组合可读写任意 32 位
- JS bridge 类型混乱是 hybrid 移动端 pwn 的常见入口
prerequisites:
- glibc 2.35 tcache / fastbin 行为
- Hybrid App JS bridge 调试方法（frida + chrome devtools）
- pwntools / heap 工具链
---
