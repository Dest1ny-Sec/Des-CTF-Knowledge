...
- stl_reversing
- llvm_pass_dump
- deque_iterator_memory
- vector_three_pointer
- rb_tree_node_color
- hashtable_buckets
- string_local_buf_8
- recursive_stl_containers
- ctf_n1ctf
- cplusplus_re
...
quality: high
full_path: '[原创]STL容器逆向与实战(N1CTF2022_cppmaster_wp).full.md'
meta_path: '[原创]STL容器逆向与实战(N1CTF2022_cppmaster_wp).meta.md'
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: '[原创] STL 容器逆向与实战 (N1CTF 2022 cppmaster wp)。N1CTF 2022 cppmaster 逆向：LLVM Pass dump class 结构，deque<map<unordered_map>> 三层嵌套 STL 容器 + 动调 input_check 函数确认嵌套顺序 + 还原数据。。经验：STL 容器逆向先识别底层数据结构 (rb_tree/dequ...'
category: misc
...
contest_type: open
wp_url: https://www.ctfiot.com/76880.html
reasoning_chain:
- 触发点:N1CTF 2022 cppmaster 涉及 deque<vector<unordered_map>> 等嵌套 STL → 假设:必须先识别 STL 类型内存模型 → 动作:写 LLVM Pass dump class 结构
- 动作:DumpClass pass 输出 → 观察:class.std::deque<string> { map/size/start/finish iterator(cur/first/last/map) }
- 动作:class.std::vector<string> { start/finish/end_of_storage } → 观察:三指针结构
- 动作:class.std::map<string,string> { _Rb_tree key_compare + header(node color/parent/left/right) + node_count } → 观察:rb_tree 五字段
- 动作:class.std::unordered_map { buckets[]+bucket_count+element_count+_Prime_rehash_policy } → 观察:hashtable 三字段
- 动作:class.std::string { ptr + string_length + union{allocated_capacity, local_buf[8]} → 观察:string 三字段含 SSO
- 动作:动调 input_check 函数确认嵌套顺序 deque<map<int,int>> + rb_tree → 观察:数据布局确认
- 动作:还原嵌套类型 + 数据 → 完成
failed_attempts:
- 试图直接 IDA F5 STL 代码 → 失败:STL 模板链太复杂 F5 不可读
- 试图不解 LLVM Pass → 失败:必须先 dump class 结构
- 试图不解 SSO → 失败:string 的 local_buf[8] 是关键
key_observations:
- STL 容器逆向先识别底层数据结构 (rb_tree/deque/hashtable/vector/string) 再识别模板类型
- LLVM Pass dump 是学习 C++ 类内存模型的黄金工具
- deque 三件套 (map/size + start/finish iterator) + rb_tree 五字段 + string SSO 是 STL 通用模型
- map 是 rb_tree 包装 + unordered_map 是 hashtable 包装
- 递归嵌套 STL (deque<map<unordered_map>>) 必须分步解构
prerequisites:
- LLVM Pass 编写 + StructType dump
- STL 容器底层 (rb_tree / deque / hashtable / vector / string)
- C++ 模板类型推导
- IDA + 动态调试结合
---
