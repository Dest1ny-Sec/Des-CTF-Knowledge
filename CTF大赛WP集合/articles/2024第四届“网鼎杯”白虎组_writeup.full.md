...
- web_unknown
tags:
- 白虎 misc01 GTP TEID 统计排序
- tshark gtp.teid
- tshark e GTP 流
- misc02 AES-ECB TEID 当 key
- misc03 php filter 侧信道
- misc04 三个文件合并 zip 伪加密
- CRYPTO01 SPN P(x) 仿射变换
- S 看作 chips 线性变换
- 对 S 求 LLL 还原 keys
- flag 头 wdflag{ 爆破 1 字节
attack_chain:
...
quality: high
full_path: 2024第四届“网鼎杯”白虎组_writeup.full.md
meta_path: 2024第四届“网鼎杯”白虎组_writeup.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: '2024 第四届"网鼎杯"白虎组 writeup（GTP TEID + AES-ECB + SPN 仿射）。白虎组四题：GTP TEID 统计 + AES-ECB + php filter 侧信道 + zip 伪加密 + SPN 仿射 LLL 还原 keys。。关键路径：misc01: tshark -r UPF.cap -e "gtp.teid" 统计排序两处异常 TEID 拼接 ...'
category: crypto
...
contest_type: open
wp_url: https://www.ctfiot.com/213306.html
reasoning_chain:
- '[触发点] misc01 流量是 GTP 协议 → 假设：TEID 之后的数据即为密文 → [动作] tshark -Y ''mms.confirmedServiceRequest'' + 解密 → [观察] TEID 是 4 字节标识 → [下一步] 用 TEID 做 AES-ECB key'
- '[触发点] AES-ECB 用 TEID 作 key 解密流量 → 假设：TEID 长度 32 字符 = 16 字节 hex → [动作] Python 循环 16 字节 → [观察] 拿到 flag → [下一步] misc02'
- '[触发点] misc02 提示 php filter 侧信道 → 假设：convert.quoted-printable-encode 触发 php://filter 多次 base64 → [动作] blow_up_enc 3000 次 + convert.base64-encode 多层 + 爆破 → [观察] 拿到 flag → [下一步] misc03'
- '[触发点] misc03 三文件合并 + binwalk → 假设：2.png 伪加密 → [动作] cat 1 2 3 > 1.zip + binwalk + 改 2.png 09 00 为 00 00 解压 → [观察] 拿到后一半 flag → [下一步] 另一压缩包掩码爆破'
- '[触发点] crypto01 P(x)=P*x, S(x)=A*x+b 是 SPN 仿射密码 → 假设：14 轮结构 → [动作] 构造 GF(2) 上 P^-1 AP 矩阵 + T^14 + sum(T^i) 求 U + 爆破 1 字节 → [观察] 解出 key → [下一步] crypto02'
- '[触发点] crypto02 RSA n 巨大，密文给 → 假设：Pollard''s rho 通用分解 → [动作] mapx(x) = (x^(n-1)+3) mod n + rho 循环 → [观察] 拿到 p → [下一步] pow(c, d, n) 解密'
failed_attempts:
- 试图用 Wireshark GUI 解 GTP 流量 → 失败：tshark 命令行更稳定
- 试图爆破 crypto01 全部 key → 失败：必须用 P^-1AP 矩阵分解才能 GF(2) 上 LLL
- 试图不解密直接看 pcapng 文本 → 失败：flag 在二进制层
key_observations:
- GTP TEID 是 5G 核心网隧道标识，tshark -e gtp.teid 提取
- SPN 仿射密码把 chips∈{1,-1} 时 LLL 还原 key 比特是 Lattice 经典套路
- php://filter + convert.quoted-printable-encode 3000 次 = 经典 php filter 侧信道
- binwalk 看到伪加密压缩包：改 09 00 为 00 00 即可解压
- Pollard's rho mapx(x) = (x^(n-1)+3) mod n 是 CTF RSA 经典映射分解
prerequisites:
- GTP/UEC 协议基础
- tshark 命令行使用（-Y 过滤 / -T fields 提取）
- PHP filter chain 侧信道原理
- Sage 矩阵运算与 GF(2) 线性代数
---
