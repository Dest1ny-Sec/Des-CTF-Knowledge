...
quality: high
full_path: 春秋杯WP_-_2024春秋杯冬季赛第一天题目部分解析.full.md
meta_path: 春秋杯WP_-_2024春秋杯冬季赛第一天题目部分解析.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: 春秋杯WP | 2024春秋杯冬季赛第一天题目部分解析。春秋杯2024冬季赛D1六题混合：ez_vm魔改XTEA+ezre custom_md5+RSA1 Franklin-Reiter+Coppersmith。经验：魔改XTEA常通过改delta/移位位数/异或常数实现；RSA Franklin-Reiter可联立Coppersmit...
category: reverse
subcategory: reverse
...
contest_type: open
wp_url: https://www.ctfiot.com/224900.html
reasoning_chain:
- 六题混合：EzRSA/ez_vm/ezre/RSA1/Gender_Simulation/Gotar → 触发点：ez_vm 是 VM 魔改 XTEA
- 动作：手撕 VM 一遍循环 → 观察：运算表达式带 (v1<<4)^(v1>>6)^0x42 + v1 特征
- 假设：标准 XTEA 但 delta 改 0x20252025、右移 6、异或 0x42、异或 3
- 动作：写 decipher(32, m, key={2,0,2,5}) → 拿到 ez_vm flag
- EzRSA：模数 n^3 离散对数 → 触发点：二项式定理展开
- RSA1：Franklin-Reiter + Coppersmith → 触发点：相同 msg 不同 e 的 RSA
- 动作：联立方程 gcm 解 m1 + Coppersmith X=2^128 beta=1 epsilon=0.05
- 观察：拿到 flag{e8ff1a85-...}
- Gotar：Golang tar 处理 → 假设：符号链接穿越
- 动作：构造恶意 tar 释放到任意路径 → 拿到 flag
failed_attempts:
- 直接当标准 XTEA 解 → 失败：delta/移位/异或常数全被改
- Franklin-Reiter 单独用 → 失败：必须联立 Coppersmith 才解得 e
key_observations:
- 魔改 XTEA 常通过改 delta/移位位数/异或常数实现
- Franklin-Reiter + Coppersmith 联立可解 e
- 模数 n^k 离散对数用二项式定理展开降低复杂度
- Go tar 处理符号链接是任意文件写入入口
prerequisites:
- XTEA 算法变种识别
- RSA Coppersmith + Franklin-Reiter 攻击
- 二项式定理 + 模运算
- Go tar 符号链接处理
---
