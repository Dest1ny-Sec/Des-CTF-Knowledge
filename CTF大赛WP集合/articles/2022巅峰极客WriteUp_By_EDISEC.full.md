...
- strangeTempreture
attack_chain:
- 'Web babyweb: Padding Oracle + padbuster → 密码 0f90844e-f214-4f0b-aaa3-687a8098b896'
- 'Web ezWeb: /readf/feedback.php 注入 startid=0*, ban.php url=file:///etc/passwd* 任意文件读'
- 'Crypto point-power: ECC 加法公式 y²=x³+ax+b，联立解方程 a,b → flag bytes'
- 'Crypto strange curve: long_to_bytes(x) → flag 字节'
- 'Pwn Gift: libc-2.27 tcache dup → __free_hook = system → /bin/sh'
- 'Pwn smallcontainer: large bin / unsorted bin 残留指针 leak + UAF'
- 'Pwn happy_note: libc-2.31 PROTECT_PTR 绕过 (xor = heap_base>>12) + tcache stash unlink'
...
quality: high
full_path: 2022巅峰极客WriteUp_By_EDISEC.full.md
meta_path: 2022巅峰极客WriteUp_By_EDISEC.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: '2022 巅峰极客 WriteUp By EDISEC。巅峰极客 2022 6 大题：Padding Oracle + Web + ECC + 三道 pwn。关键路径：Web babyweb: Padding Oracle + padbuster → 密码 0f90844e-f214-4f0b-aaa3-687a8098b896 → Web ezWeb: /readf/feedback.ph...'
category: crypto
subcategory: oracle
subcategories:
- oracle
...
contest_type: open
wp_url: https://www.ctfiot.com/53840.html
reasoning_chain:
- '[触发点] babyweb 提示 Padding Oracle → 假设：padbuster 攻击 / [动作] padbuster / [观察] 0f90844e-f214-4f0b-aaa3-687a8098b896 / [下一步] ezWeb'
- '[触发点] ezWeb /readf/feedback.php 注入 startid=0*, ban.php url=file:///etc/passwd* → 假设：任意文件读 / [动作] / [观察] /etc/passwd / [下一步] point-power'
- '[触发点] point-power y²=x³+ax+b 联立 → 假设：ECC 加法公式 / [动作] 解 a,b / [观察] flag bytes / [下一步] strange curve'
- '[触发点] strange curve long_to_bytes(x) → flag / [动作] / [观察] flag / [下一步] Gift pwn'
- '[触发点] Gift libc-2.27 tcache dup → __free_hook = system →假设：double free / [动作] tcache dup / /bin/sh'
- '[触发点] smallcontainer large bin/unsorted bin 残留指针 leak + UAF + happy_note PROTECT_PTR 绕过 → 假设：3 道 pwn 综合 / [动作] leak libc + heap_base / [观察] flag'
failed_attempts:
- 试图手写 Padding Oracle 脚本 → 失败：padbuster 更快
- 试图覆盖 __free_hook → 失败：libc-2.31 移除 → PROTECT_PTR 绕过
key_observations:
- Padding Oracle 永远可破
- ECC 加法公式代数化是基础
- PROTECT_PTR 用 xor = heap_base>>12 绕过
- libc-2.27 tcache dup 是入门
prerequisites:
- Padding Oracle + padbuster
- ECC 加法公式代数
- SageMath 离散数学
- libc-2.27 tcache dup
- PROTECT_PTR (libc-2.31+)绕过
---
