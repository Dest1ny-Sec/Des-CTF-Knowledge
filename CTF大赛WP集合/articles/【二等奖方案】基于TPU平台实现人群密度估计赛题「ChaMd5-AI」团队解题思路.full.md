...
quality: high
full_path: 【二等奖方案】基于TPU平台实现人群密度估计赛题「ChaMd5-AI」团队解题思路.full.md
meta_path: 【二等奖方案】基于TPU平台实现人群密度估计赛题「ChaMd5-AI」团队解题思路.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: CCF BDCI 2022 二等奖 - TPU人群密度估计 ChaMd5-AI。华中科大博士用遗传算法+Network Activations在TPU人群密度估计上第3名,得分252.14。。经验：TPU平台低FLOPs≠低推理时间 — depth-wise类(EfficientNet)在TPU上因访存带宽限制实际更...
category: misc
...
contest_type: open
wp_url: https://www.ctfiot.com/122999.html
reasoning_chain:
- 赛题：TPU 平台人群密度估计 → 触发点：精度+速度双指标 (MAE/RMSE/NAE/TIME)
- 假设：低 FLOPs ≠ 低推理时间 (TPU 访存瓶颈) → 动作：FLOPs+Network Activations 双粗指标
- Network Activations = 网络激活总量 = 访存代理指标 → 假设：depth-wise 类 (EfficientNet) 在 TPU 上反而慢
- 动作：先训练大尺度伪真值模型 → 假设：让小模型贴近大模型分布 → 观察：MAE ↓ 3%
- 选 VGG backbone → 假设：传统 CNN 在 TPU 编译成熟 → 动作：fp32 bmodel 量化转算能芯片
- 超参调优用遗传算法 → 触发点：网格搜索 5×5×5=125 种太慢 → 假设：遗传算法高效搜索
- 动作：200 代/种群 35/交叉 0.4/变异 0.2/格雷码编码 → 观察：比网格快 5 倍
- 适应度 = 粗指标 FLOPs + Network Activations + MSE 对比伪真值 → 假设：多目标反sigmoid融合
- 动作：大权重 FLOPs + 小权重 Network Activations 反sigmoid 拼 → 观察：得分 252.14 第 3
- 最终 B 榜第 3 → 触发点：遗传算法在量化参数搜索上比随机/网格快且准
failed_attempts:
- 试图用 EfficientNet-B3 → 失败：depth-wise 在 TPU 上因访存带宽限制实际更慢
- 试图网格搜索 125 种参数组合 → 失败：每次转 bmodel 10 分钟，全跑需 21 小时
- 试图 INT8 量化 → 失败：精度下降超阈值 (NAE +5%)
key_observations:
- TPU 平台低 FLOPs ≠ 低推理时间 (depth-wise 类因访存瓶颈反慢)
- Network Activations (网络激活总量) 作为访存代理指标比 FLOPs 单独准
- 量化参数优化用遗传算法 + 伪真值模型比网格搜索更高效
- 大权重 FLOPs + 小权重 Network Activations 反sigmoid 融合是经验调参技巧
- VGG 比 CSRNet/MCNN 在 TPU 上综合分高 (186.74 vs 169.76/68.34)
prerequisites:
- TPU / 算能芯片推理原理 (bmodel 量化)
- FLOPs vs Network Activations 性能指标区别
- 遗传算法 (格雷码编码 + 交叉变异算子)
- 伪真值模型蒸馏思想
---
