...
quality: high
full_path: ACTF_flutter逆向学习.full.md
meta_path: ACTF_flutter逆向学习.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: ACTF flutter 逆向学习。ACTF Flutter 逆向学习 — blutter 解析 DartVM 快照 + IDA 符号还原 + Frida hook 按钮回调 + 真机 root 动态调试。。关键路径：apk 改后缀 .zip 解压得 libapp.so (DartVM 快照) → reflutter/flutter 逆向助手对新 SDK 无效 → 用 blutter 解析 ...
category: reverse
subcategory: reverse
tools_used:
- Frida
...
contest_type: open
wp_url: https://www.ctfiot.com/169656.html
reasoning_chain:
- apk 改后缀 .zip 解压得 libapp.so (DartVM 快照) → 触发点：Flutter 应用结构
- 假设：reflutter/flutter 逆向助手对新 SDK 无效 → 动作：用 blutter 解析
- blutter 产物 objs.txt / blutter_frida.js / ida_script / asm → 假设：可还原 Dart 符号
- AndroidKiller 改 AndroidManifest.xml：android:debuggable="true" + targetSdkVersion 27/28 → 重打包
- adb install 重打包 APK + adb push android_server64 /data/local/tmp/as + chmod 777 → root 真机调试
- adb forward tcp:12345 tcp:12345 → IDA 附加 → 触发点：动态调试入口
- 加载 libapp.so + run addName.py → 假设：可还原对象池符号
- 观察：main.dart 找 onChanged/onSubmitted 按钮回调 → 下一步：Frida hook 输入函数
- Frida hook 输入/输出函数 → 观察：截获明文密码 → flag
failed_attempts:
- 试图用 jadx 反编译 Dart → 失败：DartVM 快照是 native .so 不是 Java 字节码
- 试图用 reflutter → 失败：新版 SDK 不兼容
key_observations:
- Flutter APK = assets/flutter_assets + libapp.so (DartVM AOT 快照)
- blutter (pyelftools + capstone + libicu-dev) 是开源首选反编译工具
- 高 SDK 版本必须降 targetSdkVersion 才能重签名 debuggable
- 真机动态调试需 root + adb + android_server64 三件套
- IDA + Frida 双工具配合 = 静态分析 + 运行时 hook
prerequisites:
- Android APK 结构 + AndroidKiller 重打包
- DartVM 快照格式与 blutter 用法
- Frida + IDA 联合调试
---
# ACTF flutter逆向学习
