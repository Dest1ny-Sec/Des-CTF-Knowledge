...
quality: high
full_path: 【2025春节】解题领红包所有题writeup.full.md
meta_path: 【2025春节】解题领红包所有题writeup.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: 2025 春节 解题领红包所有题 writeup。2025 春节解题领红包全题 writeup，覆盖 XXTEA + 自定义 SPN + AES OFB + 魔改 XTEA + 自定义 VM 5 大密码学/逆向方向。。经验：phoenixAES 用于 AES DPA 侧信道攻击或已知末轮密钥恢复；魔改 XTEA 把 delta 改成 0xB9...
category: crypto
...
contest_type: open
wp_url: https://www.ctfiot.com/227568.html
reasoning_chain:
- 题 1 XXTEA：6+52/n 轮 + delta=0x9E3779B9 + key='my-xxtea-secret\x00' → 触发点：标准 XXTEA
- 动作：解密 enc = b64decode('hjyaQ8jNSdp+mZic7Kdtyw==') → 观察：解出 SPN 密码 24 字节 S-box
- 题 2 自定义 SPN：24 字节 S-box 拆位 (32 字节 16 字节) 查表 → 触发点：SPN 16 轮 Feistel
- 动作：直接反查 S-box 表还原明文 → 观察：拿 flag
- 题 3 AES-OFB：key=7E13141528AED2A6ABF7158809CF4F3C iv=0 → 触发点：AES DPA 侧信道或已知末轮密钥恢复
- 动作：phoenixAES.crack_file AES OFB 已知 key → 观察：解出密文
- 题 4 魔改 XTEA：delta=0xB979379E + 12 轮加密 timestamp+uid+hash → 触发点：魔改 delta
- 动作：写 decrypt 还原 → 观察：解出 flag
- 题 5 自定义 VM：17 种操作 xor/neg/pop/or/nop/ret/cmpnz/swap/and/shl/div/add 栈机 → 触发点：op>>3 区分操作 op&7 取立即数
- 动作：写 Python 反汇编器模拟栈机 → 观察：解出 flag{...} + md5(uid) 拼接 → 完成
failed_attempts:
- 题 3 试图直接 AES-OFB decrypt → 失败：phoenixAES 是侧信道恢复路径
- 题 4 试图用标准 XTEA delta → 失败：delta 被改成 0xB979379E
- 题 5 试图用 IDA 静态分析 → 失败：自定义 VM 必须模拟栈机执行
key_observations:
- phoenixAES 用于 AES DPA 侧信道攻击或已知末轮密钥恢复
- 魔改 XTEA 把 delta 改成 0xB979379E 是非标准变种
- XXTEA 的 6+52/n 轮数随 n 变化
- 自定义 VM 栈机 op>>3 区分操作 op&7 取立即数 7 表示后续字节为 16 位立即数
prerequisites:
- XXTEA / XTEA / TEA 算法（delta / rounds / shift 函数）
- AES-OFB / AES-CBC / AES-DPA 侧信道
- 自定义 VM 反汇编（栈机模拟）
- phoenixAES 工具使用
---
