...
quality: high
full_path: 祥云杯-WriteUp.full.md
meta_path: 祥云杯-WriteUp.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: 祥云杯-WriteUp。祥云杯fastjson反序列化+BCEL字节码+Codeception/Guzzle/OpisClosure多链触发system。经验：fastjson+BCEL+Spring路径穿越(..;)+Codeception/Guzzle/OpisClosure...
category: web
...
contest_type: open
wp_url: https://www.ctfiot.com/1349.html
reasoning_chain:
- 触发点：内网 fastjson 反序列化+Spring 路径穿越 /xxxx/..;/admin/test→假设：Java gadget 链
- 假设：BCEL 字节码可执行任意 Java→动作：构造 $$BCEL$$ 编码的 TemplatesImpl 类→观察：fastjson 反序列化触发
- 假设：Codeception RunProcess 是触发命令入口→动作：构造 CodeceptionExtensionRunProcess + GuzzleHttpPsr7AppendStream 链
- 观察：PumpStream 中 include('closure/autoload.php') 加载 OpisClosure→下一步：用 OpisClosure\serialize 包 system('cat /flag.txt')
- 假设：路径穿越 /xxxx/..;/admin/test 让 Tomcat 把请求转到管理接口→动作：触发反序列化点→观察：cat /flag.txt 执行成功
failed_attempts:
- 试图用 Shiro 反序列化 → 失败：环境是 fastjson 不是 Shiro
- 试图用 ysoserial 原始 CC 链 → 失败：反序列化点是 PHP gadget 不是 Java
- 试图直接触发 system → 失败：触发点需要走 OpisClosure 包装
key_observations:
- fastjson + BCEL 字节码可以执行任意 Java 方法而无需上传 class
- Codeception RunProcess → Guzzle AppendStream → CachingStream → PumpStream 是 PHP 反序列化经典 gadget 链
- OpisClosure\serialize($a) 把任意 PHP 闭包对象序列化进 PumpStream.buffer
- Spring Tomcat 路径穿越 ..; 绕过对 /admin/test 路径限制
prerequisites:
- Java 反序列化 gadget 链(CC / TemplatesImpl / TrAXFilter)
- BCEL 字节码加载原理
- PHP 反序列化 POP 链构造
- Spring/Tomcat URL 解析与路径穿越
---
# 祥云杯-WriteUp
