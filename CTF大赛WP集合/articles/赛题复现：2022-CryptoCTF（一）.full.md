...
quality: high
full_path: 赛题复现：2022-CryptoCTF（一）.full.md
meta_path: 赛题复现：2022-CryptoCTF（一）.meta.md
images_removed: true
images_removed_count: 2
schema_version: v3.0.0-P0
summary: '赛题复现:2022-CryptoCTF(一)。2022 CryptoCTF复现第一部分,涵盖Congruence同余式+AES已知明文攻击+3DES meet-in-the-middle+RSA多素数等经典密码学考点。。关键路径：Klamkin (Congruence): 已知(ax+by)%q=0对任意(ar+bs)%q=0成立,求(x,y)满足y是12-bit → 设x或y为getPri...'
category: crypto
subcategory: math
tools_used:
- Sage
...
contest_type: open
wp_url: https://www.ctfiot.com/58892.html
reasoning_chain:
- Klamkin (Congruence) 题：已知(ax+by)%q=0 对任意满足(ar+bs)%q=0 的 (r,s) 也成立 → 触发点：同余线性相关
- 假设：(x,y) 与 (r,s) 在 (mod q) 下线性相关 → 动作：设 x = getPrime(n), y = inv(x)·a·b mod q 求逆元
- 动作：循环 G/S 选项 + 逆元算 x 或 y → 观察：flag = CCTF{f1nDin9_In7Eg3R_50Lut1Ons_iZ_in73rEStIn9!}
- Baphomet 题：plain.txt + cipher.txt + key 已知 → 触发点：AES 已知明文攻击
- 动作：用 key 直接 AES 解密 cipher.txt → 还原明文
- 多题覆盖：Congruence + 逆向数学题 + 3DES meet-in-the-middle + RSA 多素数攻击
- 3DES MITM：枚举中间值前 56 位 + 反向计算右半侧 + 比对中间值
failed_attempts:
- 试图直接解同余方程组求 (x,y) → 失败：x,y 位数大且只给定一个约束
- 试图枚举所有 12-bit y → 失败：x 范围太大
- 把 AES 当未知密钥穷举 → 失败：key 直接给了
key_observations:
- 任何 (a,b) 满足 (ar+bs)%q=0 时 (ax+by)%q=0 也成立，说明 (x,y) 与 (r,s) 同余
- 设 x = getPrime(n) 然后 inverse 算另一个变量是构造特定位数解的通用手法
- AES 已知明文攻击只需 plain + cipher + key 三件套
- 3DES MITM 复杂度 2^56+2^56 是 2DES 经典攻击，3DES 仅是加固
prerequisites:
- 模运算同余式与逆元
- AES / 3DES 分组密码原理
- SageMath / SymPy 离散数学工具
- RSA 多素数攻击与 RsaCtfTool
---
