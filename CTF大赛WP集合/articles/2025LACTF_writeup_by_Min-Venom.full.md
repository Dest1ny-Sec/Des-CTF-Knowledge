...
- heap_exploit
tags:
- LACTF 2password %i$p 格式化串泄栈
- hunter2_cfc0xz68 flag
- state-change 0xF1EEEE2D 校验常量
- read=0x4012C1 通用 read
- door=0x4011D6 后门
- 0x404540+0x20-1 栈迁移
- minceraft 看我世界 游戏化菜单
- AE64 shellcode 编码
attack_chain:
...
quality: medium
full_path: 2025LACTF_writeup_by_Min-Venom.full.md
meta_path: 2025LACTF_writeup_by_Min-Venom.meta.md
images_removed: true
images_removed_count: 3
schema_version: v3.0.0-P0
summary: '2025 LACTF writeup by Min-Venom（2password 格式化串+state-change+minceraft）。2025 LACTF 三 pwn：2password 格式化串泄栈 + state-change read 栈迁移到 0xF1EEEE2D 后门 + minceraft 看我世界游戏菜单。。关键路径：2password: sendline(f"%{i...'
category: pwn
...
contest_type: open
wp_url: https://www.ctfiot.com/227551.html
wp_author: Min-Venom
reasoning_chain:
- '[触发点] 2password 题 printf 格式化串 → 假设：%N$p 泄栈 → [动作] payload = b''a''*0x100 + b''%9$p-%12$s'' → [观察] 拿到地址 → [下一步] ROP'
- '[触发点] state-change 题 read + 后门函数 0xF1EEEE2D → 假设：栈迁移到目标地址触发后门 → [动作] payload = b''a''*(0x20) + p64(0x404540+0x20-1) + p64(read) → [观察] 第二次 send 写 p32(0xF1EEEE2D)*9 + p64(door) → [下一步] 触发后门'
- '[触发点] minceraft 题菜单菜单游戏 + World Name 栈溢出 → 假设：覆盖返回地址到 read gadget → [动作] payload = b''\x00''*(0x40) + p64(0x404a40) + p64(0x40114D) + p64(0x0000000000401016) + p64(0x401243) → [观察] 拿到 libc_base → [下一步] system(''/bin/sh'')'
- '[触发点] lucky-flag web 题 = let enc = JSON 字符串 → 假设：JSON.parse 反复解析直到稳定 → [动作] while: try enc = JSON.parse(enc) catch → [观察] 解到字符 → [下一步] XOR 0x62 → flag'
- '[触发点] 多 pwn/web/reverse 综合 + 自定义 VM tradre → 假设：VM 字节码 = 结构体数组 → [动作] stru_base = 0x606AC0, 32*181 步长读 → [观察] basic_block_stru_t 数组 → [下一步] dfs topo 排序'
- '[触发点] state_trans CALL/EXT_CALL/JMP/JLE/JNE/JE/RET → 假设：dfs 拓扑排序恢复控制流 → [动作] capstone disasm + keystone 重定位 → [观察] recovered.elf → [下一步] IDA 分析'
- '[触发点] MMXEncode packed_decrypt → 假设：减 key → 循环右移 1 → 2 字节交换 → xor key 4 步组合 → [动作] def packed_decrypt(x) → [观察] 解出 flag{butter_fly_mmx_encode_7778167}'
- '[触发点] XSLT 内存马 + OGNL 注入 → 假设：Java 反序列化 → [动作] TemplatesImpl → XSLT → RCE → [观察] 内存马注入'
failed_attempts:
- 试图直接 system → 失败：buffer 不够必须栈迁移
- 试图不走 0xF1EEEE2D 后门 → 失败：题目设计必须这个魔数
- 试图用 World Name 直接覆盖 → 失败：必须先泄露栈地址
key_observations:
- 格式化串 `%N$p` 泄栈 + 小端序 hex 转 flag 是入门 pwn 第一招
- 栈迁移要算准 `target_addr - 1` 让 rbp 写入后 leave 触发跳
- World Name 等菜单 pwn 漏洞常出在覆盖返回地址后接 read gadget
- JSON.parse 反复解析 + XOR 是 CTF misc 经典组合
prerequisites:
- 格式化串漏洞利用（%N$p 泄栈）
- 栈迁移 + 后门地址触发
- pwntools libc 偏移计算
- JavaScript JSON.parse 反复解析
---
