...
quality: medium
full_path: 第三届广东省大学生网络攻防竞赛WriteUp.full.md
meta_path: 第三届广东省大学生网络攻防竞赛WriteUp.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: 第三届广东省大学生网络攻防竞赛WriteUp。第三届广东大学生网安赛：php://filter文件包含+phar反序列化。经验：phar协议反序列化是PHP文件上传常见漏洞链
category: web
...
contest_type: school
wp_url: https://www.ctfiot.com/181138.html
reasoning_chain:
- WEB 消失的 flag 提示 'flag 隐藏在文件中' → 触发点：php://filter 文件包含
- 动作：/?file=php://filter/convert.iconv.UTF-8.UCS-2/resource=/flag → 观察：XFF 头 + UTF-8 转 UCS-2 读 flag
- 第二题目录扫描发现 www.tar.gz 备份文件 → 触发点：代码审计入口
- 动作：解包 www.tar.gz → 审计反序列化 + phar 协议 → 假设：phar 生成 + __destruct 触发
- 动作：写 exp.phar setStub('GIF89a'.'<?php __HALT_COMPILER(); ?>') + setMetadata(new File()) → gzip 压缩 + 改 3:{→4:{ + 改签名 → 上传
- 假设：上传文件用 phar:// 协议解析触发 __destruct → 动作：构造 include('phar://res.png') 触发
- mypdf 题注册用户 invite=-3.3e99999999999999 绕过两种 JSON 解析 → 假设：服务端 fallback 解析
- 假设：title 插入 gopher://127.0.0.1:8082 SSRF + Content-Type text/css → 动作：内网 /users 注册 test456 → 触发 admin 访问
- MISC 猜一猜 → 动作：压缩包密码是名字 md5 解密 → winhex 修复图片头 → 扫描二维码 → 花朵解密
- 你要的就在这 → 假设：图片尾 'stegpy' 隐写 + Π 3.1415 + luckyone key → DES CBC vi=1 → flag{believe_you_are_lucky}
- encipher RSA 给私钥 → 动作：pow(c,d,n) 解密 + strxor 长 key 拼接 → flag{1s_Pa33w0rd_1y2u22}
failed_attempts:
- 直接 cat /flag 读文件 → 失败：web 1 是 php://filter 不是命令执行
- 直接 phar:// 解析上传的 phar → 失败：服务端验签名 hash，必须改 3:{→4:{ + 重算 sha1
- mypdf invite 写 0/null → 失败：必须 -3.3e99999999999999 才能触发 JSON fallback
key_observations:
- php://filter/convert.iconv.UTF-8.UCS-2/resource=/flag 是 UTF-8 转宽字符读文件经典技巧
- phar 反序列化三步走：setStub 伪装 + setMetadata 触发类 + 改 3:{→4:{ 绕过 __wakeup
- JSON 解析 bypass：-3.3e99999999999999 让 float 转 int 失败触发 fallback
- gopher:// 协议打内网是 SSRF 王牌，配合 302 跳转实现 click→backup→exec 链
- RSA 给私钥 + strxor 简单密钥 → pow(c,d,n) 直解是 CRYPTO 入门套路
prerequisites:
- php://filter 协议链（convert.iconv 系列）
- phar 反序列化（setStub + setMetadata + 签名修改）
- gopher:// 协议 SSRF 构造
- DES/AES 对称加密 + strxor 长密钥拼接
---
