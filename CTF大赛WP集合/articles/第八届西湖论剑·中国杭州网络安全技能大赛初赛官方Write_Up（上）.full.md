...
tags:
- Crypto-随机数shuffle
- 预言机+Coppersmall_roots
- 矩阵RSA+non-commutative
- QuarticRing
- DigitalSig-DSA篡改
- 内存取证Win7SP1
- AntSword编码
...
quality: high
full_path: 第八届西湖论剑·中国杭州网络安全技能大赛初赛官方Write_Up（上）.full.md
meta_path: 第八届西湖论剑·中国杭州网络安全技能大赛初赛官方Write_Up（上）.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: 第八届西湖论剑·中国杭州网络安全技能大赛初赛官方Write Up(上)。第八届西湖论剑初赛官方上半:420所高校758队3960人,涵盖已悟(sha256+shuffle预言机)+matrixRSA(p0+Coppersmall_roots+矩阵RSA+non-commutative)+NewYearRing4+DSASignatureData(DSA verify识别篡改)+easyraw...
category: misc
...
contest_type: open
wp_url: https://www.ctfiot.com/225639.html
reasoning_chain:
- 触发点：已悟题给 SHA256(key) 当 random seed + 16 字节 shuffle 验证 → 假设：固定 seed → 可逆 shuffle
- 动作：枚举 10万 key，每个 seed+shuffle 验证 S1[i] == S1.index(S1[i]) and S1[id]+id == 15 → 观察：key 命中即输出 S1
- matrixRSA 触发点：n 是 100bit × 矩阵 → 假设：low-bit 攻击 Coppersmith
- 动作：p0*2^100+x 做 PR.<x> small_roots(beta=0.3) → 观察：解 x=low 100bit → 还原 p
- matrix non-commutative 触发点：乘积不是可交换 → 假设：设未知向量解矩阵方程
- DSA 篡改：触发点：给出 r/s/公钥 → 假设：r 重用
- 动作：找到 (r1,s1) (r2,s2) 用同一 r → 观察：解 k = (h1-h2 s1 / s2) / (s2-s1) mod q → 还原私钥
- QuarticRing RSA：触发点：n 是 4 次扩域多项式 → 假设：扩域 gcd 分解
failed_attempts:
- 已悟题 brute force 所有 16! 排列 → 失败：太大
- matrixRSA 直接分解 → 失败：n 不是普通整数
- DSA 试图穷举 k → 失败：k 在 Fp 太大
key_observations:
- hash+seed 固定 → shuffle 可重放，整数解线性约束 (S1[i] == S1.index(id))
- Coppersmith 小根攻击：已知 p 高位 p0+2^kbits+x 时 small_roots(X=2^kbits) 可解
- DSA 同一 r 两次签名 → 直接线性方程解 k
- 矩阵 RSA 不交换律：要按矩阵运算顺序构造方程
prerequisites:
- Python random.seed 确定性 + shuffle 顺序
- Sage Coppersmith small_roots 用法
- DSA 签名方程推导
- 扩域 PolynomialRing(GF(p^2)) 数学
---
