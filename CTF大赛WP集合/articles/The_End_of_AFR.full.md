...
quality: high
full_path: The_End_of_AFR.full.md
meta_path: The_End_of_AFR.meta.md
images_removed: true
images_removed_count: 3
schema_version: v3.0.0-P0
summary: The End of AFR? (php://filter 终极玩法)。php://filter 终极技巧 The End of AFR：使用 CSUNICODE/UCS-4LE 字符集链 + dechunk 单字符探测 + base64 截断，绕过几乎所有 disable_function + open_basedir，任意文件读取无限制。。经验：dechunk 是 php://filte...
category: misc
...
contest_type: open
wp_url: https://www.ctfiot.com/59318.html
reasoning_chain:
- file_get_contents('php://filter/dechunk/resource=data:,a') → 触发点：dechunk 是单字符探测神器
- 假设：dechunk + base64-encode + iconv 链可逐位爆破任意字符 → 动作：测试 dechunk+iconv+base64-encode 链
- 观察：convert.iconv.CSUNICODE.UCS-2BE 2 字节 swap → convert.iconv.UCS-4LE.10646-1:1993 4 字节 r4 移位
- 假设：CSUNICODE.CSUNICODE 产生填充字符 → 组合 base64-encode 截断冗余 → 触发点：实现全字符测试集
- 动作：探测每一位字符 (0-3 -> M / 4-7 -> N / 8-9 -> O / 0->CDEFGH / 1->STUVWX / 2->ijklmn / 3->yz*)
- 观察：能区分全部 256 字节 → 实现任意文件读取，绕过 disable_function + open_basedir
failed_attempts:
- 试图单 base64-encode 探测 → 失败：单次编码区分度不够（仅区分 64 种字符）
- 试图 utf7/utf8 转换 → 失败：iconv 字符集链需要精确组合
key_observations:
- dechunk 是 php://filter 中单字节探测的王者，逐位爆破任意文件内容
- convert.iconv CSUNICODE 链加 base64-encode 截断可构造全字符测试集
- UCS-4LE 的 r4 移位是 4 字节重排关键
prerequisites:
- php://filter 协议流（resource / read / write）
- iconv 字符集转换（CSUNICODE / UCS-2BE / UCS-4LE）
- base64-encode 截断机制（字符填充对齐）
---
# The End of AFR?
