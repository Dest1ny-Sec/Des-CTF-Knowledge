...
quality: high
full_path: 复盘第五期_-突破15-16题的关键，解题技巧倾囊相授-简慕.full.md
meta_path: 复盘第五期_-突破15-16题的关键，解题技巧倾囊相授-简慕.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: 斟茶王者第五期复盘 - 简慕(始兴公安)突破15/16题。始兴公安简慕16题突破后13题原创,核心亮点:Electron XSS→RCE反弹shell。。经验：调证/重明/二维码/反编译/数据库/AES/电子取证/模拟器抓包/Electron XSS→RCE 完整链条演练;XSS...
category: misc
...
contest_type: open
wp_url: https://www.ctfiot.com/201679.html
reasoning_chain:
- '斟茶王者第五期复盘 (简慕/始兴公安) 突破 15/16 题, 1-14 原创 → 触发点: 调证/重明/二维码/反编译/AES/取证全链路'
- '假设: 模拟调证平台调取 NH1782952295925185 + NH1719788591914672 转账记录 → 动作: 平台查询 → 观察: NH1719788591914672 转出合计最多账户是答案'
- 'Q5 浏览器开发者模式抓包 → 触发点: 浏览器自带的 dev tools 抓包 → 假设: 解题新思路不靠 jadx'
- 'Q11 AES-128 key=4JQVC6Y3DNICA8NYBVKFLQQG0E6J89UT → 假设: key 在 manifest 或 SDK → 动作: AES/CBC/PKCS5Padding 解密聊天'
- 'Q13 模拟器抓包改日期 → 群聊分析 + 电子取证 → 触发点: 模拟器是必备工具'
- 'Q15 Electron XSS: <object data:text/html;base64> 注入修改用户名 → script 过滤但放过 object → 假设: 浏览器渲染 object data → 动作: require(''child_process'').exec 反弹 shell'
failed_attempts:
- '试图用 script 标签 XSS → 失败: 过滤了'
- '试图不开内网穿透 → 失败: 必须 NATAPP TCP 隧道'
key_observations:
- 调证平台查一级卡 + 重明查卡号 + 二维码解析 URL + 浏览器开发者模式抓包 + APK 反编译找 AES key + 数据库备份下载+恢复 + 模拟器抓包改日期 + 群聊分析 + Electron XSS→RCE 完整链条演练
- XSS 注入点选修改用户名 (过滤 script 但放过 object), 用 data:text/html;base64 绕过
- Electron 漏洞通过 require('child_process').exec 触发系统命令
- AES-128 key 是 32 hex (4JQVC6Y3DNICA8NYBVKFLQQG0E6J89UT)
prerequisites:
- 调证 / 重明 / 二维码 / APK 反编译 / 数据库备份 / AES 解密 / 模拟器抓包 / Electron XSS→RCE 链条演练
- 浏览器开发者模式抓包 (替代 jadx)
- object data:text/html;base64 绕过 script 过滤
- require('child_process').exec RCE
---
