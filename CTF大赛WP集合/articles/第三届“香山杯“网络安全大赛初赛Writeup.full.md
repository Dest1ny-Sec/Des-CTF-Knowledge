...
quality: high
full_path: 第三届“香山杯“网络安全大赛初赛Writeup.full.md
meta_path: 第三届“香山杯“网络安全大赛初赛Writeup.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: '第三届香山杯网络安全大赛初赛 Writeup。香山杯初赛 4 题：PHP 反序列化 POP 链绕 preg_match + rot23 + 内存 dump ou.exe + Chaquopy XXTEA 9 round。。关键路径：Web PHP_unserialize_pro: Welcome.__destruct (name=''A_G00d_H4ck3r'') → echo arg → H...'
category: misc
...
contest_type: open
wp_url: https://www.ctfiot.com/139097.html
reasoning_chain:
- 题目给出 PHP 源码 Welcome/H4ck3r/G00d 三类 + 反序列化入口 → 触发点：标准 PHP POP 链题
- 假设：Welcome.__destruct → echo $this->arg → 触发 __toString → 假设：arg 是 H4ck3r 类
- 动作：构造 H4ck3r.func = G00d 实例 → 触发 H4ck3r.__toString → 调用 $this->func() → 触发 G00d.__invoke
- 观察：G00d.__invoke 内 preg_match 黑名单 '/f|l|a|g|*|?/i' → 假设：必须 glob 字符类绕 → 动作：用 sort /[!q]1[!q][!q] 绕
- 动作：cmd='sort /[!q]1[!q][!q]' + shell='system' → 观察：成功 eval('system(sort /[!q]1[!q][!q])') → 读 flag
- MISC 签到 base64 解一次 + rot13 + rot23 → 观察：iodj{zh1f0p3_2_Fwi}
- RE URL从哪儿来 → 触发点：32 位无壳 exe → 动作：动调从内存导出 ou.exe + base64 解密
- RE hello_py Chaquopy 技术调用 Python → 触发点：嵌入式 Python 字节码 → 动作：提取 hello.py + 识别 XXTEA 变种
- 动作：encrypt(n, v, key) delta=0x9e3779b9 + rounds=6+52//n + MX 函数 4 参数 + key_str 9 个 uint32 → 观察：decrypt(9, v, key) 还原 flag
failed_attempts:
- 试图用 cat flag 走 G00d.__invoke → 失败：preg_match 黑名单含 f/l/a/g/* 全部关键字
- 试图用系统命令 dir / ls → 失败：题目不报文件列表直接 eval($shell($cmd))
- 试图 XXTEA 标准解密 hello_py → 失败：delta rounds MX 都是变种参数
key_observations:
- PHP 反序列化 POP 链经典三件套：__destruct → echo → __toString → __invoke
- preg_match 黑名单绕用 glob 字符类 [!q] 排除特定字符 + 通配符组合
- Chaquopy 嵌入式 Python 调用 = 静态资源中提取 hello.py 再解密
- XXTEA 变种看 delta + rounds + MX 函数参数即可识别
- RE URL题常见 '内存导出 + base64 解密' 双步套路
prerequisites:
- PHP 反序列化 POP 链构造（__destruct/__toString/__invoke）
- preg_match 黑名单绕（glob 字符类 / 通配符）
- XXTEA 算法变种识别（delta/rounds/MX）
- Chaquopy 嵌入式 Python 提取（静态资源分析）
---
