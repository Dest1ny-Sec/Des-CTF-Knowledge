...
- house_of_minho
- fake_io_list_all_wfile_jumps
- custom_shellcode_rop
- open_read_write_flag
- double_arb_write_loop
...
quality: high
full_path: '[2024长城杯初赛]_Pwn题SomeHash_SomeTime_shutup题解.full.md'
meta_path: '[2024长城杯初赛]_Pwn题SomeHash_SomeTime_shutup题解.meta.md'
images_removed: true
images_removed_count: 0
schema_version: v3.0.0-P0
summary: '[2024 长城杯初赛] Pwn 题 SomeHash SomeTime shutup 题解。2024 长城杯初赛 3 道 Pwn：SomeHash fmt-string 栈泄露 + %hn 任意地址写 ROP + SomeTime 堆菜单 house_of_minho _IO_list_all 攻击 + shutup 自定义 shellcode+ROP 读 flag。。经验：fmt-str...'
category: pwn
...
contest_type: open
wp_url: https://www.ctfiot.com/170259.html
reasoning_chain:
- 触发点:SomeHash name 长度 -0x98 + 触发 fmt-string → 假设:%6$p %19$p %21$p 泄栈/libc/elf → 动作:payload = b'xxx>%6$p->%19$p->%21$p-'
- 观察:泄栈/libc/elf → 下一步:用 %hn 写大值 0x100 → cnt 偏移 0x5078
- 动作:多次 %23$hn 写 stack_target → 观察:栈布局控制成功
- 动作:%53$hhn 单字节 6 次写 pop_rdi+ret+/bin/sh+system → 观察:ROP 链布置完成
- 触发点:SomeTime 堆菜单 add/free → 假设:可堆 OOB → 动作:循环 add/free 0x30-0x70+0xa0-0xf0 切割 unsorted bin 残余
- 动作:add 0x70 覆盖 size=0x5e1 fake chunk → add 0x100 + show 泄 libc → 观察:成功泄 libc_base
- 动作:多次 free 触发 _IO_list_all 攻击 + house_of_minho fake_file_addr → 观察:shell 触发
- 触发点:shut up 走 shellcode+ROP → 假设:open/read/write flag shellcode 拼接 → 动作:拼到 ROP 链尾部 → 完成
failed_attempts:
- 试图直接 ret2libc → 失败:libc 2.35 缺传统 hook
- 试图 tcache poisoning → 失败:glibc tcache safe linking 防 simple poison
- 试图单字节 %hhn 一次写 → 失败:64 位地址需多次 %hn 拼接
key_observations:
- fmt-string 多次 %hn 写 stack 后用 %hhn 单字节写是经典的 '64 位地址任意写' 模板
- house_of_minho 利用 0x70+ 小于等于 0x80 触发 _IO_list_all 攻击是 2024 新型 _IO 攻击
- SomeHash fmt 写 6 字节 pop_rdi+ret+/bin/sh+system 是 fmt-string PWN 标配
- SomeTime 切割 unsorted bin 残余 0x30-0xf0 是 house_of_minho 的必备前置
- size=-1 整数溢出 + 2048+ 字节 payload 是栈溢出 ROP 通用套路
prerequisites:
- fmt-string 漏洞利用 (%p 泄 / %hn 写 / %hhn 单字节写)
- house_of_minho / _IO_list_all 攻击原理
- ROP 链构造 (pop_rdi+ret+/bin/sh+system)
- glibc 2.35 堆利用 (tcache safe linking / 切割 unsorted bin)
---
