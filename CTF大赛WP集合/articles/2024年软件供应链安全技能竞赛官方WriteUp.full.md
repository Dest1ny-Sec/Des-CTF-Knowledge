...
- web_unknown
tags:
- 禅道 Zentao RCE v18.0.beta1
- YARN REST API new-application
- 向日葵 RCE CVE-2022-10270
- ThinkPHP5 invokefunction
- jumpserver Celery 模板注入
- Docker Registry 5000/_catalog
- Socks5 代理
- frida Java.choose
- docker pull 私有镜像
- Bash /dev/tcp 反弹
- ansible delegate_to localhost
- Apache Druid 命令执行
attack_chain:
...
quality: high
full_path: 2024年软件供应链安全技能竞赛官方WriteUp.full.md
meta_path: 2024年软件供应链安全技能竞赛官方WriteUp.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: '2024 软件供应链安全技能竞赛官方 WriteUp（4 网段 14 flag 综合渗透）。14 flag 全网段综合：禅道/YARN/向日葵/ThinkPHP/Docker Registry/jumpserver 模板/ThinkPHP 链——把企业软件开发-测试-生产-运营商四段打通到底。。关键路径：flag1: 禅道 Zentao v18.0.beta1 captcha RCE + c...'
category: web
subcategory: rce
subcategories:
- rce
...
contest_type: open
wp_url: https://www.ctfiot.com/207510.html
reasoning_chain:
- 触发点：靶场 4 网段 14 flag → 假设：横向渗透 → 动作：nmap 扫所有网段端口
- 动作：flag1 禅道 v18.0.beta1 captcha RCE → curl /tmp/a + chmod + /bin/bash → 拿 flag1
- 动作：cat /www/zentaopms/config/my.php 拿 admin@mail.yym1ng.com/1qazcde3!@# → flag2
- 触发点：YARN REST API 8088 → 假设：new-application → 动作：am-container-spec command bash -i /dev/tcp → 拿 flag3
- 动作：ftp wsfxsdfrsaqwczdsa 192.168.10.55 windows → flag4
- 触发点：向日葵 sunlogin_rce CVE-2022-10270 → 动作：reg fDenyTSConnections=0 + Guest 启用 → flag5
- 动作：yujianxg MZCiP6gfusWj3 → flag6
- 触发点：frida Java.perform Java.choose java.lang.String dump凭据 → flag7
- 动作：admin/123456 默认口令 → flag8
- 触发点：ThinkPHP s=/api/thinkapp/invokefunction call_user_func_array file_put_contents → flag9
- 动作：ThinkPHP passthru bash base64 → flag10
- 触发点：Docker registry /v2/_catalog + insecure-registries + auth=base64 → flag11
- 动作：admin gygsud7gbskdbbxd3rtqak → flag12
- 触发点：jumpserver /manage/script 模板注入 bash base64 ansible delegate_to=localhost → flag13/14
failed_attempts:
- 试图只打单个网段 → 失败：14 flag跨 4 网段必须横向
- 试图向日葵旧版 RCE → 失败：题目用 sunlogin_rce CVE-2022-10270
- 试图不打 ansible delegate_to=localhost → 失败：必须本机 localhost 反弹
key_observations:
- 软件供应链靶场核心：开源自带漏洞 + 默认口令 + 内网横向
- 禅道/向日葵/jumpserver 几乎年年有未授权 RCE
- YARN REST API 默认 8088 暴露就送 shell
- Docker Registry 5000 端口 + auth=base64(user:pass) 是无认证的私有镜像金库
prerequisites:
- nmap 端口扫描 + 横向渗透思路
- 禅道 / 向日葵 / jumpserver / ThinkPHP 历史 CVE 利用
- YARN REST API + Docker Registry 利用
- frida hook + Java.choose 凭据 dump
---
