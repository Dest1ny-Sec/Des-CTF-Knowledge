...
- 纸质密码信封 第二张纸
- 过度紧张 基础薄弱
- PHP likeadmin/admin 默认密码
- 后台任意文件上传 UploadController file()
- 123.php multipart/form-data
- 未授权重置管理员密码 auth.admin/edit
- 用户操作未鉴权
- ThinkPHP 鉴权漏洞
- awd 一题多漏洞
- 离线资料包 应急
...
quality: medium
full_path: 2025年广西网络与信息安全职业技能竞赛决赛_awd_web部分_赛后WriteUP以及自我检讨.full.md
meta_path: 2025年广西网络与信息安全职业技能竞赛决赛_awd_web部分_赛后WriteUP以及自我检讨.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: 2025 广西网络与信息安全职业技能竞赛决赛 AWD Web 复盘（断网惨痛教训+ThinkPHP+upload+jwt）。2025 广西决赛 AWD Web 复盘：纸质密码信封错过 + PHP likeadmin 默认 + UploadController 上传 + 未授权改密 + ThinkPHP 鉴权绕过。断网教训。。关键路径：默认密码 admin/likeadmin（很多人试不成功，...
category: web
...
wp_url: https://www.ctfiot.com/283303.html
reasoning_chain:
- 触发点：AWD 线下赛断网 + 纸质密码信封 2 张纸 → 假设：第二张是 SSH 密码 → 动作：再读信封补读出 SSH 密码
- 观察：SSH 密码错过导致开局完全没拿到机器 → 下一步：复盘赛后漏洞
- 触发点：默认 admin / likeadmin 密码 + 锁 5 次错 30 分钟 → 假设：仍可登录但慢爆破 → 动作：登录进入后台
- 观察：UploadController.file() 任意文件上传 → 假设：multipart form-data filename=123.php → 动作：POST /adminapi/upload/file 上传 PHP
- 触发点：后台 /adminapi/auth.admin/edit 不鉴权 → 假设：ThinkPHP 框架删除 token 仍可操作 → 动作：POST {"id":1, "password":"admin1"}
- 下一步：拿到 admin1 重置密码登后台 + 上传 PHP + 改 admin
failed_attempts:
- 试图用 Burp 扫所有路由 → 失败：AWD 断网没字典
- 试图依赖 Wi-Fi/AI 在线补漏洞 → 失败：断网环境，必须熟记漏洞特征
- 试图找 PHP type juggling 一键登录 → 失败：本题是默认密码 + 上传 + 未授权组合
key_observations:
- AWD 一题多漏洞：默认密码 + 上传 + 未授权编辑并存，不能只审一个入口
- ThinkPHP 框架鉴权绕过常见 = 删除管理员 token 仍能操作（路由缺 middleware 校验）
- 赛前必备：本地代码审计脚本、上传 webshell 工具、ssh 弱密码字典、离线漏洞库
- 纸质材料必读：AWD 一般会发纸质密码信封 + 多张说明纸，要逐张看完
- 看到 file() 方法就要想到上传，看到 /auth 就要想到 JWT/未授权
prerequisites:
- AWD 比赛流程（断网 + 限时防御 + 攻击其他队伍）
- ThinkPHP 鉴权漏洞理解
- UploadController 类任意文件上传定位
- Burp + curl multipart/form-data 手工发包
---
