...
tags:
- php_trick_md5_substr
- control%0a绕preg_match
- str_replace_data://绕
- USB键盘流量
- SlientEye隐写
- AES秘钥爆破
...
quality: high
full_path: 2023年江苏省信息通信行业网络与信息安全职业技能竞赛决赛–WriteUp.full.md
meta_path: 2023年江苏省信息通信行业网络与信息安全职业技能竞赛决赛–WriteUp.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: '2023 年江苏省信息通信行业网络与信息安全职业技能竞赛决赛 – WriteUp。江苏信息通信综合 7 题：php_trick 多重绕+USB 流量+SlientEye+AES 爆破+LCG+RSA+花指令。。关键路径：Web php_trick: substr(path, 32) === md5(path) + path[] 数组绕过 → con=control%0a 绕 /^contro...'
category: web
...
wp_url: https://www.ctfiot.com/147695.html
reasoning_chain:
- 触发点：php_trick substr(path, 32) === md5(path) → 假设数组绕过 → 动作：path[]=1
- actor con=control%0a 绕 /^control$/ 正则但 !== control → 假设：换行字符绕过
- actor flag=dadata://ta://text/plain;base64,Y3RmZXI=绕 str_replace → 假设：data 协议拆分
- 动作：file_get_contents($b) === 'ctfer' → 观察：拿到 flag
- actor USB 键盘流量 vim flag.txt → 假设：tshark 提取 HID 数据 → 动作：9f0adccb...ff0c2f091e49a5dd96
- actor SlientEye 隐写 →观察：flag{8a497aff3184d4b33000c44150760559}
- actor AES 秘钥爆破：key 前 15 位 1016Aes128L12l2 + 末位爆破 → actor LCG 6 组输出还原 a,b,m
- actor RE 花指令 patch jmp + 密文两位对调后 -32
failed_attempts:
- 试图手工爆破 32字节 AES key → 失败：前 15 位已知 + 末位 16 种枚举
- 试图单点 LCG 救 a,b,m → 失败：需 6 组连续输出解方程
key_observations:
- substr 数组绕过 + con %0a 绕正则 + dadata 协议绕 str_replace
- LCG 6 组输出即可还原 a,b,m 三参数
- USB 键盘流量 vim flag.txt 是护网取证基本功
- SlientEye 是 LSB 隐写经典工具
prerequisites:
- PHP 类型混淆与数组绕过
- USB 流量解析（HID 协议）
- LCG 线性同余算法还原
