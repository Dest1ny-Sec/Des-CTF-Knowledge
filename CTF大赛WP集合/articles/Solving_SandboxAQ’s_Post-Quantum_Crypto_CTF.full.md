...
quality: high
full_path: Solving_SandboxAQ’s_Post-Quantum_Crypto_CTF.full.md
meta_path: Solving_SandboxAQ’s_Post-Quantum_Crypto_CTF.meta.md
images_removed: true
images_removed_count: 0
schema_version: v3.0.0-P0
summary: Solving SandboxAQ's Post-Quantum Crypto CTF。SandboxAQ 后量子密码 CTF：Module-LWE 攻击 + Kyber-1024 PRNG 爆破 + X25519+Kyber768 混合 TLS 解密。。关键路径：rotMatrix poly cyclotomic=True 构造 X^N+1 上的旋转矩阵 → module(polys, 2...
category: crypto
subcategory: lattice
time_required: long
difficulty_score: 4
...
contest_type: open
wp_url: https://www.ctfiot.com/169657.html
reasoning_chain:
- 拿到 Module-LWE 公钥 (A,t) + 密文 u,v → 触发点：后量子密码攻击
- 假设：小 q=251 + 短 N=16 → 假设：s1,s2 ∈ {0,1}^16 可爆破
- 动作：DBDD_predict 估计 δ=1.045280 → 观察：error 范围确认
- NTL ZZ_pX + OpenMP 并行爆破 → 假设：找到 s1,s2 后 m = v - s*u^T 解密
- decompress 还原 flag bits → flag
- Kyber-1024 PRNG seed 24 bit → 假设：16777216 可暴力枚举
- 动作：循环 OQS_randombytes_nist_kat_init_256bit + OQS_KEM_kyber_1024_keypair 对比 test_public_key
- 找到 seed → ECDH x25519+Kyber768 → TLS 抓包解密 → flag
failed_attempts:
- 试图用 LLL 降维 → 失败：参数太小不需要 LLL
- 试图不解 PRNG 直接解 Kyber → 失败：必须先恢复 seed 才能 decaps
key_observations:
- 后量子密码实现若 PRNG 种子空间过小，仍可通过暴力枚举恢复 shared secret
- Module-LWE 在小参数 (N=16, q=251) 下 brute-force s ∈ {0,1}^N 可解
- Kyber NIST KAT 24 bit entropy_input 是测试向量，非安全实现
- X25519+Kyber768 混合 TLS (x25519kyber768draft00) 是 Cloudflare/Google 部署方案
prerequisites:
- LWE/Ring-LWE/Module-LWE 基础
- Pohlig-Hellman / Sage discrete_log
- Kyber-1024 NIST KAT 测试向量机制
- TLS 1.3 X25519Kyber768Draft00 混合密钥交换
---
