...
quality: high
full_path: DataCon_2023漏洞分析赛题二官方题解.full.md
meta_path: DataCon_2023漏洞分析赛题二官方题解.meta.md
images_removed: true
images_removed_count: 6
schema_version: v3.0.0-P0
summary: 'DataCon 2023漏洞分析赛题二官方题解。DataCon 2023漏洞分析题二：OpenWrt魔改+CodeQL参数特征查询。关键路径：OpenWrt源码魔改题目 → 设置3种固定漏洞模式 → 第一种: strcat第二参数是变量。经验：CodeQL+Joern按callee+index查询是漏洞分析高效模式'
category: misc
...
contest_type: open
wp_url: https://www.ctfiot.com/148134.html
reasoning_chain:
- OpenWrt 源码魔改 → 触发点：3 种固定漏洞模式（strcat/memmove/system/popen）
- 假设：strcat 第二参数是变量 = 可能溢出 → 动作：Cypher MATCH (n:identifier) WHERE n.callee='strcat' AND n.index=1
- 观察：查询到 2 条结果 → 下一步：人工验证
- 假设：memmove 3 参数 vs 1 参数无 dfg = size 未申请 → 动作：not (m)-[:dfg]-(n)
- 观察：约 20 条疑似 → 下一步：人工核验反编译代码
- 假设：system/popen 第一参数是变量 = 命令注入 → 动作：callee=system/popen, index=0
- 假设：VQL 污点追踪交叉验证 → 动作：MATCH (n) WHERE n.is_source=1 → CALL VQL.taintPropagation
- 观察：签到题 0x0804F571 ustpd 格式化字符串 → 假设：自定义 source 点提升召回率
failed_attempts:
- 试图只查 strcpy 不查 strcat → 失败：strcat 第二参为变量也是溢出源
- 试图靠默认 source/sink 找到所有漏洞 → 失败：自定义 source 必须手工标注
key_observations:
- CodeQL/Joern 按 callee+index 查询是漏洞分析高效模式
- memmove 第 3 vs 第 1 参数 dfg 缺失 = size 未按 len 申请
- VQL taintPropagation 配合默认 is_source/is_sink 可定位签到级漏洞
- 自定义 source 点（malloc size）可识别整数溢出
prerequisites:
- CodeQL/Joern cypher 语法
- VQL 污点传播 API
- OpenWrt 项目结构（odhcpd 等）
- CVE 漏洞模式识别
---
