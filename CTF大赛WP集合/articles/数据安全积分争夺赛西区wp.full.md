...
quality: high
full_path: 数据安全积分争夺赛西区wp.full.md
meta_path: 数据安全积分争夺赛西区wp.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: 数据安全积分争夺赛西区WP - 2.31 UAF+RC4魔改+表替换。数据安全积分争夺赛西区WP:libc-2.31 UAF+RC4魔改+表替换。。经验：libc-2.31 UAF经典:0x508 chunk进unsorted bin+0x38 tcache+__free_...
category: misc
...
contest_type: open
wp_url: https://www.ctfiot.com/240445.html
reasoning_chain:
- 题 3 数据安全：上传 phtml 文件 → 触发点：web 上传绕过后翻文件 → 观察：得 flag
- 题 5 libc-2.31 UAF → 触发点：菜单 add/free/edit/show → 假设：0x508 chunk 进 unsorted bin 泄 libc
- 动作：add(0x508) + free + show → 假设：能算 libc 基址
- 动作：0x38 tcache edit 改 __free_hook = one_gadget 0xe3b01 → 观察：free 触发 one_gadget → shell
- 题 7 自解密 shellcode → 触发点：XOR 0xd3 魔改 RC4（key='103906d6c9429372' + init + 4 常数 + t=box[i]+box[j]+15）
- 假设：表替换 208EC37FD94165AB → 0123456789abcdef → 动作：字典逆替换得原文 → 观察：得 flag
failed_attempts:
- 试图直接 __free_hook 改 system → 失败：libc 2.31 one_gadget 0xe3b01 才匹配
- 试图不解魔改 RC4 → 失败：标准 RC4 不匹配
- 试图不解表替换字典 → 失败：表替换是确定性映射必须逆替换
key_observations:
- libc-2.31 UAF 经典：0x508 chunk 进 unsorted bin + 0x38 tcache + __free_hook 劫持
- one_gadget 0xe3b01 是 libc-2.31 经典 0xe3b01
- 自解密 shellcode 用 XOR + RC4 特征调试
- RC4 魔改 init+4 常数+exchange t=box[i]+box[j]+15
- 表替换 208EC37FD94165AB → 0123456789abcdef 字典逆替换
prerequisites:
- libc-2.31 UAF 堆利用（tcache + unsorted bin）
- one_gadget 工具使用
- XOR 自解密 shellcode 编写
- RC4 魔改识别 + 字典表替换
---
# 数据安全积分争夺赛西区wp
