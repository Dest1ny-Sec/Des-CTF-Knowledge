...
quality: high
full_path: 2023年第八届上海市大学生网络安全大赛Writeup.full.md
meta_path: 2023年第八届上海市大学生网络安全大赛Writeup.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: '2023 年第八届上海市大学生网络安全大赛 Writeup。上海大学生 8 题：cookie 窃取+vm2 sandbox+Spring 反序列化+无参数 RCE+Python 沙箱+KeyBox 整数溢出。。关键路径：CookieBack: 注释提示 send cookie to /cookie?data endpoint → /cookie?data=connect.sid=... 自取...'
category: web
...
contest_type: school
wp_url: https://www.ctfiot.com/117690.html
reasoning_chain:
- 触发点：CookieBack 注释提示 send cookie to /cookie?data endpoint → 假设窃取 cookie
- 动作：访问 /cookie?data=connect.sid=... → 观察：自取 cookie还原 flag
- actor easy_node vm2 3.9.16 sandbox escape → 假设：length 属性覆盖绕黑名单
- 动作：Error.toString Proxy + apply 拿到 process → 观察：RCE
- actor fun_java SpringBoot 原生反序列化 → 假设：AliyunCTF 2023 bypassit1 路线
- 动作：Jackson POJONode + BadAttributeValueExpException + TemplatesImpl → 观察：触发反序列化
- actor easy_log username[]=PHP 代码注入 → 假设：无参数 RCE → 动作：getallheaders()['Tao']=system
- actor PWN KeyBox 整数溢出 -2^63+0xc 绕 key 验证 + 堆重叠 → ssql off-by-one + 0x291 fake size
failed_attempts:
- 试图标准 vm2 沙箱逃逸 → 失败：3.9.16 需 Error+Proxy+apply 绕过
- 试图常规 Java 反序列化链 → 失败：AliyunCTF 2023 bypassit1 路线特殊
key_observations:
- vm2 3.9.16 用 Error+Proxy+apply 绕过
- Spring Boot 原生反序列化走 AliyunCTF bypassit1 路线
- 无参数 RCE 用 getallheaders() 拿 HTTP头触发 system
- 整数溢出 -2^63+0xc 绕 key 验证是 PWN 经典
prerequisites:
- vm2 沙箱逃逸原理
- Spring Boot 反序列化链
- PHP 无参数 RCE 技巧
