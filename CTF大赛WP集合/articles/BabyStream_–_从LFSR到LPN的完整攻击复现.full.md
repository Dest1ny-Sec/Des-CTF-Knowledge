...
quality: high
full_path: BabyStream_–_从LFSR到LPN的完整攻击复现.full.md
meta_path: BabyStream_–_从LFSR到LPN的完整攻击复现.meta.md
images_removed: true
images_removed_count: 0
schema_version: v3.0.0-P0
summary: BabyStream – 从 LFSR 到 LPN 的完整攻击复现。BabyStream LFSR 流密码 — 128位状态 + feedforward 4/16/63次非线性 + 线性化攻击 (矩阵 A·y 加 6.25% 噪声) + 多次随机子集 2000 次 + O(n) 优化 16384 倍加速 + 93.854% 准确率还原 flag。。关键路径：128 位 LFSR + feed...
category: crypto
subcategory: rsa
time_required: long
difficulty_score: 4
...
contest_type: open
wp_url: https://www.ctfiot.com/281936.html
reasoning_chain:
- '[触发点] 题目是 128 位 LFSR + feedforward 非线性 + 2048 字节输出 → 假设：恢复 128 位初始状态 / [动作] 分析线性部分 x[127] + Σ(x[i] ⊕ x[box[i]]) 覆盖 127 比特 / [观察] 线性部分齐次覆盖 / [下一步] 看非线性'
- '[触发点] 非线性: 4 次项 x[11]&x[22]&x[33]&x[53] (概率 1/16 = 6.25%) + 16 次 + 63 次 → 假设：93.75% 输出位只受线性影响 / [动作] 列矩阵 A ∈ GF(2)^(m×n) y = A·s + noise / [观察] 传统 O(n·128²) ≈ 268M 次运算 / [下一步] 优化'
- '[触发点] O(n) 优化: 列向量左移 → 行向量右移 + 反馈位注入 mask → 假设：复杂度从 268M 降到 16K / [动作] 写位运算代码 + 矛盾方程多次随机子集 / [观察] trials=2000, subset=128, 准确率 93.854% / [下一步] 取最高解'
- '[触发点] 矛盾方程（93.75% 干净 + 6.25% 噪声）→ 假设：多次子集投票可还原 / [动作] 重复 trials=2000 + subset=128 + 准确率筛选 / [观察] 100% byte 匹配 / [下一步] flag{ce2980ad9763b8468882f292dbe9f365}'
failed_attempts:
- 试图用 Gröbner 基 → 失败：完全非线性系统指数级
- 试图用传统 O(n·128²) 矩阵乘法 → 失败：太慢
key_observations:
- LFSR + 弱 feedforward = LPN 风格
- 线性化是 LPN 标准攻击
- 6.25% 噪声下需多次子集投票
- O(n) 矩阵乘法比 O(n²) 快 16384 倍
prerequisites:
- LFSR 流密码基础
- LPN 问题与线性化攻击
- GF(2) 线性代数
