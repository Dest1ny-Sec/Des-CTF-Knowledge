...
quality: high
full_path: 首届CCF智能汽车大赛(CCF_IVC_2025)_Mini-Venom(第一名).full.md
meta_path: 首届CCF智能汽车大赛(CCF_IVC_2025)_Mini-Venom(第一名).meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: 首届CCF智能汽车大赛(CCF IVC 2025) Mini-Venom(第一名)。首届CCF IVC 2025 Mini-Venom第一名:车联网pwn-3(0x88栈溢出+ret2libc)+easylogin(hash长度扩展)+serp(git+PHP反序列化SplFileObject读f1a9.php)+Curve(Pell方程x^2-Dy^2=1+ECDLP简化DLP+sieve...
category: pwn
...
contest_type: open
wp_url: https://www.ctfiot.com/263528.html
reasoning_chain:
- pwn-3 0x88 栈溢出 + write 函数 → 假设：write_got leak → 动作：payload=b'a'*0x88+p64(pop_rdi)+p64(1)+p64(pop_rsi)+p64(write_got)*2+p64(write_plt)
- 观察：libc_base=u64(recv(7).rjust(8)) → libc-2.27 + ret2libc
- easylogin → 触发点：?source=1 源码 → 假设：hash 长度扩展
- 动作：hash(secret+padding+append) + new_hash 算 token → 提交
- serp → 触发点：git 泄露 → 假设：posts.php 反序列化链
- 动作：new Maker('SplFileObject', ['f1a9.php', 'r']) → (new Post('test'))->setContent($a) → [$b]
- 观察：__toString 触发 __construct('SplFileObject',['f1a9.php','r']) → 读 f1a9.php
- Curve Pell 方程 → 假设：gx^2-D*gy^2-1 = ax^2-D*ay^2-1 移项 gcd
- 动作：p_approximate=gcd(kp1,kp2) → sieve_base[:20] 试除 → GF(p) iroot(D,2)
- 动作：离散对数 discrete_log(pell_a, pell_g) → share=pell_b^n_a
- 动作：shared_secret=(share+share_inv)/2 → sha256 long_to_bytes → AES-ECB 解
- nfsr → 假设：lfsr 128 位 + ff(x0,x1,x2,x3)=SHA512+输出 256 位
failed_attempts:
- 试图直接 ret2system → 失败：缺 libc base
- 试图不解 Pell 直接解 ECDLP → 失败：必须先求 p
key_observations:
- 车联网 pwn 标配：0x88 栈溢出 + write_got leak + ret2libc system
- hash 长度扩展攻击 hash(secret+padding+append) 是签名绕认证经典
- 'SplFileObject 反序列化读源: new Maker(''SplFileObject'', [''f1a9.php'',''r''])'
- Pell 方程解的 gcd 还原 p 是 ECDLP 简化题通法
- LFSR + ff 函数 (SHA512 折叠) + bool 约束爆破 = 流密码还原套路
prerequisites:
- ROP 链 pop_rdi/pop_rsi + write@plt leak
- hash 长度扩展原理（MD5/SHA1）
- PHP 反序列化 POP 链构造
- Pell 方程 / ECDLP / 离散对数 (Sage)
---
