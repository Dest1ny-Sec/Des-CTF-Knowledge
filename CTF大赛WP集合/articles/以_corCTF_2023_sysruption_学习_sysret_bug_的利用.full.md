...
quality: high
full_path: 以_corCTF_2023_sysruption_学习_sysret_bug_的利用.full.md
meta_path: 以_corCTF_2023_sysruption_学习_sysret_bug_的利用.meta.md
images_removed: true
images_removed_count: 7
schema_version: v3.0.0-P0
summary: corCTF 2023 sysruption SYSRET bug 利用学习。corCTF 2023 sysruption：Linux Kernel 6.3.4 删除 SYSRET canonical 检查 + EntryBleed 侧信道绕过 KASLR，内核 RCE。。关键路径：qemu-system-x86_64 -cpu host -enable-kvm + Linux 6.3.4 ...
category: misc
...
contest_type: open
wp_url: https://www.ctfiot.com/166547.html
reasoning_chain:
- 启动脚本 qemu-system-x86_64 -cpu host -enable-kvm + Linux 6.3.4 → 触发点：-cpu host 触发 EntryBleed
- 假设：EntryBleed 侧信道绕过 KASLR → 动作：观察 syscall 时序差
- 'Patch entry_64.S: 删除 SYSRET 前的 canonical address check'
- shl $(64 - (__VIRTUAL_MASK_SHIFT+1)), %rcx + sar + cmpq %rcx, %r11 + jne slow_exit
- '删除后 → SYSRET 返回 non-canonical RCX/RIP 在内核态 #GP'
- 假设：本质让用户接管内核 → 用户态 syscall → 内核态处理 → SYSRET 跳 non-canonical
- '动作：构造特定 non-canonical 地址 → 准备 #GP handler 用户态控制'
- '#GP 处理路径用户可控 → 内核代码执行 → 内核 RCE'
failed_attempts:
- 试图用传统 ret2usr → 失败：SMEP/SMAP 拦截
- 试图不改 entry_64.S 利用 SYSRET → 失败：默认有 canonical 检查
- 试图用 ret2dir → 失败：physmap 不可写
key_observations:
- SYSRET canonical check 是 Intel/AMD 长期存在的硬件 bug 缓解措施
- 删除后用户态可控制内核 RSP 触发内核执行
- EntryBleed (-cpu host) 是 KASLR 旁路经典侧信道
- 'non-canonical 地址 + #GP handler 是内核攻击金钥匙'
prerequisites:
- Linux Kernel 内核源码与 entry_64.S 流程
- SYSRET/SYSCALL 指令机制
- canonical address 概念（x86_64 虚拟地址规范）
- KVM/qemu 启动参数
---
