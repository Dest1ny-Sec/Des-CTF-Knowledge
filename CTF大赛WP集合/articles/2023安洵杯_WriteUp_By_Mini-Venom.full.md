...
quality: high
full_path: 2023安洵杯_WriteUp_By_Mini-Venom.full.md
meta_path: 2023安洵杯_WriteUp_By_Mini-Venom.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: '2023 安洵杯 WriteUp By Mini-Venom。6 道综合：CommonsBeanutils 反射 + Spring FreeMarker + RSA 位翻转 + AES padding oracle + CRT RSA。。关键路径：ezJava: 黑名单 (jndi/jackson/spring/JdbcRowSetImpl/SignedObject/TemplatesImp...'
category: web
...
contest_type: open
wp_url: https://www.ctfiot.com/153097.html
reasoning_chain:
- 触发点：ezJava 黑名单 jndi/jackson/spring/JdbcRowSetImpl/TemplatesImpl/Runtime/ProcessBuilder/PriorityQueue 全 ban → 假设换链
- 动作：走 PropertyUtilsBean 反射 + BeanComparator + TiedMapEntry → 假设 BadAttributeValueExpException.readObject
- 观察：TiedMapEntry.toString → LazyMap.get → BeanComparator.compare → PropertyUtilsBean.getProperty
- 动作：invoke Method → getConnection → PostgreSQL 攻击链 → 观察：JDBC 反连
- actor Spring FreeMarker springMacroRequestContext.webApplicationContext → 假设绕过沙箱
- 动作：setNewBuiltinClassResolver + ${Execute?new()(id)} 模板执行 → 观察：命令执行
- actor D0g3 RSA p[i]=0 改 1 试 q = p+2^(len-i-1) → 假设 gcd(q,n) 验证因式分解
- actor AES padding oracle pt[::-1] 后 N 个 num 字符结尾则 True → 假设逐字符爆破
failed_attempts:
- 试图用 JdbcRowSetImpl JNDI 注入 → 失败：黑名单拦截
- 试图用 TemplatesImpl 字节码加载 → 失败：TemplatesImpl 被 ban
key_observations:
- 黑名单 ban反射 + Runtime + TemplatesImpl 改用 PropertyUtilsBean
- FreeMarker 用 springMacroRequestContext 绕沙箱
- RSA 位翻转 gcd(q,n) 验证是经典分解方法
- AES padding oracle 倒序爆破需逐字符验证
prerequisites:
- Java 反序列化 POP 链（CommonsBeanutils）
- FreeMarker SSTI 与 Spring 集成
- RSA 数学与 gcd 因式分解
- AES padding oracle 攻击
---
