...
quality: high
full_path: 利用msg-msg结构体进行提权.full.md
meta_path: 利用msg-msg结构体进行提权.meta.md
images_removed: true
images_removed_count: 8
schema_version: v3.0.0-P0
summary: '利用msg-msg结构体进行提权。D3CTF d3kheap 内核提权：msg_msg+sk_buff+pipe_buffer 三结构堆喷交叉，pipe_buf_operations->release 劫持 ROP 提权。。关键路径：题目: 2022 D3CTF d3kheap 内核 PWN → 漏洞: ioctl 0x1234 分配 + 0xdead 释放, 1024 字节, 可释放两次 ...'
category: pwn
...
contest_type: open
wp_url: https://www.ctfiot.com/75106.html
reasoning_chain:
- 'D3CTF d3kheap 内核 PWN, ioctl 0x1234 分配 + 0xdead 释放 1024 字节, 可释放两次 → 触发点: UAF'
- 'msg_msg 结构体 0x30 头部 (含 m_ts 长度字段) + 用户消息, msgsnd/msgrcv 分配回收 → 假设: msg_msg 可作 0x400 大小块控制结构 → 动作: 堆喷 0x60 + 0x400 msg_msg'
- '观察: 中途释放 kheap 触发 UAF, 后续 0x400 占用同地址 → 下一步: 第二次释放制造 dangling pointer'
- 'sk_buff 喷射 1024-320=704 字节 → 假设: sk_buff 跨 kmalloc-1k 占位命中释放的 kheap → 动作: socketpair + sendmsg 多次'
- 'msgrcv MSG_COPY 读取 socketpair 命中 kheap → 触发点: sk_buff 改 m_ts=0x1000 越界读 → 下一步: 伪造 msg_msg->next 任意地址读'
- 'pipe_buffer 喷射命中 kheap → 读 pipe_buf_operations 泄内核基址 → pipe_buf_operations->release 劫持 ROP → 动作: PREPARE_KERNEL_CRED=0x810d2ac0 + COMMIT_CREDS=0x810d25c0 + POP_RDI_RET + swapgs_restore_regs_and_return_to_usermode → 完成'
failed_attempts:
- '试图直接 ret2usr → 失败: SMEP/SMAP 开启'
- '试图只用 msg_msg 不引入 sk_buff → 失败: msg_msg 0x30 头太小, 无法跨 0x400 块占位'
key_observations:
- 内核 UAF 三结构交叉堆喷 (msg_msg + sk_buff + pipe_buffer) 是现代 Linux 内核 PWN 标准姿势
- msg_msg 控制 0x400 大小块, sk_buff 跨 320 字节头占位, pipe_buffer 泄内核基址
- pipe_buf_operations->release 是劫持触发点 (close 时调用)
- PUSH_RSI_POP_RSP_POP_4VAL_RET 是栈迁移 ROP 必备 (5 函数组合)
prerequisites:
- Linux 内核 UAF 原理 (kmalloc cache + SLUB allocator)
- msg_msg IPC 机制 (msgsnd/msgrcv + MSG_COPY)
- sk_buff / pipe_buffer 内核对象结构
- ROPgadget 工具链
---
# 利用msg-msg结构体进行提权
