...
quality: high
full_path: real_world_ctf_2023_HappyFactory_and_realwrap.full.md
meta_path: real_world_ctf_2023_HappyFactory_and_realwrap.meta.md
images_removed: true
images_removed_count: 0
schema_version: v3.0.0-P0
summary: real world ctf 2023 HappyFactory and realwrap。RWCTF 2023 HappyFactory 区块链题，自定义 UniswapV2 复刻的 LP 攻击。。关键路径：自定义 KonohaFactory/KonohaPair → 复刻 UniswapV2 接口 → 提供 WETH/KP3R 流动性。经验：当 DEX 是 UniswapV2 复刻时，要...
category: web
...
contest_type: open
wp_url: https://www.ctfiot.com/90963.html
reasoning_chain:
- HappyFactory 合约 IKonohaFactory + IKonohaPair 复刻 UniswapV2 接口 → 触发点：feeTo/feeToSetter 是管理函数
- 假设：管理函数可调用 + 复刻版本可能有 mint/burn 漏洞 → 动作：通读 IKonohaPair.mint / burn / swap
- 观察：mint 计算 amount0/amount1 与 UniswapV2 一致但 balanceOf 检查略不同 → 下一步：尝试 LP 累积攻击
- 假设：先 deposit WETH + KP3R 到 SushiSwap 拿 LP，再用 flashloan 借 KP3R → 动作：removeLiquidity 抽走 token
- 观察：swap 操纵 KP3R 价格让 K=K*fee 失衡 → 下一步：再 removeLiquidity 抽更多 ETH+KP3R
- 动作：repay flashloan + 拿走剩下的 ETH → 完成
failed_attempts:
- 试图直接调 feeToSetter 修改 feeTo → 失败：feeToSetter 权限被限定
- 试图用 swap 抽干 KP3R/WETH 池 → 失败：UniswapV2 k=x*y 守恒下 swap 抽不干净
key_observations:
- 自定义 UniswapV2 复刻时要特别留意 feeTo/feeToSetter 等管理函数是否可调用
- flashloan + 多笔 addLiquidity 累积 LP + 单币 deploy 操纵价格是经典三件套
- 复刻 UniswapV2 的 _mintFee 函数实现差异会导致 K 失衡可被利用
- impermanent loss 视角下攻击者构造的单边 swap 能拿到超额份额
prerequisites:
- Solidity 0.5/0.8 合约阅读
- UniswapV2 Pair 数学（k=x*y、mintFee、burn 计算）
- Solidity flashloan 调用模式
- Hardhat fork 主网调真实流动性池
---
