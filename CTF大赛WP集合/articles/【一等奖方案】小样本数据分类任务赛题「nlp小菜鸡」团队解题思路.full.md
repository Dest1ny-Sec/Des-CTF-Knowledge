...
quality: high
full_path: 【一等奖方案】小样本数据分类任务赛题「nlp小菜鸡」团队解题思路.full.md
meta_path: 【一等奖方案】小样本数据分类任务赛题「nlp小菜鸡」团队解题思路.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: 小样本专利数据分类 CCF BDCI 2022 一等奖方案。5 个中文+英文预训练模型集成 + 领域预训练 + AWP/SWA + 伪标签，小样本专利分类 CCF BDCI 2022 一等奖。。关键路径：数据：958 条专利训练 + 20839 测试 + 20890 复赛，36 类脱敏标签，长尾分布 → 短文本（title+assignee+abstract 拼接 ≤ 657 字符） → 5...
category: misc
...
contest_type: open
wp_url: https://www.ctfiot.com/133251.html
reasoning_chain:
- 小样本专利分类 958 训练 + 20839 测试 → 触发点：长尾 36 类 → 假设：通用预训练 + 领域微调
- 动作：5 模型 Nezha / Ernie-Gram-zh / Ernie-3.0-xbase-zh / Bert-for-patents / Chinese-RoBERTa-wwm-ext-large
- 阶段 1：通用预训练 → 阶段 2：领域预训练（用测试集 MLM 增量）→ 阶段 3：领域微调
- Bert-for-patents 翻译英文 + 5 段输入 → 触发点：长专利文本 657 字符 → 动作：title+assignee+head+body+tail 分段
- AWP 双扰动（Embedding + model）+ SWA 局部最优集成 + Multi-Sample Dropout → 观察：单模型 +1.5% F1
- 集成：加权平均（Ernie-gram/xbase 权重小）+ 二阶段伪标签 → 动作：先训集成模型生成第一阶段伪标签
- 第二阶段伪标签：训练集 + 初赛 70% + 复赛 70% 训练 Nezha 10 折集成 → 观察：macro F1 = 0.61387745 第一
failed_attempts:
- 试图单模型直接 BERT → 失败：macro F1 仅 0.55，差第一 6 个点
- 试图用全部预训练模型等权集成 → 失败：Ernie-gram/xbase 权重需调小
- 试图只伪标签初赛数据 → 失败：复赛分布更广，必须两阶段都伪
key_observations:
- 小样本学习 = 数据增强（伪标签）+ 模型多样性（5 个不同架构）+ 鲁棒性优化（AWP+SWA+MSD）
- 三阶段训练（通用→领域 MLM→微调）是 NLP 小样本稳态流程
- Bert-for-patents 翻译英文 + 5 段输入解决长文本 657 字符上限
- AWP (Adversarial Weight Perturbation) 双扰动比单扰动鲁棒性更好
- 10 折集成 + 二阶段伪标签是 NLP 比赛冠军标配
prerequisites:
- PyTorch + Transformers 预训练模型使用
- AWP / SWA / Multi-Sample Dropout 优化技巧
- 伪标签 + 10 折集成 NLP 比赛经验
- Bert-for-patents 等专利领域模型
---
