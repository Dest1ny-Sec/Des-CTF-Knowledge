...
quality: medium
full_path: 铸网-2025”山东省工业互联网网络安全职业技能竞赛WriteUp（职工组）.full.md
meta_path: 铸网-2025”山东省工业互联网网络安全职业技能竞赛WriteUp（职工组）.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: '铸网-2025"山东省工业互联网网络安全职业技能竞赛WriteUp（职工组）。铸网-2025山东省工业互联网职业技能竞赛WriteUp,涵盖流量分析PK文件提取+CRC32爆破+PNG高度修改隐写+RE魔改XXTEA+自定义Base64方向。。关键路径：MISC安全流量: Net-A嗦哈解码得class数据→反编译按序拼接得PK文件头→恢复压缩包 → CRC32爆破得密码ICTF_so_I...'
category: misc
...
wp_url: https://www.ctfiot.com/270057.html
reasoning_chain:
- pcap Net-A 嗦哈解码成功 → 触发点：得到 class 数据需反编译
- 假设：class 反编译按序拼接得 PK 文件头 → 动作：按字节序拼接
- 观察：50 4B 03 04 ZIP 头 → 恢复压缩包 → 下一步：解压遇 CRC32 校验
- 假设：短密码 CRC32 爆破 → 动作：字典/字符爆破 6 位
- 观察：密码 ICTF_so_Intrest1ng 解压得 PNG
- 假设：PNG 高度被改藏 flag → 动作：随波 PNG 高度字段改大
- 观察：flag{08ca4a8d32bd08b13f260f224a834b75}
- RE 段 自定义 B64 表 ZYX…#$ → 假设：表替换 + XXTEA 魔改
- 动作：sum 递减 + key=abcdef9876543210 还原 XXTEA → 观察：everflag{cd00b495…}
failed_attempts:
- 试图不解码 Net-A 直接读 pcap → 失败：协议未知需专用解码
- 试图不解密就 cat PNG → 失败：CRC32 校验不过
key_observations:
- Net-A 嗦哈协议解码得到 class 二进制 = 字节拼接恢复文件的通用套路
- PNG 高度/宽度修改藏 flag 是图像隐写入门点
- XXTEA 魔改重点关注 sum 递增/递减方向 + delta + key 长度 + pack 模式
- 自定义 B64 表 + pack_string_le_with_len 是 RE 加密组合拳
prerequisites:
- Net-A/嗦哈等工控协议流量分析
- CRC32 爆破（hashcat/字典）
- PNG 文件结构（高度字段偏移）
- XXTEA 算法结构与 Python 还原
---
