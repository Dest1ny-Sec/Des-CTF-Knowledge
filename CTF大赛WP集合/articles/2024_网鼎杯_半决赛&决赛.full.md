...
quality: high
full_path: 2024_网鼎杯_半决赛&决赛.full.md
meta_path: 2024_网鼎杯_半决赛&决赛.meta.md
images_removed: true
images_removed_count: 8
schema_version: v3.0.0-P0
summary: 2024 网鼎杯 半决赛 & 决赛。p 高位 Coppersmith + LFSR+NFSR 序列密码 + 有限域同构 AES key 还原。。经验：Coppersmith 攻击当 p 已知 256/512 bit 时爆破 7 bit 是 38 分钟；有限域同构问题本质...
category: crypto
subcategory: lattice
subcategories:
- lattice
...
contest_type: open
wp_url: https://www.ctfiot.com/217849.html
reasoning_chain:
- 触发点：半决赛 RSA 已知 p 高 256 bit + (p>>248)<<248 → 假设：Coppersmith X=2^248 beta=0.49 → 动作：f = (p>>248)<<248 + x; f.small_roots(X=2^248, beta=0.49, epsilon=0.014)
- 观察：1.7% 成功率 → 假设：爆破 7 bit 还原 p 完整 512 bit → 动作：循环 i 验证
- 触发点：序列密码 LFSR(12bit seed/mask) + LFSR + LFSR + NOISE 12bit → 假设：每局 512 次请求 128 字节密文流 → 动作：多次采样
- 观察：noise bit = 1/3 prob when lfsrn=1, 2/3 prob when lfsrn=0 → 假设：求 noise 期望还原 → 动作：统计多次
- 触发点：还原 12bit 噪声 + 2 个 128bit LFSR → 假设：种子已知 → 动作：解 LFSR
- 观察：决赛 piff F_41^128 三个不可约多项式 f/F1/F2 → 假设：找 2 个同构映射 φ1/φ2 → 动作：构造 F1(a) = F2(b)
- 触发点：AES-ECB 加密 flag → 假设：复合 GCD 还原 primitive → 动作：GCD(φ1 - 1, φ2 - 1)
- 观察：二元域上取系数转 AES key → 假设：解 AES-ECB → 动作：Python decrypt
- 触发点：还原 AES key + ciphertext → 假设：解出 flag → 动作：decrypt
- 观察：所有 3 个子题完成 → 下一步：总结 Coppersmith + 序列密码 + 有限域
failed_attempts:
- 试图不解 Coppersmith X=2^248 beta=0.49 → 失败：必须 small_roots
- 试图不解 LFSR noise 统计 → 失败：必须多次采样求期望
- 试图不解 F_41^128 有限域 → 失败：必须复合 GCD 求 primitive
- 试图直接猜 7 bit → 失败：必须 small_roots 框架
key_observations:
- Coppersmith 攻击当 p 已知 256/512 bit 时爆破 7 bit 是 38 分钟（pwntools mbruteforce）
- 有限域同构问题本质是 GCD(φ1 - 1, φ2 - 1)
- LFSR + NOISE 12bit 多次采样求期望是流密码统计攻击
- 网鼎杯偏攻击 p 高位的论文级 Coppersmith 是出题趋势
- F_41^128 41 元域 128 次不可约多项式是代数密码高端考点
prerequisites:
- Coppersmith 小根攻击（sage small_roots）
- LFSR 流密码 + 噪声统计攻击
- 有限域同构 + 复合 GCD primitive 还原
- pwntools mbruteforce 多线程爆破
- AES-ECB 解密 + 系数转换
---
# 2024 网鼎杯 半决赛&决赛
