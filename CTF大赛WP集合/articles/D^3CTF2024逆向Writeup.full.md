...
quality: high
full_path: D^3CTF2024逆向Writeup.full.md
meta_path: D^3CTF2024逆向Writeup.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: D^3CTF 2024 逆向 Writeup。D^3CTF 2024 逆向 2 道：XTEA 变体解密 + IDA Python 递归函数图。。关键路径：XTEA 变体 delta=0xE8017300 → 32 轮减法 + 32 轮加法 → 暴力逆向还原 fakeflag。经验：XTEA 变体只需调整 delta 和轮数；IDA Python recur() 适合还原复杂调用图。
category: reverse
subcategory: reverse
tools_used:
- IDA
...
contest_type: open
wp_url: https://www.ctfiot.com/176701.html
reasoning_chain:
- XTEA 变体 delta=0xE8017300 → 触发点：delta 与标准 0x9E3779B9 不同
- 假设：32 轮减法 + 32 轮加法对称结构 → 动作：循环逆推 fakeflag
- v6 = v6 - (((v7 + ((v7<<5)^(v7>>6))) ^ (key[(a2>>11)&3]+a2) ^ 0x33) & 0xFFFFFFFF) → 触发点：XTEA 类算法
- 动作：flag 数组解密 → 观察：8 个 uint32 明文
- 下一个题：32 轮右移 1 位 + 0x84A6972F 异或 → 假设：可暴力逆向还原 fakeflag
- IDA Python recur：找函数关系图 → 动作：recur() 遍历函数引用
- 假设：xor 推导下一函数 → rand 关系图遍历 → IDA 静态分析完成
failed_attempts:
- 试图不改 delta 直接逆 XTEA → 失败：a2 = a2 + 0x100000000 - k3 顺序颠倒
- 试图 IDA Python 手动遍历函数 → 失败：函数图太大改用 recur()
key_observations:
- XTEA 变体只需调整 delta 和轮数，32 轮结构不变
- IDA Python recur() 适合还原复杂调用图
- XTEA-like 算法结构是 (v<<4)+k0 ^ (v+sum) ^ (v>>5)+k1
- fakeflag 在加密前先混淆再加密是 CTF reverse 套路
prerequisites:
- XTEA 算法原理
- IDA Python API + recur() 函数调用图
---
# D^3CTF2024逆向Writeup
