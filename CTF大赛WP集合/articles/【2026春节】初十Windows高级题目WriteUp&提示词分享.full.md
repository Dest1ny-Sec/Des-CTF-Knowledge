...
quality: high
full_path: 【2026春节】初十Windows高级题目WriteUp&提示词分享.full.md
meta_path: 【2026春节】初十Windows高级题目WriteUp&提示词分享.meta.md
images_removed: true
images_removed_count: 1
schema_version: v3.0.0-P0
summary: 【2026春节】初十 Windows 高级题目 WriteUp & 提示词分享。2026 春节初十 Windows 高级题，~95M 指令白盒密码 + MBA 混淆 + Unicorn 全指令模拟逆向。。经验：MBA(mixed boolean arithmetic) 表达式 n*(n+1) 或 n*(n-1) 是不透明谓词；白...
category: reverse
subcategory: reverse
tools_used:
- IDA
...
contest_type: open
wp_url: https://www.ctfiot.com/304400.html
reasoning_chain:
- PE 40MB 二进制 + 数据段嵌入查找表 → 触发点：~95M 指令的庞然大物
- 假设：标准 IDA F5 失败（混淆太重）→ 动作：PE 映射到 Unicorn 内存全指令模拟
- 观察：HookD3B20 比较函数读 rdx 期望值 → flag 字节逐一捕获
- 白盒密码 20 轮 PRISMWB3 → 假设：查找表嵌入 → 动作：定位 RVA 0x154E50 (~2.7MB) + RVA 0xAD7660 (~28.8MB)
- 642 行 SSE/MBA 派生函数 → 假设：MBA(mixed boolean arithmetic) 混淆 → 动作：n*(n+1) / n*(n-1) 不透明谓词识别
- MBA 状态机包裹 memcpy → 假设：必须先剥离状态机 → 动作：Unicorn 单步执行剥离
- 280 字节派生 + 32 字节 SipHash 变体 → 假设：标准 SipHash → 动作：z3 还原 → flag
failed_attempts:
- 试图 IDA F5 反编译 40MB 二进制 → 失败：~95M 指令反编译卡死
- 试图静态分析 MBA 表达式 → 失败：n*(n+1) 偶数不透明谓词需符号执行
key_observations:
- MBA 表达式 n*(n+1) 或 n*(n-1) 是不透明谓词
- 白盒密码 20 轮 + 28.8MB 查找表让 IDA F5 失败
- Unicorn 全指令模拟 + Hook 关键函数是混淆巨无霸唯一可行路径
- SipHash 变体可识别为标准实现 z3 还原
prerequisites:
- Unicorn 全指令模拟引擎
- MBA / opaque predicate 识别
- 白盒密码查找表结构
- SipHash 算法变体分析
---
