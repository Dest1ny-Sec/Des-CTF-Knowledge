...
tags:
- caps-case-control
- tar-symlink-webshell
- PHP-Incomplete-Class-deserialize
- CVE-2024-2961
- php-filter-iconv
- Behinder-RC4
- safetensors-export
...
quality: high
full_path: 第五届长城杯-京津冀_writeup_by_Mini-Venom.full.md
meta_path: 第五届长城杯-京津冀_writeup_by_Mini-Venom.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: '第五届长城杯 京津冀 Writeup by Mini-Venom。第五届长城杯京津冀 Mini-Venom 10 题：caps 路径穿越 + tar 软连接 + Incomplete_Class + CVE-2024-2961 + safetensors 导出 + duckdb RCE。。关键路径：Web 文曲签学: #help 信息, caps 控制大小写, read ....//.......'
category: misc
...
contest_type: open
wp_url: https://www.ctfiot.com/271264.html
wp_author: Mini-Venom
reasoning_chain:
- Web 文曲签学 → 触发点：#help 信息 + caps 控制大小写 + read 路径穿越
- 假设：read ....//....//....//....//....//....//....//flag → 假设：caps 可调大小写穿越
- Web EZ_upload → 触发点：tar 解压 + 软连接 ln -s /var/www/html aaa + aaa/aaa.php 一句话木马 + 蚁剑
- Web SeRce → 触发点：__PHP_Incomplete_Class 反序列化
- 动作：exp=O:22:"__PHP_Incomplete_Class":2:{s:4:"name";s:8:"RedHeart";s:6:"nation";s:5:"China";}
- Web CVE-2024-2961 → 触发点：php://filter iconv + file:///proc/self/maps + 下载 libc + /readflag > /tmp/lier.txt
- 数据安全 RealCheckIn-1 → 假设：tcp stream 1102 写入 flag
- 数据安全 RealCheckIn-3 → 触发点：冰蝎 RC4 解密 key='supernov@'
- AI easy_poison → 假设：FlippedPredictor 包装 TextClassifier 翻转二分类输出
- AI Mini-modelscope → 假设：tf.io.read_file('/flag') + tf.saved_model.save + ascii 转字符串
- AI 数据投毒 → 假设：safetensors 导出 numpy 数组 + flag{po2iso3ning_su4cces5sfully_triggered}
- AI eztalk → 假设：duckdb SQL 注入 + install shellfs from community + load shellfs + read_csv('bash /tmp/exploit |')
failed_attempts:
- read 路径穿越直试 → 失败：必须 caps 控制大小写
- tar 解压直传 PHP → 失败：必须先 ln -s 软连接才绕路径限制
- CVE-2024-2961 直打 → 失败：必须 php://filter iconv + 下载 libc
key_observations:
- Web caps 控制大小写 + 路径穿越是长城杯京津冀特色
- tar 软连接 ln -s /var/www/html aaa 是 tar 上传 webshell 经典套路
- __PHP_Incomplete_Class 反序列化适用于类未定义场景
- CVE-2024-2961 php://filter iconv + /proc/self/maps + libc 下载是新型 RCE
- AI 题 duckdb SQL 注入 + read_csv('bash /tmp/exploit |') RCE 是 2024 新题型
prerequisites:
- 路径穿越 + 大小写控制
- tar 软连接 + Webshell 上传
- __PHP_Incomplete_Class 反序列化
- AI 模型投毒 + duckdb SQL 注入 RCE
---
