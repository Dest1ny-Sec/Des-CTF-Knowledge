...
lesson: '|'
quality: medium
full_path: NepnepxCATCTF部分WriteUp.full.md
meta_path: NepnepxCATCTF部分WriteUp.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: 'NepnepxCATCTF 部分 WriteUp (MISC 6 题速查)。NepnepxCATCTF 部分 WriteUp: 6 道 MISC 速查 (vmdk flag 搜索 / QQ 空间 OSINT / DeepSound 兽语 / rabbit+Ook 编码 / png 隐藏)。。经验：|'
category: misc
...
contest_type: open
wp_url: https://www.ctfiot.com/89555.html
reasoning_chain:
- Cat_Jump 题：vmdk 文件 → 触发点：十六进制编辑器直接搜 CatCTF{
- 动作：HxD 搜索 → 观察：FLAG：CatCTF{EFI_1sv3ry_funn9}
- Peekaboo 题：jpg 尾部 QQ 号 → 触发点：QQ 空间 OSINT
- 动作：登录王者荣耀搜 QQ → 观察：仅 3 名打野英雄，最后用百里玄策
- FLAG：CatCTF{bailixuance}
- miao~ 题：jpg 尾部 wav → 触发点：手动 binwalk 分离
- 动作：audacity 看频谱图 → 假设：DeepSound 兽语隐写
- 下一步：DeepSound 工具解 → 观察：得到 rabbit+Ook 编码
- 动作：rabbit + Ook 解码 → 观察：得明文
- 其他题：png 隐藏密文、兽语、rabbit+Ook → 假设：依次试工具
failed_attempts:
- 试图不解 OSINT 直接试 flag → 失败：必须验证打野英雄
- DeepSound 默认密码 → 失败：题目需自定义密码
- 不解 rabbit 只试 Ook → 失败：必须两套解码串联
key_observations:
- vmdk 文件可十六进制直接搜 flag
- QQ 空间 OSINT + 王者荣耀战绩联动
- jpg/wav 拼接是 MISC 文件尾分离基础
- DeepSound 频谱图藏文本是音频隐写经典
- rabbit + Ook 是 CTF 文本编码常见组合
prerequisites:
- MISC 文件分离工具（binwalk/HxD）
- OSINT 社交平台调查
- DeepSound 工具使用
- rabbit/Ook 解码基础
---
