...
term* parse_term_in(std::istream &in) { boost::archive::text_iarchive ar(in); term *ret = nullptr; ar >> ret; return ret;}
term* parse_term_s(const char* s) { std::istringstream ss(s); return parse_term_in(ss);}

2、将替换后的结果弄成 python 的样子，PP是结合双目运算；

def _ADD(a, b): return a + bdef _MULT(a, b): return a * bdef _bind(f, a): return lambda b: f(a,b)
...
def _SUB(a, b): # 一定要加，不加无解 s.add(a>=b) return a - bdef SUB(a): return _bind(_SUB, a)
