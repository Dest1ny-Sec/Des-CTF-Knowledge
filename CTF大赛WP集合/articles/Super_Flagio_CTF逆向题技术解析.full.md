...
quality: high
full_path: Super_Flagio_CTF逆向题技术解析.full.md
meta_path: Super_Flagio_CTF逆向题技术解析.meta.md
images_removed: true
images_removed_count: 0
schema_version: v3.0.0-P0
summary: Super Flagio CTF 逆向题技术解析。Super Flagio CTF：cocos2d-x Android 游戏逆向，XXTEA 解 LuaJIT + 自定义 opcode VM 还原。。关键路径：unzip flagio.apk 提取 assets/src/ 加密 Lua 脚本 → strings libgame.so | grep XXTEA 发现 xctf-flagio-2...
category: reverse
subcategory: reverse
tools_used:
- Frida
...
contest_type: open
wp_url: https://www.ctfiot.com/283888.html
reasoning_chain:
- unzip flagio.apk → assets/src/ 加密 Lua → 触发点：cocos2d-x 游戏逆向
- strings libgame.so | grep XXTEA → 观察：密钥 'xctf-flagio-2dx'
- 'LuaJIT 2.1.0-beta3 字节码格式 → 字节 0-1: 0xFFFF 签名; 2-5: 0xDBEE66 XXTEA 标志'
- 假设：luaL_loadbuffer 接收解密后字节码 → 动作：Frida hook 拦截
- 自定义 opcode (0x41 PUSH_IMM / 0x42 PUSH_REG / 0x32 PUSH_IN / 0x25 POP_REG)
- 假设：模拟执行 12 指令 + 链式 XOR → 动作：还原加密算法
- output[0] = (input[0]^30)-1; output[i] = (output[i-1]^input[i])±1 (奇偶 ±1 差异)
- 32 字节密文从后向前 XOR+±1 → flag = A766957A53EDA9290CCF8E03F1A9B7E0
failed_attempts:
- 试图直接看 .lua 文件 → 失败：加密字节码
- 试图不解 XXTEA 直接 hook → 失败：Frida hook 之前必须先解密
key_observations:
- 加密字节码 + 自定义 opcode 是游戏反外挂常见组合
- Frida hook luaL_loadbuffer 是 cocos2d-x/LuaJIT 标准破法
- XXTEA 密钥常以字符串形式硬编码在 .so 中 (strings 提取)
- 链式 XOR+±1 加密从后向前解是数学推导通用思路
prerequisites:
- APK 文件结构 (assets/lib/classes.dex)
- LuaJIT 字节码格式与 XXTEA 加密
- Frida hook 编写 (Interceptor.attach)
- 自定义 VM opcode 还原与模拟执行
---
