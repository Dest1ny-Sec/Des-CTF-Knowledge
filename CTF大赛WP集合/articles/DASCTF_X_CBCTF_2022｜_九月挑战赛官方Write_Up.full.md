...
year: 2022
difficulty: medium
vuln_type: crypto_hash
tags:
- ssti-reverse
- prototype-pollution
- file-read-url
- cc11-injectortocontroller
- gopher
- post
- easykey
attack_chain:
...
quality: high
full_path: DASCTF_X_CBCTF_2022｜_九月挑战赛官方Write_Up.full.md
meta_path: DASCTF_X_CBCTF_2022｜_九月挑战赛官方Write_Up.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: 'DASCTF X CBCTF 2022 九月挑战赛官方 Write Up。DASCTF X CBCTF 2022 九月赛官方 WP，涵盖 SSTI/JS 原型污染/Spring 内存马。。关键路径：Sign_in: md5(score+salt) 时间戳爆破 → Text Reverser: 字符串反转触发 Jinja2 反射 → zzz_again: __proto__ 污染 Object...'
category: web
...
contest_type: open
wp_url: https://www.ctfiot.com/57949.html
reasoning_chain:
- Sign_in：md5(score+salt) 时间戳爆破 → 假设：salt 是当前秒数 → 爆破几秒内 score → 动作：循环 md5 + 时间戳
- Text Reverser：输入字符串触发反射 → 假设：反转函数存在 SSTI 漏洞 → 动作：构造 Jinja2 反射 payload
- zzz_again：__proto__ 污染 + Object.assign → 触发点：JS 原型链污染
- 动作：设置 __proto__.xxx = malicious → 后续创建对象继承恶意属性 → 假设：可触发 RCE
- a_proxy_server：URL.href 协议 file:///fl%61g → 触发点：URL href 协议允许 file
- 假设：构造 file:// + URL 编码 flag 路径（/fl%61g → /flag）→ 动作：发 GET http://server/file:///fl%61g
- JavaMaster：file:// 读 /etc/hosts → 触发点：SSRF 读内网 Spring Boot 服务
- 动作：注入 cc11 InjectToController 内存马 → gopher:// POST readObject payload → 触发 TemplatesImpl 反序列化
- 动作：写 cc11 gadget 链 → base64 编码注入 → 完成内存马注册
failed_attempts:
- 试图直接 SSRF http://127.0.0.1 → 失败：SSRF 限制 file 协议必须用 URL 编码
- 试图用 fastjson 反序列化 → 失败：题目是 Spring Boot + jackson，需要 cc11 链
- 试图用 gadget 直接打 TemplatesImpl → 失败：需要先内存马注册（InjectToController）
key_observations:
- URL.href 协议 file:// + URL 编码字符（%61 = a）是 SSRF 经典手法
- __proto__ 污染 + Object.assign 是 JS 原型链污染标志
- JavaMaster 这类无出网 Spring 内存马是现代 CTF WEB 高频考点
- InjectToController 内存马通过 gopher + Spring Controller 反射注册
- cc11 链（TemplatesImpl + TiedMapEntry + LazyMap）是无 CommonsCollections 1-7 依赖时的标准 gadget
prerequisites:
- Jinja2 SSTI 反射 + Unicode 编码
- JS 原型链污染（__proto__ + Object.assign）
- SSRF + URL 协议（file:// / gopher://）
- Spring Boot 反序列化（InjectToController / cc11）
- Java 反序列化 gadget 链（TemplatesImpl / TiedMapEntry / LazyMap）
---
