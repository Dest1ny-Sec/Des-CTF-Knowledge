...
lesson: '|'
quality: medium
full_path: MineCraftCTF、Solar应急响应月赛_内存取证题目.full.md
meta_path: MineCraftCTF、Solar应急响应月赛_内存取证题目.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: MineCraftCTF + Solar 应急响应月赛 内存取证。MineCraftCTF + Solar 月赛内存取证：搜日志找攻击者 IP (10.10.0.1) + netscan 找外联 IP + 端口。。经验：|
category: forensic
...
contest_type: open
wp_url: https://www.ctfiot.com/265387.html
reasoning_chain:
- 触发点：Minecraft 服务器被勒索组织投放勒索病毒 + 服务器内存镜像打包排查 → 假设：内存取证找攻击者 IP/外联 IP
- 动作：volatility 检查镜像 + 搜 PHP webshell (shell.php) → 观察：找到 POST /uploads/shell.php 请求日志
- 假设：REMOTE_ADDR 是攻击者 IP → 动作：grep 'POST /uploads/shell.php' → 观察：10.10.0.1 是攻击者 IP
- 动作：volatility imageinfo → profile=Win7SP1x64 → volatility netscan → 假设：找监听端口对应进程
- 观察：wininit.exe (PID 372) 监听 49152 端口 → 假设：外联 IP
- 第 6 题未解（占位）
failed_attempts:
- 试图用 volatility hivelist 直接找攻击者 IP → 失败：攻击者 IP 在应用层日志，不在注册表
- 试图找勒索病毒进程 → 失败：内存中没找到 ransomware 进程残留，只能走日志侧
key_observations:
- 内存取证找攻击者 IP：volatility imageinfo + filescan + netscan 三件套
- Webshell 日志 REMOTE_ADDR = 攻击者 IP（Apache/Nginx access.log 模式）
- netscan 找 LISTENING 端口对应进程 PID + 镜像名 = wininit.exe PID 372 :49152 = 外联入口
- 勒索病毒攻击后内存镜像常残留：webshell + 应用日志 + 网络连接（不残留进程本身）
- Volatility 2.6 standalone.exe 是应急响应月赛常用版本（无需 Python 环境）
prerequisites:
- Volatility 内存取证（imageinfo / pslist / netscan / filescan / clipboard）
- Windows 注册表/事件日志结构
- Web 日志分析（Apache/Nginx access.log REMOTE_ADDR）
- 应急响应流程（webshell 排查 + 进程网络关联）
---
