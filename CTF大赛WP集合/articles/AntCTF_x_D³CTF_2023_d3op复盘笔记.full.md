...
quality: high
full_path: AntCTF_x_D³CTF_2023_d3op复盘笔记.full.md
meta_path: AntCTF_x_D³CTF_2023_d3op复盘笔记.meta.md
images_removed: true
images_removed_count: 6
schema_version: v3.0.0-P0
summary: AntCTF x D³CTF 2023 d3op 复盘笔记。AntCTF x D³CTF 2023 d3op — OpenWrt 22.03.3 固件 diff 找 base64 ubus 模块 + /etc/shadow 哈希 + /flag + network.interface 攻击面。。关键路径：解 gambox 固件 → OpenWrt 22.03.3 r20028-43d71ad...
category: misc
...
contest_type: open
wp_url: https://www.ctfiot.com/116000.html
reasoning_chain:
- '[触发点] 题目 hint 提示 diff rootfs → 假设：官网下同版本 OpenWrt 22.03.3 / [动作] 解 gambox 固件 + 下载 OpenWrt 22.03.3 squashfs-root / [观察] diff 显示多了 /flag + /etc/shadow root $6$JlPmKq/ZhqQ0I6V6$ 哈希密码 + /usr/libexec/rpcd/base64 / [下一步] 看 base64 ubus 模块'
- '[触发点] ubus list 看到 base64 模块 → 假设：base64 encode/decode 是入口 / [动作] ubus -v list base64 看 encode/decode + ubus call base64 encode ''{"input" : "z1r0"}'' / [观察] 拿到编码输出 / [下一步] 看 main 函数'
- '[触发点] main 函数 init_base64 + read_input(0, v6, 0xFFF) + sub_402478 解析 + sub_403C90 找 "input" 字段 + sub_4059D0 验证 → 假设：漏洞点在 base64 模块调用 / [动作] 逆向 main + 调用 stack / [观察] network.interface 配置是攻击面 / [下一步] 构造 exploit'
- '[触发点] base64 + network.interface 配置可构造未授权调用 → 假设：ubus unauthenticated.json 加 base64 encode/decode / [动作] 看 unauthenticated.json + network.interface 配置 / [观察] 可执行未授权 base64 / [下一步] 解 flag'
failed_attempts:
- 试图直接 ubus call base64 → 失败：需要未授权接口
- 试图破解 root 哈希 → 失败：$6$ sha512 crypt 难爆破
key_observations:
- OpenWrt 固件题标准做法：官网下同版本 diff
- base64 是 ubus 微系统总线模块
- $6$ 是 sha512 crypt
- unauthenticated.json 控制 ACL
prerequisites:
- OpenWrt 固件解包（binwalk / squashfs）
- ubus 系统总线原理
- Linux diff 命令
- sha512 crypt 哈希格式
---
