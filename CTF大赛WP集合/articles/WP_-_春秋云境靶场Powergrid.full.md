...
- swagger_api_doc
- weak_password_admin
- mail_phishing_exe_av_bypass
- openvpn_ovpn_file
- lateral_movement
- pth_hash_pass_the_hash
- safe_dog_kill
- wmiexec_pth
- mysql_spring_application_yml
- larkmt_admin_db
...
quality: high
full_path: WP_-_春秋云境靶场Powergrid.full.md
meta_path: WP_-_春秋云境靶场Powergrid.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: WP | 春秋云境靶场 Powergrid。春秋云境 Powergrid 5 flag 全流程：swagger 弱口令 → 邮件钓鱼 + 免杀 → OpenVPN 改远端 IP → PTH 哈希传递 + WMIExec → Spring application.yml MySQL 凭据取数据 md5。。经验：当 OpenVPN .ovpn 配置文件中 remote 是内网 IP 时，直接改成...
category: web
...
contest_type: open
wp_url: https://www.ctfiot.com/299107.html
reasoning_chain:
- 触发点：Powergrid 靶场 5 个 flag → nmap 扫端口 22/25/80/110/143/443/465/587/993/995 + dirsearch
- 假设：swagger-resources /v2/api-docs 暴露 API 文档 → admin 123456 弱口令登录 → flag1
- 动作：dirsearch + admin 123456 → admin 用户登录系统 → flag1{...}
- flag2 触发点：zs@powergrid.com 123456 邮箱 → 假设：邮件钓鱼 + 免杀 exe
- 动作：邮件钓鱼 → 上线 exe → 改 Administrator Admin123 → 桌面 vpn/zhangsan.ovpn
- 假设：OpenVPN remote 是内网 IP → 改外网入口 IP → 172.27.236.3 vpn IP → flag3
- flag3 触发点：fping 172.16.200.0/24 → 172.16.200.78 DATA + 200.81 MySQL + 200.76 Windows + 200.87 OpenVPN
- 动作：p0wny-shell 传图 → net stop SafeDogGuardCenter + powershell Set-MpPreference 关杀软 → eval 后门
- flag4 触发点：wmiexec administrator@172.16.200.78 -hashes pth → 假设：PTH 哈希传递
- 动作：reg add DisableRestrictedAdmin + type flag.txt → flag4{...}
- flag5 触发点：application.yml Spring 配置 → mysql root rjS8K2RW7KE4E1vk 密码 → job_user_datas select → md5(库名+xxx)
- 观察：Swagger 弱口令 → 邮件钓鱼 → OpenVPN 改 remote → PTH 哈希 → application.yml 凭据 五连击
failed_attempts:
- flag1 试图暴力破解 admin 之外的账号 → 失败：admin 123456 是经典弱口令
- flag4 试图走 PTK → 失败：必须用 PTH（pass the hash）绕过 NTLM 限制
- flag5 试图直接 SELECT flag → 失败：必须 md5(库名+xxx) 算 hash 拼接
key_observations:
- 当 OpenVPN .ovpn 配置文件中 remote 是内网 IP 时，直接改成外网入口 IP 即可绕过 NAT
- Spring Boot 应用的 application.yml 默认存数据库密码是经典配置泄露
- 邮件钓鱼 + 免杀 exe + 改 Administrator 密码是内网横向标配
- SafeDog 杀软 net stop + Set-MpPreference 关闭是腾云进设通用方法
- PTH（pass the hash）通过 impacket-wmiexec 横向是 Windows 域标配
prerequisites:
- nmap / dirsearch 扫描技巧
- 邮件钓鱼 + 免杀 exe 制作
- OpenVPN .ovpn 配置文件解析
- PTH（pass the hash）通过 impacket 横向
- Spring Boot application.yml 配置审计
---
# WP | 春秋云境靶场Powergrid
