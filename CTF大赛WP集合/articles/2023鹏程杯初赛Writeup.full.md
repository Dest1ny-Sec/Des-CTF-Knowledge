...
quality: high
full_path: 2023鹏程杯初赛Writeup.full.md
meta_path: 2023鹏程杯初赛Writeup.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: 2023 鹏程杯初赛 writeup。PHP 反序列化 + Spring Boot Swagger + Tera SSTI + 数组绕 strlen + ORW + babyheap off_by_null + XOR0x23 PE。。经验：Tera Rust SSTI 走 get_env 旁路 + 数组绕 strlen 是 web 反复出现的 trick。
category: web
...
contest_type: open
wp_url: https://www.ctfiot.com/143076.html
reasoning_chain:
- 触发点：PHP web 入口含 H→welcome→A 类调用链 → 假设：反序列化 POP 链 → 动作：grep __destruct __wakeup 找 gadget
- 观察：Hacker.system('cat /flag') 终点类 → 假设：拼完整 POP 链触发 system → 动作：构造序列化 payload base64 提交
- 触发点：/proxy/url?url= 路径 → 假设：SSRF 文件包含 → 动作：填 netdoc:///flag%23.html
- 观察：返回 flag 内容 → 假设：netdoc 协议未禁用 → 下一步：换 ftp/gopher 试 RCE
- 触发点：Tera Rust SSTI 输入框 → 假设：模板注入 → 动作：填 `{% set t=galf|reverse %}{% set f=get_env(name=t) %}`
- 观察：get_env 拿到 flag 内容 → 假设：绕 {{ }} 黑名单走 get_env 旁路 → 下一步：换 matching 正则盲注
- 触发点：PWN silent 栈溢出 + magic gadget → 假设：覆盖 _IO_2_1_stdout_ → 动作：payload='A'*0x48+p64(csu_1)+ROP
- 观察：FSOP 触发 _IO_wfile_jumps 走 syscall;ret → 假设：ORW 沙盒绕 read/open/write → 动作：构造 open flag read write 三段 ROP
- 触发点：babyheap off_by_null → 假设：unlink 重叠 chunk → 动作：构造 prev_size 跨 chunk
- 观察：unlink 后 chunk 重叠 → 假设：environ 栈泄露 → 动作：写 chunk 覆盖 environ 指针打 ROP
- 触发点：bad_pe 全文件异或 0x23 → 假设：标准 PE 头还原 → 动作：异或 0x23 还原 DOS 头
- 观察：还原 PE 后 IDA 看到 RC4 函数 → 假设：RC4 密钥 th3k3y! → 动作：RC4 解密 flag
failed_attempts:
- 试图直接拼 __destruct 不走 POP 全链 → 失败：缺 Hacker 终点类引用
- 试图用 file:// 协议读 flag → 失败：需 netdoc 协议绕过 disable_function
- 试图用标准 PHP filter chain → 失败：服务端禁用 PHP 协议
- 试图用 one_gadget 直提 shell → 失败：沙盒只允许 open/read/write
- 试图不解 XOR 0x23 直接 IDA → 失败：PE 头错位反编译不出符号表
- 试图爆破 RC4 密钥 → 失败：密钥 th3k3y! 已知直接解
key_observations:
- PHP 反序列化 POP 链必经 welcome→A→C→E→K→R→Hacker 完整链路
- netdoc 协议比 file:// 更稳，因 file:// 在 disable_function 下被禁
- Tera Rust SSTI get_env(name=reverse_string) 是绕 {{ }} 黑名单的杀手锏
- PWN silent 用 _IO_2_1_stdout_ 改 magic + vtable 走 ORW 是 libc-2.27 标配
- babyheap off_by_null + unlink + environ 三连是 glibc 2.23 经典利用
- XOR 0x23 是常见 PE 头混淆还原，RC4 密钥 th3k3y! 写死在 init 数组
prerequisites:
- PHP 反序列化 POP 链构造（__destruct/__wakeup 触发链）
- SSRF 协议选择（netdoc/file/gopher/ftp）
- Tera Rust 模板引擎语法与 get_env 旁路
- glibc heap off_by_null 与 unlink 重叠原理
- RC4 流密码与密钥恢复
- PE 文件结构与 IDA 静态分析
---
# 2023鹏程杯初赛Writeup
