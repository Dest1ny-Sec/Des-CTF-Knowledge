...
quality: medium
full_path: 比赛中遇到的一些简单的逆向题目.full.md
meta_path: 比赛中遇到的一些简单的逆向题目.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: 比赛中遇到的一些简单的逆向题目。6关CrackMe ELF逆向：长度+前缀+末位+十六进制位运算+异或+第三段。经验：多关CrackMe常以_分隔多段子串拼接；位运算(v1<<4)|v2需爆破
category: reverse
subcategory: reverse
time_required: quick
difficulty_score: 2
...
contest_type: open
wp_url: https://www.ctfiot.com/60997.html
reasoning_chain:
- 拿到 ELF 文件先跑 → 提示输入 key，6 关串行校验 → 触发点：6 关 CrackMe
- 'check1: strlen(a1) != 34 → 假设：flag 总长 34 → 动作：先构造 34 字节 ''A'' 串测试'
- 'check2: strncmp(a1, ''flag{'', 5) → 假设：前缀 flag{ → 观察：第 1-5 字节固定'
- 'check3: a1[strlen-1] == 125 → 假设：末位 } → 观察：34 字节末位 0x7D'
- 'check4: 找第一个 _ 位置 → 把第 5 到 _ 之间的每 2 字符按 (v2|16*v1) 位运算 → 假设：s1 = ''olympics'''
- 动作：爆破 v1,v2 ∈ [33,127] → 满足 (v2|16*v1)==olympics[i] 的字符对 → 观察：得到第一段子串
- 'check5: in -> 异或 32/33 → 假设：IO 是 ''in'' 大写变换 → 动作：解码得第二段 ''IO_xxx'''
- 'check6: 第三段直接拼接 → 动作：flag{第一段_第二段_第三段}'
failed_attempts:
- 试图硬编码 34 字节 'A' 全串 → 失败：check2 校验 flag{ 前缀
- 试图爆破全部 256 种字符组合 → 失败：空间 2^32 太大
- check4 跳过位运算直接试 str → 失败：s1[i] = v2|16*v1 是位拼接
key_observations:
- CrackMe 多关常以 _ 分隔多段子串拼接
- (v2 | 16*v1) 是十六进制双字符解析模式（v1 是高位，v2 是低位）
- ^32 / ^33 是大小写翻转 + 偏移的常见做法
- strlen-1 末位校验和 strncmp 前缀校验是 CrackMe 标配
prerequisites:
- C 语言位运算（| & << >>）
- IDA / Ghidra 基本反编译
- ELF 文件结构与运行机制
