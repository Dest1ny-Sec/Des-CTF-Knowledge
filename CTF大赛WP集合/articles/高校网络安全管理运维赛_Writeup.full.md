...
quality: high
full_path: 高校网络安全管理运维赛_Writeup.full.md
meta_path: 高校网络安全管理运维赛_Writeup.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: '高校网络安全管理运维赛 Writeup。高校网络安全管理运维赛WP集合,涵盖PWN栈溢出+RE bitwise爆破+WEB JWT/MongoDB/Pickle/SSRF/XXE+MISC ZIP钓鱼邮件DKIM综合,运维安全方向。。关键路径：babypwn: token+username+password 0x38字节溢出+p64(0x40117A) ret2win → Login: to...'
category: web
...
contest_type: school
wp_url: https://www.ctfiot.com/180828.html
reasoning_chain:
- 'babypwn: token+username+password 流程 → read 0x38 字节缓冲 → 假设溢出 ret2win → 动作: payload = a*0x38 + p64(0x40117A)'
- '观察: flag{KooD1EijiemeePH8ieNei2XoL8iCh5de}'
- 'Login 题: admin/1q2w3e4r 登录回显含 ELF 头 → 假设: Core dumped 写入可执行 → 动作: 二次登录 + 0x98 字节溢出 p64(0x40127E) 改 ret'
- 'babyre: upx -d 脱壳 → 4 段 bitwise 表达式 → 假设解方程爆破 → 动作: part1=3821413212 / part2=0x98124621 / part3=78769651 / part4=67321987'
- 'pyssrf: Python 3.7 urllib 已知 CVE-2019-9740 / CVE-2019-9947 → 假设 CRLF 注入 redis 命令 → 动作: %0d%0aSET key pickle payload'
- '观察: pickle exec 反弹 → flag'
- 'fileit: XXE 无回显 → VPS 起 http 服务挂 xxe.php + xxe.xml → 假设: 外带回 flag'
- 'phpsql: admin''||1 万能密码 → 假设 OR 永真 → 动作: 直接登录'
- 'Messy Mongo: jwt 登录 ctfer + assert.notEqual(''admin'') → 假设聚合操作符 $toLower 绕过 → 动作: 先改 username=Admin 再用 {$set:{username:{$toLower:''$username''}}}'
- 'zip 题: 输入开头 5 字符是 flag{ 即可通过 check → 假设 ASCII 0x7f 是 DEL 删除 → 动作: flag{\x7f\x7f\x7f\x7f\x7f + token'
- '钓鱼邮件: flag_part1/2/3 散落在 SPF/DKIM/DMARC TXT → 假设 DNS 枚举 → 动作: dig txt +short 三条记录拼接'
- 'Apache 题: 给 Flask socket 创建任意 TCP 连接 → port=80 + Apache CVE-2021-41773 path traversal → 假设 /cgi-bin/.%2e/.%2e/.%2e/.%2e/bin/sh → 动作: POST 构造 exp'
failed_attempts:
- '试图 Login 题用密码爆破 → 失败: 默认密码 1q2w3e4r'
- '试图 babyre part4 用 Python 暴力 → 失败: Python 太慢, 必须 C'
- '试图用回显 XXE → 失败: 必须 OOB 外带'
- '试图覆盖 __free_hook → 失败: glibc 2.31 已无 hook'
- '试图爆破 5 字符压缩包密码 → 失败: 改用 bkcrack 已知明文'
key_observations:
- Python 3.7 + urllib CVE-2019-9740 是稳定 CRLF + redis SSRF 链
- MongoDB aggregation 的 $toLower 能绕过 assert.notEqual 字符串比较
- ASCII 0x7f (DEL) 是 zip check 漏洞, 能删除前 5 字符 flag{
- 钓鱼邮件 DKIM/SPF/DMARC TXT 记录是 flag 三段式经典分布
- Apache CVE-2021-41773 路径穿越 + CGI 执行是 2.4.49 通杀
prerequisites:
- Python pwntools + Redis 操作
- MongoDB 聚合操作符 ($toLower / $set)
- DNS 记录查询 (dig / nslookup)
- bkcrack 已知明文攻击 + Apache CVE-2021-41773
---
# 高校网络安全管理运维赛 Writeup
