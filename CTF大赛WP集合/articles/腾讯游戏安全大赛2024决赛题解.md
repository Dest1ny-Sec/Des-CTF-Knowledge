...
    target = NtCreateFile;
    target2 = NtReadFile;
    kprintf(("Line %d:xia0ji233: NtCreateFile=%p NtReadFile=%pn"), __LINE__, target,target2);
...
        kprintf(("xia0ji233:Backtrace[%u]: %pn"), i, backtrace[i]);
    }
...
    kprintf(("Line %d:xia0ji233: KeBugCheckEx=%pn"), __LINE__, target);
    g_Object = DriverObject;
    if (target) {
        for (int i = 0; i < sizeof(oldcode); i++) {
            oldcode[i] = target[i];
