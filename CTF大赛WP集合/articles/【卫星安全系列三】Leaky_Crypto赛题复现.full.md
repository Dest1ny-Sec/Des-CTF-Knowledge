...
quality: high
full_path: 【卫星安全系列三】Leaky_Crypto赛题复现.full.md
meta_path: 【卫星安全系列三】Leaky_Crypto赛题复现.meta.md
images_removed: true
images_removed_count: 0
schema_version: v3.0.0-P0
summary: 【卫星安全系列三】Leaky Crypto赛题复现。AES T-表 Cache 侧信道攻击：100000 (明文, 时间) 数据 + 链式密钥字节还原 + AES 解密。。关键路径：题目：100000 个 (明文 hex, 加密时间 ns) 数据 → AES-128-ECB + 密钥前 6 字节 97ca6080f575 已知 → 密文 96 字节 flag 加密结果。经验：AES 软件实现...
category: crypto
subcategory: oracle
time_required: long
difficulty_score: 4
...
contest_type: open
wp_url: https://www.ctfiot.com/151037.html
reasoning_chain:
- Leaky Crypto 100000 个 (明文 hex, 加密时间 ns) → 触发点：AES-128-ECB + 密钥前 6 字节 97ca6080f575 已知
- AES 软件实现 = SubBytes+ShiftRows+MixColumns 合并为 4 张 T-表 → 假设：T-表索引 = p[i] ^ key[i]
- 触发点：相同索引触发 Cache Hit → 假设：加密时间差可测 Cache 命中
- 动作：Cache Miss=10776ns / Cache Hit=10704ns (差 ~70ns) → 观察：时间差稳定
- 公式：p[i] ^ key[i] == p[j] ^ key[j] → 触发点：T-表同索引 = Cache Hit
- 动作：对每对 (i,j) 测时间差 → r = mean(Miss) - mean(Hit) → r > THRESHOLD=10 才采纳
- 链式扩展：0→4→8 / 12→5→9→13 / 1→10→14→2 / 6→15→3→7 / 11 → 假设：相邻字节链构造
- 假设：key[6]={0xe4,0xe5,0xe6,0xe7} 4 候选 → 动作：itertools.product 笛卡尔积 (4^10=1048576)
- AES.new(k, MODE_ECB).decrypt(c) + unpad → 验证 'flag' 字符串 → 观察：完整密钥 97ca6080f575e646e557f755bf15685e
- 最终 flag{uniform54349juliet:GL2aGs7ys8ygcW0kFBPLbwEdjLbwNlti...}
failed_attempts:
- 试图用单点对 (i, j) 爆破 → 失败：信息量不够，误报率高
- 试图不分组整段扫描 → 失败：100000 行 × 256×256 比较太慢
- 试图用 Triplet loss 思路分组 → 失败：Cache 命中是离散事件，不像 ML 连续
key_observations:
- AES 软件实现 T-表 (SubBytes+ShiftRows+MixColumns 合并) 因输入命中同一表项触发 Cache Hit 加速
- T-表索引 p[i] ^ key[i] 同值 → Cache Hit → 加密时间短 (~70ns 差)
- 16 字节链 + THRESHOLD 过滤可还原未知密钥字节
- itertools.product 笛卡尔积在 4 候选 × 10 字节 = 1M 种组合可暴力
- AES-ECB + Cache timing 是经典侧信道 (CTF 密码题常考)
prerequisites:
- AES 加密原理 (4 步 + T-表实现)
- CPU Cache 命中原理 (L1/L2)
- Python itertools.product + 二维列表操作
- AES ECB 解密 + unpad
---
