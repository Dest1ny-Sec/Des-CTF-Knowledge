...
quality: high
full_path: idek_2022-_Web_&&_Crypto_Writeup_by_r3kapig.full.md
meta_path: idek_2022-_Web_&&_Crypto_Writeup_by_r3kapig.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: 'idek 2022: Web && Crypto Writeup by r3kapig。idekCTF 2022 r3kapig 战队 Web + Crypto：Go 验证器顺序读 + 固定种子。。关键路径：/just-read-it POST Orders 数组 → 每次 WithValidatorCtx 传 reader + o(int) → 限制 MaxOrders = 10。经验：G...'
category: web
...
contest_type: open
wp_url: https://www.ctfiot.com/93433.html
wp_author: r3kapig
reasoning_chain:
- 触发点：tag 'golang' / 'validator-context' / 'orders' / 'seed-1337' → 假设：Go 验证器链 + 固定种子
- 动作：读 /just-read-it 源码 → 观察：每次 WithValidatorCtx 传 reader + o(int) → 触发点：受控 o(int) 索引
- 假设：MaxOrders=10 限制 + CheckReadOrder(o) 校验 0 < o <= 100 → 假设：o 可控 → 顺序读 0-99
- 动作：构造 Orders 数组 → 假设：每个 o 读取 randomData 第 o 字节 → 观察：泄 password 字段（offset=12）
- 触发点：initRandomData 用 rand.Seed(1337) 复现 → 假设：randomData 24576 字节可重现
- 动作：用 Go rand.New(rand.NewSource(1337)) → 假设：生成相同 randomData → 观察：泄露其他 position 内容
- 下一步：找到 password 字段 → 假设：用作后续题认证 → flag
failed_attempts:
- 试图不重写随机种子直接爆破 → 失败：种子固定必须 Go 代码复现
- 试图单次 order 读全部 → 失败：MaxOrders=10 限制需分批
- 试图用 Python 复现 Go rand → 失败：Go rand 算法与 Python 不同，必须 Go 代码
key_observations:
- Go rand.Seed(1337) + 24576 字节随机数 + 已知 password 偏移是经典的'复现随机序列'题目
- Go WithValidatorCtx 链式传递 reader 是订单系统的常见模式，可控 o(int) 索引制造越界
- Go 随机种子固定 → 题目自带 oracle → 任意字段可预测
- MaxOrders + 单次 offset 校验的组合是经典 Go web 利用模式
prerequisites:
- Go rand 包（rand.New / rand.Seed）
- Go context 包（WithValidatorCtx）
- Go HTTP handler 路由与 JSON 解析
- 字节顺序读取与索引控制
---
