...
quality: medium
full_path: 解析2025强网拟态EZMiniAPP.full.md
meta_path: 解析2025强网拟态EZMiniAPP.meta.md
images_removed: true
images_removed_count: 0
schema_version: v3.0.0-P0
summary: 解析2025强网拟态EZMiniAPP。强网拟态2025 EZMiniAPP微信小程序逆向,wxapkg解包(二进制格式+大端索引+UTF-8文件名)+enigmaticTransformation加密函数+XOR/rotl逆运算推导+密钥newKey2025!。。关键路径：微信小程序:__APP__.wxapkg文件 → 二进制格式,包含JS代码+页面模板+样式表+配置文件 → wxapk...
category: reverse
subcategory: reverse
...
contest_type: open
wp_url: https://www.ctfiot.com/278076.html
reasoning_chain:
- 题目给__APP__.wxapkg → 触发点：微信小程序逆向
- 假设：wxapkg二进制格式含JS代码+页面模板+样式表 → 动作：用unwxapkg/wxappUnpacker解包
- 观察：struct.unpack('>I', data)大端4字节解包索引 → 假设：多字节整数大端存储
- JS代码找到enigmaticTransformation混淆名函数 + 密钥newKey2025! → 触发点：加密函数识别
- 密文数组[1,33,194,133,...] → 假设：位运算加密 → 动作：识别XOR/rotl组合
- XOR性质：自反A^B^B=A+恒等A^0=A → rotl逆运算是ror → 假设：逆运算等于自身
- 动作：逐字节XOR密钥+逆rotl → 观察：还原flag字符串
failed_attempts:
- 试图不解包直接strings匹配 → 失败：JS代码被压缩,密文是数组非字符串
- 试图把字节序当小端 → 失败：wxapkg用大端网络字节序,struct.unpack要'>I'
- 试图找AES/SHA标准库调用 → 失败：本题是自定义位运算不是标准算法
key_observations:
- 微信小程序逆向从__APP__.wxapkg开始,unwxapkg/wxappUnpacker必备
