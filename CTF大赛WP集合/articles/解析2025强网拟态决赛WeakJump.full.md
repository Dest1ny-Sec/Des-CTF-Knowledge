...
quality: high
full_path: 解析2025强网拟态决赛WeakJump.full.md
meta_path: 解析2025强网拟态决赛WeakJump.meta.md
images_removed: true
images_removed_count: 0
schema_version: v3.0.0-P0
summary: 解析2025强网拟态决赛WeakJump。强网拟态2025决赛WeakJump逆向,静态链接+stripped+ptrace反调试(用GDB catch syscall ptrace改$rax=0绕过)+控制流平坦化(将函数视为黑盒+动态分析)+TEA Feistel(8字节块+4组+8轮+delta=0x9E3779B9)。。关键路径：ELF 64位LSB+statically linke...
category: reverse
subcategory: reverse
time_required: long
difficulty_score: 4
...
contest_type: open
wp_url: https://www.ctfiot.com/285718.html
reasoning_chain:
- 题目ELF 64位+statically linked+stripped → 触发点：stripped+statically linked逆向
- ptrace(PTRACE_TRACEME,0,0,0)反调试 → 动作：GDB catch syscall ptrace + commands + set $rax=0
- 观察：ptrace返回0(成功) → 程序继续 → 触发点：反调试绕过
- axt @0x401639查字符串引用 → 控制流平坦化:switch-case+dispatcher+大量跳转 → 假设：黑盒动态分析
- 位运算+左移4位+右移5位+XOR+add+32轮 → 假设：delta=0x9E3779B9是TEA特征
- 动作：sum初值=delta*32=0xC6EF3720, delta补码=0x61C88647 → Feistel结构识别
- 加密/解密对称,只需反向使用密钥序列 → 标准TEA解密 → 还原flag
failed_attempts:
- 试图深入分析switch-case每个分支 → 失败：控制流平坦化导致时间爆炸
- 试图在stripped下找函数符号 → 失败：静态链接+stripped无符号
- 试图不解反调试直接F5反编译 → 失败：F5函数被ptrace干扰
key_observations:
