...
- reverse
tags:
- easyre SSE _mm_movemask_epi8
- _mm_loadu_si128 128-bit load
- _mm_cmpeq_epi8 字节级比较
- _mm_and_si128 位与
- for 循环 input[i] ^= input[i+1-42*(i+1/42)]
- data1+data2 拼接 60 字节 cipher
- 正面爆破 1 字符逆推
attack_chain:
...
quality: medium
full_path: 2024长城杯初赛RE（部分题目）.full.md
meta_path: 2024长城杯初赛RE（部分题目）.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: '2024 长城杯初赛 RE（easyre SSE 异或链）。长城杯 easyre：43 字节 input 自异或链 + SSE _mm_cmovemask_epi8 == 0xFFFF 校验 32 字节，爆破 1 字符逆推。。关键路径：43 字节 input，循环 i ∈ [0, 43) → input[i] ^= input[i+1-42*(i+1/42)] → i < 41: input...'
category: reverse
subcategory: reverse
time_required: quick
difficulty_score: 2
...
contest_type: open
wp_url: https://www.ctfiot.com/206847.html
reasoning_chain:
- '[触发点] 反编译出现 _mm_loadu_si128/_mm_cmpeq_epi8/_mm_and_si128/_mm_movemask_epi8 SSE 指令 → 假设：SSE 128-bit 一次比较 16 字节 → [动作] 分析 data1 和 data2 两个 16 字节常量 → [观察] 全 0xFFFF = 全部相等 → [下一步] input 自异或链'
- '[触发点] cipher 数组是 input 自异或链（每字节 = 上一字节 ^ key^ 当前字节）→ 假设：已知 flag 头 ''f'' 反推 → [动作] x = ord(''f''); for i in cipher: for num in 32..128: if x^num == cipher[i]: input.append(num); x = num; break → [观察] flag{fcf94739-da66-467c-a77f-b50d12a67437}'
- '[触发点] tmaze 命令行传参 + 42 字节 path + switch-case (x/y/z) → 假设：T 型迷宫 + 地址链表走节点 → [动作] idapython 取 start_loc 到 end_loc 当作迷宫 → [观察] 构造 DFS 找 xyz 路径 → [下一步] UUID(md5(your input))'
- '[触发点] 反编译出现 _mm_loadu_si128/_mm_cmpeq_epi8/_mm_and_si128/_mm_movemask_epi8 → 假设：SSE 128-bit 比较 → [动作] 分析 data1 和 data2 16 字节常量 → [观察] 全 0xFFFF = 全部相等 → [下一步] input 自异或链'
- '[触发点] cipher 数组 + 自异或关系 → 假设：已知 flag 头 ''f'' 反推 → [动作] x = ord(''f''); for i in cipher: for num in 32..128: if x^num == cipher[i]: input.append(num) → [观察] flag{fcf94739-da66-467c-a77f-b50d12a67437}'
- '[触发点] tmaze 42 字节 path + switch-case → 假设：T 型迷宫 + DFS → [动作] idapython DFS + start_loc/end_loc → [观察] 构造 xyz 路径 → [下一步] UUID(md5(your input))'
failed_attempts:
- 试图反推 tmaze 迷宫生成函数 sub_7FF7D50A1230 → 失败：逻辑复杂，直接用 DFS 即可
- 试图不解自异或链直接遍历 256^42 → 失败：自异或链只需爆破 1 字节
- 试图用 IDA Pro 字符串搜索定位迷宫入口 → 失败：必须先调试拿到 argc=1
key_observations:
- SSE 比较 128-bit 一次比较 16 字节，全 0xFFFF = 全部相等
- input 自异或链 = 已知密文 + 知道异或关系 → 倒推明文只需 1 字节爆破（已知 flag 头 'f'）
- T 型迷宫 = 地址链表 + switch-case 控制方向 + 调试拿 argc=1 入口
- DFS 算法 idapython 实现 + 记录已访问节点加速
prerequisites:
- SSE intrinsics 识别（_mm_cmpeq_epi8 / _mm_movemask_epi8）
- 自异或链密码还原
- 迷宫类题目 DFS 解法
- idapython 自动化分析
---
