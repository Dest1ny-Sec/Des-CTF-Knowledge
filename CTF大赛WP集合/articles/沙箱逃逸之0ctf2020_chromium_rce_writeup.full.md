...
quality: high
full_path: 沙箱逃逸之0ctf2020_chromium_rce_writeup.full.md
meta_path: 沙箱逃逸之0ctf2020_chromium_rce_writeup.meta.md
images_removed: true
images_removed_count: 2
schema_version: v3.0.0-P0
summary: 沙箱逃逸之0ctf2020 chromium_rce writeup。0CTF/TCTF 2020 Chromium RCE：typed-array-set bypass+ArrayBufferDetach+UAF tcache。经验：V8 typed-array-set中EnsureAttached是关键安全检查；parser-base.h允许%Arr...
category: web
subcategory: rce
code_blocks_count: 1
images_count: 2
...
contest_type: open
wp_url: https://www.ctfiot.com/94732.html
reasoning_chain:
- 题目给 tctf.diff 改 V8 源码 → 触发点：diff 移除 typed-array-set.tq 的 EnsureAttached 检查
- diff 显示 EnsureAttached(target) → %RawDownCast<AttachedJSTypedArray>(target) → 假设：取消 IsDetachedBuffer 校验
- parser-base.h 取消 --allow-natives-syntax 限制 %ArrayBufferDetach → 假设：JS 可直接调 detach intrinsic
- 动作：a=new Uint8Array(0x200) + b=new Uint8Array(0x200) + %ArrayBufferDetach(a.buffer)
- 观察：a.buffer 进 tcache 释放但 a 仍持有 typed_array 句柄 → 假设：UAF
- 动作：a.set(b) → 在 tcache 已释放的 0x600 chunk 上写入 → 观察：覆盖 tcache fd
- 动作：b.set(a) → 从已释放 chunk 读 fd → 观察：拿到堆布局信息
- 假设：再 alloc 同样大小可拿到 a 的旧 buffer → 动作：触发任意读写
- 观察：构造任意读写 → 写 shellcode / ROP → 完成 Chromium RCE
failed_attempts:
- 试图直接 %PrepareFunctionForOptimization → 失败：diff 把该 intrinsic 屏蔽了
- 试图用 ArrayBuffer.transfer() detach → 失败：diff 把除 %ArrayBufferDetach 外的 detach 路径全屏蔽
- 试图跑发行版 V8 → 失败：必须用 diff 后编译的 d8 二进制
key_observations:
- V8 typed-array-set 的 EnsureAttached 是 IsDetachedBuffer 强校验点
- '%ArrayBufferDetach 是 V8 intrinsic 的 detach 入口（仅 --allow-natives-syntax 可用）'
- parser-base.h 的 ParseV8Intrinsic() 可被题目修改为无 flag 调用
- typed_array set 跨越 detach 状态时写的是 freed memory → tcache fd 篡改
- V8 RCE 通用链：detach → UAF → 覆盖 → 任意读/写 → shellcode
prerequisites:
- V8 TypedArray 实现细节（backing store + view）
- glibc tcache 机制与 UAF 利用
- TurboFan / Sparkplug JIT 基础
- Chromium 沙箱模型（renderer/browser 进程分离）
---
