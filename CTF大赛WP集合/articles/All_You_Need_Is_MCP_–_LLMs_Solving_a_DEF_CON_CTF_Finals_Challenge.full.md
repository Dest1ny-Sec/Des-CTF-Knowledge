...
quality: high
full_path: All_You_Need_Is_MCP_–_LLMs_Solving_a_DEF_CON_CTF_Finals_Challenge.full.md
meta_path: All_You_Need_Is_MCP_–_LLMs_Solving_a_DEF_CON_CTF_Finals_Challenge.meta.md
images_removed: true
images_removed_count: 0
schema_version: v3.0.0-P0
summary: All You Need Is MCP – LLMs Solving a DEF CON CTF Finals Challenge。DEF CON CTF Finals 2025 — ico 二进制 byte 协议 (0x10/0x22/0x32) + 服务端 fork + /flag → tEXt Author chunk + MCP LLM 自动化 + IDA 改名 (g_FlagStr...
category: reverse
subcategory: reverse
tools_used:
- IDA
...
contest_type: open
wp_url: https://www.ctfiot.com/257948.html
reasoning_chain:
- '[触发点] 拿到 ico 二进制 → 假设：amd64-64-little, Partial RELRO, No canary, NX, No PIE / [动作] checksec + strings 看协议 / [观察] 是裸 byte 协议 0x10 enable / 0x22 build / 0x32 metadata / [下一步] IDA 反编译 server_main'
- '[触发点] 服务端 fork + drop_privileges_to_user + set_alarm(16) → 假设：init_flag_from_file 加载到全局 / [动作] IDA 找 init_flag_from_file: open /flag + read → g_FlagString off_51C2E0 / [观察] g_FlagString 是 flag 字符串指针 / [下一步] 看 cmd_send_metadata_bundle'
- '[触发点] cmd_send_metadata_bundle 返回 PNG 格式含 tEXt Author chunk → 假设：Author 字段是 g_FlagString / [动作] 写 exp send b''\x10'' enable + b''\x22'' build default + b''\x32\x01'' request bundle / [观察] 拿到 Author chunk 即 flag / [下一步] parse_kv_payload 解码'
- '[触发点] MCP 自动化协议交互 → 假设：LLM 调 send_cmd 工具自动打 / [动作] 用 MCP LLM 调 send_cmd(0x10) → 0x22 → 0x32 + parse_kv_payload / [观察] 自动化拿 flag / [下一步] 改名 IDA 让 LLM 更好理解'
failed_attempts:
- 试图用 HTTP 协议交互 → 失败：服务端是裸 byte 协议不是 HTTP
- 试图从二进制 strings 找 flag → 失败：flag 是动态加载的
key_observations:
- 服务端常用 byte 协议 (tag+length+payload) 替代 HTTP
- flag 通常藏在 metadata chunk (PNG tEXt) 中
- MCP 让 LLM 直接调工具打 CTF Finals 是新趋势
- IDA 改名（server_main / accept_fork_loop / handle_connection / dispatch_loop）是逆向分析标准做法
prerequisites:
- IDA Pro / Ghidra 静态分析能力
- MCP (Model Context Protocol) 概念
- Python pwntools 网络交互
- IDA 改名 + 注释技巧
---
