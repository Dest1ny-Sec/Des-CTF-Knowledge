...
lesson: '|'
quality: medium
full_path: Midnight_Sun_CTF_2023_Writeup_by_VP-Union.full.md
meta_path: Midnight_Sun_CTF_2023_Writeup_by_VP-Union.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: 'Midnight Sun CTF 2023 Writeup by VP-Union (Polaris + ChaMd5 Venom 联合战队)。Midnight Sun CTF 2023 联合战队 (Polaris + ChaMd5 Venom = VP-Union) 排名第 23: pyttemjuk ret2libc + gets 写 shellcode + MemeControl pi...'
category: pwn
...
contest_type: open
wp_url: https://www.ctfiot.com/108846.html
wp_author: VP-Union
reasoning_chain:
- 触发点：pyttemjuk gets_plt=0x40263C + bss_addr=0x405090 → 假设：32 位 ret2libc + gets 写 bss + 自构造 shellcode
- 动作：payload1 = b'a'*0x14 + 4 + 0x8 + p32(gets_plt) + p32(0x401575) + p32(bss_addr) → sendlineafter 'Enter your name:'
- 假设：gets_plt 写入 bss_addr → 下一步：把 shellcode（GetProcAddress+LoadLibraryA+CreateProcessA）写到 bss
- 动作：send shellcode 到 bss → p3 = b'a'*0x14 + 4 + p32(bss_addr) + p32(bss_addr) → sendlineafter
- 假设：跳到 bss_addr 执行 shellcode → 观察：cmd.exe CreateProcessA('cmd.exe /c cmd') → 拿 flag midnight{i_prefer_sun_solaris_doors_over_microsoft_windows}
- MemeControl：torch 模块默认启用 pickle → 假设：pickle 任意执行漏洞 → 动作：payload = 'cos\nsystem\n(S\"/bin/sh\"\ntR.' base64
- SCAAS：1 功能保存程序到本地 → 动作：ret2libc + libc searcher → 完成
failed_attempts:
- pyttemjuk 试图走 system + /bin/sh → 失败：32 位 Windows 程序，要走 cmd.exe /c cmd
- pyttemjuk 试图用现成 shellcode → 失败：环境是受限 Windows，需要自构造 GetProcAddress + LoadLibraryA + CreateProcessA 链
- MemeControl 试图用 eval() → 失败：题目用 torch 序列化，pickle 默认开启，只能用 pickle opcode 直接构造
key_observations:
- 32 位 ret2libc + gets 写 bss + 自构造 shellcode 是无 NX/无 PIE Windows 经典三阶段攻击
- Windows shellcode 必走 PEB→LDR→GetProcAddress→LoadLibraryA→CreateProcessA 链（PEB_LDR_DATA 遍历）
- torch.save 默认用 pickle 协议 = 任意代码执行入口（os.system / subprocess.Popen 一行 opcode）
- pickle opcode 'cos\nsystem\n(S\"/bin/sh\"\ntR.' 是 Python pickle 攻击的最小化 payload
- VP-Union 是 Polaris + ChaMd5 Venom 联合战队，排名第 23
prerequisites:
- x86 汇编 + PEB_LDR_DATA 遍历（Windows shellcode 编写）
- pwntools + LibcSearcher 工具使用
- Python pickle 协议 + 任意代码执行
- ret2libc + gets 写 bss 三阶段攻击套路
---
