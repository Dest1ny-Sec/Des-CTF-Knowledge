...
tags:
- 软件系统安全赛 2026 初赛
- 64 位 .elf Loader dropper
- video.mp4 黑白色块
- stager_pyc_base64 内嵌
- .pyc 反编译 decompyle3 uncompyle6
- AES 解密视频数据
- 魔改算法双重保护
- AI 梭解两道题
- re2 re3 没解出来
...
quality: medium
full_path: 2026软件系统安全赛_初赛-re1（dropper+stager_.pyc反编译_视频解码）.full.md
meta_path: 2026软件系统安全赛_初赛-re1（dropper+stager_.pyc反编译_视频解码）.meta.md
images_removed: true
images_removed_count: 8
schema_version: v3.0.0-P0
summary: 2026 软件系统安全赛初赛 re1（dropper+stager .pyc 反编译 视频解码）。2026 软件系统安全赛 re1：64 位 .elf Loader dropper + 内嵌 base64 .pyc stager + 视频黑白色块解码。。关键路径：64 位 .elf Loader 加载 video.mp4 → file_exists(video.mp4) 检查 → 内嵌 st...
category: reverse
subcategory: reverse
subcategories:
- reverse
...
contest_type: open
wp_url: https://www.ctfiot.com/302112.html
reasoning_chain:
- 64 位 .elf Loader 加载 video.mp4 → 触发点：dropper 模式
- file_exists(video.mp4) 检查 → 假设：必须存在视频文件才执行
- 假设：内嵌 stager_pyc_base64[] → 动作：strings ELF 找 stager_pyc_base64
- 观察：base64_decode 后是 .pyc 字节码 → 下一步：decompyle3 / uncompyle6 反编译
- 动作：解出 .pyc → Python 源码 → 假设：解视频黑白色块
- 观察：黑白色块对应 flag 字符 → 动作：ffmpeg 抽帧 → PIL 识别黑白 → 拼 ASCII
- 魔改算法双重保护 → 假设：.pyc 内有自实现 AES / 自定义 SBOX
- 动作：写逆向脚本 → 观察：解出 flag
failed_attempts:
- 试图直接运行 .elf → 失败：缺 video.mp4
- 试图 IDA 静态分析 .elf → 失败：内嵌 base64 体积大，F5 难读
- 直接 strings 找 flag → 失败：flag 在视频解码后动态生成
- re2/re3 .so 加密逻辑找不到 → 失败：本题 re1 是 .pyc 字节码，难度低于 .so
key_observations:
- Loader 加载 video.mp4 + 内嵌 stager_pyc 是 2026 新型 ELF 逆向套路
- .pyc 字节码用 decompyle3/uncompyle6 反编译
- 视频像素作密文/明文是新颖 stego 形式
- AI 辅助解 CTF 逆向题（re1 AI 梭出）正在普及
- dropper + stager 两阶段加载是高级 ELF 设计模式
prerequisites:
- ELF 逆向 + IDA F5 反编译
- decompyle3 / uncompyle6 .pyc 反编译
- Python 内嵌 base64 字节码执行原理
- ffmpeg 抽帧 + PIL 像素识别
---
