...
quality: high
full_path: SECCON_2024：动态二进制插桩trace_破解函数式编程混淆.full.md
meta_path: SECCON_2024：动态二进制插桩trace_破解函数式编程混淆.meta.md
images_removed: true
images_removed_count: 7
schema_version: v3.0.0-P0
summary: SECCON 2024 - 动态二进制插桩 trace 破解函数式编程混淆。SECCON 2024 函数式编程混淆 C++ 逆向：std::variant + std::shared_ptr<Cons> 节点链表 + pyda 动态二进制插桩 trace cmp + Cons 节点 4 步加密 (sub_byte + mul + rotl + XOR) 还原。。关键路径：SECCON 202...
category: misc
...
contest_type: open
wp_url: https://www.ctfiot.com/220833.html
reasoning_chain:
- 题目 SECCON 2024 + C++ 函数式编程混淆 → 触发点：std::variant + std::shared_ptr<Cons>
- IDA 反汇编 5 层模板调用链：M_invoke → __invoke_r → __invoke_impl → lambda → Cons 节点
- Cons 节点 (rdi, rsi, rdx) 构造链表 → 假设：动态二进制插桩跟踪 cmp
- pyda cmplog.py + objdump 找 cmp 位置 → cmp_locs 解析 + p.hook(0x1891b, cmp_hook)
- 用户输入 ABCDEFGH 0x40 字节触发多次 cons() → 跟踪 cmp 比较密文 vs 期望值
- 观察：每次 cmp 失败 = 还原单步加密结果 → 4 步加密：sub_byte + mul(Galois) + rotl+XOR
- sub_byte(0x41414141) = 0x4e4e4e4e → mul → rotl^rotl^rotl^key → 再 sub_byte
- '逆运算还原 AES-like 加密 → flag: SECCON{fUnCt10n4l_pRoGr4mM1n6_...}'
failed_attempts:
- 试图直接静态分析 Cons 链表 → 失败：函数式编程混淆逻辑嵌套太深
- 试图用 gdb 手动断点 → 失败：cmp 数量太大需自动 hook
- 试图猜测 sub_byte S 盒 → 失败：必须从 cmp trace 反推
key_observations:
- 函数式编程混淆 (variant + shared_ptr<Cons>) 是高级 C++ 反混淆难点
- pyda 动态二进制插桩跟踪 cmp/cons 节点是还原 Cons 链表算法的高阶工具
- cmp 位置 + cmp 值对比可爆破密钥
- AES-like SubBytes + Galois mul + rotl 是加密 4 步常见组合
prerequisites:
- IDA Pro 反汇编 + 模板函数调用链识别
- pyda / frida 动态二进制插桩
- AES S 盒 + Galois mul 原理
- objdump + gdb + cmp 位置识别
---
