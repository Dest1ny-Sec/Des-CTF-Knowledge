...
quality: high
full_path: RWCTF_5th_ShellFind_Write-up.full.md
meta_path: RWCTF_5th_ShellFind_Write-up.meta.md
images_removed: true
images_removed_count: 5
schema_version: v3.0.0-P0
summary: RWCTF 5th ShellFind Write-up。RWCTF 5th ShellFind 长亭科技官方 WP：IoT 固件模拟 + UDP ipfind base64 解码栈溢出 + mips 无防护 ROP 改 GOT + dup2 busybox sh 回连。。关键路径：出题背景：IoT 安全 (Pink 僵尸网络) - 模拟 IoT 设备调试接口 → 题目类型 PWN (Nor...
category: pwn
subcategory: rop
tools_used:
- ROP
...
contest_type: open
wp_url: https://www.ctfiot.com/91517.html
reasoning_chain:
- 出题背景：Pink 僵尸网络 + IoT 模拟 → 触发点：only one UDP service to shell
- FIRMADYNE + FIRMae + TAP 虚拟网络 → 假设：仿真真实 IoT 设备
- netstat 发现 ddp 62976 + ipfind 62720 + tcp 31338 + http 80 → 假设：ipfind UDP 0xF580 是目标
- UDP 27 字节协议：4 magic FIVI + 4 pad + 1 type=10 + 2 cmd → sub_4013F4 → sub_400F50 base64 decode 到栈 → 栈溢出
- checksec：No RELRO + No canary + NX disabled + No PIE + Has RWX → ret2shellcode 可行
- qemu 中 ASLR 开启 + mipsrop 无输出 → 自构 ROP → s_log_nothing addiu $v0,$sp,0x10+arg_4 泄栈地址
- 改 GOT：0x00400c9c lw $gp,0x10($sp); lw $ra,0x1c($sp); jr $ra → 控制 rip
- 单命令受数百字符限制 + busybox nc nx → dup2(2) + execve /bin/busybox sh → sys_connect 回连 attacker
failed_attempts:
- 试图直接 ROP 调 system → 失败：单命令字符限制
- 试图用 mipsrop 自动链 → 失败：需手构 jr $ra gadget
- 试图走 HTTP 80 端口 → 失败：未提供攻击面
key_observations:
- IoT 攻防人员需掌握 FIRMADYNE/FIRMae 自动化仿真
- mips 自构 ROP 关键是 jr $ra + lw $gp gadget
- UDP 服务复用 fd 是反连 RCE 唯一路径
- s_log_nothing 类函数常常泄露栈地址
- Pink 僵尸网络利用非 Web 类调试服务是 IoT 攻击面
prerequisites:
- FIRMADYNE/FIRMae IoT 仿真
- MIPS ROP 自构 (jr $ra gadget)
- UDP dup2 复用 fd
- s_log_nothing 栈地址泄露模式
---
