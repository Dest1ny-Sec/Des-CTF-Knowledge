...
quality: medium
full_path: 【WP】2024年春秋杯夏季赛“初探勒索病毒”出题思路详解.full.md
meta_path: 【WP】2024年春秋杯夏季赛“初探勒索病毒”出题思路详解.meta.md
images_removed: true
images_removed_count: 8
schema_version: v3.0.0-P0
summary: 【WP】2024年春秋杯夏季赛 初探勒索病毒 出题思路详解。2024 春秋杯夏季赛初探勒索病毒，基于 Black Basta Buster 解密工具的签到题。。经验：Black Basta 用 ChaCha 密钥流加密 64 字节块 + XOR；解密需已知 64 字节明文 + 块位置；...
category: forensic
...
contest_type: open
wp_url: https://www.ctfiot.com/194219.html
reasoning_chain:
- 服务器 banana.jpg 被 Black Basta 加密为 .sah28vut5 → 触发点：已知 Black Basta 加密算法
- Black Basta 用 ChaCha 密钥流加密 64 字节块 + XOR + 加密前 5000 字节
- 假设：Known Plaintext Attack → 已知 64 字节明文位置 → 动作：xorblocks.py 生成 key.block
- wget black-basta-buster → 观察：项目是签到题，提供 key.block
- 导出 SRL_IGNORE_MAGIC=1 跳过 magic 检查 → ./decryptblocks.py banana.jpg.sah28vut5 key.block 还原图片
- 还原后 /flag 可读 → flag
failed_attempts:
- 试图直接解密 → 缺 key.block（已加密 64 字节明文）
- 试图 sed -i 修复 decryptblocks.py flags → 字符引号问题
- 试图不设 SRL_IGNORE_MAGIC → magic check 失败
key_observations:
- Black Basta 用 ChaCha 密钥流加密 64 字节块 + XOR
- Known Plaintext Attack 是勒索软件解密经典方法
- SRLabs Black Basta Buster 是开源解密工具
- SRL_IGNORE_MAGIC 环境变量跳过 magic 检查
- No More Ransom Project 是勒索软件解密工具轮合站
prerequisites:
- 勒索软件加密原理（ChaCha 流密码）
- Known Plaintext Attack 模型
- Linux 环境变量使用
- wget + bash 命令基础
---
