...
lesson: '|'
quality: high
full_path: MISC｜西湖论剑·2022中国杭州网络安全技能大赛初赛官方Write_Up.full.md
meta_path: MISC｜西湖论剑·2022中国杭州网络安全技能大赛初赛官方Write_Up.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: MISC｜西湖论剑 2022 官方 Write Up。西湖论剑 2022 官方 MISC 5 题 (Volatility VRAM + Rabin / npbk 模拟器 + alpha 隐写 / Kylin CNVD-2022-78421 / mp3stego + jjencode / 已知明文 zip 攻击)。。经验：|
category: misc
...
contest_type: open
wp_url: https://www.ctfiot.com/96709.html
reasoning_chain:
- 触发点：5 个子题（Memory Analysis / 机你太美 / KP-Basic / mp3 / take_the_zip_easy）涉及 Volatility、模拟器、Kylin、mp3stego、zip 已知明文 → 假设：覆盖 MISC 全方向（取证+隐写+工控+密码）
- Memory Analysis：拿到 ELF core dump + VirtualBox VRAM → 动作：volatility + VRAM 还原桌面 → 观察：剪贴板有 RSA 公钥 + Python IDLE 加密脚本
- 假设：RSA E=2 不是 RSA 是 Rabin 密码 → 动作：factordb 分解 N → P/Q → pow(c, (P+1)//4, P) + CRT 拼 4 个解
- 机你太美：夜神模拟器 npbk → 动作：rm locksettings.db 绕开机密码 + Skred 聊天工具 → 1.png alpha=251/255 转二进制
- 假设：1.png alpha 是 XOR key 编码 → 动作：cyberchef binary + XOR key 异或 → flag
- KP-Basic：Kylin 系统 ping → 触发点：CNVD-2022-78421 Kylin-Update-Manager 命令注入 + dbus 提权 → 假设：sudo su 拿 root
- 动作：rockyou.txt 爆破文件保险箱密码 'maganda' → 假设：得到 flag
- mp3：mp3 文件尾藏 png → 动作：png 二值化 0/255 转 bit → struct.pack → flag.zip → mp3stego 空密码 → 47.txt jjencode 变种 → 浏览器控制台跑 → DASCTF{f8097257d699d7fdba7e97a15c4f94b4}
- take_the_zip_easy：加密 zip 套 zip → 假设：已知明文攻击 → 动作：用 pcapng 作为明文 → bkcrack → 解出 inner zip
failed_attempts:
- Memory Analysis 尝试按 RSA 解密 → 失败：E=2 不是 RSA 是 Rabin 密码，4 个解需要 CRT 拼接
- KP-Basic 试图用普通提权 → 失败：必须走 Kylin-Update-Manager CVE + dbus 链路
- mp3 试图直接读 mp3 → 失败：flag 藏在文件尾拼接的 png 二值化位图中
key_observations:
- Rabin 密码（E=2）的解有 4 个，必须 CRT 拼接 + 试每个解找 flag
- VRAM 还原是 VirtualBox core dump 取证的关键（桌面截屏 + 剪贴板 + Python IDLE）
- 1.png alpha 通道值作为 0/1 编码是 CTF MISC 经典模式（PIL 读 pixel alpha）
- Kylin-Update-Manager CVE-2022-78421 是国产系统工控渗透典型入口（dbus 提权链）
- mp3stego 空密码 + jjencode 变种（$→a）说明 MISC 永远是工具+编码组合战
prerequisites:
- Volatility 内存取证（imageinfo / filescan / netscan / clipboard）
- PIL 像素操作 + alpha 通道位图编码
- mp3stego / bkcrack / cyberchef 等 MISC 工具链
- 国产系统工控漏洞利用（Kylin-Update-Manager CVE）
---
