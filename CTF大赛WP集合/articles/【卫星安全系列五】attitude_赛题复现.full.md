...
quality: high
full_path: 【卫星安全系列五】attitude_赛题复现.full.md
meta_path: 【卫星安全系列五】attitude_赛题复现.meta.md
images_removed: true
images_removed_count: 9
schema_version: v3.0.0-P0
summary: '【卫星安全系列五】attitude 赛题复现。卫星姿态 attitude: 2500 恒星向量 + Kabsch 算法 SVD 旋转矩阵 + 四元数提交 20 次。。关键路径：题目: 给 2500 行恒星 (X,Y,Z,星等) 参考向量 dataA_list → nc 拿到 4 浮点观测向量 (id, x, y, z) → 卫星姿态: 星体坐标系 → 惯性坐标系 旋转关系。经验：卫星姿态用四...'
category: crypto
subcategory: math
tools_used:
- Python
...
contest_type: open
wp_url: https://www.ctfiot.com/153356.html
reasoning_chain:
- HackASat 2020 attitude 题 → 触发点：2500 行恒星 (X,Y,Z,星等) + nc 拿 4 颗观测向量 (id,x,y,z)
- 卫星姿态 = 星体坐标系 → 惯性坐标系 旋转关系 → 假设：四元数 q=[w,x,y,z] 描述无万向锁
- 动作：构造参考向量 A (2500 恒星) vs 观测向量 B (nc 返回 4 颗) → 假设：A 和 B 坐标都中心化
- 触发点：找最优旋转使两组点对齐 → 假设：Kabsch 算法 (SVD 求解)
- 动作：rmsd.kabsch(A, B) → 中心化 → 协方差矩阵 H = A^T @ B → SVD H = U S V^T → R = V diag(1,1,d) U^T
- 矩阵转四元数 → 假设：scipy.spatial.transform.Rotation 可转换 → 动作：Rotation.from_matrix(R).as_quat()
- 发送 ','.join(str(x) for x in sol) 四元数 → 观察：服务端接受 1 次
- 触发点：需 20 次验证 → 假设：循环跑 → 动作：循环 20 次 recvdata + cal_matrix
- rmsd 库 + scipy Rotation = Python 卫星姿态题双神器 → 最终 20 次通过拿 flag
failed_attempts:
- 试图用欧拉角 Roll/Pitch/Yaw 描述姿态 → 失败：万向锁 (gimbal lock)
- 试图用方向余弦矩阵 (DCM) 提交 → 失败：服务端只认四元数
- 试图手写 Kabsch → 失败：SVD 数值不稳，rmsd 库已实现
key_observations:
- 卫星姿态用四元数 (w,x,y,z) 描述避免欧拉角万向锁 (gimbal lock)
- Kabsch 算法通过 SVD 找两组点集最佳旋转 (中心化 + 协方差 + SVD)
- rmsd 库 + scipy.spatial.transform.Rotation 是 Python 卫星题双神器
- 旋转矩阵 3×3 正交 + 行列式 = 1 是合法性约束
- 20 次循环验证是服务端防爆破常用套路
prerequisites:
- 四元数 / 旋转矩阵 / Kabsch 算法数学
