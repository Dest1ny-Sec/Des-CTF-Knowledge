...
quality: high
full_path: 由SUCTF一血题SU_Harmony初探鸿蒙应用安全分析.full.md
meta_path: 由SUCTF一血题SU_Harmony初探鸿蒙应用安全分析.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: 由SUCTF一血题SU_Harmony初探鸿蒙应用安全分析。SUCTF 2024 SU_Harmony：鸿蒙HAP包逆向+abc-decompiler+大整数运算链+z3解。经验：鸿蒙HAP = Zip结构，逆向modules.abc (ArkTS) + libentry.so (NAPI)；大整数...
category: reverse
subcategory: reverse
tools_used:
- Z3
...
contest_type: open
wp_url: https://www.ctfiot.com/224542.html
reasoning_chain:
- 触发：拿到 entry-default-unsigned.hap，file 判定为 Zip → 动作：unzip 解压
- 假设：核心代码在 modules.abc (ArkTS) + libs/libentry.so (NAPI) → 动作：定位两文件
- 动作：abc-decompiler（jadx 衍生）打开 modules.abc → 观察：pages/Index 入口 func_main_0
- 假设：flag 输入走 TextInput → 动作：找 TextInput onSubmit 事件
- 观察：check 函数入口跳转 libentry.so → 动作：IDA 打开 libentry.so
- 假设：sub_57B0 是大整数运算链 → 观察：sub_62F0/6D20/8270/9890/A8F0/C160 串联
- 假设：链可逆 → 动作：z3 建模 (input^2+2*input-3)//2 = array 反推 input
failed_attempts:
- 试图用 jadx 直接打开 modules.abc → 失败：abc 是 ArkTS 自定义字节码格式
- 试图只反编译 libentry.so → 失败：函数名是 sub_xxx 无业务提示
- 试图不解 modules.abc 直接逆 .so → 失败：check 函数入口定位不到
key_observations:
- 鸿蒙 HAP = Zip 结构，逆向 modules.abc (ArkTS) + libentry.so (NAPI) 双层
- abc-decompiler 基于 jadx，入门成本低，反编译 ArkTS 字节码
- 大整数加减乘除混合链 → 单步可逆公式（这里是 (input^2+2*input-3)//2）
- z3 建模需把 //2 (整除) 作为整数约束，不是浮点除法
- 鸿蒙 NAPI 桥接 .so 是 native 层核心
prerequisites:
- 鸿蒙 HAP 包结构（entry/feature 模块 + .ets + .abc + .so）
- ArkTS 基础 + abc-decompiler 工具使用
- IDA 识别 sub_xxx 调用链 + NAPI 入口定位
- z3 求解器建模（Int 变量 + ToInt 整除约束）
---
