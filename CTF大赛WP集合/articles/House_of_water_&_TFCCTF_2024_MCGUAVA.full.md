...
quality: high
full_path: House_of_water_&_TFCCTF_2024_MCGUAVA.full.md
meta_path: House_of_water_&_TFCCTF_2024_MCGUAVA.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: House of water + TFCCTF 2024 MCGUAVA。把 tcache_perthread_struct 变成 unsorted bin 的玩法，让 fake chunk 的 fd/bk 把 libc 指针反向注入到 tcache entries 数组。。经验：任意大小 malloc + UAF 是 House of Water 的入场券；爆破 heapbase 第 4 ...
category: pwn
...
contest_type: open
wp_url: https://www.ctfiot.com/205266.html
reasoning_chain:
- menu 1/2 (malloc/free) + 没有 edit/print + free 后 guava_gius[i] 未清零 → 触发点：典型 UAF + 任意大小申请
- 假设：满足 House of Water 前置（UAF + 申请到 0x10000 范围）→ 动作：先申请 0x3d8 / 0x3e8 再 free 拿到 fake unsorted chunk 的 size 字段 0x10001
- 动作：再申请 7 个 0x90 填 tcache + 3 组 0x90 中间隔 0x20 guard 防合并 + 一个 0xf000 padding → 观察：3 个 0x90 chunk 进入 unsorted bin
- 动作：改 unsorted_start 上方 8 字节为 0x31 fake size + free 制造 fake FWD chunk；同样 unsorted_end 上方改 0x20 + free 制造 fake BCK → 观察：fake unsorted chunk 的 fd/bk 被覆盖
- 动作：恢复 unsorted_start/unsorted_end 的 size 字段（tcache key 破坏需要修复）→ 释放 3 个 0x90 → 观察：3 chunk 进入 unsorted bin
- 动作：unsorted_start.fd = unsorted_end.bk = fake chunk 地址（替换 unsorted_middle）→ 下一步：触发 malloc(0x288) 让 unsorted bin 把 fake chunk 扔进 largebin
- 观察：fake chunk 链入 largebin 后 fd/bk 出现 libc 指针 → 动作：再申请 0x288 触发切割 → libc 指针被 malloc 进 tcache_perthread_struct entries 区
- MCGUAVA 没有 leak 路径只能爆破 heapbase 低 4 位第 4 位（1/16）→ 动作：脚本循环 16 次 brute force → 观察：成功获得 libc write primitive
- 动作：写 tcache_perthread_struct 里的 entries[i] 覆盖成 __free_hook / system 地址 → 触发 free('/bin/sh') → 完成
failed_attempts:
- 试图让同一 chunk 同时进 unsorted 和 smallbin 留 libc 地址 → 失败：chunk 状态冲突且无法稳定复用
- 试图直接爆破 heapbase 第 4-8 位 → 失败：题目限制 ASR 爆破成功率 1/16 是上限
- 试图用普通 House of Orange / House of Botcake → 失败：缺少 unsorted bin attack 的稳定链
key_observations:
- House of Water 的核心是把 tcache_perthread_struct 当 unsorted bin 节点，绕过 size 检测让 fd/bk 写入 libc
- fake FWD / BCK chunk 必须用 tcache key 反向还原 size 字段，否则 unsorted bin 检测会 abort
- 没有 leak 也能在 tcache 留下 libc 指针，但必须爆破 heapbase 1/16 概率
- 任意大小 malloc + UAF 是 House of Water 唯一前置；glibc 2.31+ 通用
- blue water 战队的 UDP 提出的新型 heap 利用手法是 heap exploitation 的新方向
prerequisites:
- glibc heap 数据结构（tcache_perthread_struct / unsorted bin / largebin）
- House of 系列利用原理（House of Force / Orange / Botcake / Water）
- how2heap 复现调试（gdb + set print pretty on）
- pwntools 脚本编写（brute force / remote 交互）
---
