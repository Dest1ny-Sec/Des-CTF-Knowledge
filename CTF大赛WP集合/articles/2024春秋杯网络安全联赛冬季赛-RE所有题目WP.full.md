...
- misc_unknown
tags:
- custom_md5_init 自实现 MD5
- qmemcpy 1024 字节函数自身
- MD5_Init+Update+Final
- srand(seed[0])
- rand()%127 ^ input[i]
- xor_string_with_rand
- IDA 数据段 ida_chars
- sub_402B60 v6[]/v7[] 负数 char 数据
- 字符串"nY" qmemcpy
attack_chain:
...
quality: high
full_path: 2024春秋杯网络安全联赛冬季赛-RE所有题目WP.full.md
meta_path: 2024春秋杯网络安全联赛冬季赛-RE所有题目WP.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: '2024 春秋杯冬季赛 RE 所有题目 WP（custom_md5_init + srand XOR）。春秋杯冬季赛 RE 套题：custom_md5_init 拷自身 1024 字节做 MD5 + srand seed + rand()%127 异或，42 字节异或直出 flag；sub_402B60 是同样套路的负数数组。。关键路径：main_program: 42 字节 input，c...'
category: reverse
subcategory: reverse
subcategories:
- reverse
...
contest_type: open
wp_url: https://www.ctfiot.com/229862.html
reasoning_chain:
- 触发点：main_program 42 字节 input + custom_md5_init → 假设：自实现 MD5 init 函数 → 动作：读反汇编
- 观察：custom_md5_init qmemcpy(v3, custom_md5_init, 0x400) 把函数自身 1024 字节当 MD5_Update 数据 → 假设：seed 是函数自身 MD5
- 动作：srand(seed[0]) 初始化 PRNG → xor_string_with_rand(s, s1) → s1[i] = rand()%127 ^ s[i]
- 动作：memcmp(s1, &ida_chars, 0x2B) 比较 → 假设：直接解
- 动作：xor[i] ^ cmp[i] 出 flag（42 字节） → 观察：拿到 flag
- 触发点：sub_402B60 v6[]/v7[] 含负数 char 数据 → 假设：字符串拼接 → 动作：拼接 'nY' + 后续负数
- 观察：直接还原 flag
failed_attempts:
- 试图不解 custom_md5_init 直接看 IDA 数据段 → 失败：必须先 qmemcpy 1024 字节算 MD5
- 试图爆破 rand()%127 → 失败：rand()%127 空间太小但 seed 受函数 MD5 约束
- 试图不看 sub_402B60字符串拼接 → 失败：负数 char 必须拼 'nY'
key_observations:
- 逆向题看到 qmemcpy(v3, custom_md5_init, 0x400) + MD5_Update 基本就是自实现 MD5 init 函数
- 把函数自身二进制当数据 MD5 = 自实现 MD5 的标准套路
- rand()%127 异或只要看 IDA 数据段 cmp 数组，直接 XOR 出明文
- 负数 char 数组字符串拼接是常见编码思路（无符号 / 有符号歧义）
prerequisites:
- IDA Pro 反汇编 + qmemcpy 识别
- MD5 算法原理与 Init/Update/Final 三阶段
- C srand/rand PRNG + 异或密码学
- 字符串拼接与负数 char 处理
---
