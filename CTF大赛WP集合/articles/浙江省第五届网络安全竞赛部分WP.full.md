...
quality: high
full_path: 浙江省第五届网络安全竞赛部分WP.full.md
meta_path: 浙江省第五届网络安全竞赛部分WP.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: '浙江省第五届网络安全竞赛部分 WP by 7coin。浙江省信安赛 6 题综合：mt_srand 时间种子预测 + USB HID 字典 + 仿射密码 + Coppersmith small_roots + ORW ROP。。关键路径：Web ezphp: mt_srand(time) 预测 shuffle 后 system/ls 在数组中的下标 → 调用 system("cat /fla...'
category: misc
...
wp_url: https://www.ctfiot.com/58989.html
reasoning_chain:
- 6 题综合：web ezphp + base64/32 + USB HID + m4a 反序 + 仿射 + Coppersmith RSA + ORW ROP → 触发点：每题独立
- 'ezphp: 源码显示 shuffle 打乱顺序但 seed 来自 time() → 假设：预测 time() 后能确定 system/ls 下标'
- 动作：mt_srand(time_)+ array_push + shuffle + 遍历找 system 和 ls 的下标
- 观察：$a[$_GET['b']]($a[$_GET['c']]); → 调用 system('cat /flag') → flag
- 'base64/32: 双层编码 → 动作：先 base32 再 base64 → 解明文'
- 'USB HID: 同 m0usb → 动作：tshark usb.capdata → dict 映射 → ''mik<DEL>mae...'' 还原'
- 'm4a 反序: 字节反转恢复 zip → 动作：bytes[::-1] 写文件'
- '仿射: c = (a*x + 7) mod 37 → 动作：ans = (num - 7) * inverse(key, 37) mod 37'
- 'RSA: 已知 p 高 560 bit → 假设：Coppersmith small_roots 还原剩余低位'
- 动作：small_roots(X=2^(1024-560-i.nbits()), beta=0.4) + 2^15 迭代 → 找到完整 p
- 'Pwn: pop_rax(2) + ''/flag'' + pop_rsi(0) + syscall(open) + read/write ORW → 读 flag'
failed_attempts:
- ezphp 用 mt_rand → 失败：PHP mt_rand 用 Mersenne Twister，但这里用 mt_srand(time) 显式设种子
- RSA 用费马分解 → 失败：p 高位已知但 p,q 接近仍需 Coppersmith
- Pwn 直接 system → 失败：libc 未知，必须 ORW
key_observations:
- PHP mt_srand(time()) + shuffle 是 shuffle 预测经典
- Coppersmith small_roots 是已知 p 部分位的 RSA 攻击方法
- ORW (open-read-write) 是无 libc 时的标准 ROP 套路
- 仿射密码解法：c = (a*x+b) mod m → x = (c-b) * inverse(a, m) mod m
- M4A 文件字节反序恢复 ZIP 是音频混淆隐藏 zip 的常用手法
prerequisites:
- PHP Mersenne Twister 伪随机数预测
- tshark USB HID 流量提取
- 仿射密码解法
- Sage Coppersmith small_roots 使用
---
# 浙江省第五届网络安全竞赛部分WP
