...
quality: medium
full_path: SunshineCTF_pwn.full.md
meta_path: SunshineCTF_pwn.meta.md
images_removed: true
images_removed_count: 2
schema_version: v3.0.0-P0
summary: 'SunshineCTF pwn。SunshineCTF 2022 Pwn (3 题)：simulator libc srand 预测 + chess ANSI color PIE 泄漏 + Elden Ring 主题题。。关键路径：simulator 22000: 团队名 20 字节后 4 字节 seed → libc.srand(seed) + libc.rand()%i+1 预测后续 →...'
category: pwn
...
contest_type: open
wp_url: https://www.ctfiot.com/84746.html
reasoning_chain:
- 题目 22000 simulator → 团队名 20 字节后 4 字节 seed → 触发点：seed leak
- 动作：recv(4) → u32(seed) → libc.srand(seed) → libc.rand() 预测
- i=10,100,1000...999999999 → 假设：循环 %i+1 命中正确答案
- 22001 chess → menu(3) 触发 ANSI color escape \x1B[30;...m → 假设：栈地址泄露
- 动作：5 字节 char_list.index 拼接 → PIE base → getflag = ans+0x1349
- 22003 Elden Ring → taunt 写 got['free'] 任意地址 → Mergot dropped → libc base
- NG+ 二轮堆溢出 → /bin/sh + freehook+system → shell
- Magic 22003 MTG → load(offset, ...) 任意读 puts got → libc → one_gadget
failed_attempts:
- 试图不拿 seed 预测 libc.rand → 失败：必须先 leak seed
- 试图直接 NG+ 不拿 libc → 失败：第二轮堆溢出需要 freehook 地址
key_observations:
- libc.srand 接受 4 字节 seed，team name 缓冲区紧接其后即可泄 seed
- ANSI color escape 序列 \x1B[30;...m 触发是 PIE 泄露常见路径
- Elden Ring 风格题关键是找到 got 写入路径 (taunt 字段长度控制)
- NG+ 二轮机制是 libc leak + freehook 利用的组合套路
prerequisites:
- ctypes + libc.srand/rand PRNG 预测
- ANSI color escape 解析 (\x1B[...m)
- pwntools ELF/got/PLT 操作
- libc-2.27 tcache + glibc 2.31+ 区别
---
# SunshineCTF pwn
