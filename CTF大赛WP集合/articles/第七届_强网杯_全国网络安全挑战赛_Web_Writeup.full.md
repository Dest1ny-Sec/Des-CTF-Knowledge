...
tags:
- 强网杯Web
- xcache read-only protection
- mmap RO/RW
- /tmp/clean_b3
- xc_php_find_unlocked hook
- 函数指针patch
- mprotect
...
quality: high
full_path: 第七届_强网杯_全国网络安全挑战赛_Web_Writeup.full.md
meta_path: 第七届_强网杯_全国网络安全挑战赛_Web_Writeup.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: 第七届 强网杯 全国网络安全挑战赛 Web Writeup。第七届强网杯Web：xcache只读保护+mmap两次+xc_php_find_unlocked hook+函数指针patch。经验：xcache启用read-only protection时同文件被mmap两次（ro+rw），攻击者需patch映射到r...
category: web
...
contest_type: open
wp_url: https://www.ctfiot.com/151907.html
reasoning_chain:
- 触发点：xcache 启用 read-only protection，同一文件被 mmap 两次(ro_addr + rw_addr, 0x4000000 间隔)→假设：可 patch rw 映射
- 动作：open /tmp/clean_b3 + mmap(0x7fb5ed09c000, RO) + mmap(0x7fb5ed09c000+0x4000000, RW)→观察：两次 mmap 成功
- 假设：hook xc_php_find_unlocked 第一次返回 fake mmap 地址→动作：is_replaced_1==0 时强制 return ptr=0x7fb5f10bc1e8
- 观察：xcache 用 mmap 地址而非文件路径作为键→下一步：从 mmap 区域读函数指针
- 假设：用函数指针后 3 位(0xff\xb5\x7f)找 libphp 基址→动作：本地用正则匹配 .*\xf6\xff\x7f 替换
- 观察：把远程函数指针替换成本地→下一步：触发 xcache 解析逻辑
- 动作：发送含恶意 PHP 代码请求→观察：xcache 用本地指针执行，反序列化 RCE
failed_attempts:
- 试图直接写 ro 映射 → 失败：ro 区域只读
- 试图走 libphp 直接调用 → 失败：xcache 用 mmap 隔离
key_observations:
- xcache read-only protection 同时映射 ro+rw 两块 0x4000000 区间，攻击者需 patch rw 区域
- 函数指针后 3 位 0x__\xff\x7f 是匹配 libphp 基址的特征字节
- xc_php_find_unlocked 是 hook 点(第一次调用返回 fake mmap 替换)
- 本题是 PHP 扩展层漏洞，需要理解 xcache internals
prerequisites:
- xcache 内部工作原理(read-only protection 实现)
- Linux mmap(MAP_PRIVATE|MAP_FIXED)
- PHP 扩展 hook 编写
- 函数指针替换技术
---
