...
- PRNG
attack_chain:
- IDA 看 main 是巨大的 switch state machine（OLLVM flatten 混淆）
- 分析 lib0opsPass.so 找 Oops::CryptoUtils::prng_seed + scramble32
- crc32('FTC0', &key, 3) == 0xF9E319A6 爆破前 3 字节 = 179, 197, 140
- scramble32 = 4 轮 AES-like XorKey+SubBytes+MixColumn+XorKey
- 用 AES_PRECOMP_TE[i](x) = MixColumns(SBox(x) << 8*i) 验证 = AES SBox
- key 用于 scramble plains[65806] 数组
- PRNG 是 8 位非线性 S-box TABLE + 线性 bit 累加，构造 GF(2) 线性系统
- Sage 解 128×16 线性方程组 → 16 字节 seed
key_payload: ''
...
quality: high
full_path: 0CTF_2017-zer0IIvm题目分析.full.md
meta_path: 0CTF_2017-zer0IIvm题目分析.meta.md
images_removed: true
images_removed_count: 1
schema_version: v3.0.0-P0
summary: 0CTF 2017 - zer0IIvm 题目分析。OLLVM flatten + AES-like scramble + CRC32 爆破前 3 字节 + GF(2) 线性方程恢复 PRNG seed。关键路径：IDA 看 main 是巨大的 switch state machine（OLLVM flatten 混淆） → 分析 lib0opsPass.so 找 Oops::CryptoU...
category: reverse
subcategory: reverse
subcategories:
- reverse
...
contest_type: open
wp_url: https://www.ctfiot.com/44501.html
reasoning_chain:
- main 函数 4000+ 行全是 switch(state) + goto → 触发点：典型 OLLVM flatten 混淆模式
- 假设：直接反编译无法读逻辑 → 动作：在同目录 lib0opsPass.so 里 grep CryptoUtils/prng/seed
- 观察：定位到 Oops::CryptoUtils::prng_seed/get_bytes/scramble32 → 下一步：分析这三个函数
- scramble32 = 4 轮 XorKey+SubBytes+MixColumn+XorKey → 假设：AES-like 自定义 S 盒
- 动作：写 precomp.py 用 AES 标准库 MixColumn 逆运算还原 AES_PRECOMP_TE[i] → 观察：与 TABLE 完全一致
- CRC32('FTC0', &key, 3) == 0xF9E319A6 → 假设：硬编码校验可爆破前 3 字节 → 动作：循环 256³ 爆破
- 观察：前 3 字节 = (179, 197, 140) → 下一步：AES 密钥还差 13 字节，遍历 2^104 不可能
- 假设：必须恢复 PRNG seed → 动作：分析 encrypt() 的 8 轮循环
- 观察：每轮 buf 更新是 TABLE(buf) 整体置换 → 假设：buf 在 GF(2) 上与 seed 线性相关
- 动作：Sage 构造 128×128 矩阵解 GF(2) 线性方程组 → 观察：恢复 16 字节 seed
failed_attempts:
- 试图直接调 AES 标准解密 plains[65806] 数组 → 失败：缺完整 16 字节密钥
- 试图猜 AES S-Box 内容 → 失败：TABLE 是自定义 256 字节 S 盒
- 对 TABLE 直接做 GF(2) 化简 → 失败：TABLE 是非线性映射，整体非线性
key_observations:
- OLLVM flatten 必有 PRNG seed + state machine，定位 state 是去混淆第一步
- AES_PRECOMP_TE[i](x) = MixColumns(SBox(x) << 8*i) 是 AES 标准 S 盒的等价表达
- CRC32 校验 + 已知明文爆破是经典密码侧信道手法
- 非线性 S 盒 + 线性 bit 累加 = GF(2) 线性系统（Side-channel 攻击通用思路）
- Sage + z3 是 CTF Crypto 必备工具链
prerequisites:
- OLLVM 反混淆原理（理解 flatten pass 的 switch state machine 模式）
- AES 密钥调度（SubBytes / MixColumns 关系）
- GF(2) 线性代数基础
- Sage 数学工具使用
---
