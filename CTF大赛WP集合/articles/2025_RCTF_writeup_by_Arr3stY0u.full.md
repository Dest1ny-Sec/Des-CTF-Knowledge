...
quality: high
full_path: 2025_RCTF_writeup_by_Arr3stY0u.full.md
meta_path: 2025_RCTF_writeup_by_Arr3stY0u.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: 2025 RCTF writeup by Arr3stY0u（GMP MT19937 LSB 格攻击 + seed2 缩放）。2025 RCTF 高级 crypto：GMP MT19937 自实现 + WARM_UP 2000 + 二次幂模映射 + LSB 1 比特输出，624 LSB 完整观测后用 F2 上 LLL 反推 seed2 → 还原 secret。。关键路径：用户 secret=...
category: crypto
...
contest_type: open
wp_url: https://www.ctfiot.com/282850.html
wp_author: Arr3stY0u
reasoning_chain:
- '[触发点] GMP MT19937 + 自实现 WARM_UP=2000 + 取 LSB 1 bit 输出 → 假设：624 个连续 LSB 可还原 seed → [动作] 矩阵元素 = seed2 → mt[] → twist/temper → LSB 线性变换 → F2 上 LLL → [观察] 还原 seed2'
- '[触发点] 自定义哈希 SuanHash + AES_ECB_perm 未知密钥但固定 → 假设：挤出阶段线性关系 → [动作] 构造 blk1||R vs blk2||(R⊕Δ) 两个消息 → [观察] 两次哈希输出有可逆关系 → [下一步] 恢复 Δ'
- '[触发点] 二次幂模映射 + seed2 缩放 → 假设：缩放后位填充 mt[0..623] → [动作] 第 1 位 = mt[0] 最高位 bit31 + 剩余位按 32 位小端拆分 → [观察] 完整 mt 还原'
- '[触发点] Lattice RSA 4 组 (e, n, c) 共享 d → 假设：构造 5x5 格 M = diag(N, M1, M2, M3, 1) → [动作] B = [[M, e0, e1, e2, e3], [0, -n0, 0, 0, 0], ...] LLL → d = L[0][0]/M → [观察] 拿到 d 解密'
failed_attempts:
- 试图用现成 MT19937 crack 工具 → 失败：自定义 WARM_UP + 缩放 + 取 LSB
- 试图不解 SuanHash 挤出线性关系 → 失败：AES 密钥未知也无法逆
- 试图爆破所有 4 组 d → 失败：必须 Lattice 共私钥攻击
key_observations:
- GMP 内置 MT19937 是 gmpy2 / pycryptodome 测试向量的常见来源
- 自定义 WARM_UP + temper 后取 LSB 是抗直接 MT crack 套路，但只要能取 624 个连续 LSB，F2 上 LLL 仍可还原
- AES-CTR 未知密钥但固定时，利用挤出阶段线性关系可恢复 Δ
- 多 RSA 共享 d 是经典 Lattice 攻击（Howgrave-Graham / Nguyen）
prerequisites:
- MT19937 内部状态还原
- F2 上 LLL 算法（Sage LLL().change_ring(GF(2))）
- AES-CTR 哈希挤出性质
- Lattice RSA 共私钥攻击
---
