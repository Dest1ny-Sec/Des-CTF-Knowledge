...
quality: high
full_path: ASIS_CTF_Quals_2022_WP.full.md
meta_path: ASIS_CTF_Quals_2022_WP.meta.md
images_removed: true
images_removed_count: 6
schema_version: v3.0.0-P0
summary: 'ASIS CTF Quals 2022 WP (狼组版)。ASIS 2022 Quals 狼组复盘 — eval f.name 拿 flag + ret2libc 二次 payload + (a+1)^n mod n^3 还原 + Chaffymasking LTC 矩阵 salt 漏洞。。关键路径：Beginner ducks: /duck?what=f.name eval 触发 f.na...'
category: web
...
contest_type: open
wp_url: https://www.ctfiot.com/63407.html
reasoning_chain:
- 'Beginner ducks: /duck?what=f.name → 触发点：eval() 触发 f.name'
- 假设：eval('f.name') 返回文件名 → 动作：访问 /duck?what=f.name
- 观察：返回 /flag.txt 路径 → 直接读 flag.txt
- 'Baby scan I: gets(g.input_buf) + printf(l->description) → 触发点：fmt-string + stack overflow'
- 假设：fmt-string 泄栈地址 → 动作：覆盖 description + %3$lx
- 观察：ret_addr 拿到 → 假设：再 gets 覆盖 choice struct
- 动作：payload 0x2028 + p64(stack+0x1428*11) → 观察：跳到 SECRET ROOM
- 'Binned: (a+1)^n mod pubkey^3 → 触发点：3 项式展开'
- 假设：h = (enc-1) - pubkey → c = h // pubkey // pubkey → 假设：c*2 = 中间系数
- 动作：解出 m → Chaffymasking LTC 矩阵
- 'Chaffymasking: salt 128 bit + half1 XOR half2 != 0 → 假设：跳过 os.urandom 注入'
- out_1, out_2 = LTC·vec1 mod n² → 假设：XOR 还原 → 动作：solve LTC
failed_attempts:
- 试图 gets 溢出到 system → 失败：NX + 缺 libc 知识
- 试图直接逆 (a+1)^n → 失败：mod n³ 只剩 3 项必须用模运算拆
key_observations:
- Python eval(f.name) 文件描述符属性泄露路径
- mod n³ 后 (a+1)^n 展开剩 3 项 m + m(m-1)/2*pubkey + m(m-1)(m-2)/6*pubkey²
- Chaffymasking half1 XOR half2 != 0 漏洞绕过 os.urandom
- gets + printf 描述信息 = fmt-string 泄漏栈地址
- text-adventure 类游戏 SECRET ROOM 常是隐藏 flag 点
prerequisites:
- Python eval 边界与 f.name 等文件描述符属性
- gets + printf 描述信息 fmt-string 泄漏栈
- LTC 矩阵 + CRT 复合 (n², n² mod)
---
