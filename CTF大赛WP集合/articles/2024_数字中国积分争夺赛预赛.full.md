...
quality: high
full_path: 2024_数字中国积分争夺赛预赛.full.md
meta_path: 2024_数字中国积分争夺赛预赛.meta.md
images_removed: true
images_removed_count: 3
schema_version: v3.0.0-P0
summary: 2024 数字中国积分争夺赛预赛。SM3 哈希扩展 + RSA p^4+tp 隐变量多项式 resultant small_roots。。经验：RSA 模数 p^4+tp 时可以把 a 当成未知数构造多项式环 mod n,Resultant GCD 是还原 m 的...
category: crypto
subcategory: hash
subcategories:
- hash
...
contest_type: open
wp_url: https://www.ctfiot.com/179722.html
reasoning_chain:
- 触发点：hash_append 服务接收 AppendData + MySecretInfo Hash → 假设：SM3 哈希扩展 → 动作：分析 sm3_hash 调用
- 观察：64 字节盐 + padding1 64 字节（固定） → 假设：HashValue = sm3(secret||padding1) 的 IV → 动作：构造 AppendData = padding2
- 触发点：sm3_hash_ext(64, '', known_hash, AppendData) → 假设：复用 HashValue 作 IV → 动作：计算 NewHashValue
- 观察：还原 NewHashValue 与服务端一致 → 假设：拿到 flag → 动作：submit
- 触发点：trivial_RSA 模数 p^4 + tp（512-bit 随机 + 20-bit tp） → 假设：隐变量多项式攻击 → 动作：构造 g(a) = sum(r[i]*a**i) - z mod n
- 观察：a^65537 mod g fast_pow → 假设：构造 f(m, a) = m*256 + flag_known - a_e - c^e → 动作：Sylvester resultant(h, a).det()
- 触发点：h.small_roots(X=2^72, epsilon=0.07) → 假设：还原 m → 动作：sage solve
- 观察：解出 m → 假设：flag 明文 → 动作：long_to_bytes
- 触发点：mbruteforce 验证 → 假设：sage small_roots 返回多个解 → 动作：遍历候选取正确 m
failed_attempts:
- 试图不解 SM3 哈希扩展 → 失败：必须 64 字节块长 IV 复用
- 试图不解 Sylvester resultant → 失败：p^4+tp 必须消元
- 试图不解 small_roots → 失败：必须 X=2^72 + epsilon=0.07
- 试图不解 20-bit tp → 失败：必须多项式环降阶
key_observations:
- RSA 模数 p^4+tp 时可以把 a 当成未知数构造多项式环 mod n,Resultant GCD 是还原 m 的关键
- SM3 哈希扩展与 MD5/SHA1 同理：64 字节块长 IV 复用
- Sylvester resultant(h, a).det() 是多项式环 GCD 标准做法
- small_roots X=2^72 epsilon=0.07 是 Coppersmith 类小根攻击参数
- mbruteforce 是 sage small_roots 多解验证的标准技巧
prerequisites:
- SM3 哈希扩展攻击原理
- RSA p^4+tp 隐变量多项式构造
- Sylvester resultant + 多元多项式 GCD
- Coppersmith small_roots + epsilon 参数选择
- Sage 数学工具
---
# 2024 数字中国积分争夺赛预赛
