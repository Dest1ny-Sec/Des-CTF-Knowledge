...
quality: medium
full_path: ACTF_2025_writeup_by_Mini-Venom.full.md
meta_path: ACTF_2025_writeup_by_Mini-Venom.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: 'ACTF 2025 writeup by Mini-Venom。ACTF 2025 — upload 任意文件读 + admin backdoor 密码爆破 + os.system 命令注入 + not so web 1 AES-CBC cookie 用户态加密。。关键路径：ACTF upload: upload?file_path=../../../../app/app.py 任意文件读源...'
category: web
...
contest_type: open
wp_url: https://www.ctfiot.com/243984.html
wp_author: Mini-Venom
reasoning_chain:
- upload?file_path=../../../../app/app.py 返回源码 → 触发点：任意文件读
- 假设：源码含 admin 密码 hash → 动作：cat admin 字段 sha256=32783cef...
- 观察：撞库 sha256 → 假设：这是 backdoor 不是真实密码哈希
- 动作：Google 32783cef 字符串 → 观察：撞字典出 backdoor 密码
- admin 路由走 os.system(f'base64 {file_path} > /tmp/...') → 触发点：命令注入
- '动作：file_path=; ls / > /tmp/aaa # → 观察：tmp 文件显示 Fl4g_is_H3r3'
- 动作：file_path=../../../../Fl4g_is_H3r3 → 观察：base64 解码得 flag
- not so web 1 AES-CBC cookie：iv + pkcs7 pad + AES 加密 → 触发点：cookie 伪造
- 假设：管理员 cookie 含 ADMIN_PASSWORD = raw 值 → 动作：分析 generate_cookie/validate_cookie
- 观察：自定义 APPUser + dataclass → 假设：自定义 AES 可重放 iv → 动作：构造合法管理员 cookie
- 假设：iv 随机但密钥固定 → 动作：复用 iv + 改 plaintext 加密 → 观察：服务端 json.loads 成功校验
failed_attempts:
- 试图注册新账号 → 失败：cookie 加密需 ADMIN_PASSWORD
- 试图爆破 IV 32 字节 → 失败：iv 长度正确但缺 key
key_observations:
- Flask 路由 upload?file_path=XX 经典路径穿越 → 任意文件读
- 硬编码 hash=backdoor 是 CTF 管理员入口常见骚套路
- os.system 拼接 file_path + base64 > /tmp = 反引号命令注入
- 自定义 AES-CBC cookie 用 iv 随机但 key 固定 → 可重放
- json.loads + validate_cookie 双层校验是 Web 鉴权标准链
prerequisites:
- Flask 路径穿越基础（../ 与 route 参数）
- Python hashlib 与字典爆破
- AES-CBC cookie 加密与 PKCS7 padding
---
