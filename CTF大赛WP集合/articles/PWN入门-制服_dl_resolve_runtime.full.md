...
quality: high
full_path: PWN入门-制服_dl_resolve_runtime.full.md
meta_path: PWN入门-制服_dl_resolve_runtime.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: PWN入门 - 制服 _dl_runtime_resolve。深度逆向 glibc 动态链接器 _dl_runtime_resolve_fxsave 内部，从 PLT 跳转到 _dl_fixup 查找符号的完整调用链。。关键路径：介绍 ELF 动态链接的两类时机：启动期 vs 调用期（延迟绑定） → 详解 .plt/.plt.sec/.got/.got.plt/.rela.plt/.dyns...
category: pwn
...
contest_type: open
wp_url: https://www.ctfiot.com/206959.html
reasoning_chain:
- 触发点:程序调动态库函数首次未解析时跳 PLT → jmp *GOT → push reloc_index + jmp _dl_runtime_resolve → 假设:深入 _dl_runtime_resolve_fxsave 内部
- _dl_runtime_resolve_fxsave 序言 push rbx / mov rsp,rbx / and 0x10 对齐 / sub 0x240 → 假设:栈帧对齐 + FPU 上下文保存
- 动作:fxsave 0x40(%rsp) 保存 SSE/MMX → 观察:6 个参数寄存器入栈 + 浮点上下文
- 从 rbx 取 link_map+reloc_index 调 _dl_fixup(link_map, reloc_index) → 假设:_dl_fixup 才是符号解析本体
- _dl_fixup 从 link_map[0x70] 取 SYMTAB → l_info[] 检索 .dynsym/.dynstr → 假设:SYMTAB 是符号表起点
- version check + strcmp 比 GLIBC_x.y → 假设:GLIBC 版本号匹配 → 动作:返回值 r11 → jmp *r11 真正调到目标
- WP quality=high 但偏教程,直接记录 _dl_runtime_resolve → _dl_fixup 全链路
failed_attempts:
- 试图只反汇编 _dl_runtime_resolve 入口 → 失败:仅看到 push/jmp,需要进 _dl_fixup
- 试图跳过 fxsave 直接看 _dl_fixup → 失败:_dl_fixup 假设栈帧已对齐
key_observations:
- GOT[plt] 首次未解析时跳 _dl_runtime_resolve,完成后 GOT 写真实地址
- link_map[0x70] 偏移是 ELF 动态链接的固定布局
- strcmp GLIBC 版本号是 dl_fixup 兼容性的关键
prerequisites:
- ELF 格式(.plt/.got/.rela.plt/.dynsym/.dynstr)
- glibc 动态链接器源码阅读
- FPU/MMX/SSE 上下文保存(fxsave/fxrstor)
---
