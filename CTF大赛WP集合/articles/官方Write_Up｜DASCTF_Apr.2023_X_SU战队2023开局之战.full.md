...
quality: high
full_path: 官方Write_Up｜DASCTF_Apr.2023_X_SU战队2023开局之战.full.md
meta_path: 官方Write_Up｜DASCTF_Apr.2023_X_SU战队2023开局之战.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: DASCTF Apr.2023 X SU战队2023开局之战官方WP。DASCTF Apr.2023 X SU战队开局之战BLOCKCHAIN 4题:闪电贷操纵预言机/Conflux eSpace签名/UnCrackableGame delegatecall/Guess Range重入。。经验：闪电贷借出大量代币操纵预言机兑换比例(1:1→1000:1)是经典DeFi攻击;构造特殊Inten...
category: misc
...
contest_type: open
wp_url: https://www.ctfiot.com/111908.html
reasoning_chain:
- DASCTF Apr.2023 BLOCKCHAIN 4 题：闪电贷操纵预言机 + Conflux eSpace 签名 + UnCrackableGame delegatecall + Guess Range 重入 → 触发点：DeFi 经典攻击面
- 这必不可能是预言机：Hacker 合约构造 Intention + 链下 ECDSA 签名 + 闪电贷借 999 WETH → 假设：操纵兑换比例 1:1000 → 动作：存 1 WETH 换 1000 FLAG → 转给 Challenge
- 观察：链上 Poseidon.Blockchain 交互成功 → 到国链之光：Conflux eSpace Testnet 水龙头领 gas → block.timestamp + keccak256 猜 key → getFlag 触发 CaptureTheFlag 事件
- easyCoin：delegatecall 存 owner 可改 → 假设：直接 call → 动作：写 attacker 合约
- Guess：Range + 重入 → 假设：用重入绕过 checkEffect → 动作：fallback 二次猜
- 观察：4 题全解
failed_attempts:
- 试图用真金白银买 FLAG → 失败：必须闪电贷
- Conflux eSpace 直接 send 主网 → 失败：必须 Testnet 领 gas
- 易受攻击合约用 transfer 不行 → 失败：必须用 send 触发 fallback
- Guess 不用回退函数 → 失败：必须 fallback 重入
key_observations:
- 闪电贷借大额代币操纵预言机兑换比例是 DeFi 经典攻击
- 构造特殊 Intention + 链下 ECDSA 签名 (v,r,s) 绕过 _verifyIntention
- Conflux eSpace Testnet 需先领 gas
- UnCrackableGame 经典 delegatecall + storage 覆盖
- block.timestamp + keccak256 作 key 可预测
prerequisites:
- Solidity 0.8 合约 + IERC3156FlashBorrower 接口
- DeFi 闪电贷机制（Aave/dYdX/Uniswap）
- ECDSA 签名 + ecrecover 验证
- Conflux eSpace Testnet 交互
---
