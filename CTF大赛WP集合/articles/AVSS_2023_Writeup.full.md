...
quality: high
full_path: AVSS_2023_Writeup.full.md
meta_path: AVSS_2023_Writeup.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: 'AVSS 2023 Writeup。AVSS 2023 移动安全+内核 — Parcel LaunchAnywhere + broadcast 触发 + Android 7/8/9/10 内核栈溢出 ROP + canary 泄漏 + mmap 共享内存 + cross-slab funcptr + msg_msg UAF。。关键路径：APP-VulnParcel: Parcel.write...'
category: pwn
...
contest_type: open
wp_url: https://www.ctfiot.com/132758.html
reasoning_chain:
- 'APP-VulnParcel: Parcel.writeInt + Bundle.appendFrom 构造 Bundle[-1]=4c444e42 → 触发点：LaunchAnywhere'
- 假设：触发 VulnParcelable 反序列化 → 动作：构造 IGenerateMalformedParcel
- 观察：am broadcast -a com.avss.testreceiver.GET_FLAG 触发接收器
- 'expReceiver: --es sms_body ''1'' + logcat -d | grep flag → 触发点：logcat 日志泄漏'
- 'Kernel-kStackOverflow: __NR_stackof 601 + my_ctu 触发栈溢出 → 假设：ROP 提权'
- 动作：buf[0x100:]=0x61 + prepare_kernel_cred + 0x63 + commit_creds + 0x65 + ret_to_user + 0x67 + get_root
- 观察：get_root(0) 读 /system/flag → mmap 0xabcd00000 共享内存写 flag
- 'Android 10: canary 必查 → syscall len=0x108^0xdeadbeef 读 buf+0x100 leak'
- 动作：mmap 共享内存传 flag → Kernel-kHeapUserCopy
- 'kHeapUserCopy: __NR_easyof 602 + dmesg 读 slab 地址 → 假设：改 funcptr'
- 'kSysUAF: kfree 不置 NULL → msg_msg 复用 → 触发点：cross-cache UAF'
failed_attempts:
- 试图直接 prepare_kernel_cred(0) → 失败：缺 ret_to_user 桥
- 试图单 gadget 提权 → 失败：ret_to_user 后才能切回用户态
key_observations:
- Parcel Bundle[-1]=4c444e42 是 LaunchAnywhere 反序列化漏洞经典 trigger
- ROP 链 4 个 0x61/0x63/0x65/0x67 是 gadgets 占位 padding
- Android 10 起 SMEP/SMAP + canary 必查，先 leak 再 ROP
- mmap 共享内存是内核态读 flag 传到用户态的常用手法
- kfree 不置 NULL 是经典内核 UAF 漏洞模式（msg_msg cross-cache）
prerequisites:
- Android Parcel/Bundle 反序列化机制
- Linux 内核 ROP (commit_creds + prepare_kernel_cred + ret_to_user)
- msg_msg 内核对象 cross-cache 利用
---
# AVSS 2023 Writeup
