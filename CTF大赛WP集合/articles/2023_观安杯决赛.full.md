...
quality: medium
full_path: 2023_观安杯决赛.full.md
meta_path: 2023_观安杯决赛.meta.md
images_removed: true
images_removed_count: 3
schema_version: v3.0.0-P0
summary: 2023 观安杯决赛。AWDP 实战：pma 弱口令+CVE-2018-12613 读 flag + ThinkPHP6 反序列化 RCE。。关键路径：web1 blog 框架，admin/pma 是 phpMyAdmin 后台 → database.php 找到弱口令 ctf:ctf → SQL UPDATE ctf SET pass='' where user_id=1 改密。经验：AW...
category: web
...
contest_type: open
wp_url: https://www.ctfiot.com/133165.html
reasoning_chain:
- 触发点：AWDP 模式 web1 blog 框架 admin/pma 目录 → 假设 pma 后台 → 动作：翻 database.php
- 观察：MYSQL_USER=ctf, MYSQL_PASSWORD=ctf 弱口令 → 下一步：SQL UPDATE 改密防别人打
- 动作：pma 弱口令登录 + CVE-2018-12613 任意文件读 → 观察：根目录 flag 可读
- 动作：admin弱口令（爆破）打其他队伍 → 观察：拿到大部分队伍的 flag
- 触发点：web2 ThinkPHP6 框架 public/sendtodatabase.php → 假设命令注入 → 动作：observed 找链
- 动作：app/encrypted.py 用 e=23 + 6 个 modulus加密 flag → 假设低指数广播
- 动作：构造 think\\Model + think\\Request(filter=system,param=[whoami]) POP 链 → 观察：触发反序列化
- 动作：think\\session\\Store + think\\log\\Channel + think\\route\\Url + think\\model\\Pivot 链 → 观察：RCE 拿 /flag
failed_attempts:
- 试图写 shell 进 pma → 失败：ctf 用户权限低不能写文件
- 试图手工读其他队伍 encrypted_flag → 失败：没弱口令只能等系统漏洞
key_observations:
- AWDP 关键是抢弱口令（ctf:ctf 通用）+ 立即改密
- phpMyAdmin CVE-2018-12613 是经典后台任意文件读漏洞
- ThinkPHP6 反序列化链：Model+Request+Store+Channel+Url+Pivot 全链路触发
- e=23 + 6 个 modulus 是低指数广播可 CRT 还原
prerequisites:
- AWDP 比赛模式与攻防时间窗口
- phpMyAdmin 后台利用与 CVE 漏洞
- ThinkPHP6 反序列化 POP 链构造
- RSA 低指数广播攻击
---
# 2023 观安杯决赛
