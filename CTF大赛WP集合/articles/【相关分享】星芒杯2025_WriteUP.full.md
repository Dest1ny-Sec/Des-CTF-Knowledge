...
quality: medium
full_path: 【相关分享】星芒杯2025_WriteUP.full.md
meta_path: 【相关分享】星芒杯2025_WriteUP.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: 【相关分享】星芒杯 2025 WriteUP。星芒杯 2025 Web WP，代码审计 call_user_func RCE + ISO-8859-1/UTF-8 编码还原 + 盲文解码 + LFI 文件读取。。经验：call_user_func($func, $param) 是 PHP 经典 RCE；ISO-8859-1 → UTF-...
category: web
...
contest_type: open
wp_url: https://www.ctfiot.com/285553.html
reasoning_chain:
- '代码审计题: PHP 源码里 call_user_func($func, $param) → 触发点: 用户可控函数名'
- '假设: 直接传 system+id 可 RCE → 但题目问的是''导致 flag 输出的核心函数名'''
- '观察: define(''FLAG'', ... ) + show_flag($key) 调用 call_user_func → 答案 flag{call_user_func}'
- '布莱切利庄园: username 是乱码 (双重编码) → 假设: latin1 字节流 → utf8 重解'
- '动作: ''乱码''.encode(''latin1'').decode(''utf-8'') → 还原 中文用户名'
- 'password 是盲文 → 假设: 标准盲文编码 → 动作: 盲文解码表 → 得 darkguard98/secretgate45'
- 登录后 /archives?file= 任意文件读取 → 读 /flag 是 sstv 音频 → base64 导出 → 听 → flag
failed_attempts:
- '试图传 system(''cat /flag'') → 失败: 题目问函数名不是拿 flag'
- '用 utf-8 直接解乱码 → 失败: 双重编码, 必须 latin1→bytes→utf-8 反向'
key_observations:
- call_user_func($func, $param) 是 PHP 经典 RCE 入口, 函数名即可控就是灾难
- ISO-8859-1 (latin1) → UTF-8 双重编码还原是乱码恢复通用思路
- 盲文密码 = 标准 6 点制, 直接查表可解码
- ?file= LFI 必须配合 filter/base64 才能读二进制 (Servlet class)
prerequisites:
- PHP 代码审计基础 (call_user_func/可变函数/preg_replace)
- 字符编码原理 (latin1/utf-8/gb2312 区别)
- 盲文 (Braille) 6 点编码表
- SSTV 音频解码 + LFI 文件读取
---
