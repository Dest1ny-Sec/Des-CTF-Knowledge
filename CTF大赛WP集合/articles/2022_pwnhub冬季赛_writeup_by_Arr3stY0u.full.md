...
attack_chain:
- 'Web Reset: 上传表单 ../.git/objects/5f/f1ef5c03448a1eb5571dd348cf717a7bad7402 + zlib 0x78 0x9C 头'
- 'PPC TCP_show: 解析 PCAP 包 16 字节/行 hexdump'
- 'MISC 飞驰人生: 二分法找超速报文 244#000000A60000 + 锁车报文 19B#00000F000000 → flag{244#000000A60000_19B#00000F000000}'
- 'MISC 坐井观天: 拼 chr() 字符串绕过字符限制 eval'
- 'Crypto ASR: gcd(RSA1, N) → s 大因子 → invert(65537, s-1) → flag'
- 'Crypto NN: LLL 短向量同时还原两个明文'
- 'Crypto POINT: 已知 e*d 范围 → randint g^((ed-1)/2^k) mod n → gcd(x-1, n) → p,q'
- 'Crypto: hlextend_bytes hash length extension attack'
...
quality: high
full_path: 2022_pwnhub冬季赛_writeup_by_Arr3stY0u.full.md
meta_path: 2022_pwnhub冬季赛_writeup_by_Arr3stY0u.meta.md
images_removed: true
images_removed_count: 6
schema_version: v3.0.0-P0
summary: '2022 pwnhub 冬季赛 writeup by Arr3stY0u。pwnhub 2022 冬季赛：git-objects + TCP hexdump + CAN-bus + RSA-gcd + LLL + 长度扩展。关键路径：Web Reset: 上传表单 ../.git/objects/5f/f1ef5c03448a1eb5571dd348cf717a7bad7402 + zlib...'
category: web
subcategory: lfi
subcategories:
- lfi
...
contest_type: open
wp_url: https://www.ctfiot.com/87197.html
wp_author: Arr3stY0u
reasoning_chain:
- '[触发点] Web Reset 上传 ../.git/objects/5f/f1ef5c03448a1eb5571dd348cf717a7bad7402 + zlib 0x78 0x9C 头 → 假设：git 还原 / [动作] 上传 / [观察] git 历史 / [下一步] TCP_show'
- '[触发点] TCP_show 16 字节/行 hexdump → 假设：解析 PCAP / [动作] scapy 解析 / [观察] 拿到 flag / [下一步] 飞驰人生'
- '[触发点] 飞驰人生 244#000000A60000 超速 + 19B#00000F000000 锁车 → 假设：CAN-bus 二分 / [动作] 二分找报文 / [观察] flag{244#000000A60000_19B#00000F000000} / [下一步] 坐井观天'
- '[触发点] 坐井观天 chr() 拼字符串 → 假设：eval 字符限制 / [动作] 拼 chr() / [观察] RCE / [下一步] ASR'
- '[触发点] ASR RSA gcd(RSA1, N) = s → invert(65537, s-1) → 假设：RSA 共享素数 / [动作] gcd / [观察] flag / [下一步] NN + POINT'
- '[触发点] POINT已知 e*d 范围 → randint g^((ed-1)/2^k) mod n → gcd(x-1, n) → 假设：双素数 / [动作] / [观察] p,q / [下一步] hash长度扩展'
failed_attempts:
- 试图直接 unpack git object → 失败：缺 zlib 头
- 试图手算 CAN ID → 失败：必须二分
key_observations:
- PCAP 协议逆向 + git-objects 还原是 MISC 入门
- gcd(RSA1,N) 是 RSA 经典
- LLL 双明文恢复
- hash length extension 需 hlextend 库
prerequisites:
- git object 还原 + zlib
- scapy PCAP 解析
- CAN-bus 协议
