...
quality: high
full_path: 官方WP_-_AI对抗样本攻击赛题AdvDetPatch解析.full.md
meta_path: 官方WP_-_AI对抗样本攻击赛题AdvDetPatch解析.meta.md
images_removed: true
images_removed_count: 8
schema_version: v3.0.0-P0
summary: AI对抗样本攻击赛题AdvDetPatch官方WP(yolov5对抗补丁)。中科大+Maple_Leaves一血+哈工大Lilac二血yolov5目标检测对抗补丁+偷梁换柱(替换模型)非预期解。。经验：对抗样本白盒攻击经典方法:冻结模型+对input梯度下降+Adam优化+NMS前25200候选bbox置信度最小化;对抗...
category: misc
...
wp_url: https://www.ctfiot.com/118789.html
reasoning_chain:
- 伽玛实验场 AdvDetPatch yolov5 目标检测对抗补丁 + 一血偷梁换柱非预期 → 触发点：训练好的 yolov5l.pt 加载路径
- 一血思路：题目代码用 yolov5l.pt 相对路径加载 → 假设：可替换 → 动作：ln -s yolov5n.pt yolov5l.pt
- 观察：yolov5n 检出率大幅下降 → 触发非预期偷梁换柱得分
- 二血思路：白盒对抗补丁 = 冻结模型 + 对 input 梯度下降 + 保留梯度绝对值最大 20480 像素 + Adam 14 轮 → 假设：补丁是 mask+image 叠加 → 动作：每步更新 patch 参数
- 观察：conf_thres=0.25 下 NMS 前 25200 bbox 置信度被最小化 → 模型几乎检测不到
- 信安国赛 Deep_Learning 题：DeepFool 低扰动对抗样本骗过 wing 识别 → 假设：DeepFool 通用对抗算法可迁移 → 动作：torchattack foolbox DeepFool
failed_attempts:
- 试图训练 CNN 检测替代 yolov5 → 失败：题目代码固定加载 yolov5l.pt
- 试图用 PGD 全图像扰动而非补丁 → 失败：补丁面积约束 ≤5%，全图扰动违反约束
- 直接替换 yolov5l.pt 为空权重 → 失败：检查校验和
key_observations:
- 白盒对抗补丁方法：冻结模型 + 对 input 梯度下降 + Adam 优化最小化 NMS 前 bbox 置信度
- 相对路径加载 + ln -s 替换模型文件是非预期偷梁换柱的经典手法
- DeepFool 是信安国赛 Deep_Learning 当年 0 解题的正常路线
- 对抗补丁设计三要素：形状（网格/方形/圆/星形）、位置（检测框中心）、纹理
prerequisites:
- PyTorch 自动求导与 Adam 优化器
- yolov5 模型加载与检测流程
- DeepFool/FGSM 对抗样本原理
- Linux 命令行基础（ln -s 替换）
---
