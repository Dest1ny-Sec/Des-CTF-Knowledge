...
quality: medium
full_path: 津⻔杯-WriteUp.full.md
meta_path: 津⻔杯-WriteUp.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: 津⻔杯-WriteUp。津门杯：.swp源码+反序列化+flflag双写+gopher SSRF+时间盲注+USB流量。经验：.swp文件vim临时文件泄露+flflag双写绕WAF+gopher://SSRF打内网HTTP
category: web
subcategory: ssrf
...
contest_type: open
wp_url: https://www.ctfiot.com/1992.html
reasoning_chain:
- web 入口 /index.php 提示 .swp 文件 → 触发点：vim 临时文件 .index.php.swp 源码泄露
- 动作：访问 .index.php.swp → 观察：得到完整源码 + 反序列化入口
- 源码用 file_get_contents 读 file 参数 → 假设：路径穿越读 /flag
- 动作：/index.php?file=../../flag → 观察：flflag 双写绕过 WAF（关键词 'flag' 被拦）
- 假设：服务端对入参有 SQL 注入 → 动作：构造 admin.php 二次请求
- 动作：gopher://127.0.0.1:80/_POST%20/admin.php 走 SSRF 二次跳
- 盲注：mid((select group_concat(table_name)),{i},1)='{c}' and sleep(1) → 时间盲注
- 动作：Python 写爆破脚本 → 观察：得到 tables = emails,flag,referers,uagents,users
- 'MISC m0usb: 给一串 USB HID 8 字节数据 → 触发点：USB HID 键盘流量'
- 动作：tshark 提取 usb.capdata + normalKeys/shiftKeys 字典 → 还原按键
failed_attempts:
- 试图直接 file=/flag → 失败：file 关键词被 WAF 拦
- 试图 gopher:// 直连不打二次 → 失败：必须经 web 网关
- 试图用 union select 注入 → 失败：黑名单过滤 union/select
- 试图手动查 USB 数据 → 失败：数据量太大，必须脚本
key_observations:
- .swp 是 vim 临时文件，源码泄露经典漏洞
- flflag/flflag 双写绕 WAF 是字符串叠加过滤绕过模式
- gopher://127.0.0.1:80 可构造完整 HTTP 包打 SSRF
- mid((...),i,1)='c' and sleep(1) 是时间盲注通用 payload
- USB HID 数据 8 字节：第 0 字节 modifier + 第 2 字节 keycode
prerequisites:
- vim 临时文件命名规则（.swp / .swo）
- PHP 反序列化基础
- gopher 协议构造 HTTP 包
- 时间盲注原理（sleep + 比较）
---
