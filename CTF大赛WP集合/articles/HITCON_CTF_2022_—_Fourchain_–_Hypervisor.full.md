...
quality: high
full_path: HITCON_CTF_2022_—_Fourchain_–_Hypervisor.full.md
meta_path: HITCON_CTF_2022_—_Fourchain_–_Hypervisor.meta.md
images_removed: true
images_removed_count: 0
schema_version: v3.0.0-P0
summary: HITCON CTF 2022 — Fourchain – Hypervisor。HITCON CTF 2022 Fourchain Hypervisor：E1000 MMIO fake pDevIns劫持callback。关键路径：VirtualBox E1000 MMIO 任意读写 → 泄露 Ring 3 DeviceE1000->pDevIns = 0x178 偏移 → 读 pDevI...
category: misc
...
contest_type: open
wp_url: https://www.ctfiot.com/81562.html
reasoning_chain:
- 触发点：题目自定义了两个 IEM opcode（0F25=ReadTable, 0F27=WriteTable），且只允许 ring0+64-bit 模式调用 → 假设：可读写 host 端 E1000 设备的 MMIO 区域
- 动作：在 Guest 中读 /proc/iomem → 观察：e1000 mmio 落在 fd5c0000-fd5dffff → 下一步：执行 ReadTable/WriteTable 把 E1000_MMIO_BASE 写入 Table[i]
- 触发点：E1000 设备结构中偏移 0x178 = pDevIns → 假设：泄露 pDevIns 即可获得 R3/R0 共享结构指针 → 动作：用 ioremap(fd5c0000, 0x1000) 后 ReadTable 读相邻偏移
- 观察：拿到 Ring3 DeviceE1000->pDevIns + Ring0 pDevIns → 动作：在 Guest 中构造 fake_pDevIns，把 R3 pDevIns->pfnWriteCallback 改写为 system 地址
- 假设：触发 R3 MMIO write 会调到 system() → 动作：WriteTable 触发 E1000 寄存器写入，payload = "touch /tmp/456" → 观察：宿主机 /tmp/456 被写入
failed_attempts:
- 试图直接调 ReadTable 索引超出 Table 范围 → 失败：Table[0x10] 大小限制，越界会触发 IEMOP_RAISE_INVALID_OPCODE
- 试图改 Ring0 pDevIns → 失败：R0 端有 CR0/CPL 校验，custom opcode 只在 Ring0 模式下有效
key_observations:
- IEMAllInstructionsTwoByte0f.cpp.h 中 0F25/0F27 指令位是 VirtualBox 编译期就 patch 进 binary 的，逆向时必须先在 Ghidra/IDA 中 patch 0x25/0x27 的 Invalid 表
- E1000 MMIO 区域 fd5c0000 是 Guest 与 Host VMM 共享内存的关键，伪造 pDevIns->pfnWriteCallback = system 是通用 hypervisor 逃逸模板
- ReadTable/WriteTable 把 rbx 当索引、rax 当值，是把任意宿主内存当 bit 写入 R3/R0 寄存器的标准自定义 opcode 设计套路
- ring0 ring3 校验失败时只返回 IEMOP_RAISE_INVALID_OPCODE，不会 panic，便于 fuzzing
prerequisites:
- VirtualBox IEM (Instruction Emulation Module) 自定义 opcode patch 流程
- E1000 网卡 MMIO 寄存器布局（CTRL/STATUS/EERD/CTRL_EXT/FLA/MDIC 等 0x20 偏移）
- ioremap + ioread8/iowrite8 的 host 侧 mmio 访问
- Ghidra/IDA 逆向 VBox VMMAll 静态结构
---
