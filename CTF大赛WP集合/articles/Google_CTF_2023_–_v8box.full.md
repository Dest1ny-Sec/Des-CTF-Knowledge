...
quality: high
full_path: Google_CTF_2023_–_v8box.full.md
meta_path: Google_CTF_2023_–_v8box.meta.md
images_removed: true
images_removed_count: 9
schema_version: v3.0.0-P0
summary: 'Google CTF 2023 – v8box。Google CTF 2023 v8box：V8 sandbox+code pointer sandbox+JITless。关键路径：fetch v8: git switch -d d90d4533b05301e2be813a5f90223f4c6c1bf63d → gclient sync → git apply v8.patch + 000...'
category: web
...
contest_type: open
wp_url: https://www.ctfiot.com/251187.html
wp_author: applicable
reasoning_chain:
- v8box 题目附件 v8.patch + 0001-Protect-chunk-headers-on-the-heap.patch → 触发点：两 patch 是环境搭建核心
- 假设：v8 sandbox 启用 + code pointer sandbox → 动作：v8_enable_sandbox=true + v8_code_pointer_sandboxing=true
- 观察：JIT 启用会被 sandbox 限制 → 假设：必须 jitless 模式
- 动作：v8_jitless=true + 关 turbofan + maglev → 下一步：编译 d8
- 触发点：V8_EXPOSE_MEMORY_CORRUPTION_API 暴露漏洞触发函数 → 假设：用 API 模拟 heap 损坏
- 动作：gn gen out.gn/x64.release + ninja -C out.gn/x64.release -j 12 d8 → 观察：d8 编译成功
- 假设：d8 接受 JS 触发 API 调用 → 动作：编写 JS payload → 观察：内存损坏 API 调用
- 下一步：构造 heap chunk header 保护绕过逻辑 → 动作：分析 0001 patch diff
- 观察：patch 加固 chunk header 校验 → 假设：需找未保护字段触发 corruption
failed_attempts:
- 试图启用 turbofan/maglev JIT → 失败：sandbox 模式禁止 code pointer 越界
- 直接打 patch → 失败：git apply 顺序错导致冲突
- 试图用现成 d8 二进制 → 失败：必须按 patch 重新编译
key_observations:
- V8 sandbox + code pointer sandbox 是 Google CTF 防护标配
- jitless 模式是 sandbox 下唯一可行的攻击面
- V8_EXPOSE_MEMORY_CORRUPTION_API 是 CTF 专用漏洞暴露开关
- chunk header 保护 patch 提示漏洞位置
- V8 编译参数组合（sandbox / no-JIT / code pointer）必须全开
prerequisites:
- V8 源码编译流程（depot_tools / gn / ninja）
- V8 sandbox 机制原理
- JS 引擎 JITless 模式理解
- patch apply 与冲突解决
---
