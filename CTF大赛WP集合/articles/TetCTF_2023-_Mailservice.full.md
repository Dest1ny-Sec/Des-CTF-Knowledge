...
quality: high
full_path: TetCTF_2023-_Mailservice.full.md
meta_path: TetCTF_2023-_Mailservice.meta.md
images_removed: true
images_removed_count: 0
schema_version: v3.0.0-P0
summary: 'TetCTF 2023: Mailservice。TetCTF 2023 Mail Service：mailclient 读邮件栈溢出 + 攻击 mailserver update.event 写任意文件 + ROP。。关键路径：Mail server 监听 eventfd 接收 4 类命令: UPD/SET/NEW/ATE/ATF → UPD 触发 system("echo 1 > /t...'
category: pwn
...
contest_type: open
wp_url: https://www.ctfiot.com/90423.html
reasoning_chain:
- Mail server 监听 eventfd + sockfd → 4 类命令 UPD/SET/NEW/ATE/ATF → 触发点：mailclient 读邮件时栈溢出
- 假设：UPD 触发 system('echo 1 > /tmp/need_update') → 攻击者发邮件给 update.event@hackemall.live 触发事件
- 'build_packet: cmd + 3 位 length + data → SET /tmp/evil; NEW 0x10000 空数据; ATE 0x804 A'
- 假设：写 /home/mailserver/data/<user> 主题 'NEW34|...' 注入 payload → 动作：邮件客户端读邮件触发栈溢出
- canary 0x1d5f307b787f8c00 + PIE + libc base 0x7feaecc9a000 已知 → 假设：构造 ROP 链
- 动作：ATF 追加 0x804 填充 + ROP chain → execve('/bin/sh', 0, 0) → flag = TetCTF{2b15f22179fc01196b2e673764e45a7f}
failed_attempts:
- 试图直接发邮件触发栈溢出 → 失败：邮件主题长度受 3 位 length 字段限制
- 试图覆盖返回地址只用 NEW 命令 → 失败：单次 NEW 受 0x10000 长度限制
key_observations:
- 邮件主题字段若允许任意长度写入邮件客户端栈缓冲，是栈溢出入口
- canary 0x1d5f307b787f8c00 是 64 位 Linux 默认 __stack_chk_guard
- update.event 邮箱触发 + SET/NEW/ATE/ATF 协议组合是攻击链核心
prerequisites:
- ELF 栈溢出 + canary 绕过（爆破或已知）
- Linux eventfd + 邮件协议实现
- ROP chain 构造（execve syscall）
---
