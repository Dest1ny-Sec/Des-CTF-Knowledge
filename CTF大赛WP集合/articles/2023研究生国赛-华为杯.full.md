...
- call $+5 混淆
- XOR
- ChaCha20 init/enc 复用
- 汇编动调
- RC4打乱+异或链
- Unity Assembly-CSharp
- dnSpy
- 日文假名密码
...
quality: high
full_path: 2023研究生国赛-华为杯.full.md
meta_path: 2023研究生国赛-华为杯.meta.md
images_removed: true
images_removed_count: 7
schema_version: v3.0.0-P0
summary: 2023 研究生国赛华为杯 4 题 WP。call $+5 混淆去 NOP + ChaCha 复用动调 + RC4+异或链 + Golang memequal + Unity Assembly-CSharp 日文假名密码。。经验：复杂的 init/enc 函数若只与输入无关则直接动调拿中间结果；Unity 题走 dnSpy 看 Assembly-C...
category: reverse
subcategory: reverse
subcategories:
- reverse
...
contest_type: school
wp_url: https://www.ctfiot.com/140776.html
reasoning_chain:
- 触发点：IDA 看不全代码 → call $+5 混淆 → 假设内联无用函数 → 动作：NOP 掉 0x401560~0x4015B8
- 动作：重新 F5 看 init+enc 函数 → 假设：动调拿 xor_array → 观察：xor_array 数组
- actor cipher ^ xor_array = flag → actor 二叉树前序遍历断 val2/val3 反调试
- actor RC4 key=GoodLuck → 假设倒序异或链解 part3 → 动作：还原 part4 索引
- actor Golang base64 密文 → 假设 memequal 断点 → 动作：凯撒还原 flag
- actor dnSpy 打开 Assembly-CSharp.dll → 假设：Win() 解 data1[i] ^ i → 动作：加载成 DLL
- actor Crypt.Decode 日文假名表 → actor RC4 打乱+异或链 → actor 还原 part4 索引
- actor Unity Assembly-CSharp 入口函数 Win() → actor 解 data1[i] ^ i
failed_attempts:
- 试图静态分析 call $+5 混淆函数 → 失败：必须 NOP 后动调拿 xor_array
- 试图直接 dnSpy 反编译 Win() → 失败：要先解 data1[i] ^ i 异或链
key_observations:
- 复杂的 init/enc 函数若只与输入无关则直接动调拿中间结果
- Unity 题走 dnSpy 看 Assembly-CSharp.dll 是基本盘
- call $+5 是经典花指令混淆手法
- ChaCha20 init/enc 函数复用可动调拿密钥流
prerequisites:
- IDA Pro 汇编级调试与 NOP 补丁
- RC4 算法实现
- dnSpy .NET 反编译
- ChaCha20 流密码结构
---
