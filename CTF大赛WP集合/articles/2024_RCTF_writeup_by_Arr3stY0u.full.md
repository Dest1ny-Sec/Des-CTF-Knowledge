...
quality: high
full_path: 2024_RCTF_writeup_by_Arr3stY0u.full.md
meta_path: 2024_RCTF_writeup_by_Arr3stY0u.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: 2024 RCTF writeup by Arr3stY0u。UUID列抽样+键盘流量+pwn 2048+House of Apple 2+Boneh-Durfee+32位VM hook。。经验：RCTF 风格综合：pwn 改分 + webp IO_FILE + crypto XTEA 变种 delta 全方位。
category: misc
subcategory: stego
subcategories:
- stego
...
contest_type: open
wp_url: https://www.ctfiot.com/184051.html
wp_author: Arr3stY0u
reasoning_chain:
- 触发点：flag0-9.png 共 10 张 → 假设：每张抽 1/2 列拼接 → 动作：python PIL 抽样
- 观察：拼成 UUID → 假设：flag{c4baf0eb-e5ca-543a-06d0-39d72325a0} → 动作：提交
- 触发点：USB 键盘流量 → 假设：tshark 解析 HID data → 动作：tshark -T fields -e usb.capdata
- 观察：解出 'niao ybufmefhui kjqillxdjwmi uizebuui...' rot13 → 假设：rot13 → 命令 → 动作：rot13 decode
- 触发点：pwn 2048 模拟 → 假设：BFS+1024 满行合并 → 动作：写 BFS 求最大分
- 观察：改分 payload → 假设：触发 → 动作：pwntools 打远程
- 触发点：webp House of Apple 2 stderr → 假设：_wide_data 劫持 system → 动作：payload flat({0x0:' sh', 0xa0:p64(stdout-0x130+0xd8)...})
- 观察：system('/bin/sh') 触发 → 假设：webp IO_FILE 链 → 动作：pwntools interactive
- 触发点：web 注入 '||love_key regexp binary 'x' → 假设：SQL regex 注入 → 动作：构造 payload
- 触发点：crypto XTEA delta=0x12345678 → 假设：标准 XTEA 解 → 动作：写 Python decrypt
- 观察：解出密文 → 下一步：Boneh-Durfee 大 e RSA
- 触发点：32 位 VM add/sub/xor → 假设：hook 还原 → 动作：unicorn hook 拦截指令
failed_attempts:
- 试图不解 USB 键盘流量 → 失败：必须 tshark 解 HID
- 试图不解 rot13 → 失败：解出是密文
- 试图用 House of Orange 单链 → 失败：必须 Apple 2
- 试图不解 Boneh-Durfee → 失败：大 e RSA 必须 Wiener-like 攻击
key_observations:
- RCTF 风格综合：pwn 改分 + webp IO_FILE + crypto XTEA 变种 delta 全方位
- USB 键盘流量 tshark 解 HID data + rot13 是 misc 常见套路
- House of Apple 2 stderr 路径比 stdout 更隐蔽
- Boneh-Durfee continued_fraction 收敛子是经典大 e RSA 还原法
- 32 位 VM 用 unicorn hook add/sub/xor 是反混淆通用手段
- 列抽样拼 UUID 是图像 stego 常见出题思路
prerequisites:
- tshark USB 键盘流量解析 + rot13
- House of Apple 2 stderr _wide_data 劫持
- Boneh-Durfee continued_fraction 收敛子还原 d
- XTEA delta 变种
- unicorn 32 位 VM hook 指令反混淆
- 列抽样拼图图像 stego
---
