...
tags:
- WoW64
- Heaven's_Gate, retf, CS段寄存器, 32位调用64位, XTEA, 魔改, 反调, AntiDebug, Wow64Cpu, syscall, IDA反编译
...
quality: high
full_path: 天堂之门(WoW64技术)总结及CTF中的分析.full.md
meta_path: 天堂之门(WoW64技术)总结及CTF中的分析.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: 天堂之门 (WoW64) 总结及CTF分析 - ctfshow月饼杯2 EasyTea。看雪Sh4d0w详解WoW64天堂之门(32位程序调64位函数)技术+ctfshow月饼杯2 EasyTea实战魔改XXTEA。。经验：WoW64下32位进程用CS=0x33/0x23切换32/64位模式,retf是关键指令;32位IDA无法反编译64位指...
category: reverse
subcategory: reverse
time_required: long
difficulty_score: 4
...
wp_url: https://www.ctfiot.com/208119.html
reasoning_chain:
- 'ctfshow 月饼杯 2 EasyTea WoW64 天堂之门, 32 位 PE 切天堂之门 CS=0x33 → 触发点: x64 系统上 32 位进程切 64 位模式'
- '假设: 0xEA jmp far ptr 跳到 64 位函数 → 动作: 跟踪 unk_427A50 → 观察: 实际是 64 位 XXTEA 加密 → 假设: 32 位 IDA 无法反编译 64 位指令'
- '动作: IDApython patch 把 64 位 Hello World 二进制 patch 到 IDA → 假设: 手动分析机器码还原函数 → 观察: P 创建函数得魔改 XTEA'
- '假设: XXTEA delta=0x77B7EEBB + 自定义密钥 → 动作: 反编译 XXTEA 加密逻辑 → 观察: delta 不同 + sum 不同'
- '动作: 还原解密 → flag = XXTEA_dec(cipher, key, delta=0x77B7EEBB) → 完成'
- '观察: 天堂之门实战多为反调反沙箱 (沙箱+火绒剑都识别), 但 CTF 主要考反静态分析'
failed_attempts:
- '试图不解天堂之门直接反编译 → 失败: 32 位 IDA 看到 0xEA 就懵'
- '试图用动态调 (双机调试) → 失败: 沙箱识别天堂之门会断下'
key_observations:
- WoW64 下 32 位进程用 CS=0x33/0x23 切换 32/64 位模式, retf 是关键指令
- 32 位 IDA 无法反编译 64 位指令, 需手动分析机器码 + IDApython patch 到 64 位程序还原函数
- 天堂之门实战多为反调反沙箱 (沙箱+火绒剑都识别), 但 CTF 主要考反静态分析
- 动态天堂之门只能动调手撕
prerequisites:
- x86/x64 段寄存器 (CS=0x23/0x33)
- retf far jmp 指令
- XXTEA 算法 (delta + sum)
- IDApython patch (合并 64 位二进制)
---
