...
lesson: '|'
quality: high
full_path: NKCTF_2024_Writeup_–Polaris战队.full.md
meta_path: NKCTF_2024_Writeup_–Polaris战队.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: 'NKCTF 2024 Writeup by Polaris 战队 (ThinkPHP + KodExplorer)。NKCTF 2024 Polaris 战队: ThinkPHP 反序列化 (Windows 类) + KodExplorer Mcrypt 加密 (RC4-like) + 弱密码爆破。。经验：|'
category: web
...
contest_type: open
wp_url: https://www.ctfiot.com/170176.html
reasoning_chain:
- 触发点：ThinkPHP + KodExplorer + Mcrypt 加密 + RC4 密钥流 + Windows 文件包含 → 假设：ThinkPHP 反序列化触发 Windows 类 + KodExplorer 弱密码登录
- ThinkPHP Windows 类反序列化：namespace think\process\pipes; class Windows extends Pipes {filename, files} → 动作：构造 EXP 攻击 ExistingStock
- 假设：transferFrom/approve + setflag → 动作：序列化 Windows 对象 + base64 → 触发反序列化
- KodExplorer 加密类 Mcrypt：encode(string, key='', expiry=0, cKeySet='', encode=true) → 算法 md5(key) + md5(substr(key,0,16)) + md5(substr(key,16,16)) + cKeySet
- 动作：keya = md5(substr(key,0,16)) 数据完整性 + keyb = md5(substr(key,16,16)) 密文生成 + cryptkey = keya + md5(keya + keyc)
- 假设：RC4 KSA 阶段 box = range(0,255) 打乱 + PRGA 阶段生成密文 → 动作：结果 = keyc + base64(密文)，替换 +/-/= 为 -_/.
- Windows 文件包含 + KodExplorer 弱密码：密码 !@!@!@!@NKCTFChu0 → 假设：弱密码爆破
- 动作：用 ThinkPHP EXP + 弱密码登录 KodExplorer → Windows 文件包含读 flag → 完成
failed_attempts:
- 试图直接登录 KodExplorer → 失败：密码不是默认，需要爆破或 ThinkPHP 链先拿入口
- 试图用标准 ThinkPHP 5.x 反序列化链 → 失败：题目用的是 Windows 文件包含而非标准 gadget
- 试图不解 KodExplorer 加密 → 失败：必须先还原 RC4 密钥流 + base64 替换才能解密
key_observations:
- ThinkPHP Windows 类反序列化 = think\process\pipes\Windows extends Pipes {filename, files} 是 ThinkPHP 经典 gadget
- KodExplorer Mcrypt 是 RC4 变种：md5(key) + keya/keyb + cryptkey + 替换 +/-/= 为 -_/. 编码
- RC4 KSA = box = range(0,255) 打乱 + PRGA 异或生成密文是 RC4 标准实现
- Windows 文件包含 + KodExplorer 弱密码 = 密码爆破（!@!@!@!@NKCTFChu0）
- Polaris 战队 NKCTF 2024 走 ThinkPHP+KodExplorer 多层组合攻击链
prerequisites:
- ThinkPHP 反序列化（Windows 类 + Collection 类 gadget）
- PHP Mcrypt/RC4 加密实现（md5(key) + KSA + PRGA）
- KodExplorer 文件管理（弱密码爆破 + Windows 文件包含）
- Base64 字符替换（+/-/= → -_/. 编码）
---
