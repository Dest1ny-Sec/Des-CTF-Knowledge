...
quality: medium
full_path: SHCTF-2024-Week1_官方WP.full.md
meta_path: SHCTF-2024-Week1_官方WP.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: SHCTF-2024-Week1 官方WP。看雪 ctfiot 招新题库，Week1 全是 8 个类别的签到 + 工具题 (RC4/PNG/zip/MD5 碰撞/WiFi/Java/PHP 链)。。关键路径：用 RC4 硬编码 key 解 Windows Defender Quarantine 加密 zip → rockyou 字典爆破解压密码 → 010editor 补全 PNG 头尾 (...
category: misc
...
contest_type: open
wp_url: https://www.ctfiot.com/209352.html
reasoning_chain:
- WEEK1 Quarantine Windows Defender 隔离文件 → 触发：RC4 硬编码 key
- 搜索 cuckoo-modified GitHub commit → key 已知 → 直接 RC4 解密 → base64 → zip
- rockyou 字典爆破解压密码 → 拿到原始文件 → flag
- WEEK1 拜师之旅① PNG 头尾补全 → 010editor 写入 89 50 4E 47 0D 0A 1A 0A
- '尾: AE 42 60 82 → 修改高度 07→08 露出 flag 文本'
- WEEK1 zip 伪加密 → 改 09 00 为 00 00 → 解压成功
- WEEK1 fastcoll MD5 碰撞 → 同前缀 MD5 = master! → flag
- WEEK1 ROP pop rdi + plt puts 泄 got → 一字节覆写 ret_addr 为 backdoor
- WEEK1 chain 5 PHAR 反序列化 → system('cat /f*') → flag
- WEEK1 /api?SSHCTFF=cmd RCE → os.popen 调系统命令 → flag
failed_attempts:
- Quarantine 试图直接解压 → 失败：需要先 RC4 解密
- PNG 试图 cat 看图片内容 → 失败：必须补全头尾
- ROP 试图构造完整 ROP 链 → 失败：一字节覆写更简单
key_observations:
- 真实取证 Quarantine 文件结构 + cuckoo-modified RC4 key 是常见 pattern
- rockyou 字典对短密码永远先试
- PNG 头尾 89 50 4E 47 0D 0A 1A 0A / AE 42 60 82 是固定 magic
- PHAR 反序列化 + pop chain 是 PHP 链式攻击经典模式
prerequisites:
- 010editor / winhex 十六进制编辑
- RC4 / fastcoll MD5 碰撞工具
- Burp Suite + ROP gadget 基础
- PHAR 反序列化链构造
---
