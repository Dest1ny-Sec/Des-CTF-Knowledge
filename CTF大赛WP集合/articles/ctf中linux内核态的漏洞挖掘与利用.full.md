...
quality: high
full_path: ctf中linux内核态的漏洞挖掘与利用.full.md
meta_path: ctf中linux内核态的漏洞挖掘与利用.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: ctf 中 linux 内核态的漏洞挖掘与利用。V-lab 实验室 CTF 内核漏洞挖掘教程，覆盖 core.ko 栈溢出 + zerofs 内核文件系统越界读写两套思路。。关键路径：core.ko ioctl 0x6677889C 设置 off 全局变量 → ioctl 0x6677889B 触发 core_read 泄 canary + ret_addr → 写入 0x100 字节到 n...
category: pwn
...
contest_type: open
wp_url: https://www.ctfiot.com/57334.html
reasoning_chain:
- 触发点：core.ko ioctl 0x6677889A + 0x6677889B + 0x6677889C → 假设：栈溢出 + ret2usr 提权
- 动作：解包 core.cpio → 改 init 删 poweroff → 观察：kallsyms 写到 /tmp/kallsyms → 下一步：kptr_restrict=1 无法直接读
- ioctl 0x6677889C 设置 off 全局变量 → 动作：构造 off 偏移 → 触发点：core_read 泄 canary + ret_addr
- ioctl 0x6677889B 触发 core_read → 假设：泄漏 canary 后可写 0x100 字节到 name → 动作：构造栈溢出 payload
- ioctl 0x6677889A 传 -0xa000000000000008 → 触发点：有符号长度检查绕过 → 假设：负数当大整数溢出
- 动作：core_copy_func 栈溢出 0x58 字节 → 观察：SMEP/SMAP 关闭时直接 ret2usr → 内核 shellcode
- 假设：commit_creds(prepare_kernel_cred(0)) → 提权 → 动作：构造 ROP chain → 观察：swapgs + iretq 返回用户态
- SMEP/SMAP 开启时改 ROP 链 → 假设：用 kpti_trampoline 或 swapgs 旁路 → 动作：写 trampoline + ret2usr
- zerofs 内核文件系统：OOB 读 cred → 触发点：read_iter 没限制 → 假设：跨页读 cred 结构 → flag
failed_attempts:
- 试图走普通用户态 pwn → 失败：题目是内核模块
- 试图不开 SMEP/SMAP 旁路 → 失败：高版本默认开启，必须旁路
- 试图用 ret2usr 不改 ROP → 失败：SMEP 阻断了用户态跳转
key_observations:
- 内核栈溢出要点：先泄 canary/ret_addr，再 ROP 走 prepare_kernel_cred(0) → commit_creds 提权，最后 iretq 返用户态
- SMEP/SMAP 开启必须 ROP，不能 ret2usr
- ioctl 负数传参绕过有符号检查是内核栈溢出经典入口
- zerofs 等内核文件系统的 OOB 读是另一类内核漏洞利用面
prerequisites:
- Linux 内核编译与 qemu 启动脚本
- kernel ROP 链（prepare_kernel_cred / commit_creds / iretq）
- SMEP/SMAP/KASLR 旁路
- ioctl 命令构造与负数溢出
---
