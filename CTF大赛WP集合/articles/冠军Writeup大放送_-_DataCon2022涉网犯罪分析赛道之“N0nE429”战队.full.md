...
quality: high
full_path: 冠军Writeup大放送_-_DataCon2022涉网犯罪分析赛道之“N0nE429”战队.full.md
meta_path: 冠军Writeup大放送_-_DataCon2022涉网犯罪分析赛道之“N0nE429”战队.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: '冠军Writeup大放送 | DataCon2022涉网犯罪分析赛道之"N0nE429"战队。DataCon2022 涉网犯罪分析冠军 N0nE429：WAF 日志+frame-tag 攻击者画像+同源关联 + Mirai IOC 自动化提取。。关键路径：武汉大学 N0nE429 战队 (JackNoire/Jiryu/Scorpion) 师从傅建明 → 攻击者画像 6 类: 安全公司/...'
category: misc
...
contest_type: open
wp_url: https://www.ctfiot.com/91525.html
reasoning_chain:
- 武汉大学 N0nE429 战队 (JackNoire/Jiryu/Scorpion) 师从傅建明 → 触发点：WAF 日志 + 攻击者画像
- '攻击者画像 6 类: 安全公司/非恶意互联网应用/僵尸网络/白帽黑客/雇佣黑客/未知'
- '风险评级 6 档: 基本可信/低风险/中风险/中高风险/高风险/特殊'
- '假设：数据源: attack-log (WAF web 告警) + frame-tag (IP属性/DNS/hack-tool)'
- 观察：MongoDB 存储 (记录长度受限, 略大文件单独处理) → 冬奥 2.4-2.20 期间 602 个 IP 切片
- '指标: is_dangerous/is_proxy/is_gateway/is_tor/is_gov_targeted/has_webshell/has_hacktool'
- '假设：同源: 相同风险+相同身份+IP 主机号接近 → Mirai IOC: idapython 定位 main'
- '动作：signal(SIGTRAP=5, &anti_gdb_entry) → resolve_cnc_addr 三种形式: 直接赋值/字符串传参/table 加密'
- 'table 加密: TABLE_CNC_DOMAIN/TABLE_CNC_PORT → table_retrieve_val → 加密函数 logic 7-9 + airthmetic 2-4 指令'
failed_attempts:
- 试图不解 attack-log → 失败：日志量太大
- 试图不解帧而不解析不解析指标 → 失败：画像需多维
- 试图手工反编译 Mirai → 失败：样本多需自动化
key_observations:
- 攻击者画像核心是建立指标体系 (is_dangerous/is_proxy/is_gov_targeted 等)
- Mirai 反编译定位 main→signal(SIGTRAP)→anti_gdb_entry→resolve_cnc_addr→table 加密是自动化 IOC 提取关键链
- 同源关联靠风险等级+身份分类+IP 主机号接近
- MongoDB 单文档 16MB 限制需切分大文件
prerequisites:
- WAF 日志格式与攻击类型识别
- MongoDB 数据存储
- IDA Python 自动化（idapython）
- 恶意样本 IOC 提取与同源关联
---
