...
quality: medium
full_path: 【三等奖方案】基于TPU平台实现人群密度估计赛题「加速器」团队解题思路.full.md
meta_path: 【三等奖方案】基于TPU平台实现人群密度估计赛题「加速器」团队解题思路.meta.md
images_removed: true
images_removed_count: 8
schema_version: v3.0.0-P0
summary: CCF BDCI 2022 三等奖 - TPU 人群密度估计 加速器团队。高级工程师单挑用VGG+精确尺寸组合在算能TPU上拿下三等奖。。经验：模型加速首先选对基础模型 — 直接转FP32bmodel对比4个模型(CANNet/CSRNet/MCNN/VGG)选V...
category: misc
...
contest_type: open
wp_url: https://www.ctfiot.com/123014.html
reasoning_chain:
- TPU 密度估计 → 触发点：单挑选手 → 动作：直接转 FP32 bmodel 对比 4 个模型（CANNet/CSRNet/MCNN/VGG）
- 观察：VGG 在 FP32 精度最高 → 假设：VGG + 算能 BM1684 部署最优
- 动作：试不同 batch_size（1/4/8） → 观察：batch=8 反而比 batch=1 慢 → 假设：BM1684 batch 上限 4
- 动作：放弃大 batch_size，保持 batch=1 → 观察：推理速度稳定
- 预处理图片最大 780×780 / 最小 144×144 → 触发点：原图大小不一 → 动作：双线性插值归一化
- 模型输入 144×144 → 触发点：小输入快 → 动作：模型接受 144×144 → 观察：精度损失 < 5%，速度 +30%
- 综合 score 公式 → 观察：B 榜三等奖（前 10%）
failed_attempts:
- 试图用 batch_size=16 → 失败：BM1684 推理变慢，单 batch 上限 4
- 试图保持原图尺寸输入 → 失败：不同尺寸输入推理慢
- 试图用 CANNet → 失败：FP32 精度不如 VGG
key_observations:
- 模型加速首先选对基础模型 — 直接转 FP32 bmodel 对比 4 个模型选 VGG
- batch_size 从 1 换 8 反而速度加倍，故放弃大 batch
- 预处理图片最大/最小尺寸 + 模型输入尺寸组合对最终成绩影响最大
- FP32 比 INT8 精度高 2-3%，但速度慢 2-4×，按 score 公式权衡
- 高级工程师单挑用 VGG+精确尺寸组合也能拿三等奖
prerequisites:
- 深度学习模型 FP32/INT8 部署对比
- 算能 BM1684 TPU 工具链
- 图像预处理（双线性插值 + 尺寸归一化）
- batch_size 与推理速度关系（BM1684 上限 4）
---
