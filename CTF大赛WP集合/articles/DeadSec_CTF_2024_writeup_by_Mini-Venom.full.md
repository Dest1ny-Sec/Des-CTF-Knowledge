...
quality: high
full_path: DeadSec_CTF_2024_writeup_by_Mini-Venom.full.md
meta_path: DeadSec_CTF_2024_writeup_by_Mini-Venom.meta.md
images_removed: true
images_removed_count: 4
schema_version: v3.0.0-P0
summary: 'DeadSec CTF 2024 writeup by Mini-Venom。DeadSec CTF 2024 user-management fmt-string三段写GOT表。关键路径：菜单 1=root_login/2=add_user/3=login/4=logout/5=vuln → root_login: manage users\x00\x00\x00\x00 / Mr...'
category: pwn
...
contest_type: open
wp_url: https://www.ctfiot.com/196213.html
wp_author: Mini-Venom
reasoning_chain:
- 菜单 1=root_login/2=add_user/3=login/4=logout/5=vuln → 触发点：user_management 用户态格式化字符串
- root_login 绕过：manage usersaaaax00 + MrAlphaQ + x00 → 假设：栈变量覆盖 username/password
- add_user(name, passwd='0rb1t', desc) → 假设：desc 段可控 → 动作：写 %p.%p.%p.%p.%p
- logout/login 切换 ctx 让 desc 不被释放 → vuln 触发 desc 格式化字符串 → 观察：栈/elf/libc 三段地址
- 假设：%hn 按 2 字节写 → 动作：generate_fmt 把 libc.symbols['gets'] 拆 3 段 2 字节写入
- 观察：printf 返回地址被覆盖为 gets → 下一步：gets 写 ROP 链
- rdi_ret + /bin/sh + system → 动作：sendline(b'a'*0x2200 + ROP) → 触发 system('/bin/sh')
failed_attempts:
- 试图一次写完整 8 字节地址 → 失败：%n 输出字符数爆炸，用 %hn 分 3 次写 2 字节
- 试图直接栈溢出覆盖返回地址 → 失败：栈 canary + 距离过远，靠 fmt-string 链路
key_observations:
- '%hn 可按 2 字节写入，3 次 %hn 覆盖 6 字节地址'
- add_user/login/logout 控制描述生命周期，避免 desc 被释放
- rdi_ret + /bin/sh + system 是经典 libc ROP 三件套
- 格式化字符串能同时泄露 stack/elf/libc 三段地址
prerequisites:
- 格式化字符串漏洞原理（%p/%n/%hn）
- pwntools（p64/ELF/ROP/cyclic）
- libc 偏移查找
- ROP gadget 构造（pop rdi; ret）
---
