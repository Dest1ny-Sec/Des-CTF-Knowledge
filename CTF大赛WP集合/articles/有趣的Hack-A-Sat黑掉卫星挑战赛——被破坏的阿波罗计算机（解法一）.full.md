...
quality: high
full_path: 有趣的Hack-A-Sat黑掉卫星挑战赛——被破坏的阿波罗计算机（解法一）.full.md
meta_path: 有趣的Hack-A-Sat黑掉卫星挑战赛——被破坏的阿波罗计算机（解法一）.meta.md
images_removed: true
images_removed_count: 0
schema_version: v3.0.0-P0
summary: Hack-A-Sat 阿波罗导航计算机被破坏的 PI 值。VirtualAGC 复现 Apollo AGC 的 DSKY 命令，从 ROM rope-memory 中挖出被篡改的 PI/16 DP 值。。关键路径：用 VirtualAGC 模拟器定位 Commanche055 固件中 PI/16 存储的 0o27 Bank 0o3355 address → netcat 连 172.17.0...
category: misc
...
contest_type: open
wp_url: https://www.ctfiot.com/97721.html
wp_author: how
reasoning_chain:
- 题目：阿波罗 AGC 中 PI/16 被改 → 触发点：VirtualAGC 模拟器 + DSKY 协议
- 动作：netcat 172.17.0.1:19008 → 观察：提示 Commanche055 固件 + V/N 命令
- 假设：DSKY V27N02E 是 '读 ROM' 模式 → 动作：发送 V27N02E + 输入 Bank×0o2000+addr-0o2000
- 观察：需要按 ENTR 循环读 → 触发点：0o27 Bank 0o3355 address 是 PI/16 邻位
- 动作：0o57355 → 循环 ENTR → 假设：找 0o37777 标记反向读
- 观察：拿到 0o6413 0o11416 两个 DP 字 → 动作：按 AGC DP 浮点格式解
- 观察：PI = 3.26103293895721435546875 → 完成
failed_attempts:
- 直接搜 PI/16 常量 → 失败：必须用 DSKY V/N 命令查表
- 用现代十进制算 PI → 失败：题目要 AGC DP 浮点格式
key_observations:
- 历史向 CTF 考察领域知识（AGC 八进制 + Bank 寻址 + DP 浮点 + V/N 命令）
- V27N02E 是 DSKY 查 ROM 模式的标准动词
- AGC DP 浮点格式 = 1 位符号 + 14 位指数 + 27 位尾数
- 绳存储器（rope-memory）寻址用 Bank×0o2000+addr-0o2000
prerequisites:
- VirtualAGC 模拟器使用
- DSKY 接口协议（V/N 动词名词命令）
- AGC DP 浮点格式
- 八进制 Bank 寻址
---
