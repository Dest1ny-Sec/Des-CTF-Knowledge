...
- APlayer
attack_chain:
- '15-puzzle: 数字华容道 + A* 求解 80 步内还原'
- '量子计算 1+1=?: 提示板扰动 → 反推'
- '旅行商 NP-hard: 小规模穷举 / 启发式'
- 'RSA 教学: 课本 p,q,e → d = inverse(e, (p-1)*(q-1))'
- 'IDA 静态分析: 找 main 函数 main_loop 状态机还原'
- 'angr 符号执行: 找路径条件解迷宫 / 表达式'
- 'Z3 SMT 求解: 线性方程组 / 不等式组'
- 'LaTeX 渲染: 注入 RCE / 读取文件'
...
quality: medium
full_path: 2022_USTC_Hackergame_WriteUp_0x02.full.md
meta_path: 2022_USTC_Hackergame_WriteUp_0x02.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: '2022 USTC Hackergame WriteUp 0x02。USTC Hackergame 0x02 量子计算 + 拼图 + RSA + 旅行商 + LaTeX 渲染。关键路径：15-puzzle: 数字华容道 + A* 求解 80 步内还原 → 量子计算 1+1=?: 提示板扰动 → 反推 → 旅行商 NP-hard: 小规模穷举 / 启发式。经验：USTC 信安赛题目趣味性强 +...'
category: crypto
subcategory: math
subcategories:
- math
...
contest_type: open
wp_url: https://www.ctfiot.com/71319.html
reasoning_chain:
- '[触发点] 15-puzzle + 80 步内还原 → 假设：A* / [动作] 写 A* + 曼哈顿距离 / [观察] 拿到 flag / [下一步] 量子计算'
- '[触发点] 1+1=? 提示板扰动 → 假设：量子比特/反推 / [动作] 反推测量结果 / [观察] flag / [下一步] 旅行商'
- '[触发点] 旅行商 NP-hard 小规模 → 假设：穷举 / [动作] 写回溯 / [观察] flag / [下一步] RSA 教学'
- '[触发点] 课本 p,q,e → 假设：直接 modinv / [动作] d = inverse(e, (p-1)*(q-1)) / [观察] flag / [下一步] IDA 静态'
- '[触发点] IDA 找 main main_loop state machine → 假设：状态机还原 / [动作] F5 反编译 / [观察] flag / [下一步] angr + Z3'
- '[触发点] angr 符号执行 + Z3 SMT 求解 + LaTeX 渲染 RCE → 假设：多工具 / [动作] 跑 angr + Z3 + LaTeX 注入 / [观察] flag'
failed_attempts:
- 试图 BFS 15-puzzle → 失败：状态空间太大
key_observations:
- USTC 信安赛题目趣味性强 + 知识面广
- 量子计算 / 旅行商 / LaTeX 都是冷门考点
- A* + 曼哈顿距离是 16-puzzle 标准解
prerequisites:
- A* 搜索 + 启发式
- 量子计算入门
- 旅行商 NP-hard 概念
