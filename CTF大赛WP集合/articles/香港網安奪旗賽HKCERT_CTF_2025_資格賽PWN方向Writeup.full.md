...
quality: high
full_path: 香港網安奪旗賽HKCERT_CTF_2025_資格賽PWN方向Writeup.full.md
meta_path: 香港網安奪旗賽HKCERT_CTF_2025_資格賽PWN方向Writeup.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: 香港網安奪旗賽HKCERT CTF 2025 資格賽PWN方向Writeup。HKCERT 2025资格赛PWN方向5题集合,涵盖fmtstr基础题+PIE leak+GOT覆盖+ret2libc+ORW+musl FSOP fake IO_FILE。。关键路径：第一题:Question Number 负数+Result数组下标system/bin_sh/prdi直接拼地址进数组 → 第二题...
category: pwn
...
contest_type: open
wp_url: https://www.ctfiot.com/288942.html
wp_author: order
reasoning_chain:
- Q1 Question Number 接受负数 → 触发点：Result 数组下标可控
- 假设：直接 system/bin_sh/prdi 拼地址进数组 → 动作：-1→system, -2→bin_sh, -3→prdi
- 观察：执行 → flag
- Q2 printf 触发 → 假设：fmtstr 漏洞 → 动作：%41$p 泄 PIE base
- '动作：fmtstr_payload(6, {printf_got: backdoor}) 覆盖 GOT'
- Q3 栈溢出 0x70+0x10 字节覆盖返回地址 → 假设：ret2libc ORW
- 动作：open/read/write 三件套 ROP → 观察：读 flag
- Q4 login 删除后 add 复用 chunk → 假设：tcache 泄露 + edit 越界
- 动作：add+delete+show 泄 libc → edit 越界写 → __free_hook
- Q5 musl libc → 假设：FSOP fake IO_FILE 构造 → 动作：open/read/write 三件套
- 观察：触发 exit 流程时执行 _IO 链 → flag
failed_attempts:
- 试图用 glibc FSOP 套路打 musl → 失败：musl __stdout_FILE 结构差异大
- 试图不解 PIE base 直接打 → 失败：fmtstr 偏移为 G
key_observations:
- fmtstr_payload 偏移要从栈上找 %41$p 等 leak PIE
- musl 的 IO 攻击面比 glibc 小，必须自己分析 __stdout_FILE 构造 fake io file
- ORW (open/read/write) 三件套是 syscall 拦截下唯一出口
- Question Number 负数 + 数组下标是 fmtstr 前置入门套路
prerequisites:
- pwntools fmtstr_payload + GOT 覆盖
- ret2libc + ORW (open/read/write)
- musl libc IO_FILE 结构
- tcache 泄露 + edit 越界
---
