...
quality: high
full_path: 官方WP_-_关于“可信度量”赛题的设计与思考.full.md
meta_path: 官方WP_-_关于“可信度量”赛题的设计与思考.meta.md
images_removed: true
images_removed_count: 6
schema_version: v3.0.0-P0
summary: 可信度量赛题官方WP - TCM PCR寄存器+SM3摘要+可信报告防重放。北工大胡俊老师设计信安国赛可信度量题,3队解出:东北大学NEX一血/F1sh二血/北工大三血。。经验：可信计算TCM PCR寄存器写入原理:newPcrValue = SM3(oldPcrValue||digestValu...
category: crypto
subcategory: oracle
time_required: long
difficulty_score: 4
...
wp_url: https://www.ctfiot.com/118196.html
reasoning_chain:
- 信安国赛 TCM PCR 可信度量 + SM3 摘要 + 可信报告防重放 → 触发点：quote_report 内含 nonce + pcrValue + 多 digestValue
- 检查 nonce == external_data_store 中预期值 → 假设：必须消费 nonce 防重放 → 动作：遍历 external_data_store 比对
- 观察：digestValue 缺失 = 该策略无度量内容 → pcrValue 应为 SM3(oldPcrValue || 空) 全部 0 → 4 种策略（科研/生产/测试/公开）任一缺席
- 16 种组合：2^4 位对应 4 种策略是否启用 → 假设：遍历 0-15 试 → 动作：memdb.get(hash) 拿每策略 digestValue
- action：newPcrValue = SM3(oldPcrValue || digestValue) 逐策略扩展 → 比对 quote_report->pcrValue
- 观察：匹配即得策略组合如 '科研|生产'
- NEX 一血用 VSCode Remote-SSH 直接连环境写入 memdb → 假设：环境支持交互写入 → 动作：VSCode 远程浏览器调试
- NEX 拼接多个策略名输出答案
failed_attempts:
- 试图只检查 nonce 不验 PCR → 失败：题面要求验 PCR 防重放+完整性
- 试图固定 4 策略全启用 → 失败：必须从题面文档找到正确组合
- memdb 哈希表忽略插入顺序 → 失败：必须按策略名排序再拼接
key_observations:
- 可信计算 TCM PCR 寄存器扩展规则：newPcrValue = SM3(oldPcrValue || digestValue)
- 4 种策略 2^4=16 种组合的枚举爆破适用于这种小状态空间认证
- 防重放：比对 external_data_store 预期 nonce + quote_report 返回 nonce
- memdb 哈希表存储与查询次序无关，构造答案时必须显式排序
prerequisites:
- 可信计算基础（TCM 概念、PCR 寄存器、quote 报告）
- SM3 国密哈希算法
- 2^N 枚举策略（位运算枚举）
- VSCode Remote-SSH / 远程调试环境
---
