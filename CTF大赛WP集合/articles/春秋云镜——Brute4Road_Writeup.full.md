...
quality: high
full_path: 春秋云镜——Brute4Road_Writeup.full.md
meta_path: 春秋云镜——Brute4Road_Writeup.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: 春秋云镜——Brute4Road Writeup。春秋云镜Brute4Road：Redis主从+SUID+WPCargo RCE+约束性委派Rubeus S4U2Self+DCSync。经验：域内机器账户+约束性委派可被Rubeus S4U2Self扩展代表域管访问LDAP服务导出域内凭据
category: web
subcategory: rce
...
contest_type: open
wp_url: https://www.ctfiot.com/65358.html
reasoning_chain:
- fscan 入口开放 21/22/80/6379 → 触发点：ftp 匿名 + redis 未授权
- redis 写 shell 失败 → 假设：主从复制可打 → 动作：use exploit/linux/redis/redis_replication_cmd_exec
- 观察：msf 通，GitHub 脚本没通 → 下一步：SUID 提权
- 动作：find / -user root -perm -4000 → 观察：base64 有 suid
- 动作：base64 "/home/redis/flag/flag01" | base64 --decode → 拿到 flag1
- frp 代理进内网 → proxychains wpscan --url http://172.22.2.18 → 观察：WPCargo 6.x 插件
- 假设：CVE-2021-25003 未授权 RCE → 动作：用 magic PNG payload 写 webshell
- 假设：内网有 MSSQL → 动作：爆破 ElGNkOiC 弱口令 → 观察：成功
- MSSQL xp_cmdshell → 假设：需要 system 权限 → 动作：SweetPotato.exe 提权
- 观察：拿到 system → mimikatz 导出 MSSQLSERVER$ NT hash:cea3e66a2715c71423e7d3f0ff6cd352
- 动作：Rubeus asktgt /user:MSSQLSERVER$ /rc4:<hash> → 拿到 TGT
- S4U2Self 申请 LDAP 服务票据 → DCSync 导出域管 → wmiexec 登录 DC
failed_attempts:
- redis 写 webshell 直接读 flag1 → 失败：必须 SUID base64 间接读
- GitHub 的 redis_replication 脚本 → 失败：msf 内置模块更稳定
key_observations:
- Redis 主从复制 + SUID 提权是 redis 入门组合拳
- WPCargo 6.x + magic PNG payload 是未授权 RCE 经典利用
- Rubeus S4U2Self 扩展代表域管访问 LDAP 服务是约束性委派核心
- MSSQL xp_cmdshell + SweetPotato = system 权限
- 机器账户 + S4U2 = 间接获取域管凭据
prerequisites:
- Redis 主从复制（msf exploit/linux/redis/redis_replication_cmd_exec）
- SUID 提权（GTFOBins base64）
- MSF / msfvenom payload 生成
- MSSQL 弱口令 + xp_cmdshell
- Rubeus S4U2Self 约束性委派
---
