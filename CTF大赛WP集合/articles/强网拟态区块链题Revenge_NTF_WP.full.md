...
quality: high
full_path: 强网拟态区块链题Revenge_NTF_WP.full.md
meta_path: 强网拟态区块链题Revenge_NTF_WP.meta.md
images_removed: true
images_removed_count: 0
schema_version: v3.0.0-P0
summary: 强网拟态区块链 Revenge NTF - NFT Market CouponVerifierBeta ECDSA签名。强网拟态NFT Market Revenge:伪造NFT合约ownerOf返回market+purchaseTest绕过+CouponVerifierBeta ECDSA签名。。经验：NFT Market常见漏洞:FakeNFT合约实现ownerOf返回指定地址绕过isNF...
category: web
...
contest_type: open
wp_url: https://www.ctfiot.com/81173.html
reasoning_chain:
- 强网拟态 NFT Market Revenge：Solidity + ERC721 + ERC20 + CouponVerifierBeta ECDSA 签名 → 触发点：NFT + Coupon 链
- 部署 CtfMarket + 3 NFT (1/2/3) + CtfToken + CouponVerifierBeta → 假设：要先 airdrop 领 5 token
- 部署 FakeNFT 合约实现 ownerOf 返回 market 地址（绕过）→ 假设：isNFTApprovedOrOwner 可以自定义
- 部署 Attacker 合约实现 onERC721Received 返回正确 selector → purchaseTest 绕过 tested=true + createOrder(nftAddress, tokenId, price) + approve + purchaseOrder → 假设：tested 标志一写入即通过
- purchaseWithCoupon：伪造 SignedCoupon(orderId=0, newprice=1, issuer=market, user=attacker, reason) + ECDSA 签名 'because' + abi.encode('I, the issuer' + issuer + offer + user + order + at + newprice + because + reason)
- 假设：CouponVerifierBeta 用 abi.encode 内容 ECDSA 验签
- win：要求 ctfNFT.ownerOf(1/2/3) == msg.sender → 用 FakeNFT 给的 token 触发
failed_attempts:
- 直接调用 purchaseOrder → 失败：必须 purchaseTest 过 tested=true
- 用真 NFT → 失败：题目 NFT 已不流通，必须 fakeNFT
- ECDSA 签整 message 串 → 失败：必须 abi.encode 顺序正确
- onERC721Received 不返回 selector → 失败：safeTransferFrom 必校验
key_observations:
- NFT Market 常见漏洞：FakeNFT 合约实现 ownerOf 返回指定地址绕过 isNFTApprovedOrOwner
- tested 标志一次写入后可绕过 purchaseTest 复用 = 检查不严
- CtfMarket.deleted = orders[orderId] = orders[last] + pop 但 index 0 被删后 newOrderId=0
- CouponVerifierBeta 的 abi.encode 含 'because' + reason 字段可 ECDSA 签名伪造
prerequisites:
- Solidity 0.8.16 + OpenZeppelin ERC721/ERC20
- ECDSA 签名与 abi.encode 顺序敏感性
- NFT safeTransferFrom 流程与 onERC721Received 回调
- Foundry 部署/调用 NFT Market
---
