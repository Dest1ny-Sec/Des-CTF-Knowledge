...
quality: high
full_path: WMCTF-WriteUp.full.md
meta_path: WMCTF-WriteUp.meta.md
images_removed: true
images_removed_count: 8
schema_version: v3.0.0-P0
summary: WMCTF WriteUp (ChaMd5 Venom)。WMCTF 早期 ChaMd5 Venom 战队杂烩 WP，AES-CBC 密码爆破 + OCB 模式 oracle 标签伪造 + 32 元子集和 + Rabin 链解 e=4096 + LWE 格密码，多道密码学题经典攻击模板。。经验：OCB 模式 arbitrary_encrypt oracle 可直接伪造 PMAC 标签；子集和...
category: crypto
subcategory: oracle
time_required: long
difficulty_score: 4
...
contest_type: open
wp_url: https://www.ctfiot.com/1224.html
reasoning_chain:
- v2x_misc.conf 触发点：AES-CBC 16 字节密码 ljust\x00 → 假设：可爆破短密码
- OCB 模式触发点：Arbitrary_encrypt oracle 重放 + my_pmac 标签伪造 → 假设：xor_block(tag, pmac_admin) 可任意伪造 PMAC
- 动作：构造 (nonce, ad, plaintext) 让 OCB 输出 pmac_admin = my_pmac(bytearray(b'from admin')) → 假设：admin 消息可任意认证
- 32 元子集和问题：BKZ-22 求最短向量 → 触发点：super-increasing + 前几位已知可用 BKZ-22 而非 LLL
- 动作：M = matrix(ZZ, m); v = M.BKZ(blocksize=22) → 拿到最短向量 → 解子集和
- RSA p^5：Coppersmith small_roots X=2^369 → 假设：p^5 = N + lower bits → Sage small_roots 解 p
- Rabin 链：c^(1/2^i) mod p 穷举 2^13 候选 → sympy.root(x, 2) 取整数 → 假设：e=2^i 是 4096 = 2^12
- LWE q=2^24 格密码：to_mat 16 维 + Matrix(F).solve_left 求解向量 → 假设：q-ary LWE 是格密码基础
- 观察：WMCTF{x1aoma0_wants_a_girlfriend} / WMCTF{L1near_has_double_girlfriends}
failed_attempts:
- OCB 试图直接 decrypt oracle → 失败：OCB 是认证加密，必须先伪造 PMAC 标签
- 子集和问题 试图 LLL → 失败：必须用 BKZ-22 高 blocksize 才能解
- RSA p^5 试图直接开五次方 → 失败：必须用 Coppersmith small_roots
key_observations:
- OCB 模式 arbitrary_encrypt oracle 可直接伪造 PMAC 标签
- 子集和 + super-increasing + 前几位已知可用 BKZ-22 而非 LLL
- RSA p^5 + Coppersmith small_roots 是大幂次 RSA 标配攻击
- Rabin 链 c^(1/2^i) mod p 穷举候选是 e=2^k 攻击标准流程
- LWE q-ary 格密码 + solve_left 求解向量是 LWE 入门
prerequisites:
- AES-CBC 密码学基础
- OCB 模式 + PMAC 认证标签
- 格密码 LLL + BKZ 算法
- Coppersmith small_roots（Sage 实现）
- Rabin 密码 E=2 解法 + CRT 拼接
---
# WMCTF-WriteUp
