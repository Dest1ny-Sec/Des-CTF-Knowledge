...
quality: high
full_path: D3CTF_2023_Writeup-_Polaris战队.full.md
meta_path: D3CTF_2023_Writeup-_Polaris战队.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: 'D3CTF 2023 Writeup - Polaris 战队。D3CTF 2023 Polaris 战队 WP，Web/Misc/RE 三大块，含 Casino/Base64 绕过/gif 像素解。。关键路径：Escape Plan: unicode 数字替代 + base64 → d3casino: vanity 生成 0x00 地址 → 部署 91 字节合约 fallback。经验：C...'
category: web
...
contest_type: open
wp_url: https://www.ctfiot.com/114689.html
reasoning_chain:
- Escape Plan Web：目标给 POST 端点 + 数字 unicode 替代 + base64 → 触发点：unicode 旁路关键字
- 动作：构造 ?val(vars(?val(list(dict(...))...))) 等 unicode 替代 payload → CMD.translate({ord(i):u[i]}) → 观察：base64 编码 POST → 触发 eval
- d3casino：Solidity 合约 D3Casino.bet 要求 sender code ≤ 0x64 (100) 字节 + sender/origin 特定偏移为 0x00 + scores >= 10
- 假设：必须部署多个合约让 EOA 通过多个合约代理调用 → 动作：vanityeth 生成 0x00 前缀地址 + 缩短字节码到 91 字节
- d3readfile：locatedb 路径 /var/cache/locate/locatedb → 搜 'flag_in_here' → 直接 cat 拿 flag
- d3gif：题目名 (x,y,bin) → 像素坐标 + 二进制位 → PIL Image 读每帧像素 → 二进制写到对应 (x,y) 黑白图 → save 成 png
- d3rc4：RC4 key 被异或 43 加密 + is_prime 拓展 key 长度 → 假设：先 key[i]^=43 还原 + is_prime 后追加字节 → 模拟 RC4 解密
- d3sky：NAND VM 逆向 → 条件断点取 opcode → 模拟 ~（a&b) → 求解 flag
failed_attempts:
- 试图用普通 eval() 直接执行 payload → 失败：被关键字黑名单拦截
- 试图直接调 D3Casino.bet 10 次累加 scores → 失败：每个 sender 只能成功一次且 code 长度 ≤ 100
- 试图不看密钥文件直接手动算 RC4 → 失败：key 在内存里，必须动态取
key_observations:
- unicode 数字替代 + base64 是 Python sandbox 绕过经典手法（CTF 频繁出）
- Solidity 合约 fallback + assembly.call 能缩短到 91 字节（满足 ≤ 100）
- vanityeth 生成 0x00 前缀 EOA + 合约地址可绕过 sender/origin 偏移检查
- PIL Image.seek + getpixel 读 GIF 帧序列是 GIF pixel stego 标准思路
- is_prime() 在 RC4 key 派生时改变 keylen，必须先模拟函数再 RC4
- 全栈 CTF 越来越需要 Solidity + Python 图像 + RC4 数学 全栈能力
prerequisites:
- Python unicode 转义（\u005f 等）
- Solidity 汇编（assembly call/staticcall/mstore）
- vanityeth / ethereum cryptography 库
- PIL Image 帧遍历（seek/tell/getpixel）
- RC4 算法 + is_prime 数论基础
- NAND VM 模拟（Python 数组实现 ~ &）
---
