...
quality: high
full_path: 伽玛实验场_-_PDC面壁计划管理系统-出题人视角.full.md
meta_path: 伽玛实验场_-_PDC面壁计划管理系统-出题人视角.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: '伽玛实验场｜PDC面壁计划管理系统-出题人视角。aiortc WebRTC 证书从 RSA 改 EC + token 验证 substring PK 漏洞，借伟思身份 WebRTC 拿 flag。。关键路径：附件包: aiortc 库 + app.py + cmdHostory + important.pcapng → diff aiortc 主分支与附件: RSA 改为 EC (SECP2...'
category: web
subcategory: logic
tools_used:
- Python
...
contest_type: open
wp_url: https://www.ctfiot.com/100552.html
reasoning_chain:
- '附件包: aiortc 库 + app.py + cmdHostory + important.pcapng → 触发点：WebRTC + pcap'
- 动作：diff aiortc 主分支与附件 → RSA 改为 EC (SECP256R1) 证书体系变更
- 假设：Wireshark SSLKEYLOGFILE 解密 DTLS 信道 → 动作：pcap 发现 tell2me?token=... + WebRTC SDP
- 观察：DataChannel 'xiaolanlan:cat flag' (Permission denied) → 假设：身份校验 token 缺陷
- 动作：token=md5(timestamp-sk)[:32]+timestamp+pk + 漏洞 substring PK 匹配
- 假设：服务端遍历 pk2sk 如果 pkKey in pk (substring) → 拿到 sk
- 动作：伪造 weisi 的 token, timeStamp 1674991453000 距今 < 600000ms
- weisi=8d509c28896865f8640f328f30f15721 → 用伟思身份建立 WebRTC Client
- 重连重发 'xiaolanlan:cat flag' → 拿 flag
failed_attempts:
- 试图不解 DTLS 直接看 pcap → 失败：DTLS 加密
- 试图用 SSLKEYLOGFILE 之外的方式 → 失败：EC 证书没法用 RSA 私钥解
- 试图用普通 token 登录 → 失败：需要 ≤ 600000ms 时戳
key_observations:
- 字典 key in user_substring 漏洞 (应该用 == 严格匹配)
- WebRTC 内网穿透靠 TUN/STUN 公司/学校可能判定违规
- SSLKEYLOGFILE 通用 DTLS 解密法
- aiortc 自定义证书 (RSA→EC) 是出题常见魔改
prerequisites:
- WebRTC DTLS-SRTP 协议
- aiortc Python WebRTC 库
- Wireshark SSLKEYLOGFILE 配置
- PK-SK 映射与 substring 漏洞利用
---
