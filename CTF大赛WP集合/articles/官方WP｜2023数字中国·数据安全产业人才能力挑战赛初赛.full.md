...
quality: high
full_path: 官方WP｜2023数字中国·数据安全产业人才能力挑战赛初赛.full.md
meta_path: 官方WP｜2023数字中国·数据安全产业人才能力挑战赛初赛.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: 2023数字中国·数据安全产业人才能力挑战赛初赛官方WP。2023数字中国数据安全赛3题官方WP:SQL盲注+AES/XXE jar+SCXML RCE/Go TCP SYN包构造。。经验：盲注用substr+hex提取完整密码;XXE+jar协议上传临时文件+object SCXML执行;XXE p1.xm...
category: misc
...
contest_type: open
wp_url: https://www.ctfiot.com/108889.html
reasoning_chain:
- 数字中国数据安全赛 3 题：端口管理 SQL 盲注 + AES 解密 + key 弱口令 + XxMe XXE jar 协议 + LetsGO Go TCP SYN 包 → 触发点：含多题型混合
- 端口管理：源码 SQL 盲注 admin password（AES 加密存储） → 假设：substr(hex(password)) 逐步注 → 动作：admin' and substr((select hex(password) from users where Username='admin'),1,1)>'0'/*
- 观察：拿到密码 hex，AES 解密得明文 → key：帮助文档泄露用户名 dcic + 弱口令 000000 登录 + id 遍历 → XxMe XXE + jar 协议上传临时文件 → 假设：临时文件中转 RCE
- 动作：构造 jar://1.zip!/a.php 上传 → xxe 列 /tmp 拿文件名 → object 路由加载 poc.xml SCXML 状态机 RCE（Runtime.exec）
- LetsGO Go 逆向：找 main_handle_input → 假设：验证客户端 TCP SYN 包 → 动作：构造 TCP SYN 头 lport=99/rport=233/seqnum=0xdeadbeef/flag=2
- 计算 pseudo_header checksum → 与 server 比对一致 → 服务端返回 'win'
- 观察：SYN 包构造完整得 flag
failed_attempts:
- 端口管理 SQL 用 union select → 失败：盲注场景
- XxMe 单 xxE file:// 协议 → 失败：必须 jar 协议 + SCXML
- LetsGO 直接 connect 端口发包 → 失败：必须算 pseudo_header checksum
- 发包不指定 ip2int → 失败：必须 ip2int + 校验和逐位匹配
key_observations:
- 盲注用 substr+hex 提取完整十六进制密码比位提取快
- XXE jar 协议上传临时文件 + SCXML 状态机执行 Runtime.exec 是 Java 反序列化替代
- TCP SYN 包构造必须算 pseudo_header checksum 才能通过 server 校验
- Go 逆向找 main_handle_input 看协议格式最直接
prerequisites:
- SQL 盲注技术（substr/hex/ascii）
- AES 解密模式（ECB/CBC、padding 处理）
- XXE + jar:// 协议 + SCXML 状态机 RCE
- Go 逆向 + TCP/IP 校验和计算（pseudo_header）
---
