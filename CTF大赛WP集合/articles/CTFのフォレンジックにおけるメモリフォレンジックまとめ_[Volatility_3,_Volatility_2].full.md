...
quality: high
full_path: CTFのフォレンジックにおけるメモリフォレンジックまとめ_[Volatility_3,_Volatility_2].full.md
meta_path: CTFのフォレンジックにおけるメモリフォレンジックまとめ_[Volatility_3,_Volatility_2].meta.md
images_removed: true
images_removed_count: 0
schema_version: v3.0.0-P0
summary: CTF のフォレンジックにおけるメモリフォレンジックまとめ [Volatility 3, Volatility 2]。Volatility 3 vs 2 内存取证总结 (日语) — Windows 关键命令 pstree+netscan+cmdline+dumpfiles+hashdump+mutantscan + Linux bash+psaux + 自制 profile (module....
category: forensic
...
contest_type: open
wp_url: https://www.ctfiot.com/165189.html
reasoning_chain:
- 触发点：内存取证 .raw/.dmp/.lime dump 文件 → 假设：Volatility 3 是当前推荐工具（V2 已停止更新）
- 动作：strings -n 10 mem.bin | grep -E 'ubuntu|windows' 识别 OS → 观察：Linux kernel string 'Ubuntu' 或 Windows 版本字符串
- 下一步：Volatility 3 windows.info.Info → 观察：NtMajorVersion/MinorVersion/ProductName 列出
- 动作：pstree 看进程树 → 观察：cmd.exe/powershell.exe 是可疑进程入口；netscan 看网络连接
- 下一步：cmdline 拉可疑进程的完整命令行 → 观察：看到 webshell/cmd 注入痕迹；dumpfiles --pid=1676 dump 完整文件
- 动作：hashdump 取所有 SAM/SYSTEM 凭据；mutantscan 看互斥量作为恶意软件标志
- Linux 路径：linux.bash 看 bash_history；linux.psaux 复现 ps -aux → 观察：拿到 root/sudo 痕迹
- 假设：profile 不识别 → 动作：自制 profile 用 module.dwarf + System.map 打包 zip → 观察：python setup.py build/ install 装载
- 下一步：VirusTotal 验证可疑 IP/dll，File Carving + strings 即使不用 Volatility 也能拿部分信息
failed_attempts:
- 试图直接 Volatility 2 windows.info 自动识别 → 失败：V2 在新版 Win10/Win11 上 profile 不全
- 试图 strings 全文搜 flag → 失败：flag 通常在网络流/进程上下文，需要解 netscan 还原
- 试图用 Volatility 1 完成题 → 失败：V1 在 V2/V3 时代已废弃
key_observations:
- 内存取证 = Volatility 3 推荐（V2 V3 是完全不同的工具，命令不通用）
- OS 识别先 strings 验字符串再使用 windows.info.Info 确认
- 关键命令：pstree / netscan / cmdline / dumpfiles / hashdump / mutantscan
- Linux 镜像关键命令：linux.bash（history）+ linux.psaux（ps -aux）
- 自制 profile 用 module.dwarf + System.map 打包 zip 放到 plugins/overlays/linux/
prerequisites:
- Volatility 3 / 2 命令基础（windows.info/pstree/netscan/cmdline）
- Linux 内核 + System.map + 符号表制作 profile
- strings / grep / binwalk 取证基础
- VirusTotal 威胁情报查询
- JPCERTCC 公开符号表
---
