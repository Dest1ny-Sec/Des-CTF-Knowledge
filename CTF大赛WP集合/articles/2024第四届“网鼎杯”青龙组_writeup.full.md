...
- fenjing
attack_chain:
- 'WEB02: /content/hash 路由 + dirsearch 扫 /flag 路由，3 按钮（提交/刷新/更新）'
- 存储型 XSS：<script>fetch('/flag').then(r=>r.text()).then(d=>fetch('/content/...', {body:'content=123'+d}))</script>
- submit 提交触发 boss 审核 → XSS 弹到攻击者平台
- 'PWN02: login 绕过 admin\x00 + admin123\x00 → leak 栈地址'
- vuln 栈溢出 → 栈迁移到栈地址 - 4 → ret_address → system('/bin/sh')
- 'REVERSE01: Android MainActivity → check 走 native 加密 → 解 apk 看 jni'
- 加密逻辑是 SM4 国密 → 找 key 解密 flag
...
quality: high
full_path: 2024第四届“网鼎杯”青龙组_writeup.full.md
meta_path: 2024第四届“网鼎杯”青龙组_writeup.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: '2024 第四届"网鼎杯"青龙组 writeup。网鼎杯 2024 青龙组：XSS admin-bot exfil + 栈迁移 pwn + SM4 加密。关键路径：WEB02: /content/hash 路由 + dirsearch 扫 /flag 路由，3 按钮（提交/刷新/更新） → 存储型 XSS：<script>fetch(''/flag'').then(r=>r.text())....'
category: web
subcategory: xss
subcategories:
- xss
...
contest_type: open
wp_url: https://www.ctfiot.com/212654.html
reasoning_chain:
- '[触发点] XSS 题目有提交按钮 + boss 审核机制 → 假设：必须用 admin bot 触发存储型 XSS → [动作] submit 提交页面 + script → [观察] 提交后 boss 触发 fetch(''/flag'') 并回显到 content 字段 → [下一步] exfil 到 /content/<hash>'
- '[触发点] content/2f9f1f36782a270b689d8c0f3e9e08df 路由 POST 表单 → 假设：admin bot 加载 XSS 后会用 fetch 把 /flag 内容 POST 回去 → [动作] XSS payload = fetch(''/flag'').then(r=>r.text()).then(d=>fetch(''/content/<hash>'',{method:''POST'',body:''content=''+d})) → [观察] 拿到 flag'
- '[触发点] PWN02 login 用户密码 admin/admin123 → 假设：栈溢出 + 栈迁移 → [动作] payload = p32(0x080485E6)+p32(0)+p32(sh_address) 填满 80 字节 + p32(stac-4) 覆盖返回地址 → [观察] leave_ret 触发栈迁移 → [下一步] system(''/bin/sh'')'
- '[触发点] REVERSE01 Android apk + check 方法 native 层 → 假设：SM4 国密 → [动作] jadx 看 MainActivity + 找 key Z0099864 → [观察] Z0099864 反转得 4689900Z → [下一步] 解 SM4'
- '[触发点] REVERSE02 四段加密：乘 2 → 异或 → 自定义 base64 → AES-ECB → 假设：分段解 → [动作] part1/2/3/4 拼接 → [观察] 拿到 wdflag{81fe0766197e1bc6809832f4d346fe66}'
- '[触发点] CRYPTO01 网上找原题脚本（cnblogs mumuhhh） → 假设：lattice 还原 d → [动作] 套脚本 → [观察] 拿到 flag'
failed_attempts:
- 试图用 hackbar xss payload 一把梭到 ceye.io → 失败：payload 不合适，需走 /content 回显
- 试图用 img 标签触发 XSS → 失败：响应不匹配
- PWN 试图直接栈溢出写 shellcode → 失败：buffer 太小，需栈迁移
key_observations:
- 存储型 XSS 必须靠 admin-bot 触发，submit 提交机制是触发入口
- 栈迁移到泄露地址可绕小 buffer，system 函数地址 + leave_ret 经典组合
- SM4 是国产商用密码（分组 128-bit 密钥 128-bit），apk native 层加密常见
- 分段加密题先分段解再拼接，注意端序（如 Z0099864 → 4689900Z）
- WP 找不到原题直接搜代码 hash 经常能找到原作者
prerequisites:
- XSS admin bot 触发 + exfil 思路
- 栈迁移 + leave_ret gadget 利用
- SM4 国密对称加密原理
- AES-ECB / 自定义 base64 解码
---
