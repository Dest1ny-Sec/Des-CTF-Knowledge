...
quality: high
full_path: 强网杯行业领域专项车联网赛道(Writeup).full.md
meta_path: 强网杯行业领域专项车联网赛道(Writeup).meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: 强网杯行业领域专项车联网赛道 WriteUp - AES-CPA侧信道+VIN_RC4+CAN数据。强网杯车联网赛道:AES-CPA侧信道攻击+变种RC4 VIN加密+CAN总线数据最频值提取。。经验：AES-128 CPA侧信道:TRS文件ntraces/nsamples/dlen=32(16B PT+16B CT) ...
category: crypto
subcategory: rsa
time_required: long
difficulty_score: 4
...
wp_url: https://www.ctfiot.com/276876.html
reasoning_chain:
- 题目是车联网 AES 侧信道 → 触发点：拿到 AES128.trs 文件，header 含 ntraces/nsamples/dlen=32（16B PT+16B CT）
- 假设：标准 CPA 攻击可还原 AES-128 主密钥 → 动作：parse_trs_header 解析 TRS 文件 + 读 pts/cts/traces
- 动作：traces 标准化 (去均值+单位方差) → 观察：得到可比的相关性矩阵
- 假设：First_round_SubBytes + Hamming_Weight 是泄漏模型 → 动作：CPA 循环 16 字节，猜 0-255 算 SubBytes(p^b)+HW，与 trace 相关
- 动作：选 argmax → 观察：得到 16 字节主密钥 K*
- 触发点：flag{UPPERCASE_HEX} 校验用标准 AES-128 加密随机 PT 与 cts 对比 → 观察：校验通过
- 触发点：VIN_RC4 变种加密 → 假设：variantRC4Encrypt 双 KSA+反向 KSA+3 常数 + S 流 + K^(k%256)
- 动作：已知 correctEncrypted=7AF2C74EAD5C2D4505E94B820275CA8C52 + key=Z1X3Y4E5Z8V2A6H6 → 密文 ^ K ^ (k%256) 得 VIN
- 触发点：CAN 数据 → 假设：CSV signal_map 解析 CAN 帧 → 动作：Counter 统计每信号最频值
failed_attempts:
- 试图不解 TRS 直接做相关性 → 失败：没 pts/cts/traces 不能算 HW 模型
- 试图用整字节相关性代替 HW 模型 → 失败：相关性弱
- 试图不解 CAN 数据 → 失败：VIN_RC4 缺 CAN 信号没法校验
key_observations:
- AES-128 CPA：TRS 文件 ntraces/nsamples/dlen=32 → trace 标准化 → 16 字节独立猜 argmax(SubBytes+HW 相关性)
- variantRC4：双 KSA+反向 KSA+S 流+ciphertext^K^(k%256) 双异或
- CAN 数据：CSV signal_map(can_id+byte+len) 解析 CAN 帧 + Counter 统计最频值
- KSA 常数 +3 是变种 RC4 关键标志
- 车联网题常混 AES 侧信道 + RC4 变种 + CAN 总线三层
prerequisites:
- AES-128 第一轮 SubBytes + Hamming Weight 泄漏模型
- TRS 文件格式（Riscure 侧信道）
- CPA（Correlation Power Analysis）原理
- RC4 KSA + 变种识别
---
