...
quality: high
full_path: Pwn-oneday题目解析.full.md
meta_path: Pwn-oneday题目解析.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: Pwn - oneday 题目解析 - House of Apple × House of Emma 联动。House of Apple + House of Emma 高级堆攻击联动：House of Apple 写 pointer_guard 解密 pcop，House of Emma 用 pcop 跳 ROP+mprotect+orw 读 flag。。关键路径：菜单 add/delet...
category: pwn
...
contest_type: open
wp_url: https://www.ctfiot.com/164936.html
reasoning_chain:
- 触发点:菜单 add/delete/edit/show + key=10 → 假设:常规堆题
- 多个 0x20-0x80 堆块 add/delete 触发 tcache 复用 + unsorted bin → 假设:可泄 libc/heap base
- show(3) 泄 libc_base (0x1f2cc0) + heap_base (0x17f0) → 假设:用 largebin attack 写 _IO_list_all
- largebin attack:bk_nextsize 指向 _IO_list_all-0x20 → 触发 _IO_flush_all_lockp 时进入 fake file 链
- House of Apple:伪造 _IO_wstrn_jumps vtable 触发 _IO_wstrn_overflow 改 pointer_guard
- House of Emma:第二个 fake file vtable=_IO_cookie_jumps+0x58 → 触发 _IO_cookie_read → 覆盖 read 指针为 magic_gadget(pcop)
- 动作:magic_gadget=mov rdx,[rdi+8]; mov [rsp],rax; call [rdx+0x20] → 用 rax=rol(magic_gadget^expected, 0x11) 解密 pcop
- 跳 mov_rsp_rdx_ret → rsp 指向 ROP 链 → mprotect rwx 0x4000 → shellcraft.open('./flag', 0) + read + write
failed_attempts:
- 试图只走 House of Apple → 失败:pointer_guard 解 pcop 必须搭配 House of Emma
- 试图单次 largebin attack 写两处 → 失败:largebin 一次只写一地址,需要 House of Apple + Emma 联动
- 试图 system('/bin/sh') → 失败:full RELRO 改 GOT 不行,必须走 pcop 解密
key_observations:
- 单一 largebin attack 只能一次任意地址写,House of Emma 需两次
- House of Apple 改 pointer_guard 解锁加密指针,House of Emma 跳 ROP
- _IO_wstrn_jumps+_wide_data 是改 pointer_guard 经典入口
prerequisites:
- largebin attack 原理(bk_nextsize 写)
- FSOP(_IO_flush_all_lockp)与 fake file 链构造
- pointer_guard + pcop 解密机制(mov rdx,[rdi+8]; mov [rsp],rax; call [rdx+0x20])
---
# Pwn-oneday题目解析
