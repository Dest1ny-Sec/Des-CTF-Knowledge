...
lesson: '|'
quality: medium
full_path: Jade_CTF_WP.full.md
meta_path: Jade_CTF_WP.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: Jade CTF WP (HTTP smuggling / SSTI / 邮件取证)。Jade CTF 2022 多类目 writeup (HTTP request smuggling via WebSocket upgrade / Jinja2 SSTI / vol.py mbox 邮件取证)。。经验：|
category: web
...
contest_type: open
wp_url: https://www.ctfiot.com/68004.html
reasoning_chain:
- HTTP smuggling：WebSocket upgrade 头 → 触发点：CL/TE smuggling
- 动作：构造 upgrade 请求触发服务器解析差异 → 假设：内部代理与后端不一致
- 观察：内部请求被 smuggled → 完成
- SSTI Jinja2：render_template_string(user_input) → 触发点：模板注入
- 动作：lipsum.__globals__.os.popen('id') → 假设：模板全局可访问 os
- 假设：禁关键字 → 动作：lipsum|attr("\x5f\x5f...") → 完成
- mbox 邮件取证：vol.py dumpfiles → 假设：内存中 mbox 文件 → vol.py -f mem.dmp --profile=Win7SP1x64 dumpfiles → grep flag → 完成
failed_attempts:
- SSTI 直接 {{7*7}} → 失败：服务端校验
- smuggling 试普通 CL.TE → 失败：必须 upgrade 头
- mbox 试 strings → 失败：必须 vol.py
key_observations:
- WebSocket upgrade 头是 CL/TE smuggling 的变种
- Jinja2 SSTI lipsum.__globals__.os 是经典 gadget
- Volatility dumpfiles 命令对 mbox / pst 文件支持
- 模板注入绕关键字 = chr() + \x5f 编码
- Volatility 必须先 profile 才能 dump
prerequisites:
- HTTP request smuggling (CL.TE / TE.CL) + Jinja2 SSTI gadget 链
- Volatility dumpfiles 命令 + mbox 邮件格式
- Python attr() 反射
---
# Jade CTF WP
