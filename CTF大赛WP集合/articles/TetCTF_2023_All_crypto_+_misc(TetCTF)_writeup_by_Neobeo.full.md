...
quality: high
full_path: TetCTF_2023_All_crypto_+_misc(TetCTF)_writeup_by_Neobeo.full.md
meta_path: TetCTF_2023_All_crypto_+_misc(TetCTF)_writeup_by_Neobeo.meta.md
images_removed: true
images_removed_count: 1
schema_version: v3.0.0-P0
summary: TetCTF 2023 All crypto + misc writeup by Neobeo。TetCTF 2023 Neobeo 综合：负数赌场 + biased PRNG + 1-byte nonce ECDSA LLL + AES-CBC 位翻 LLL + PostgreSQL SQLi。。关键路径：Casino 注册 + Bet 负数 -10^100 拿无限余额 → ShowBal...
category: crypto
subcategory: rsa
time_required: long
difficulty_score: 4
...
contest_type: open
wp_url: https://www.ctfiot.com/89718.html
wp_author: Neobeo
reasoning_chain:
- Casino 题目：Bet 接受 Amount 参数 → 触发点：负数 -10**100 让余额溢出
- 假设：服务端 int 类型足够大但 wrap-around 不校验 → 动作：发 Bet -10^100 拿无限余额
- 观察：ShowBalanceWithProof 返回 Balance + Proof → PrintFlag 拿 flag = TetCTF{fr0m_n3g4t1v3n3ss_t0_b4nkruptcy}
- Biased PRNG：seed(2023) + 多次 random.shuffle → 触发点：37 字节 hex 共 29 行 → 假设：1 bit/行 + majority voting
- 动作：bit 出现 > len/2 次 → 1 → 还原 seed → TetCTF{fr0m_buggy_sw4p_t0_r4nd0m_b14s!}
- ECDSA 1 byte nonce：64 个签名 + LLL reduce → 假设：nonce 范围 2^8 = 256 → BKZ 还原私钥
failed_attempts:
- 试图 Bet 正数拿足够余额 → 失败：需要负数触发整数 wrap-around
- 试图标准 ECDSA nonce 攻击 → 失败：实际 nonce 是 1 byte 256 候选，需要矩阵降维
key_observations:
- 整数 wrap-around 不校验 → -10^100 直接成 '无限余额'
- biased PRNG seed(2023) + shuffle 后 majority voting 还原
- 1-byte ECDSA nonce + LLL/BKZ 是经典弱 nonce 私钥还原
prerequisites:
- Python int 类型无溢出与 wrap-around 概念
- ECDSA 签名 + nonce 长度与私钥还原
- LLL / BKZ lattice 还原算法
---
...
get_profile(token) = {'avatar': '[图片已移除]', 'email': 'A', 'name': 'A', 'coin': 1000}
