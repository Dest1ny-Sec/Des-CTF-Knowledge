...
quality: high
full_path: 2023IdekCTFWriteup.full.md
meta_path: 2023IdekCTFWriteup.meta.md
images_removed: true
images_removed_count: 0
schema_version: v3.0.0-P0
summary: '2023 idekCTF WriteUp。5 道高质量 Web：pydash 链污染+urllib fragment 拆分+flask session 伪+Go bufio 越界+php filter chain 预置 FREE。。关键路径：TaskManager: pydash.set_ 走 __init__.__globals__ 链污染 app.env / jinja_env.glob...'
category: web
...
contest_type: open
wp_url: https://www.ctfiot.com/92876.html
reasoning_chain:
- 'Q1 manage_tasks: pydash.set_(self, task, status) → 触发点：pydash 走属性路径可污染 Flask jinja_env'
- 假设：__init__.__globals__.jinja_env 可被 set_ 路径覆盖 → 动作：构造 task='__init__.__globals__.jinja_env.globals.cyclic'
- 观察：jinja_env 被污染 → 下一步：构造 SSTI 触发
- 'Q2 urllib fragment: _splittrag 用 %23 切 URL 绕过 SSRF → 假设：fragment 部分不被解析'
- 动作：构造 url=http://allowed.com%23@attacker.com → 观察：实际访问 attacker
- 'Q3 Flask SECRET_KEY: random.seed(time+offset) → 假设：可还原种子'
- 动作：收集响应时间戳 + brute force 偏移 → 观察：恢复 SECRET_KEY → 伪造 session
- 'Q4 Go bufio 越界: bufio.NewReader 包装超过底层 buffer → 假设：套娃越界'
- 动作：构造多次 Read 越界读取 → 观察：拿到 flag
- 'Q5 php://filter iconv 链 FREE 预置: 在 base64 decode 之前预置 FREE → 假设：可前置任意字符串'
- 动作：用 iconv 链 + base64 编码 FREE → 观察：LFI include 触发
failed_attempts:
- 试图直接 union select → 失败：API 接口 JSON 数据不接 SQL
- 试图用 Python eval(input) → 失败：题目无 input 函数
- 试图用标准 urllib.parse → 失败：fragment 切分未考虑
key_observations:
- pydash.set_ 走 Python 属性路径可污染 Flask jinja_env
- urllib._splittag 用 %23 切 URL 绕过 SSRF 检查
- Flask SECRET_KEY 靠 random.seed(time+offset) 可还原
- Go bufio 包装是经典套娃越界（Reader in Reader）
- php://filter iconv 链可前置任意字符串到 PHP 代码
prerequisites:
- pydash 库属性路径原理
- urllib URL 解析 + fragment 切分
- Python random 模块与 MT19937 预测
- Go bufio 包原理
---
# 2023IdekCTFWriteup
