...
quality: high
full_path: DEFCON33-Quals_nfuncs_angr、unicorn_快速自动化逆向.full.md
meta_path: DEFCON33-Quals_nfuncs_angr、unicorn_快速自动化逆向.meta.md
images_removed: true
images_removed_count: 8
schema_version: v3.0.0-P0
summary: DEFCON33-Quals nfuncs angr、unicorn 快速自动化逆向。DEFCON 33 Quals nfuncs 逆向题，angr + unicorn 联合自动化求解 8 字节表查找链。。关键路径：angr 项目用 read hook 收集 symbolic byte → capstone 提取 8 个 cmp/test 值 → 约束求解 (input[i] == targ...
category: reverse
subcategory: reverse
time_required: long
difficulty_score: 4
...
contest_type: open
wp_url: https://www.ctfiot.com/253727.html
reasoning_chain:
- 拿到 nfuncs1.exe → IDA 看到主函数 8 个 read(0, DstBuf, 1) → DstBuf[0..7] 8 字节输入 → 触发点：自定义表查找 + 8 字节比较链
- 观察：v0...v105 大量 if (DstBuf[i] & bit) v_n = v_(n-1) | const → 假设：每字节输入映射到一系列 8 位查找表
- 假设：sub_14003A1C0 接收 *DstBuf + 2815032436 → 假设：这是查表入口
- 动作：sub_14003A1C0(V110, *DstBuf + 2815032436) → if (v0==-77 && v45==0 && v60==32 && v75==107 && v90==73 && v105==-24) → 假设：6 个 cmp 命中触发 VirtualProtect
- VirtualProtect(sub_140055A50, 0x17960, 0x40, ...) → XOR smc 内容 → sub_140055A50() 调用 → 触发点：SMC 自修改代码
- 动作：angr 项目 + ReadHook 收集 symbolic byte + 每次 read 后用 capstone 提取 cmp/test 值 → 添加约束 (input[i] == target - 13)
- 假设：VirtualProtectHook 用 unicorn 模拟执行获取 smc 内容 → 动作：把 smc 写回 PE 文件
- 动作：反复 hook 循环直到 memcmp 命中 → 提取最终 ans → 下一轮 FUN_VA → 循环 N 次直到找到 flag
failed_attempts:
- 试图静态分析 sub_140055A50 函数 → 失败：函数内容是 SMC 自修改，静态看是乱码
- 试图单次 angr 求解所有约束 → 失败：SMC 多层需要 unicorn 模拟 + 写回重新分析
- 试图不 hook read 直接构造 symbolic byte → 失败：read 不是标准 simprocedure，必须自定义 hook
key_observations:
- 现代 DEFCON 逆向题常用 SMC 自修改代码 + 多层函数调用，需要 angr 符号执行 + unicorn 模拟混合
- VirtualProtect + 内存 XOR 是 SMC 触发模式，必须用 unicorn 模拟才能提取真实代码
- capstone 反汇编 cmp/test 提取立即数是自动化逆向标准手法
- Custom ReadHook + angr SimProcedure 是处理 read byte-by-byte 约束求解的关键
- memcmp 命中触发下一轮 FUN_VA 是 CTF 嵌套查表典型模式
prerequisites:
- angr 符号执行（SimProcedure / state.solver.add）
- unicorn 模拟执行（VirtualProtect Hook + smc 提取）
- capstone 反汇编（CS_ARCH_X86 / CS_MODE_64 + operands）
- claripy 符号化字节（BVS / BVV）
- PE 文件格式（rva_to_foa / section）
---
