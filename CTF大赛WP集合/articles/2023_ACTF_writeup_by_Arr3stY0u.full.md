...
- libc-2.23
attack_chain:
- 'Q1 BabyNote: tcache dup 任意地址写, tcache_perthread_struct 攻击 → __free_hook = system'
- 'Q2 QEMU pwn: MMIO/PMIO 攻击 qemu-system-x86_64 模拟器, iopl(3) 拿 I/O 权限, leak qemu/elf base, 写 shellcode'
- 'Q3 flashhacker: ACTF{REv3rsE_2_bE_A_fLAshH4cK3r_c001}'
- 'Q4 QEMU seccomp: BPF 沙箱限制 read/write/open, 用 name_to_handle_at/open_by_handle_at 绕过'
- 'Q5 fxxk: io_uring + sendmmsg 走 ORW (open via io_uring_prep_openat + sendmmsg 把 mmap 数据发给 server)'
- Q6 memfd + mmap + recv 自写 loader 加载 ELF 反弹
...
quality: high
full_path: 2023_ACTF_writeup_by_Arr3stY0u.full.md
meta_path: 2023_ACTF_writeup_by_Arr3stY0u.meta.md
images_removed: true
images_removed_count: 3
schema_version: v3.0.0-P0
summary: '2023 ACTF writeup by Arr3stY0u。2023 ACTF：QEMU 用户态 pwn + io_uring + MMIO/PMIO + 自写 loader。关键路径：Q1 BabyNote: tcache dup 任意地址写, tcache_perthread_struct 攻击 → __free_hook = system → Q2 QEMU pwn: MMIO/PM...'
category: pwn
...
contest_type: open
wp_url: https://www.ctfiot.com/142224.html
wp_author: Arr3stY0u
reasoning_chain:
- 'Q1 RE: enc_flag 是 8 个 uint64 + 异或表 → 触发点：差分密码学'
- 假设：每轮 XOR + 加减 → 动作：claripy BVS 符号执行 → 观察：ACTF{REv3rsE_2_bE_A_fLAshH4cK3r_c001}
- 'Q2 Pwn: Brainfuck 风格虚拟机，命令 wd/sd/8a → 触发点：栈布局可预测'
- 假设：8d8w = 移动 + 写 → 动作：gen_payload(old, new) 构造 ROP → leak libc/elf/buf
- 观察：pop rdi = buf, ret, system → shell
- 'Q3 QEMU 用户态 pwn: MMIO + PMIO 直接打设备 → 触发点：leak heap_base 通过 MMIO 读 ptr'
- 假设：mmap resource0 + iopl(3) 提权 → 动作：leak_u64(heap_base + 0x000e28) - 0xa23d4f = qemu base
- 观察：heap 找 shellcode (USAS...) + 写 shellcode + jump → cat ./flag
- 'Q4 seccomp 绕过的 ROP: seccomp-tools dump 看允许的 syscall → 假设：read/write/open 已禁'
- 动作：用 io_uring open + sendmmsg 替代 → 观察：read flag
failed_attempts:
- 试图用 IDA 直接分析 QEMU 源码 → 失败：QEMU 用户态逃逸需要 MMIO 思路
- 试图用传统 ROP 系统调用 → 失败：seccomp 拦截
- 试图用 one_gadget → 失败：栈对齐
key_observations:
- QEMU 用户态逃逸是 2023+ 高级 pwn
- io_uring + sendmmsg 可绕 seccomp
- MMIO/PMIO 直接打设备是 QEMU pwn 入口
- Brainfuck 风格虚拟机差分密码可符号执行解
prerequisites:
- QEMU 用户态逆向 + MMIO/PMIO 协议
- seccomp 绕过 (io_uring / sendmmsg)
- Brainfuck VM 符号执行 (claripy)
- ROP 链 + libc/elf leak
---
