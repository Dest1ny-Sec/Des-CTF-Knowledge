...
quality: high
full_path: 冠军Writeup大放送_-_DataCon2022域名体系安全赛道之“NDNS”战队.full.md
meta_path: 冠军Writeup大放送_-_DataCon2022域名体系安全赛道之“NDNS”战队.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: '冠军Writeup大放送 | DataCon2022域名体系安全赛道之"NDNS"战队。DataCon2022 域名体系安全冠军 NDNS：8 类域名分类判定指标 + 5 关域名接管 (CNAME + TXT 归属权验证)。。关键路径：国防科技大学 NDNS 战队 (DNSLab) 域名体系安全冠军 → 域名分类: 8 类 (CDN/动态/隐藏/Sinkhole/Parking/Webh...'
category: misc
...
contest_type: open
wp_url: https://www.ctfiot.com/92019.html
reasoning_chain:
- 国防科技大学 NDNS 战队 (DNSLab) 域名体系安全冠军 → 触发点：DNS 域名体系安全
- '域名分类: 8 类 (CDN/动态/隐藏/Sinkhole/Parking/Webhosting/p2p/FastFlux)'
- 'CDN 特征: 大量 CNAME, SLD FQDN 多, IP 数量多分布广 → 假设：SLD CNAME > 100, IP 地理位置数 > 0/2'
- '观察：得分 100/22 → 隐藏域名 (GFW): IP ASO 集中 (Google/Twitter/Facebook/Dropbox), IP 解析 > 14W'
- '动态域名: 花生壳/3322/dnspod/aliyun/huaweicloud → Sinkhole: NS 改研究者控制 DNS, IP 集中'
- '假设：Domain Parking: 停放服务商 Sedo/Bodis/IONOS → Webhosting: 000Webhost/Wix.com/Squarespace'
- '动作：FastFlux: storm worm 2006, 频繁换 IP/NS'
- '域名接管 level1-5: CNAME 注销 + TXT 归属权验证 → level1: 无验证直接接管'
- '假设：level2-5: TXT 记录/文件/API 验证差异化 → 动作：枚举验证接口'
failed_attempts:
- 试图只用单维度判定 CDN → 失败：CDN 需 CNAME+IP 分布多维
- 试图不解 GFW 边界矩阵 → 失败：IP ASO 集中是关键
- 试图不解 CNAME 注销 → 失败：DNS 缓存延迟
key_observations:
- CDN 域名为厂商 SLD 下的 CNAME 链 (如 admin.datacon.cdn.)
- GFW 隐藏域名 IP ASO 集中 (Google/Twitter/Facebook) 解析 >14W
- 域名接管是 CNAME 注销 + TXT 验证绕过
- FastFlux 特征是 NS/A 频繁变 (分钟级)
prerequisites:
- DNS 基础 (A/CNAME/NS/TXT)
- CDN/动态域名/Sinkhole/Parking 等域类型特征
- 域名接管 (CNAME 注销 + TXT 验证) 流程
- Python DNS 数据处理 (dnspys / scapy)
---
