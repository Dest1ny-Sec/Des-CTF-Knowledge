...
quality: high
full_path: DamCTF_and_Midnight_Sun_CTF_Qualifiers_pwn部分wp.full.md
meta_path: DamCTF_and_Midnight_Sun_CTF_Qualifiers_pwn部分wp.meta.md
images_removed: true
images_removed_count: 8
schema_version: v3.0.0-P0
summary: 'DamCTF and Midnight Sun CTF Qualifiers pwn部分wp。DamCTF 2023 Golden Banana：gets栈溢出+fmt-string劫持房间指针。关键路径：Golden Banana: 文字冒险游戏+Bing AI生成的剧情 → main中game结构体存房间信息 → 输入选项处gets栈溢出。经验：gets+printf描述信息可fmt-s...'
category: pwn
...
contest_type: open
wp_url: https://www.ctfiot.com/112814.html
wp_author: the
reasoning_chain:
- Golden Banana 文字冒险游戏 + Bing AI 剧情 → 触发点：Bing AI 生成分支剧情
- main 中 game 结构体存房间信息 + choice 结构体存跳转指针 → 假设：栈布局可溢出
- gets(g.input_buf) 栈溢出 → 触发点：gets 无界读
- printf(l->description) 格式化字符串 → 假设：description 可控触发 fmt-string
- 动作：覆盖 description 到 fmt 串 '%3$lx' → 观察：泄漏栈地址
- 假设：已知 SECRET ROOM → 动作：覆盖 choice 结构体跳转地址
- payload = '1\x00' + '1'*(0x2028-2) + p64(stack+0x1428*11) → 假设：覆盖 choice 跳到 SECRET
- 观察：SECRET ROOM 输出 flag → 关键观察：gets 是 \x0a 截断不是 \x00 截断
- scm 后续题：read 输入 + system 链 → 类似 fmt-string + gets 利用
failed_attempts:
- 试图 payload \x00 截断 gets → 失败：gets 是 \x0a 截断（\n）不是 \x00
- 试图覆盖 return address → 失败：需要先 leak 栈地址
key_observations:
- gets + printf 描述信息可 fmt-string 泄漏栈地址
- gets 是 \x0a 截断不是 \x00 截断（重要陷阱）
- text-adventure 房间结构体在栈上 → gets 覆盖 choice 跳转 SECRET
- Bing AI 生成的剧情 = 隐藏 SECRET ROOM 文本可作为 flag 触发点
- fmt-string %3$lx 等位置参数泄漏栈 64 位地址
prerequisites:
- gets / printf 边界（\x0a vs \x00 截断）
- 栈布局分析 + pwntools 编写 fmt-string payload
- fmt-string 64 位地址泄漏
---
