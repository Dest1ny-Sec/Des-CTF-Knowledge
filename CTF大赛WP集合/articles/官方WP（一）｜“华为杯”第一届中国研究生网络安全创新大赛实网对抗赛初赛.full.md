...
quality: high
full_path: 官方WP（一）｜“华为杯”第一届中国研究生网络安全创新大赛实网对抗赛初赛.full.md
meta_path: 官方WP（一）｜“华为杯”第一届中国研究生网络安全创新大赛实网对抗赛初赛.meta.md
images_removed: true
images_removed_count: 12
schema_version: v3.0.0-P0
summary: 华为杯第一届中国研究生网络安全创新大赛实网对抗赛初赛(一)官方WP。华为杯研网赛初赛官方WP(一):hashCode+QLExpress+CSP+50x50DP+迷宫花指令+LZSS六题综合。。经验：hashCode=s[0]*31+s[1]可逆推;QLExpress第一参数是表达式但有黑名单,绕Nashorn Scr...
category: misc
...
contest_type: school
wp_url: https://www.ctfiot.com/76054.html
reasoning_chain:
- 华为杯研网赛初赛 6 题：babyql hashCode 绕过 + QLExpress + CSP-Report-Only + 50x50 DP + 迷宫花指令 + LZSS 解码 → 触发点：六类型组合
- babyql hashCode = s[0]*31 + s[1] → 假设：可逆推 → 动作：传入 guanzhujiarandundunjiechbO
- QLExpress 第一参数表达式执行 → runner.execute(cmd) 黑名单 → 假设：绕黑名单 → 动作：Nashorn ScriptEngineManager(java.lang.Runtime.getInputStream) URL 编码反弹 shell
- bbzl_shell：eval($_REQUEST) 限制 REMOTE_ADDR=127.0.0.1 → 假设：CSP-Report-Only report-uri 可触发 → 动作：CSP-Report-Only report-uri=shell.php? + &bbzl'shell=system → CSP 上报时触发 shell
- 50x50 动态规划最大路径和 DP[i][j] = max(DP[i][j-1], DP[i-1][j]) + arr[i][j] 求 max_val → 假设：验证 input[i] R/D 坐标移动累加 → 动作：构造 RRDRR 路径
- 迷宫题：花指令 75 03 74 01 e8 nop → LZSS 解码 bPreBufSizeBits=8/bWindowBufSizeBits=8/bThreshold=3 解 out 文件
failed_attempts:
- babyql 试图用完整 hash 碰撞 → 失败：只逆推 s[0]*31 + s[1] 两字节
- QLExpress 试图直接 new Runtime().exec() → 失败：黑名单拦截
- CSP 试图用 iframe srcdoc 内嵌 → 失败：report-uri 才是突破口
- DP 用 BFS 穷举 50x50 → 失败：超时 DP 标准做法
key_observations:
- hashCode = s[0]*31 + s[1] 这种短碰撞可逆推
- QLExpress 第一参数是表达式，Nashorn ScriptEngineManager(java.lang.Runtime) 可绕黑名单
- CSP-Report-Only 的 report-uri 可触发任意 URL = CSP 绕过
- 50x50 路径问题必有 DP[i][j] = max(DP[i][j-1],DP[i-1][j]) + arr[i][j] 模板
- 迷宫花指令 75 03 74 01 e8 + LZSS 参数（bPreBufSizeBits=8、bWindowBufSizeBits=8、bThreshold=3）
prerequisites:
- Java hashCode 公式与短字符串逆推
- QLExpress + Nashorn 表达式执行链
- PHP eval + REMOTE_ADDR + CSP-Report-Only 绕过
- 动态规划 DP 路径模板
---
