...
quality: high
full_path: 京麒CTF_2024_初赛_Writeup.full.md
meta_path: 京麒CTF_2024_初赛_Writeup.meta.md
images_removed: true
images_removed_count: 3
schema_version: v3.0.0-P0
summary: '京麒CTF 2024初赛Writeup。京麒 2024 初赛全栈：迷宫 shellcode + WASM 爆破 + HSP3 AES + Tauri ECDSA-LLL + Java UTF-8 Overlong Rome 反序列化。。关键路径：MazeCodeV1: DFS 走迷宫得路径 + 末位&3 序列当 shellcode → 序列: 111122110011221122110011...'
category: misc
...
contest_type: open
wp_url: https://www.ctfiot.com/184181.html
reasoning_chain:
- 'MazeCodeV1: 走迷宫路径末位&3 当 shellcode → 触发点：4 个 xchg 指令映射方向'
- 动作：DFS 遍历 11112211001122112211... 序列 → 0/2/1/3 = nop/xchg_esi/push_rcx/xchg_edx
- asm(sc1+sc2+asm(sc3)) + cyclic(999).replace(p64(0x404dd0)) → /bin/sh\x00
- 'easy-wasm: window[''😘😘❤️😘😘''] 是加密函数 → 假设：每位独立加密 16 字节'
- 动作：splitStringIntoChunks(密文,16) + bruteForce 逐位爆破 → atob 还原
- 'Hot Soup: HSP3 + HSPdeco → srand(2333333) 固定 → SHA256 派生 AES-128'
- 'possible-door: Tauri 解包 index.html + index-C9fLaX_M.js → rust pdb 看 main'
- 假设：get_rand_num 与 enc 函数用同一 call_once 闭包 → 观察：get_rand_num 生成 secp256k1 私钥
- 动作：解析 DER 公钥 → secp256k1 → 41 比特 nonce + 7 个签名 → LLL 7×7 矩阵 (BB=2×timestamp)
- 私钥 = 39313219724394204510065149548180909443668279642741674773372964155008434357587
- 'ezjvav: admin/admin → /source jsrc base64 作 jwt key 伪 root → ByteCompare 黑名单'
- UTF-8 Overlong 2 字节表示 1 字节绕过字节比对 → Rome 链 EqualsBean+TemplatesImpl+HashMap
failed_attempts:
- 试图用浏览器 DevTools 调 wasm 加密函数爆破 → 失败：手速太慢
- 试图直接 ECDSA 暴力破解 256 位私钥 → 失败：天文数字
- 试图用 ysoserial 触发 ByteCompare → 失败：HashMap/TemplatesImpl/fastjson 黑名单
key_observations:
- ECDSA 弱 nonce (41 比特时间戳) 用 LLL 7×7 矩阵 (BB=2×timestamp) 一发还原私钥
- UTF-8 Overlong Encoding (2 字节表示 1 字节) 可绕过字节黑名单比对
- 迷宫路径末位 &3 当 shellcode 是逆向约束编码经典思路
- call_once 闭包在 Rust 是单例随机数生成器，多处共享同一 PRNG 种子
prerequisites:
- ECDSA 签名结构与 LLL 格基约化算法
- Tauri/Rust 程序解包 + 符号查看（pdb）
- Java 反序列化 Rome 链构造
- UTF-8 Overlong Encoding 编码原理
---
