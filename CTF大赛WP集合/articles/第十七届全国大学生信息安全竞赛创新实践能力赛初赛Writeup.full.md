...
quality: high
full_path: 第十七届全国大学生信息安全竞赛创新实践能力赛初赛Writeup.full.md
meta_path: 第十七届全国大学生信息安全竞赛创新实践能力赛初赛Writeup.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: '第十七届全国大学生信息安全竞赛创新实践能力赛初赛 Writeup。国赛信安 3 题 PWN：Go gostack syscall ROP 一血 + orange_cat_diary top chunk 缩 + __malloc_hook one_gadget + ezbuf protobuf 协议。。关键路径：PWN gostack (一血): Go 二进制 + 0x208 长度判断 + 填...'
category: pwn
...
wp_url: https://www.ctfiot.com/182350.html
reasoning_chain:
- gostack 触发点：Go 二进制栈大小 0x208 + 字符串长度检查 → 假设：填充 0x00 绕字符串检查
- 动作：payload = b'\x00'*(0x208-0x8*7) + pop_rdi + pop_rsi + pop_rdx + pop_rax(0x3b) + syscall → 观察：触发 execve(/bin/sh)
- 假设：Go 字符串验证太严格 → 动作：用 null 字节填实字符串检查
- orange_cat_diary 触发点：UAF + 堆溢出 8 字节 → 假设：top chunk 缩
- 动作：堆溢出修改 top chunk size → 申请大 chunk 让 top 进入 unsorted bin
- 假设：申请 0x68 chunk 时用 bk 残留指针泄 → 动作：show 触发 leak libc
- 动作：UAF 申请到 __malloc_hook → 写 one_gadget → malloc 触发
- third PWN：触发点：libc 2.23 one_gadget → 假设：one_gadget 满足 execve 约束
failed_attempts:
- gostack 用普通字符串 payload → 失败：被字符串检查过滤
- orange_cat_diary 直接 show 触发 → 失败：tcache 已清空
- 修改 __free_hook → 失败：glibc 2.23 one_gadget 失败
key_observations:
- Go 二进制字符串验证：长度比较 + 字符类型检查，填 \x00 可绕过
- top chunk 缩攻击：堆溢出改 top size + 申请大 chunk 让 top 进 unsorted bin
- glibc 2.23 __malloc_hook one_gadget 是历史经典攻击
- UAF 修改 tcache fd 实现任意地址申请
prerequisites:
- Go 二进制逆向（gopclntab/runtime）
- top chunk 缩技术
- pwntools ELF + one_gadget
- libc 2.23 hook 利用
---
