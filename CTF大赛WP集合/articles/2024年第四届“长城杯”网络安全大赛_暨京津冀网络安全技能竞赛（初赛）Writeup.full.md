...
quality: high
full_path: 2024年第四届“长城杯”网络安全大赛_暨京津冀网络安全技能竞赛（初赛）Writeup.full.md
meta_path: 2024年第四届“长城杯”网络安全大赛_暨京津冀网络安全技能竞赛（初赛）Writeup.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: 2024 年第四届"长城杯"网络安全大赛（初赛）Writeup。SQL %% + .htaccess + 数字 md5 爆破 + 原型链污染 + SSTI 八进制 + pwn 3 题。。经验：数字 md5（如 0cc175b9c0f1b6a831c399e269772661 = md5('a')）是单字节爆破关...
category: web
...
contest_type: open
wp_url: https://www.ctfiot.com/204463.html
reasoning_chain:
- 触发点：SQLUP 提示 %% 通配符登录 → 假设：LIKE 匹配任意串 → 动作：username=admin password=%%
- 观察：登录成功 → 触发点：上传位置禁 p 字符 → 假设：可上传 .htaccess
- 动作：上传 .htaccess (AddType application/x-httpd-php .xxx) + 1.xxx (<?php system) → 观察：解析成功
- 动作：tac /flag （tac 是 SUID 提权）→ 拿到 flag
- 触发点：CandyShop Flask secret=a123456 弱密钥 → 假设：flask-unsign 爆破 → 动作：爆破 secret
- '动作：伪造 session {identity: admin, sold: 857} → 观察：登录 admin 成功'
- 触发点：模板渲染 + 字母限制 → 假设：八进制 SSTI 绕过 → 动作：\137\137class\137\137
- 观察：拿到 secret.txt → 原型链污染 → 拿到 flag
- 触发点：FlowerShop 整数溢出改金币 → 假设：买玫瑰+卡布奇诺 → ret2libc → 拿到 flag
- 触发点：consumption cJSON_Parse 栈溢出 → 假设：覆盖 v13 → 任意地址写
- 动作：printf_got 写 heap[1] → show 泄露 libc → free_got 改 system → free chunk0
- 触发点：Kylin_Heap UAF + unsorted bin → 假设：tcache UAF 改 __malloc_hook → one_gadget 拿 flag
failed_attempts:
- 试图直接 admin/123456 登录 SQLUP → 失败：必须 LIKE 通配符
- 试图 .php 后缀上传 → 失败：黑名单禁 p 字符
- 试图数字 md5 爆破 4 位 → 失败：必须 md5('a')=0cc... 单字符映射
- 试图直接打 one_gadget → 失败：必须先 malloc_hook 写 one_gadget
key_observations:
- 数字 md5（如 0cc175b9c0f1b6a831c399e269772661 = md5('a')）是单字节爆破关键
- Flask secret 弱密钥爆破可伪造 session（flask-unsign）
- tac 是 SUID 提权工具，tac /flag 比 cat 更隐蔽
- cJSON_Parse 栈溢出是 2024 流行 pwn 题入口（解析器漏洞）
prerequisites:
- flask-unsign 工具 + Flask session 伪造
- SSTI 八进制字符串绕过字母限制
- tcache UAF + one_gadget 利用
- PHP .htaccess 配置 + Apache AddType
---
