...
quality: high
full_path: SECCON_CTF_2022_Quals_writeup_–_English.full.md
meta_path: SECCON_CTF_2022_Quals_writeup_–_English.meta.md
images_removed: true
images_removed_count: 0
schema_version: v3.0.0-P0
summary: SECCON CTF 2022 Quals writeup - English。SECCON CTF 2022 Quals 5 题：qs 1000 参数 limit 绕过 + Flask curl file:// LFI WAF 绕过 + HTTP 请求走私偷 HttpOnly cookie + DOMPurify trustedTypes RegExp.rightContext 泄露 + ...
category: web
subcategory: ssrf
tools_used:
- Flask
...
contest_type: open
wp_url: https://www.ctfiot.com/77788.html
wp_author: 11
reasoning_chain:
- 题目 1 skipinx nginx ${args}&proxy=nginx + Express 限制 → 触发：qs parameterLimit=1000
- 1000 个 & 拼参数耗尽 limit → 第一个 proxy=nginx 被丢弃 → 绕过 Express includes 检查
- 题目 2 easylfi Flask + curl file:// + WAF 拦 ../ 和 % → filename=.{.}./.{.}./flag.txt 绕过
- WAF 检查 SECCON → 用 {! 吃掉 SECCON{ 字符 → template replace 还原
- 题目 3 ldx HTTP request smuggling + cookie 窃取 → waitress parser 接受第一个 HTTP/1.1 line
- evilHeader q-encoded + contentLength=24 → bot fetch + xss → cookie 外带
- 题目 4 DOMClobber DOMPurify trustedTypes + RegExp.rightContext bypass
- emoji=0/ownerDocument/defaultView/RegExp/rightContext → 泄露过滤前 SECCON{...}
- 题目 5 Deno prototype pollution → Reflect.has + Object.setPrototypeOf Proxy 链
failed_attempts:
- 题目 1 试图单次塞 1000+ 参数 → 失败：HTTP 请求体过大被截断
- 题目 2 试图双 URL 编码 → 失败：curl 内部会解码
- 题目 3 试图绕过 smuggled cookie HttpOnly → 失败：必须用 smuggling
key_observations:
- Express qs 默认 parameterLimit=1000 是经典 HTTP 参数污染绕过
- DOMPurify + trustedTypes 仍可通过 RegExp.input/rightContext 泄露过滤前内容
- Deno 原型链污染限制 input/output 仍可 Reflect.has 触发 Proxy
- waitress parser 接受第一个 HTTP/1.1 line 是 smuggling 经典 sink
prerequisites:
- Express qs 库 + parameterLimit 配置
- Flask + curl file:// + WAF 绕过
- HTTP Request Smuggling (CL.TE / TE.CL)
- DOMPurify + trustedTypes + RegExp 注入
---
