...
quality: high
full_path: 从零开始的工控固件逆向之旅_–_VxWorks密码哈希破解实战.full.md
meta_path: 从零开始的工控固件逆向之旅_–_VxWorks密码哈希破解实战.meta.md
images_removed: true
images_removed_count: 0
schema_version: v3.0.0-P0
summary: 从零开始的工控固件逆向之旅 – VxWorks密码哈希破解实战。VxWorks loginDefaultEncrypt 加权求和+魔数+32 位掩码导致大量碰撞，FLAG+4 字符 0.07 秒爆破。。关键路径：key.bin 973KB 固件 + extracted 4.6MB → VxWorks 6.40 RTOS PowerPC 施耐德 Quantum PLC → file/hexdu...
category: crypto
subcategory: oracle
tools_used:
- C
...
contest_type: open
wp_url: https://www.ctfiot.com/283309.html
reasoning_chain:
- key.bin 973KB 固件 + extracted 4.6MB → 触发点：固件类型识别
- 动作：file/hexdump 看偏移 0x20 '140-NOE-771-01' 设备型号 → Quantum Ethernet Executive firmware Ver. 6.40
- 假设：VxWorks 6.40 RTOS PowerPC 施耐德 Quantum PLC → 动作：strings 搜 loginDefaultEncrypt @ 0x22F124
- IDA Pro 反编译 PowerPC 汇编还原 C 算法 → 加权求和 passwdInt += ASCII(char[i]) * (i+2) ^ (i+1)
- 假设：乘魔数 31695317 (0x1E3A1D5) & 0xFFFFFFFF 掩码 → 动作：字符变换 <'3'+33 / <'7'+47 / <'9'+65
- 字典攻击 + 智能暴力破解 (FLAG+4字符, A-Z+0-9 36^4=1,679,616)
- 动作：0.07 秒 / 403,259 次/秒 → 找到 FLAGAWYZ
- 32 位掩码丢弃高 5 位 → 434 个碰撞密码 → 验证 vx_hash(FLAGAWYZ) = cQwwddSRxS
failed_attempts:
- 试图直接 SHA256 爆破 → 失败：是 VxWorks 自定义哈希
- 试图还原完整 C 源码 → 失败：汇编优化太复杂
- 试图不解掩码爆破 → 失败：32 位掩码导致多碰撞
key_observations:
- VxWorks CVE-2010-2965 弱密码哈希无盐值 + 32 位掩码 (丢失 5 位) = 大量碰撞
- 防御要用 bcrypt/Argon2 + 随机盐值 + 多迭代 + 2FA
- 施耐德 Quantum 140-NOE-771-01 模块是经典 PLC 固件
- PowerPC 大端序逆向要小心字节序问题
prerequisites:
- VxWorks RTOS 固件结构
- PowerPC 大端汇编
- IDA Pro 反编译基础
