...
quality: high
full_path: The_Art_of_PHP_–_My_CTF_Journey_and_Untold_Stories!.full.md
meta_path: The_Art_of_PHP_–_My_CTF_Journey_and_Untold_Stories!.meta.md
images_removed: true
images_removed_count: 0
schema_version: v3.0.0-P0
summary: 'The Art of PHP – My CTF Journey and Untold Stories! (Orange Tsai)。Orange Tsai 大佬回忆录：PHP 内核 + CTF 二十年 + 真实 CVE + FSOP + LFI 进化 + PHAR 反序列化 + Scoreboard 0day。。关键路径：PHP 7.0 Throwable 格式串 bug #71105 配合...'
category: web
...
contest_type: open
wp_url: https://www.ctfiot.com/257942.html
reasoning_chain:
- 'PHP 7.0 Throwable $name → 触发点：format string bug #71105 → 假设：可控 $name 触发 sprintf 漏洞'
- 动作：new $model() 配合 Throwable 触发 → 观察：任意代码执行 → RCE
- max_input_vars=1000 触发 warning → 触发点：warning 破坏 HTTP header 输出 → 假设：CSP 头丢失
- 假设：WordPress maybe_unserialize 自动反序列化 DB 字符串 → Column Truncation 攻击 → 假设：特殊字符截断
- 动作：构造超长 username 触发 column 截断 → 反序列化字符串注入 → RCE
- LFI 二十年进化 → filter chain → AFR → base64 → 假设：base64 编码是 LFI 终极 payload
- PHAR 反序列化 → Laravel + mPDF 链 → 假设：mPDF 解析 phar:// 时触发 unserialize
failed_attempts:
- 试图用普通 echo + include 经典 LFI → 失败：现代 PHP 多数 disable_functions
- 试图直接 unserialize() 用户输入 → 失败：现代框架基本都禁止
key_observations:
- PHP 7.0 Throwable $name 格式串 bug 是真实 CVE，#71105 编号可查
- max_input_vars 触发 warning 能破坏后续 header 输出，绕过 CSP
- Laravel + mPDF + phar:// 是经典 PHAR 反序列化链
prerequisites:
- PHP 内核源码（Throwable / __toString 实现）
- PHP filter chain 进化史（iconv / base64 / phar）
- WordPress / Laravel 等框架的反序列化触发点
---
