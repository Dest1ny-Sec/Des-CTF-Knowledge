...
quality: high
full_path: 看雪2023_KCTF年度赛_-_第七题·智能联盟计划-设计思路及解析.full.md
meta_path: 看雪2023_KCTF年度赛_-_第七题·智能联盟计划-设计思路及解析.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: 看雪2023 KCTF年度赛 | 第7题·智能联盟计划-设计思路及解析。KCTF2023年度赛Q7《智能联盟计划》x86汇编混淆+keystone+capstone去混淆脚本。经验：大量x86混淆用keystone/capstone做汇编/反汇编引擎+Python遍历提取
category: reverse
subcategory: reverse
tools_used:
- Python
...
contest_type: open
wp_url: https://www.ctfiot.com/135974.html
reasoning_chain:
- main函数没混淆但其他函数全混淆 → 触发点：混淆代码范围大需脚本批量处理
- 假设：混淆模式固定（push eax/ebx/ebx_ebp等flag） → 动作：用keystone汇编+capstone反汇编
- 观察：ob_start=0x403AC0 ob_end=0x617DE0遍历区间 → 下一步：extract_block提取
- 假设：每个混淆块后有真实指令 → 动作：Python IDA脚本遍历+识别混淆flag组合
- is_zero/ebx_ebp/push_eax/push_ebx多flag识别 → 观察：extract_block函数还原真实指令
- 动作：去混淆后重建可读代码 → 观察：核心算法清晰可见
- 下一步：分析重建后的算法逻辑
failed_attempts:
- 试图逐函数手动nop混淆 → 失败：混淆函数数量太多（ob_end-ob_start=0x214320）
- 试图用IDA内置反混淆 → 失败：IDA的混淆识别对自定义模式无效
- 直接f5反编译混淆函数 → 失败：F5输出全是垃圾指令
key_observations:
- 大量x86混淆必须脚本批量处理（keystone/capstone引擎）
- 混淆模式固定=flag组合可枚举（push eax/ebx等）
- Python写IDA脚本是去混淆最高效方式（可调用ida_bytes）
- ob_start-ob_end地址区间是混淆代码边界的关键信号
- extract_block函数封装混淆→真实指令的转换逻辑
prerequisites:
- keystone/capstone汇编/反汇编引擎API使用
- x86指令混淆模式识别（push/pop/lea组合）
- Python IDA插件开发（ida_bytes/idaapi）
- IDA Pro脚本编写能力
---
