...
- USB-Hacker
attack_chain:
- 海特实验室开发板：ARM924EJS (Linux IoT) + AVR (单片机)
- OTG → 网口；UART 115200 串口；TF 卡 hsqs 文件加载赛题
- 'dfu-util -l 看到 5 分区: rom/kernel/env/u-boot/all'
- dfu-util -R -a kernel -U kernel.itb 导出 FIT Image
- 'dumpimage -l 看 3 个子镜像: kernel / initrd / fdt'
- dtc -I dtb -O dts fdt.dtb 改 bootargs 加 rdinit=/bin/sh
- 改 dts → 打包回 itb → dfu-util -D 刷回开发板
- /bin/sh → 抹 /etc/shadow → /sbin/init → 完整 shell
- 'MQTT 题目: mosquitto_pub -t 2022/hatlab/flag -m ''oiU7m9ipyqFdzkUFb1vfkabZ7IqiAefslrc3ovql2dA='
- 向日葵 sunloginclient 11.0.0.36662 命令注入（CVE 复现）
- 'USB Hacker: 把板子改造成 USB Mass storage 攻击目标硬件'
...
quality: high
full_path: 2021西湖论剑IOT_RW-WriteUp.full.md
meta_path: 2021西湖论剑IOT_RW-WriteUp.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: 2021 西湖论剑 IOT RW WriteUp。2021 西湖论剑 IoT RW：FIT Image dts 注入 + dfu 刷回 + MQTT 协议 + USB 设备模拟。关键路径：海特实验室开发板：ARM924EJS (Linux IoT) + AVR (单片机) → OTG → 网口；UART 115200 串口；TF 卡 hsqs 文件加载赛题 → dfu-util -l 看到 ...
category: pwn
...
contest_type: open
wp_url: https://www.ctfiot.com/30841.html
reasoning_chain:
- '[触发点] 开发板 ARM924EJS+AVR + dfu-util -l 5 分区 → 假设：FIT Image 内核可改 → [动作] dfu-util -R -a kernel -U kernel.itb / [观察] 拿到 FIT / [下一步] dumpimage 看子镜像'
- '[触发点] dumpimage -l 显示 kernel/initrd/fdt → 假设：改 fdt 加 rdinit=/bin/sh → [动作] dtc -I dtb -O dts fdt.dtb → 改 bootargs → mkimage -F 打包回 itb / [观察] 新 itb / [下一步] dfu-util -D 刷回'
- '[触发点] 刷回后引导 → /bin/sh → 假设：完整 root / [动作] mount /抹 /etc/shadow / [观察] 拿到 shell / [下一步] MQTT 题'
- '[触发点] mosquitto_pub -t2022/hatlab/flag -m ''...'' → 假设：MQTT 协议 → [动作] mosquitto 订阅 / [观察] 拿到 flag / [下一步] 向日葵'
- '[触发点] sunloginclient 11.0.0.36662 CVE 复现 → 假设：命令注入 / [动作] 构造 CVE payload / [观察] RCE / [下一步] USB Hacker 模拟'
failed_attempts:
- 试图只 dump kernel 不改 →失败：拿不到 root
- 试图直接抹 rootfs → 失败：dfu 只刷 kernel 分区
key_observations:
- FIT Image 是嵌入式 Linux 启动标准（kernel+initrd+fdt）
- dfu-util 是 IoT 调试必备
- MQTT 协议是 IoT 通讯主流
- 向日葵 11.0.0.36662 CVE 是经典 IoT 攻击面
prerequisites:
- ARM 嵌入式 Linux 基础
- FIT Image / dtb / dtc 工具链
- dfu-util 刷机流程
- MQTT 协议（mosquitto）
- UART 串口调试
---
