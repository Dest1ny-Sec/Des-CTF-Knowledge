...
- 压缩包密码爆破 882401
- exe 实际是压缩包改后缀解压
- 全局搜索 flag+hex 解码 666c61677b
- 工业流量分析 按时间排序 STOP
- 0k4o rot13
- Base32+Base64+Ascii85 混合解码 3 次
- 工业控制 s7comm
- IEC 60870-5 104
attack_chain:
...
quality: medium
full_path: 2025御网杯【本科组】线下半决赛CTF+总决赛应急响应综合渗透WriteUp.full.md
meta_path: 2025御网杯【本科组】线下半决赛CTF+总决赛应急响应综合渗透WriteUp.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: '2025 御网杯【本科组】线下半决赛 CTF+总决赛应急响应综合渗透 WriteUp。2025 御网杯本科组线下半决赛 Misc+Crypto 多题：USB 键盘流量+压缩包爆破+exe 改后缀+工业流量+rot13+Base 混合解码。。关键路径：键盘流量: iloveyou<DEL>...flag{}inKJ<DEL>...op97ya<DEL>...bc6g9 → 删 DEL 前内容 ...'
category: misc
subcategory: stego
subcategories:
- stego
...
contest_type: open
wp_url: https://www.ctfiot.com/267352.html
reasoning_chain:
- 触发点：USB 键盘流量 iloveyou<DEL><DEL><DEL><DEL><DEL><DEL><DEL><DEL>flag{}inKJ<DEL><DEL>op97ya<DEL><DEL>bc6g9 → 假设：<DEL> 是 Backspace
- 动作：8 个 DEL 删 8 字符 → 留 flag{}in；2 个 DEL 删 KJ → 留 op97；2 个 DEL 删 ya → 留 bc6g9
- 下一步：flag{inop97bc6g9}
- 触发点：压缩包密码爆破 → 假设：6 位数字常见 → 动作：用字典爆 882401 → 解压 → 文档清除格式
- 触发点：exe 16 进制头 50 4B 03 04 → 假设：实际是 ZIP 改后缀 → 动作：改 .zip 解压出 pcapng 流量包
- 下一步：全局搜关键字 flag + 16 进制编码 → 666c61677b44477377546667793147443233366673327366463264736b4c6e677d → flag{DGswTfgy1GD236fs2sfF2dskLng}
- 触发点：工业流量分析 → 假设：按时间排序 → 找最先 STOP 的包 → flag{ac6417423bb3000c}
- 触发点：Crypto 0k4o rot13 → 假设：拆 0k + 字母 + 数字，字母 rot13 → 数字保留 → 动作：每个 token rot13
- 下一步：Base32 → Base64 → Ascii85 三层混合解码 → flag{hnctfqwer54321}
failed_attempts:
- 试图忽略 <DEL> 还原键盘输入 → 失败：<DEL> 是退格键必须真删前面字符
- 试图当正常 exe 运行 → 失败：50 4B 03 04 是 ZIP magic 不是 PE
- 试图单次 Base64 解全部 → 失败：rot13 → B32 → B64 → Ascii85 必须按序逐层解
key_observations:
- USB 键盘流量 <DEL> (Backspace) 退格键每个删前面 1 字符，要计算 <DEL> 数还原真实输入
- exe 改后缀解压是流量包杂项最常见套路（ZIP magic 50 4B 03 04）
- Base32 → Base64 → Ascii85 混合解码是 CTF crypto 经典链
- 工业流量分析按时间排序找最异常状态（STOP）
- 全局搜关键字 flag + hex 解码是 pcapng 流量题快速路径
prerequisites:
- USB 流量分析（tshark + hid data 提取）
- ZIP 文件 magic 识别 + 改后缀解压
- rot13 / Base32 / Base64 / Ascii85 编码识别
- tshark 时间排序 + 工业控制协议
---
