...
quality: high
full_path: Hack-A-Sat_4_Qualifiers_pwn部分题解.full.md
meta_path: Hack-A-Sat_4_Qualifiers_pwn部分题解.meta.md
images_removed: true
images_removed_count: 8
schema_version: v3.0.0-P0
summary: 'Hack-A-Sat 4 Qualifiers pwn部分题解。Hack-A-Sat 4 Magic Space Bussin：奇数hex越界读。关键路径：Magic Space Bussin: 模拟消息队列 → 两个管道+消息节点 → 十六进制解析：2字符=1字节。经验：解析函数中奇偶处理不一致是经典OOB漏洞'
category: pwn
...
contest_type: open
wp_url: https://www.ctfiot.com/113118.html
reasoning_chain:
- 触发点：Magic Space Bussin 模拟消息队列 + 十六进制解析 2字符=1字节 → 假设：奇数长度 hex 串会触发越界读
- 动作：审计 SB_Pipe::ParsePayload → 观察：CalcPayloadLen(ishex, s) 对奇数长度保留原长，分配 msg_s 时按奇数分配但按偶数读取 → 下一步：构造奇数 hex payload
- 假设：奇数长度 hex 可越界读相邻堆块 → 动作：构造 1 个奇数 hex 节点 + 多个堆块 → 观察：unsorted bin 释放后再申请触发 OOB 泄露 libc
- 下一步：利用广播机制 + msg_max_subs 限制触发 double free → 动作：填满第一个队列后 broadcast → 观察：pipe 删除 node data 形成 A->B->A fastbin
- 假设：fastbin A->B->A 可用 tcache poisoning → 动作：申请 tcache 触发 tcache->next 指向 __free_hook → 观察：改 __free_hook 为 system
- 下一步：free 含 '/bin/sh' 的 chunk → 动作：io.interactive() → 观察：弹 shell 拿到 flag
failed_attempts:
- 试图直接 fastbin double free → 失败：glibc 2.27 后 fastbin 有 double free 检测，需先填 tcache 绕过
- 试图只 OOB 读不构造 double free → 失败：OOB 读只能泄露 libc，RIP 控制仍需 write primitive
key_observations:
- 解析函数中奇偶处理不一致（CalcPayloadLen 返回原长但循环按偶数）是经典 OOB 漏洞模式
- tcache poisoning 需先 free 1 个再修改 password（tcache_key）防止 double free 检测，是 libc 2.27+ 必备套路
- broadcast 机制在多 pipe 共享同一 payload 时会触发 copy-on-write 失败，是 C++ 多指针共享的常见 bug
- string 扩容（std::string 自动成倍分配）足以触发 unsorted bin 合并，是 OOB 泄露 libc 的关键
prerequisites:
- C++ std::string 内存管理 + 解析器奇偶不变量审计
- glibc 2.31 tcache poisoning + tcache_key 防 double free 检测
- fastbin + unsorted bin OOB 读泄露 libc
- libc-2.31 ELF 结构 + __free_hook 偏移
---
