...
quality: medium
full_path: pem类文件解析及2022蓝帽杯crypto详解.full.md
meta_path: pem类文件解析及2022蓝帽杯crypto详解.meta.md
images_removed: true
images_removed_count: 0
schema_version: v3.0.0-P0
summary: pem 类文件解析及 2022 蓝帽杯 crypto 详解。2022 蓝帽杯 PEM 文件 RSA 密码学详解。。关键路径：PEM 文件 base64 + 头尾标记 → 解析 n/e/d/p/q/u 6 个参数 → RSA.construct 重建密钥。经验：PEM 文件本质是 base64 编码的 ASN.1 DER 序列，含 6 个 RSA 参数。
category: crypto
subcategory: rsa
...
contest_type: open
wp_url: https://www.ctfiot.com/49973.html
reasoning_chain:
- 题目给出 .pem 公私钥文件 + 密文 → 触发点：先识别 PEM 编码结构（base64 + BEGIN/END 标记）
- 假设：PEM 是 base64 包 ASN.1 DER → 动作：base64 解码 .pem 看二进制 DER 序列
- 观察：DER 序列头部 0x30 + 长度字节，序列里嵌套 INTEGER 节点 → 下一步：用 PyCryptodome RSA.construct 还原
- 动作：from Crypto.PublicKey import RSA；RSA.construct((n,e,d,p,q)) → 观察：构造成功
- 假设：n/e/d/p/q 6 个参数完整 → 动作：d = inverse(e, (p-1)*(q-1)) 算 d
- 观察：解密 ciphertext 拿 plaintext → 完成
failed_attempts:
- 试图手工解 ASN.1 DER 嵌套 INTEGER → 失败：嵌套层级多且长度字段变长
- 试图用 RSA.importKey 读私钥 → 失败：需要完整 6 参数才能无密码还原
key_observations:
- PEM 文件本质是 base64 编码的 ASN.1 DER 序列，含 BEGIN/END 文本标记
- RSA 公钥 ASN.1 序列：AlgorithmIdentifier + BIT STRING(n,e)；私钥：version + n + e + d + p + q + dp + dq + qinv + u 等 9 字段
- Crypto.PublicKey.RSA.construct((n,e,d,p,q)) 是手无密码组装 RSAKey 的标准接口
- PEM vs DER：PEM = base64(DER) + 头尾，DER 是裸二进制 ASN.1
prerequisites:
- RSA 公私钥数学基础（n=pq / phi / d=inv(e,phi)）
- PyCryptodome RSA 模块使用（construct/exportKey）
- ASN.1 DER 编码基础（SEQUENCE / INTEGER / BIT STRING）
- base64 编解码 + 文件 IO
---
