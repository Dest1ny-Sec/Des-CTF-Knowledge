...
quality: high
full_path: SCTF2024_Writeup.full.md
meta_path: SCTF2024_Writeup.meta.md
images_removed: true
images_removed_count: 12
schema_version: v3.0.0-P0
summary: 'SCTF 2024 Writeup - XMCVE-Polaris 战队第 8 名。SCTF 2024 XMCVE-Polaris 第 8 名 (PWN Go UAF + REV ARM TEA/RC4 + Crypto 隐藏 phi Coppersmith + Dual RSA 同 e 不同 n) 多方向全 WP。。关键路径：PWN: Go binary 4 函数 new_user/sho...'
category: pwn
...
contest_type: open
wp_url: https://www.ctfiot.com/211528.html
reasoning_chain:
- PWN Go 4 函数 new/show/delete user → 触发点：show_user 读 [rax+0x100] content + [rbx+0x10C] length
- delete 后指针未清零 → UAF → 假设：劫持 tcache 改 show_user 指针
- allocate 触发 fastbin/tcache → 改 heaparray[2] 为 puts 地址 → show_user 泄 libc
- 改 free.got=system 或后门触发 → shell
- REV Uds ARM + TEA delta=0x61C88647 → 实际 0x9E3779B9 取负 → 假设：32-bit 标准 TEA
- RC4 key 硬编码 8 字节 → 直接解密 → flag
- Crypto Signin 隐藏 phi=(p^2+p+1)(q^2+q+1) → 假设：与 N 量级关系用 Coppersmith
- small_roots m=2 d=3 找 p+q 偏移 → phi = p^4*(p-1)*q*(q-1) → 求 d → 解密
- Crypto Whisper Dual RSA 同 e 不同 n → Liqiang et al. LLL 攻击 → 还原私钥
failed_attempts:
- PWN 试图直接 overwrite GOT → 失败：Go 编译器生成汇编较复杂
- REV 试图硬猜 delta 数值 → 失败：必须从 0x61C88647 推算 0x9E3779B9
- Crypto Whisper 试图直接因式分解 n → 失败：bit 太大需 LLL
key_observations:
- Go UAF 劫持 tcache 改 show_user 指针是 Go 漏洞经典模式
- TEA 实际 delta 0x61C88647 是 -(0x9E3779B9) 的 32-bit 表示
- Coppersmith small_roots 是大整数 p+q 偏移恢复关键
- Dual RSA 同 e 不同 n 用 LLL+unravelled linearization 攻击
prerequisites:
- Go 二进制漏洞基础（tcache + UAF）
- TEA/XTEA 算法 + delta 推导
- Coppersmith small_roots (Sage)
- LLL 算法 + Dual RSA 攻击
---
# SCTF2024 Writeup
...
{"status":
200,"msg":"图片上传成功","data":{"name":"749e04b0905aa5bf0a364f86a70f7d74.jpg.jpg","url":"[图片已移除]"},"code":"100009"}
