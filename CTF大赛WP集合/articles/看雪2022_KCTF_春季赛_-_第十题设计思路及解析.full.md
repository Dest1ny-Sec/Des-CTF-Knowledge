...
quality: high
full_path: 看雪2022_KCTF_春季赛_-_第十题设计思路及解析.full.md
meta_path: 看雪2022_KCTF_春季赛_-_第十题设计思路及解析.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: 看雪 2022 KCTF 春季赛 第十题 陷入轮回 Rust VM Pwn。Rust VM pwn：size_hint CVE 触发越界读写，Ubuntu 16 one_gadget 改 __libc_start_main_ret 拿 shell。。关键路径：题目由 Rust 编写 VM pwn 变种，未删符号表 → 漏洞来自 CVE-2021-2993 size_hint 错误实现 → R...
category: pwn
...
contest_type: open
wp_url: https://www.ctfiot.com/44357.html
reasoning_chain:
- 触发：Rust 编写 VM pwn + 未删符号表（debug 版）
- 假设：CVE-2021-2993 size_hint 错误实现 → 越界读写
- 观察：StackVec<u64,64> 模拟虚拟机栈 → 栈上分配，长度指示内容
- 假设：指令集 vec/adj/cal/cmd/jmp/je/switch_stack/halt/print → 动作：模拟执行
- 假设：pop_back/push_back/pop_front/stack_ext 触发越界 → 改返回地址
- 假设：Ubuntu 16 one_gadget 改 __libc_start_main_ret → 动作：找 one_gadget
- 观察：shell → 完成
failed_attempts:
- 试图把 Rust 当 C 直接逆 → 失败：Rust 内存模型不同（Vec/StackVec）
- 试图普通栈溢出 → 失败：Rust 安全检查（除 unsafe 块）
- 试图用其他 libc 版本 one_gadget → 失败：仅 Ubuntu 16 直接可用
key_observations:
- Rust 安全性 ≠ 绝对安全，第三方库 size_hint 错误实现越界
- 带 debug 符号的 Rust 程序逆向更容易（函数名保留）
- StackVec（栈上分配）比 Vec（堆分配）越界更易触发
- one_gadget 仅 Ubuntu 16 可直接利用，其他需多个 gadget 组合
- print 指令可泄 rsp/libc_base/main 等地址
prerequisites:
- Rust 内存模型（Vec/StackVec）理解
- CVE-2021-2993 size_hint 漏洞原理
- one_gadget 利用工具 + __libc_start_main_ret 改写
- VM 反汇编 + 指令模拟执行
---
