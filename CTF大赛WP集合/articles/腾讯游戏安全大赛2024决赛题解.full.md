...
quality: high
full_path: 腾讯游戏安全大赛2024决赛题解.full.md
meta_path: 腾讯游戏安全大赛2024决赛题解.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: 腾讯游戏安全大赛2024决赛题解。腾讯游戏安全大赛2024决赛题解,Windows内核驱动Hook NtCreateFile/NtWriteFile,CR0关闭WP+inline hook+DbgBreakPoint反调试+L"\??\C:\card.txt"过滤+DPC IRQL防竞态。。关键路径：Windows内核驱动:Hook NtCreateFile和NtWriteFile...
category: reverse
subcategory: reverse
tools_used:
- C
...
contest_type: open
wp_url: https://www.ctfiot.com/175170.html
reasoning_chain:
- 触发点：Windows 内核驱动 Hook NtCreateFile/NtWriteFile → 假设：inline hook 拦截文件操作
- 动作：CR0 关闭 WP（WPOFFx64）→ 写入 mov rax, hook_addr + jmp rax → 观察：12 字节 inline hook
- 假设：DbgBreakPoint 反调试 → 动作：DbgBreakPoint 检查 → 跳过
- 假设：未签名驱动加载被检测 → 动作：DbgPrint 输出绕过
- 动作：构造 newcode='mov rax, myFunc; jmp rax' + oldcode 备份原 12 字节 → 观察：hook 成功
- 动作：Unhook() → WPONx64() 还原 → 观察：unhook 流程
- 假设：anti-hook 检测 jmp 头部 → 动作：动态修改 jmp 目标
- 动作：MDL 锁定内存防止页交换 → 观察：MDL MmProbeAndLockPages
failed_attempts:
- 直接写入 hook 代码 → 失败：CR0 WP 阻止
- 未备份 oldcode → 失败：unhook 时无法还原
- DbgBreakPoint 不检测 → 失败：DbgPrint + 异常
key_observations:
- WPOFFx64 = CR0 &= ~0x10000 关闭写保护（CR0 第16位）
- inline hook 12 字节标准 = mov rax (10 字节) + jmp rax (2 字节)
- MDL 锁定内存保证 hook 代码页不被换出
- NtCreateFile/NtWriteFile 是文件 I/O 必经入口
prerequisites:
- Windows 内核驱动开发
- CR0 写保护位 WP
- inline hook + MDL 锁定内存
- NtCreateFile/NtWriteFile 内核 API
---
# 腾讯游戏安全大赛2024决赛题解
