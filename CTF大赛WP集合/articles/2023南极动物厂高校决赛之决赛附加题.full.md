...
quality: high
full_path: 2023南极动物厂高校决赛之决赛附加题.full.md
meta_path: 2023南极动物厂高校决赛之决赛附加题.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: 2023 南极动物厂高校决赛之决赛附加题。内核检测 ReadProcessMemory Length：置 MmTrackPtes+MiTrackPtesAborted 遍历 PTE_TRACKER 链表查 0x2023。。关键路径：ida 打开 ring3 程序看做了什么 → ida 打开 ring0 程序，几乎全 v 了（tvm0 保护） → ReadProccesMemory 打断点不蓝...
category: reverse
subcategory: reverse
time_required: long
difficulty_score: 4
...
contest_type: open
wp_url: https://www.ctfiot.com/116183.html
reasoning_chain:
- 触发点：ring3 程序用 ReadProcessMemory 触发 ring0 检测 → 假设 tvm0 保护让 IDA 看不全
- 动作：ida 打开 ring3 + ring0 → 观察：ring0 全 v 了
- actor ReadProcessMemory 打断点不蓝屏 → 假设非句柄表检测 → 动作：改 0x2023 不蓝屏
- 动作：Ntoskrnl.i64 来到 NtReadVirtualMemory → 走进 MmCopyVirtualMemory 跟 ByteCount
- 观察：MmProbeAndLockPages + MmMapLockedPagesSpecifyCache + MmUnlockPages
- actor MiInsertPteTracker 中 v8 存到 PTE_TRACKER.Length → 假设链表记录 ReadProcessMemory
- 动作：MmMapLockedPagesSpecifyCache 当 MmTrackPtes=1 时调 MiInsertPteTracker → 观察：链表节点
- 动作：置 MmTrackPtes=0 不再插入 + 置 MiTrackPtesAborted=1 遍历链表 Length=0x2023 → 蓝屏复现
failed_attempts:
- 试图断在 ReadProcessMemory 入口 → 失败：句柄表检测已排除
- 试图用 tvm 静态还原 ring0 → 失败：tvm0 全 v 看不到具体函数
key_observations:
- tvm 保护下抓导入表+黑盒分析
- WRK 反推内核结构（PTE_TRACKER.Length 是关键）
- MmTrackPtes + MiTrackPtesAborted 组合触发 PTE_TRACKER 链表
- MmMapLockedPagesSpecifyCache 是 ReadProcessMemory 必经链路
prerequisites:
- Windows 内核调试与 WRK 源码
- PTE_TRACKER / MmTrackPtes 内核结构
- tvm0 反混淆原理
- ReadProcessMemory 内部调用链
---
# 2023南极动物厂高校决赛之决赛附加题
