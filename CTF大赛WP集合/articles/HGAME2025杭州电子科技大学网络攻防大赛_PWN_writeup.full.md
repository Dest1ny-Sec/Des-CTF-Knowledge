...
quality: high
full_path: HGAME2025杭州电子科技大学网络攻防大赛_PWN_writeup.full.md
meta_path: HGAME2025杭州电子科技大学网络攻防大赛_PWN_writeup.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: HGAME2025杭州电子科技大学网络攻防大赛 PWN writeup。HGAME 2025 PWN：format字符串+diary keeper off-by-null合并。关键路径：3字节格式化字符串漏洞 → vuln=-1绕过 → %*d+%s 泄露libc地址。经验：负索引vuln可绕过；PROTECT_PTR指针保护可用低12位异或绕过
category: pwn
...
contest_type: open
wp_url: https://www.ctfiot.com/228151.html
reasoning_chain:
- format 题 → 触发点：仅有 3 字节格式化字符串窗口
- 假设：vuln=-1 可绕过边界检查 → 动作：填 -1 越界
- 动作：%*d+%s 泄 libc 地址 → 观察：成功 leak → 假设：system() 必跳
- AE64 编码 system ROP 链 → 假设：payload 必须 3 字节分段 → 动作：分多次 AE64 写入
- 观察：触发 system('/bin/sh') → 完成 week1 format 系列
- week2 diary keeper → 触发点：glibc 2.35 patchelf 替换
- PROTECT_PTR 指针保护 → 假设：低 12 位异或绕过 → 动作：堆地址 xor PROTECT_PTR_KEY
- 申请 chunkA/B/C/D → 假设：A+B+C 紧邻 → 动作：off by null 触发合并
- 观察：合并大 chunk 包含 B 内容 → 下一步：读写合并 chunk 操作 B
- 假设：B 中指针被破坏 → 动作：伪造 chunk 头 → 观察：tcache 任意地址分配
failed_attempts:
- 试图用长 payload 单次注入 → 失败：只有 3 字节窗口
- 试图直接打 GOT 表 → 失败：glibc 2.35 RELRO 严格
- 不 patchelf 用系统 libc → 失败：版本不匹配 chunk 结构
key_observations:
- 短窗口格式化字符串必须分多次 AE64 写入
- vuln=-1 边界绕过是格式化字符串经典手法
- PROTECT_PTR 用低 12 位异或绕过
- off-by-null 合并相邻 chunk 是 glibc 2.35 关键
- patchelf 替换 libc 是多版本兼容解题必备
prerequisites:
- AE64 编码工具链
- glibc 2.35 heap chunk 结构
- PROTECT_PTR 保护原理
- patchelf 工具使用
---
