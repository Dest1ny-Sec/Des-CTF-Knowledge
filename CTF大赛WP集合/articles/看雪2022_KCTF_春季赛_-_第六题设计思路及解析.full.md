...
quality: high
full_path: 看雪2022_KCTF_春季赛_-_第六题设计思路及解析.full.md
meta_path: 看雪2022_KCTF_春季赛_-_第六题设计思路及解析.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: 看雪2022 KCTF 春季赛 | 第六题设计思路及解析。KCTF2022春Q6《废土末世》PWN：纯汇编+SROP+vsyscall syscall;retn 0xffffffffff600000。经验：无libc栈溢出用SROP（sigreturn 15/77）+vsyscall=0xffffffffff600000 g...
category: pwn
subcategory: rop
time_required: long
difficulty_score: 4
...
contest_type: open
wp_url: https://www.ctfiot.com/42334.html
reasoning_chain:
- 触发：纯汇编栈溢出 + read 返回字节数触发 SROP
- 假设：栈缓冲区 0x10 溢出 → 动作：直接栈覆盖返回地址
- 观察：没有 libc, 无法普通 ROP → 假设：必须打系统信号
- 假设：syscall 号 15(64)/77(32) 是 sigreturn → 动作：构造 sigframe
- 观察：vsyscall 0xffffffffff600000 = syscall;retn → 调 read+execve(/bin/sh)
- 假设：SROP 构造完整 sigframe → 动作：填充所有寄存器
- 观察：read 返回字节数可作 SROP 触发器 → 完成
failed_attempts:
- 试图找 libc 函数 gadget → 失败：纯汇编无 libc 符号
- 试图 ret2libc → 失败：题目只有 1 个符号表
- 试图普通 ROP 链 → 失败：缺足够 gadget
key_observations:
- 无 libc 栈溢出用 SROP（sigreturn 15/77）
- vsyscall = 0xffffffffff600000 提供 syscall;retn gadget（无 ASLR）
- read 返回字节数可作为 SROP 触发器
- SROP 可一步构造所有寄存器值
- BROP 盲打是远程无源码利用基础
prerequisites:
- x86/x64 系统调用号（sigreturn 15/77）
- vsyscall 固定地址原理（0xffffffffff600000）
- SigreturnFrame 结构（ucontext_t 布局）
- ROP/SROP 基础 + BROP 盲打思路
---
