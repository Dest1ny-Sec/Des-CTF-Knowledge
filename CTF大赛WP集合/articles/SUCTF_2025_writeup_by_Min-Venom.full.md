...
quality: high
full_path: SUCTF_2025_writeup_by_Min-Venom.full.md
meta_path: SUCTF_2025_writeup_by_Min-Venom.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: SUCTF 2025 writeup by Min-Venom。SUCTF 2025 Min-Venom：cakephp 5.1.4 新链 (RejectedPromise→ConstTypeNode→Table→MockClass) + pydash jinja2 + secp256k1 签名延展性。。关键路径：cakephp 5.1.4 加 __wakeup 限制，重新找入口 → src...
category: web
...
contest_type: open
wp_url: https://www.ctfiot.com/223976.html
wp_author: Min-Venom
reasoning_chain:
- cakephp 5.1.4 旧链 __wakeup 限制 → 触发点：需新入口
- RejectedPromise.__destruct 拼 reason → 假设：触发 __toString
- PHPStan\PhpDocParser ConstTypeNode.__toString → 调 $this->constExpr->__call
- Table.__call('rewind') → BehaviorRegistry.call → 假设：_methodMap 控制分发
- _methodMap['__tostring']=['MockClass','generate'] → 调 MockClass.generate
- MockClass.generate eval(classCode) → 动作：classCode = "system('find /etc/passwd -exec tac /flag.txt ;')"
- 观察：构造完整链子 RejectedPromise→ConstTypeNode→Table → base64 → 弹 shell
failed_attempts:
- 试图直接用 Process 入口 → 失败：5.1.4 加了 __wakeup
- 试图中间绕过 BehaviorRegistry → 失败：必须经 __call 才能进 _methodMap 分发
key_observations:
- 椭圆曲线 secp256k1 签名天然有两个 s 值 (s 和 n-s)，hash 比较不等于 ecrecover 是常见绕过
- PHPStan PhpDocParser ConstTypeNode.__toString 是 CakePHP 反序列化新入口
- pydash.__init__.__globals__.sys.modules.jinja2.runtime.exported[2] 注入是 Python jinja2 SSTI 路径
- 签名延展性 (signature malleability) 在 ECDSA/Solana 中是普遍风险
prerequisites:
- PHP 反序列化 + 命名空间 chain
- secp256k1 椭圆曲线签名 (r,s,n,v) 结构
- Python jinja2 SSTI 原理
- pydash 原型链污染与 __globals__ 跨模块访问
---
