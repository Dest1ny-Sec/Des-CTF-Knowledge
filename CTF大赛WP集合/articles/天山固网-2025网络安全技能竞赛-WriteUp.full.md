...
quality: high
full_path: 天山固网-2025网络安全技能竞赛-WriteUp.full.md
meta_path: 天山固网-2025网络安全技能竞赛-WriteUp.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: 天山固网 2025 网络安全技能竞赛 - 5题WP(MISC+CRYPTO+REVERSE)。天山固网2025赛涵盖Dvorak脑洞/random种子RSA/pollard分解/p^q异或剪枝/AndroidDemo AES-ECB/50字符IDA反推5题。。经验：Dvorak键盘脑洞题需要图片描述提示;random.seed(0)固定+myPrime伪随机可预测2000个64-bi...
category: misc
...
contest_type: open
wp_url: https://www.ctfiot.com/267054.html
reasoning_chain:
- '天山固网 2025 5 题, MISC Children of the stream → 触发点: 流量包导出 rar'
- '假设: rar 需要密码, 流量里有线索 → 动作: 导出全部文件, 看图片属性描述 I''ve heard of Dvorak → 触发点: Dvorak 键盘提示'
- '流量里大串英文字符 → 大小写转换成 0/1 → 42×42 二维码 → Dvorak 键盘解码 rar 密码 → 假设: Dvorak 键盘顺序与 QWERTY 不同'
- 'CRYPTO random.seed(0) 固定 shuffle + myPrime 爆破 → Pollard p-1 分解 n → RSA 解密 → 假设: 种子固定可预测'
- 'CRYPTO Xor_Mast3r: pq_high_xor 剪枝 + sage 小根求低位 → Coppersmith small_roots → 假设: p^q 高位异或泄露'
- 'REVERSE AndroidDemo: check 函数 AES-ECB → REVERSE You_say_i_do: 输入 50 字符 + cmp 从 25 位开始 + IDApython 遍历 30E0C3 模式反推 flag 前段'
failed_attempts:
- '试图不解 Dvorak 直接爆破 rar → 失败: 字典太大'
- '试图不解 random.seed(0) → 失败: 必须用固定种子爆破'
key_observations:
- Dvorak 键盘脑洞题需要图片描述提示 (I've heard of Dvorak)
- random.seed(0) 固定 + myPrime 伪随机可预测 2000 个 64-bit 素因子
- Pollard p-1 分解光滑 p-1 的 RSA
- p^q 高位异或泄露时用剪枝 + Coppersmith small_roots 求低位
- 50 字符 flag 分两段校验用 IDApython 反推 retf 前段
prerequisites:
- Dvorak 键盘布局 + random 伪随机种子预测
- Pollard p-1 分解 + Coppersmith small_roots (Sage)
- Android JNI AES-ECB + IDApython patch
---
