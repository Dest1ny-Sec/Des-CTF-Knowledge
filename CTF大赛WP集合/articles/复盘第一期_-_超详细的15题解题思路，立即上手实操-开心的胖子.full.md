...
quality: high
full_path: 复盘第一期_-_超详细的15题解题思路，立即上手实操-开心的胖子.full.md
meta_path: 复盘第一期_-_超详细的15题解题思路，立即上手实操-开心的胖子.meta.md
images_removed: true
images_removed_count: 12
schema_version: v3.0.0-P0
summary: '复盘第一期 | 超详细的15题解题思路，立即上手实操-开心的胖子。斟茶王者战队模式 15 题复盘：银行流水/重明/二维码/Android SDK/AES-CBC/Electron XSS+内网穿透综合实战。。关键路径：战队: 童永鳌(二哥) + 钟sir (Rayankun) + M先生 + 小康(开心) 3 天通关 → Q1 资金初探: 转账银行账户+金额时间流向 → Q2 运气爆棚: 重...'
category: web
...
contest_type: open
wp_url: https://www.ctfiot.com/199104.html
reasoning_chain:
- '斟茶王者战队模式 15 题, 战队 4 人 3 天通关 → 触发点: 复合型研判 + Electron XSS RCE + 内网穿透'
- 'Q1 资金初探: 转账银行账户+金额时间流向 → 假设: Excel 透视表借贷标志分组 → 动作: 拖拽求和'
- 'Q4 转移阵地: 二维码解析 → apk 下载链接末尾 FEJXUUM04OT4 → 假设: 二维码内容是 URL → 动作: 在线解析'
- 'Q7 藏起来的钥匙: 解包 Telefram APP 找 SDK com.xingguang.PUSHSDK.appKey → 假设: 推送 SDK 是密钥藏点'
- 'Q10 人真不少: Bearer JWT + /api/chater/list 爬 250 页 × 100 条 = 25000 → 假设: 分页接口可爬 → 动作: Python requests 循环'
- 'Q15 悄悄潜入: Electron app.asar preload.js execCommand(1818) + XSS + this.parent → 假设: chatClientApi.execCommand 是 RCE 入口 → 动作: 内网穿透 NATAPP 开 TCP 隧道监听 + Web 隧道加载 js → 完成'
failed_attempts:
- '试图不解 AES 直接读聊天 → 失败: 备份 SQL 是密文'
- '试图不开内网穿透直连 → 失败: Electron 客户端无法直连攻击机'
key_observations:
- 涉网犯罪研判复合型 + Electron preload.js execCommand(1818) + this.parent chatClientApi 上下文是 XSS→RCE 关键
- 内网穿透 NATAPP 双隧道 (TCP+Web) 是反向 shell 必备
- AES key 在 SDK appKey 常点 (com.xingguang.PUSHSDK.appKey)
- 二维码解析 APK 下载链接, 末尾 ID 是关键标识
- Bearer JWT 头 + 分页爬虫是名单获取标准姿势
prerequisites:
- 银行流水数据透视表
- 二维码解析 + Android SDK 藏密钥识别
- Electron XSS RCE (preload.js execCommand)
- NATAPP 内网穿透 (TCP + Web 双隧道)
---
