...
quality: high
full_path: bi0sCTF2022_Writeup_by_r3kapig.full.md
meta_path: bi0sCTF2022_Writeup_by_r3kapig.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: bi0sCTF 2022 Writeup by r3kapig。bi0sCTF 2022 r3kapig 战队多题 WP，菜单类 PWN 注入与跨题 ROP 链构造。。关键路径：逆向菜单 1-5 选项 add/delete/show/edit/encrypt → 发现静态密钥 2111485077978050 → encrypt(0, decrypt(payload)) 触发注入。经验：CT...
category: pwn
...
contest_type: open
wp_url: https://www.ctfiot.com/93593.html
wp_author: r3kapig
reasoning_chain:
- 触发点：菜单类 add/delete/show/edit/encrypt + 静态密钥 2111485077978050 → 假设：encrypt 解密复用注入 + 跨题 ROP
- 动作：逆向 notes 主程序 → 观察：menu 1-5 选项，选项 5 encrypt(idx, content) 调用内部 AES → 触发点：硬编码 key
- 假设：encrypt(0, decrypt(payload)) 是经典注入模式 → 动作：先用静态 key 解密 payload → 再 encrypt → 观察：返回密文即原始 payload
- 动作：构造 payload = shellcode + '/bin/sh' 字符串 → 观察：通过 encrypt 链绕过内容检查 → 写回 notes
- 下一步：触发选项 4 edit，让堆指针改写 → 假设：可触发 ROP 链 pop rdi + pop rax + syscall
- 动作：构造 ROP chain → execve /bin/sh → 观察：拿到 shell → 完成
- 跨题利用：encrypt 题拿到的 AES key 可作为其他 Pwn 题的密钥复用 → 假设：CTF 内部跨题 oracle
failed_attempts:
- 试图只利用 edit 漏洞做 UAF → 失败：菜单类题目 UAF 不完整，需配合 encrypt 注入
- 试图爆破 AES key → 失败：key 硬编码在二进制，反汇编即可
- 试图在 encrypt 选项外构造 ROP → 失败：栈空间有限，需用 bss 二段栈劫持
key_observations:
- encrypt-decrypt 注入链是菜单类 PWN 经典套路：解密可控 → 再加密触发漏洞
- CTF 比赛内跨题 oracle 是真实场景：encrypt 题的 key 可作为其他题的密钥复用
- AES 硬编码 key 在反汇编中通常以 hex 数组出现，搜索特征常量更快定位
- 栈空间不足时用 bss 段做二次栈劫持是扩展 ROP 容量的有效手段
prerequisites:
- pwntools 菜单类交互脚本（sla / sa / ru）
- AES 加解密原理与静态密钥识别
- ROP chain 构造（pop rdi + pop rax + syscall）
- ELF 逆向（Ghidra/IDA 找硬编码常量）
---
