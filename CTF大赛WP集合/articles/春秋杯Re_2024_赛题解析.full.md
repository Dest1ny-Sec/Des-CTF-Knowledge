...
quality: high
full_path: 春秋杯Re_2024_赛题解析.full.md
meta_path: 春秋杯Re_2024_赛题解析.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: 春秋杯Re 2024 赛题解析。春秋杯Re 2024三题：pygame RC4+UPX脱壳+IDC patch+TLS TEA+Nim时间反调试+二叉树TEA。经验：异或patch绕过反调试、TLS回调函数生成密钥、TEA delta=0x9E3779B9/0x61CBB648变种
category: reverse
subcategory: reverse
tools_used:
- IDA
...
contest_type: open
wp_url: https://www.ctfiot.com/193685.html
reasoning_chain:
- 题一 Snake：pygame 框架 → 触发点：把 pyc 扔在线反编译
- 观察：代码是 RC4 + 循环次数异或 → 动作：还原 RC4 + 解循环异或 → flag
- 题二 HardSignin：UPX 字样改成 VMP → 触发点：伪装压缩壳
- 假设：改回 UPX 即可脱壳 → 动作：手动改字符串 + UPX -d
- 观察：脱壳后 IDA F5 看 TLS_3 回调 → 触发点：TLS 回调里有 main 异或
- 动作：IDC 脚本 PatchByte(x, Fbin^0x66) 还原 main 函数 → 观察：代码可读
- base64+RC4+TEA 三层解密 → 动作：写三层解密脚本 → 拿到 flag
- 题三 Bedtea：Nim 语言 → 触发点：时间反调试
- 动作：改跳转恒成立绕过 → IDA 看二叉树先序后序遍历
- 观察：得到倒序 0x33 异或 → TEA decipher(num_rounds=100, k={0x0CAA5BDD,...}) → flag
failed_attempts:
- CE 改 Snake 分数 9999 → 失败：游戏无法暂停，新建进程属另一进程
- HardSignin ESP 断点手脱 → 失败：被反调试结束
key_observations:
- UPX 改 VMP 字样是最简单的反脱壳伪装，改回即可
- TLS 回调函数常用于反调试和密钥生成，必须先于 main 执行
- Nim 语言二进制 IDA 可识别特征（函数 prologue）
- TEA delta 变种 0x9E3779B9/0x61CBB648 是识别魔改 TEA 的关键
prerequisites:
- IDC 脚本基础（IDA 自动化 patch）
- TLS 回调函数原理
- TEA/XTEA 算法变种识别
- Nim/Go/Rust 编译特征识别
---
