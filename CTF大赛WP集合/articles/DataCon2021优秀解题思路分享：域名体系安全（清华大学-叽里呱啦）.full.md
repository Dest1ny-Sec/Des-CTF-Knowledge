...
quality: high
full_path: DataCon2021优秀解题思路分享：域名体系安全（清华大学-叽里呱啦）.full.md
meta_path: DataCon2021优秀解题思路分享：域名体系安全（清华大学-叽里呱啦）.meta.md
images_removed: true
images_removed_count: 1
schema_version: v3.0.0-P0
summary: DataCon2021优秀解题思路分享：域名体系安全（清华大学-叽里呱啦）。DataCon2021域名体系安全：27万域名分类+并查集团伙分析+HTML相似度优化。关键路径：27万黑白域名分类：涉赌/涉黄/涉诈 → 黑产团伙关联分析 → 7种关联边（强到弱）：CA Alternative Name/HTML IP邮箱/统计ID/动态跳转URL/统计链接/人工特征/HTML相似度。经验：大规模...
category: misc
...
contest_type: open
wp_url: https://www.ctfiot.com/75695.html
reasoning_chain:
- 27 万黑白域名分类：涉赌/涉黄/涉诈 → 触发点：大规模域名分类
- 假设：dns-crawler + Playwright/Chromium 抓取 + 多进程 → 动作：批量抓 HTTPS CA + HTML + 动态 URL
- 观察：CA cert + HTML + 统计 ID 拿到 → 下一步：构建 7 种关联边
- 7 种边（强到弱）：CA Alternative Name / HTML IP 邮箱 / 统计 ID / 动态跳转 URL / 统计链接 / 人工特征 / HTML 相似度
- 动作：并查集维护子图 → 假设：同一团伙域名在同一并查集
- HTML 相似度：最长公共子序列 (LCS) + 桶排序优化 → 触发点：LCS O(N²L²) 太慢
- 优化：判定问题（占比 > 90% + 相差 < 100 字符）+ 桶排序预过滤 + 提前退出 → O(100N)
- 动作：C++ 实现 + 多进程 → 观察：完成 27 万域名团伙分析
- 部分人工干预：去掉过期跳转域名商、相同模板误判 → 触发点：聚类去噪
failed_attempts:
- 试图用 Python LCS → 失败：太慢改 C++
- 试图用 NN 嵌入聚类 → 失败：黑产模板雷同导致 embedding 接近但实属不同团伙
key_observations:
- 并查集 + LCS 桶排序优化 + 多进程是大规模域名分析三件套
- CA Alternative Name 是最强关联边（无需人工干预）
- HTML 相似度聚类需排除 404/模板雷同误判
- 桶排序预过滤使 LCS 候选集远小于 O(N²)
- 判定问题（阈值）替代数值问题可提前退出加速
prerequisites:
- 并查集算法 (Union-Find / Disjoint Set)
- LCS 动态规划 + 桶排序预过滤优化
- C++ 多进程 + Playwright 动态渲染
---
