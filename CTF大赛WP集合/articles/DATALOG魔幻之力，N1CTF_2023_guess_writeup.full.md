...
quality: high
full_path: DATALOG魔幻之力，N1CTF_2023_guess_writeup.full.md
meta_path: DATALOG魔幻之力，N1CTF_2023_guess_writeup.meta.md
images_removed: true
images_removed_count: 7
schema_version: v3.0.0-P0
summary: DATALOG 魔幻之力，N1CTF 2023 guess writeup。N1CTF 2023 guess 题用 Soufflé 静态分析引擎 + Datalog 推理规则恢复 flag 序号。。关键路径：阅读 Soufflé 文档了解符号表机制 → 理解 ordinal number 表示法 → 利用 ord(@GETFLAG()) 推理规则。经验：Soufflé 等静态分析引擎的 sy...
category: crypto
subcategory: math
time_required: long
difficulty_score: 4
...
contest_type: open
wp_url: https://www.ctfiot.com/141277.html
reasoning_chain:
- 题目给 Soufflé 源文件 + chal.py → 触发点：Datalog 静态分析推理规则
- 假设：必须构造 GUESS 关系让 x = @GETFLAG() → 36 轮外部临时 flag 都对 → 动作：分析 Soufflé 文档
- 观察：@GETFLAG()/@hash1/@hash2 是外部库函数 → SALT_HASH1/HASH/HINT 等关系暴露部分信息
- 假设：直接 FLAG(x) 或 x=@GETFLAG() → 失败：chal.py 禁止 F f . 三个字符 → 假设：必须用其他间接方式
- 假设：Soufflé 把所有 symbol 存 symbol table → ord(@GETFLAG()) 得 flag 序号 → 序号→字符串转换函数 as(n, symbol)
- 假设：每轮盐数变化导致 flag 序号变化 → 动作：用 __LINE__ 计算盐的个数 + 序号差值固定 → 写规则 x=as(__LINE__-N, symbol)
- 观察：Soufflé 优化删除未引用的关系 → 假设：必须让 GUESS 间接引用 FLAG（如 HINT(_)） → 动作：GUESS(x) :- HINT(_); x=as(__LINE__-N, symbol)
- 假设：SALT 关系被输出 → 如果 GUESS 引用 SALT 则 Soufflé 先输出 SALT → chal.py 报错 → 假设：不引用 SALT
- 动作：构造规则 36 轮不重置 → 完成
failed_attempts:
- 试图直接写 FLAG(x) :- @GETFLAG() → 失败：被 F 字符禁止
- 试图用 SALT_HASH1 反推 flag → 失败：hash 函数保留信息有限，无法逆推
- 试图不引用 FLAG 关系 → 失败：Soufflé 优化删除 FLAG 后 GETFLAG() 不再插入 symbol table
key_observations:
- Soufflé 等静态分析引擎的 symbol table + 推理规则是新型 CTF 出题方向
- ord() 把 symbol 转序号，as(n, symbol) 把序号转 symbol，是 Soufflé 字符串操作核心
- __LINE__ 在 Soufflé 中是宏，可用于动态计算规则位置（影响 salt 个数）
- Soufflé 优化会删除 '未被输出关系直接或间接引用' 的关系，必须用 HINT(_) 等保持 FLAG 存活
- chal.py 用 base64 解码盐字典 + random.choices 随机盐数 + 36 轮稳定猜测
prerequisites:
- Soufflé Datalog 静态分析引擎使用
- Datalog 推理规则（:- 形式）
- Soufflé symbol table / ordinal number 机制
- Functor 外部库函数调用机制
- Python subprocess + base64 + random.choices
---
