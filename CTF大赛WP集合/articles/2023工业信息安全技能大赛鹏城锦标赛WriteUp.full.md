...
quality: high
full_path: 2023工业信息安全技能大赛鹏城锦标赛WriteUp.full.md
meta_path: 2023工业信息安全技能大赛鹏城锦标赛WriteUp.meta.md
images_removed: true
images_removed_count: 5
schema_version: v3.0.0-P0
summary: '2023 工业信息安全技能大赛鹏城锦标赛 WriteUp。工业信息安全综合：IEC104 + CAN + ICMP 隐写 + SMC TEA 变种 + AES + 手撕 PEM RSA。。关键路径：工控-Welcome: IEC 60870-5 流量追踪异常 base64 解密 flag{sign_in_successfully!} → 解 Hex0 + Base64 + 键盘密码 → CA...'
category: forensic
...
contest_type: open
wp_url: https://www.ctfiot.com/139444.html
reasoning_chain:
- 触发点：工控-Welcome IEC 60870-5 流量追踪异常 → 假设 base64 解密 → 观察：flag{sign_in_successfully!}
- 动作：解 Hex0 + Base64 + 键盘密码 → 假设 RSA PEM 公钥 → 动作：手撕 PEM 结构
- actor RSA PEM corrupt → 假设 RsaCtfTool 找 e → 观察：factorDB 分解找 p,q
- '动作：RsaCtfTool issue #304 找 e + Wiener 攻击兜底 → 观察：解出明文'
- actor CAN 包 ISO 15765 ID=730 导出二进制 → 假设 strings找假 flag → 动作：反编译 pyc
- actor 源码逆向每条 ICMP data 长度转 ASCII（192.168.3.73 源） → 假设 ICMP 隐写
- actor SMC TEA 变种 delta=0x11451419 移位改 → 假设去花指令还原 → 动作：idapython
- 动作：AES-ECB key=rGzuwTc31NRH9tsT → 观察：解密 flag
failed_attempts:
- 试图用 PyCryptodome 解 PEM → 失败：corrupt 后段需手撕结构
- 试图直接爆破 e → 失败：e 未知需 Wiener攻击兜底
key_observations:
- PEM corrupt 时手撕结构体是 RSA 标配
- SMC TEA 变种要看 delta 和移位
- Wiener 攻击是大 e RSA 兜底方案
- ICMP data 长度隐写是工控常见手法
prerequisites:
- IEC 60870-5 工控协议
- ISO 15765 CAN 协议
- PEM 结构与 RSA 数学
