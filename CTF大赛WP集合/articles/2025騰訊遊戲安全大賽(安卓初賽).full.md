...
quality: high
full_path: 2025騰訊遊戲安全大賽(安卓初賽).full.md
meta_path: 2025騰訊遊戲安全大賽(安卓初賽).meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: 2025 騰訊遊戲安全大賽 (安卓初賽)。腾讯游戏安全安卓初賽 — UE4 + Disable Depth 透视 + LineTrace 渲染控制 + pthread_create 钩子 anti-bypass + RecoilPitch 改 0 去后座 + 多个 libUE4.so 偏移 aimbot 替代实现。。关键路径：Disable Depth Test + Custom Depth...
category: reverse
subcategory: reverse
time_required: long
difficulty_score: 4
...
contest_type: open
wp_url: https://www.ctfiot.com/240160.html
reasoning_chain:
- UE4 渲染管线检查 → 触发点：Disable Depth Test + Custom Depth 是 UE 透视经典实现
- LineTraceSingle(FirstPersonCharacter_C, ThirdPersonCharacter_C) → 假设：HitResult.Distance==0 时未被遮挡
- 动作：根据 Distance==0 切换 ThirdPersonCharacter_C MainPass 渲染 → 观察：墙体后的对手变透明
- 子弹射脚底才有碰撞 → 假设：游戏碰撞体设置在 Character 下半身
- Frida hook pthread_create → 观察：libGame.so+7068 偏移线程是 anti-detect bypass 钩子
- 动作：拦截 7068 偏移线程，其他原样调用 → 下一步：CharacterMovementComponent +0x1a0 改 MaxWalkSpeed=1e9 → 飞天
- MyProjectCharacter 0x528 bUsingMotionControllers / 0x52c RecoilPitch / 0x530 RecoilYaw / 0x534 RecoverySpeed / 0x538 AccumulationRate 全改 0 → 去后座
- patch libUE4.so+0x8B3861C (mb_aimbot) → NativeCallback 替代实现 aimbot
- hook libUE4.so+0x6711D34 (OnHit) 打印 camera rotation → aimbot 数据来源
failed_attempts:
- 试图直接读写 GameMemory → 失败：UE4 GWorld/GName 全局对象偏移动态变化
- 试图全量 hook libUE4.so → 失败：反作弊常驻检测 libGame.so+7068 偏移线程拦截
- 仅改 MaxWalkSpeed 而不改 Acceleration → 失败：走路依然慢
key_observations:
- UE4 透视 = Disable Depth Test + LineTrace + MainPass 切换是渲染管线的标准做法
- libGame.so+7068 偏移线程是安卓反作弊的 anti-bypass 钩子点
- MyProjectCharacter 0x52c RecoilPitch / 0x534 RecoverySpeed 是去后座经典位
- aimbot patch 比写一整套目标追踪更稳定（NativeCallback 替代实现）
- ue4dumper64 --sdku --gname/guobj 是 UE4 对象枚举的必备工具
prerequisites:
- UE4 渲染管线（Disable Depth / Custom Depth / MainPass）
- Frida Native API（Interceptor.replace/NativeFunction）
- UE4 SDK 偏移识别（ue4dumper64）
- 安卓 SO 注入与 inline hook（libUE4.so）
---
# 2025騰訊遊戲安全大賽(安卓初賽)
