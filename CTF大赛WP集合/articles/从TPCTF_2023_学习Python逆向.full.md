...
quality: high
full_path: 从TPCTF_2023_学习Python逆向.full.md
meta_path: 从TPCTF_2023_学习Python逆向.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: '从TPCTF 2023学习Python逆向。TPCTF 2023 nanoPyEnc AES-ECB + maze Cython 编译产物 IDA 符号重建。。关键路径：nanoPyEnc: enc 列表 48 字节先 XOR 1 再 AES-ECB 解密 key=''2033-05-18_03:33 → maze: Cython 编译产物 IDA base=0x44960 重建 PyObje...'
category: reverse
subcategory: reverse
tools_used:
- IDA
...
contest_type: open
wp_url: https://www.ctfiot.com/149063.html
reasoning_chain:
- 'nanoPyEnc: enc 列表 48 字节 → 触发点：硬编码密文 + AES-ECB 解密'
- 假设：先 XOR 1 再 AES-ECB key='2033-05-18_03:33' → 动作：bytes([x^1 for x in enc]) + AES.decrypt
- 'maze: Cython 编译产物 → IDA base=0x44960 重建 PyObject*/PyTypeObject* 数组'
- create_qword + SetType + set_name 还原符号 ys, ns
- 还原 MazeLang 类 (TWF6ZUxhbmc) 含 add_function/cell/step/get_pos/op/__init__/__repr__
- check c29sdmU(SvL6VEBRwx) 比对 33 字节 → aW5pdF9zZWNyZXQ 走 EqdU3uQNCi 33 元素排列
- 反变换：c2VjcmV0[i], c2VjcmV0[EqdU3uQNCi[i]] 互换 → key='HITPCTF' XOR c2VjcmV0[i] 取后 33 字节
failed_attempts:
- 试图用 pycdc 反编译 maze → 失败：Cython 不是标准 .pyc
- 试图直接标准 AES 解密 → 失败：先 XOR 1
- 试图不重建符号盲逆 → 失败：函数调用关系不清楚
key_observations:
- Python 字节码逆向先识别编译方式 (CPython .pyc, Cython, PyInstaller, Nuitka)
- Cython 编译产物的 PyObject 数组可通过 IDA Python 重建符号
- AES-ECB 短密钥直接字典/已知时间戳爆破
- 排列置换反演：已知目标字节反推初始数组
prerequisites:
- Cython 编译产物逆向（PyObject* 数组重建）
- IDA Python 脚本（create_qword/SetType/set_name）
- AES-ECB 加解密原理
- Python 类与 __init__/__repr__ 方法
---
