...
quality: high
full_path: 四川省职工职业技能大赛网络安全决赛题解.full.md
meta_path: 四川省职工职业技能大赛网络安全决赛题解.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: '四川省职工职业技能大赛网络安全决赛题解。四川省职工决赛 6 题: phar 反序列化 + 楞伽经 AES + 自定义 Sbox AES + tcache one_gadget + 流量 csv 统计。。关键路径：题1 phar.php: class A __destruct + class B __invoke + system($cmd) → $a->a = new B(); $b->cm...'
category: misc
...
wp_url: https://www.ctfiot.com/197155.html
reasoning_chain:
- '四川省职工决赛 6 题, 题 1 phar.php class A __destruct + class B __invoke → system($cmd) → 触发点: PHP 反序列化 gadget 链'
- '$a->a = new B(); $b->cmd = ''cat /flag'' → Phar::setMetadata($a) + .phar 打包 → upload.php 把 .phar 改 .png 上传 → 触发点: 文件上传 + phar:// wrapper'
- '假设: file_get_contents($_GET[''file'']) 接受 phar:// → 动作: ?file=phar://uploads/uniqid().png → 观察: 反序列化触发 → 完成'
- '题 2 DASCTF 楞伽经 ruShiWoWen 表 256 字符 → 假设: 自造字符表索引加密 → 动作: index ^ 64 得字节 → KEY=DASCTF@Key@^_^@Encode!!Buddha!!!, IV=IV|DASCTF|OvO|IV'
- '题 3 自定义 AES 256 字节 Sbox 自实现, Rcon 标准 → 假设: key=''Hi Android!0oO00'' + table 置换 → 动作: IDApython 还原 AES'
- '题 4 pwn_1 libc 2.27: add 9 个 + free 8 个 + show 泄 heap/libc → edit(3, p64(__free_hook)) + add(one_gadget 0x4f302) + free(0) → shell'
failed_attempts:
- '试图不解 phar 直接拿 flag → 失败: 必须反序列化触发'
- '试图爆破楞伽经字符表 → 失败: 表是固定的, 必须知道顺序'
key_observations:
- phar:// wrapper 是 PHP 文件上传配合反序列化的金钥匙 (.phar 改 .png 上传)
- 楞伽经类题是自造字符表 (256 字符) 索引加密
- tcache __free_hook + one_gadget 0x4f302 是 glibc 2.27 标准 UAF 利用
- 自定义 AES Sbox 需还原 Rcon + 逆 Sbox
- uniqid().png 命名可猜测文件路径
prerequisites:
- PHP 反序列化 (Phar 触发 + class __destruct 链)
- AES 自实现逆向 (Sbox 还原 + Rcon)
- tcache UAF (__free_hook + one_gadget)
- tshark 流量分析 (csv 统计)
---
# 四川省职工职业技能大赛网络安全决赛题解
