...
quality: high
full_path: 看雪2023_KCTF年度赛_-_第三题设计思路及解析.full.md
meta_path: 看雪2023_KCTF年度赛_-_第三题设计思路及解析.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: 看雪2023 KCTF年度赛 | 第三题设计思路及解析。KCTF2023年度赛Q3《秘密计划》SOUI界面+4个定时器反调试+ARM代码明文SHA256验证。经验：SOUI界面逆向先提取dlg_main.xml定位vftable；定时器反调试patch jz→jmp绕过
category: reverse
subcategory: reverse
tools_used:
- IDA
...
contest_type: open
wp_url: https://www.ctfiot.com/134376.html
reasoning_chain:
- GUI程序用SOUI框架 → 触发点：先拖Resource Hacker看资源
- 动作：提取dlg_main.xml → 观察：input_va输入框+check_va按钮 → 下一步：IDA找WinMain
- simulation::vftable偏移2a8 → 假设：vftable定位是GUI逆向切入点
- 0x4078C5 CloseHandle((HANDLE)0x99999999) → 触发点：异常反调试
- 动作：触发EXCEPTION_INVALID_HANDLE调试器捕获 → 假设：必须patch绕过
- 4个CreateTimerQueueTimer（NtSetInformationThread/QueryInformationProcess/Mutex） → 假设：4种反调试叠加
- 动作：0x406102处 jz→jmp绕过反调试 → 观察：定时器反调试失效
- sub_405C50 switch 0x47A-0x47E自定义消息 → sub_4061D0 ARM代码明文SHA256验证
failed_attempts:
- 试图逐个patch定时器回调 → 失败：4个定时器分散调用栈各处
- 试图hook CloseHandle过滤异常 → 失败：异常码在kernel层传递，user hook不到
- 忽略ARM代码直接反编译主exe → 失败：核心check在ARM代码里
key_observations:
- SOUI界面逆向先看dlg_main.xml找LAUNCHER类
- CloseHandle异常反调试 (HANDLE)0x99999999 是经典手法
- CreateTimerQueueTimer + 多重反调试 (NtSetInformationThread/Mutex)
- jz→jmp patch是定时器反调试通用绕过
- ARM代码明文对照=SHA256爆破节省反编译工作量
prerequisites:
- SOUI/Resource Hacker GUI逆向
- Windows异常反调试机制（CloseHandle/NtQueryInformationProcess）
- ARM汇编基础
- SHA256 hashcat/字典爆破工具
---
