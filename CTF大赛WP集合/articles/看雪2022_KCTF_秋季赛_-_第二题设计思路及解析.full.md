...
quality: high
full_path: 看雪2022_KCTF_秋季赛_-_第二题设计思路及解析.full.md
meta_path: 看雪2022_KCTF_秋季赛_-_第二题设计思路及解析.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: 看雪2022 KCTF 秋季赛 | 第二题设计思路及解析。KCTF2022秋Q2《盗贼作乱》：62进制key+大数+31*x=1mod n数论+BigNumMul/Div干扰计算。经验：大数模逆用gmpy2.invert；自定义大数库的隐藏干扰点（第31次触发）
category: reverse
subcategory: reverse
tools_used:
- gmpy2
...
contest_type: open
wp_url: https://www.ctfiot.com/76287.html
reasoning_chain:
- 触发：14 位 key 按 - 拆 2 部分 → 62 进制转大数
- 假设：sbase=0-9A-Za-z 大数 → 动作：baseConv 转回 int
- 假设：num=10^19 cnt=32 → 动作：calc(num, cnt) 求 (31*x=1 mod n) 整数解
- 观察：num*x+1 / (cnt-1) = n → 假设：x 是解范围 (1, cnt)
- 假设：31*x=1 mod n 大数模逆 → 动作：gmpy2.invert(31, n)
- 假设：BigNumMul/BigNumDiv 干扰计算（第 31 次触发 +4） → 观察：注册成功
- 假设：key=ZSxZerX4xb4-jyvP7x12lI7 → 完成
failed_attempts:
- 试图不解大数直接逆算法 → 失败：自定义大数库干扰
- 试图穷举 cnt 范围 → 失败：num=10^19 范围太大
- 试图忽略 BigNumMul/Div 干扰 → 失败：flag 永远 < 10
key_observations:
- 62 进制 (0-9A-Za-z) = 标准 base62 编码
- 大数模逆用 gmpy2.invert(a, n)
- 自定义大数库的隐藏干扰点（第 31 次触发 +4）
- num 和 cnt 与注册码无明显联系 → 需数学特性分析
- cnt-1 整除 num*x±1 是核心判断条件
prerequisites:
- 数论基础（大数模逆）
- base62 编码 + 大数运算
- gmpy2 库使用（invert）
- 自定义大数库 IDA 分析（BigNumMul/BigNumDiv）
---
