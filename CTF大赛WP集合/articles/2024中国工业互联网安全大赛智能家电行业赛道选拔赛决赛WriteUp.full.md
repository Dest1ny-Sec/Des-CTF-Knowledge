...
quality: medium
full_path: 2024中国工业互联网安全大赛智能家电行业赛道选拔赛决赛WriteUp.full.md
meta_path: 2024中国工业互联网安全大赛智能家电行业赛道选拔赛决赛WriteUp.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: 2024 中国工业互联网安全大赛智能家电行业赛道选拔赛决赛。工控赛事综合：Ble+路由器+shiro+Modbus+FINS+IEC104+智能家居 IoT。。经验：工控流量 modbus 寄存器值 35152 是 PNG \x89P magic；FINS 协议头 46494e53。
category: misc
subcategory: stego
subcategories:
- stego
...
wp_url: https://www.ctfiot.com/197061.html
reasoning_chain:
- 触发点：决赛检材 BLE 流量 → 假设：link key 在 BLE Security Manager → 动作：wireshark 提取 link_key
- 观察：拿 link_key 解压 → 拿到第一题 flag
- 触发点：RSA 同模攻击题 → 假设：e1=3 e2=17 → 动作：gcdext(3,17) 求逆元
- 动作：m = c1^s * c2^t mod n → iroot → 观察：恢复明文 flag
- 触发点：vigor_router keyPath → 假设：命令注入 → 动作：构造 %27%0A/bin/cat${IFS}/flag.txt
- 触发点：shiro 反序列化梭 → 动作：默认密钥 → CC gadget chain → rce
- 触发点：Modbus regval_uint16 → 假设：35152 = PNG magic → 动作：tshark 提寄存器值拼 PNG
- 观察：拼出 PNG → 拿到 flag
- 触发点：FINS 协议头 46494e53 → 假设：响应构造 → 动作：按规范填响应字段
- 触发点：IEC104 ioa 字段 → 假设：信息体地址含 flag → 动作：tshark -e iec60870_asdu.ioa
- 触发点：libc.so 含 XOR 0x17 字符串 → 动作：test[i] ^ 0x17 解密 → 拿到 flag
- 触发点：智能家居 scrcpy 远控 + flipper zero 红外遥控 + vim SUID 提权 → 组合攻击链
failed_attempts:
- 试图用 Modbus fc06 写寄存器反向注入 → 失败：题目只读 fc03
- 试图对 libc.so XOR 0x17 后不解密直接 strings → 失败：strings 看的是二进制不是解出来的
- 试图直接 vim /flag →失败：vim SUID 是提权点不是直接读 flag
key_observations:
- BLE link key 在流量中的位置 = Security Manager LTK
- FINS 协议头 46494e53 是工控 CTF 标志性 magic
- Modbus 寄存器值 35152 = PNG \x89P 起点 magic
- IEC104 ioa 信息体地址字段是工控 CTF 取证核心
- libc.so 字符串 XOR 0x17 是常见字符串加密模式（基于 XOR 单一字节）
prerequisites:
- BLE / Modbus / FINS / IEC104 协议栈
- tshark 命令行多协议字段提取
- RSA 同模攻击 gcdext
- scrcpy / flipper zero 智能家居工具基础
---
