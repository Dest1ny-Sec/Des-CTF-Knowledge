...
- dompdf 远程字体
- Kryo 反序列化
- Spring RequestMappingHandlerMapping 内存马
- JNI exec
attack_chain: '|'
key_payload: '|'
...
lesson: '|'
quality: high
full_path: MRCTF2022_stuuuuub_题解.full.md
meta_path: MRCTF2022_stuuuuub_题解.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: 'MRCTF 2022 stuuuuub 题解 (Android 一代壳 + dompdf + Kryo + Spring JNDI)。MRCTF 2022 stuuuuub: Android 一代壳 (JNI_OnLoad + /readflag 算术) + dompdf CVE-2022-28368 字体 RCE + Kryo 反序列化 + Spring RequestMappingHan...'
category: web
...
contest_type: open
wp_url: https://www.ctfiot.com/95798.html
reasoning_chain:
- 触发点：Android 一代壳 + dompdf + Kryo + Spring JNDI 四阶段链 → 假设：mobile→web→反序列化→内存马→native exec 完整攻击链
- Android 一代壳（StubApp）：smali 注入 + JNI_OnLoad + start_subprocess 启动 /readflag → 动作：unicorn + androidemu 模拟 libstub.so + hook UC_HOOK_MEM_WRITE 抓 str_datas
- 假设：readnum() 算术题 (a+b) 答错就重启 → 动作：写 UnicornTraceDebugger + RegisterNatives 还原方法表
- 'dompdf RCE (CVE-2022-28368)：@font-face src: url(''http://localhost:81{php_location}'') → 动作：上传 exp.php → 拿 php_location → 拼 exp.css 含 @font-face → 拼 exp.html'
- 假设：print2pdf(html) 会让 dompdf 下载远程字体 → 动作：md5 路径计算 vendor/dompdf/dompdf/lib/fonts/{font_name}-normal_{md5}.php → 触发 PHP 执行
- Kryo 反序列化：POST /coffee/demo {polish:true,References:true,...} → 动作：反射 kryo.setReferences(true) 等 setter → kryo.register(Mocha.class)
- 动作：写 Mocha → base64 返回 → 假设：触发 Kryo 反序列化链
- Spring 内存马：反射 mappingHandlerMapping + urlLookup Map → 动作：registerMapping(info, evilClass.newInstance(), method2) 注册 /evil 路由
- 假设：内存马注入后再走 JNI exec → 动作：MSpringJNIController doExec(String cmd) → execmd() → popen + strcat → env->NewStringUTF
failed_attempts:
- 试图直接读 smali 找 JNI 方法 → 失败：smali 被 StubApp 一代壳加密，必须 hook UC_HOOK_MEM_WRITE 抓内存数据还原 libstub.so
- 试图不用字体缓存路径猜解 → 失败：必须 md5(URL) 才能算出 dompdf 缓存文件名
- 试图不调 setReferences(true) 序列化 Mocha → 失败：Kryo 默认 references=false 会把对象引用全展开，反序列化时类字段对不上
key_observations:
- CVE-2022-28368 dompdf 远程字体加载 = 上传 .php + .css（@font-face）+ .html（<link>）三件套 + md5 缓存路径计算
- Android StubApp 一代壳 = JNI_OnLoad + smali 注入 + libstub.so 加密 + unicorn/androidemu 模拟 + UC_HOOK_MEM_WRITE hook
- Kryo 反序列化的关键：setReferences(true) + register(Mocha.class) + 反射 setter 是 Mocha 攻击链标配
- Spring RequestMappingHandlerMapping 内存马 = 反射 urlLookup Map + registerMapping 注册恶意路由（无文件落地）
- JNI exec = popen + strcat 是 Java/JNI exec 后门最小化实现（env->NewStringUTF 返回 result）
prerequisites:
- Android Smali/JNI 逆向（Unicorn + androidemu 模拟 .so）
- PHP/CMS 历史 CVE 库（CVE-2022-28368 dompdf 字体 RCE）
- Java 反序列化（Kryo/Yaml/JSON 三件套 + 反射 setter 利用）
- Spring 内存马（RequestMappingHandlerMapping 反射注册）
---
