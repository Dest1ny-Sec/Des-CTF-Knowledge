...
- 1835996258校验
- alictf{}
- JNI FindClass GetMethodID CallIntMethod
- IDA F5
- ARM F5
attack_chain:
...
quality: high
full_path: Bugku_CTF安卓逆向LoopAndLoop.full.md
meta_path: Bugku_CTF安卓逆向LoopAndLoop.meta.md
images_removed: true
images_removed_count: 9
schema_version: v3.0.0-P0
summary: 'Bugku CTF 安卓逆向 LoopAndLoop。Bugku 安卓逆向 LoopAndLoop — JNI chec 反射调 check1+check2+check3 三函数累加 + check(input,99)==1835996258 + stringFromJNI2 还原 alictf{} flag。。关键路径：APK jadx 看 MainActivity: input → ch...'
category: reverse
subcategory: reverse
tools_used:
- ARM
...
contest_type: open
wp_url: https://www.ctfiot.com/167027.html
reasoning_chain:
- '[触发点] APK MainActivity: check(input, 99) == 1835996258 → 假设：check 函数是关键 / [动作] jadx 看 MainActivity + IDA 看 so chec / [观察] chec 是 JNI native 函数 / [下一步] 看 chec 反射调用'
- '[触发点] chec 用 FindClass("net/bluelotus/.../MainActivity") 反射拿 check1/check2/check3 methodID → 假设：JNI 反弹调 Java 函数 / [动作] CallIntMethod 调 (a2, v10[2*s%3], t, s-1) / [观察] 2*s%3 索引选函数 / [下一步] 分析 check1/2/3'
- '[触发点] check1: t += 1..99 / check2: s%2==0 → t += 1..999; else t -= 1..999 / check3: t += 1..9999 → 假设：循环递归累加 / [动作] 写 Python 模拟循环 / [观察] s=99 → v10[0] = check1; s=98 → check2; s=97 → check3; s=96 → check1 / [下一步] 算 t'
- '[触发点] 循环直到 s=1 返回 t → 假设：t 必须等于 1835996258 / [动作] 倒推还原 input / [观察] 拿到 input → stringFromJNI2(in_int) → alictf{} / [下一步]'
failed_attempts:
- 试图静态分析 so 拿 t → 失败：t 是动态递归累加
- 试图直接猜 input → 失败：必须从 t 反推
key_observations:
- JNI 反弹调 Java 函数是 Android 逆向难点
- 2*s%3 索引巧妙选函数
- stringFromJNI2(in_int) 是 flag 生成函数
- check1/2/3 累加算法差异点（s%2 是否翻转）必须区分
prerequisites:
- JNI 反射调用（FindClass / GetMethodID / CallIntMethod）
- APK 反编译（jadx）
- IDA ARM 反汇编
- Android Java/native 互操作
---
