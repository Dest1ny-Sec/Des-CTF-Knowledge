...
quality: high
full_path: DefCon_REST-AND-ATTEST.full.md
meta_path: DefCon_REST-AND-ATTEST.meta.md
images_removed: true
images_removed_count: 4
schema_version: v3.0.0-P0
summary: 'DefCon REST-AND-ATTEST。DEFCON 31 REST-AND-ATTEST：智能锁固件证明Rust程序。关键路径：socat tcp4-listen:4444 fork exec:./wrapper.sh → wrapper.sh exec ./uploader → 菜单: upload/download/run/quit。经验：固件证明（attestation）基于R...'
category: web
...
contest_type: open
wp_url: https://www.ctfiot.com/118801.html
reasoning_chain:
- DEFCON 31 REST-AND-ATTEST → 触发点：智能锁固件证明 Rust 程序
- socat tcp4-listen:4444 fork exec:./wrapper.sh → wrapper.sh exec ./uploader → 触发点：本地服务
- 菜单 upload/download/run/quit → 假设：上传 image 替换 trusted_firmware.raw
- 观察：include_bytes!("trusted_firmware.raw").to_vec() 读二进制 → 假设：upload 覆盖 image
- 假设：run_device() 启动 Secure Firmware Module 验证 image → 动作：构造伪造 image 通过 SFM 证明
- 假设：image 头包含 launcher 可识别的 metadata → 动作：观察 launcher 接口
- 安装 seccomp 过滤白名单 syscalls → 假设：避免危险 syscall 阻断漏洞利用
failed_attempts:
- 试图直接上传恶意 image → 失败：SFM 固件证明校验签名/哈希
- 试图绕过 seccomp → 失败：白名单硬限制
key_observations:
- socat fork + wrapper.sh 是 CTF 服务暴露的常见入口
- include_bytes! 宏把外部固件嵌入 Rust 二进制
- 固件证明（attestation）= Root of Trust 签名链
- seccomp 白名单是固件运行的硬约束
prerequisites:
- Rust include_bytes! 宏
- socat tcp4-listen fork 用法
- Secure Firmware Module (SFM) 证明流程
- seccomp 系统调用过滤
---
