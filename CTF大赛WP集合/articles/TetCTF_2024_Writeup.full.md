...
quality: high
full_path: TetCTF_2024_Writeup.full.md
meta_path: TetCTF_2024_Writeup.meta.md
images_removed: true
images_removed_count: 0
schema_version: v3.0.0-P0
summary: 'TetCTF 2024 Writeup。TetCTF 2024 WebAssembly + check_key：WASM 字节码 Z3 求解 + FNV1a + xorshift128 矩阵求逆。。关键路径：baby-asm: WASM 二进制 + WebAssembly.Module 加载 → processInput 检查 text.length === 27 + 起始 TetCTF{ ...'
category: reverse
subcategory: reverse
tools_used:
- Python
...
contest_type: open
wp_url: https://www.ctfiot.com/160243.html
reasoning_chain:
- baby-asm 拿到 wasm 二进制 → 触发点：WebAssembly.Module 加载 + processInput 检查 text.length === 27
- 假设：wasm.init(20) 创 20 元素数组 + wasm.array_fill(arr, i, text.charCodeAt(i+7)) 写入 c+83
- 动作：wasm.check() 验证 20 个约束 → 每 4 字符一组 (add/sub/mul/xor) + 4 个 global 常量
- 观察：globals = [115,82,52,149,...] 已知 → 假设：用 z3 BitVec 求解 20 个 flag[i+7] 字符
- check_key 题：0x54 长度字符串 + 白名单 21 字符 → 假设：每 4 字符一组 (l << 2) hash_64_fnv1a
- xorshift128(x=0x75bcd15, y=0x159a55e5, z=0x1f123bb5, w=0xdeadbeef) → mem2[a] += mem1[b] * (xorshift128() % 0x400)
- 动作：枚举 21^4 哈希表 + xorshift128 矩阵 + np.linalg.solve 解 21 元一次方程
failed_attempts:
- 试图手算 20 个 BitVec 约束 → 失败：变量太多需 z3 自动求解
- 试图 BF 爆破 xorshift128 seed → 失败：4 个 32-bit 寄存器空间太大
key_observations:
- WASM 反编译为 WAT 后用 z3 BitVec 求解简单运算链（add/sub/mul/xor）
- xorshift128 矩阵求逆：用 np.linalg.solve 解 21 元一次方程
- fnv1a 哈希碰撞可枚举（21^4 较小）
prerequisites:
- WebAssembly 反编译为 WAT
- z3 BitVec 约束求解
- numpy.linalg.solve 线性方程组
---
# TetCTF 2024 Writeup
