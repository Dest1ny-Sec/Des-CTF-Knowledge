...
quality: medium
full_path: SUSTF-WriteUp.full.md
meta_path: SUSTF-WriteUp.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: SUSTF WriteUp - ChaMd5 Venom。SUSCTF 招新题库，ChaMd5 Venom IOT/工控组，涵盖 LFSR/Browser/Web 三类。。关键路径：lfsr(seed2, mask2, n2) 枚举 (2^12, 2^12) 找与 plain "Date" XOR 匹配 → 已知 plain 头 16 字节构造方程组 → gauss 消元解 mask1。...
category: misc
...
contest_type: open
wp_url: https://www.ctfiot.com/28093.html
reasoning_chain:
- magic_box 文件名 base64 时间戳 → 触发点：解密顺序敏感
- 第一文件密文头 4 字节 + 已知明文 'Date' → 假设：LFSR (seed2, mask2) 枚举
- 动作：lfsr(seed2, mask2, n2=12) 遍历 (2^12, 2^12) → 观察：匹配 'Date'
- 第二文件已知明文 8 字节 → 假设：推断 seed3 范围
- 假设：mask1 可由方程组解 → 动作：gauss 消元 64x64 矩阵 → 观察：解出 mask1
- XOR 还原明文 → SUSCTF flag
- 'browser 0day: DataView.setUint32 prototype 注入 + 三实例 OOB 触发任意读写'
failed_attempts:
- 试图直接遍历 LFSR mask 全空间 → 失败：2^64 太大
- 试图不解密直接看文件字符串 → 失败：全是密文
key_observations:
- LFSR 已知明文 + 短长度即可用高斯消元解掩码 (与 0CTF zer0IIvm 思路同源)
- 浏览器 DataView prototype 注入 + ArrayBuffer 重叠构造 OOB 读写是新型 0day 利用
- fxxkcors 报告页 admin bot + changeapi.php 提权是 web cors/csrf 经典链
- heap menu 整数溢出 add(-0xa0) 是 unsortedbin 残余利用入口
prerequisites:
- LFSR 原理与高斯消元
- JavaScript ArrayBuffer/DataView prototype 注入
- Pwn heap 基础 (tcache/unsortedbin)
- Web 报告页 admin 触发机制
---
# SUSTF-WriteUp
