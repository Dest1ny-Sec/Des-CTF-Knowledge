...
importsysimportmathdata= list(map(int, sys.stdin.read().split()))ptr=0T= data[ptr]ptr+=1for_ in range(T): n= data[ptr] q= data[ptr +1] ptr+=2 a= data[ptr:ptr + n] ptr+= n prefix=[0] * (n + 1) for i in range(n): x = a[i] ifx %2==0: prefix[i+1] = prefix[i] +1 else: s= math.isqrt(x) ifs %2==1and s * s == x: prefix[i+1] = prefix[i] +1 else: prefix[i+1] = prefix[i] for__ in range(q): l= data[ptr] r= data[ptr +1] ptr+=2 print(prefix[r] - prefix[l-1])
