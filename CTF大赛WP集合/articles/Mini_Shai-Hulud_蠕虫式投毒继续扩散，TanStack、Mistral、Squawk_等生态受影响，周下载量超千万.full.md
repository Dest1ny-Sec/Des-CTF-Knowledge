...
lesson: '|'
quality: medium
full_path: Mini_Shai-Hulud_蠕虫式投毒继续扩散，TanStack、Mistral、Squawk_等生态受影响，周下载量超千万.full.md
meta_path: Mini_Shai-Hulud_蠕虫式投毒继续扩散，TanStack、Mistral、Squawk_等生态受影响，周下载量超千万.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: 'Mini Shai-Hulud 蠕虫式投毒事件分析 (TanStack/Mistral/Squawk/OpenSearch/Guardrails AI/UiPath)。2026-05 Mini Shai-Hulud 蠕虫式供应链投毒事件分析: TanStack + Mistral + Squawk + OpenSearch + Guardrails AI + UiPath 多生态被投毒。。经验：|'
category: misc
...
contest_type: open
wp_url: https://www.ctfiot.com/307839.html
reasoning_chain:
- 触发点：2026-05-12 凌晨 3 点墨菲安全检测到 TanStack 42 个 @tanstack/* npm 包被投毒 → 假设：GitHub Actions cache poisoning 攻击链
- 受害范围：42 @tanstack/* + 84 版本 + Mistral AI SDK + Squawk + OpenSearch + Guardrails AI + UiPath → 假设：pull_request_target 漏洞被多个组织复用
- 动作：分析 pull_request_target 工作流 → 观察：fork PR 上执行了不可信代码 → 假设：攻击者通过 cache poisoning 把恶意内容写进发布流程
- 假设：发布流程有写权限，恶意代码从 runner 进程内存提取 OIDC token → 动作：分析 runner 内存 dump → 观察：OIDC token 被盗
- '注入点 1（package.json）：''optionalDependencies'': {''@tanstack/setup'': ''github:tanstack/router#<commit>''} → 假设：第三方 github: 协议链接触发 fetch'
- 注入点 2（@tanstack/setup 生命周期脚本）：下载并执行远程 Python artifact → 假设：postinstall 脚本是攻击入口
- 防御建议：限制 pull_request_target 触发条件 + 限制发布流程权限（仅 main branch push）+ 检查 GitHub Actions cache 内容
- ATT&CK 映射：T1195.002 供应链投毒 + T1552.001 凭据窃取 + T1041 C2 外传
failed_attempts:
- 试图按 namespace 粗暴判断可信包 → 失败：npm 投毒专门选大 namespace 包伪装，必须看 commit + 锁文件
- 试图通过 npm 公告发现投毒 → 失败：恶意 commit 不在公告里，要看 commit hash 是否在白名单
- 试图在 PR 阶段拦截 → 失败：攻击者用合法 release workflow + cache poisoning，PR 阶段根本看不出来
key_observations:
- pull_request_target 在 fork PR 上执行不可信代码 = GitHub Actions 供应链攻击最大入口
- GitHub Actions cache poisoning = 恶意代码写入主分支发布流程恢复的缓存（缓存完整性未校验）
- OIDC token 从 runner 内存提取 + 合法 publish 链路 = npm registry 无法识别攻击
- github:tanstack/router#<commit> 链接 + postinstall 生命周期脚本 = npm 投毒双触发
- ATT&CK T1195.002（供应链投毒）+ T1552.001（凭据）+ T1041（C2）是 npm 投毒事件三件套
prerequisites:
- GitHub Actions 工作流机制（pull_request_target / OIDC token / cache）
- npm 依赖图与生命周期脚本（postinstall / optionalDependencies）
- 供应链投毒分析（ATT&CK T1195.002 框架）
- GitHub OIDC 身份验证与 token 提取
---
