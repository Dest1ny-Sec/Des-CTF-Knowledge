...
quality: high
full_path: STACK_the_Flags_CTF_2022.full.md
meta_path: STACK_the_Flags_CTF_2022.meta.md
images_removed: true
images_removed_count: 0
schema_version: v3.0.0-P0
summary: STACK the Flags CTF 2022 - cursed_grimoires。STACK the Flags 2022 cursed_grimoires：菜单 1/2/3 二进制 + 1MB 大块跨 mmap 边界 + 单字节 edit 改 stdout。。关键路径：选 Create Grimoire 一次，edit 单字节写 → menu() 调 puts 触发 stdout w...
category: pwn
...
contest_type: open
wp_url: https://www.ctfiot.com/84747.html
reasoning_chain:
- 题目 checksec Full RELRO + Canary + NX + PIE 全开 → 触发：FSOP via _IO_wfile_jumps
- 菜单 Create Grimoire 一次 + edit 单字节写 → 假设：可改 stdout 任意字节
- malloc 1MB 大块跨过 mmap threshold → mmap 区域位于 libc_base-0xf7ff0
- edit 改 stdout->_flags = 0x1800 (NO_WRITES 清除 + APPEND/CURRENTLY)
- edit 改 stdout->_IO_write_ptr 末字节为 0x50 → 调 menu 触发 puts
- 泄 _IO_stdfile_1_lock 算 libc_base → chunk_addr = libc_base - 0xf7ff0
- 改 stdout->vtable 至 _IO_wfile_jumps → 改 _wide_data->_wide_vtable 伪造 _IO_wfile_overflow
- ret2libc → system → 弹 shell
failed_attempts:
- 试图走 free_hook → 失败：glibc 2.35 已移除
- 试图覆盖栈返回地址 → 失败：canary 阻挡
- 试图用 fastbin/tcache → 失败：Full RELRO 限制
key_observations:
- Full RELRO + Canary + NX + PIE 全开下，FSOP via _IO_wfile_jumps 仍可作为终极利用面
- 1MB mmap 跨 threshold 是定位 libc 周围 chunk 的常用技巧
- _IO_write_ptr LSB 单字节改可控制 stdout 输出范围
- _wide_data->_wide_vtable 是 vtable 检查绕过的高阶 sink
prerequisites:
- Glibc 2.35 FILE 结构 + FSOP 原理
- _IO_wfile_jumps + _wide_vtable 绕过
- mmap + libc 周围地址布局
- pwntools + checksec + cyclic 工具链
---
