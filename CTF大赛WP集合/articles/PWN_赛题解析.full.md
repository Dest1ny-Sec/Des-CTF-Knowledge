...
quality: high
full_path: PWN_赛题解析.full.md
meta_path: PWN_赛题解析.meta.md
images_removed: true
images_removed_count: 9
schema_version: v3.0.0-P0
summary: PWN 赛题解析。4 道 PWN 题合集，涵盖 ret2libc 泄地址循环填充、seccomp 下 openat+mmap+writev orw、fmt-string 泄三件套栈迁移、自定义 VM 解释器 PC 劫持。。关键路径：一 stdout 题：read 0x60 越界写覆盖 rbp/rip → 跳 vuln 函数循环 20 次填满 stdout 缓冲区 → 泄 read@got 计...
category: pwn
subcategory: rop
...
contest_type: open
wp_url: https://www.ctfiot.com/196469.html
reasoning_chain:
- 触发点:四道 PWN 合集,第一题 stdout:read(0, buf, 0x60) 但 buf 只有 0x50 → 假设:可越界覆盖 rbp
- 动作:payload='a'*0x58+p64(vuln) → 循环 20 次跳 vuln 把 stdout 缓冲区填满 → 观察:puts 输出未缓冲的 read@got 地址
- 泄 read@got 算 libc 偏移 → pop_rdi+rsi+rdx+execve → getshell
- 第二题 Shuffled_Execution:seccomp 禁 execve 系 → 假设:必须走 openat+mmap+writev orw
- 动作:写入 orw shellcode(openat+mmap+writev 链) → 触发 → 观察:读 flag 写入 stdout
- 第三题 SavethePrincess:rand 派生密钥 + 偏序密码爆破 → 假设:泄 canary/libc/stack 三件套
- 动作:栈迁移到 mprotect rwx 区域执行 shellcode → getshell
- 第四题 spiiill:自定义 VM 字节码解释器 → 假设:PC 越界控制跳转
- 动作:payload p64(0xa)+p64(0xc)+p64(0xfffffffffffffc02)+'sh\x00' → system 执行 sh
failed_attempts:
- 题 1 试图直接 system('/bin/sh') → 失败:未填满 stdout 缓冲区,puts 不输出
- 题 2 试图 execve('/bin/sh') → 失败:seccomp 禁 execve
- 题 4 试图直接覆盖 opcode → 失败:VM 有 PC 边界校验,需构造合法三元组
key_observations:
- stdout 关闭时,必须循环调用 puts 触发输出(缓冲区填满才会 flush)
- seccomp 禁 execve 时 orw(openat+mmap+writev)是固定套路
- VM 字节码解释器的 PC 越界是 PWN 通用攻击面
prerequisites:
- ret2libc + GOT 泄露 + pop 链
- seccomp 规则与 prctl/seccomp 系统调用
- 栈迁移(stack pivot)与 mprotect 构造 rwx
---
# PWN 赛题解析
