...
quality: medium
full_path: SHCTF-2024-Week2_官方WP.full.md
meta_path: SHCTF-2024-Week2_官方WP.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: SHCTF-2024-Week2 官方WP。看雪 ctfiot 招新 Week2 完整 WP，涵盖 Crypto/JWT/Pickle/Flask/Pwn/Reverse 多类基础题。。关键路径：从两端向中间搜索 pxorq 重建 p/q 二进制 → QRazyBox 修复受损二维码 + Reed-Solomon 解码 → 自定义 base64 字母表 (ABCDjp0yIJKL...234...
category: web
...
contest_type: open
wp_url: https://www.ctfiot.com/210419.html
reasoning_chain:
- 'Crypto worde 很大: dp 泄露 + e 较大 → 触发：从两端向中间搜索 pxorq 重建 p/q 二进制'
- QRazyBox 修复受损二维码 → Reed-Solomon 解码 → 自定义 base64 字母表解密
- JWT HS256 6 位弱口令爆破 → jwt-cracker 工具 → secret=222333 → payload role:user→admin
- Python pickle find_class() 黑名单 → posix.system 绕过（带前缀模块）→ RCE
- random.seed(1000000-9999999) 爆破 → Mersenne Twister 预测 next → flag
- MD5 长度扩展攻击 + SECRET_KEY=Th1s_is_5ecr3t_k3y → 伪造 session
- flask-session-cookie-manager → is_admin:1 → flag
- 'SQL 注入: -1") union select 1,scretdata from ctf.flag → flag'
- Pwn seccomp 59 号 + open/read/write 链式 sys_open → 0xBB 字节溢出一字节改 ret_addr
- picoGadget 泄露 libc → ret2libc → 0x100 字节异或 shellcode
failed_attempts:
- Crypto worde 试图遍历 e 找 k → 失败：e 太大无法穷举
- JWT 试图穷举 8+ 位密码 → 失败：6 位以内才能爆破
- Pickle 试图 os.system → 失败：被黑名单拦截，需 posix.system 绕过
key_observations:
- pickle 显式 import 黑名单时 posix.system 这类「带前缀的等价模块」常被遗忘
- dp 泄露 + 大 e 不能简单遍历，需用 pxorq 从两端向中间搜索
- JWT 弱口令爆破（6 位以内）是 HS256 常见攻击
- flask-session-cookie-manager 是伪造 Flask session 标准工具
prerequisites:
- RSA dp 泄露 + pxorq 二进制搜索
- JWT HS256 + jwt-cracker 工具
- Python pickle + find_class 黑名单绕过
- SQL 注入 + union select 联合查询
---
