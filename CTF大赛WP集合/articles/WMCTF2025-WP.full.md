...
quality: high
full_path: WMCTF2025-WP.full.md
meta_path: WMCTF2025-WP.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: WMCTF 2025 WP (XMCVE-Polaris, 星盟安全团队)。WMCTF 2025 星盟安全团队 XMCVE-Polaris 第 6 名杂烩 WP，BPF 内核利用 + LFSR 子集和 + pdfminer pickle RCE + Flask MT19937 状态恢复，多题型覆盖。。经验：gzip 双成员 polyglot 是 PDF+pickle 联动 RCE 的经典技巧...
category: web
...
contest_type: open
wp_url: https://www.ctfiot.com/271862.html
reasoning_chain:
- BPF 触发点：write_cmd 0x159*2=0x2b2 越界淹下一个 bpf_array 前 2 字节 → 假设：bpf_array map 越界改 map_free
- 动作：map_lookup_elem=map_free → 触发点：堆喷 array_of_map → read_cmd 泄 ops 前 4 字节 → 假设：拿到内核基地址
- 假设：劫持 index_mask/max_entries/value_size 越界改 task_struct 提权 → kernel ret2usr
- LFSR 触发点：32 元子集和 BKZ-22 + split_master 重组 6 段 512 bit → 32 维 LLL + 10 维格爆破
- pdfminer：app.py → pdf_to_text → extract_pages → PDFCIDFont → CMapDB._load_data → pickle.loads()
- 动作：pickle.dumps(Exploit()) + gzip 双成员 polyglot（第一个成员 pickle + 第二个成员 PDF 头）→ 触发 RCE
- Flask MT19937：app.secret_key = generate_random_string() → rd.getrandbits(32) → 收集 624 个 32-bit 输出 → MT19937Predictor.setrandbits(val, 32) → 恢复内部状态
- 假设：拿到 secret_key → 预测 key2 → 通过 key1 == key2 校验
- 动作：split_master(B_decimal, segment_bits) 重组 6 段 bit → BKZ-22 解 32 维格
- 观察：完整 BPF kernel + LFSR 子集和 + pdfminer pickle + MT19937 多解
failed_attempts:
- BPF 试图直接改 map_free 函数指针 → 失败：必须先越界改 array_of_map 拿到 ops
- LFSR 试图线性代数解 → 失败：必须先用 BKZ-22 + split_master 重组 bit
- Flask MT19937 试图爆破 secret_key → 失败：必须收集 624 个 32-bit 输出恢复状态
key_observations:
- gzip 双成员 polyglot 是 PDF+pickle 联动 RCE 的经典技巧
- MT19937 收集 624 个 32-bit 输出即可恢复内部状态
- BPF map 越界 + array_of_map 堆喷是 Linux kernel bpf 子系统提权路径
- split_master 重组 bit 流是 LFSR 多段密钥常用解法
- pdfminer CMapDB._load_data pickle.loads() 是 Python PDF 库典型 RCE 入口
prerequisites:
- Linux BPF 子系统内核漏洞利用
- MT19937 状态恢复（624 输出定理）
- 格密码 BKZ 算法 + Sage split_master
- pdfminer 源码阅读（_load_data 入口）
- gzip 文件格式多成员结构
---
# WMCTF2025-WP
