...
quality: high
full_path: PWN｜西湖论剑·2022中国杭州网络安全技能大赛初赛官方Write_Up.full.md
meta_path: PWN｜西湖论剑·2022中国杭州网络安全技能大赛初赛官方Write_Up.meta.md
images_removed: true
images_removed_count: 9
schema_version: v3.0.0-P0
summary: PWN - 西湖论剑 2022 中国杭州网络安全技能大赛初赛官方 Write Up。西湖论剑 2022 三道 PWN 官方 WP：babycalc 单字节溢出+栈迁移、jit 自研 IR 编译器整数溢出、Message Board seccomp 沙箱下 orw+栈迁移双跳。。关键路径：一 babycalc：read 末尾置 0 单字节溢出覆盖 rbp LSB → 任意偏移写 1 字节改 r...
category: pwn
subcategory: rop
tools_used:
- Z3
...
contest_type: open
wp_url: https://www.ctfiot.com/97886.html
reasoning_chain:
- 触发点:三道题合集,babycalc 有 read 末尾置 0 单字节溢出 → 假设:可改 rbp LSB
- 任意偏移写 1 字节改 retaddr 为 leave;ret → 触发栈迁移 → 假设:可控数据区布置 ret*21 + pop_rdi+got+puts+start
- 动作:泄 libc → 计算 one_gadget 0xf1247 → 二次栈迁移 → 观察:getshell
- 中间算术方程逆向:z3 BitVec 16 字节 ×16 方程 → 解 0x13 0x24 0x35 ... 0x03
- 题 2 jit:自研 IR(mov/and/or/xor/call) → mmap 编译为机器码 → 假设:审计 var2idx 边界
- var2idx 整数溢出:variba=32 时 -8*32=0 绕过 fatal → 假设:可用 jop 链写 shellcode
- movabs rsi 写 6 字节 shellcode(push /sh; pop rax; shl 32; push /bin; or; syscall 0x3b) → 改 retaddr 后 12 位跳
- 题 3 Message Board:seccomp 禁 execve 59 → fmt-string 泄 libc + 栈溢出 0x10 控 rbp+ret
- 跳 mov [rbp] rsi gadget 任意地址写 0xC0 → 二次 leave;ret 栈迁移到 bss → open+read+puts orw 链
failed_attempts:
- 题 1 试图直接栈溢出改 retaddr → 失败:read 只溢出 1 字节 rbp,需 leave;ret 接力
- 题 2 试图 ret2libc → 失败:JIT 自定义 IR,需要审计编译产物
- 题 3 试图 system('/bin/sh') → 失败:seccomp 禁 execve
key_observations:
- read 末尾 NUL 字节可单字节溢出改 rbp;leave;ret 是经典链
- 自研 JIT 必须审计 var2idx 等边界检查(整数溢出)
- seccomp 禁 execve 时 orw(openat+read+puts)是必走路径
prerequisites:
- 栈迁移 + leave;ret gadget
- z3 求解 BitVec 方程
- seccomp 沙箱绕过(orw 链)
---
