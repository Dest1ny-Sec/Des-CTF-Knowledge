...
quality: high
full_path: DASCTF｜2022DASCTF_May出题人挑战赛官方Write.full.md
meta_path: DASCTF｜2022DASCTF_May出题人挑战赛官方Write.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: 'DASCTF 2022 5 月出题人挑战赛官方 Write。DASCTF 2022 5 月出题人挑战赛官方 WP，6 Web + 6 Misc + 3 Crypto + 2 Pwn。。关键路径：hackme: 上传 users.go 白名单绕过 → getme: CVE-2021-42013 目录穿越读 access_log → fxxkgo: golang jwt {{.}} 模板注入。经...'
category: web
...
contest_type: open
wp_url: https://www.ctfiot.com/42240.html
reasoning_chain:
- hackme：看到 users.go 不能解析 → 上传目录白名单 → 假设：必须上传同名文件 → 动作：上传 users.go（含 exec.Command cat /flag）
- getme：Apache 2.4.50 → CVE-2021-42013 目录穿越 → 假设：构造 .%%32%65/.%%32%65 双重 URL 编码 .
- 动作：http://127.0.0.1:11777/icons/.%%32%65/.%%32%65/.../logs/access_log → 读 access_log
- 假设：日志中找真实 flag 请求路径 → 动作：grep 'fffffflalllallalagggggggggg' access_log → 找到路径
- fxxkgo：golang web 框架 SSTI → jwt {{.}} 模板注入 → 动作：构造 jwt 含 {{.}} payload
- 假设：golang 模板 {{.}} 直接渲染所有字段 → 注入 {{.}} → 拿到 key/secret
- 魔法浏览器：UA 修改 + JS 反混淆 → Power Cookie：cookie=admin=1 → 假设：浏览器改 cookie 直接过
- ezcms：Mc_Encryption_Key 解密 update.php → 假设：源码泄露 + RC4 解密
- delflag：docker save + layer extract → 假设：镜像层含 flag 文件
- Yusa MT19937 线性攻击 → 假设：MT19937 输出序列可反推 seed
- off-by-one 改 size 0x481 → house of kiwi 劫持 → 假设：house of kiwi 是新型 glibc 利用
failed_attempts:
- 试图上传任意 go 文件 → 失败：users.go 是白名单
- 试图 CVE-2021-42013 一次 .%2e → 失败：Apache 2.4.50 已修，需双重 URL 编码 %%32%65
- 试图不解密读 update.php → 失败：被 Mc_Encryption_Key 加密
key_observations:
- golang web 框架 SSTI 比 Python 难利用，但模板渲染时同样注入 . 用 {{.}} 即拿到 key
- CVE-2021-42013 修复后必须双重 URL 编码（%%32%65）才能绕
- docker save + layer extract 是 CTF 容器逃逸/取证标准手法
- MT19937 输出 624 个 32-bit 数后用线性代数反推 state 即可还原 seed
- house of kiwi 是 glibc 2.35+ 新型利用（用 _IO_FILE 结构劫持 exit handlers）
- Apache 日志目录穿越 + 长 URL flag 路径爆破是 web/misc 混合题
prerequisites:
- Golang web SSTI（template 渲染 + {{.}}）
- Apache CVE-2021-42013 目录穿越
- JWT 注入 + 加密 cookie 解密
- MT19937 线性代数攻击（z3/sage）
- House of Kiwi 利用（glibc 2.35+）
- Docker 镜像层分析（docker save + tar extract）
---
