...
quality: high
full_path: 看雪2023_KCTF年度赛_-_第五题·争分夺秒-设计思路及解析.full.md
meta_path: 看雪2023_KCTF年度赛_-_第五题·争分夺秒-设计思路及解析.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: 看雪2023 KCTF年度赛 | 第五题·争分夺秒-设计思路及解析。KCTF2023年度赛Q5《争分夺秒》Windows CrackMe：模式识别nop混淆+逆元模逆+双幻方对称加密。经验：计算混淆代码用模式识别特征+静态nop；逆元小整数方程a*x mod n=1直接枚举
category: reverse
subcategory: reverse
...
contest_type: open
wp_url: https://www.ctfiot.com/135393.html
reasoning_chain:
- Windows CrackMe大量计算混淆代码 → 触发点：混淆干扰逆向算法
- 假设：混淆有特征可模式识别 → 动作：先patch inside_dbg()0x402220调试器检测
- 观察：inside_dbg不patch则*Block+=rand()%3+1暗坑 → 下一步：模式识别混淆
- 计算混淆nop后看到核心算法 → 假设：Base64+CRC32+幻方对称加密结构
- Key格式=4+2+key1+4+2+key2+4校验和 → 动作：识别结构再分段
- a*x%n=1小整数方程 → 假设：模逆元多解 → 动作：0x7D45*x%0x346F8717=1枚举
- 观察：A=4解(0x3153622a/0x65c2e941/0x9a327058/0xcea1f76f) → 下一步：构造两个幻方key
- 幻方key1/key2异或对称加密 → 逆元验证key1/key2唯一性 → 通过
failed_attempts:
- 试图不patch inside_dbg直接调试 → 失败：rand()%3+1会让Block字段错乱
- 试图直接遍历幻方对称加密逆运算 → 失败：key1/key2长度不定
- 用sage解a*x%n=1 → 失败：小整数方程sage太重，直接枚举更快
key_observations:
- 计算混淆代码有模式可nop（特征码对比）
- 模逆元 a*x mod n=1 在小整数范围可枚举（多解）
- 幻方对称加密 = 用魔方矩阵生成密钥流做对称加密
- key格式化为 幻方key+len+data+幻方key+len+data+校验和 是序列化套路
- 动静结合：日志断点定位核心算法+静态nop清理
prerequisites:
- x86汇编混淆模式识别
- 模逆元数学基础（扩展欧几里得）
- 幻方构造算法
