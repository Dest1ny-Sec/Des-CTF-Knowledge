...
year: 2025
difficulty: medium
vuln_type: ai_security
tags:
- text2sql
- nl2sql
- llm_sql_generation
- vector_recall_field_simulation
- multi_level_recall
- agent_think_act_loop
- prompt_engineering
- few_shot_example
- error_recovery_loop
- replan_loop_breaker
...
quality: high
full_path: Text2SQL_比赛实战心得：从难点到解决方案.full.md
meta_path: Text2SQL_比赛实战心得：从难点到解决方案.meta.md
images_removed: true
images_removed_count: 4
schema_version: v3.0.0-P0
summary: Text2SQL 比赛实战心得：从难点到解决方案。Text2SQL 比赛实战心得，多级召回（DB→表→字段）+ 向量检索模拟问题 + 上下文重写 + Agent ReAct 循环 + 复读机熔断，覆盖自然语言到 SQL 的全流程难点。。经验：Text2SQL 的核心难点不是 SQL 本身，而是 schema 检索 + 上下文补全 + 多步推理；向量检索字段模拟...
category: misc
...
contest_type: open
wp_url: https://www.ctfiot.com/240361.html
reasoning_chain:
- 数据库表数 100+ → 触发点：LLM Token 装不下全部 schema → 假设：必须做 schema 召回
- 动作：多级召回 LLM DB 选库 → LLM 选表 → LLM 选字段 → 观察：但传统 LLM 召回粒度差
- 假设：用 LLM 给每个字段生成 '模拟问题' 做向量检索 → 动作：用户问题 → 向量化 → 字段向量 top-k
- 观察：向量检索字段比多级 LLM 召回更精准 → 下一步：表级聚合 + 两层筛选 Top N 表 Top M 字段
- Agent Think/Write SQL/Execute/Think Again ReAct 循环 → 触发点：单次生成 SQL 易出错 → 假设：多轮迭代带反馈
- 动作：完整 SQL 执行错误反馈 + 最大迭代次数熔断 + 复读机检测
failed_attempts:
- 试图一次 LLM 生成完整 SQL → 失败：缺少上下文 + 字段歧义大
- 试图多级 LLM 全程召回 → 失败：Token 爆炸且精度不如向量检索
key_observations:
- schema 检索 + 上下文补全是 Text2SQL 核心难点，不是 SQL 生成本身
- 向量检索字段模拟问题比多级 LLM 召回更精准且省 Token
- 复读机熔断 + 最大迭代次数是 Agent 循环必备保护
prerequisites:
- LLM Prompt Engineering + Few-shot 示例设计
- 向量数据库召回（embedding + cosine similarity）
- Agent ReAct 循环与错误反馈机制
---
