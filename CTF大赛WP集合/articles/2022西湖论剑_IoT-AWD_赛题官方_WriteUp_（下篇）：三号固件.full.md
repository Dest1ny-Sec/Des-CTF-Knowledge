...
- ENV-var-bypass
attack_chain:
- 'Q1 root shell: dumpimg 解包 itb, 看 dts, uartlite 状态 ''disabled'' → 改 ''okay'' → mkimage 重打包 → 串口 root'
- uartlite 0x1e000c00 变 0x1e000d00, reg-shift 2, reg-io-width 4
- Q2 默认密码重置
- 'Q3 dsd: 逆向发现 ubus_connect, 走 OpenWrt ubus 协议'
- 'Q4-Q10 各题: OpenWrt 漏洞 + 嵌入式 web + cgi-bin'
- 哈希长度扩展拿 token, ${PATH:offset:length} 绕大小写转换
...
quality: high
full_path: 2022西湖论剑_IoT-AWD_赛题官方_WriteUp_（下篇）：三号固件.full.md
meta_path: 2022西湖论剑_IoT-AWD_赛题官方_WriteUp_（下篇）：三号固件.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: '2022 西湖论剑 IoT-AWD 赛题官方 WriteUp (下篇：三号固件)。西湖论剑 IoT-AWD 3 号固件：dts uartlite 启停 + ubus 协议 + OpenWrt。关键路径：Q1 root shell: dumpimg 解包 itb, 看 dts, uartlite 状态 ''disabled'' → 改 ''okay'' → mkimage 重打包 → 串口 root ...'
category: pwn
...
contest_type: open
wp_url: https://www.ctfiot.com/105793.html
reasoning_chain:
- 三号固件启动参数 console=ttyS0 → 触发点：UART 串口地址从 0x1e000c00 变 0x1e000d00
- 假设：uartlite 被禁用 → 动作：dumpimg 解包 itb → 看设备树 dts
- 观察：uartlite@c00 status='disabled'，uartlite2@d00 status='okay' → 假设：改 dts 重新打包
- 动作：删除 uartlite disabled 行 + mkimage 重新打包 itb + 刷入设备 → 观察：拿到 root shell
- 'ubus 协议: OpenWrt 通用的 RPC 机制 → 假设：ubus call 接口可触发命令'
- 动作：分析 ubus object + call nc_device XXXXX → 观察：命令执行
failed_attempts:
- 试图用默认密码登录 → 失败：默认密码已被修改
- 试图直接读写 itb 文件 → 失败：itb 是打包格式需要 dumpimg
- 试图用 UART1 (0xc00) → 失败：状态 disabled 无输出
key_observations:
- dts 是嵌入式硬件配置（设备树）修改 status='okay' 即启用外设
- mkimage 重新打包 itb 是嵌入式刷机流程
- OpenWrt ubus 是 IoT 通用 RPC 协议
- UART 串口地址变化是版本差异标志
prerequisites:
- 设备树 dts 语法（status / reg / compatible）
- dumpimg 解包 + mkimage 重打包工具
- OpenWrt ubus 协议
- 嵌入式 UART 启动参数 console= 设置
---
