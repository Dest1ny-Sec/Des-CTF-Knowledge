...
quality: high
full_path: 解析n1ctf2025_(n1)³.full.md
meta_path: 解析n1ctf2025_(n1)³.meta.md
images_removed: true
images_removed_count: 0
schema_version: v3.0.0-P0
summary: n1ctf 2025 Crypto (n1)³ 张量分解攻击。n1ctf 2025 Crypto 高难度：把 35 维立方加密视为对称张量 f(x) = Σ w_k (v_k^T x)³，用 Jennrich 算法做 CP 分解恢复明文。。关键路径：enc(x) = (xU)^{[3]} · T，35 维 FLAG 用 .digits(257) 编码为 35 个 0-256 数字 → 35 ...
category: crypto
subcategory: rsa
time_required: long
difficulty_score: 4
...
contest_type: open
wp_url: https://www.ctfiot.com/278359.html
reasoning_chain:
- 题目 enc(x)=(xU)^{[3]}·T, 35维FLAG用digits(257)编码 → GF(257)立方加密 → 触发点：张量分解
- 假设：35个三次多项式=对称张量S, f(x)=Σ_k w_k (v_k^T x)³ → 触发点：CP分解目标
- 动作：Jennrich算法 → 随机收缩a,b∈GF(257)^n → 构造A=S_c×_3 a, B=S_c×_3 b → 假设：特征分解
- 动作：X=A·B^(-1) → 扫t∈[0,256]找det(X-tI)=0 → 收集特征值 → 触发点：特征向量
- '零空间求特征向量v_k → 归一化首非零=1 → 解W: A_rows=V^∘³·W → 假设：CP分解完成'
- 动作：解u³=(W^T)^(-1)·y → 开立方u=pow(u³, 171, 257) → inv3=171(因为3*171 mod 257=1)
- 解V^T·x=u → 组装35个0-256数字 → bytes → FLAG
failed_attempts:
- 试图直接求A^(1/3) → 失败：立方根在GF(257)需inv3=171调pow,但整体x不是直接A^(1/3)
- 试图不解v_k直接猜 → 失败：v_k是35维单位向量组合无法手算
- 试图用sage的tensor()直接CP分解 → 失败：sage无原生对称张量CP分解,需自实现Jennrich
key_observations:
- 张量密码学的安全性依赖维度与域大小,Jennrich O(n³)在GF(257)上对n=35完全可行
- GF(p)立方根唯一(p满足3∤(p-1)时inv3存在)
- 对称张量CP分解=Σ v_k⊗v_k⊗v_k
- Jennrich是1970年代经典方法但在GF(257)立方加密场景被现代密码分析者武器化
prerequisites:
- 对称张量CP分解
- Jennrich算法
- 有限域GF(p)立方根(逆元inv3)
