...
quality: high
full_path: Real_World_CTF_5_–_Teewars.full.md
meta_path: Real_World_CTF_5_–_Teewars.meta.md
images_removed: true
images_removed_count: 0
schema_version: v3.0.0-P0
summary: Real World CTF 5 - Teewars (Teeworlds) Writeup。Real World CTF 5 Teewars (Teeworlds) 自定义 map 文件 CDataFile 格式 Envelope_Points 整数除法溢出 + 写 ROP 链到 bss 触发 execve /bin/sh -c "cat /home/rwctf/flag"。。关键路径...
category: pwn
...
contest_type: open
wp_url: https://www.ctfiot.com/91481.html
reasoning_chain:
- Teeworlds (DDNet) 自定义 CDataFile 格式 → 触发点：header(40) + items + zlib deflate data
- 头 40 字节：aID='DATA' + Version + Size + Swaplen + NumItemTypes + NumItems + NumRawData + ItemSize + DataSize
- CDatafileItem m_Type：Version/Info/Image/Envelope/Group/Layer/Envelope_Points → 假设：Envelope_Points 解析漏洞
- Envelope_Points 解析：m_Size/sizeof(CEnvPoint) 数组 + POINT_VERSION → CEnvPoint m_Time + m_Curvetype + m_aValues[4] 22.10 fixed point
- 漏洞：envelope channel 数量 + 数组 size 计算溢出 → 假设：(len(rop_chain)+0x90)/4 整数除法
- 动作：准备 prepared.map 复制 start + 改 num_channels + 复制 items 0x250-0x750+0x90 + ROP chain
- ROP：pop rdi 0x6c0000 bss + pop rax argv strings + mov [rdi],rax + execve 59
- 完整 chain：写 argv[0]=/bin/sh + argv[1]=-c + argv[2]=cat /home/rwctf/flag → execve(/bin/sh, argv, 0)
failed_attempts:
- 试图直接 shellcode → 失败：bss 段无 shellcode 权限
- 试图覆盖 num_items → 失败：必须改 num_channels
- 试图用 Image 类型触发 → 失败：Envelope_Points 才是漏洞点
key_observations:
- 自定义二进制格式 CDataFile 解析时整数除法溢出是经典漏洞
- Teeworlds/DDNet 引擎 ROP 利用 bss 段写入 argv 是无 shellcode 场景必备
- Envelope_Points m_Size/sizeof(CEnvPoint) 是 size 算术溢出点
- execve(/bin/sh, argv, 0) 需完整 argv 数组 + NULL 结尾
prerequisites:
- Teeworlds CDataFile 二进制格式
- 整数除法溢出原理
- ROP + bss 段写 argv
- execve 系统调用参数构造
---
