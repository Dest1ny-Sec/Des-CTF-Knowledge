...
quality: medium
full_path: Huntress_CTF_2023_—_Write-up.full.md
meta_path: Huntress_CTF_2023_—_Write-up.meta.md
images_removed: true
images_removed_count: 0
schema_version: v3.0.0-P0
summary: Huntress CTF 2023 Write-up。Huntress CTF 2023 多题大杂烩，UHA 压缩包 / Splunk / Firefox 密码 / VBA 宏 / WMI 沙箱检测 / DNS TXT / 文件 XOR 经典套路都有。。经验：Misc 题考的不是新漏洞，是对常见工具链 (oletools / volatility / impacket / fi...
category: misc
...
contest_type: open
wp_url: https://www.ctfiot.com/147388.html
reasoning_chain:
- 题目给出 Tecuha.exe + hot_off_the_press.uha → 触发点：UHA 压缩包 + 自解压 exe
- 动作：strings / exe 静态分析 → 观察：Tecuha.exe 接受 -p password 解压参数 + 内嵌 gz 数据
- 假设：密码可能在 exe 字符串中 → 动作：strings Tecuha.exe | grep -i pass → 观察：找到 winfected
- 动作：Tecuha.exe -pwinfected hot_off_the_press.uha → 观察：解压出后续 payload
- WMI challenge：题目要求输出进程 → 触发点：WMI 沙箱检测类
- 动作：分析 WMI 查询（SELECT * FROM Win32_Process WHERE...）→ 假设：用 impacket wmiexec 远程执行即可
- DNS challenge：拿到 TXT 记录作为 flag → 假设：DNS 外带
- 动作：Resolve-DnsName -Type TXT chal.ctf → 观察：flag 在 TXT 记录
- XOR 文件：拿到密文文件 → 假设：单字节 XOR → 动作：暴力遍历 256 key → 观察：找到 'Huntress{' 前缀
- Splunk challenge：索引日志 → 假设：grep eventtype → 观察：拼出 flag
failed_attempts:
- 试图直接 7z/x 解压 uha → 失败：UHA 不是标准 zip
- 试图用 impacket psexec 跑 WMI → 失败：WMIC 需要正确 namespace
- 试图用 nslookup 拿 TXT → 失败：要先查 _flag 子域名
key_observations:
- Huntress 杂项题常考工具链熟练度（oletools / volatility / impacket / firepwd）
- UHA 压缩包 + wmie 沙箱检测 + Resolve-DnsName TXT 外带是 Huntress 历年高频考点
- 单字节 XOR 爆破靠前几个字节明文（flag 格式）即可识别
- impacket 是 CTF 内网 / AD 题的瑞士军刀
- Splunk 类题直接走 web UI search 即可
prerequisites:
- oletools + impacket 工具集
- Volatility 内存取证（profiles / plugins）
- firepwd（Firefox 密码解密）
---
# Huntress CTF 2023 — Write-up

> 原文: https://www.ctfiot.com/147388.html
> ID: 147388
...

...
wget --user=opendir --password=opendir --recursive --level 5 http://chal.ctf.games:
31407/
...
try {
 ...
 Write-Host([System.Text.Encoding]::
UTF8.GetString([System.Convert]::
FromBase64String($5GMLW)))
 }
 }
 }
catch { }
Resolve-DnsName eventlog.zip -Type txt | ForEach-Object { $_.Strings }
curl -L --max-redirs 80 http://chal.ctf.games:
31430/site/flag.php -v 2>&1 | grep "GET" | awk -v OFS='' '{print "http://chal.ctf.games:
31430",$3}' | for line in $(</dev/stdin); do curl -s $line; done | sed 's/.*is //' | tr -d '\n'
...
curl https://chal.ctf.games:
32479/ -H "User-Agent: Mozilla/5.0 93bed45b-7b70-4097-9279-98a4aef0353e" -k -L
