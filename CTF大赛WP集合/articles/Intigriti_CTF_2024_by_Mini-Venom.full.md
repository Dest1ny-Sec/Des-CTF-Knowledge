...
lesson: '|'
quality: medium
full_path: Intigriti_CTF_2024_by_Mini-Venom.full.md
meta_path: Intigriti_CTF_2024_by_Mini-Venom.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: Intigriti CTF 2024 by Mini-Venom。Intigriti CTF 2024 Mini-Venom 战队的 Pwn 题合集 (RiggedSlot / Retro2Win)，ret2libc + AE64 shellcode 编码。。经验：|
category: pwn
...
contest_type: open
wp_url: https://www.ctfiot.com/215809.html
wp_author: Mini-Venom
reasoning_chain:
- RiggedSlot：ret2libc + 栈溢出 → 触发点：标准 pwn
- 动作：泄露 libc + ret2libc → 假设：canary 关闭 / 可猜
- 假设：使用 AE64 编码 shellcode → 动作：构造 AE64 payload → jmp rsp / call rsp 执行 shellcode → 完成
- Retro2Win：libc-2.23 / libc-2.35 双版本 → 假设：ret2libc 通用 → 栈溢出 + pop rdi + system → 完成
- 约束委派备用链：getST.py S4U2Self + S4U2Proxy → 直接拿 DC ticket
failed_attempts:
- RiggedSlot 直接 ret2shellcode → 失败：栈执行关闭
- Retro2Win 试 one_gadget → 失败：约束不满足
key_observations:
- AE64 是把 shellcode 编码成 alnum 字符的工具，绕过字符限制
- ret2libc 是 pwn 入门模板但适用所有 libc 版本
- Mini-Venom 战队风格：ret2libc + AE64 + 栈迁移组合
- libc 版本切换是 pwn 比赛常见干扰项
- 栈迁移 + magic gadget 是栈空间不足的经典解
prerequisites:
- ret2libc 模板 + AE64 工具使用
- 栈迁移（leave; ret）+ pwntools 脚本
- one_gadget 工具
---
