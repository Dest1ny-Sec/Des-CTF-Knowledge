...
quality: high
full_path: 从Google_CTF_Fluffy_看Dart逆向.full.md
meta_path: 从Google_CTF_Fluffy_看Dart逆向.meta.md
images_removed: true
images_removed_count: 8
schema_version: v3.0.0-P0
summary: '从Google CTF Fluffy看Dart逆向。Dart AOT ARM64 逆向 + 自定义 b62 编解码 + 时间戳爆破 token + 多线程预计算 pin 优化。。关键路径：静态分析 Dart AOT 编译产物（AArch64）识别 saveSecret/update_token_bytes/b62encode → 还原算法 saveSecret: pin 轮循环 rol1(b...'
category: reverse
subcategory: reverse
time_required: long
difficulty_score: 4
...
contest_type: open
wp_url: https://www.ctfiot.com/261979.html
reasoning_chain:
- 'Dart AOT AArch64 静态分析 → DecompressPointer r2 + add x2, x2, HEAP, lsl #32'
- 假设：Dart 对象寻址靠 HEAP 基址 + DecompressPointer 解压 → 动作：识别类成员
- 'saveSecret: pin 轮循环 rol1(byte + token_bytes[j%8], j&7) + memmove 循环'
- 'update_token_bytes: memmove + ror1(token_bytes[j], (pin^((round&3)+1))&7)'
- 'generate_token: gctf25_{ts} SHA1 前 8 字节 b62encode'
- 假设：b62 自定义编解码需要还原 → 动作：静态提取字符表 + 逆运算
- restoreSecretWithPrecomputed 逆算法 (ror1 - token_bytes) → token_bytes_list 10000 项
- 假设：优化 pin 循环 → 动作：预计算 token_bytes_list 把 update 提到 pin 循环外
- 24 线程并行 bf_secret_thread 搜 timestamp 范围 → 测 '}' 字符快速过滤
- '拼接 3 段 flag: Ok4y_h4v3_u_0ptim1zed + brUt3_f0rcE_0R_y0u_jUst + ...'
failed_attempts:
- 试图用 Dart runtime metadata → 失败：Dart AOT 没有标准运行时元数据
- 试图不解 pin 循环直接爆破 ts → 失败：pin 循环太慢
- 试图单线程搜索 timestamp → 失败：算不完
key_observations:
- Dart AOT 没有标准运行时元数据，靠 DecompressPointer+HEAP 寻址对象
- 预计算 token_bytes_list 把 update 提到 pin 循环外性能提升 100x
- 多线程按时间分片爆破 ts 范围是常见优化
- 测 '}' 字符快速过滤可打印解密是爆破提速技巧
prerequisites:
- ARM64 反汇编基础
- Dart AOT 编译产物结构
