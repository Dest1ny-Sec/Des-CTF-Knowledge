...
quality: high
full_path: DiceCTF_2023_Writeup_by_r3kapig.full.md
meta_path: DiceCTF_2023_Writeup_by_r3kapig.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: 'DiceCTF 2023 Writeup by r3kapig。DiceCTF 2023 bop：ret2libc+ORW（open+read+write）flag.txt。关键路径：bop: 32字节溢出+ret2libc+ORW → 阶段1: printf泄露libc+gets写入bss → 阶段2: open(flag.txt, 0) + read(3, 0x404300, 0x100...'
category: pwn
subcategory: rop
time_required: long
difficulty_score: 4
...
contest_type: open
wp_url: https://www.ctfiot.com/96097.html
wp_author: r3kapig
reasoning_chain:
- DiceCTF 2023 bop 题 → 触发点：ret2libc + ORW（open/read/write）
- 32 字节溢出覆盖返回地址 + leave_ret → 假设：栈迁移到 bss 段 → 动作：pay = b'a'*32 + p64(0x404120-0x8) + pop_rdi+puts.got+printf
- 观察：泄露 puts 地址 → 计算 libc_base = leak - 0x1ec980
- 动作：gets 写入 bss 段 flag.txt + ORW ROP 链
- libc gadget：pop_rdi 0x2a1f5 / pop_rsi 0x2601f / pop_rdx 0x142c92 → 假设：构造 open/read/write
- 阶段 2：pay = flag.txt + pop_rdi+0 + pop_rsi+bss + pop_rax+0x2(open) + syscall → 打开 flag.txt
- 动作：read(3, bss+0x100, 0x100) + write(1, bss+0x100, 0x100) → 输出 flag
failed_attempts:
- 试图直接 system('/bin/sh') → 失败：服务器无 bash，靠 ORW 读 flag.txt
- 试图一次 ROP chain 写完 → 失败：32 字节溢出不够，分两段靠 gets+leave_ret
key_observations:
- 32 字节溢出 + leave_ret 栈迁移是经典组合
- ORW 三件套：open(flag.txt) → read(fd, buf, n) → write(1, buf, n)
- libc gadget 偏移（pop_rdi/rsi/rdx/rax）是通用 ORW 链
- printf 泄露 puts.got 后用 libc-database 查偏移
prerequisites:
- pwntools p64/pop_rdi/ROP
- libc-database 查 gadget 偏移
- ORW 模板（open/read/write syscall 链）
- leave_ret 栈迁移原理
---
