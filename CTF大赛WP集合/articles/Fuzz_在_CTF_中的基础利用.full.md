...
quality: high
full_path: Fuzz_在_CTF_中的基础利用.full.md
meta_path: Fuzz_在_CTF_中的基础利用.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: Fuzz 在 CTF 中的基础利用。Fuzz在CTF基础利用：树形结构+随机操作+EOFError检测漏洞。关键路径：MBR A树形结构：MBR A1/MBR A2→obj1, obj2... → 树满TooManyElementsError重试 → EOFError=找到漏洞。经验：堆漏洞Fuzz可降低堆风水调试难度
category: pwn
...
contest_type: open
wp_url: https://www.ctfiot.com/262435.html
reasoning_chain:
- 触发点：堆题 + 树形结构 MBR A/MBR A1/MBR A2/obj1, obj2 → 假设：堆风水调试繁琐
- 假设：0-1000 循环 add/delete 找 double free → 动作：随机 a=0-8, b=0-8
- 动作：i%10==0 时 add(a,b,str(i)) → i%2==0 时 delet(a,b) → 观察：删不存在则跳过
- 动作：写 log.txt 记录有效 add/delete → 假设：日志回放就能复现漏洞
- 假设：TooManyElementsError 表示树满但未找到漏洞 → 动作：重投 + os.remove log
- 假设：EOFError 表示服务端崩溃 → 假设：找到漏洞 → 动作：检查 log 行数 < 20
- 动作：line_count = sum(1 for _ in open('log.txt')) → 假设：20 行内 = 可读堆风水
- 动作：log.txt 是 add(1,1,10)/add(0,6,20)/delet(0,6)/... → 假设：手动分析 chunk 复用链
- 动作：edit(1,3,p64(heap+0x10+0x48)) → tcache poisoning → leak libc → 改 __free_hook → system('/bin/sh')
failed_attempts:
- 试图人工设计堆风水 → 失败：复杂度高调试慢
- 试图 AFL/libFuzzer 真实 fuzz 引擎 → 失败：题目 IO 协议不是文件
- 试图记录全部 add/delete（含不存在） → 失败：日志爆炸
key_observations:
- fuzz 配合 log.txt 回放 = CTF 堆题调试加速器
- EOFError 是 pwntools 检测服务端崩溃最稳信号
- TooManyElementsError 自定义异常重投 = 状态机循环
- line_count < 20 是堆风水复杂度控制阈值
- 树形结构 MBR A/A1/A2/obj 是堆题常见状态空间
prerequisites:
- pwntools process / remote + recv error 检测
- random.randint / Python try-except 异常类自定义
- tcache poisoning 堆风水基础
- libc unsorted bin 泄露
---
# Fuzz 在 CTF 中的基础利用
