...
- container-escape
attack_chain:
- nc 202.38.93.111 10338 → iPXE → SeaBIOS → Linux 5.19.9 启动
- /challenge 是 setuid 0 → 普通用户跑
- cat /flag2 权限 -r-------- 1 1337 1337 → 只有 uid 1337 能读
- env 收集：kernel cmdline / uname / mount / df / ps
- /tmp 共享可写（tmpfs）
- chall 有 SUID 标志 + 路径 + 大小 20352 → static-linked
- pwndbg / ROPgadget / objdump 分析 chall 找漏洞
- 'exploit: 提权到 uid 1337 读 flag2 / 继续提权到 root 读 /flag'
...
quality: high
full_path: 2022_USTC_Hackergame_WriteUp_0x03.full.md
meta_path: 2022_USTC_Hackergame_WriteUp_0x03.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: 2022 USTC Hackergame WriteUp 0x03 (Linux VM pwn)。USTC Hackergame 真机 Linux VM pwn：iPXE 启动 → busybox shell → SUID 提权链。关键路径：nc 202.38.93.111 10338 → iPXE → SeaBIOS → Linux 5.19.9 启动 → /challenge 是 set...
category: pwn
...
contest_type: open
wp_url: https://www.ctfiot.com/71369.html
reasoning_chain:
- '[触发点] nc 202.38.93.111 10338 → iPXE → SeaBIOS → Linux 5.19.9 启动 → 假设：真机 VM / [动作] 进 busybox shell / [观察] 拿到 init / [下一步] /challenge'
- '[触发点] /challenge setuid 0 + uid 1337 owner of /flag2 → 假设：SUID 提权 / [动作] pwndbg 分析 chall / [观察] static-linked 20352 字节 / [下一步] 找漏洞'
- '[触发点] pwndbg/ROPgadget/objdump 分析 chall → 假设：buffer overflow / [动作] ROP 链 / [观察] 提权到 uid 1337 →读 flag2 / [下一步] 继续提权 root → /flag'
failed_attempts:
- 试图直接 root 读 /flag → 失败：/flag 权限 -r-------- root
- 试图覆盖 /challenge 二进制 → 失败：磁盘 read-only
key_observations:
- iPXE + SeaBIOS 模拟远程固件启动是 CTF 高端考法
- 多层提权是 Linux pwn 实战
- static-linked 20352 字节说明挑战是 micro-bin
prerequisites:
- Linux SUID 提权原理
- iPXE/SeaBIOS 启动流程
- pwndbg / ROPgadget / objdump 工具链
- uid 切换 syscall
---
