...
quality: medium
full_path: n00bz_CTF_2024_Writeup.full.md
meta_path: n00bz_CTF_2024_Writeup.meta.md
images_removed: true
images_removed_count: 0
schema_version: v3.0.0-P0
summary: 'n00bz CTF 2024 Writeup。n00bz CTF 2024 Writeup：PowerShell XOR 加密 + brainfuck → C 翻译。。关键路径：PowerShell: cat flag.txt | ForEach { $_ -bxor 3 } → 输出 `m33ayxeqln\sbqjp\twk\{lq~` 加密 → brainfuck → C 翻译 ...'
category: misc
...
contest_type: open
wp_url: https://www.ctfiot.com/197699.html
wp_author: bftoc
reasoning_chain:
- 触发点：tag 'powershell-xor-3' / 'brainfuck-c-translation' / 'qbasic' / 'vm-qbasic' → 假设：PowerShell 加密 + BF + QBasic VM
- '动作：PowerShell: cat flag.txt | ForEach { $_ -bxor 3 } → 触发点：XOR 3 加密 → 输出 ''m33ayxeqln\sbqjp\twk\{lq~'''
- '假设：反转 XOR 3 → 动作：python for c in ''...'': chr(ord(c)^3) → 假设：''Lorem ipsum dolor{'''
- 下一步：brainfuck → C 翻译 → 触发点：bftoc.py by Paul Kaefer → 假设：BF 是变种 / 标准 BF
- 动作：模拟 tape[1000] 跑 BF VM → 观察：输出 32 字符明文
- QBasic VM：tag 'vm-qbasic' → 假设：自实现 QBasic 解释器 → 动作：反编译 → 模拟 → flag
failed_attempts:
- 试图不解 PowerShell 反推 → 失败：PowerShell -bxor 是 BigInt XOR，与字符串 XOR 不同
- 试图不解 BF 直接打印 → 失败：BF 必须模拟 tape
- 试图不解 QBasic VM → 失败：自实现 VM 必须反编译
key_observations:
- PowerShell -bxor 是逐字节 XOR 3，python 反推即可（但要用 ord/chr 而非 bytes）
- BF → C 翻译（bftoc.py）是 BF 逆向的标配工具
- QBasic VM 是 n00bz 系列特色（'写多语言'风格）
- XOR 3 加密被 `\\` 包装是 PowerShell 路径风格（实际是反斜杠）
prerequisites:
- PowerShell 语法（-bxor / ForEach / Get-Content）
- Brainfuck 编程模型（tape[1000] / 8 个指令）
- Python 反编译工具（bftoc.py）
- QBasic 解释器原理
---
