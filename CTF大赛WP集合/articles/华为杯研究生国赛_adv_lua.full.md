...
quality: high
full_path: 华为杯研究生国赛_adv_lua.full.md
meta_path: 华为杯研究生国赛_adv_lua.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: '华为杯研究生国赛 adv_lua。华为杯 adv_lua：bytes 自定义结构 move 同地址 UAF + 堆头 address 劫持 + 函数指针改 system 执行 /readflag。。关键路径：题目: 华为杯研究生国赛 adv_lua (Lua 字节码逆向) → 自定义 bytes 数据结构: new(size) 0x30 头部 (size+address) + move(ds...'
category: pwn
...
contest_type: school
wp_url: https://www.ctfiot.com/151498.html
reasoning_chain:
- '华为杯 adv_lua Lua 字节码逆向, 字符串搜索 ''bytearray'' → 触发点: 自定义 bytes 数据结构'
- '假设: bytes.new(size) 分配 0x30 头部 (含 size+address) + 用户数据, move(dst,src) 跨引用 → 动作: IDA 分析 move 函数'
- '观察: move(barr, barr) 同样地址 → free 后再指向 → 触发点: UAF'
- 'libc 泄露: a=bytes.new(0x430) + b=bytes.new(0x20) + a.move(a,b) + c=bytes.new(0x20) → 假设: 0x20 占 0x430 释放的地址 → 动作: get_int64(c,0)'
- '观察: libcbase = get_int64(c,0) - 0x219ce0 → 下一步: 算 system 偏移'
- 'UAF 申请到函数表上, 改 size+address 字段 → 触发点: 系统检查 v2>>40-85 < 2 → 假设: 构造 (87<<40) 防止申请到 libc → 动作: 改 address 字段为 system'
- '控制 rdi: target=heapbase+0x2a0, 写 ''/readflag'' → 假设: bytes.get_int64 触发函数指针 → 动作: set_int64(heap_head+0x28, target) → system(''/readflag'') 完成'
failed_attempts:
- '试图不绕过 (87<<40) 检查 → 失败: 系统拒绝分配到 libc'
- '试图走 ret2libc → 失败: Lua 字节码函数调用约定不同'
key_observations:
- Lua 自定义 C 数据结构逆向要找 move(dst,src) 同样地址漏洞
- 堆头 size+address 字段是 UAF 利用金矿, 劫持 address 实现任意地址读写
- (87<<40) 是 Lua GC 检查 magic number 的标志 (高位字节比对)
- /readflag 是 SUID 程序, 走 system() 即可触发
prerequisites:
- Lua C API 逆向 (lua_pushcfunction / lua_setfield)
- UAF 原理 (释放后引用)
- glibc heap chunk 结构
- pwntools get_int64 / set_int64 用法
---
# 华为杯研究生国赛 adv_lua
