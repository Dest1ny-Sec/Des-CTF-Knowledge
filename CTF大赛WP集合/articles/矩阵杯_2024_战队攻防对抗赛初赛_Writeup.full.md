...
quality: high
full_path: 矩阵杯_2024_战队攻防对抗赛初赛_Writeup.full.md
meta_path: 矩阵杯_2024_战队攻防对抗赛初赛_Writeup.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: '矩阵杯 2024 战队攻防对抗赛初赛 Writeup。矩阵杯 2024 Reverse：UPX 修复脱壳 + pycdc 反编译 wtf.pyc + IDEA 算法 pyd 逆向复用开源实现。。关键路径：1.1 packpy: UPX 加壳修复 (.upxrecoverytool.py) + pyinstaller 解包 + pycdc 反编译 → 内部含 base58 + zlib + m...'
category: reverse
subcategory: reverse
tools_used:
- Python
...
contest_type: open
wp_url: https://www.ctfiot.com/185241.html
reasoning_chain:
- packpy程序显示UPX加壳但工具识别失败 → 触发点：修改版UPX
- 假设：.upxrecoverytool.py能修复+脱壳 → 动作：python .upxrecoverytool.py -i packpy -o packpy2
- pyinstaller解包得Python字节码 → 假设：内部含base58+zlib+marshal编码的scrambled_code_string
- 动作：exec(marshal.loads(zlib.decompress(base58.b58decode(scrambled_code_string))))
- zlib.decompress后是缺文件头的pyc → 假设：补550D0D0A头 → wtf.pyc
- pycdc反编译wtf.pyc → 观察：random.seed(len(flag)) + key[byte]^95加密
- 写decrypt：inv_key={x:i for i,x in enumerate(key)} → 256 seed全爆破
- 下一步：ccc是Python 3.10 .pyd逆向+IDEA算法mod 0x10001
- 复用razvanalex/IDEA-encryption-algorithm开源实现 → 解密密文
failed_attempts:
- 试图不解UPX直接跑packpy → 失败：脱壳是必要前置
- 试图不补pyc头反编译 → 失败：缺magic number解析失败
- 试图逆向IDEA自行实现 → 失败：IDEA mod 655复杂，直接复用开源更快
key_observations:
- Python字节码保护=base58+zlib+marshal编码+缺文件头
- random.shuffle(range(256))做key置换可逆（inv_key字典）
- IDEA是经典mod 65537 (0x10001) 块密码，8轮×6步+最后4步
- keyExpend 52位（6×8+4）是IDEA标志
- .upxrecoverytool.py是修复修改版UPX的标准工具
prerequisites:
- UPX加壳修复（.upxrecoverytool.py）
- pyinstaller解包 + pycdc反编译
- Python字节码dis/marshal/
- IDEA算法结构（mod 65537 + 8轮6步）
---
