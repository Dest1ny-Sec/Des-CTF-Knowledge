...
- Capstone 脚本
- X64Dbg scylla dump
- IDAPython ctree_visitor_t 删垃圾指令
- pushf/popf + global var 化简
- readeflags/writeeflags 删除
- '全局变量 push ds: 还原'
...
quality: high
full_path: 2024年KCTF水泊梁山-反混淆.full.md
meta_path: 2024年KCTF水泊梁山-反混淆.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: 2024 年 KCTF 水泊梁山 - 反混淆。机器码特征 NOP + Capstone 线性扫描 + IDAPython ctree 化简 + push eax 还原。。经验：KCTF 反混淆标准流程：特征匹配 (第一遍) → CFG 还原 (第二遍) → 全局变量化简 (第三遍) → ctre...
category: reverse
subcategory: reverse
subcategories:
- reverse
...
contest_type: open
wp_url: https://www.ctfiot.com/211585.html
reasoning_chain:
- 触发点：二进制两个区段 obf obf 一存数据一执行 → 假设：仅有指令变形无代码乱序
- 动作：扫描机器码找固定模式 → 观察：9C FF 35 ?? ?? ?? ?? 8F 05 ?? ?? ?? ?? 9D 反复出现
- 假设：pushf + push ds:[gvar] + pop ds:[gvar2] + popf模式 → 动作：NOP 全部替换为 90
- 动作：X64Dbg scylla dump 修复 PE → 观察：还原干净 PE
- 触发点：剩余 jb jmp call 等控制流混淆 → 假设：线性扫描 + Capstone 可还原
- 动作：capstone Cs(CS_ARCH_X86, CS_MODE_32) 线性扫描 → 观察：识别 call imm + xor imm 还原基本块
- 动作：把 jmp call 直接还原 CFG → 观察：IDA 打开已可读
- 触发点：剩余 push ds:[gvar] + mov [esp], eax 全局变量访问 → 假设：ctree AST 化简
- 动作：idaapi.ctree_visitor_t(CV_PARENTS) 遍历 → isjunk(cot_asg + cot_helper) → 动作：nop + push eax
- 观察：IDA 反编译可直接读
- 触发点：readeflags/writeeflags 模式 cot_asg + cot_helper → 动作：直接整行删除 → 完成反混淆
failed_attempts:
- 试图用 IDA插件 Hex-Rays 自动反混淆 → 失败：混淆是自定义指令变形不在已知模式库
- 试图一次性 patch 所有混淆 → 失败：脚本 MAX_REF_COUNT=5000 必须分批跑
- 试图用 Unicorn仿真还原 IAPI 推测寄存器 → 失败：混淆不是基于虚拟机的
key_observations:
- KCTF 反混淆标准流程：特征匹配→CFG 还原→全局变量化简→ctree AST 操作四遍走
- pushf + push ds:[gvar] + pop ds:[gvar2] + popf 是早期混淆固定模式（机器码 9C FF 35 ?? 8F 05 ?? 9D）
- Capstone 线性扫描比 IDA 反汇编引擎对指令变形混淆更稳
- IDAPython ctree_visitor_t + cot_asg/cot_helper 检测可批量 AST 节点删除
prerequisites:
- x86 机器码与汇编指令映射（pushf/popf 编码）
- Capstone 反汇编框架 Python 绑定
- IDA Python API（idaapi / ctree_visitor_t）
- PE 文件结构与 X64Dbg Scylla dump 插件
---
# 2024年KCTF水泊梁山-反混淆
