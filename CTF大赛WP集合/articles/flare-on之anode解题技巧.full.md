...
quality: high
full_path: flare-on之anode解题技巧.full.md
meta_path: flare-on之anode解题技巧.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: flare-on 之 anode 解题技巧。FLARE-ON anode NodeJS NEXE 打包逆向，从十六进制编辑提取 JS 源码 + 状态机逆推。。关键路径：识别 NEXE 打包的 NodeJS 可执行 → 十六进制编辑搜 "Enter flag" 特征字符串 → 提取 JS 源码。经验：NEXE 把 JS 源码直接打包进 EXE（无加密无压缩），十六进制编辑就能还原。
category: reverse
subcategory: reverse
...
contest_type: open
wp_url: https://www.ctfiot.com/80126.html
reasoning_chain:
- 触发点：IDA 显示是 NodeJS 程序 + 提示 'NEXE 打包' → 假设：JS 源码直接打包进 EXE，无加密无压缩
- 动作：十六进制编辑搜 'Enter flag' 特征字符串 → 观察：定位 JS 源码位置 → 下一步：直接复制 JS 代码
- 动作：提取 JS 源码 → 假设：flag 经长 switch 运算后与 target 数组比较 → 假设：可逆推
- 触发点：if (1n) { console.log('uh-oh, math is too correct...'); } → 假设：1n 永真，走 if 分支 → 下一步：绕过 else 分支
- 观察：state = 1337; state ^= random(2^30) → switch(state) → 假设：state 跳到特定 case 执行 b[i] 异或/加减
- 动作：逆推 b[i] 的运算序列 → 观察：b[i] 经过多次加减和异或，最后与 target[i] 比较 → 假设：从 target 数组倒推
- 动作：写脚本逆推每个 b[i] 的初始值 → 假设：还原 char → 拼接 flag
- 观察：成功还原 44 字符 flag → 完成
failed_attempts:
- 试图跑 EXE 程序在 IDE 中调试 → 失败：用户输入 flag 后程序退出，无调试机会
- 试图不解密 NEXE 编译源码 → 失败：JS 源码以字符串形式打包，必须十六进制提取
- 试图不绕过 1n 直接走 else → 失败：else 是 'math is broken' 错误分支
key_observations:
- NEXE 把 JS 源码直接打包进 EXE（无加密无压缩），十六进制编辑就能还原
- state = 1337 ^ random(2^30) 是 CTF 状态机逆推的经典模式：从 target 数组倒推 b[i]
- 1n (BigInt) 永真 → 走 if 分支是 NodeJS 代码审计常见 trick
- 44 字符输入通常意味着 16 个 long state × 多字符运算
prerequisites:
- NEXE 打包原理
- NodeJS BigInt / switch / readline API
- 状态机逆推思路
- 十六进制编辑器使用（HxD / 010 Editor）
---
# flare-on之anode解题技巧
