...
quality: high
full_path: DownUnderCTF_2022_Writeups.full.md
meta_path: DownUnderCTF_2022_Writeups.meta.md
images_removed: true
images_removed_count: 0
schema_version: v3.0.0-P0
summary: 'DownUnderCTF 2022 Writeups。DownUnderCTF 2022 14题writeup：OFB+RSA+NoSQL+Symlink+SQLi+Python。关键路径：ofb 加密：已知前缀+两次加密还原明文 → rsa-interval-oracle-i: 区间预言机二分爆破 → rsa-interval-oracle-ii: Manger 攻击。经验：OFB可利用前...'
category: crypto
subcategory: rsa
tools_used:
- Python
...
contest_type: open
wp_url: https://www.ctfiot.com/59624.html
reasoning_chain:
- 触发点：14 题 writeup 含 OFB / RSA / NoSQL / Symlink / SQLi / Python bytecode → 假设：OFB 双 IV 可还原明文
- OFB 加密 iv1+iv2 → ct1+ct2 → pt2[i] = ct1[i+1] XOR ct2[i] → 动作：因 message 含 300 字节随机数 → 已知前缀字节可还原
- 假设：rsa-interval-oracle-i 通过二分判断 m∈[a,b] → 动作：写二分脚本 → 观察：单次 round 1.29M oracle 调用
- Manger 攻击：1 oracle → 爆破 m → 动作：bisect 范围 → 观察：解密成功
- 'pay-to-win: ECC 阶 N → discrete_log 求 K → 动作：Sage multiplicative_order'
- 'NoSQL mongo: payload {''username'':''admin'',''password'':{''$ne'':1}} → 观察：登录绕过'
- 'Symlink web: 上传 tar 含软链 → 服务端解压读 /flag → 动作：ln -s /flag a; tar cf exploit.tar a'
- 'sqli-quine: payload 必须自包含输出自己 → 动作：用 REPLACE 构造自复制'
- 'CVE-2020-10735: mod_deflate hash 截断碰撞 → 假设：爆破短前缀'
- 'ast-parse: pyc 编译错误信息回显源码片段 → 假设：触发 SyntaxError 泄露'
- 'compilerbot: bool 返回值二分搜索 flag 字符 → 动作：bisect 字符表'
failed_attempts:
- 试图直接解密 OFB 单次加密 → 失败：缺第二份密文
- 试图标准 RSA Wienner/Boneh-Durfee → 失败：oracle 是区间不是 LSB
- sqli 用普通注释符绕过 → 失败：payload 不能含 self-referential 注释
key_observations:
- OFB 双加密 ct2 XOR ct1[i+1] = pt2[i] 是 OFB 模式最经典 IV 还原思路
- RSA 区间预言机 = Manger 攻击 + 二分爆破
- mongo $ne/$gt 是 NoSQL 注入最简手段
- tar 软链解压 → 服务端读取任意文件（Zip Slip 同源）
- SQLi 自包含 payload（quine）需要 REPLACE() 函数构造
prerequisites:
- AES-OFB/CBC/CTR 模式区别
- RSA 模运算 / discrete_log（Sage）
- NoSQL 注入语法（mongodb $ne/$gt）
- tar/zip 软链/路径穿越原理
---
