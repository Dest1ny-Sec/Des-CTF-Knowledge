...
- RSA
attack_chain:
- 'CyberSpace: b=[32,38,27,...] flag += chr(b[i]+70) = ''flag{different_xor}'
- 'strange_rsa1: gift 是 0.9878... 高精度小数，n/gift ≈ q² → iroot 拿 p'
- n 2048 位，gift 1024 位，p ≈ iroot(n/gift, 2) ± 偏移爆破
- 'strange_rsa2: 多 e 共享 n，CRT 攻击 e1*e2*...'
- 'strange_rsa3: 已知 p 高位 + 私钥泄露'
- 'web: Java 反序列化 / Spring 全家桶'
- 'pwn: pwncli 库简化 EXP'
...
quality: high
full_path: 2022_美团MTCTF初赛_writeup_by_Arr3stY0u.full.md
meta_path: 2022_美团MTCTF初赛_writeup_by_Arr3stY0u.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: '2022 美团 MTCTF 初赛 writeup by Arr3stY0u。美团 MTCTF 6 大类：math + RSA + web + pwn + reverse + misc。关键路径：CyberSpace: b=[32,38,27,...] flag += chr(b[i]+70) = ''flag{different_xor} → strange_rsa1: gift 是 0.98...'
category: crypto
subcategory: math
subcategories:
- math
...
contest_type: open
wp_url: https://www.ctfiot.com/57358.html
wp_author: Arr3stY0u
reasoning_chain:
- '[触发点] CyberSpace b=[32,38,27,...] → flag += chr(b[i]+70) → 假设：偏移70 / [动作] 解 / [观察] ''flag{different_xor}'' / [下一步] strange_rsa1'
- '[触发点] strange_rsa1 gift = 0.9878... → 假设：gift = n/p/q 高精度小数 / [动作] RealField 还原精确 n/gift → iroot(n/gift, 2) = p / [观察] p ± 偏移爆破 / [下一步] strange_rsa2'
- '[触发点] strange_rsa2 多 e 共享 n → 假设：CRT / [动作] CRT 攻击 e1*e2*... / [观察] m / [下一步] strange_rsa3'
- '[触发点] strange_rsa3 已知 p 高位 + 私钥泄露 → 假设：Coppersmith / [动作] Coppersmith 还原低 p / [观察] flag / [下一步] web + pwn'
- '[触发点] web Java 反序列化 / Spring 全家桶 + pwn pwncli 库 → 假设：标准 web + pwn / [动作] ysoserial / pwncli / [观察] 拿分'
failed_attempts:
- 试图直接分解 n → 失败：n 是 2048 位 RSA 不可直接分解
- 试图猜 p 偏移 → 失败：必须 ±1000 候选爆破
key_observations:
- gift = 高精度小数 → RealField 还原精确 n/gift → iroot 开方爆破 p
- 多 e 共享 n 用 CRT 攻击是经典
- Coppersmith 已知 p 高位是 RSA 高级技巧
prerequisites:
- Sage RealField 高精度小数还原
- iroot 开方爆破
- CRT 多 e 攻击
- Coppersmith 已知 p 高位
- Java 反序列化 ysoserial
