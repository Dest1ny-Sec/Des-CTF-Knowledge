...
lesson: '|'
quality: high
full_path: Naughty_List_Challenge_Write-Up_–_X-MAS_CTF.full.md
meta_path: Naughty_List_Challenge_Write-Up_–_X-MAS_CTF.meta.md
images_removed: true
images_removed_count: 0
schema_version: v3.0.0-P0
summary: 'Naughty List Challenge – X-MAS CTF (C++ unordered_map rehashing)。X-MAS CTF Naughty List: C++ unordered_map insert 11 个 Naughty 触发 rehash, 让 PinkiePie 移出前 16 元素触发 flag 输出。。经验：|'
category: reverse
subcategory: reverse
tools_used:
- C
...
contest_type: open
wp_url: https://www.ctfiot.com/87572.html
wp_author: the
reasoning_chain:
- naughty_list.cpp 题 → 触发点：ask PinkiePie for the flag
- 假设：PinkiePie 必须在 naughty_list unordered_map 中 → 观察：否则输出 flag.txt
- add 操作：max size ELF_MAX_NAUGHTY_COUNT=16 → 触发点：满 16 后阻止添加
- C++ unordered_map bucket 依赖 hash → 假设：bucket 数会随元素增加 rehash
- 动作：构造 Naughty 名字插入 → 观察：触发 rehash 时 PinkiePie 移出前 16 元素
- 假设：ask PinkiePie 前 PinkiePie 必须不在 map → 动作：先 insert 11 个 Naughty
- 观察：rehash 后 PinkiePie 落到 bucket 17+ → 下一步：add 满 16 触发 max 限制
- 动作：再 ask PinkiePie → 观察：PinkiePie 已不在 map → 输出 flag.txt
failed_attempts:
- 试图删除 PinkiePie → 失败：题目未提供 delete 操作
- 插入 16 个 Naughty → 失败：max 限制阻止
- 直接构造 PinkiePie 不在 map → 失败：PinkiePie 必须先存在
key_observations:
- C++ unordered_map rehash 会改变元素 bucket 位置
- max_size 限制 + rehash = 元素位置可控
- 前 N 个元素在 bucket 0..15，超出后被 rehash
- PinkiePie 是否在 unordered_map 是核心逻辑判断
- hash 碰撞触发 rehash 是 C++ 容器特性
prerequisites:
- C++ unordered_map 内部原理
- hash 函数与 rehash 触发条件
- bucket 与 max_load_factor 关系
- C++ 标准库容器调试
---
