...
quality: high
full_path: DataCon2023软件安全赛道，冠军战队WP分享.full.md
meta_path: DataCon2023软件安全赛道，冠军战队WP分享.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: 'DataCon2023软件安全赛道，冠军战队WP分享。DataCon2023软件安全：Metasploit/CobaltStrike shellcode stager分析。关键路径：200个Metasploit/CobaltStrike PE样本 → 6个UPX壳：upx -d脱壳 → 无Encoder: hash(0x6B8029 WSAStartup)+call ebp。经验：shika...'
category: misc
...
contest_type: open
wp_url: https://www.ctfiot.com/150561.html
reasoning_chain:
- 200 个 Metasploit/CobaltStrike PE 样本 → 触发点：shellcode stager 配置提取
- 假设：6 个 UPX 壳直接 upx -d → 动作：批量脱壳
- 无 Encoder 样本 push 0x6B8029 + call ebp → 假设：hash(WSAStartup) 常量 → 动作：ida_search.find_binary 找 '29 80 6B 00'
- 32 位 IP 0x80BEA8C0 → C0A8BE80 → 192.168.190.128；端口 0x33050002→0x533=1331 → 假设：小端字节序转换
- shikata_ga_nai 编码器 → 假设：内存特征 xd9 x74 x24 xf4 = fnstenv 指令 → 动作：grep 该 4 字节定位
- fcmovu st,st(1)+fnstenv byte[esp-0Ch]+xor+loop → 假设：SMC 多轮自解密 → 动作：等解密后用无 Encoder 方法提取
- CobaltStrike InternetConnectA 调用前 push 0C69F8957h → 假设：向前偏移取端口，call 下一条指令压栈 IP → 动作：pop ebx 后 ebx 指向 IP
- AgentTesla V3/V4 SMTP/FTP 协议 → 假设：明文配置或异或字符串 → 动作：strings + XOR 提取 host/pass
- Sliver mTLS → 假设：C2/CA/证书/私钥四个字段 → 动作：内存 dump 导出
failed_attempts:
- 试图动态跑样本拿 C2 → 失败：沙箱内存 dump 已下载，直接静态分析效率高
- 试图正向反汇编 shikata_ga_nai → 失败：多轮自修改解密，循环次数 XORLEN 决定
key_observations:
- shikata_ga_nai 内存特征 xd9 x74 x24 xf4 是 fnstenv 指令（fcmovu 之后）
- CobaltStrike 32/64 位 IP 用 pop + jmp+jmp+call 链路取回 IP
- Metasploit hash 常量 0x6B8029(WSAStartup)/0xC69F8957h(InternetConnectA) 是稳定指纹
- CobaltStrike 简单混淆：0x1000 对齐开头 4 字节长度+4 字节密钥 + 8 字节填充 + shellcode
prerequisites:
- Metasploit/CobaltStrike shellcode 生成原理
- IDA Python 脚本开发（idc.SEARCH_DOWN/ida_search）
- shikata_ga_nai 自修改解密循环结构
- AgentTesla V3/V4 + Sliver C2 协议（SMTP/FTP/mTLS）
---
