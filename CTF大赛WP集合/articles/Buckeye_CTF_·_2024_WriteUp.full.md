...
quality: medium
full_path: Buckeye_CTF_·_2024_WriteUp.full.md
meta_path: Buckeye_CTF_·_2024_WriteUp.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: 'Buckeye CTF · 2024 WriteUp (狼组版)。BuckeyeCTF 2024 狼组复盘 — Web SSFS uuid 目录穿越 + Pwn runway0/1/2 栈溢出 + calc canary 泄 + color 字符串回显 + 4 个 flag 涵盖 web/pwn/reverse。。关键路径：Web fu: SSFS 文件服务, uuid 存储, 目录穿越读 ...'
category: web
...
contest_type: open
wp_url: https://www.ctfiot.com/207473.html
reasoning_chain:
- '[触发点] Web fu: SSFS 文件服务 + uuid 存储 → 假设：目录穿越读 flag.txt / [动作] 看源码 + 试 ../flag.txt / [观察] 拿到 bctf{4lw4y5_35c4p3_ur_p4th5} / [下一步] Pwn runway0/1/2'
- '[触发点] Pwn runway0 一行 sh exec 1>&2 + cat flag.txt → 假设：sh 弹 shell / [动作] nc + 输入 sh / [观察] 拿到 bctf{0v3rfl0w_th3_M00m0ry_2d310e3de286658e} / [下一步] Pwn runway1'
- '[触发点] Pwn runway1: 0x4c padding + p32(win) 覆盖返回 → 假设：栈溢出 / [动作] p.sendline(b''a''*0x4c + p32(win)) / [观察] 跳转到 win / [下一步] 拿到 bctf{I_34t_fl4GS_4_bR34kf4st_7c639e33ffcfe8c2}'
- '[触发点] Pwn color: 字符串回显输入含 bctf{1_d0n7_c4r3_571ll_4_m1d_c010r}!?!? + Pwn runway2: 0x14*0x09 + p32(0x804A025+0x1fdb) + ''aaaa'' + p32(0x8049253) + Pwn calc canary 泄 pi + 10014 + 0x28 + canary + rbp + ret → 假设：多 flag / [动作] 依次触发 / [观察] 4 个 flag / [下一步]'
failed_attempts:
- 试图直接爆破 admin → 失败：需要栈溢出或 weak password
- 试图在 runway1 用 canary 旁路 → 失败：没有 canary
key_observations:
- 入门级赛事风格：目录穿越/栈溢出/canary 泄 全覆盖
- uuid 文件服务常见目录穿越
- calc 类题目 'pi' 触发 canary 泄是经典 trick
- color 字符串回显是 regex 题目 XSS 风格
prerequisites:
