...
quality: medium
full_path: 2024年羊城杯粤港澳大湾区网络安全大赛WP-AI_AK篇.full.md
meta_path: 2024年羊城杯粤港澳大湾区网络安全大赛WP-AI_AK篇.meta.md
images_removed: true
images_removed_count: 5
schema_version: v3.0.0-P0
summary: 2024 年羊城杯粤港澳大湾区网络安全大赛 WP - AI AK 篇。DistilBERT 情感分类对抗样本：反向情绪前缀 + very 提相似度。。经验：NLP 模型对抗样本常用策略：反向情绪关键词前缀 + 强度副词（very）保相似度。
category: reverse
subcategory: reverse
subcategories:
- reverse
...
contest_type: open
wp_url: https://www.ctfiot.com/203028.html
reasoning_chain:
- 触发点：题目给 DistilBERT 三分类 positive/negative/neutral 模型 + 数据集 → 假设：构造对抗样本
- 动作：加载 Sentiment_classification_model + tokenizer → 观察：模型正常分类
- 假设：verify_similarity 余弦相似度 ≥ 0.7 → 动作：需要加扰动但保持相似度
- 触发点：argmax(label) → 假设：翻转预测即可 → 动作：对文本加反向情绪前缀
- 动作：加 'happy' 'nothing' 'unhappy' 反向 → 观察：模型预测翻转
- 假设：相似度可能掉到 0.7 以下 → 动作：加 'very' 强度副词提相似度
- 观察：calc_accuracy 79% → 95% → 拿到 flag
failed_attempts:
- 试图直接替换同义词绕过 → 失败：同义词不影响情感方向
- 试图随机加字符 → 失败：相似度掉到 0.6 以下
- 试图 fine-tune 模型自己再训 → 失败：题目禁止改模型
key_observations:
- NLP 模型对抗样本常用策略：反向情绪关键词前缀 + 强度副词（very）保相似度
- DistilBERT 三分类情感模型对单情绪词很敏感
- verify_similarity 余弦相似度阈值 0.7 是攻击平衡点
- calc_accuracy ≥ 90% 是 NLP 对抗样本通关标准
prerequisites:
- DistilBERT / Transformer 模型加载与推理
- PyTorch 余弦相似度计算
- NLP 情感分析三分类基础
- 对抗样本攻击方法（字符级扰动）
---
...
pre_failed 16 2 nothing  Stress test: [图片已移除]  Good luck!
...
pre_failed 16 2 very nothing Stress test: [图片已移除] Good luck!
...
very nothing  Stress test: [图片已移除]  Good luck!
