...
quality: high
full_path: UKFC2024_CISCN国赛WP.full.md
meta_path: UKFC2024_CISCN国赛WP.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: 'UKFC2024 CISCN 国赛 WP (合并多个 WP)。UKFC2024 CISCN 国赛多 WP 合并：PHP_CMS 命令注入 + QR SSRF + frame 反射 + House of Apple + CryptoAPI + 魔改 TEA。。关键路径：PHP_CMS 命令注入: cmd=php+-r+system(hex2bin(...)) → SELECT * FROM P...'
category: web
...
contest_type: open
wp_url: https://www.ctfiot.com/182308.html
reasoning_chain:
- PHP_CMS → cmd 参数 php -r system(hex2bin(...)) → 触发点：system 函数直接被传入
- 动作：hex2bin('SELECT * FROM PHP_CMS.F1ag_Se3Re7') → 观察：SQL 命令注入
- API qrcode thumb → requestrepo.com dnslog → /readflag 外带 → 假设：SSRF 触发命令
- frame.f_back.f_back.f_back.f_globals['__builtins__'] → 跨 frame 反射逃逸沙箱
- gostack → pop rdi_5_ret 调 6 次 reg 链 → execve('/bin/sh')
- orange_cat_diary → 0xfe1 错位 fake size → unsorted bin → __malloc_hook 改 one_gadget
- EzHeap → largebin attack + setcontext SROP → 假设：libc 2.35 主流打法
- CryptoAPI → CryptAcquireContextA + CryptDeriveKey(AES-256) → CryptDecrypt 还原 v78 32 字节
- 魔改 TEA delta=0x114514 还原 → 假设：rounds 32, sum=delta*rounds
- brotli + XOR secret 6 字符密钥 + DP LCS 还原 flag
failed_attempts:
- 试图直接 SELECT * FROM PHP_CMS.users → 失败：表名是 F1ag_Se3Re7 隐藏
- 试图用 z3 求解魔改 TEA → 失败：delta 已知不需要 z3，逆运算直接还原
key_observations:
- frame.f_back 反射是 Python generator 沙箱逃逸强大攻击面
- House of Apple 配合 setcontext+61 是 libc-2.35 主流打法
- 魔改 TEA delta=0x114514 仍可用 rounds 32 逆运算
- brotli + XOR secret + LCS DP 还原长 flag 是多步组合套路
prerequisites:
- PHP 命令注入 + SQL 注入
- Protobuf / frame 反射逃逸沙箱
- Windows CryptoAPI（CryptDeriveKey / CryptDecrypt）
- TEA / XTEA 加密算法逆向 + House of Apple
---
# UKFC2024 CISCN国赛WP
