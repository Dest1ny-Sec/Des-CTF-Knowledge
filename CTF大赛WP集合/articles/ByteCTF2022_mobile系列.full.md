...
- /local_cache/ 路径
- 腾讯COS POC URL
- am broadcast SET_FLAG
- FlagReceiver exported=false
- root bypass
- FileInputStream
- .myqcloud.com白名单
- byte_unzip
...
quality: high
full_path: ByteCTF2022_mobile系列.full.md
meta_path: ByteCTF2022_mobile系列.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: ByteCTF 2022 mobile 系列 - Silver Droid。ByteCTF 2022 Silver Droid — 攻击者提供腾讯 COS POC URL + 模拟器启动 + am broadcast SET_FLAG 走 root 旁路 exported + MainActivity WebView + shouldOverrideUrlLoading 验证 .myqclo...
category: misc
...
contest_type: open
wp_url: https://www.ctfiot.com/87778.html
reasoning_chain:
- '[触发点] 攻击者提供 https://...cos.ap-nanjing.myqcloud.com/.../poc.apk → 假设：白名单 + POC apk 绕过 / [动作] server.py 启动 emulator pixel_xl_api_30 + adb install poc.apk / [观察] 安装成功 / [下一步] am broadcast SET_FLAG'
- '[触发点] am broadcast -W -a com.wuhengctf.SET_FLAG -n com.bytectf.silverdroid/.FlagReceiver -e flag ''flag{...}'' → 假设：root 状态绕过 exported=false / [动作] am broadcast 发送 / [观察] FlagReceiver 收到 flag 参数 / [下一步] am start MainActivity'
- '[触发点] am start -W -n com.bytectf.silverdroid/.MainActivity -d "https://...cos..." → 假设：WebView loadUrl + Intent getData / [动作] 看 MainActivity getIntent().getData() / [观察] 拿到 URL / [下一步] shouldOverrideUrlLoading'
- '[触发点] shouldOverrideUrlLoading 拦截 scheme=https + host .myqcloud.com 结尾返回 false 不拦截 → 假设：shouldInterceptRequest 拦截 /local_cache/ → 假设：读 cacheDir 文件返回 / [动作] shouldInterceptRequest 拦截 /local_cache/ 路径 / [观察] 文件读取 / [下一步] 拿到 flag'
failed_attempts:
- 试图非 root 状态 am broadcast → 失败：root 才能 bypass exported=false
- 试图用 http 协议 → 失败：usesCleartextTraffic 未开
key_observations:
- WebView shouldOverrideUrlLoading + shouldInterceptRequest 是 Android 经典链
- root 状态下 exported=false 可被 am broadcast bypass
- cos.ap-nanjing.myqcloud.com 白名单是 bypass 套路
- am broadcast 携带 -e 参数可触发 receiver
prerequisites:
- Android WebView 拦截机制
- adb am 命令（am broadcast / am start）
- root + exported=false bypass
- 腾讯 COS 域名白名单
---
