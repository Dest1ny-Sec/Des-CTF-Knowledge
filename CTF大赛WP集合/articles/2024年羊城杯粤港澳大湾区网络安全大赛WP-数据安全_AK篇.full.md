...
quality: medium
full_path: 2024年羊城杯粤港澳大湾区网络安全大赛WP-数据安全_AK篇.full.md
meta_path: 2024年羊城杯粤港澳大湾区网络安全大赛WP-数据安全_AK篇.meta.md
images_removed: true
images_removed_count: 2
schema_version: v3.0.0-P0
summary: 2024 羊城杯数据安全 AK 篇（CSV 数据清洗 + 身份证校验）。数据安全 AK 流水线：tshark 抽 POST→正则按字段分类→身份证 GB 11643 校验+性别位+手机号段白名单→写 garbage_data.csv。。关键路径：tshark -Y http.request.method==POST -T fields -e json.value.string 抽 POST ...
category: misc
...
contest_type: open
wp_url: https://www.ctfiot.com/203016.html
reasoning_chain:
- 触发点：题目给 PCAP + CSV 人员数据 → 假设：抽 POST body → 动作：tshark -Y http.request.method==POST -T fields -e json.value.string
- 观察：拿到 JSON 字段 → 触发点：8 字段正则分类 → 假设：手机/身份证/出生日期/性别/密码/姓名/编号/用户名
- 动作：re.match field_patterns 分类 → 观察：每行字段归位
- 触发点：身份证 GB 11643 校验 → 动作：加权 [7,9,10,5,8,4,2] % 11 + check_codes 10X98765432
- 动作：身份证倒数第 2 位奇偶 vs 男/女 → 动作：53 段手机号前缀白名单
- 观察：标准化完成 → 触发点：error.log 解析 → 动作：re.search 'username=.*' + '为' 关键字
- 动作：parser2list 用户名/姓名首末+中间 * 化 + 身份证 6+4+8 * 化 + 手机号 3+4+4
- 动作：hashlib.md5(password.encode()).hexdigest() → 观察：写入 garbage_data.csv
failed_attempts:
- 试图用 python pandas 读 CSV → 失败：题目要求字段重排
- 试图不解身份证校验直接用 → 失败：必须 GB 11643 强校验
- 试图不分段直接 md5 整段 → 失败：身份证必须6+4+8 * 化
key_observations:
- 身份证 18 位校验码是 GB 11643 强约束（加权 [7,9,10,5,8,4,2,1,6,3,7,9,10,5,8,4,2] % 11）
- 手机号段归属地可作反欺诈证据（53 段黑名单）
- error.log 解析要按业务中文关键字定位上下文（'为' = 输入密码提示）
- tshark -T fields -e json.value.string 是抽 POST body 标准命令
prerequisites:
- tshark 命令行 POST body 提取
- Python re 正则分类
- GB 11643 身份证校验码算法
- md5 哈希 + 数据脱敏（* 化）
---
