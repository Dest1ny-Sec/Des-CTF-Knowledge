...
quality: high
full_path: 2023年“羊城杯”网络安全大赛Writeup.full.md
meta_path: 2023年“羊城杯”网络安全大赛Writeup.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: '2023 年"羊城杯"网络安全大赛 Writeup。羊城杯综合：PHP 反序列化字符串重组 + Flask session 伪造 + musl libc UAF + 零宽/LSB 隐写。。关键路径：D0n''t pl4y g4m3: php 反序列化，prohib 过滤 system/exec/passthru 等 → syssystemtem 字符串重组绕过 preg_replace →...'
category: misc
...
contest_type: open
wp_url: https://www.ctfiot.com/133282.html
reasoning_chain:
- 触发点：D0n't pl4y g4m3 跳转响应包提示 hint.zip → 假设 php 反序列化
- 动作：观察 prohib 过滤 system/exec/passthru → 假设字符串重组 syssystemtem → 观察：preg_replace 绕
- 动作：Bei__destruct → Yang__call → call_user_func 链 → 观察：触发 system
- actor Serpent Flask session → 假设 secret_key 伪造 → 动作：secret_key=GWHT1GGAvNAKvF
- 动作：pickle.loads(base64) + cos system bash -i 反弹 → 观察：python3.8 SUID 读 /flag
- actor ArkNights /proc/1/environ 非预期读 secret_key → 假设读进程环境 → 观察：拿到 secret_key
- actor shellcode 自写 ORW 汇编 → actor cookieBox musl libc UAF →假设 FSOP
- actor EZ_misc Gronsfeld 密码 + CVE-2023-28303 截图恢复 → Matryoshka blind_watermark + VeraCrypt
failed_attempts:
- 试图用 system/exec 直接命令 → 失败：preg_replace 黑名单拦截
- 试图标准 glibc 走 cookieBox → 失败：musl libc 需 FSOP 劫持 __stdin_FILE
key_observations:
- preg_replace 可用 syssystemtem 重组绕
- Flask session secret_key 可从 /proc/1/environ 拿
- musl libc UAF 用 FSOP 劫持 __stdin_FILE
- CVE-2023-28303 是 libcurl URL 解析截图恢复
prerequisites:
- PHP 反序列化字符串重组
- Flask session 伪造与 /proc 进程环境
- musl libc FSOP 与 __stdin_FILE 结构
- blind_watermark + VeraCrypt 隐写
---
