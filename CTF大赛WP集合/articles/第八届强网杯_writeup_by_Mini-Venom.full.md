...
- SUID-find
- platform
- PHP-session-deserialize
- notouchitsclass-eval
- proxy-SSRF
- snake-BFS
attack_chain:
...
quality: high
full_path: 第八届强网杯_writeup_by_Mini-Venom.full.md
meta_path: 第八届强网杯_writeup_by_Mini-Venom.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: '第八届强网杯 writeup by Mini-Venom。第八届强网杯 Mini-Venom：baby_heap 改 libc.got + PyBlockly 全角 Unicode 绕 + platform session 反序列化 + proxy SSRF + snake BFS。。关键路径：Pwn baby_heap: malloc(0x700) + malloc(0x10) + fre...'
category: misc
...
contest_type: open
wp_url: https://www.ctfiot.com/213637.html
wp_author: Mini-Venom
reasoning_chain:
- 触发点：baby_heap malloc(0x700)+malloc(0x10) 释放1后show(1) 泄出 chunk fd 残留指针 → 假设：未清零即 UAF
- 动作：show(1) 收8字节用 u64 解码扣 0x21ace0 得 libc_base → 观察：libc_base 0x7f... 命中本地 libc
- 假设：got 表可写 → 动作：edit chunk 写 p64(got)+p64(puts) 二次分配把 libc.got[puts] 覆盖成 libc.sym['puts'] 地址
- 观察：libcdb-search 搜 puts 偏移确认 libc 版本 → 下一步：构造 puts(puts@got) 链触发再次 leak
- PyBlockly 全角 Unicode 触发：x.__init__.__globals__['__builtins__']['eval']('os.popen("head /flag")') → 触发点：服务端未限制 unicode 等价符
- 动作：convert_to_chinese_unicode 把 - . / = > 替换成 \uFF0D \uff0e \uff0f \uff1d \uff1e → 观察：payload 绕过后端 filter
- platform session 反序列化：触发点 session_data = unserialize($_COOKIE['session']) → 假设：魔术方法触发 RCE → 动作：构造 User→__destruct 链
- Suid 提权：触发点 /blockly_json 接口接受 JSON 内部有 user-defined class → 动作：构造 POST 数据让 exec shell
failed_attempts:
- 试图直接 free(0) 后 show(0) 泄 libc → 失败：tcache 已清空但 unsorted bin 未触发
- PyBlockly 直接用 __import__('os').system → 失败：被黑名单过滤
- 试图单次 PUT cookie 触发反序列化 → 失败：需要多个 set-cookie 累积
key_observations:
- UAF 后 show(chunk) 收 fd 残留 = 经典 libc_base 泄露，但需要先 unsorted bin
- libc.got 覆盖成 system 等价函数 = GOT hijack，需要 2 次任意写
- 全角 Unicode 字符 \uFF0D 等与 ASCII 同形 = WAF 过滤绕过经典手法
- libcdb-search 是 CTF 必备 libc 偏移查询库（无需本地 libc 时）
prerequisites:
- glibc 堆结构（tcache / unsorted bin / fd bk）
- PHP 反序列化魔术方法链构造
- Unicode 等价字符绕过 WAF 知识
- pwntools 模板 + LibcSearcher/libcdb
---
