...
quality: medium
full_path: SANS_Offensive_CTF_–_Taskist--_01–04.full.md
meta_path: SANS_Offensive_CTF_–_Taskist--_01–04.meta.md
images_removed: true
images_removed_count: 0
schema_version: v3.0.0-P0
summary: 'SANS Offensive CTF - Taskist 01-04。SANS Offensive CTF Taskist 4 题：IDOR 遍历 tasks/64 拿 flag1 + 改 user_id 改 admin 密码 + SSRF 测 LFI 读源码 + file:///root/flag.txt 读根目录。。关键路径：Taskist::01 IDOR: /api/tasks/64...'
category: web
subcategory: ssrf
tools_used:
- BurpSuite
...
contest_type: open
wp_url: https://www.ctfiot.com/166045.html
reasoning_chain:
- 题目 IDOR/SSRF/LFI 经典组合 → 触发点：Burp Intruder 遍历数字 ID
- Taskist::01 /api/tasks/64 → 直接访问 admin 任务 → flag-1
- Taskist::02 update password 接口 user_id 参数可改 → 假设：改 admin user_id 重置密码
- 改 user_id 为 admin → 重置密码登录 → 拿到 admin token → flag-2
- Taskist::03 site_config 导入 url 参数 → Burp 拦截改 http://attacker → SSRF confirmed
- SSRF + LFI 读 /proc/self/environ 拿环境变量 → 假设：可读 /app/index.js 源码
- Taskist::04 文件上传触发 file:/// → 测根目录 file:///root/flag.txt → flag-4
failed_attempts:
- Taskist::01 试图盲扫大量 ID → 实际只到 64 即可，Burp 数字遍历即可
- Taskist::03 试图找其它 SSRF 入口 → 失败：site_config 导入是唯一入口
- Taskist::04 试图读 /etc/passwd 间接信息 → 直接 file:///root/flag.txt 更快
key_observations:
- IDOR + 权限提升 + SSRF + LFI 是 OWASP Top 10 经典 4 链
- Burp Intruder 数字遍历是 IDOR 探查基础
- /proc/self/environ 读环境变量是 SSRF 提权必经之路
- file:// 协议可读根目录文件是 LFI 终极利用面
prerequisites:
- Burp Suite Intruder / Collaborator 使用
- OWASP Top 10 漏洞类型
- Linux /proc 文件系统（LFI 进阶利用）
- HTTP 协议 + URL scheme 基础知识
---
