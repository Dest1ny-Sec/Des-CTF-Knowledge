...
quality: high
full_path: 【Hack_The_Box】PC【WriteUp】.full.md
meta_path: 【Hack_The_Box】PC【WriteUp】.meta.md
images_removed: true
images_removed_count: 0
schema_version: v3.0.0-P0
summary: 【Hack The Box】PC【WriteUp】。Hack The Box PC 题，gRPC 50051 端口反射枚举 + LoginUser 用户枚举 + pyload SSH 登录。。经验：rustscan 比 nmap 快的现代端口扫描器；gRPC 服务默认在 50051 端口，需用 grpcurl/eva...
category: web
...
wp_url: https://www.ctfiot.com/136588.html
reasoning_chain:
- rustscan --top 扫 10.129.95.145 发现 22 和 50051 端口 → 触发点：50051 是 gRPC 默认端口
- go install grpcurl → grpcurl -plaintext list 看到 SimpleApp + grpc.reflection.v1alpha 反射服务 → 假设：可枚举方法
- go install evans → evans -r show service 拿到 LoginUser/RegisterUser/getInfo 三个 RPC → 动作：写 Python gRPC client
- desc LoginUserRequest 字段 username+password → 写脚本遍历 saulgoodman/admin/siegfried 与常见密码 → 观察：弱密码超击
- getInfo 返回 pyload 下载地址 → 假设：可上传 id_rsa.pub 到 pyload 默认 9666
- 写 Python 多部分 gRPC stub 上传公钥 → 动作：ssh -i id_rsa sau@10.129.95.145 → 观察：get shell
failed_attempts:
- 试图用 nmap -p- 扫全端口 → 太慢，改 rustscan --top
- 试图浏览器访问 50051 → 失败：gRPC 是二进制协议无 HTTP UI
- 试图 evans 交互手输密码 → 太慢，改 Python 批量破解
key_observations:
- rustscan 比 nmap 快 10×，ulimit 需 ≥5000 避免缓冲
- grpc.reflection.v1alpha 是反射服务，必装才能 list service
- SimpleApp 弱密码（用户名枚举）是 gRPC 题常见入口
- pyload 9666 端口 + SSH 公钥植入是 HTB 常见三拓
prerequisites:
- rustscan/nmap 端口扫描
- grpcurl/evans gRPC 客户端使用
- Python gRPC client 编写 (grpc-tools + protoc)
- SSH 公私钥登录原理
---
