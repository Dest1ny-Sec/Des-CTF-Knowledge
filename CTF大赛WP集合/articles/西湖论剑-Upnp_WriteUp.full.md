...
quality: high
full_path: 西湖论剑-Upnp_WriteUp.full.md
meta_path: 西湖论剑-Upnp_WriteUp.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: 西湖论剑-Upnp WriteUp。西湖论剑UPnP WriteUp,NETGEAR R7000系列路由器固件,UPnP SOAPAction协议11个service+SOAPLogin+Cookie sess_id会话+GetMacList白名单+acosNvramConfig_get(lan_ipaddr)比对,经典IoT路由器攻击面。。关键路径：路由器固件:NETGEAR R7000系...
category: reverse
subcategory: reverse
time_required: long
difficulty_score: 4
...
contest_type: open
wp_url: https://www.ctfiot.com/110766.html
reasoning_chain:
- 题目给NETGEAR R7000固件二进制 → 触发点：UPnP SOAPAction HTTP入口点
- 反编译发现 v11=stristr(http_v4,'SOAPAction:') → 假设：11个service逐项匹配
- action_v13=v11+11 → strchr找'r'+strncpy 127字节 → 11项while轮询stristr → 触发点：服务名分发
- 假设：'(v21-2)==r && (v21-1)==n' 识别 SOAPLogin 格式 → 动作：定位 DeviceConfig#SOAPLogin
- 未登录走 acosNvramConfig_get('lan_ipaddr')+GetMacList+strstr白名单 → 触发点：鉴权链
- 假设：Cookie sess_id=xxx 绕过 → 动作：伪造空 sess_id 触发服务端默认分支
- lan_ipaddr 比较可指定 → 观察：bypass+processAction(serverIdx=0~10+ifSSL+pass_v7) → RCE
failed_attempts:
- 试图调用9个非ParentalControl/DeviceConfig的service → 失败：serverIdx越界返回-1
- 试图写入/tmp/opendns_auth.tbl注入MAC → 失败：tmpfs只读且需root权限
key_observations:
- NETGEAR R7000 UPnP 11个service是公开接口(ParentalControl/DeviceConfig/WANDevice等)
- acosNvramConfig_get(lan_ipaddr)是路由器通用配置API,lan_ipaddr对比可被请求伪造
- Cookie sess_id+MAC白名单+lan_ipaddr三层校验的bypass点都在请求侧
- SOAPAction urn:NETGEAR-ROUTER:service:ParentalControl:1#Authenticate是稳定攻击向量
prerequisites:
- 路由器UPnP协议基础(SOAPAction header)
- NETGEAR固件逆向(IDA/Ghidra)
- C字符串处理(stristr/strncpy/strstr)
- IoT路由器认证绕过常见模式
---
# 西湖论剑-Upnp WriteUp
