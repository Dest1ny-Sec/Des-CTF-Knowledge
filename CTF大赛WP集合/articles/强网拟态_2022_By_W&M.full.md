...
quality: high
full_path: 强网拟态_2022_By_W&M.full.md
meta_path: 强网拟态_2022_By_W&M.meta.md
images_removed: true
images_removed_count: 0
schema_version: v3.0.0-P0
summary: 强网拟态 2022 WriteUp - W&M战队。强网拟态2022 W&M全方向9+题:Node原型链命令注入/SQL时间盲注/Jinja2 Unicode转义/JDBC MySQL_Fastjson反序列化/JMX RMI反序列化/fmtstr改got。。经验：Node json注入command数组+constructor.prototype.argv0可执行任意命令;SQL时...
category: misc
...
contest_type: open
wp_url: https://www.ctfiot.com/70190.html
reasoning_chain:
- 强网拟态 2022 W&M 战队全方向 9+ 题：Node 原型链 / SQL 时间盲注 / Jinja2 SSTI / JDBC MySQL_Fastjson / JMX RMI / fmtstr 改 got → 触发点：综合混淆攻击面
- Web user：json.dumps command 数组 + constructor.prototype.argv0 → 假设：argv0 可被原型链覆盖 → 动作：构造 POST json='{"command":["-c","cat /flag"]}'
- Sql：benchmark 时间盲注 STRCMP(hex(right(flag,a)), i) + spaces 注释替换 → 假设：benchmark 高延时反映条件 → 动作：二分爆破
- SSTI：__class__ 八进制 + subclasses()[247] + os.popen + cat /flag → 假设：Jinja2 __class__ = str → 动作：\137\137\143\154\141\163\163\137\137 Unicode 转义
- JDBC mysql MySQL_Fastjson：allowLoadLocalInfile + autoDeserialize + ServerStatusDiffInterceptor + Groovy bash 反弹 → 假设：MySQL JDBC 反序列化
- JMX：BadAttributeValueExpException + MyBean + Connect(allowLoadLocalInfile) + MyObjectInputStream hashCode 密钥比对 → 假设：TiedMapEntry + LazyMap
- 'Pwn fmtstr：fmtstr_payload(8, {printf_got: system_plt}) + sendline(/bin/sh) → 动作：printf got 改 system'
failed_attempts:
- Web user 直接 exec() → 失败：必须 prototype.argv0
- Sql 单次 benchmark → 失败：必须多层 hex 嵌套
- SSTI 用 subclasses() 直接 → 失败：黑名单挡 __class__
- JDBC 单 connection 反序列化 → 失败：必须 autoDeserialize + allowLoadLocalInfile
- JMX 用 CommonsCollections 链 → 失败：必须自实现 MyObjectInputStream hashCode 比对
key_observations:
- Node json 注入 command 数组 + constructor.prototype.argv0 可执行任意命令
- SQL 时间盲注用 benchmark(9999999,md5)+STRCMP+hex(right) 是模板
- Jinja2 绕黑名单用 __class__ 八进制 + subclasses()[247].__init__.__globals__.os.popen
- MySQL Connector/J allowLoadLocalInfile + autoDeserialize + Groovy = 经典反序列化链
- fmtstr_payload 改 printf_got=system 是 fmtstr 经典
prerequisites:
- Node.js prototype pollution
- Jinja2 SSTI Unicode 八进制转义
- Java JDBC / JMX 反序列化链
- pwntools fmtstr_payload 使用
---
# 强网拟态 2022 By W&M
