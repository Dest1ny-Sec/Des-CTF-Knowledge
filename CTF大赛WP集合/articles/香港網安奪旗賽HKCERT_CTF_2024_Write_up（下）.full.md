...
quality: high
full_path: 香港網安奪旗賽HKCERT_CTF_2024_Write_up（下）.full.md
meta_path: 香港網安奪旗賽HKCERT_CTF_2024_Write_up（下）.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: '香港網安奪旗賽HKCERT CTF 2024 Write up（下）。HKCERT 2024下篇涵盖RSA-LCG多线程爆破+几何序列求根+Proxy隐写+数组置换+未知加密z3约束+pyc反编译,密码学综合难度顶级。。关键路径：多线程爆破RSA-LCG seed: bits=128 LCG (a,c已知) 生成4个1024位素数p0-p3做multiprime RSA → 第二阶段用nth...'
category: misc
...
contest_type: open
wp_url: https://www.ctfiot.com/221684.html
reasoning_chain:
- RSA LCG bits=128 a,c 已知生成 4 个 1024 位素数 → 假设：seed 可爆破
- 动作：多线程枚举 seed 重建 LCG 序列 → 观察：恢复 p0-p3
- 动作：multiprime RSA 解密 → flag
- 第二阶段 → 假设：S0 初始种子 → 动作：nth_root(4, all=True) 求根 → 反推 LCG
- ISA 101 PTY 空白字符 + IFS Proxy 隐写 → 假设：Proxy has trap 触发 eval
- 动作：触发 Proxy.eval → 观察：执行命令
- 数组映射 XOR → 假设：array+array2 置换 + array3^num → 动作：还原字节去 0x01
- 未知加密 → 假设：3 未知数线性约束 → 动作：z3 求解
- 动作：s0+s1+s2=300, 2s0+s1+2s2=496, s0+3s1+s2=508 → 还原 key
- sser.pyc → 假设：pycdc/pyLingual 反编译 → 观察：AES-CFB + sha256(nonce+pw) 做 key+IV
- 动作：逆算法解 → hkcert24{c0mpu71n9_…}
failed_attempts:
- 试图不解 LCG 直接因式分解 N → 失败：N 为 multiprime 无标准库解法
- 试图不解 pyc 直接读明文 → 失败：flag 藏在加密里
key_observations:
- RSA LCG 生成素数时，bits 小+seed 少可用 nth_root 或暴力重建
- 未知加密不要硬逆，先列方程组用 SMT/z3 求解
- pyc 反编译（pycdc/pyLingual）是 CTF 必备技能
- PTY 空白字符 IFS + Proxy 隐写触发 eval 是 RE 高级套路
prerequisites:
- RSA-LCG 攻击 (bits=128 + multiprime)
- nth_root (Sage) 求根
- z3/SMT 约束求解
- pycdc/pyLingual pyc 反编译
---
