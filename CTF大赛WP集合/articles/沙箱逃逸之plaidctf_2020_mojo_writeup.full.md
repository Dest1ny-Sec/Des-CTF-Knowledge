...
quality: high
full_path: 沙箱逃逸之plaidctf_2020_mojo_writeup.full.md
meta_path: 沙箱逃逸之plaidctf_2020_mojo_writeup.meta.md
images_removed: true
images_removed_count: 1
schema_version: v3.0.0-P0
summary: 沙箱逃逸之 plaidctf 2020 mojo writeup。PlaidCTF 2020 mojo：Chrome 沙箱逃逸，Mojo PlaidStore 接口 IsRenderFrameLive 悬空指针 + 越界读 OOB + 跨 frame UAF。。关键路径：Chrome headless + MojoJS + MojoJSTest + 远程调试 1338 端口 → PlaidS...
category: misc
...
contest_type: open
wp_url: https://www.ctfiot.com/85249.html
reasoning_chain:
- 题目给 plaidstore.diff 改 content/browser/plaidstore/plaidstore_impl.h → 触发点：PlaidStore 是新增 Mojo interface
- PlaidStore.StoreData(key, array) + GetData(key, count) → 假设：GetData 的 count 可控可越界
- PlaidStoreImpl 持 RenderFrameHost* render_frame_host_ → 假设：IsRenderFrameLive() 后悬挂
- 动作：pwn.html 加载 mojo_bindings.js + plaidstore.mojom.js + PlaidStore.getRemote(true)
- 动作：堆喷射 0x200 次 storeData('xxxxx', Uint8Array(0x28).fill(0x41)) → 观察：连续分配 PlaidStoreImpl
- 动作：p = plaidStorePtrList[0] + getData('xxxxx', 0x2000) → count=0x2000 远超实际 0x28
- 观察：GetData 用 vector(begin, begin+count) 构造 result，无 size 检查 → OOB 越界读
- 假设：越界读到 PlaidStoreImpl 虚表 → 动作：从偏移 0x28/8 开始扫 vtable
- 观察：拿到 vtable 地址 → 算 base → 任意读 → UAF：父 frame 持子 frame Ptr
- 动作：addFrame iframe srcdoc 注入 uaf() → 父 frame 跨 frame 引用 → 触发 UAF
- 假设：UAF 后 fake object + 覆盖 vtable → RCE → 沙箱逃逸
failed_attempts:
- 试图只跑 storeData 不堆喷射 → 失败：vtable 地址不可预测
- 试图跳过越界读直接打虚表 → 失败：必须先 leak vtable 地址
- 试图只搞 OOB 不搞 UAF → 失败：OOB 只能读，不能稳定写
key_observations:
- GetData(key, count) 用 vector(begin, begin+count) 是经典 OOB 模式
- IsRenderFrameLive() 返回后指针悬空是 RenderFrameHost 经典 UAF
- 堆喷射 0x200 次同 size 构造 deterministic layout
- 跨 frame 引用（parent/child iframe）+ srcdoc 是 Chromium UAF 通用触发
- Chrome headless --remote-debugging-port=1338 是 fuzz 标配
prerequisites:
- Chromium Mojo IPC（mojom 接口 + JS binding）
- C++ vector OOB 漏洞模式
- 堆喷射与堆布局控制
- iframe srcdoc + cross frame Mojo Ptr
---
