...
quality: high
full_path: 春秋云境MagicRelay_WP.full.md
meta_path: 春秋云境MagicRelay_WP.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: 春秋云境MagicRelay WP。春秋云境MagicRelay：Redis DLL劫持+向日葵RCE+AD CS域提权(CVE-2022-26923)+RBCD全链路。经验：域内AD CS证书服务+机器账户+passthecert可实现无域管密码登录域控
category: web
subcategory: logic
tools_used:
- C
...
contest_type: open
wp_url: https://www.ctfiot.com/230509.html
reasoning_chain:
- fscan 扫到 39.98.125.24:6379 redis unauthorized + redis 3 → 触发点：Redis 3 Windows 主机
- 假设：redis 3 可主从复制 → 动作：尝试 → 观察：机器没 web 不能写 webshell
- 假设：Redis 3 可写 .so 上线 → 动作：RedisWriteFile.py --rhost ... --rfile C:\ProgramData\test.dll
- 观察：CS 客户端收到 beacon → 拿到 flag1 → 下一步：横向
- 内网扫描发现向日葵 SunloginClient → 触发点：sunRce RCE
- 动作：sunRce.exe -h <ip> -t rce -c 'type C:\flag02.txt' → 观察：flag2
- Sunlogin 进程注入 + SeImpersonatePrivilege → 假设：SweetPotato 可提权 system
- 动作：SweetPotato.exe → 观察：system shell
- certify.exe 发现 AD CS 服务 → 触发点：CVE-2022-26923 域提权
- 动作：certipy account create + passthecert.py -action write_rbcd → 申请 cifs 服务票据
- 动作：impacket-getST xiaorang.lab/simho$ -spn cifs/WIN-SERVER → DCSync → PTH 到 WIN-AUTHORITY
failed_attempts:
- Redis 主从复制 → 失败：redis 3 不支持
- 直接写 webshell → 失败：机器没 web 服务
- 试图用 mimikatz sekurlsa logonpasswords → 失败：缺 system 权限
key_observations:
- Redis 3 + Windows 可写 .so 上线 CS（C/S 分离木马场景）
- AD CS 证书服务 + 机器账户 + passthecert.rbcd = 无域管密码登录 DC
- SweetPotato 利用 SeImpersonatePrivilege 是 Windows 服务提权通用套路
- RBCD + S4U2Self 是绕过域管密码的现代 AD 攻击路径
prerequisites:
- Redis 未授权利用（主从/.so 劫持）
- impacket 工具链（secretsdump/wmiexec/getST）
- AD CS 证书服务（CVE-2022-26923）
- RBCD + S4U2Self 约束委派原理
- 向日葵 RCE（CVE-2019-13298 等）
---
# 春秋云境MagicRelay WP
