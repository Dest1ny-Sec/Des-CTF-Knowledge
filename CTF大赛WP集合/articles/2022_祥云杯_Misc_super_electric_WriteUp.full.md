...
quality: high
full_path: 2022_祥云杯_Misc_super_electric_WriteUp.full.md
meta_path: 2022_祥云杯_Misc_super_electric_WriteUp.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: 祥云杯 2022/Misc super_electric。MMS 工控流量 tshark 提取 exe → 反调试脱壳 → XOR 解出 Python 还原脚本 → AES/CBC padding 爆破 key 倒推 IV。关键路径：拿到 pcapng 流量，IPv4-TCP-MMS 占绝大多数，确认是 MMS 协议 → 过滤 mms.confirmedServiceRequest，发现 L...
category: misc
subcategory: stego
subcategories:
- stego
...
contest_type: open
wp_url: https://www.ctfiot.com/68853.html
reasoning_chain:
- 拿到 pcapng → 协议分布 IPv4-TCP-MMS 占绝大多数 → 触发点：MMS 协议分析
- 动作：tshark -Y 'mms.confirmedServiceRequest' 过滤 → 观察：两种 itemId
- LLN0$CO$FunEna1$SBOw (开头) / FunEna1$Oper / FunEna2$Oper → 假设：后者才是数据通道
- FunEna2$Oper 全程传同一值 172.20.1.23 → 假设：噪声；转向 FunEna1$Oper
- 动作：tshark -Y 'mms.itemId == "LLN0$CO$FunEna1$Oper"' -T fields -e mms.octet_string → data.txt
- 观察：hex 数据，开头 4d5a9000 → 触发点：MZ 头，是 exe！
- 动作：写 Python bytes.fromhex 拼接 → test.exe → 观察：得到二进制
- 假设：exe 含还原脚本 → 动作：IDA 静态分析 → 观察：发现 AES/CBC + Python bytecode
- 假设：脱壳后反编译 Python → 动作：decompile3 / uncompyle6 → 观察：拿到 AES 加密逻辑
- 动作：已 sha256(key)[:10] + 已知 message → AES/CBC padding 爆破 16 字节 key → 倒推 IV
- 观察：还原 flag{72713126e9b90eab}
failed_attempts:
- 试图直接读 exe 内的字符串 → 找到的不是 flag 是密文
- pwsh 跑 tshark → 失败：'$' 在 pwsh 里被当变量展开
- 试图不解密直接看 pcapng 文本字段 → flag 不在文本层
key_observations:
- MMS 协议是工控 SCADA 标准，mms.octet_string 字段可作二进制传输通道
- 4d5a9000 (MZ 头) 是识别 Windows PE 文件的最快方法
- AES/CBC 已知 message 倒推 IV 是 padding oracle 反向思路
- tshark 命令行比 Wireshark GUI 更稳（pwsh 兼容性问题）
prerequisites:
- MMS / IEC 61850 工控协议基础
- tshark 命令行使用（-Y 过滤 / -T fields 提取）
- AES/CBC/PKCS7 padding 原理
- Python 反编译（uncompyle6 / decompyle3）
---
