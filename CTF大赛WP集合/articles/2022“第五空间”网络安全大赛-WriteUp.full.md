...
quality: high
full_path: 2022“第五空间”网络安全大赛-WriteUp.full.md
meta_path: 2022“第五空间”网络安全大赛-WriteUp.meta.md
images_removed: true
images_removed_count: 2
schema_version: v3.0.0-P0
summary: '第五空间 2022/Web + Pwn 合集。phar 反序列化读 /flag + 报错注入 + 命令注入 + ret2libc 三题合集。关键路径：Web1: phar 反序列化读 /flag → upload 类：filename=__construct 设 /flag；__wakeup 读 file_get_contents(filename) → phar metadata 存这个对...'
category: web
...
contest_type: open
wp_url: https://www.ctfiot.com/58322.html
reasoning_chain:
- '[触发点] Web1 phar 反序列化 → 假设：upload 类 __wakeup 触发 file_get_contents / [动作] 生成 phar +改 png 后缀 + phar:// 触发 / [观察] /flag / [下一步] Web2'
- '[触发点] Web2 登录框 admin%df'' 宽字节 → 假设：GBK 编码报错注入 / [动作] admin%df''^(SUBSTRING(select(user()),1,1)<>0x61)# / [观察] user/dababase / [下一步] 弱口令 + 命令注入'
- '[触发点] admin admin123 弱口令 → 假设：撞库 / [动作] 登录 / [观察] 进后台 / [下一步] 命令注入'
- '[触发点] 127.0.0.1%0acd${IFS}ky*%0als%0atac${IFS}fl* → 假设：%0a + ${IFS} 绕空格 / [动作] / [观察] / [下一步] Pwn'
- '[触发点] Pwn 0x400753 pop rdi; ret → puts@GOT → puts@PLT → main → 假设：ret2libc / [动作] 泄 libc / /bin/sh + system'
failed_attempts:
- 试图直接 cat flag → 失败：被过滤
- 试图普通空格 → 失败：被过滤 → ${IFS}
key_observations:
- phar:// 是 file_exists/exif/finfo 等函数都触发的反序列化入口
- ${IFS} 绕过空格过滤
- 宽字节报错注入 admin%df'
- pop rdi; ret 是 ret2libc 标准 gadget
prerequisites:
- PHP phar 反序列化
- GBK 宽字节报错注入
- bash ${IFS} 空格绕过
