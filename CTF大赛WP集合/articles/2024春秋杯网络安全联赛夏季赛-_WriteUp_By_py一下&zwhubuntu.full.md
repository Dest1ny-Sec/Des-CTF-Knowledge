...
- block_cipher
tags:
- ENV/DIFF/FILE/FUN POP 链
- Hijack LD_PRELOAD 绕 disable_functions
- brother 反弹 MySQL udf
- ezecc ECC 256bit p=gcd 求曲线参数+爆破 k
- Signatrue DSA LCG nonce 复用 HNP 格
- 2024happy N^2+2024 Coppersmith + HSSP 正交格
attack_chain:
...
quality: high
full_path: 2024春秋杯网络安全联赛夏季赛-_WriteUp_By_py一下&zwhubuntu.full.md
meta_path: 2024春秋杯网络安全联赛夏季赛-_WriteUp_By_py一下&zwhubuntu.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: 2024 春秋杯夏季赛 WP（py一下&zwhubuntu 视角：Hijack / ezecc / Signatrue / 2024happy）。春秋杯夏季赛三大 crypto + 一 web：Hijack LD_PRELOAD + ezecc gcd 求 ECC 曲线 + Signatrue DSA nonce 复用 HNP 格 + 2024happy N^2 Coppersmith + ...
category: web
...
contest_type: open
wp_url: https://www.ctfiot.com/193435.html
reasoning_chain:
- 触发点：Hijack ENV/DIFF/FILE/FUN POP 链 → 假设：ENV.__toString 调 putenv + system → 动作：构造 ENV→DIFF→FILE 链
- 动作：ENV->math=DIFF->callback=FILE->enviroment=ENV key='LD_PRELOAD' value='/tmp/...so'
- 动作：unserialize → putenv LD_PRELOAD + system → 拿 shell → mysql 凭据 → udf 提权 → flag
- 触发点：ezecc 给 a,b,K=(G,kG) + c1,c2 → 假设：p = gcd(f1, f2) 求曲线素数
- 动作：p = gcd(G[0]³+a*G[0]+b-G[1]², c1[0]³+a*c1[0]+b-c1[1]²) → 观察：拿到 p
- 动作：k 私钥 < 1e6爆破 → m = C1 - k*C2 → flag 前半
- 触发点：Signatrue DSA 8 次签名同 nonce 复用 → 假设：HNP 格
- 动作：构造 A=R[i]*s0/(r0*S[i]), B=(r0*H[i]-R[i]*h0)/(r0*S[i]) 矩阵
- 动作：Ge = [q*I; A; B; 0 0 K 1], BKZ-30 → 还原 d → 重签 admin → flag
- 假设：pass_proof 爆破 2^19 大小写 → flag
- 触发点：2024happy N=n²+2024, hint=(3^2022 p² + 5^2022 q²) mod N → 假设：Coppersmith 还原
- 动作：4x4 LLL 矩阵 [[1,0,Ta*2^pbit,0],[0,1,Tb*2^qbit,0],[0,0,TN,0],[0,0,T*hh,N]] 找 p,q
- 动作：AES-CBC key=mylove_in_summer + HSSP 正交格 find_ortho_fp kernel_LLL → flag
failed_attempts:
- 试图直接 php://input LD_PRELOAD → 失败：必须经 POP 链触发 putenv
- 试图不爆破 k 直接解 ezecc → 失败：k 私钥 < 1e6 是必爆点
- 试图不打 HNP 格直接对 DSA 暴力 → 失败：DSA 8 次签名同 nonce 必须 HNP
key_observations:
- 'ezecc 关键观察：K = (..., 4359... : 4359...) K = (G, kG)，求 p 时所有点代入曲线方程再 gcd'
- Signatrue 用 A,B 矩阵构造 LLL 求 d 是教科书 HNP
- 2024happy 把 RSA 与 HSSP 拼接是密码学综合题经典套路
- LD_PRELOAD绕 disable_functions 是 POP 链经典组合
prerequisites:
- PHP 反序列化 POP 链构造（ENV/DIFF/FILE/FUN）
- ECC 曲线方程 + gcd 求 p
- DSA nonce 复用 HNP 格 + LLL/BKZ 算法
- Coppersmith + HSSP 正交格综合
---
