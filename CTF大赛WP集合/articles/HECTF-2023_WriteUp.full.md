...
quality: high
full_path: HECTF-2023_WriteUp.full.md
meta_path: HECTF-2023_WriteUp.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: 'HECTF-2023 WriteUp。HECTF 2023 多关MISC：OSINT+牌型密码+拼图+NTLM。关键路径：第1关: Welcome HECTF{Welcome_To_HECTF_2023} → Osint: 河北省邯郸市永年区永年太极广场 → 牌型密码: 黑桃s 红桃h 梅花c 红尖d。经验：综合MISC题常结合OSINT+牌型+拼图+NTLM'
category: misc
...
contest_type: open
wp_url: https://www.ctfiot.com/147010.html
reasoning_chain:
- 第 1 关 Welcome → 触发点：HECTF{Welcome_To_HECTF_2023}
- Osint → 触发点：题目给出图像需定位 → 假设：太极广场图形特征
- 动作：Google 搜索 '河北省邯郸市永年区永年太极广场' → 观察：找到地址 + 密码 X2z0Um23RF
- 导出 gif 图片 → 触发点：牌型密码 → 假设：黑桃s 红桃h 梅花c 红尖d
- B 站视频 https://www.bilibili.com/video/BV1Lj41187kx/ → 假设：binwalk 出密码
- small joker=kaf, big joker=wi1 → 动作：农民牌解出 flag 图片
- 观察：HECTF{Dou_di_Zhu_uhZ_id_uoD_hEccctf_TY6d145A57R7WVz}
- 百度网盘题 → 触发点：二维码拼图 24 张 → 假设：拼图还原完整二维码
- 动作：手工/AI 拼图 → 观察：扫码得 HECTF{java_ca_fei_bao_bei}
- NTLM 流量题 → 触发点：pcap 含 NTLM 认证 → 假设：ntlmssp 过滤 + NTProofStr 提取
- 动作：tshark -Y 'ntlmssp' -T fields → 观察：得 NTLM hash 还原
failed_attempts:
- 直接搜太极广场 → 失败：需 '永年区' 行政区划
- 不解牌型密码试 binwalk → 失败：B 站视频需先看拼图密码
- 不解 NTLM 直接暴力 flag → 失败：NTProofStr 必须解析
key_observations:
- OSINT 行政区划查询（省/市/区/街道）是基本功
- 牌型密码映射表（黑桃/红桃/梅花/红尖）是非标密文
- B 站 BV 号定位 + binwalk 分离是 MISC 套路
- 二维码拼图还原 = 24 张 piece 排序
- NTLM NTLMv2 NTProofStr 提取是 Windows 认证基础
prerequisites:
- OSINT 地理定位基础
- tshark NTLM 流量分析
- 二维码 + 拼图还原思路
