...
quality: high
full_path: 解析Dancing_Circle.full.md
meta_path: 解析Dancing_Circle.meta.md
images_removed: true
images_removed_count: 0
schema_version: v3.0.0-P0
summary: 解析Dancing Circle。Dancing Circle把"加密题"藏回数独与DLX,大整数生成数独初盘+DLX精确覆盖求解+5步×60次×4旋转=1200次cover+6次字节查表合成净变换T_net,IDA+Python复现全流程。。关键路径：双处数据校验→生成大数Num1,Num1×Num2得数独初盘Data1 → 解析Data1→初始化DLX(Dancing Links)精...
category: reverse
subcategory: reverse
tools_used:
- IDA
...
contest_type: open
wp_url: https://www.ctfiot.com/282475.html
reasoning_chain:
- 题目双处数据校验+大数Num1 → Num1×Num2得数独初盘Data1 → 触发点：数独+DLX包装
- 假设：DLX(Dancing Links)精确覆盖问题 → 81候选+4种约束(格子/行/列/宫)=324列 → 触发点：算法识别
- 动作：DLX递归cover()/uncover()解数独 → 9个调试检查函数推得中心格=7固定
- 用户输入80位hex在cover()中分5小步做'交换+移位' → 触发点：cover()函数被重用为混淆容器
- 每60次交换=1次向右90°旋转,共4次旋转回原位 → 5×60×4=1200次cover()调用 → 假设：6张256字节表T1-T6
- 动作：动态抓初盘+查表(x64dbg) → 合成净变换T_net[0-9]=[11,181,163,154,228,232,129,94,134,151]
- L[T_net(D_i)]==S_i (∀i≠center) → 逐格穷举x∈[0..15] → 验证cnt==79 → flag=32415867057146038208672345171508423626351804840632517408215763137846025652307148
failed_attempts:
- 试图静态分析cover()内5小步逻辑 → 失败：1200次调用+6张表静态看爆炸
- 试图不解DLX直接求UserData2 → 失败：UserData2与DLX解耦合,需DLX先解数独
- 试图穷举80位hex全部组合 → 失败：16^80太大不可行,需逐格逆向
key_observations:
- 复杂加密题常包装经典算法(数独+DLX),识别算法本质是关键
