...
- user32
attack_chain:
- PE 工具看导入表：GetWindowTextA / DrawTextA / SetWindowTextA / TextOutA 等
- MFC 程序 GetDlgItem 消息循环拿控件文本（不是 GetDlgItemTextA）
- 用户名长度 6-20 位，序列号长度 27 位
- encrystUsernameFirst 加密用户名：循环 16 次 (username[edx] * 0x1339E7E - &data) * edi，存到 data[0x14]
- encrystUsernameSecond 第二轮加密
- base64 自定义码表编码
- if (a&&b&&c&&d&&e) success else fail — 多个 jne 都得跳过去
- 注册失败时 eax=0，成功时 eax=1
- 用户名 'helloworld' 序列号 'VASdBcsKGhNDMRwPXv85%61MNQR'（最后字符 Q/R/S/T 都行）
...
quality: high
full_path: 2016腾讯游戏安全技术竞赛题PC第一题.full.md
meta_path: 2016腾讯游戏安全技术竞赛题PC第一题.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: 2016 腾讯游戏安全技术竞赛题 PC 第一题。腾讯 GS Lab MFC 注册机：用户名 + 二次加密 + 自定义 base64 码表。关键路径：PE 工具看导入表：GetWindowTextA / DrawTextA / SetWindowTextA / TextOutA 等 → MFC 程序 GetDlgItem 消息循环拿控件文本（不是 GetDlgItemTextA） → 用户名长...
category: reverse
subcategory: reverse
subcategories:
- reverse
...
contest_type: open
wp_url: https://www.ctfiot.com/44316.html
reasoning_chain:
- 触发点：MFC 注册机程序，点击注册无弹框只提示'注册失败' → 假设：得从 import 表下手，strings搜不到关键
- 动作：PE 工具看导入表 → 观察：GetWindowTextA / SetWindowTextA / DrawTextA / TextOutA，但 GetDlgItemTextA 没导入
- 假设：MFC 经典 GetDlgItem + WM_GETTEXT 消息循环模式 → 动作：bp user32.GetDlgItem →观察：注册按钮按下断下
- return 后看栈 → 观察：esp+0x9C 是 username 文本，esp+0x1A0 是 serial 文本
- encrystUsernameFirst trigger：data[0x14] 开 0x14 字节，循环 16 次 (username[edx] * 0x1339E7E - &data) * edi
- 动作：翻译为 C++ 写死未知值 → 观察：与实际 data[0x14] 输出匹配
- encrystUsernameSecond trigger：二次加密 data[0x14] → 写入 5 个 int 到 total[0x2c..0x40] + 5 个 int 到 total[0x40..0x54]
- test3/test4/test5 trigger：5 个 jne 串联跳到 xor eax,eax → 假设：a[5] + b[5] 必须同时满足 5 个线性方程
- 动作：解 5 元方程组求 b[i] 表达式 → 观察：b[0]=2a[2]+a[3], b[1]=3a[2]-a[4] 等
- 序列号 trigger：4 字节循环 + 64 字符表 + 4→3 字节压缩 → 假设：自定义 base64
- 动作：hexdump 64 字符 → 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789@%=' → 不是标准表
- 观察：'5678' → 0xD7 0x6D 0xF8 转换规则就是 base64 的 38→46 bit 中间过程 → 反向 base64 decode 求密文数组
- 动作：用 a[5] 求 b[5] + 自定义码表 base64 encode → 观察：serial = 'VASdBcsKGhNDMRwPXv85%61MNQR' 注册成功
- 末位 Q/R/S/T 都能行 → 假设：base64 '*' 填充 vs 标准 '='，最后 4 位冗余
failed_attempts:
- 试图 bp GetWindowTextA → 失败：MFC 不调它，用 GetDlgItem + SendMessage(WM_GETTEXT)
- 试图用 OD 字符串搜索找 'success'/'fail' → 失败：编译时 inline 掉了，搜不到
- 试图暴力穷举 27 字符序列号 → 失败：94^27 不可能
- 试图完整翻译 base64 编码函数 → 失败：复杂位运算，转 C++ 验证 D7/6D/F8 后简化
- 试图让 jne 跳走 → 失败：5 个 jne 都得满足条件才行，一个不满足就 fail
key_observations:
- MFC 文本提取 = GetDlgItem + WM_GETTEXT 消息循环，不是 GetDlgItemTextA
- PE import 表列出的 API 是 MFC 程序文本逆向的唯一线索
- 5 个 jne 串联跳到同一目标 = 等价 if(a&&b&&c&&d&&e) 复合条件
- 64 字符常量多次出现是 base64 自定义码表的强信号
- base64 encode 是 4→3 字节，'*' 填充替代 '=' 是非标 base64 标志
- 加密流程 = username → encrypt1 → encrypt2 → base64(custom table)
prerequisites:
- OD (OllyDbg) 动态调试 + 硬件断点
- MFC 消息循环机制（GetDlgItem + WM_GETTEXT）
- PE 文件结构 / import 表分析
- x86 汇编（mov/jne/imul/shift + 寄存器 eax/ecx/edx/esi/edi）
- C++ 反汇编模式识别（memcpy/imul/位移/XOR）
- base64 编码原理与自定义码表识别
---
