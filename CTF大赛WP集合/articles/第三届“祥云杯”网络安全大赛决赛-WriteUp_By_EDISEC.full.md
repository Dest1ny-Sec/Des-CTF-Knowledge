...
quality: high
full_path: 第三届“祥云杯”网络安全大赛决赛-WriteUp_By_EDISEC.full.md
meta_path: 第三届“祥云杯”网络安全大赛决赛-WriteUp_By_EDISEC.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: 第三届祥云杯决赛 WriteUp。祥云杯决赛多方向题集：SoapClient SSRF 登录 + XXE expect + mp3 LSB + 自定义 VM + UAF + 线程环形越界 + XOR 密钥 + ORW。。关键路径：Web1 message_board 用 SoapClient null+location+uri 构造 SSRF 到 127.0.0.1/index.php?s=...
category: misc
...
contest_type: open
wp_url: https://www.ctfiot.com/135246.html
reasoning_chain:
- 触发点：Web1 message_board 用 SoapClient 反序列化触发 SSRF→假设：构造 SSRF 到本地 login 传 admin:password
- 动作：SoapClient(null, location='http://127.0.0.1/index.php?s=login', user_agent=CRLF注入)→观察：触发 SSRF 携带 PHPSESSID
- 假设：Web2 babyweb 是 XXE→动作：<!ENTITY xxe SYSTEM 'expect://mv$IFSxx.png$IFSxx.php'>→观察：命令执行
- 假设：Misc1 Twin shadow 按 list.txt 偏移读 mp3 LSB→动作：seek(steps[i]) 读 1 byte LSB→观察：拼回 1.txt
- 假设：PWN1 pwn_ad3 自定义 VM→动作：show 指令泄栈堆地址→下一步：free_hook 写 system
- 假设：PWN3 message_system 0xf 线程环形链路+越界 show→动作：show_idx(9, 0xffffffff) 读 0x18 字节泄 libc→观察：拿到 ORW 链
- 假设：PWN4 note XOR 密钥+堆重叠+environ 泄栈→动作：0x4e0 重叠泄漏→0x4f2 越界读 environ→ORW 读 flag
failed_attempts:
- 试图用 file:// 协议读 flag → 失败：file:// 被 disable
- 试图直接弹出计算器 → 失败：GLIBC 不允许 popen
- 试图走 unsorted bin 泄 libc → 失败：题目没填满 tcache 链
key_observations:
- SoapClient null 模式 + user_agent CRLF 注入是 PHP SSRF 经典手法
- XXE expect:// + $IFS 替换空格是 PHP 命令执行传统套路
- 多线程环形链路通过 show_idx(9,0xffffffff) 越界读泄 libc 是绕深度限制的常用手法
- ORW (open/read/write) 是 shell 拿不到时的标准 fallback
prerequisites:
- PHP SoapClient 反序列化与 CRLF 注入
- XXE + expect:// 协议
- ORW 系统调用链构造
- 多线程环形数据流绕深度限制
---
