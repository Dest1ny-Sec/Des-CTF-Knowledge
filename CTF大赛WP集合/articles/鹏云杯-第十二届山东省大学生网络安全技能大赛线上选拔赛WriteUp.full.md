...
quality: high
full_path: 鹏云杯-第十二届山东省大学生网络安全技能大赛线上选拔赛WriteUp.full.md
meta_path: 鹏云杯-第十二届山东省大学生网络安全技能大赛线上选拔赛WriteUp.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: '鹏云杯 第十二届山东省大学生网络安全技能大赛 WriteUp。鹏云杯 4 题：迷宫 game 字节 XOR + 3 轮 ror+swap 加密 + 4 阶 RSA 连分数攻击。。关键路径：Reverse game 迷宫: wasd 控制 + 4 个 boss 影响 enc + 固定 key → byte_4020[i] += 4 + byte_4060[i] ^ byte_4020[i] =...'
category: misc
...
contest_type: open
wp_url: https://www.ctfiot.com/271997.html
reasoning_chain:
- 'game 题 IDA 拖入看 wasd 控制迷宫 → 4 个 boss 影响 enc → 触发点: print_flag() 在终点'
- 'print_flag() 调 byte_4020[] += 4 + byte_4060[] XOR byte_4020[] → 假设直接按公式还原 → 动作: 写脚本 XOR'
- '观察: flag{Th1s_1s_My_S1mpl3_Fl4g_f0r_CTF} (题出错时有 110 解)'
- 'error 题: encrypt() 是 3 轮 (ror(round+1) + 奇数反转偶数相邻交换) → 假设按逆序解密 → 动作: for round in range(2,-1,-1)'
- '观察: 每轮 ror(round+1) + 异或 key[i%16] → 还原得到不可见字符但题目认 correct'
- 'Crypto 4 阶 RSA: n = p^4 * q^4 → phi = (p^4-1)(q^4-1) → 假设 Wiener / 连分数 攻击小 d'
- '动作: cf(e, n^4) 连分数展开 → 遍历收敛子找 230-bit d → 假设 d.bit_length 在 200~300'
- '观察: pow(c, d, n) 解出 flag{fc3f4ce8dc3eaca8807812b8c0435cd4}'
- '下一步: RSAEncryptor 含 g,a,b 满足 N=2*g*(2*g*a*b+a+b)+1 → 假设可分解 → 动作: 暂存未解'
failed_attempts:
- '试图用标准 RSA 公式解 4 阶 RSA → 失败: phi = (p^4-1)(q^4-1) 不是 (p-1)(q-1)'
- '试图 Wiener 攻击 e 大 → 失败: e 太大, 改连分数对 n^4'
- '试图按 round=0/1/2 正序解密 → 失败: 必须倒序解密'
key_observations:
- 迷宫 + boss 影响 enc 是 CTF 出题套路, 看 print_flag 才是终点
- ror 旋转 + 奇偶不同置换必须按 round 倒序才能还原
- n=p^k q^k 高阶 RSA phi 是 (p^k-1)(q^k-1) 不是简单欧拉
- 小 d 的 RSA 用连分数展开 n^k 是 Wiener 攻击的通用变种
prerequisites:
- IDA 静态分析 + 状态机识别
- RSA 欧拉函数 + Wiener/连分数攻击
- Python 位运算 / 字节处理
- ror 循环右移 + 奇偶反转操作
---
