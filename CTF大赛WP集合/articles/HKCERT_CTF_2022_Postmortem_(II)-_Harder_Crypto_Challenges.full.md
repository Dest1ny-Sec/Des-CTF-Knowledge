...
quality: high
full_path: HKCERT_CTF_2022_Postmortem_(II)-_Harder_Crypto_Challenges.full.md
meta_path: HKCERT_CTF_2022_Postmortem_(II)-_Harder_Crypto_Challenges.meta.md
images_removed: true
images_removed_count: 1
schema_version: v3.0.0-P0
summary: 'HKCERT CTF 2022 Postmortem (II): Harder Crypto Challenges。HKCERT 2022 Harder Crypto：Windows批处理AES+双密钥密文对比。关键路径：Windows批处理实现AES加密（aes.bat） → 已知明文+密文对 → 红色 4czfHjwa9rl+Xds/1EbFuJioVRnYL0ym86UZ2WDMQPg...'
category: crypto
subcategory: rsa
time_required: long
difficulty_score: 4
...
contest_type: open
wp_url: https://www.ctfiot.com/87575.html
reasoning_chain:
- 触发点：题目把 AES-128 (SubBytes+ShiftRows+MixColumns+AddRoundKey 共 10 轮) 完整用 Windows .bat 实现 → 假设：batch 文件本身可作为加密 oracle（已知明文 → 密文）
- 动作：用 ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/ 作为已知明文运行两次（红色 + 蓝色）→ 观察：得到两组密文 4czfHjwa9rl+Xds/... 和 4czIHjwaYd8F1St/...
- 假设：两轮密文差异对应密钥差异，红色密文中 'Y0ym86UZ2WDMQPgBKGT5AN3Cqe7OShpvkxIt' 与蓝色 'nygf6UGZWDMQPuBClqKAX35Te9ONhsv2mkp' 是 key 偏移 → 动作：分析两轮密文逐位差异
- 下一步：把密文位差异 × AES-128 S-box 逆向 → 观察：恢复 key 候选
- 假设：batch 中循环 for /l %%r in (1, 1, 9) 是 9 轮完整 AES-128 → 动作：写 Python 把 batch 还原成 AES 标准库并对 flag ciphertext 解密 → 观察：拿到 flag hkcert22{pr09r4mm1ng_in_b47ch_1s_s0_d1fficul7}
failed_attempts:
- 试图直接逆向 batch 文件的 KEY[] 数组内容 → 失败：KEY 用 set 命令动态存储，逆向混淆里需结合 plaintext/ciphertext 对照
- 试图把红色/蓝色密文直接当 key 候选 → 失败：差分分析必须按 S-box 逆运算还原
key_observations:
- 'AES-128 完整 SubBytes+ShiftRows+MixColumns+AddRoundKey 用 Windows .bat 实现是 CTF 形式创新（hint: ''batch 跑 AES''）'
- 已知明文 + 密文对 + 多轮密钥差异是破解同类自定义密码的通用思路（差分分析思路）
- batch 文件的 for /l 循环 + set /a 异或就是 AES 的本质结构，逆向就是把动态语言还原成静态算法
- CTF 密码题常把标准算法放在非标准语言（bash/python/batch），关键是逆向还原成标准库
prerequisites:
- AES-128 完整算法（SubBytes/ShiftRows/MixColumns/AddRoundKey 10 轮）
- Windows batch 语法（for /l、set /a、call :label）
- 差分密码分析思路（明文密文对 + 多密钥）
- Python AES 库（pycryptodome）
---
