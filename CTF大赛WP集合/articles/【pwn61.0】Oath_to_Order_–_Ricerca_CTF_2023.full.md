...
quality: high
full_path: 【pwn61.0】Oath_to_Order_–_Ricerca_CTF_2023.full.md
meta_path: 【pwn61.0】Oath_to_Order_–_Ricerca_CTF_2023.meta.md
images_removed: true
images_removed_count: 0
schema_version: v3.0.0-P0
summary: 【pwn61.0】Oath to Order - Ricerca CTF 2023。Ricerca CTF 2023 Oath to Order 高级堆题，aligned_alloc size=0 整数零化触发任意长输入 + fastbin 残留 libc 指针泄地址。。经验：aligned_alloc(align, size) 内部走 __int_memalign → __int_mall...
category: pwn
...
contest_type: open
wp_url: https://www.ctfiot.com/111898.html
wp_author: aligned_alloc
reasoning_chain:
- 'NOTE allocator: create/edit(idx, align, size, data) 多次写同 index → 触发点：不可 free 但可重 alloc'
- getstr(buf, size) 内部 while (--size) 读循环 → 触发点：size=0 时 --size = (size_t)-1 → 任意长写
- aligned_alloc(0xF0, 0) → 触发点：size=0 但 align 仍生效 → 假设：触发 memalign → __int_malloc(size+align)
- 动作：split 3 块 → 释放时残留 main_arena libc 指针 → 观察：拿到 libc 0x7f1773219ce0
- libc 基址已知 → 假设：堆溢出可改 next chunk fd → 动作：覆盖 fastbin fd 指向 __malloc_hook/_IO_list_all
- 观察：fastbin attack 拿到 libc 任意地址 → 写 system → get shell
- Fuzzing 是发现 size=0 漏洞的最初方法（fuzzer 跑 + 看异常）
failed_attempts:
- 试图从 size > 0 找溢出 → 失败：size 检查严格，只有 size=0 才进 bug 分支
- 试图绕过 aligned_alloc 直接 malloc → 失败：题目只能用 aligned_alloc
- 试图 free 后用残留指针写 → 失败：题目不让 free
key_observations:
- aligned_alloc(align, 0) 触发 memalign → __int_malloc(0 + align) split 3 块机制
- getstr while(--size) 在 size=0 时 --size = (size_t)-1 进入无限循环可任意长写
- fastbin 残留 main_arena 指针是经典 libc 泄地址方法
- split chunk 释放后残留指针在 glibc main_arena 是稳定 leak 点
- Fuzzer 是 size 整数边界类 bug 的最高效发现手段
prerequisites:
- glibc heap 基础（chunk 结构 / fastbin / split 机制）
- aligned_alloc + memalign 源码分析
- Fuzzer 基础使用（AFL/libFuzzer）
- GDB + pwntools 联合调试
---
