...
- sm4
attack_chain:
- 'Q1 流量: rtsp://ubnt:wbox123@172.10.0.12:8554/live 摄像头流'
- 'Q2 GPS: pynmea2 解析 $GPGGA → folium PolyLine 画地图'
- 'Q3 PHP 反序列化: show.filename = php://filter/read=convert.base64-encode/resource=trueflag.php'
- 'Q4 SSTI: {{ config.__class__.__init__.__globals__[''os''].popen(...) }}'
- 'Q5 PJL: gs -o xx.pdf -sDEVICE=pdfwrite xx.pjl'
- 'Q6 pwn: libc-2.31 HTTP 服务器 ROP 链 → system(''/bin/sh'') dup2(SOCKFD)'
- 'Q7 crypto: AES/ECB/ZeroBytePadding key=esa7*4'
- 'Q8 .pyc 反编译: pycdc 1.pyc → encrypt AES-CBC PKCS7'
- 'Q9 RSA: LLL 降维 + 短向量恢复 p q (GaussLatticeReduction)'
- 'Q10 Android: jni getFlag 字符串异或爆破 ^ 3 + (j mod 26)'
- Q11 加密私钥 ENCRYPTED PRIVATE KEY 离线解密 flag
- 'Q12 SM4: cipflag + randomiv → 大数 long_to_bytes 恢复'
...
quality: high
full_path: 2024_WIDC_writeup_by_Arr3stY0u.full.md
meta_path: 2024_WIDC_writeup_by_Arr3stY0u.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: '2024 WIDC writeup by Arr3stY0u。WIDC 2024 综合靶场 12 大题：RTSP/GPS/PHP/SSTI/PJL/pwn/AES/pyc/LLL/JNI。关键路径：Q1 流量: rtsp://ubnt:wbox123@172.10.0.12:8554/live 摄像头流 → Q2 GPS: pynmea2 解析 $GPGGA → folium PolyLin...'
category: misc
subcategory: stego
subcategories:
- stego
...
contest_type: open
wp_url: https://www.ctfiot.com/183712.html
wp_author: Arr3stY0u
reasoning_chain:
- 触发点：rtsp://ubnt:wbox123@172.10.0.12:8554/live 摄像头流 → 假设：明文认证 RTSP → 动作：抓包看 RTSP 协议
- 观察：用户名密码明文 → 假设：弱口令 → 动作：wbox123 试登
- 触发点：GPS-NMEA $GPGGA → 假设：pynmea2 解析 → 动作：python 解析 NMEA
- 观察：folium PolyLine 画地图 → 假设：轨迹取证 → 动作：folium 输出 HTML
- 触发点：PHP 反序列化 show.filename → 假设：php://filter 读源码 → 动作：php://filter/read=convert.base64-encode/resource=trueflag.php
- 观察：base64 解码得 PHP 源码 → 假设：触发反序列化 → 动作：构造 payload
- 触发点：SSTI {{ config.__class__.__init__.__globals__['os'].popen(...) }} → 假设：Jinja2 模板注入 → 动作：填入用户名/邮箱
- 观察：命令执行返回 → 假设：读到 flag → 动作：cat /flag
- 触发点：gs -o xx.pdf -sDEVICE=pdfwrite xx.pjl → 假设：PJL 打印任务还原 → 动作：ghostscript 转 PDF
- 观察：PDF 内容含 flag → 假设：PJL 头信息 → 动作：grep flag PDF
- 触发点：libc-2.31 HTTP 服务器 ROP → 假设：ret2libc → 动作：dup2(SOCKFD) + system('/bin/sh')
- 观察：system 触发 → 下一步：pwntools interactive
failed_attempts:
- 试图不解 USB RTSP → 失败：必须知道 wbox123 弱口令
- 试图不解 GPS-NMEA → 失败：必须 pynmea2 解析
- 试图直接 SSTI {{7*7}} → 失败：必须 __globals__ os.popen 链
- 试图不解 PJL → 失败：必须 ghostscript 转 PDF
key_observations:
- WIDC 综合靶场覆盖取证/RCE/反序列化/密码/逆向/堆 12 大类
- rtsp://ubnt:wbox123@ 是常见摄像头弱口令
- pynmea2 + folium PolyLine 是 GPS 取证标配
- php://filter 是 PHP 反序列化读源码标配
- Jinja2 SSTI __globals__ os.popen 是经典 RCE 链
- GaussLatticeReduction 是 RSA LLL 短向量还原 p q 标准
prerequisites:
- RTSP 协议 + 摄像头弱口令 wbox123
- pynmea2 GPS-NMEA 解析 + folium 地图
- PHP 反序列化 php://filter 读源码
- Jinja2 SSTI __globals__ chain
- PJL + ghostscript PDF 还原
- SM4 cipflag + randomiv 大数恢复
---
