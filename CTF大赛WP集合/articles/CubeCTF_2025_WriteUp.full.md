...
quality: high
full_path: CubeCTF_2025_WriteUp.full.md
meta_path: CubeCTF_2025_WriteUp.meta.md
images_removed: true
images_removed_count: 4
schema_version: v3.0.0-P0
summary: CubeCTF 2025 WriteUp。CubeCTF 2025 Operator取证题，pcap提取ELF后门逆向解密。关键路径：pcap筛选tcp.flags==0x0018 [PSH,ACK] → 发现攻击者执行id, nc -lvp 2025 > /tmp/xcat传输ELF → 三段TCP重组为ELF文件/tmp/xcat。经验：多段TCP重组需按顺序拼接；自写加密工具逆向需识别...
category: reverse
subcategory: reverse
tools_used:
- netcat
...
contest_type: open
wp_url: https://www.ctfiot.com/262340.html
reasoning_chain:
- 题目给 pcap 文件说 'someone has been hiding secrets' → 触发点：取证 pcap 分析
- 动作：tshark/wireshark 翻看 → 第 4 条 id 命令、第 6 条返回 uid=0(root) → 观察：攻击者已 root
- 动作：tcp.flags == 0x0018 [PSH,ACK] 过滤 → 观察：58338->1776 id / nc -lvp 2025 > /tmp/xcat
- 假设：47924->2025 三段 TCP 流是 nc 传输的 ELF 文件 → 动作：tshark -Y 'tcp.stream eq 1' 导出三段 → 顺序拼接
- 假设：xcat -l 2025 是加密后门 → 观察：后续 53930->2025 六段密文传到 /tmp/supersecret
- 动作：sub_15D0 函数处理 -l 参数 → socket+bind+listen+accept → 调 sub_1420 → 加密逻辑
- 假设：sub_15D0 调 sub_1420 加密发送 → 假设：sub_16A0 是解密入口（带 IP 参数）
- 动作：IDA 反编译 sub_16A0 看解密算法 → 假设：sub_15D0(sub_16A0(s1)) 双重函数调用结构
- 动作：写 Python 复现解密 → 观察：六段密文解出明文 → 完成
failed_attempts:
- 试图看 pcap 文本字段找 flag → 失败：flag 不在文本层，是密文
- 试图直接拼三段 TCP 流不按序 → 失败：TCP segment 是顺序依赖，重组需按 stream+seq
- 试图不解密读 ELF 字符串找 flag → 失败：ELF 是后门，flag 在密文里
key_observations:
- tcp.flags == 0x0018 过滤 [PSH,ACK] 是 CTF 取证快速定位交互流量标准手法
- nc -lvp PORT > /tmp/xcat 模式传输 ELF 时必须按 stream+seq 重组否则文件损坏
- sub_15D0/sub_16A0 双重函数调用模式（监听 vs 发送）是后门特征：监听者调加密发送函数
- sub_1420 是真正的加密函数，sub_15D0 只是 server 框架，逆向关键在 1420
- 攻击者已 root 后部署加密后门 = 典型 APT 取证场景
prerequisites:
- tshark/wireshark 命令行过滤（-Y / -T fields / tcp.stream eq）
- TCP 重组（按 stream index + seq 排序）
- IDA/Ghidra 反编译（FastCall 调用约定 / sub_15D0 模式）
- nc/socket 编程（socket+bind+listen+accept）
- AES/RC4/XOR 基础加密算法识别
---
