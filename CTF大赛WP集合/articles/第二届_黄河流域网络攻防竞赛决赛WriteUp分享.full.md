...
quality: high
full_path: 第二届_黄河流域网络攻防竞赛决赛WriteUp分享.full.md
meta_path: 第二届_黄河流域网络攻防竞赛决赛WriteUp分享.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: 第二届黄河流域网络攻防竞赛决赛WriteUp分享。AWDP赛制7题(WEB×4+PWN×3),核心链为SSRF+gopher+SQL注入+302跳转三层跳板打backup exec实现RCE,Flask密码爆破+SSTI,sign签名md5算法控制role+php://filter iconv爆破,Redis RESP+Pickle+SSTI三件套。经验：1) SSRF打本机需绕gopher...
category: web
...
contest_type: open
wp_url: https://www.ctfiot.com/195732.html
reasoning_chain:
- AWDP 赛制 7 题（WEB×4+PWN×3）→ 触发点：SSRF+gopher+SQL 注入+302 跳转三层跳板
- sitemap 题 → 假设：getPageInfoFromUrl SSRF + batchDelete SQL 注入 → 动作：UPDATE pages SET url=gopher+302 跳转
- 假设：click() 跳转 → 打 ApiContronller.php/backup exec → 动作：unzip-command='sh -c cat</flag|tee>flag'
- 简单渗透题 Flask Login 爆破 → 触发点：admin header 'admingivemeflag' → 假设：直接带 header 通过 → RCE
- 动作：admingivemeflag{os.popen('cat /flag').read()} → eval
- 重生之清华北大题 → 触发点：注册时间绕过 + sign md5 签名控制 role → 假设：role=admin + php://filter charset 爆破
- 动作：php://filter/convert.iconv.UTF8.CSISO2022KR 路径穿越 → include 读 flag
- 重生之最强管理员题 → 触发点：任意文件读取 + Redis RESP + Pickle + SSTI 三件套
- 假设：Redis RESP 协议 SET user:xxx base64(pickle.dumps({'passw0rd':'expexp','r0le':'adm1n'})) → SSTI author 字段触发
- PWN account 题 → 触发点：tcache_struct + environ 泄露栈 → 假设：禁用 execve → ORW_rop
- PWN alarm/fmt/just_a_heap 三大 pwn 题组合
- 动作：environ 泄漏栈地址 → 改返回地址为 orw_rop → 读 flag
failed_attempts:
- SSRF 直接 gopher://127.0.0.1:80/ → 失败：需 SQL 注入写入 DB + click 跳转
- Flask password 全 36 字符爆破 → 失败：results[0:short] 逐位爆更省力
- PHP 反序列化简单走 __destruct → 失败：必须走 SSTI + Redis RESP 三件套
key_observations:
- AWDP 赛制需修服务器 + 攻击别人双重挑战，常用 SSRF+gopher+SQL 三件套
- php://filter convert.iconv 串可逐层降维/升维，UTF8→CSISO2022KR 是经典起手
- Redis RESP 协议 + Pickle 反序列化 + SSTI 是 Web 综合高级套路
- seccomp 禁 execve 时 ORW 是 PWN 必备
- AWDP 防御方需打 patch，攻击方需抢分
prerequisites:
- AWDP 攻防赛制规则理解
- SSRF + gopher + SQL 注入 + 302 跳转综合
- Redis RESP 协议 + Pickle + SSTI 三件套
