...
- 祥云杯
- ezjava CC4链
- CommonsCollections4 TrAXFilter
- TemplatesImpl
- Javassist动态类
- DNS外带flag
- python_jwt CVE绕认证
...
quality: high
full_path: 第三届“祥云杯”CTF初赛WriteUp_By_EDISEC.full.md
meta_path: 第三届“祥云杯”CTF初赛WriteUp_By_EDISEC.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: 第三届"祥云杯"CTF初赛WriteUp By EDISEC。第三届祥云杯初赛Web：CC4链+Javassist动态类+DNS外带flag+python_jwt绕认证。经验：CC链curl/wget被ban时用URL+HttpURLConnection发GET请求外带flag到oastify
category: web
...
contest_type: open
wp_url: https://www.ctfiot.com/68739.html
reasoning_chain:
- 触发点：ezjava 是 Java 反序列化+curl/wget 被 ban→假设：必须用 URL 外带 flag
- 动作：构造 CC4 链 ChainedTransformer(ConstantTransformer(TrAXFilter.class)+InstantiateTransformer)+TransformingComparator+PriorityQueue→观察：触发反序列化
- 假设：用 Javassist 动态生成 AbstractTranslet 子类→动作：makeClassInitializer().insertBefore(cmd)→观察：静态块执行命令
- 假设：cmd 是读 /flag + URL 发 GET 到 oastify.com→动作：HttpURLConnection GET→观察：flag 通过 DNS 传出
- 假设：FunWEB 是 python_jwt CVE 绕认证→动作：alg=none 或 kid 注入→观察：拿到管理员 token
- 假设：内存马+OGNL 注入组合→动作：Filter/Servlet/Listener 注入→观察：完成
failed_attempts:
- 试图 curl -d @/flag 外带 → 失败：curl 命令被 ban
- 试图直接 Runtime.exec("bash -i") → 失败：反弹 shell 端口被 ban
- 试图用 CC1/CC6 链 → 失败：本题要求 CC4 (TrAXFilter)
key_observations:
- curl/wget 被 ban 时 URL+HttpURLConnection+GET 是 DNS 外带 flag 通用方案
- Javassist CtClass.makeClassInitializer().insertBefore(cmd) 是动态类静态块注入
- CC4 链 vs CC1/CC6 区别在 TrAXFilter 作为命令触发器
- oastify.com / Burp Collaborator 是 DNS 外带接收端标配
prerequisites:
- Java 反序列化 CC4 链(CommonsCollections4 + TrAXFilter)
- Javassist 动态类生成
- python_jwt 漏洞(CVE-2022-29217)
- OGNL 表达式注入与内存马
---
