...
quality: high
full_path: CTF专辑–花指令阅读与分析.full.md
meta_path: CTF专辑–花指令阅读与分析.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: 'CTF 专辑 – 花指令阅读与分析。花指令阅读与分析 — JunkCode 算数拆分 + push/pop/retn 等价解释 + 抵消型 (add-sub/xor-xor) + jmp db 型 (EB 03 跳 55 E8 AA) + 间接赋值 + IDA LinearSweep vs Recursive Traversal 反汇编算法差异。。关键路径：JunkCode: mov eax...'
category: reverse
subcategory: reverse
tools_used:
- IDA
...
contest_type: open
wp_url: https://www.ctfiot.com/194547.html
reasoning_chain:
- 触发点：反汇编看到 EB 03 55 E8 AA → 假设：EB 03 是 jmp +3 跳过 55（push ebp）+ E8 AA（call rel32）
- 动作：识别 jmp db 型花指令 → 观察：jmp 跳过垃圾字节真实指令在 jmp 后
- 下一步：识别等价替换 push 0x11111 → sub esp,4 + mov [esp],0x11111；retn 0x5 → mov eip,[esp] + add esp,0x9
- 假设：entrance enter 8,0 = push ebp + mov ebp,esp + sub esp,8 → 动作：手写还原 enter/leave 指令
- 动作：识别 add eax,5 → add eax,2 + add eax,1 + inc eax + inc eax 等算数拆分等价替换
- 下一步：识别抵消型 push-pop / add-sub / inc-dec / xor-xor → 动作：nopping 这些无意义配对
- 假设：IDA Linear Sweep 反汇编 vs Recursive Traversal 区别：前者逐行后者按代码顺序 → 动作：切换 IDA 配置查看
- 下一步：call+5 拿 eip 是经典 trick → 动作：mov reg, [esp]; ret 用 ret reg 或 add esp,4 跳过返回地址
failed_attempts:
- 试图用 IDA F5 自动反编译 → 失败：花指令破坏控制流图，F5 大片红 X
- 试图静态 patch 全部 jmp db → 失败：必须保留 jmp 跳过的真实指令
- 试图用 Pin/Angr 符号执行 → 失败：花指令路径爆炸，符号执行爆炸
key_observations:
- 花指令分 4 类：① 垃圾代码 ② 等价替换 ③ 抵消型 ④ jmp db 型
- IDA LinearSweep 逐行 vs Recursive Traversal 按执行顺序（前者易踩坑）
- call+5 拿 eip 是经典 trick（offset+5 = 下一条指令地址）
- 算数拆分 add eax,5 = add eax,2 + add eax,1 + inc eax + inc eax
- enter/leave 等价 push ebp + mov ebp,esp + sub esp,N
prerequisites:
- x86 汇编基础（mov/jmp/call/ret/push/pop）
- IDA 反汇编模式（LinearSweep vs Recursive Traversal）
- 花指令 4 大类型识别
- 等价替换模式（push/pop/retn/enter/leave/jmp）
- OllyDbg/IDA 动态调试识别跳转目标
---
# CTF专辑–花指令阅读与分析
