...
quality: medium
full_path: 第一届“长城杯”信息安全铁人三项赛线下决赛-_WriteUp.full.md
meta_path: 第一届“长城杯”信息安全铁人三项赛线下决赛-_WriteUp.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: 第一届“长城杯”信息安全铁人三项赛线下决赛- WriteUp。第一届长城杯信息安全铁人三项赛线下决赛：12+flag企业环境渗透+取证溯源综合WP。经验：铁人三项赛综合渗透：企业环境+移动应用+取证溯源
category: misc
...
contest_type: open
wp_url: https://www.ctfiot.com/183955.html
reasoning_chain:
- 触发点：12+ flag(flag1-14) 三台靶机(202.0.6.193-195)→假设：企业环境渗透 + 多 flag 综合
- 动作：端口扫描发现 22/80/3306/3389→观察：多服务暴露
- 假设：80 端口应用含 php://filter LFI→动作：?file=php://filter/read=convert.base64-encode/resource=flag.php→观察：读到 flag.php 源码
- 假设：202.0.5.236 是第二阶段取证环境→动作：SSH root:Szivfn4bLZ 登录→观察：进系统
- 假设：取证溯源关键在 data2024.zip 流量包→动作：tshark 过滤 HTTP 找攻击 IP→下一步：解关卡 1
- 假设：flag 散布于多台机器+多阶段→动作：批量验证 flag1-14 →观察：完成
failed_attempts:
- 试图直接 ssh root 空密码 → 失败：密码在第二阶段才给出
- 试图爆破 mysql 弱口令 → 失败：3306 是 mysql 5.7 默认拒绝远程
key_observations:
- 铁人三项综合赛典型结构：企业环境渗透(多 flag)+ 取证溯源(SSH/VNC)
- php://filter/read=convert.base64-encode/resource=X.php 是 PHP LFI 读源码经典 payload
- 本题含多 flag (1-14)，需要逐个对应不同靶机/阶段
- 取证阶段常给 SSH root:Szivfn4bLZ + data2024.zip 流量包组合
prerequisites:
- 端口扫描(nmap)
- PHP LFI / php://filter 利用
- SSH/VNC 远程取证
