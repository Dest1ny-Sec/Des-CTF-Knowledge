...
quality: high
full_path: 看雪2022_KCTF_秋季赛_-_第十题设计思路及解析.full.md
meta_path: 看雪2022_KCTF_秋季赛_-_第十题设计思路及解析.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: 看雪2022 KCTF 秋季赛 | 第十题设计思路及解析。KCTF2022秋Q10《两袖清风》：AES前4枚举+RSA后缀解+shellcode验证+迷宫路径算法。经验：python+vc混合枚举速度；AES-CBC shellcode按执行结果筛选key
category: reverse
subcategory: reverse
tools_used:
- Python
...
contest_type: open
wp_url: https://www.ctfiot.com/85185.html
reasoning_chain:
- 触发：两段 shellcode (sc1/sc2) + AES-CBC 加密 + RSA 私钥
- 假设：RSA n 可快速分解 → 动作：求私钥
- 假设：私钥解后 = AllIsNothing（后缀） → 假设：前 4 字符枚举
- 假设：前 4 字符 (0-9) 横向纵向相加相等 → 动作：python+vc 混合枚举
- 观察：133 秒得 6 结果 → 假设：选能正常执行 shellcode 的 1010
- 假设：AES key=1010AllIsNothing 解密 sc1 → 生成 10x10 迷宫
- 假设：sc2 验证路径 30 31 32 33 43 53 52 51 61 71 72 73 74 84 94 → 完成
failed_attempts:
- 试图 Python 单独枚举 → 失败：纯 python 太慢
- 试图固定前 4 字符不解 → 失败：sc1 异常执行
- 试图绕 RSA 直接试 key → 失败：后半部分必须 RSA 解
key_observations:
- python+vc 混合枚举速度（python 脚本调 vc 加速）
- AES-CBC shellcode 按执行结果筛选 key（不异常）
- RSA n 快速分解（factordb / yafu）→ 私钥 → 后缀解
- 10x10 迷宫 + 路径验证是 sc2 核心
- IV=ABCDEF0123456789 固定
prerequisites:
- AES-CBC 算法 + IV/Key 关系
- RSA n 分解（yafu/msieve/factordb）
- 迷宫生成算法理解 + 路径搜索
- python + ctypes 调用 vc 加速
---
