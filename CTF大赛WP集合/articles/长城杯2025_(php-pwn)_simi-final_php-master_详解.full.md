...
quality: high
full_path: 长城杯2025_(php-pwn)_simi-final_php-master_详解.full.md
meta_path: 长城杯2025_(php-pwn)_simi-final_php-master_详解.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: 长城杯2025 (php-pwn) simi-final php-master 详解。长城杯2025 simi-final php-master详解:PHP文件上传+Docker gdbserver调试+zend内存管理(heap+bin+free_slot)逆向+vuln.so扩展的_efree.got劫持为system→/readflag>/var/www/html/flag。经验：1)...
category: pwn
...
contest_type: open
wp_url: https://www.ctfiot.com/249023.html
reasoning_chain:
- 题目给 docker image + web 目录 → 触发点：PHP 文件上传 + 自定义扩展 vuln.so
- 假设：move_uploaded_file 无 filter 直接写 web 目录 → 动作：读 PHP 源码
- 观察：扩展函数 construct/allocate/clear/overwrite 操控 zend 内存
- 假设：扩展是 _emalloc/_efree hook → 动作：IDA 反编译 vuln.so
- 观察：zend_mm_heap 结构 use_custom_heap + free_slot[ZEND_MM_BINS]
- 假设：gdbserver 远程调试暴露栈/堆 → 动作：docker run -p 9191 + gdb attach
- 动作：/proc/self/maps 正则提 libc_base + vuln.so_base
- 假设：覆盖 vuln.so _efree.got = system → 动作：overwrite(addr, system)
- 观察：free(内容='/readflag>/var/www/html/flag\x00') → flag 文件落地
failed_attempts:
- 试图直接覆盖 libc __free_hook → 失败：glibc 2.27+ 已弱化 hook
- 试图不开 gdbserver 静态分析 → 失败：vuln.so 混淆需运行时栈
key_observations:
- PHP-PWN 核心是扩展 vuln.so 内 _efree.got → system 劫持
- zend_parse_parameters 类型字符 l/s/b/z/a/O/| 是 PHP 扩展签名
- 'zend_mm_heap 三层 _emalloc: small(bin) → large(页) → huge'
- gdbserver + docker port 映射是 PHP/容器 PWN 调试标配
prerequisites:
- PHP 扩展开发（zend_parse_parameters / zval）
- glibc 堆管理（hook / GOT hijack）
- Docker + gdbserver 远程调试
- pwntools ELF/GOT 符号读取
---
