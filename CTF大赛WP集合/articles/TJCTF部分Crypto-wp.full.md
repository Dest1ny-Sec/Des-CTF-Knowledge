...
quality: medium
full_path: TJCTF部分Crypto-wp.full.md
meta_path: TJCTF部分Crypto-wp.meta.md
images_removed: true
images_removed_count: 0
schema_version: v3.0.0-P0
summary: TJCTF 部分 Crypto-wp。TJCTF 2025 部分 Crypto：4 题 Baconian + 自定义 RC4 + 多模数 RSA + LCG 伪随机爆破。。关键路径：Baconian 加密 + 大小写编码 → chr(ord(c)-13) 减位 → 解码 → 大写/小写对应 1/0。经验：多模数 RSA 当 e 较小时 e 次方根可解；LCG 种子若用 time.asctim...
category: crypto
subcategory: rsa
tools_used:
- C
...
contest_type: open
wp_url: https://www.ctfiot.com/273223.html
reasoning_chain:
- 拿到 out.txt → 触发点：每个字符是 chr(ord(c)-13) 偏移 → 假设：chr+13 还原
- 还原后字符有大写有小写 → 假设：大写=1 小写=0 是 Baconian 编码 → 动作：5 字符一组映射回二进制 → 映射回 a-z
- 观察：解出 'tictfoinkooinkoooinkooooink' → 假设：'tictf' 应是 'tjctf' 笔误 → flag = tjctf{oinkooinkoooinkooooink}
- alchemist-recipe 自定义 RC4-like → SHA256 派生 key + 256 字节 S-box → 假设：密钥派生 + S-box 置换可逆
- 动作：deschuffle 排序还原 + XOR 还原 + S-box 逆映射 → 观察：解密成功
- 多模数 RSA 不同 N 同 e → 触发点：C1 C2 多份密文 → 假设：CRT 合并 + e 次方根
failed_attempts:
- 试图直接 chr+13 后得明文 → 失败：还需要大小写→Baconian 二进制→字母映射
- 试图单密码爆破 RC4 key → 失败：key 派生涉及 SHA256
key_observations:
- Baconian 编码可用字符大小写 1bit/字符承载信息
- 多模数 RSA 当 e 较小时 e 次方根 (iroot) 可解
- LCG 种子用 time.asctime() 时 2 小时窗口可爆破
prerequisites:
- Baconian 5-bit 字母编码
- RC4 KSA + PRGA 流程
- RSA 多模攻击与 LCG 时间种子爆破
---
# TJCTF部分Crypto-wp
