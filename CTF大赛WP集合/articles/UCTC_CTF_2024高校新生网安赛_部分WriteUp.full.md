...
- 码表替换
attack_chain:
- 'MISC 神秘的文件: 010 Editor 看到 zip 头被反转 → reverse() 还原 → 改头 50 4B 03 04 → 解压'
- 'PWN EZ_Stack: 栈溢出 offset=0x4c → pop_rdi_ret + /bin/sh + system'
- 'PWN userlogin: format string %13$s 泄漏 rootPassword → buffer overflow ret2backdoor 0x401262'
- 'REVERSE hash&py: pyinstxtractor 解包 → pyc 修 magic 0x0A0D0D55 (3.8) → md5 4 段拼接'
- 'REVERSE just_re_java: jadx 看 smali → Enc.ah 改码表 base64 → Flag 类 $() 拼结果'
...
quality: high
full_path: UCTC_CTF_2024高校新生网安赛_部分WriteUp.full.md
meta_path: UCTC_CTF_2024高校新生网安赛_部分WriteUp.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: 'UCTC CTF 2024 高校新生网安赛 部分 WriteUp。UCTC 高校新生赛 5 大题：MISC zip 反转 + PWN 栈溢出 + 格式化串 + py 反编译 + Java smali。关键路径：MISC 神秘的文件: 010 Editor 看到 zip 头被反转 → reverse() 还原 → 改头 50 4B 03 04 → 解压 → PWN EZ_Stack: 栈溢出 ...'
category: forensic
...
contest_type: school
wp_url: https://www.ctfiot.com/218709.html
reasoning_chain:
- MISC 神秘文件 → 用 010 Editor 看到 zip 头字节序反转 → 触发点：文件头异常
- 假设：数据被 reverse() 处理 → 动作：Python 反向拼接字节流 + 改头 50 4B 03 04 → 观察：能正常解压
- PWN EZ_Stack → 栈溢出 offset=0x4c → pop_rdi_ret 链 ROP → 假设：直接 ret2libc
- 动作：构造 '/bin/sh' + system payload → 观察：弹 shell 得 flag
- PWN userlogin → menu 函数 print('username=' + input) → 触发点：format string
- 假设：%13$s 泄漏栈上 rootPassword → 动作：%13$s payload → 观察：拿到 password 后 buffer overflow
- 动作：ret 到 backdoor 0x401262 → 观察：返回 flag
- REVERSE hash&py → PyInstaller 包 → 触发点：pyinstxtractor 解包
- 假设：pyc magic 0x0A0D0D55 → Python 3.8 → 动作：修 magic 反编译 → 观察：看到 md5(4 段) 拼接逻辑
- 动作：还原 md5 链 → 观察：得到原始 flag
- REVERSE just_re_java → jadx 看 smali → Enc.ah 调改码表 base64 → Flag 类 $() 拼结果
failed_attempts:
- 试图直接 jadx 反编译 pyinstxtractor 结果 → 失败：magic 不对，反编译报错
- 试图用 %p 泄漏栈 → 失败：%s 才能拿字符串指针
key_observations:
- 文件头字节反转是常见出题手法，010 Editor 改头最直观
- print 函数的 format string 漏洞用 %13$s 偏移定位密码
- PyInstaller 反编译需要修 pyc magic 才能被 decompyle3 接受
- md5 多段拼接 + base64 改码表 = 新生赛经典 reverse 套路
prerequisites:
- 010 Editor 文件头查看基础
- pwntools ROP 链构造
- Python 3.8 pyc magic 修复
- jadx 反编译 smali 基础
---
