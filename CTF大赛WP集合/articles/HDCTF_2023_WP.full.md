...
quality: high
full_path: HDCTF_2023_WP.full.md
meta_path: HDCTF_2023_WP.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: 'HDCTF 2023 WP。HDCTF 2023 3道MISC：zsteg+zip CRC爆破+分卷flag。关键路径：hardMisc: zsteg得到数据解base64 → flag: HDCTF{wE1c0w3_10_HDctf_M15c} → ExtremeMisc: IDAT.png文件尾分离zip。经验：zip CRC爆破+明文攻击(bkcrack)组合解分段压缩'
category: misc
...
contest_type: open
wp_url: https://www.ctfiot.com/111656.html
reasoning_chain:
- hardMisc 题拿到 png → 触发点：zsteg LSB 隐写可能性
- 动作：zsteg -a → 观察：得到一串隐藏文本 → 假设：base64 编码
- 解 base64 → 观察：HDCTF{wE1c0w3_10_HDctf_M15c}
- ExtremeMisc → 触发点：IDAT.png 文件尾分离出 zip → 假设：追加数据格式
- 动作：binwalk/foremost 分离 → 观察：得到 Dic.zip
- Dic.zip 提示字典爆破 → 假设：rockyou 字典 → 动作：fcrackzip -D -p rockyou.txt
- Reverse.piz 字节倒序 → 触发点：解两位一组倒序算法 → 假设：每对字节 swap
- 动作：int((('%02x'%i)[::-1],16)).to_bytes(1,'little') → 观察：得到 reverse.zip
- bkcrack 明文攻击 → 触发点：已知密文 + 明文 → 假设：构造 12 字节明文
- MasterMisc → 分卷压缩爆破 → 触发点：topic.png 末尾拼接 png+wav+png
- 动作：分离 wav + 改 png 高度 → 观察：第二段 flag NSSCTF{e67d8104-...}
failed_attempts:
- 试图不解 base64 直接看 zsteg → 失败：zsteg 输出是 base64
- 用普通字典爆破 Dic.zip → 失败：必须用 rockyou
- 不解字节倒序直接 bkcrack → 失败：bkcrack 需要正确密文
key_observations:
- zsteg 是 LSB 隐写标配工具
- 文件尾追加 zip/binwalk 分离是 MISC 入门
- rockyou 字典爆破短密码 zip 是常见路线
- bkcrack 明文攻击需构造已知明文
- PNG 高度字节位修改可恢复被裁剪的图像
prerequisites:
- zsteg / binwalk 工具使用
- base64 解码基础
- bkcrack 明文攻击原理
