...
quality: high
full_path: 2023_强网杯_writeup_by_Arr3stY0u.full.md
meta_path: 2023_强网杯_writeup_by_Arr3stY0u.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: '2023 强网杯 WriteUp by Arr3stY0u。12+ 道题：RSA p^5+BSGS+ADS-B+pyModeS+XTEA+VM+LD_PRELOAD+phoenixAES+ARC4+ISIS 论文题。。关键路径：not only rsa: n=p^5, e=641747, sage Mod(c,n).nth_root(e, all=True) 找 flag → discret...'
category: misc
...
contest_type: open
wp_url: https://www.ctfiot.com/152049.html
wp_author: Arr3stY0u
reasoning_chain:
- 'RSA p^5: n = p^5, e=641747 → 触发点：n 是素数 5 次方'
- 假设：sage Zmod(p^5) 直接 nth_root(e) → 动作：K=Zmod(p^5); a=K(c).nth_root(e); b=K(1).nth_root(e)
- 观察：a*b=e 次方根 → flag{c19c3ec0-d489-4bbb-83fc-bc0419a6822a}
- 'BSGS qwb Discrete Log: g^x = c mod p, flag 填充已知 → 假设：p-1=2q 特殊 DLP'
- 动作：sage bsgs(g, c, p) → 观察：去掉填充得到 flag
- 'ADS-B pyModeS: 飞机 ADS-B 报文 → 假设：pyModeS 解码 ICAO + altitude'
- 动作：pyModeS 解码 → 观察：flag 在飞机代码
- 'XTEA 32 轮: 标准 XTEA + delta=0x9E3779B9 + 32 轮 → 动作：解 XTEA → 观察：flag'
- 'LD_PRELOAD 拦截: 加载自定义 so 拦截 libc 函数 → 动作：写 hook.c → 观察：拦截 flag'
- 'phoenixAES: AES 末轮 key 已知 → 假设：反推完整 key schedule'
- 动作：phoenixAES.py decrypt → 观察：flag
- 'QAM 音频 flag.wav: I/Q 数据映射 → 假设：±3/±1 是 4-QAM → 动作：2-bit 解码 → flag'
failed_attempts:
- 试图直接 nth_root(c, e) → 失败：必须 mod p^5 不是 mod n
- 试图 Pollard rho → 失败：n 不是合数
- 试图标准 AES 解密 → 失败：末轮 key 是反推的
key_observations:
- RSA p^5 用 sage nth_root 一步搞定
- AES 末轮 key 已知反推完整 key schedule
- XTEA 32 轮是入门题标配
- pyModeS 是 ADS-B 解码标配库
prerequisites:
- Sage nth_root (mod p^k)
- BSGS 算法 + DLP
- XTEA 加密原理
- AES key schedule 反推 (phoenixAES)
---
