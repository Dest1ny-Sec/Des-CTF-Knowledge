...
quality: high
full_path: 【2026春节】全自动AI做题的实现及初8逆向AIAgent对话记录及wp.full.md
meta_path: 【2026春节】全自动AI做题的实现及初8逆向AIAgent对话记录及wp.meta.md
images_removed: true
images_removed_count: 7
schema_version: v3.0.0-P0
summary: 【2026春节】全自动AI做题的实现及初8逆向AI Agent对话记录及wp。2026 春节 Tokeii 的全自动 AI Agent 做题系统分享，逆向题 Android JNI 实战。。经验：AI Agent 多模型协作可降低单模型偏差；常见工具 calling func 大幅减少 token；OpenViki...
category: reverse
subcategory: reverse
tools_used:
- IDA
...
contest_type: open
wp_url: https://www.ctfiot.com/304188.html
reasoning_chain:
- 题目8 → Android APK → 触发点：jadx 反编译 Aclasses.dex (Java/Kotlin)
- 假设：Java 层调用 native 方法 → 动作：IDA 分析 libhajimi.so (x86_64) 找 JNI 函数
- 观察：定位到关键 native 函数 JNI_OnLoad + 调用点
- 动作：提取密钥和常量（debug_key, AES_IV 等）→ 观察：拿到 crypto 参数
- 假设：Java 层验证逻辑调 native → 动作：写脚本调用 native 库 → 观察：解出 flag
- AI Agent 多模型协作 + 总结反思 + 点子系统 → 触发点：单模型偏差大
- 假设：OpenViking 上下文压缩 → 动作：实现 calling func 减少 token → 观察：稳定解题
failed_attempts:
- 试图单模型直接解题 → 失败：复杂逆向需多模型交叉验证
- 试图不写 calling func → 失败：token 消耗过大
key_observations:
- AI Agent 多模型协作可降低单模型偏差
- 常见工具 calling func 大幅减少 token
- OpenViking 上下文压缩是会话管理参考实现
- Android JNI 逆向两件套 jadx (Java) + IDA (native) 是标准流程
prerequisites:
- jadx 反编译 Android APK
- IDA 分析 .so 多架构
- JNI 调用栈理解
- 多模型 AI Agent 协作框架设计
---
