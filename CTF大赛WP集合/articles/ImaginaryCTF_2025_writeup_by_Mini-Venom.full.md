...
lesson: '|'
quality: medium
full_path: ImaginaryCTF_2025_writeup_by_Mini-Venom.full.md
meta_path: ImaginaryCTF_2025_writeup_by_Mini-Venom.meta.md
images_removed: true
images_removed_count: 8
schema_version: v3.0.0-P0
summary: ImaginaryCTF 2025 Pwn by Mini-Venom。ImaginaryCTF 2025 Pwn 多题 (ret2libc 模板 / GOT patch / 多次栈迁移 / fmt-string) 速查。。经验：|
category: pwn
...
contest_type: open
wp_url: https://www.ctfiot.com/269585.html
wp_author: Mini-Venom
reasoning_chain:
- babybof：NX + Canary + libc → 触发点：ret2libc 标准链
- 动作：泄露 canary + libc → 假设：payload = 'A'*0x38 + canary + pop_rdi + binsh + ret + system
- 观察：拿到 shell → ictf{arent_challenges_written_two_hours_before_ctf_amazing}
- addition：atoll.got → system 偏移 → 触发点：GOT patch
- 假设：写入 atoll.got = system → 动作：add where 0x4020-0x4069 / add what (system-atoll) / add where /bin/sh
- 观察：atoll("/bin/sh") 等价 system("/bin/sh") → ictf{i_love_finding_offsets_4fd29170cb90}
- cascade：需要 one_gadget + 多次栈迁移 → 触发点：栈空间不足
- 动作：payload1='a'*0x40+0x404600+leave_ret → 假设：迁移到 bss → 链式栈迁移 + magic gadget 拼 one_gadget → 完成
- twowrite：___stack_chk_fail.got → gets → 触发点：canary 错误触发 gets → 触发 canary 异常 → 调用 gets 读入 /bin/sh → ret2libc
- Web imaginary-notes：Supabase anon key + URL → 动作：JS 里直接查 users 表 → 完成
- Crypto redacted：单字节 XOR cycle → 假设：key 重复模式 → 观察：ictf{xor_is_bad_bad_encryption}
- Crypto leaky-rsa：题目输出 key_m → 假设：交互 1025 次取最后一次 key_m → AES 解密 → Reverse comparing：奇偶数 ascii 拼接还原 flag
failed_attempts:
- babybof 直接 ret2shellcode → 失败：NX
- twowrite 直接改 __stack_chk_fail → gets 不可控大小
- leaky-rsa 用单次输出解 → 失败：key_m 必须 1025 次取最后一次
key_observations:
- ret2libc 标准模板：canary leak + pop rdi + system(/bin/sh)
- GOT patch 是绕过 NX + Canary 的最干净路径（不用暴露 libc）
- 多次栈迁移 + magic gadget 拼 one_gadget 是栈空间受限的解法
- Supabase anon key 默认能读所有表，JS 端泄漏 anon key = 直接查库
- AES key_m 取最后一次 = 攻击者利用服务侧错误恢复明文
prerequisites:
- ret2libc 模板（canary / pop rdi / ret / system）
- GOT patch + 栈迁移 + magic gadget
- Supabase anon key 利用 + AES-CBC + PKCS7 padding
---
