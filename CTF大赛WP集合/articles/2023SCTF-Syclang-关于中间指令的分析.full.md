...
quality: medium
full_path: 2023SCTF-Syclang-关于中间指令的分析.full.md
meta_path: 2023SCTF-Syclang-关于中间指令的分析.meta.md
images_removed: true
images_removed_count: 7
schema_version: v3.0.0-P0
summary: 2023 SCTF Syclang 关于中间指令的分析。Syclang 自定义 IR 逆向：4 轮差分/加减/累加/索引置乱混合加密。。关键路径：解析 Syclang 自定义 IR 中间指令语法 → 识别 4 个 exp 结构体 (var22/var23/var24/var25) → var22.L/R/X 8 组索引+偏移量硬编码。经验：自定义 IR 逆向关键是识别循环结构 + 操作类型，...
category: reverse
subcategory: reverse
...
contest_type: open
wp_url: https://www.ctfiot.com/125684.html
reasoning_chain:
- Syclang 自定义 IR → 触发点：800 行 IR 看似 Go 风格
- 假设：分析典型指令如 STRUCT exp + FUNCTION main + LABEL/IF/GOTO → 动作：识别循环结构
- 观察：每个循环都是 for (i = 0; i < 24; i++) 输入 → var22.key[23-i] = input[i]
- 假设：4 个 exp 结构体存储中间态 → 动作：跟踪 key[i] 跨结构体传递
- 观察：第二轮循环 var22.L[i] += var22.key[i] + var22.key[i+1]（差分累加）→ 假设：差分加密
- 动作：第三轮加减 + 第四轮索引置乱 → 观察：逆向解密
- 动作：写 Python 解密脚本反向 4 轮操作 → 观察：得到 flag
failed_attempts:
- 试图用 IDA 反编译 Syclang → 失败：自定义 IR 非标准
- 试图 Ghidra 反编译 → 失败：自定义 bytecode
- 试图直接对 800 行 IR 做模式匹配 → 失败：循环结构需手动识别
key_observations:
- 自定义 IR 逆向关键是识别循环结构 + 操作类型
- 差分加密逆向就是反向累加
- STRUCT/FUNCTION/LABEL 模式是自定义字节码常见语法
- <+offset> 表示结构体内偏移，理解即可
prerequisites:
- 自定义 IR/字节码阅读能力
- 差分加密原理
- C 语言循环结构与变量作用域
