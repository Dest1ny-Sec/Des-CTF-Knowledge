...
- call_5_flower
- getprocaddress_via_sm3
- tls_callback
- child_debugger_debug_only_this_process
- int3_exception_vm
- excep_breakpoint
- excep_access_violation
- pusha_popa_vm
...
quality: high
full_path: '[HITCTF_2022]re3_–_debug_maze详细Writeup.full.md'
meta_path: '[HITCTF_2022]re3_–_debug_maze详细Writeup.meta.md'
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: '[HITCTF 2022] re3 – debug_maze 详细 Writeup。HITCTF 2022 re3 debug_maze 逆向：IsDebuggerPresent 抗反调试 + call $+5 花指令 + PEB 找 kernel32 + SM3 模拟 GetProcAddress + TLS 异或解密 + 子进程调试 + int 3 trap 三寄存器 VM 模拟。。经验...'
category: misc
...
contest_type: open
wp_url: https://www.ctfiot.com/87278.html
reasoning_chain:
- 触发点:Win32 控制台程序 + IsDebuggerPresent → 假设:有 anti-debug → 动作:看入口点花指令
- 观察:call $+5 是经典花指令 → 假设:跟 PEB 找 kernel32 → 动作:静态分析 mul eax 两次
- 动作:跟 PEB->Ldr->InMemoryOrderModuleList → 观察:找到 kernel32.dll + GetProcAddress 模拟 (用 SM3 哈希解析)
- 触发点:TLS 回调修改字符串 → 假设:字符串被异或 → 动作:静态 dec_xored (第 1 字节往后 ^ 第 0 字节)
- 观察:还原字符串 → 下一步:看子进程调试
- 触发点:DEBUG_ONLY_THIS_PROCESS 子进程调试父进程 → 假设:int 3 trap 触发 EXCEPTION_BREAKPOINT → 动作:看 int 3 trap 处理
- 动作:EXCEPTION_BREAKPOINT case 1=jnz 2=push 4=pop 8=correct 0x10=mul 0x20=wrong + EXCEPTION_ACCESS_VIOLATION case 1=add 2=addn 4=store 8=load 0x10=cmp 0x20=jmp → 观察:VM 指令映射清晰
- 动作:pusha/popa 三寄存器 VM (ecx/eax/ebx) 模拟执行 → 观察:跑出 HITCTF2022{4311254395e7d3cf0b372d95b58325674d6117}
failed_attempts:
- 试图直接 OD 挂 → 失败:IsDebuggerPresent + 子进程调试反转
- 试图用 x64dbg 二进制 patch → 失败:花指令 call $+5 干扰
- 试图不解 TLS 回调看字符串 → 失败:字符串已被异或
key_observations:
- call $+5 是经典花指令,'执行 mul 一次' 配合 PEB 找 kernel32 是 2022 流行 Win 抗反调试组合
- EXCEPTION_BREAKPOINT/EXCEPTION_ACCESS_VIOLATION 双异常分发做 VM 是 2022 re3/anti-RE 标配
- TLS 回调改字符串 + dec_xored (异或链) 是 Win 抗反调试通用手法
- DEBUG_ONLY_THIS_PROCESS 让子进程调试父进程是 anti-debuging 反转
- pusha/popa 三寄存器 VM (ecx/eax/ebx) 是 Win VM 逆向经典模型
prerequisites:
- Windows 反调试 (IsDebuggerPresent / PEB.NtGlobalFlag)
- TLS 回调机制 + 字符串异或自解密
- EXCEPTION_BREAKPOINT / EXCEPTION_ACCESS_VIOLATION 双异常分发
- Win VM 逆向 (pusha/popa 三寄存器模型)
---
