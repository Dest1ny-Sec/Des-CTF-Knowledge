---
title: DataCon24漏洞赛道冠军分享：vuln_wp——大模型赋能的漏洞自动化分析全解析
contest: DataCon 2024 漏洞分析赛道冠军
year: 2024
difficulty: hard
vuln_type: iot
tags:
- llm
- rag
- prompt-engineering
- voting
- race-condition
- sql-injection
- double-fetch
- buff-overflow
attack_chain:
- 0817iotg战队完整框架
- BeautifulSoup4解析HTML过滤图片
- 提示工程多维度输出+两次LLM调用
- 内置投票机制剔除低置信度结果
- 扩展：版本/修复建议/POC/EXP/图像
- 文件大小排序+类型过滤+按语言函数切分
- 提示工程初步筛查+种子函数深度解析
- 不同漏洞类型(race condition/SQLi/double-fetch/buff-overflow/cmd-injection)不同策略
- RAG技术+多轮提示+结果投票
key_payload: RAG + 多次LLM调用 + 投票 + 漏洞类型策略
one_liner: DataCon24冠军 vuln_wp：LLM+RAG+投票+多类型漏洞策略
lesson: 漏洞分析LLM框架：HTML解析+提示工程+投票+RAG
quality: high
full_path: DataCon24漏洞赛道冠军分享：vuln_wp——大模型赋能的漏洞自动化分析全解析.full.md
meta_path: DataCon24漏洞赛道冠军分享：vuln_wp——大模型赋能的漏洞自动化分析全解析.meta.md
images_removed: true
images_removed_count: 4
schema_version: v3.0.0-P0
summary: DataCon24漏洞赛道冠军分享：vuln_wp——大模型赋能的漏洞自动化分析全解析。DataCon24冠军 vuln_wp：LLM+RAG+投票+多类型漏洞策略。关键路径：0817iotg战队完整框架 → BeautifulSoup4解析HTML过滤图片 → 提示工程多维度输出+两次LLM调用。经验：漏洞分析LLM框架：HTML解析+提示工程+投票+RAG
category: misc
subcategory: misc_other
time_required: long
difficulty_score: 4
code_blocks_count: 0
images_count: 4
last_verified: 2026-09-20
contest_type: open
wp_url: https://www.ctfiot.com/233750.html
reasoning_chain:
- 海量漏洞文章 + 庞大代码库 → 触发点：LLM 自动化漏洞分析
- 假设：HTML 文章解析需过滤 → 动作：BeautifulSoup4 过滤图片等无关标签
- 观察：信息维度多 → 假设：拆维度多次 LLM 调用比单次更准 → 动作：两次 LLM 分别判定
- 假设：LLM 输出有不确定性 → 动作：内置投票机制剔除低置信度
- 代码侧：文件大小排序 + 类型过滤 + 函数切分 → 假设：按语言粒度拆函数更易分析
- 提示工程初步筛查种子函数 → 假设：不同漏洞类型策略不同 → 动作：race condition/SQLi/double-fetch/buff-overflow/cmd-injection 各自模板
- 复杂漏洞引入 RAG + 多轮提示 + 结果投票 → 假设：背景知识 + 多次确认降误报
failed_attempts:
- 试图单次 LLM 提所有维度 → 失败：单次输出维度过多，准确率下降
- 试图不用 RAG 解决复杂漏洞 → 失败：上下文不够，幻觉严重
key_observations:
- HTML 解析过滤 + 提示工程多维度 + 投票 = LLM 漏洞分析稳定框架
- RAG + 多轮提示 + 投票是降低 LLM 幻觉的三板斧
- 不同漏洞类型（race condition/SQLi/double-fetch）需不同 prompt 策略
- 函数粒度切分让 LLM 上下文更聚焦
prerequisites:
- BeautifulSoup4 HTML 解析
- LLM 提示工程与多轮调用
- RAG（检索增强生成）原理
- 漏洞分类（race condition/SQLi/double-fetch/buff-overflow/cmd-injection）
---
# DataCon24漏洞赛道冠军分享：vuln_wp——大模型赋能的漏洞自动化分析全解析

> 原文: https://www.ctfiot.com/233750.html
> ID: 233750

https://github.com/123f321/datacon24_vuln_wp

https://github.com/123f321/datacon24_vuln_wp

在网络安全和漏洞挖掘领域，海量漏洞分析文章和庞大的代码库常常让传统人工和静态分析方法捉襟见肘。DataCon24_vuln_wp 正是为了解决这一问题而设计，利用大模型的深度语义理解能力，实现自动化、智能化的漏洞检测与信息提取。

项目基于赛道冠军战队 0817iotg 的完整解题框架，融合了最新的提示工程方法和多轮大模型调用机制，旨在降低误报、提高漏洞识别的准确率和效率。

利用 BeautifulSoup4 对 HTML 进行解析，精准过滤图片等无关内容。

采用精细化的提示工程方法，将信息拆分成多个输出维度，使用两次大模型调用分别判定，确保提取结果准确且具备多角度分析。

内置投票机制，对多轮大模型输出进行校验，自动剔除不合理或低置信度结果。

扩展功能：除基础信息外，还能识别版本信息、修复建议，并从文章中提取 POC/EXP 代码和图像内容。（详见 task1_source_code 中的 test.py、appendix.py 及 pic_expand.py）

先对待分析文件进行大小排序、类型过滤，再依据编程语言进行函数级别的切分，确保每个代码片段都能单独分析。

利用提示工程进行初步筛查，提取可能包含漏洞的种子函数；对种子函数内容进行深度解析，针对不同漏洞类型（如 race condition、SQL 注入、double-fetch、buff_overflow、command_injection 等）采用不同分析策略。

部分复杂漏洞类型引入 RAG 技术，结合多轮提示与结果投票，有效降低误报，并输出最有可能的漏洞函数供后续手工验证或自动修复。

利用最新大模型技术和提示工程，实现高效、精准的信息提取与漏洞定位。

通过多次调用大模型和投票比对，显著降低输出不确定性，提高漏洞检测准确率。

提供完整的 Docker 压缩包，无需复杂环境配置，支持快速部署和现场测试。

代码结构清晰、模块化设计，方便开发者二次开发、扩展与定制。

内置针对多种漏洞类型的示例与测试数据，为新手和资深安全研究者提供了宝贵的实战参考。

访问 GitHub 仓库 https://github.com/123f321/datacon24_vuln_wp 下载源代码和 Docker 镜像。

欢迎 Star ⭐、Fork 🍴、Watch 👀，关注项目动态；提出 Issue 或提交 PR，大家一起完善自动化漏洞分析工具。

利用该框架进行漏洞挖掘与安全审计，将实战经验分享到社区，共同推动安全技术的进步。

https://github.com/123f321/datacon24_vuln_wp

---
## 附图

[图片已移除]
[图片已移除]
[图片已移除]
[图片已移除]