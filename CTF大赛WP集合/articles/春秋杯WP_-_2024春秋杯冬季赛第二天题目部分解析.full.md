...
quality: high
full_path: 春秋杯WP_-_2024春秋杯冬季赛第二天题目部分解析.full.md
meta_path: 春秋杯WP_-_2024春秋杯冬季赛第二天题目部分解析.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: 春秋杯WP | 2024春秋杯冬季赛第二天题目部分解析。春秋杯2024冬季赛D2十题混合：Python jail+字符高尔夫+线性方程组+PyVM逆向。经验：Python沙箱逃逸核心：生成器gi_frame+f_back回溯+f_globals取全局；字符高尔夫用Unpacki...
category: crypto
subcategory: math
tools_used:
- Python
...
contest_type: open
wp_url: https://www.ctfiot.com/224937.html
reasoning_chain:
- python_jail：沙箱逃逸 → 触发点：生成器 gi_frame.f_back 可回溯栈帧
- '动作：def b(): def a(): yield g.gi_frame.f_back.f_back.f_back.f_back'
- 观察：嵌套 4 层 f_back 拿到外层 globals → 假设：BOX 在 globals
- 动作：g=[x for x in g][0]; return g.f_globals['BOX'] → 拿到 BOX 凭证
- golf revenge：46 字符生成 1000 以内质数 → 触发点：极短字符限制
- 动作：k,P,*s=1,1,;while k<1e3:s+=P%k*[k];P*=k*k;k+=1 → 用 Unpacking + 累乘筛质数
- right_data：mod 257 线性方程组 → 触发点：题目给 n 个模线性方程
- 动作：Sage matrix 逆推 → 拿到明文
- Pyhumor：Python VM 逆向 → 触发点：取 30bit 字段值
- 动作：定位字节码常量池 + 反编译 VM 指令 → 拿到 flag
- cpy/easy_http/b0okshelf/mygo/factor/skip/Nu1tka 等 8 题继续走标准流程
failed_attempts:
- python_jail 用 import → 失败：import 被沙箱过滤
- golf 写标准埃筛 → 失败：字符超 46
key_observations:
- Python 沙箱逃逸核心：生成器 gi_frame + f_back 回溯 + f_globals 取全局
- 字符高尔夫常用 Unpacking assignment + s+=P%k*[k] 模式
- 模线性方程组用 Sage matrix 求逆是 CTF crypto 标准解法
- Python VM 逆向需先恢复字节码再反编译
prerequisites:
- Python 生成器 + frame 对象（gi_frame/f_back/f_globals）
- Python 字符高尔夫技巧
- Sage 数学工具（mod 线性方程组）
- Python VM 字节码逆向
---
