...
quality: high
full_path: 解析2025强网杯ezran.full.md
meta_path: 解析2025强网杯ezran.meta.md
images_removed: true
images_removed_count: 0
schema_version: v3.0.0-P0
summary: '2025 强网杯 ezran - MT19937 状态恢复攻击。2025 强网杯 ezran：MT19937 624 状态 GF(2) 线性方程组恢复 + Fisher-Yates 洗牌逆向。。关键路径：题目: MT19937 PRNG 生成 3108 次 (r1, r2) → r1 = getrandbits(8), r2 = getrandbits(16) → x = (pow(r1, ...'
category: crypto
subcategory: rsa
tools_used:
- Python
...
contest_type: open
wp_url: https://www.ctfiot.com/275603.html
reasoning_chain:
- 题目MT19937 PRNG生成3108次(r1,r2) → r1=getrandbits(8), r2=getrandbits(16) → 触发点：PRNG状态恢复
- x=(pow(r1,2*i,257) & 0xff) ^ r2 → 2字节大端gift → 总6216字节 → 触发点：信息泄漏
- 假设：r2高8位^observed = (pow(r1,2*i,257)&0xff) ∈ [0,255] → 约束条件 → 动作：构造线性系统
- 动作：gf2bv库 lin=LinearSystem([32]*624) + 高斯消元GF(2) → 624个32位整数状态
- '第一个状态最高位为1: mt[0] ^ 0x80000000 → 假设：补全缺失位 → 触发点：约束闭合'
- Fisher-Yates洗牌2025次 → 记录每次(i,j)交换 → 假设：逆洗牌恢复flag顺序
- 动作：rng.setstate((3,tuple(sol+[624]),None)) + 反向应用sw → 验证gg==gift → flag
failed_attempts:
- 试图直接试3108个r1=8位穷举 → 失败：状态是624个32位整数耦合,无法独立穷举
- 试图不解状态直接逆Fisher-Yates → 失败：洗牌种子需要状态恢复
- 试图只取约束r2高8位忽略范围 → 失败：[0,255]范围是关键约束
key_observations:
- MT19937内部状态624个32位整数=19968 bit可通过足够样本恢复
- GF(2)线性代数+高斯消元是标准PRNG攻击
- Fisher-Yates洗牌记录(i,j)交换+反向应用是稳定逆算法
- gf2bv库(github.com/orisano/gf2bv)自动化线性系统求解
prerequisites:
- MT19937 PRNG原理
- GF(2)线性代数
- gf2bv库使用
