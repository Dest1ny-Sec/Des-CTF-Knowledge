...
quality: high
full_path: zkCTF_2024_WriteUp_By_ChaMd5.full.md
meta_path: zkCTF_2024_WriteUp_By_ChaMd5.meta.md
images_removed: true
images_removed_count: 1
schema_version: v3.0.0-P0
summary: zkCTF 2024 WriteUp By ChaMd5。zkCTF 2024 zkSNARK 系列，覆盖 circom groth16 入门到 halo2_proofs Rust 自定义 gate 高级用法。。经验：circom 是 zkSNARK DSL 入门工具，halo2_proofs 是 Rust 实现 zk 电路的工业级库；...
category: misc
...
contest_type: open
wp_url: https://www.ctfiot.com/162396.html
reasoning_chain:
- CheckIn 题：用 circom 写 .circom 文件 → 触发点：zkSNARK DSL 入门
- 动作：circom checkin.circom --r1cs --wasm --sym -c → 观察：产出 .r1cs + .wasm + .sym
- 动作：node generate_witness.js checkin.wasm input.json witness.wtns → 观察：witness 算完
- 动作：snarkjs groth16 prove ../CheckIn_groth16.zkey witness.wtns proof.json public.json → 观察：proof + public 输出
- 假设：中高级题用 halo2_proofs Rust 实现 FibonacciCircuit → 动作：配置 add gate + U8 range-check table + DivCircuit a/b/c/r/k 列
- 动作：mul+add 双 gate + assign_range + assign_witness + expose_public 三件套 → 观察：MockProver::run 验证通过
- generate_keys 调 ParamsKZG::setup + keygen_vk/pk → generate_proof 调 create_proof 用 KZGCommitmentScheme + ProverGWC + Challenge255 transcript → 写出 proof/param/vk 三文件 → 完成
failed_attempts:
- 试图用 circom 2 直接生成 verifier → 失败：zkSNARK 体系里证明与验证分离
- 试图用 snarkjs 做 halo2 proof → 失败：snarkjs 只支持 circom 生成的 proof
- 试图硬编码 KZG params → 失败：必须 ParamsKZG::setup
key_observations:
- circom 是 zkSNARK DSL 入门工具
- halo2_proofs 是 Rust 实现 zk 电路的工业级库
- range check 通过 (1..range).fold(expr * (value - a)) 实现
- BN256 Pasta 域 + KZG commitment 是现代 zk 标准组合
prerequisites:
- circom 语法（signal / constraint / component）
- snarkjs CLI（groth16 / generatecall / plonk）
- halo2_proofs Rust API（ConstraintSystem / Layouter / create_proof）
- KZG commitment + BN256 椭圆曲线
---
