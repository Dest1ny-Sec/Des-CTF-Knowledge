...
tags:
- 蒙山杯 Misc 签到 m13c
- 凯撒 shift=4 eccgxj{Nywx_e_W1qtpi_Q13g} → ayyctf{Jut_a_S1mple_M13c}
- shark USB 键盘流量 04-a 1c-y 06-c 17-t
- USB HID scan code map
- easymisc zip 密码 4 字节可见 ASCII CRC32 爆破
- PIL Image.split() RGB LSB 隐写
attack_chain:
...
quality: medium
full_path: 2024首届“山石·蒙山杯”网络安全大赛WriteUp.full.md
meta_path: 2024首届“山石·蒙山杯”网络安全大赛WriteUp.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: '2024 首届"山石·蒙山杯"网络安全大赛 WriteUp（凯撒+USB+CRC32 爆破）。蒙山杯 Misc 三题：凯撒 shift=4 + USB 键盘流量 scan code map + zip 4 字节密码 CRC32 爆破 + LSB 隐写。。关键路径：签到 m13c: caesar_decrypt(eccgxj{Nywx_e_W1qtpi_Q13g}, shift=4) → ...'
category: misc
...
contest_type: open
wp_url: https://www.ctfiot.com/190955.html
reasoning_chain:
- '[触发点] crypto01 babyrsa + dp 泄露 + e=65537 → 假设：dp 攻击（已知 e*dp ≡ 1 mod (p-1)）→ [动作] for x in range(1, e): if e*dp % x == 1: p = (e*dp - 1)//x + 1; if n%p == 0: break → [观察] 拿到 p,q → [下一步] pow(c, d, n) 解密'
- '[触发点] crypto02 vigenere + 自定义码表 ascii_lowercase+''{}'' → 假设：知道 flag 头 ayyctf{ 爆破 4 字节密钥 → [动作] for i in range(4): key += dicts[(ord(enc[i])-ord(known[i]))%28] → [观察] 解出 ayyctf{wecanalwaystrustvigenere}'
- '[触发点] misc01 凯撒 shift=4 → 假设：Hello, Wordl! 加密 → [动作] some_encrypt(text, 4) → [观察] 拿到 ayyctf{Jut_a_S1mple_M13c}'
- '[触发点] misc02 USB 键盘流量 + scan code 映射 → 假设：scan code → ASCII → [动作] 04=a, 1c=y, 06=c, 17=t, 09=f, 2f={, 0b=h, 20=3, 0f=l, 27=0, 2d=-, 0e=k, 08=e, 1c=y, 2d=-, 05=b, 27=0, 21=4, 15=r, 07=d, 30=} → [观察] 拿到 ayyctf{h3ll0_key_b04rd}'
- '[触发点] misc03 zip 密码 CRC32 = 0x475EDC53 → 假设：4 字节可见 ASCII 100^4 爆破 → [动作] zlib.crc32 候选字符串对比 → [观察] 拿到密码 → [下一步] 解压'
- '[触发点] reverse01 cpp 题 + (input[i]-1)^0xEA == bytes_[i] → 假设：bytes_[i] ^ 0xEA + 1 = flag[i] → [动作] 写脚本 → [观察] 拿到 flag'
- '[触发点] web01 babycode create_function → 假设：?a=create_function&v=};system(''cat /*'');/* 注入闭合 → [动作] 发送 → [观察] RCE'
failed_attempts:
- 试图手算 vigenere 全部密钥 → 失败：4 字符爆破更快
- 试图不解 CRC32 直接解压 → 失败：密码未知
- 试图用 Wireshark 解析 USB 流量 → 失败：必须手工映射 scan code
key_observations:
- USB 键盘流量 scan code → ASCII 映射：04=a, 1c=y, 06=c, 17=t, 09=f, 2f={, 0b=h, 20=3, 0f=l, 27=0, 2d=-, 0e=k, 08=e, 21=4, 15=r, 07=d, 30=}
- CRC32 爆破 4 字节可见 ASCII 100^4 = 1 亿，Python 跑 30 秒
- 'dp 攻击 RSA：for x in range(1, e): if e*dp%x==1: p=(e*dp-1)//x+1'
- vigenere 自定义码表（字母+{}）爆破：已知 flag 头爆破 4 字节密钥
prerequisites:
- dp 攻击 RSA（已知 e*dp ≡ 1 mod p-1）
- vigenere 自定义码表爆破
- USB HID scan code 映射表
- CRC32 4 字节爆破（zlib.crc32）
---
