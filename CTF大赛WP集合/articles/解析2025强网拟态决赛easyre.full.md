...
quality: high
full_path: 解析2025强网拟态决赛easyre.full.md
meta_path: 解析2025强网拟态决赛easyre.meta.md
images_removed: true
images_removed_count: 0
schema_version: v3.0.0-P0
summary: 解析2025强网拟态决赛easyre。强网拟态2025决赛easyre逆向,PE32+Windows+signal(SIGSEGV)+setjmp/longjmp控制流混淆+异常处理触发跳转+TEA解密(delta补码0x61C88647+sum初值0xC6EF3720+32轮Feistel+32字符flag)。。关键路径：文件:PE32+ x86-64 stripped to extern...
category: reverse
subcategory: reverse
tools_used:
- x86/x64
...
contest_type: open
wp_url: https://www.ctfiot.com/286199.html
reasoning_chain:
- 题目PE32+x86-64 stripped+字符串'Input flag'/'Wrong flag'/'Success'/'Wrong length 32 chars' → 触发点：Windows逆向
- 动作：查导入表发现signal函数 → 假设：signal+setjmp/longjmp控制流混淆
- signal(SIGSEGV,handler)+setjmp+mov dword[0],eax触发SIGSEGV+handler longjmp回setjmp → 触发点：异常处理跳回
- 假设：密钥计算藏在异常处理函数(handler)中 → 动作：定位handler+提取密钥
- 位运算+左移4位+右移5位+XOR+add+32轮 → delta补码0x61C88647+sum初值0xC6EF3720 → 假设：TEA算法
- 提取8个32位整数密文(4组64位) → 标准TEA解密 → 小端序转换ASCII
- 动作：每4字节反转 → 拼接32字符flag
failed_attempts:
- 试图跟踪主执行流分析 → 失败：signal+longjmp打断控制流,主函数没真逻辑
- 试图F5反编译handler → 失败：handler有自修改代码,反编译混乱
- 试图不解信号直接猜32字符flag → 失败：TEA加密无侧信道
key_observations:
- signal(SIGSEGV)+setjmp/longjmp是控制流混淆的经典手法
- TEA特征常量0x9E3779B9+0xC6EF3720+0x61C88647快速识别
- 密钥常在异常处理函数中计算,需从handler提取
- 静态分析遇到大量跳转和混淆,黑盒+动态调试更高效
prerequisites:
- Windows PE文件结构
- signal/setjmp/longjmp控制流混淆
- TEA算法(Feistel+delta)
- IDA/Ghidra调试器使用
---
# 解析2025强网拟态决赛easyre
