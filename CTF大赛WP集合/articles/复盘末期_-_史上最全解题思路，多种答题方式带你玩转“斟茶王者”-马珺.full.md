...
quality: high
full_path: 复盘末期_-_史上最全解题思路，多种答题方式带你玩转“斟茶王者”-马珺.full.md
meta_path: 复盘末期_-_史上最全解题思路，多种答题方式带你玩转“斟茶王者”-马珺.meta.md
images_removed: true
images_removed_count: 12
schema_version: v3.0.0-P0
summary: '复盘末期 | 史上最全解题思路，多种答题方式带你玩转"斟茶王者"-马珺。斟茶王者 16 题涉网犯罪研判复盘：银行流水/调证/二维码/SDK/爬虫/SQL/Electron XSS RCE 综合实战。。关键路径：圣地亚哥皮蛋厂队长马珺 复盘 15 题 → Q1 资金初探: 数据透视表, 借贷标志位, 金额求和 → Q2 运气爆棚: 重明查 6226****6978927 → 涉案群组 ID...'
category: web
...
contest_type: open
wp_url: https://www.ctfiot.com/202065.html
reasoning_chain:
- '斟茶王者 16 题涉网犯罪研判复盘, Q1 资金初探 → 数据透视表借贷标志位金额求和 → 触发点: 银行流水表'
- '假设: Excel 数据透视表按借贷标志分组求和 → 动作: 拖拽 借贷标志 到 行, 金额 到 值 → 观察: 一级卡收款金额最大'
- 'Q2 重明查 6226****6978927 → 涉案群组 ID → 假设: 重明是公安调证平台 → 动作: 输入卡号查 → 观察: 命中群组'
- 'Q5 躲猫猫 AndroidManifest.xml → com.nosugar.chatapp.key → 假设: AES key 在 manifest meta-data → 动作: jadx 反编译 APK'
- 'Q11 backup-*.sql 下载 + AES-CBC 解密 com.nosugar.chatapp.key → 假设: key 是 manifest 里那个 → 动作: AES-CBC 解密聊天记录'
- 'Q15 Electron preload.js chatClientApi.execCommand + XSS nickname + this.parent RCE → 假设: app.asar 反编译 → 动作: 修改昵称注入 XSS → execCommand 触发 RCE'
failed_attempts:
- '试图不解 AES 直接读聊天记录 → 失败: SQL 备份里聊天记录是密文'
- '试图绕过 XSS 过滤 → 失败: 必须用 object data:text/html;base64 绕过'
key_observations:
- 银行流水借贷标志位 + 金额求和是资金初探通用思路
- 调证平台 (重明) 输入卡号查群组 ID
- AES key 在 AndroidManifest meta-data 是 Android 应用通用藏点
- Electron app.asar RCE 是 XSS 升级终极 (preload.js execCommand)
- 案件卷宗 (案件+笔录+尸检+物品清单) 是综合研判信息源
prerequisites:
- 银行流水分析 (数据透视表 + 借贷标志)
- 调证平台使用 (重明)
- Android APK 反编译 (jadx)
- Electron app.asar 解包 + preload.js 注入
---
...
url = '[图片已移除]'headers = {"Authorization": "Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJ1c2VySWQiOjMsInVzZXJuYW1lIjoibHFjZnVndXQiLCJhY2Nlc3MiOiJ1c2VyIiwiaWF0IjoxNzIxMDQxNDg5LCJleHAiOjE3MjEwNTM0ODl9.4d-HWe9xY1aKCjEeEh-PhSevKvOIRfVhoMb7eTsMglM"}r = requests.get(url=url,headers=headers,verify=False)
