...
quality: medium
full_path: FE-CTF_2022-_Cyber_Demon_–_Blackbox.full.md
meta_path: FE-CTF_2022-_Cyber_Demon_–_Blackbox.meta.md
images_removed: true
images_removed_count: 0
schema_version: v3.0.0-P0
summary: 'FE-CTF 2022: Cyber Demon – Blackbox。FE-CTF Cyber Demon Blackbox：黑盒fuzz测试+EOF检测。关键路径：file: data 二进制格式 → hexdump查看80 42 4d 8a 40 38魔数 → nc blackbox.hack.fe-ctf.dk 1337。经验：黑盒题先试简单输入观察响应'
category: misc
...
contest_type: open
wp_url: https://www.ctfiot.com/82141.html
reasoning_chain:
- 触发点：题目仅给 data 二进制 + nc 连接端口 → 假设：黑盒 fuzz 探测协议
- 动作：hexdump file → 观察：魔数 80 42 4d 8a 40 38 → 假设：4-字节 header
- 动作：nc blackbox.hack.fe-ctf.dk 1337 → 假设：必须构造合法输入
- 动作：发送空行 + 多行输入 → 观察：服务不返回 → 假设：EOF 触发返回
- 动作：发送 4 字节 → 观察：EOFError（连接关闭）→ 假设：4 字节是 EOF 触发
- 动作：循环发送 1-3 字节 → 观察：服务端无响应 → 假设：长度不够触发条件
- 动作：构造 fuzz 字典 → 不同字节组合 → 观察：部分输入返回 flag
failed_attempts:
- 试图用 wireshark GUI 抓包 → 失败：黑盒无 client
- 试图 strings file 找 flag → 失败：flag 是动态返回
- 试图大量并发 nc → 失败：服务端有 rate limit
key_observations:
- 黑盒题先用最简单输入（空行、EOF）观察响应
- 4 字节 0x80424d8a 魔数是协议头常见标识
- EOFError 在 pwntools 是服务端主动关闭连接标志
- 黑盒 fuzz 用 nc 脚本而非 GUI 是 CTF 通用手法
prerequisites:
- nc / pwntools 远程通信
- hexdump/file 命令识别二进制
- 黑盒 fuzz 基本思路（边界值/EOF/空）
- Wireshark/tshark 协议分析（可选）
---
