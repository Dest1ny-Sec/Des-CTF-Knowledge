...
quality: high
full_path: DefCamp_CTF_2025_onigirl_复盘详解.full.md
meta_path: DefCamp_CTF_2025_onigirl_复盘详解.meta.md
images_removed: true
images_removed_count: 6
schema_version: v3.0.0-P0
summary: DefCamp CTF 2025 onigirl 复盘详解。DefCamp 2025 onigirl：stb_image魔改+PIC格式+像素距离变换。关键路径：PIE+Full RELRO+Canary+NX → 读取图片stbi_load_from_memory → 计算像素到中心距离。经验：stb_image读取+随机扰动+像素距离因子是复杂图像题常见模式
category: misc
subcategory: stego
time_required: long
difficulty_score: 4
...
contest_type: open
wp_url: https://www.ctfiot.com/273679.html
reasoning_chain:
- PIE + Full RELRO + Canary + NX + stbi_load_from_memory → 触发点：图像读取类 pwn
- 前置：privilege_value 计算必须 = 4919 → 假设：通过 PIC 格式图像控制像素运算
- 观察：stbi__pic_test_core 校验 4 字节头 53 80 f6 34 + 0x53 字节偏移 + 'PICT' → 假设：PICT 是内部格式
- distance_factor = 1.8*(1.0-d/max_distance) → 假设：像素到中心距离变换权重
- 假设：3*random_index 写入随机扰动 → 动作：构造字节序列触发 index_temp = 10（数组越界 11 元素）
- 观察：mod_result > 0 才不取反 → 假设：input bytes [10, 0xf0, 0x0f, 4] 触发模 11 = 0
- 动作：构造 width=0x5500/height=0x100 PIC 数据 → 输出 num = 4919 → privilege 解锁
- 后半：菜单 alloc(只 2 次)/show(UAF) → 假设：tcache + fastbin 双重释放
- 动作：11 次 add + 7 次 dele + double free tcachebin → 触发 tcache_perthread_struct 攻击
failed_attempts:
- 试图直接传 PNG/JPEG 触发 → 失败：题目要求 PICT 格式头 53 80 f6 34
- 试图只改 pixel 绕过 privilege → 失败：必须 mod_result >0 才不取反，需精确控制字节差
key_observations:
- PICT 自定义图像格式（stb_image 魔改）通过 4 字节头 + 'PICT' 校验
- distance_factor = 1.8*(1-d/max_distance) 是非线性像素权重
- tcache + fastbin 双重释放 + tcache_perthread_struct 攻击
- mod_result = (a-b)%11，>0 才不取反可控制 index_temp 范围
prerequisites:
- stb_image 库魔改（PICT 格式）
- glibc 2.32 tcache_perthread_struct 利用
- tcache/fastbin 双重释放
- PIC 数据结构解析
---
