...
quality: high
full_path: 2026阿里CTF_Writeup_by_Mini-Venom.full.md
meta_path: 2026阿里CTF_Writeup_by_Mini-Venom.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: 2026 阿里 CTF Writeup by Mini-Venom。阿里CTF 2026 — Web heartbeat 代理 + format string + SSTI + Android .webp 伪装 .class DexClassLoader + 3 batch 流量 (RSA-2048+LZRR+89ali) + BackupExec RPC NDR 16字节XOR。。关键路径...
category: web
...
contest_type: open
wp_url: https://www.ctfiot.com/295706.html
wp_author: Mini-Venom
reasoning_chain:
- Web heartbeat 代理 + action 部分 → 触发点：client Content-Type 覆盖
- 假设：multipart/form-data 注入 token → 动作：构造 token=admin
- 假设：用 {0.view_functions[index].__globals__[API_KEY]} format string 泄 API_KEY
- 'admin 读模板 SSTI: {config.__class__.__init__.__globals__[''os''].popen(''cat /flag*'').read()}'
- 假设：伪装 .webp 实际是 .class → 动作：file 命令识别头 → 观察：CAFEBABE
- DexClassLoader 动态加载 .webp 中的代码 → 假设：C0003L 扫描器遍历
- 'Runner.encrypt: 8 字节 Key + IV + Base64 指令 → 假设：置换移位对称算法'
- Il1.LZRR 自定义 LZ77 压缩（头 4C 5A 52 52, mode=15 走 RLE）
- '协议: 8 字节 Session Key 用 RSA-2048 加密 + XOR 233 文件名 + LZRR 压缩 + 89 61 6C 69 头'
- 流量包 data0/data1/data2 = 3 batch → 假设：奇数 batch algo3 keyless（rank=0）
- 偶数 batch algo2 依赖 key 需已知明文恢复 keystream
- BackupExec RPC OpenRemoteFile NDR 16字节 XOR 解密读 flag
failed_attempts:
- 直接 SSTI 跑命令 → 失败：format string 仅泄 API_KEY，需要 admin 模板才能 SSTI
- .webp 当图片分析 → 失败：头是 CAFEBABE 实际 .class
- 流量按单个 batch 解 → 失败：奇偶 batch 算法不同
- 直接 RSA-2048 解密 → 失败：Session Key 是 XOR 233 + LZRR 复合
key_observations:
- 伪装 .webp/.png/.jpg 是反检测常规手段（CAFEBABE 头 magic number 直接暴露 .class）
- 加密协议魔数 + RSA 会话密钥 + LZRR 自定义压缩是常见样本偷数据模式
- BackupExec RPC 是历史漏洞重灾区（OpenRemoteFile NDR）
- format string 配合 view_functions.__globals__ 是 Flask API_KEY 泄漏经典
- 奇偶 batch 不同算法（algo3/algo2 + keyless/known-plaintext）是流量解密分阶段套路
prerequisites:
- Flask format string 注入（view_functions.__globals__）
- Jinja2 SSTI 模板链
- DexClassLoader 动态加载 + .class magic CAFEBABE 识别
- RSA-2048 + XOR 233 + LZRR 自定义压缩协议
---
