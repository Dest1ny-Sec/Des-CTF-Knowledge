...
- tryhackme
- ssh-port-brute
- binary-search-jabberwock
- sudo-reboot-privesc
- alice-sudo-other-host
- alice-ssh-id-rsa
attack_chain:
...
quality: medium
full_path: TryHackMe_Looking_Glass_writeup.full.md
meta_path: TryHackMe_Looking_Glass_writeup.meta.md
images_removed: true
images_removed_count: 0
schema_version: v3.0.0-P0
summary: TryHackMe Looking Glass writeup。TryHackMe Looking Glass：5000 SSH 端口二分爆破 + Jabberwocky 诗 + sudo reboot 提权到 humptydumpty + alice SSH 私钥 + host sudo 越权。。关键路径：nmap -A -p- 5000 端口 SSH 服务 (9000-13999) → ...
category: misc
...
wp_url: https://www.ctfiot.com/27362.html
reasoning_chain:
- nmap -A -p- 扫 10.10.116.245 → 触发点：5000 端口 SSH 服务 (9000-13999) 开放
- 假设：唯一真实 SSH 服务在某特定端口 → 动作：二分搜索 9000-13999 区间 ssh 端口
- 动作：python3 bin-search.py → 观察：端口 13547 返回 'You've found the real service'
- ssh 端口给 Jabberwocky 诗提示 → 触发点：诗中含 'Twas brillig and the slithy toves' → 假设：用户名 jabberwock
- 动作：ssh jabberwock@10.10.116.245 -p 13547 → 登录成功
- sudo -l 看到 /sbin/reboot NOPASSWD → /etc/crontab @reboot tweedledum bash /home/jabberwock/twasBrillig.sh
- 假设：twasBrillig.sh 内容 wall $(cat poem.txt) 可注入 → 写 /home/tweedledum/poem.txt 含 'cat /home/tweedledum/humptydumpty.txt > /home/jabberwock/humptydumpty.txt'
- 动作：sudo reboot → reboot 后 wall 触发命令注入 → 拿到 humptydumpty 密码 → su humptydumpty → 读 /home/alice/.ssh/id_rsa
- sudo -h ssalg-gnikool /bin/bash (host 字段 NOPASSWD) → 拿 root
failed_attempts:
- 试图直接 su root → 失败：无密码且无 sudo 权限
- 试图线性枚举 5000 端口 → 失败：太慢，二分更快
key_observations:
- wall 命令注入是 Linux 经典 sudo reboot 提权路径
- sudo -h host NOPASSWD 越权常被忽视
- TryHackMe 风格 SSH 二分搜索是真实漏洞训练
prerequisites:
- Linux nmap + 二分算法搜索端口
- sudo / crontab / wall 命令链注入原理
- SSH 私钥权限 (drwx--x--x) 与多用户切换
---
