...
quality: high
full_path: DataCon2021优秀解题思路分享：物联网固件自动化漏洞挖掘WP_（浙江大学_go-go-go战队）.full.md
meta_path: DataCon2021优秀解题思路分享：物联网固件自动化漏洞挖掘WP_（浙江大学_go-go-go战队）.meta.md
images_removed: true
images_removed_count: 0
schema_version: v3.0.0-P0
summary: 'DataCon2021优秀解题思路分享：物联网固件自动化漏洞挖掘WP （浙江大学 go-go-go战队）。DataCon2021物联网固件angr+def-use自动漏洞挖掘多架构。关键路径：用angr跨平台静态分析+PyPy即时编译 → 危险函数表: popen/system/doSystemCmd/sprintf/snprintf/AES_cbc_encrypt等 → 获取CFG+PLT...'
category: misc
...
contest_type: open
wp_url: https://www.ctfiot.com/74660.html
reasoning_chain:
- 题目给 27 个危险函数（popen/system/sprintf/AES_cbc_encrypt 等）+ 多架构固件 → 触发点：跨平台批量漏洞挖掘
- 假设：IDA/Ghidra 脚本需逐架构适配 → 动作：选 angr 跨平台静态分析 + PyPy 加速
- 危险函数动态链接 → 假设：先获取 CFG + PLT 段前继节点定位调用地址 → 动作：find_callers
- 假设：参数 def-use 链能判别是否可控 → 动作：angr ReachingDefinitionsAnalysis
- 观察：参数指向 .rodata 段 → 触发点：直接判定 NOT_EXPLOITABLE
- mips/aarch64 寄存器传参 vs x86 栈传参 → 假设：用 angr 预定义 ABI 拿对象统一追踪
- system 调用前 0x20 字节有 sprintf → 假设：format 无 %s 则 system 不可利用 → 动作：block 内符号执行抓 format
- AES_cbc_encrypt 第 4/5 参是 IV/KEY → 假设：必须来自安全随机数 → 动作：跟踪到 AES_set_key 输出
- source func 集合（读输入/文件/网络/环境）→ 动作：DFS 深度 10 追踪到 sink 函数路径
failed_attempts:
- 试图用 IDA 插件逐架构写 → 失败：mips/arm/x86/amd64 传参差异需重复开发
- 试图用纯符号执行定位漏洞点 → 失败：路径爆炸约束难解，转 def-use 更稳
- 试图全部识别 source 函数自动化 → 失败：自定义函数反编译参数无法识别，需 Ghidra 重定义导出 gzf
key_observations:
- ReachingDefinitionsAnalysis 可定位参数 def 来源，指向 .rodata 即判定不可利用
- angr block 内符号执行可避免路径爆炸，仅在危险函数前 0x20 字节局部约束求解
- mips 延迟槽参数指令在 call 后，angr 需手动调整 PC
- PyPy JIT 比 CPython 快数倍，是大规模二进制批量分析的提速关键
prerequisites:
- angr 符号执行框架使用（CFG/RDA/符号执行）
- 多架构 ABI（mips 寄存器 vs x86 栈传参）
- PyPy 即时编译器加速原理
- dangerous function sink 模式识别
---
