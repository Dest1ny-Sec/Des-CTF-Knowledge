...
tags:
- 湾区杯 2025
- SM4 国密 32 轮 encrypt/decrypt
- base64 VCWBIdzfjm45EmYFWcqXX0VpQeZPeI6Qqyjsv31yuPTDC80lhFlaJY2R3TintdQu
- 沙箱逃逸 chr(95)*2+builtins+chr(95)*2 globals[builtins][import] os
- JWT HS256 2 字母爆破 @o70xO$0%#qR9#m0
- Quaternion DLP BSGS secret < 2^50
- AES-ECB + md5(secret) 派生密钥
- 自定义 S-box invert_table 逆推
- ROR 逆 sub_1492
attack_chain:
...
quality: high
full_path: 2025湾区杯网络安全竞赛WriteUP.full.md
meta_path: 2025湾区杯网络安全竞赛WriteUP.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: 2025 湾区杯网络安全竞赛 WriteUP（SM4 国密 + 沙箱逃逸 + JWT 爆破 + Quaternion BSGS + AES Sbox 逆推）。2025 湾区杯：SM4 国密 32 轮解密 + Python 沙箱逃逸 globals+builtins+import + JWT 2 字母爆破 + Quaternion DLP BSGS + 自定义 S-box 逆推。。关键路径：S...
category: crypto
subcategory: rsa
subcategories:
- rsa
...
contest_type: open
wp_url: https://www.ctfiot.com/269807.html
reasoning_chain:
- 触发点：SM4 国密 class + key a8a58b78f41eeb6a + 密文 base64 VCWB... → 假设：标准 SM4 32 轮 SPN
- 动作：FK[4] XOR + CK[32] 32 轮密钥扩展 → sk[32] 子密钥 → 下一步：cryptBlockDecrypt 倒序用 sk[31-i]
- 观察：base64 解密 → 16 字节 × N 块 → plaintext
- 触发点：Python 沙箱禁 __import__ __builtins__ 等关键词 → 假设：字符拼接 → 动作：chr(95)*2 + 'builtins' + chr(95)*2 绕
- 下一步：globals()['__builtins__']['__import__']('os').system('cat /fl11lag >/tmp/aaa')
- 触发点：JWT HS256 key 已知前缀 @o70xO$0%#qR9# + 缺 2 字符 → 假设：爆破 62*62=3844 次 → 动作：循环 62 字符² 验证 hash
- 触发点：Quat DLP secret < 2^50 → 假设：BSGS baby-step giant-step → 动作：bsgs(Q, R, 1<<50) → key = md5(str(secret))
- 触发点：自定义 S-box 字节映射 → 假设：sub_13E1 是 ROL3 + 16-bit 字内 swap + mod 257 pow → 动作：invert_table() 建 y→x 反向表
- 下一步：mapping[b] 还原 tmp → ror(tmp, k) 还原中间状态 → flag_bytes
- 观察：拼 bytes 输出 flag
failed_attempts:
- 试图直接 eval(input()) 拿 RCE → 失败：__import__ 等关键字被禁
- 试图用 jwt 私钥字典爆破 → 失败：JWT key 已知前缀只剩 2 位字符必须精确爆破
- 试图套现成 AES-128 解 S-box → 失败：S-box 是题给的 byte_2020 要 invert_table 还原
key_observations:
- SM4 32 轮国密 SPN：FK[4] + CK[32] 固定 + sk[32] 子密钥，cryptBlockDecrypt 用 sk[31-i] 倒序
- 字符拼接绕过 sandbox 黑名单（chr(95)*2 + 'builtins' + chr(95)*2）
- JWT HS256 弱密钥爆破 62 字符×2=3844 次是常考点
- Quat DLP BSGS secret<2^50 比暴力 Pohlig-Hellman 快得多
- 自定义 S-box 用 invert_table() 建反向映射是逆向工程标准做法
prerequisites:
- SM4 国密算法 32 轮密钥调度
- Python 沙箱逃逸（globals + builtins + chr 拼接）
- JWT HS256 弱密钥爆破
- Quaternion 四元数 + BSGS
---
