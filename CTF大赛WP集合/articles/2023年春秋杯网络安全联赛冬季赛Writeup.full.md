...
- 明文爆破
- PHP混淆还原
- CVE-2023-51385_OpenSSH_ProxyCommand
- Redis主从复制
- dict协议
- Python格式化字符串漏洞
- 沙盒bypass
...
quality: high
full_path: 2023年春秋杯网络安全联赛冬季赛Writeup.full.md
meta_path: 2023年春秋杯网络安全联赛冬季赛Writeup.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: '2023 年春秋杯网络安全联赛冬季赛 Writeup。春秋杯冬季赛综合：外卖 ZIP 隐写+明文爆破+OpenSSH ProxyCommand+Redis 主从复制+Python pickle 沙盒。。关键路径：谁偷吃了我的外卖: foremost 分离图片 → 排序 ZIP 内 _ 后 4 字符 → base64 → 补 PK 文件头 → 明文爆破 → 明文混淆: LICENSE.txt ...'
category: web
...
contest_type: open
wp_url: https://www.ctfiot.com/158635.html
reasoning_chain:
- 触发点：谁偷吃了我的外卖 foremost 分离图片 → 假设排序 ZIP 内 _ 后 4 字符 → 动作：base64
- 动作：补 PK 文件头 + 明文爆破 → 观察：还原原 ZIP
- actor 明文混淆 LICENSE.txt 明文爆破 + PHP urldecode/eval 还原 → 假设：明文攻击
- actor modules CVE-2023-51385 OpenSSH ProxyCommand 注入 → 假设：.gitmodules url 字段 nc bash
- actor ezezez_php PHP 反序列化 POP 链 + Redis 主从复制 + dict:// 协议 → 假设 SSRF RCE
- 动作：Er__set → $value($this->Flag) 调用系统函数 → 观察：dict:// 协议触发 Redis 全量同步
- actor picup Python 格式化字符串漏洞 + pickle 反序列化 → 假设沙盒 bypass → 动作：伪造 admin session
- actor nmanager 整数溢出 + 栈任意读写 + 泄露 libc 改 main 返回
failed_attempts:
- 试图直接用 bash 反连 OpenSSH → 失败：需 ProxyCommand 注入 nc
- 试图标准 SSRF 打 ezezez_php → 失败：需 Redis 主从复制 + dict 协议
key_observations:
- CVE-2023-51385 走 .gitmodules 注入 nc|bash
- Redis 主从复制 + dict:// 协议是 SSRF 经典 RCE
- Python 格式化字符串漏洞可泄露 admin 密码
- ZIP 文件名隐写排序拼接是 MISC 入门技巧
prerequisites:
- CVE-2023-51385 OpenSSH ProxyCommand
- Redis 主从复制 + dict 协议
- Python 格式化字符串漏洞利用
- PHP 反序列化 POP 链
---
