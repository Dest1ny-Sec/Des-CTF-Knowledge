...
lesson: '|'
quality: medium
full_path: Open_xINT_CTF_2024_Writeup.full.md
meta_path: Open_xINT_CTF_2024_Writeup.meta.md
images_removed: true
images_removed_count: 0
schema_version: v3.0.0-P0
summary: 'Open xINT CTF 2024 Writeup (日文 OSINT)。Open xINT CTF 2024 日文 OSINT 速查: whois gov.ru 税号 + 国外店铺电话。。经验：|'
category: misc
...
contest_type: open
wp_url: https://www.ctfiot.com/215623.html
reasoning_chain:
- whois 题 → 触发点：gov.ru 税号 → 假设：whois 命令查询
- 动作：whois gov.ru → 观察：部分字段缺失 → 假设：需到注册商查询
- Domaintools 查无果 → 假设：俄罗斯税号格式需知
- 动作：TLDpedia 找 .ru 注册商 Whois 页面 → 观察：得到税号
- flag：俄文税号格式
- all grean 题 → 触发点：店铺电话号码查询
- 动作：观察图细节 → 观察：桌贴 'hiroba.sushi' + IG 图标
- 假设：Instagram 搜 'hiroba.sushi' → 动作：找到官网 → 观察：电话号
- 格式：国号码无连字符 → flag
- bus stop 题 → 假设：日文 OSINT 多题串联 → 动作：Google Maps 巴士站 + 反向图
failed_attempts:
- Domaintools 直接查 → 失败：字段不全
- Google Lens 投图 → 失败：无果
- 不验店名直接交号码 → 失败：flag 严格匹配
key_observations:
- whois 命令并非全功能，部分需注册商页面查
- TLDpedia 是 .tld 注册商查询索引
- Instagram 搜店名是 OSINT 经典入口
- 图片细节观察（贴纸 / IG 图标）是 OSINT 基本功
- 日文 OSINT 跨国店铺电话格式：国码 + 无连字符
prerequisites:
- whois 命令使用
- TLDpedia 工具使用
- Instagram / Google Maps OSINT
- 日本 OSINT 文化背景
---
