...
quality: high
full_path: BCTF自动驾驶-WriteUp.full.md
meta_path: BCTF自动驾驶-WriteUp.meta.md
images_removed: true
images_removed_count: 2
schema_version: v3.0.0-P0
summary: 'BCTF 自动驾驶 - WriteUp。BCTF 自动驾驶 — Stanley 横向控制 + PID 纵向控制 + Lane Detection targeted attack + Adversarial NPC GPS 缩放攻击 (rate=1.2 pos[120]/pos[399] 锚点)。。关键路径：demo: Lane Detection targeted attack + ADC ...'
category: misc
...
contest_type: open
wp_url: https://www.ctfiot.com/1441.html
reasoning_chain:
- '[触发点] 拿到 ADC + Lane Detection 双 demo → 假设：自动驾驶对抗攻击 / [动作] 看 planning.py Stanley controller k_e=0.3 k_v=0.01 + delta_steer_per_iter=0.1 / [观察] 横向控制用 Stanley + crosstrack_error + yaw_diff_crosstrack / [下一步] 看纵向控制'
- '[触发点] longitudinal_control: PID(1.0, 0.1, 0.01, setpoint=5) → 假设：纵向 PID 控制速度 / [动作] 改 setpoint 干扰 / [观察] 速度控制不稳 / [下一步] 看 Lane Detection attack'
- '[触发点] Lane Detection targeted attack → 假设：KMeans 聚类 + rgb2lab + deltaE_cie76 改车道线 / [动作] 写 Python 改 image mask / [观察] 干扰车道线识别 / [下一步] Adversarial NPC'
- '[触发点] Adversarial NPC 改 npc_benign.json GPS 坐标 → 假设：rate=1.2 缩放 pos[120] vs pos[399] 锚点 / [动作] pos[120+i].latitude = rate*(pos[120].latitude - pos[399].latitude) + pos[120+i].latitude / [观察] 生成 gps_new.json / [下一步] 提交攻击 GPS'
failed_attempts:
- 试图直接改 control 参数 → 失败：参数是 demo 硬编码
- 试图不改 NPC GPS → 失败：必须攻击 NPC 才能偏离
key_observations:
- 自动驾驶对抗攻击：横向 Stanley + 纵向 PID 控制
- GPS 注入通过修改 npc JSON 即可
- 车道线检测可被 adversarial 干扰
- deltaE_cie76 颜色距离是攻击 Lane Detection 的核心
prerequisites:
- Stanley / PID 自动驾驶控制原理
- Lane Detection 视觉处理（KMeans / rgb2lab）
- deltaE_cie76 颜色距离
- Adversarial ML 攻击思路
---
# BCTF自动驾驶-WriteUp
