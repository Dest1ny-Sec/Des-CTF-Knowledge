...
lesson: '|'
quality: high
full_path: Intigriti_0823_挑戰_–_Math_jail_解法以及心得.full.md
meta_path: Intigriti_0823_挑戰_–_Math_jail_解法以及心得.meta.md
images_removed: true
images_removed_count: 0
schema_version: v3.0.0-P0
summary: Intigriti 0823 挑戰 – Math jail 解法。JavaScript Math jail 不让用 'alert' 字符串 / Function / 直接弹窗，从 Math 全局对象反射出字母和字符路径，绕过字符串黑名单。。经验：|
category: web
...
contest_type: open
wp_url: https://www.ctfiot.com/132407.html
reasoning_chain:
- JavaScript Math jail 禁用 'alert' 字符串 / Function / 直接弹窗 → 触发点：关键字黑名单
- 假设：从 Math 全局对象反射出字母和字符路径 → 动作：Math.BFS 遍历 Math 对象 keys
- 观察：找到不触发黑名单的字符串拼接路径 → 假设：拼接 'alert' 等价字符串
- 动作：用 toString / join 拼接 → eval(arr.join(''.toString())) → 观察：弹窗
- 假设：用 Function constructor 反向构造 → 动作：Function('alert(1)')()
- 观察：黑名单检测 Function 字符串失败（可拆字符）
- 动作：从 Math.__proto__.__proto__ 等链爬出 alert → 完成
failed_attempts:
- 直接 eval('alert(1)') → 失败：'alert' 黑名单
- Function('alert(1)')() → 失败：'Function' 黑名单
- 从 window 反查 → 失败：window 在 jail 里被冻结
key_observations:
- JS jail 不让用 'alert' / 'Function' 时，从 Math 全局对象反射字符是经典思路
- toString.call + join 是无 Function 字符串的拼接方法
- JS 对象的 __proto__ 链能反射出很多隐藏字符
- Math jail 关键词黑名单本质是字符串黑名单，必须绕字符串
- JS sandbox escape 的核心是找原型链路径
prerequisites:
- JavaScript 原型链（__proto__ / prototype）
- Math 全局对象属性遍历
- Function constructor 反射 + eval 黑名单绕过
---
