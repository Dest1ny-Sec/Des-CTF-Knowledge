...
quality: medium
full_path: 2024_红明谷_dualrsa.full.md
meta_path: 2024_红明谷_dualrsa.meta.md
images_removed: true
images_removed_count: 4
schema_version: v3.0.0-P0
summary: '红明谷 2024/dualrsa。83 次方根求 RCE 字符串 + eval 触发 + LSB Oracle 还原 flag。关键路径：审计 RSASystem 类发现 encrypt/decrypt 接受字符串 → encrypt/decrypt 内部 try: pow(eval(m), e, n) 存在 eval 漏洞 → eval 被限制，只能短字符串触发。经验：RSA 加密函数中 ...'
category: crypto
subcategory: rsa
subcategories:
- rsa
...
contest_type: open
wp_url: https://www.ctfiot.com/172261.html
reasoning_chain:
- 触发点：审计 RSASystem 类 encrypt/decrypt 接受字符串 → 假设：eval(m) 漏洞 → 动作：grep eval
- 观察：pow(eval(m), e, n) → 假设：RCE → 动作：构造 b"__import__('os').system('ls')" + \xff 填充
- 触发点：eval 限制短字符串 → 假设：必须让 eval(m) 返回整数 → 动作：bytes_to_long(rce)
- 观察：83 次方根（外层 RSA e=83） → 假设：iroot 求整数根 → 动作：m = gmpy2.iroot(bytes_to_long(rce), 83)[0]
- 触发点：提交 m 让服务端 encrypt 后转字符串 → 假设：eval 触发 → 动作：encrypt(m) → str → eval
- 观察：os.system('ls') 触发 → 假设：拿到 RCE → 动作：reverse shell
- 触发点：选项 3 + decrypt → 假设：LSB Oracle → 动作：decrypt(c) 取最低位
- 观察：LSB = c^d & 1 → 假设：还原 m 逐字节 → 动作：构造 2*B + r 多次解密
- 触发点：拼出完整 flag → 假设：还原明文 → 动作：long_to_bytes
- 观察：flag 输出 → 下一步：总结 eval 漏洞 + iroot 套路
failed_attempts:
- 试图不解 eval 限制 → 失败：必须 iroot 求整数根构造
- 试图不解 83 次方根 → 失败：外层 RSA e=83 不可绕过
- 试图用 SQL 注入绕过 → 失败：题目无 SQL
- 试图不构造 \xff 填充 → 失败：bytes_to_long 后长度不够
key_observations:
- RSA 加密函数中 eval 漏洞用 iroot 求整数根构造可执行 Python 字节
- LSB Oracle 配合多步签名链路还原明文
- 外层 RSA e=83 是个非标小指数 iroot 整数根必杀
- b"__import__('os').system('ls')" + b"\xff" 填充到 512 字节是固定模板
- dualrsa 名字暗示双层 RSA，LSB Oracle 是解密侧 oracle
prerequisites:
- Python eval 漏洞利用
- RSA 大数整数根 iroot（gmpy2.iroot）
- LSB Oracle 还原明文逐字节
- bytes_to_long + long_to_bytes 转换
- RSA encrypt/decrypt eval(m) 漏洞识别
---
# 2024 红明谷 dualrsa
