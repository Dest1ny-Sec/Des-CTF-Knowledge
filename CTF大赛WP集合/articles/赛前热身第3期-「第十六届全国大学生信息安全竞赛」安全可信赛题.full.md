...
quality: high
full_path: 赛前热身第3期-「第十六届全国大学生信息安全竞赛」安全可信赛题.full.md
meta_path: 赛前热身第3期-「第十六届全国大学生信息安全竞赛」安全可信赛题.meta.md
images_removed: true
images_removed_count: 9
schema_version: v3.0.0-P0
summary: 赛前热身第3期|「第十六届全国大学生信息安全竞赛」安全可信赛题。第十六届全国大学生信息安全竞赛安全可信赛题,基于cube-1.3+cube-tcm可信计算模拟器,涵盖可信报告两阶段流程+PCR寄存器设置+策略组合验证+SM2密码学。。关键路径：镜像环境:cube-1.3(通用可信软件基框架)+gm_sm2_master(开源SM2库)+cube-tcm(可信密码模块模拟器+驱动)+cube_...
category: misc
...
contest_type: school
wp_url: https://www.ctfiot.com/116929.html
reasoning_chain:
- cube-1.3 + cube-tcm + gm_sm2_master 环境 → 触发点：可信计算（TCM/TPM）方向
- 运行 tcm_quote.sh 启动可信报告机制 → 5 角色 user/term/server/crypt_user/crypt_hub
- 阶段1：user 发可信报告命令 → term 拦截 → server 拦截 → crypt_user → key_manage 生成防重放随机数
- 阶段2：term 交 key_event 添加节点+用户信息 → 拦截 crypt_user → key_manage 配可信报告模板 (TCM_PIK_DESC, PCRQUOTE)
- 动作：pcr_multirw 读 pcr 值 → quote_report 生成报告 → 报 term → server → crypt_hub key_event 接收
- key_manage 存储报告 → quote_report 验证 → 删报告内容 → key_manage 解析 → 结论回 user
- 关键：pcr_set.sh 清零 pcr，调用 pcr_set 实例向 14 号 pcr 寄存器写入策略摘要值
- 动作：修改 instance/node/pcr_set/policy_write.msg 中的 return_value（14 号寄存器+策略摘要名）
- 验证逻辑：遍历内存库所有摘要值 → policy_choice->return_value 输出
- 故意 bug：完整性报告验证过程留 bug 供研究
failed_attempts:
- 试图不重建环境直接解 → 失败：cube-1.3 部署复杂，环境初始化是关键
- 试图跳过策略组合验证 → 失败：14 号 PCR 寄存器写入是核心
- 试图直接读 key_manage 数据库 → 失败：内存库加密
key_observations:
- 可信计算（TCM/TPM）是新兴 CTF 方向，cube-1.3 是国内主流可信计算模拟器框架
- 可信报告机制（PCR_QUOTE）是核心安全机制，由 PIK 签名 PCR 值
- 策略组合验证需要遍历内存库 + 解析 return_value 才能得到完整答案
- 5 角色链式拦截（user→term→server→crypt_user→crypt_hub）是 cube 框架的设计特点
prerequisites:
- 可信计算基础（TCM/TPM 与 PCR 寄存器）
- cube-1.3 通用可信软件基框架部署
- SM2 国密算法与密钥管理
- PCR 扩展与 quote 报告生成流程
---
