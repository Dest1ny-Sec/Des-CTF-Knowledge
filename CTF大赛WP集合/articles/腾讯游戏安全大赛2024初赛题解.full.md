...
quality: high
full_path: 腾讯游戏安全大赛2024初赛题解.full.md
meta_path: 腾讯游戏安全大赛2024初赛题解.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: 腾讯游戏安全大赛2024初赛题解。腾讯游戏安全大赛2024初赛题解,Windows DLL hack.dll hook WriteProcessMemory(12字节inline hook+jmp rax+VirtualProtect)+内核驱动WPOFFx64 hook MmCopyMemory+frida反检测。。关键路径：hack.dll:DLL_PROCESS_ATTACH时Allo...
category: reverse
subcategory: reverse
tools_used:
- Frida
...
contest_type: open
wp_url: https://www.ctfiot.com/174938.html
reasoning_chain:
- 触发点：hack.dll DLL_PROCESS_ATTACH → AllocConsole + freopen stdout → 假设：可输出调试
- 动作：hook WriteProcessMemory 12 字节 inline hook → 观察：拦截所有进程写内存
- 假设：VirtualProtect 改属性后 memcpy → 动作：保存 OriginCode + 改 HookCode → 观察：hook 安装
- 动作：HackWriteProcessMemory unhook→call OriginFunc→rehook → 观察：透传原格式
- 假设：本题是 hook 检测 → 动作：用 hack.dll 反 hook WriteProcessMemory
- 触发点：WriteProcessMemory 被 inline hook → 假设：需要找 jmp 头部
- 动作：cheat engine / x64dbg 找 hook 头部 → 观察：12 字节 jmp 头部
- 假设：hack.dll 用 ZwProtectVirtualMemory 替代 VirtualProtect → 观察：修复 hook
failed_attempts:
- 直接 WriteProcessMemory → 失败：被 hook 拦截
- VirtualProtect 改权限 → 失败：也被 hook
- 不改 inline hook 直接调 → 失败：触发检测
key_observations:
- DLL inline hook 12 字节 = mov rax + jmp rax 经典写法
- VirtualProtect 被 inline hook 时用 ZwProtectVirtualMemory 替代
- WriteProcessMemory hook 是反作弊常见 hook 点
- DLL_PROCESS_ATTACH + AllocConsole 调试输出手法
prerequisites:
- Windows DLL 注入 + inline hook
- VirtualProtect / ZwProtectVirtualMemory
- Cheat Engine 反 hook 调试
- DLL_PROCESS_ATTACH 生命周期
---
# 腾讯游戏安全大赛2024初赛题解
