...
- CyberChef
attack_chain:
- 'Web F12: <!-- swpu{da45af69-6aaf-48cb-affc-4f424da5651f} -->'
- 'easy_sql: ?wllm= 参数 + order by 4# → 3 列 → union select 1,2,3 → DIOS 拿表 fllaag'
- 'happy_rce: cookie admin=1 include next.php → 访问 jiangnaij.php → 绕过 cat/flag 过滤用 $_GET 动态调 system'
- /jiangnaij.php?url=$_GET[Tao]($_GET[c]);&Tao=system&c=cat+/f*
- 'do_not_wakeup: PHP 反序列化 __destruct 触发 getenv(''FLAG'')'
- 'Genius_system: JWT alg:none 伪造 admin 权限'
- 'websec_xxe: Blind XXE OOB 读 flag 实体'
- 'finalrce: $_GET[1]($_POST[2]) 动态调用 + XFF 头'
...
quality: high
full_path: 2022_SWPUCTF_Web_Writeup.full.md
meta_path: 2022_SWPUCTF_Web_Writeup.meta.md
images_removed: true
images_removed_count: 7
schema_version: v3.0.0-P0
summary: '2022 SWPUCTF Web Writeup。SWPUCTF 2022 8 大题：F12 + 联合注入 + RCE 字符绕过 + 反序列化 + JWT + XXE。关键路径：Web F12: <!-- swpu{da45af69-6aaf-48cb-affc-4f424da5651f} --> → easy_sql: ?wllm= 参数 + order by 4# → 3 列 → uni...'
category: web
...
contest_type: open
wp_url: https://www.ctfiot.com/65440.html
reasoning_chain:
- '[触发点] F12 源码注释 → 假设：HTML 注释藏 flag / [动作] View Source / [观察] swpu{da45af69...} / [下一步] easy_sql'
- '[触发点] ?wllm= 参数 + order by 4# → 3 列 → 假设：SQLi / [动作] union select 1,2,3 / [观察] DIOS 拿表 fllaag / [下一步] happy_rce'
- '[触发点] cookie admin=1 include next.php → 访问 jiangnaij.php → 假设：动态调用绕过滤 / [动作] /jiangnaij.php?url=$_GET[Tao]($_GET[c]);&Tao=system&c=cat+/f* / [观察] RCE / [下一步] do_not_wakeup'
- '[触发点] PHP 反序列化 __destruct 触发 getenv(''FLAG'') → 假设：unserialize / [动作] 构造 payload / [观察] flag / [下一步] JWT'
- '[触发点] alg:none 伪造 admin → 假设：JWT 验签绕过 / [动作] 改 alg=none + 改 sub=admin / [观察] admin 权限 / [下一步] XXE + finalrce'
- '[触发点] Blind XXE OOB 读 flag 实体 / [动作] 构造 OOB DTD / [观察] 拿到 flag / [下一步] finalrce XFF'
failed_attempts:
- 试图 cat flag → 失败：被过滤
- 试图直接 JWT改 admin → 失败：必须 alg=none
key_observations:
- PHP $_GET[$x]($y) 动态调用绕 cat/flag 过滤是经典
- JWT alg:none 是入门 web
- Blind XXE OOB DTD 经典手法
- include 链是 PHP 文件包含 trick
prerequisites:
- SQL 注入（union/DIOS/order by）
- PHP 动态函数调用
- PHP 反序列化 __destruct
- JWT验签 + alg=none 攻击
- Blind XXE OOB
