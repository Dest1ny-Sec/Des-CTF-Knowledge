...
quality: high
full_path: 【WP】2023年春秋杯冬季赛WEB、PWN类题目解析.full.md
meta_path: 【WP】2023年春秋杯冬季赛WEB、PWN类题目解析.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: 【WP】2023年春秋杯冬季赛 WEB、PWN 类题目解析。2023 春秋杯冬季赛 Web/Pwn WP，customer-broker-merchant 队列系统 + PHP 反序列化 Redis Rogue Server + picup SSRF。。经验：消息队列三件套 customer/broker/merchant 漏洞多在 webhook 重放或订单伪造；PHP 反序...
category: web
...
contest_type: open
wp_url: https://www.ctfiot.com/159161.html
reasoning_chain:
- Active-Takeaway 三组件：customer 生成订单 → broker 队列 → merchant 接单向 webhook 发送
- webhook 重放 + 订单伪造 → 假设：可改 merchant 收件人
- ezezez_php：PHP 反序列化链 Rd.Poc.Er → __destruct → eval/assert RCE
- dict://127.0.0.1:6379/config:set:dir:/tmp → redis-rogue-server.py 加载 exp.so
- picup：抓包 302 /login.php + 404 页面收集 → 动作：SSRF 内网探测
- 内网管理页 + SQL 注入 → flag
failed_attempts:
- 试图直接改 webhook → 鉴权拒
- 试图 SSRF 内网探测全部端口 → 太多，扫 80/3306
- 试图 Redis 写 SSH 公钥 → 缺 dir 切换
key_observations:
- 消息队列 customer/broker/merchant 漏洞在 webhook 重放
- dict:// Redis module load exp.so 实现 RCE
- redis-rogue-server.py 是 SSRF→Redis RCE 经典工具
- 404 页面收集 + 302 跳转分析是 picup 入口
prerequisites:
- 消息队列 webhook 重放原理
- PHP 反序列化 POP 链
- dict:// 协议 + Redis Rogue Server
- SSRF 内网探测 + 302/404 分析
---
