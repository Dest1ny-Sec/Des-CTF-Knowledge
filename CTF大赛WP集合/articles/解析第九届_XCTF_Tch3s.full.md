...
quality: high
full_path: 解析第九届_XCTF_Tch3s.full.md
meta_path: 解析第九届_XCTF_Tch3s.meta.md
images_removed: true
images_removed_count: 0
schema_version: v3.0.0-P0
summary: 解析第九届 XCTF Tch3s。第九届XCTF Tch3s时序攻击+弱PRNG爆破,识别srand(time(NULL))弱种子+遍历时间戳匹配测试明文+LD_PRELOAD劫持+GDB调用解密函数。。关键路径：程序srand(time(NULL))初始化PRNG,16次rand()%256生成16字节key → 加密flag输出密文后,进行多轮测试(随机明文加密+记录时间) → 测试数据...
category: crypto
subcategory: oracle
tools_used:
- GDB
...
contest_type: open
wp_url: https://www.ctfiot.com/278232.html
reasoning_chain:
- ELF 用 srand(time(NULL)) 初始化 PRNG，16 次 rand()%256 生成 16 字节 key → 触发点：经典弱 PRNG 模式
- 加密 flag 后输出密文，然后多轮测试（随机明文加密+时间记录）→ 假设：测试明文是 rand()%256 输出序列
- time(NULL) 精度仅秒级，赛期几分钟内 → 假设：可遍历 ~4000 万候选 seed
- 动作：对每个候选 seed 调用 srand(seed) → 跳过 16 次 rand()（密钥生成）→ 生成 16 个数 → 动作：与第一个测试明文比对
- 观察：某个 seed 命中测试明文 → 下一步：seed 还原成功
- 假设：需用 seed 让程序重新生成相同 key → 动作：写 LD_PRELOAD 库，从 SRAND 环境变量读预设 seed 替换 time(NULL)
- GDB 在加密函数入口 0x30ba 断点 → 动作：将密文写入 rdi（假明文）
- 动作：直接调用解密函数 0x31a1（rdi/rsi/rdx 同参数）→ 观察：rsi 缓冲区出现 flag
- 字节序注意：十六进制 B789EB60 在内存小端序 60 EB 89 B7 → 整数 0x60eb89b7
failed_attempts:
- 试图仅靠密文做选择明文攻击 → 失败：密文来自未知明文（flag），无字典
- 试图用熵分析时间戳分布 → 失败：time(NULL) 只到秒级，分布平坦
- 直接 LD_PRELOAD 替换 rand() → 失败：rand() 是热点函数性能爆炸，需只替 srand
key_observations:
- srand(time(NULL)) 是 CTF 经典弱 PRNG，秒级精度几分钟遍历几年
- 测试明文输出是金矿（直接给出 PRNG 后续序列）
- LD_PRELOAD 劫持 srand + GDB 调解密函数是经典二进制复用技巧
- 'x86-64 System V ABI: rdi/rsi/rdx 是前三个参数，调用现成解密函数无需重写'
prerequisites:
- C 标准库 PRNG 状态机（srand/rand 种子与序列关系）
- LD_PRELOAD 动态链接拦截原理
- GDB 调试（断点 + 寄存器读写 + 函数调用）
- x86-64 调用约定（rdi/rsi/rdx 参数传递）
---
# 解析第九届 XCTF Tch3s
