...
quality: high
full_path: paradigm_2022_ctf_——Rescue.full.md
meta_path: paradigm_2022_ctf_——Rescue.meta.md
images_removed: true
images_removed_count: 3
schema_version: v3.0.0-P0
summary: paradigm 2022 ctf — Rescue。paradigm 2022 Rescue 区块链题，SushiSwap LP 池重入。。关键路径：MasterChefHelper 调 masterchef.poolInfo → UniswapV2RouterLike 调 swapExactTokensForTokens → addLiquidity 拿 LP。经验：'addLiquid...
category: web
...
contest_type: open
wp_url: https://www.ctfiot.com/147781.html
reasoning_chain:
- Setup 构造时把 10 ether WETH 转给 mcHelper，isSolved() 判 mcHelper.weth.balanceOf==0 → 触发点：必须抽干 mcHelper 钱包
- mcHelper.swapTokenForPoolToken 调 _swap 两次拿 tokenOut0/tokenOut1 再调 _addLiquidity 拿 LP 给 msg.sender → 假设：addLiquidity 流程有缺陷
- 动作：读 _addLiquidity 实现 → 观察：minAmountOut 形参被 router.addLiquidity 传入 amountAMin=0/amountBMin=0，require(amountOut>=minAmountOut) 但 minAmountOut 是 caller 传入
- 假设：caller 传 0 即可接受任意 amountOut → 动作：构造 Exploit 让 swapTokenForPoolToken 内部 amountOut 异常小但仍通过 require
- 观察：addLiquidity 返回 amountOut 而 msg.sender 收到 LP 凭证 → 下一步：连续 swapTokenForPoolToken 把池子两边一奶一和 tokenOut 全部抽走
- 动作：写 Exploit 合约多次调 swapTokenForPoolToken 拿光 LP 后 swapTokenForPoolToken(0, WETH, X, 0) 把 mcHelper WETH 倒回 → 完成
failed_attempts:
- 试图直接 transferFrom WETH 从 mcHelper 转走 → 失败：mcHelper 没 approve 任何地址
- 试图调 UniswapV2Pair 直接 burn LP → 失败：pair 的 LP 全在 mcHelper 而我们没 LP
- 试图用 single swap 把 WETH 全 swap 成 token → 失败：UniswapV2 k=x*y 约束下 swap 拿不完
key_observations:
- addLiquidity 的 minAmountOut 不检查 amountAMin/amountBMin = 任何人都能以 0 min 接受任意 LP
- swapTokenForPoolToken 内部先 swap 再 addLiquidity，借 0 minAmountOut 可制造 LP 但池子被抽
- Uniswap V2 pair.burn 必须先 transfer pair 给 caller 才能销毁，外部合约路径要重入或构造 caller
- MasterChefHelper 这种 helper 把 caller 资金 + 自有资金混在一起是高危模式（无 reentrancy guard）
prerequisites:
- Solidity 0.8 合约阅读（interface / external / require）
- Uniswap V2 Router / Pair / Factory 内部 swap 与 addLiquidity 流程
- Hardhat + ethers.js 部署/调用本地 fork
- EVM 重入与金额守恒（k=x*y）
---
