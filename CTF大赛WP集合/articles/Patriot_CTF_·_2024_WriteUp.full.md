...
quality: high
full_path: Patriot_CTF_·_2024_WriteUp.full.md
meta_path: Patriot_CTF_·_2024_WriteUp.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: 'Patriot CTF 2024 WriteUp。Patriot CTF 2024 狼组 WP 大全：Web (X-Forwarded/Session 伪造/XSS/XXE/EJS RCE) + Crypto (SHA-1 长度扩展) + Pwn (栈溢出/largebin/非栈 fmt)。。关键路径：giraffe notes: 加 X-Forwarded-For: 127.0.0.1 即...'
category: web
subcategory: ssti
tools_used:
- Flask
...
contest_type: open
wp_url: https://www.ctfiot.com/206569.html
reasoning_chain:
- '触发点:多题合集,giraffe notes 加 X-Forwarded-For: 127.0.0.1 → 假设:IP 白名单绕过'
- Impersonate:server_start_str='20240921153014' 派生 secret_key=sha256 → 假设:10 分钟内可猜
- 动作:/status 拿当前时间 → 枚举 server_start_str → 伪造 admin session → 观察:admin 权限命中
- Open Seasame:username 字段 XSS → adminbot 访问 /api/cal → XMLHttpRequest withCredentials → pipedream 外带 → /api/cal?modifier=|cat flag.txt
- Dogdays:SHA-1 length extension attack → view.php?pic=&hash= → 自实现 SHA-1 rotl+add+f/K 常量+块 1=1.png+块 2=/../../../flag
- Domdom:SSRF url=http://attacker/get-json 拿 XXE 注入 DOCTYPE 读 file:///app/flag.txt
- Blob:EJS settings[view options][escape]={}.constructor.constructor('return process...') → RCE
- Not So Shrimple:栈溢出写 retaddr=0x401282 走 cool... 路径 → Shellcrunch:XOR 编码+jmp rsp 重定位+read syscall
- Navigator:setpin 负索引越界+viewpin 泄 canary+libc+pie → Flight Script:largebin attack 改 loglen 溢出 0x118 → ret2libc
failed_attempts:
- 试图直接 sha256(admin) 当 secret_key → 失败:secret 与启动时间挂钩
- 试图直接调 /api/cal → 失败:需要 admin session
- 试图用字符串 fmt 泄 flag → 失败:fmt 在非栈上,需多次 sendline 拼字节
key_observations:
- Flask session secret_key 与启动时间挂钩可爆破
- X-Forwarded-For 是入门级 IP 伪造
- EJS settings[view options][escape] 是 RCE 高危面
- largebin attack 写全局变量是 ret2libc 前置
prerequisites:
- Flask session 签名原理
- SHA-1 length extension 与自实现 SHA-1
- largebin attack + ret2libc + 非栈 fmt-string
---
