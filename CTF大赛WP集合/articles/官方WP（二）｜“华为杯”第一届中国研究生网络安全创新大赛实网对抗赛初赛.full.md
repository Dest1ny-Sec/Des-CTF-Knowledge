...
quality: high
full_path: 官方WP（二）｜“华为杯”第一届中国研究生网络安全创新大赛实网对抗赛初赛.full.md
meta_path: 官方WP（二）｜“华为杯”第一届中国研究生网络安全创新大赛实网对抗赛初赛.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: 华为杯第一届中国研究生网络安全创新大赛实网对抗赛初赛(二)官方WP。华为杯研网赛初赛官方WP(二):JWT密钥混淆RS256/HS256+Coppersmith Hastad广播攻击+多项式参数化small_roots。。经验：JWT密钥混淆攻击:RS256签发HS256验证,后端secret传公钥当HS256密钥;home={pathname:...
category: misc
...
contest_type: school
wp_url: https://www.ctfiot.com/75798.html
reasoning_chain:
- 华为杯研网赛初赛 (二) 3 题：JWT 密钥混淆 + Coppersmith Hastad 广播攻击 + 多项式参数化 small_roots → 触发点：JWT 算法混淆经典漏洞
- app.js：JWT RS256 私钥签、HS256 公钥验、algorithms 含 HS256 → 假设：用公钥 HS256 伪造 admin token → 动作：claims admin+isAdmin=true+home={href/origin/protocol=file:/pathname=app.js}
- home.pathname 经 file:// 协议可写 → /upload 用 file://app.js 写 → 上传 check.js 包含 process.execSync(req.query.cmd) → 假设：home.pathname = app%2e%6a%6e
- 观察：check.js 被 require 触发 RCE
- breakMe：4 次多项式 m^4+3m^2+r*m = enc（已知 leak 函数返 n,r,enc） → 假设：4 组不同 n,r,enc 但 e=4 单变量多项式 → 动作：CRT 求 T_i
- 构造 f(m) = Σ T_i*(m^4+3m^2+r_i*m - c_i) → coppersmith small_roots(X=2^464, eps=0.03) 求 m → 假设：X 取决于位数上限
- 多项式参数化 paragen(x^3+ax+b-y^2, 3x^2+a, 3x) → 4 组用 x0, x0+i, x0+i+j, x0+s 构造三次多项式 → 假设：small_roots X=2^984 联合爆破
- 观察：找到 small_roots 得到明文
failed_attempts:
- JWT 单签 HS256 用签发密钥 → 失败：题面 secret 传的是公钥
- Coppersmith 用单一 secret 公钥 → 失败：必须 CRT 合并 4 组
- 硬编码 5 字节 'flag{' 已知头不缩小 X → 失败：X=2^464 是 e*bit+pad 的默认
- 对所有 X 全爆破 → 失败：small_roots 是有限域求解，参数化提高效率
key_observations:
- JWT 密钥混淆攻击：RS256 签 HS256 验，secret 传公钥 = 完整 admin token 伪造
- home 对象内嵌 pathname=app%2e%6a%6a 能让 file:// 协议写任意路径
- Hastad 广播攻击 4 个不同 n 同 e=4 次多项式 = CRT + Coppersmith 模板
- 已知名文头 5 字节 'flag{' 可减少 small_roots 搜索空间
- paragen 三次多项式 + 三次偏导为单变量多项式爆破提速
prerequisites:
- JWT 算法 (RS256/HS256) 与算法混淆漏洞原理
- Coppersmith / SageMath small_roots 使用
- CRT 合并多模剩余
- Sage 多项式参数化 paragen 与偏导
---
