...
lesson: '|'
quality: high
full_path: Java代码分析工具Tabby在CTF中的运用.full.md
meta_path: Java代码分析工具Tabby在CTF中的运用.meta.md
images_removed: true
images_removed_count: 9
schema_version: v3.0.0-P0
summary: Java 代码分析工具 Tabby 在 CTF 中的运用。用 Tabby (soot + Neo4j) 静态分析 Java 代码属性图 + Cypher 查询，自动化找反序列化 gadget 链和 hessian JDK-only 链。。经验：|
category: misc
...
contest_type: open
wp_url: https://www.ctfiot.com/64572.html
reasoning_chain:
- Java 反序列化 gadget 链查找 → 触发点：静态分析自动化
- 假设：手工找 gadget 太慢 → 动作：用 Tabby (soot + Neo4j) 生成代码属性图
- 观察：Cypher 查询 sink → source 路径 → 下一步：定位危险链
- 动作：tabby-path-finder 污点分析扩展 → 假设：长链路精准
- 观察：找到 Hessian JDK-only gadget → 完成
- 假设：传统 ysoserial 工具不够 → 动作：Tabby 找出未公开 gadget
failed_attempts:
- 手工 grep sink → source → 失败：链路太长
- 用 soot 直接分析 → 失败：缺少图查询能力
- 用 IDEA 插件 → 失败：分析规模有限
key_observations:
- Tabby 是 Java 静态分析利器，soot + Neo4j 是核心
- 代码属性图（CPG）+ Cypher 查询是静态分析新趋势
- Hessian JDK-only gadget 必须脱离 commons-collections
- Java 反序列化 sink 集中在 readObject / toString / hashCode
- Tabby-path-finder 解决长链路污点分析精度问题
prerequisites:
- Java 反序列化原理 + soot 框架 / Neo4j 图数据库
- Cypher 查询语言 + 代码属性图（CPG）概念
- Hessian / Jackson / Fastjson gadget 链
---
