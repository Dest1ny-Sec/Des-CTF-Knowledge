...
quality: high
full_path: 强网杯_2024_初赛_Writeup.full.md
meta_path: 强网杯_2024_初赛_Writeup.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: 强网杯 2024 初赛 Writeup - S1uM4i。强网杯2024初赛 S1uM4i全方向:Pwn堆溢出_Unwind_RaiseException+魔术Gadgets+RC4/异或mips_remem逆向。。经验：Pwn利用_Unwind_RaiseException(0x4C60偏移)+魔术Gadgets(libc-2.35+0x...
category: misc
...
contest_type: open
wp_url: https://www.ctfiot.com/213409.html
reasoning_chain:
- 题目是 S1uM4i 全方向（pwn + reverse） → 触发点：chat_with_me 是堆溢出 + off-by-null 改 size
- 假设：魔术 Gadgets 可绕过 stack check → 动作：找 _Unwind_RaiseException 在 libc 偏移 0x4C60
- 观察：libc-2.35+0x58740 是 system 偏移 → 假设：能劫持 __free_hook 或栈迁移到 _Unwind_RaiseException
- 动作：栈迁移 + 调 system → 观察：得 RCE
- 触发点：expect_number 是栈溢出 → 假设：爆破 text base（PIE 低熵）→ 动作：发 01 串构造输入 + 越界读泄 base
- 动作：栈溢出跳 _Unwind_RaiseException → 观察：得 RCE
- 触发点：逆向 mips_emu → 假设：0x23000 mmap 位置定位解密函数 → 动作：异或验证输入 → 观察：还原 flag
- 触发点：remem 是 RC4 + 字节交换 + 多层 XOR → 假设：自实现 S 盒 t<<7|>>1 + 16*((32*v3|v3>>3)^0xAD) → 动作：dump+nop 调试自生成代码 → 观察：还原 flag
failed_attempts:
- 试图直接覆盖 __free_hook → 失败：libc 2.35 已禁用 hook
- 试图不爆破 text base 越界读 → 失败：栈溢出 PIE 随机化必须 leak
- 试图对 remem 单层 RC4 解 → 失败：还有字节交换和 XOR 多层
key_observations:
- _Unwind_RaiseException(0x4C60 偏移) + 魔术 Gadgets(libc-2.35+0x58740 system) 绕过 stack check
- expect_number 爆破 text base + 栈溢出跳 _Unwind_RaiseException 是 PWN 入门套路
- mips_emu 中 0x23000 mmap 位置定位解密函数是 mips 仿真器逆向标准锚点
- remem 需自生成代码调试 + 字节序转换 + 64 位无符号整数运算
- RC4(key=b'6105t3') + swap 字节 + 多层 XOR + 自实现 S 盒(t<<7|>>1) + 16*((32*v3|v3>>3)^0xAD)
prerequisites:
- glibc 2.35 堆利用（_Unwind_RaiseException 栈迁移）
- MIPS 指令集与 mips_emu 反汇编
- RC4 流密码 + 字节交换
- 自实现 S 盒（位运算代替 AES）
---
# 强网杯 2024 初赛 Writeup
