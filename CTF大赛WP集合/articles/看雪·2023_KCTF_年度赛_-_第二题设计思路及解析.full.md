...
quality: medium
full_path: 看雪·2023_KCTF_年度赛_-_第二题设计思路及解析.full.md
meta_path: 看雪·2023_KCTF_年度赛_-_第二题设计思路及解析.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: 看雪·2023 KCTF 年度赛 | 第二题设计思路及解析。KCTF2023年度赛Q2《CN星际基地》39球称量问题+测量矩阵+IsDebuggerPresent反调试。经验：称重问题用测量矩阵(0/-1/1)表示；每次测量结果-1/0/1；最少4次测量得解
category: reverse
subcategory: reverse
...
contest_type: open
wp_url: https://www.ctfiot.com/133753.html
reasoning_chain:
- IDA打开有反调试 → 触发点：IsDebuggerPresent的if判断
- 动作：if取反patch（jne jz互换或直接jmp） → 下一步：进入main逻辑
- main里srand(time(0))初始化 → 假设：随机种子用当前时间
- 读Serial输入+测量矩阵4次测量 → 触发点：39球称量问题编码
- 测量矩阵：0不选/1右侧/-1左侧 → 假设：每次测量得-1/0/1三态结果
- 动作：4次称量得4维向量 → 下一步：反推异常球
- 假设：4维向量×矩阵逆=异常球编号 → 动作：建立矩阵方程求逆
- 观察：srand(time(0))导致每次运行测量矩阵不同 → 必须确定种子或静态分析
- 提交异常球编号对应的Serial → flag
failed_attempts:
- 试图不改srand(time(0))调试 → 失败：测量矩阵随机变化，无法重现
- 直接根据4维结果反推球号 → 失败：4维向量需配合具体测量矩阵才能求逆
- 忽略反调试patch → 失败：IsDebuggerPresent会让程序走错误分支
key_observations:
- 称重问题用测量矩阵(0/-1/1)编码每次测量（不选/左/右）
- 结果-1/0/1表示左平/平/右重
- 39球4次测量是最优称量方案
- srand(time(0))是随机种子常见模式，可固定时间重现
- 测量矩阵求逆是线性代数应用（GF(3)或整数环）
prerequisites:
- 称重问题数学建模
- IsDebuggerPresent反调试绕过
- 线性代数求逆矩阵
- srand/rand种子固定技巧
---
