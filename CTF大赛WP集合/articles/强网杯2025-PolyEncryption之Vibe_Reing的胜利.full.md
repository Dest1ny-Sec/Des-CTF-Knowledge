...
quality: high
full_path: 强网杯2025-PolyEncryption之Vibe_Reing的胜利.full.md
meta_path: 强网杯2025-PolyEncryption之Vibe_Reing的胜利.meta.md
images_removed: true
images_removed_count: 5
schema_version: v3.0.0-P0
summary: 强网杯2025 PolyEncryption - Vibe Reing的胜利 (看雪SleepAlone)。强网杯2025 PolyEncryption:AI辅助+dotnet反编译+saw.py Hack.java patch+22轮AES风格PolyEnc纯Python实现。。经验：AI辅助逆向+手动验证+patch关键模块;dotnet polyencrypt.dll分析策略:先看sa...
category: reverse
subcategory: reverse
tools_used:
- Python
...
contest_type: open
wp_url: https://www.ctfiot.com/287808.html
reasoning_chain:
- 拿到 polyencrypt.dll → 触发点：dotnet 反编译难度大，触发点：搜 saw.py/Hack.java 关键模块字符串
- 假设：AI 辅助推理数据流能加速 → 动作：把明文 abe74e3a9c375b3428bf31d1f8fa49c1 喂给 GPT 让其推加密链路 → 观察：得到逆序→小端→AES 风格 22 轮结论
- 假设：必须 Patch 掉混淆/反调试才能看真实逻辑 → 动作：用 dnSpy/ILSpy patch saw.py 和 Hack.java → 观察：得到 22 轮细节
- 触发点：Debug docker 输出日志敲定 Sbox/MixCols/ShiftRows 交错细节 → 观察：奇数轮 SubBytes+MixColumns，偶数轮 ShiftRows+AddRoundKey
- 假设：密钥级联 s[1]=A^sbox(rol(D,8))^round_key → 动作：纯 Python 实现 PolyEnc 22 轮 → 观察：加解密自校验通过
- 动作：用逆 MixColumns 矩阵 [0x0E,0x0B,0x0D,0x09] 解 flag.enc → 观察：还原明文 flag
- 下一步：整理 AI 提示词模板，便于下次 dotnet 加密题快速建模
failed_attempts:
- 试图直接用 ILSpy 反编译整个 polyencrypt.dll → 失败：混淆 + 反调试让反编译栈炸掉
- 试图逆向 22 轮写出标准 AES 还原 → 失败：奇偶轮操作顺序不同，不是标准 AES
- 试图用 .NET Reflection 调 Encrypt 方法直接拿密钥 → 失败：方法被混淆拦截
key_observations:
- dotnet 自定义加密常引入 saw.py/Hack.java 等 Python 模块做数据流，Patch 比反编译省力
- AES 风格加密若奇偶轮操作错位，就是魔改 AES，必须独立还原而不是套标准实现
- AI 辅助适合数据流推理但容易在轮数/密钥派生上瞎编，必须用 docker debug 输出对齐
- 逆 MixColumns 矩阵 [0x0E,0x0B,0x0D,0x09] 是 AES 标准逆矩阵的列置换表达
- dotnet 加密题先把混淆 patch 干净再让 AI 推链路，否则 AI 看到的代码是被搅乱的
prerequisites:
- .NET 程序集结构（dll / IL / Reflection）
- AES 密钥调度 + MixColumns/SubBytes/ShiftRows 矩阵数学
- GF(2^8) 上的乘法与逆运算
- dnSpy/ILSpy 反编译与 patch 技巧
---
