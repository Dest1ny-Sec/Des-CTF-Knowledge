...
quality: high
full_path: 解析2025LitCTF_leak.full.md
meta_path: 解析2025LitCTF_leak.meta.md
images_removed: true
images_removed_count: 0
schema_version: v3.0.0-P0
summary: 解析2025LitCTF leak。LitCTF 2025 leak方向RSA dp高位泄露攻击深度解析,180位dp低位+LLL/Coppersmith寻找小根恢复完整dp,侧信道+实现漏洞双重视角。。关键路径：题目生成2048位RSA,e=101位素数,泄露dp的高841位,hint=dp>>180 → 未知部分:0≤x<2^180 (约1.5×10^54种可能),1≤k<e (约10^...
category: crypto
subcategory: rsa
time_required: long
difficulty_score: 4
...
contest_type: open
wp_url: https://www.ctfiot.com/279542.html
reasoning_chain:
- 题目 e*dp ≡ 1 (mod p-1) → 泄露dp>>180高841位 → 触发点：RSA-CRT dp高位泄露
- 完整dp=dp_high*2^180+x, x<2^180 → 假设：dp低位x是未知小量 → 动作：构造bivariate多项式
- F(x,k)=A+e*x+k能被p整除, x≪p, k≪p → 触发点：Coppersmith小根攻击
- 动作：sage的f.monic().small_roots(X=2^180, beta=0.4) → 观察：恢复x,k
- 从短向量恢复完整dp → dp*p验证在[p+1, p+0x100] → 假设：反推p,q
- 动作：q=n//p → 标准RSA m=pow(c,d,n) → 观察：拿到flag
failed_attempts:
- 试图直接猜dp低180位 → 失败：2^180≈1.5×10^54种可能过大
- 试图对n做因数分解 → 失败：n是2048位RSA标准,无法直接分解
- 试图用Wiener攻击 → 失败：e=101位素数,不是连分数小私钥场景
key_observations:
- RSA-CRT实现泄露dp高位是致命的,180位低位+Coppersmith足以恢复完整dp
- Coppersmith小根攻击对(x≪n)的多项式F(x)有效
- sage的small_roots(X=上限, beta=0.5)是必备工具
- 实际部署应避免计算/存储dp,dq等CRT参数,启用constant-time算法+HSM
prerequisites:
- RSA-CRT实现原理
- Coppersmith小根攻击
- sage数学工具使用
