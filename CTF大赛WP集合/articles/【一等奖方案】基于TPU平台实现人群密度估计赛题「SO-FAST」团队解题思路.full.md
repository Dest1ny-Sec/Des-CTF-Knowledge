...
quality: high
full_path: 【一等奖方案】基于TPU平台实现人群密度估计赛题「SO-FAST」团队解题思路.full.md
meta_path: 【一等奖方案】基于TPU平台实现人群密度估计赛题「SO-FAST」团队解题思路.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: '【一等奖方案】基于TPU平台实现人群密度估计赛题「SO-FAST」团队解题思路。2022 CCF BDCI TPU 密度估计冠军 SO-FAST：CSRNet + INT8 量化 + 4N Batch + 2×2 图片划分 + 推理加速。。关键路径：第十届 CCF 大数据与计算智能大赛 → 队伍: SO-FAST (宋礼/柯嵩宇/包锴楠) 京东/上交/西南交大 → 任务: CSRNet 在 ...'
category: crypto
subcategory: math
time_required: long
difficulty_score: 4
...
contest_type: open
wp_url: https://www.ctfiot.com/124006.html
reasoning_chain:
- CCF BDCI TPU 人群密度估计 → 触发点：TPU 平台 BM1684 部署 CSRNet → 假设：INT8 量化比 FP32 快 2×
- 动作：CSRNet FP32 vs INT8 对比 → 观察：INT8 精度损失小，速度提升 4×
- 量化图片选择 → 触发点：校准集影响精度 → 动作：挑 NWPU-Crowd 第 178 张（大规模+广色差）→ 观察：精度 +0.5%
- 推理加速 1：图片尺寸变换 → 触发点：单张 1024×1024 太慢 → 动作：拆 2×2 四块各 512×512 → 观察：0.5s/张
- 推理加速 2：4N Batch → 触发点：INT8 + INT32 关系 → 假设：4 张拼接 batch = 一次推理 4 张
- 动作：4 张拼 batch=4 输入 → 观察：0.27s/张，比 batch=1 快近 2×
- BM1684 部署实测 → 动作：跑 score 公式 (250-MAE)*0.2 + (500-RMSE)*0.1 + (0.4-NAE)*200 + (2-time)*100
- 观察：A 榜 229.73 第一 + B 榜 263.78 第一 → 完成
failed_attempts:
- 试图直接用 FP32 CSRNet → 失败：精度高但速度慢，score 中 time 项扣太多
- 试图用 8 张 batch → 失败：BM1684 INT8 单 batch 上限 4，再大反而慢
- 试图不选量化图片用全部 → 失败：精度反而下降，过拟合校准集
key_observations:
- TPU 部署 5 步：模型选择→INT8 量化→量化图片筛选→推理加速（尺寸+4N Batch）→部署实测
- 4N Batch 利用 INT8+INT32 数据关系 4 张一次推理降一半时间
- 图片预处理的尺寸+锐度+亮度组合对 score 贡献最大
- 校准集挑选要含大规模人群+广色差，单一场景校准泛化差
- BM1684 是算能科技 TPU，INT8 推理效率高于 FP32 4×
prerequisites:
- 深度学习模型部署（量化/编译/bmodel）
- CSRNet / MCNN / VGG 等密度估计模型架构
- 算能 BM1684 TPU 工具链
- 图像预处理（尺寸归一化+增强）
---
