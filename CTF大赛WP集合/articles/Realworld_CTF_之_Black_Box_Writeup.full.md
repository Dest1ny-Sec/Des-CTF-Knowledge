...
quality: high
full_path: Realworld_CTF_之_Black_Box_Writeup.full.md
meta_path: Realworld_CTF_之_Black_Box_Writeup.meta.md
images_removed: true
images_removed_count: 5
schema_version: v3.0.0-P0
summary: Realworld CTF - Black Box Writeup (CVE-2020-14364 QEMU USB 逃逸)。Realworld CTF 之 Black Box：CVE-2020-14364 QEMU 5.15 USB UHCI 驱动 do_token_setup setup_len 用户可控 → data_buf 越界读写 → 改 UHCI frame_list 任意地址写...
category: pwn
...
contest_type: open
wp_url: https://www.ctfiot.com/27566.html
reasoning_chain:
- 题目 QEMU 5.15.0 + usb-tablet → 触发点：CVE-2020-14364 USB UHCI 驱动逃逸
- 读 QEMU 源码 uhci.c → 看到 do_token_setup 设 s->setup_len 用户可控 → 假设：data_buf 越界读写
- do_token_out 写 setup_index=-8 → 越界改 s->setup_buf[0]=USB_DIR_IN → 下一步：do_token_in OOB 读
- 物理地址 OOB 读 leak QEMU 基址 → 假设：可改 frame_list_base 任意写
- uhci_port_write 0x10/0x12 端口设 frame_list_base → timer_mod_ns 触发 uhci_frame_timer
- 覆盖 timer_list 节点伪造时钟 → 任意地址写 + RCE
- 完整链：do_token_setup(size=0x5000) → do_token_out(-8) → do_token_in(OOB) → frame_list 劫持 → system
failed_attempts:
- 试图用普通 USB 设备命令 → 失败：UHCI token 协议必须 setup_len 可控
- 试图只做一次 OOB 读 → 失败：还需写 frame_list 才能劫持控制流
- 试图找 libc 主程序漏洞 → 失败：题目是 VM 逃逸需走 UHCI 协议层
key_observations:
- USB UHCI 协议 token descriptor 中 setup_len 用户可控是经典 VM 逃逸面
- data_buf 越界 + setup_index 整数下溢是 USB 设备逃逸通用攻击面
- virt+phys 双地址空间分析是 QEMU VM 逃逸必备技能
- uhci_frame_timer 链是 UHCI 控制器劫持控制流的常用 sink
- Clone-and-Pwn 模式先看最近 CVE 修复记录定位可疑函数
prerequisites:
- QEMU softmmu + USB UHCI 驱动架构
- Linux 内核 USB 子系统协议细节
- CVE-2020-14364 漏洞原理（uhci do_token_setup 路径）
- QEMU 设备逃逸常规利用链
---
