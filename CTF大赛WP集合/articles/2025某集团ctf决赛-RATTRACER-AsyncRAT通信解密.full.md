...
tags:
- RATTRACER 2025 决赛
- AsyncRAT-C-Sharp 开源项目
- ST.p12 PKCS#12 私钥
- openssl pkcs12 -info -in ST.p12 无密码
- openssl pkcs12 -legacy -in ST.p12 -nocerts -nodes -out key.pem
- Wireshark TLS 解密 ip port 私钥
- 1f 8b 08 gzip 头
- MsgPack 解析
- 0xc4 0xc6 二进制标记
- gzip_uncompress 批量
- RDP JPEG ffd8 ffd9 提取
- remoteDesktop- 流量
attack_chain:
...
quality: high
full_path: 2025某集团ctf决赛-RATTRACER-AsyncRAT通信解密.full.md
meta_path: 2025某集团ctf决赛-RATTRACER-AsyncRAT通信解密.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: 2025 某集团 ctf 决赛 RATTRACER - AsyncRAT 通信解密（TLS 私钥 + gzip + MsgPack + RDP JPEG）。2025 某集团决赛 RATTRACER：ST.p12 私钥解 TLS + AsyncRAT 通信解密 gzip+MsgPack + RDP JPEG 截图提取 flag。。关键路径：1. 看到证书 p12 → openssl pkcs1...
category: forensic
...
contest_type: open
wp_url: https://www.ctfiot.com/285924.html
reasoning_chain:
- 触发点：拿到 ST.p12 + pcap 流量 → 假设：TLS 解密 → 动作：openssl pkcs12 -info -in ST.p12 检密码
- 观察：无密码 → 下一步：openssl pkcs12 -legacy -in ST.p12 -nocerts -nodes -out key.pem 提私钥
- 触发点：Wireshark → 编辑→首选项→protocols→tls → 填 ip port 私钥（key.pem）→ 假设：TLS 解密后看 AsyncRAT 特征
- 动作：搜索 github 'AsyncRAT-C-Sharp' → 假设：开源项目特征匹配
- 触发点：内层看到 1f 8b 08 gzip 头 → 假设：批量 gunzip → 动作：gzip_uncompress(buf[6:6+len_zipdata-4])
- 触发点：gzip 后 MsgPack 头 0xc4 / 0xc6 → 假设：0xc4=short binary / 0xc6=long binary → 动作：readbinary 解析大端长度
- 触发点：remoteDesktop- 流量 + JPEG SOI/EOI ffd8/ffd9 → 假设：RDP 截图 → 动作：ffd8ffd8 起点 + ffd9 终点切片
- 下一步：flag 在 RDP 截图里
failed_attempts:
- 试图给 p12 设默认密码 → 失败：openssl pkcs12 -info 直接回车确认无密码
- 试图不解 TLS 直接看内容 → 失败：握手完成后数据全是密文
- 试图手写 gzip 解压 → 失败：Python gzip.decompress / zlib 解压更稳
key_observations:
- AsyncRAT 通信 = TLS 1.x 外层 + gzip+MsgPack 内层 + 0xc4/0xc6 binary 标记
- p12 无密码直接 openssl 提私钥 → Wireshark TLS 解密
- 开源 RAT 项目特征识别（github）能给出完整内层结构
- RDP JPEG 切片 SOI ffd8ffd8 + EOI ffd9 是流量里挑截图的标准做法
- MsgPack 0xc4=short binary / 0xc6=long binary 是格式常量
prerequisites:
- openssl pkcs12 命令使用
- Wireshark TLS 解密配置
- gzip / zlib 解压 API
- MsgPack 0xc4/0xc6 协议格式
---
