...
- easyCAS
- ASLR-CAS
- Pwn-0x2d44溢出+ret2libc
- klang编译器漏洞
- Netatalk-CVE-2022-23121
- AppleDouble-header
- FPOpenFork
...
quality: high
full_path: 阿里云CTF_2024_writeup.full.md
meta_path: 阿里云CTF_2024_writeup.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: 阿里云CTF 2024 writeup。阿里云CTF 2024 El3ctronic战队第四名全方向:web签到Pastbin+easyCAS+Re欧拉+sign_in溢出ret2libc+klang编译器fuzz类型混淆写shellcode+Netatalk CVE-2022-23121 AppleDouble改_dl_rtld_lock_recursive=system。经验：1) ea...
category: web
...
contest_type: open
wp_url: https://www.ctfiot.com/170638.html
reasoning_chain:
- web签到 → 触发点：抓包 base64 + hash(.) → 假设：解码+拼接 submit token
- easyCAS → 假设：Go 协程并发限流 → 动作：limiter1(64)+limiter2(32)+select{}
- Pastbin → 触发点：handle(mws=append) 中间件链 → 假设：route 越权
- Re 欧拉图 → 假设：邻接矩阵 dfs+约束 → 动作：e[u][v]!use[u][v]+flag[14]=7
- Pwn sign_in → 触发点：read(0x500) 溢出 c0(0x300) 覆盖 ptr
- 动作：payload='a'*0x88+p64(pop_rdi)+p64(1)+p64(pop_rsi)+p64(write_got)*2+p64(write_plt)+p64(0x4005BD)
- 观察：write_got leak → libc base → ROP __free_hook+system
- klang → 假设：get_int/arb_read/arb_write/array_new 类型混淆 → 动作：fuzz
- 观察：写 shellcode 到栈 + 劫持 puts_got
- Netatalk CVE-2022-23121 → 假设：FPOpenFork(flag=2)+AppleDouble ADEID_FINDERI 偏移 0x32
- 动作：memmove 改 _dl_rtld_lock_recursive=system → 'cat /flag.txt > /home/…/shared/flag'
failed_attempts:
- 试图 union 注入 web 签到 → 失败：MIME+skip 中间件拦
- 试图不开 fuzz 静态分析 klang → 失败：类型系统复杂
key_observations:
- Go Router handle(mws=append) 中间件链越权是 web 经典
- limiter1+limiter2 + select{} 并发限流绕过易踩坑
- 0x2d44 read(0x500) 溢出覆盖 ptr → write_got leak → ROP 套路
- Netatalk CVE-2022-23121 通过 AppleDouble 偏移改 _dl_rtld_lock_recursive 是经典 AFL 漏洞
- klang 编译器类型混淆 fuzz 找 type confusion 漏洞
prerequisites:
- Go 中间件/Router 链 + 并发模型
- 欧拉图/邻接矩阵 dfs
- ROP 链构造 + GOT 劫持
- Netatalk AppleDouble 协议 + CVE-2022-23121
---
# 阿里云CTF 2024 writeup
