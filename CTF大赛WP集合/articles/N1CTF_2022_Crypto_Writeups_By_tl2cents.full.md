...
lesson: '|'
quality: high
full_path: N1CTF_2022_Crypto_Writeups_By_tl2cents.full.md
meta_path: N1CTF_2022_Crypto_Writeups_By_tl2cents.meta.md
images_removed: true
images_removed_count: 0
schema_version: v3.0.0-P0
summary: 'N1CTF 2022 Crypto Writeups By tl2cents (RSA 变种)。N1CTF 2022 Crypto: RSA 变种 (pubkey=(s, t, n), enc(m)=(m*r^s, m*r^t)), 字节级爆破 m (0-255) + xgcd 求 r。。经验：|'
category: crypto
subcategory: rsa
time_required: long
difficulty_score: 4
...
contest_type: open
wp_url: https://www.ctfiot.com/76390.html
reasoning_chain:
- 触发点：RSA 变种 enc(m, k) = (m*r^s, m*r^t) 字节级爆破 → 假设：每个 byte m 单独加密，可遍历 0-255 还原
- 动作：keygen p,q (512-bit) → n=pq, phi=(p-1)(q-1) → a = getrandbits(1024), b = phi + 1 - a, s = getrandbits(1024), t = -s*a * inverse(b, phi) % phi
- pubkey = (s, t, n), privkey = (a, b, n) → 假设：s*t 关系可还原
- enc(m, k)：r = getrandbits(1024) → return (m * pow(r, s, n) % n, m * pow(r, t, n) % n) → 假设：两个密文都是 m * r^xxx
- 动作：xgcd(s, t) 求 (g, x, y) 让 s*x + t*y = g → 假设：用 s/t 的 Bézout 系数拼 r
- z = pow(c1, x, n); w = pow(c2, y, n); mm = pow(m, -(x+y), n); r_ = mm * z * w % n → 假设：还原 r_
- 动作：验证 c1_ = (m * pow(r_, s//g, n)) % n == c1 → 假设：r_ 计算正确，遍历 m=0..255 找匹配的字节
- 循环所有 enc(c1, c2) 对还原 encflag → long_to_bytes 转 flag
failed_attempts:
- 试图用单次 enc 求 m → 失败：enc 引入随机 r，必须用 enc 的两次输出配对爆破
- 试图用 pow(c, d, n) 标准 RSA → 失败：privkey (a, b, n) 不是标准 RSA 私钥
- 试图遍历整个 r（1024 bit）→ 失败：r 是随机数空间太大，必须用 Bézout 系数 s*x+t*y=g 缩小空间
key_observations:
- RSA 变种 enc(m, k) = (m*r^s, m*r^t) 的核心攻击 = xgcd(s,t) 求 Bézout 系数 + 字节级爆破 m
- keygen 中 s*t 关系：t = -s*a * inverse(b, phi) % phi 是关键，Bézout 系数让 s*x+t*y=g
- 每个 byte m 单独加密 = 字节级爆破 0-255 可行（256 次而非全空间）
- xgcd / extended Euclidean algorithm / Bézout 系数是 RSA 变种攻击必备数学工具
- N1CTF Crypto 偏好 RSA 变种 + 数论难题（CRT / Bézout / Pohlig-Hellman）
prerequisites:
- RSA 数学基础（modular exponentiation / CRT / Bézout）
- 扩展欧几里得算法 xgcd（extended Euclidean）
- Python Crypto.Util.number + gmpy2 工具
- RSA 变种识别（enc(m,k)=(m*r^s, m*r^t) 模式）
---
