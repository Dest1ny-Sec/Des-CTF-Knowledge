...
quality: high
full_path: 腾讯云安全挑战赛·COS提权与利用_WP.full.md
meta_path: 腾讯云安全挑战赛·COS提权与利用_WP.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: 腾讯云安全挑战赛·COS提权与利用 WP。腾讯云安全挑战赛COS提权与利用WP,Java CrackMe反编译AES-ECB+XOR deobfuscate解出AK/SK/Token,COS Bucket Policy分析(IPCAM not-equal限制+PutBucketACL),CVM ResetInstancesPassword+SSRF绕过链。。关键路径：Java CrackMe...
category: crypto
subcategory: oracle
tools_used:
- Java
...
contest_type: open
wp_url: https://www.ctfiot.com/271105.html
reasoning_chain:
- 触发点：AbstractHashMap 中 LiLiLLLiiiLLiiLLi='Q8Ro9cHYVAdBaRD8' → 假设：AES-ECB key
- 动作：写 Python aes_decrypt(KEY, ECB) 解密 GIiIiLA 数组 → 观察：得到 AK/SK/Token
- 假设：deobfuscate 是 XOR + 偏移 → 动作：i_start=(11963830^57816743)^(15501129^52968536)
- 假设：token 是 AES 解密后 deobfuscate → 动作：计算 xor_key=(95558172^72233696)^(85510964^82109419)
- 观察：拿到 AKID3eUqV0RTdHaRQ6PeyFzouVVSsd1c5oU4dgxxMSdepyySy-3TzBV2As5bsmVZe6LW
- 动作：用 CosConfig 构造 client + get_bucket_policy → 观察：得到 bucket policy JSON
- 触发点：bucket policy 含 ip_not_equal 限制 → 假设：通过 CVM ResetInstancesPassword 提权
- 动作：用 CommonClient('cvm', '2017-03-12') ResetInstancesPassword root 密码 → 观察：提权成功
failed_attempts:
- 硬读 jar 字符串 → 失败：混淆加密
- 默认 AK/SK 测试 → 失败：key 派生自 LiLiLLLiiiLLiiLLi
- CVM 实例登录 → 失败：缺提权凭证
key_observations:
- AES-ECB + 自定义 XOR deobfuscate 是 jar 字符串反混淆标配
- 腾讯云 COS get_bucket_policy 可读 bucket ACL
- COS PutBucketACL + IP 限制 + CVM ResetInstancesPassword 提权链
- 腾讯云黑客松 GetBucketPolicy 路径:https://*.cos.*.myqcloud.com/
prerequisites:
- AES-ECB 加解密
- 腾讯云 COS Python SDK
- 腾讯云 CVM API 调用
