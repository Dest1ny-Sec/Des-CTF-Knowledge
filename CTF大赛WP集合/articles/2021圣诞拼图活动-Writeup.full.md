...
- MinBinHeapq
attack_chain:
- 题目给 4x4 拼图（16 块），一块全黑，其余 15 块打乱
- 随机滑动 64 步，理论上 ≤64 步可还原
- '原图块对应序列: 5, 1, 6, 8, 2, 11, 7, 13, 15, 14, 3, 4, 10, 16, 12, 0'
- A* 算法 + MinBinHeapq 优先队列 + 曼哈顿距离启发
- 求解 72 步（>64 步时重置）
- '输出方向: RRDDRDLULURRULDLDLUURRRDLLDRULDDRULUURDRULLDRRDLLLDRURULLDDRULURRDLDRURU'
- 步数超出时多刷几张图重试
...
quality: medium
full_path: 2021圣诞拼图活动-Writeup.full.md
meta_path: 2021圣诞拼图活动-Writeup.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: '2021 圣诞拼图活动 Writeup。16-puzzle 数字华容道 A* 自动求解。关键路径：题目给 4x4 拼图（16 块），一块全黑，其余 15 块打乱 → 随机滑动 64 步，理论上 ≤64 步可还原 → 原图块对应序列: 5, 1, 6, 8, 2, 11, 7, 13, 15, 14, 3, 4, 10, 16, 12, 0。经验：N-puzzle 是经典 AI 搜索问题；曼哈...'
category: misc
...
contest_type: open
wp_url: https://www.ctfiot.com/19654.html
reasoning_chain:
- '[触发点] 题目给 4x4 拼图，随机滑动 64 步 → 假设：数字华容道 16-puzzle / [假设] ≤80 步内必可还原（≤64 步理论解 + 一些冗余） → [动作] 写 A* 求解器，曼哈顿距离启发 / [观察] 找到序列 5,1,6,8,2,11,7,13,15,14,3,4,10,16,12,0 / [下一步] 用 A* 输出 72 步方向'
- '[触发点] A* 单次可能超 80 步 → 假设：需要重置 / [动作] 多次生成拼图重跑 / [观察] 最终步数 72< 80 / [下一步] 提交方向序列'
- 观察：拼图越小曼哈顿距离越小 → 假设：固定起点会找到最优解 → 动作：固定 (0,0) 起点跑 100 次取最短
failed_attempts:
- 试图用 BFS 暴力搜索 → 失败：状态空间 16!/2 = 10^13 太大
- 试图用 IDA* → 失败：边界函数实现复杂
key_observations:
- 16-puzzle 是经典 AI 搜索问题
- 曼哈顿距离是 admissible heuristic
- MinBinHeapq 优先队列是 A* 标准实现
prerequisites:
- A* 搜索算法
- admissible heuristic 概念
- Python heapq 优先队列
---
# 2021圣诞拼图活动-Writeup
