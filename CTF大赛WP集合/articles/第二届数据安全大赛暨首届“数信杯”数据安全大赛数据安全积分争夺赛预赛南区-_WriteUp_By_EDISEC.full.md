...
quality: medium
full_path: 第二届数据安全大赛暨首届“数信杯”数据安全大赛数据安全积分争夺赛预赛南区-_WriteUp_By_EDISEC.full.md
meta_path: 第二届数据安全大赛暨首届“数信杯”数据安全大赛数据安全积分争夺赛预赛南区-_WriteUp_By_EDISEC.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: 第二届数据安全大赛暨首届"数信杯"数据安全大赛数据安全积分争夺赛预赛南区- WriteUp By EDISEC。6道数据安全题涵盖内存取证(Win7SP1x64 volatility全流程)+SQL盲注双写绕(like binary逐位爆)+Download.php SSRF(#截断)+binwalk音频隐写+RC4+TEA(delta=555885348),末尾EDI招新招re/cry...
category: forensic
...
contest_type: open
wp_url: https://www.ctfiot.com/173946.html
reasoning_chain:
- 数据分析不安全的 U 盘 → 触发点：volatility 内存取证 + Win7SP1x64 profile
- 动作：volatility_2.6_win64_standalone -f 1.raw --profile=Win7SP1x64 printkey→hashdump→lsadump→cmdline→netscan→filescan grep .toml
- 观察：hahaha123/AcroRd32/192.168.31.238:4444/118.180.126.13:6770/attacker.toml
- 网站数据绝对安全 → 触发点：livwdaw+EccpSOIlRPolP936707
- Bitcoin 题 → 假设：transferFrom 函数调用 → 动作：分析转账记录
- Web-justsoso 题 → 触发点：双写绕 SQL 注入 aandnd/passwoorrd → 假设：like binary 逐位爆
- '假设：Download.php?img=http://127.0.0.1/flag.php%23 → SSRF + # 截断本地路径'
- Misc Magic Audio → 动作：binwalk → 菜就多练密码解 zip → flag
- Re rrrrcccc → 触发点：SMC 自修改 + RC4 + Whatareyourencryption&decryptionbasics + 三段 XOR
- drink tea → 假设：TEA delta 魔改 = 555885348 → 动作：识别 delta → 改 TEA 解密
failed_attempts:
- 直接 cat 找内存中的 flag → 失败：必须 volatility 完整流程
- 'Download.php?img= 直接 → 失败：必须 # 截断本地路径'
- TEA 标准 delta → 失败：本题魔改 delta=555885348
key_observations:
- Volatility 内存取证全流程：printkey→hashdump→lsadump→cmdline→netscan→filescan
- SQL 注入双写绕：aandnd(aand and)/passwoorrd(or)/-- ' 闭合
- like binary 逐字符二分爆
- 'Download.php?img=http://127.0.0.1/flag.php%23 SSRF # 截断'
- RC4 加密 + SMC 自修改代码是 RE 进阶套路
prerequisites:
- Volatility 内存取证命令
- SQL 注入双写绕 + like binary 爆破
- 'SSRF # 截断本地路径'
- RC4 + SMC 自修改代码识别
---
