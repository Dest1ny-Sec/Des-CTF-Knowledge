...
quality: high
full_path: 【三等奖方案】基于人工智能的漏洞数据分类赛题「终于交上了」团队解题思路.full.md
meta_path: 【三等奖方案】基于人工智能的漏洞数据分类赛题「终于交上了」团队解题思路.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: CCF BDCI 2022 三等奖 - AI漏洞分类 终于交上了战队。中科院计算所硕士用BERT+LightGBM集成在CVE漏洞分类上第5名,准确率0.4695。。经验：5个字段分开训练 + 用未标注60000条做领域MLM预训练 + LightGBM二分类更强 + Bert+Bert多分...
category: misc
...
contest_type: open
wp_url: https://www.ctfiot.com/125851.html
reasoning_chain:
- 触发点：上一题只用 TF-IDF + LGB 0.485 → 假设：加 BERT 语义能提分
- 动作：先做 LightGBM 二分类基线 (每个字段) → 观察：privilege 强、impact 弱
- 假设：BERT 能补 LGB 的语义盲区 → 动作：6 层 12 头 768 维小 BERT + MLM 预训练
- 训练数据有 6 万未标注 → 触发点：领域 MLM 可拉 → 动作：60k 全量语料 + 北大专利预训练
- 假设：通用 BERT 在 CVE 域不够 → 动作：动态掩码 MLM 续训 → 观察：loss 平滑下降
- 下游微调冻结前 5 层 → 假设：前 5 层是通用语法、CVE 域靠后层 → 动作：逐层学习率 (前 1e-5/后 5e-5)
- class imbalance → focal loss α 启发式 = 类别预测熵均值 → 假设：自动找平衡
- 触发点：LGB+BERT 单模型都到瓶颈 → 假设：两模型融合可上分 → 动作：LGB_proba × BERT_proba 逐元素相乘
- 融合后再 softmax → 假设：归一化能让融合稳定 → 观察：B 榜 0.4695 (线上第 5)
- A 榜 0.509 → B 榜 0.4695 → 触发点：换榜后衰减 < 4% → 不易过拟合
failed_attempts:
- 试图直接用预训练 bert-base + 微调 → 失败：CVE 术语太专，通用词向量撞不动
- 试图把 3 标签当一个多分类 → 失败：长尾分布差，focal loss 仍救不回来
- 试图 BERT 12 层全量微调 → 失败：过拟合灾难 (训练集 0.85、A 榜 0.45)
key_observations:
- 领域 MLM 预训练 (60k 未标) 在专业 NLP 任务上比通用预训练基线更适配
- BERT×LGB 概率逐元素相乘再 softmax = 两个模型分布融合 = 防过拟合
- focal loss α 用类别预测熵均值做启发式，比网格搜索更智能
- 冻结前 5 层 + 分层学习率 (前小后大) = BERT 微调防过拟合通用配方
- B 榜衰减率 < 4% 说明模型泛化好，过拟合检测比绝对分数更重要
prerequisites:
- BERT MLM 预训练 (HuggingFace Trainer)
- PyTorch 层级学习率 (param_groups)
- focal loss 原理 + α 参数选择
- 概率融合技巧 (相乘/相加/几何平均的差异)
---
