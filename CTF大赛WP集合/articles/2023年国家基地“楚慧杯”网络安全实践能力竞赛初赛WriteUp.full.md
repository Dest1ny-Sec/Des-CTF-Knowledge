...
quality: medium
full_path: 2023年国家基地“楚慧杯”网络安全实践能力竞赛初赛WriteUp.full.md
meta_path: 2023年国家基地“楚慧杯”网络安全实践能力竞赛初赛WriteUp.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: '2023 年国家基地"楚慧杯"网络安全实践能力竞赛初赛 WriteUp。楚慧杯初赛：哈希扩展+sqlmap+栈溢出 0x28+TEA+zip 套娃+Boneh-Durfee 大 e RSA。。关键路径：WEB: 哈希扩展攻击伪造 password123 + 文件名 SQL 注入 + sqlmap 一把梭 → PWN ez_base: shift+f12 找 flag.txt 字符串 + ...'
category: misc
...
contest_type: open
wp_url: https://www.ctfiot.com/151842.html
reasoning_chain:
- 触发点：WEB 哈希扩展攻击密码等于 password123 → 假设伪造 → 动作：直接进
- 动作：文件名出存在 SQL 注入 → 假设 sqlmap 一把梭 → 观察：盲注回显
- actor PWN ez_base shift+f12 找 flag.txt 字符串 → 假设后门函数地址 → 动作：0x404911 跳转
- 动作：栈溢出 0x28 字节 → 假设直接覆盖返回地址 → 观察：跳转后门拿 shell
- actor RE Babyre 32 轮 TEA加密 delta=0x9E3779B9 → 假设 4 个 key → 动作：解密还原
- actor MISC ez_zip 压缩包套娃 → 假设 Python 循环解压 → 动作：8 位二进制 → 字符串
- actor CRYPTO so-large-e 假设 Boneh-Durfee LLL → 动作：RSA-and-LLL-attacks 库解 d → 观察：还原
- actor 数独答案9 个 1-9 之和=405 → 假设 md5(405) → 动作：解出 sum 答案
failed_attempts:
- 试图手工拼接32 轮 TEA → 失败：delta + sum>>11 移位需反推
- 试图用 standard sqlmap 无字典 → 失败：需要延时或回显差异判断
key_observations:
- 大 e RSA 用 Boneh-Durfee LLL 攻击
- TEA delta=0x9E3779B9 + sum>>11 取 key
- 栈溢出字节数要逐个试（0x28）
- 数独答案之和恒为 405 是数学技巧
prerequisites:
- 哈希扩展攻击原理（SHA1/MD5）
- TEA 加密算法与密钥扩展
- Boneh-Durfee LLL 格攻击
- sqlmap 基础使用
---
