...
quality: high
full_path: 赛题解析之第九届(2024)全国高校密码数学挑战赛赛题三.full.md
meta_path: 赛题解析之第九届(2024)全国高校密码数学挑战赛赛题三.meta.md
images_removed: true
images_removed_count: 8
schema_version: v3.0.0-P0
summary: 赛题解析之第九届(2024)全国高校密码数学挑战赛赛题三。第九届全国高校密码数学挑战赛密码数学赛题三,AES侧信道攻击恢复密钥第一字节,涵盖CPA相关功耗分析+DPA差分功耗+互信息+KS检测+深度学习CNN方法,以及基于明文类别的均值滤波降噪。。关键路径：数据:六组三对plain/leakage(低中高噪声)+对应明文 → AES第一轮第一个S盒输出值的泄露样本,无法用汉明重量模型准确刻画...
category: crypto
subcategory: oracle
time_required: long
difficulty_score: 4
...
contest_type: school
wp_url: https://www.ctfiot.com/260473.html
reasoning_chain:
- 六组三对 plain/leakage（低中高噪声）+ 对应明文 → 触发点：AES 侧信道功耗数据
- 攻击目标：恢复 AES 密钥第一字节 → 假设：S 盒输出是功耗泄露源
- 动作：CPA 相关功耗分析 → 假设功耗函数 = HW(sbox[plain[0]^key_guess]) + Pearson 相关系数
- 动作：枚举 key_guess ∈ [0,255] → 对每个计算相关系数 → 取最大者为正确 key
- 观察：高噪声数据汉明重量模型失效 → 假设：需要更复杂特征
- 动作：DPA 差分功耗分析（均值差分 + 选择函数）→ KS 检测
- 动作：互信息 sklearn KBinsDiscretizer 离散化 + mutual_info_score
- 降噪：基于明文类别均值滤波 → 改善 SNR
- 深度学习 CNN 作为新特征提取方向 → 验证多方法
failed_attempts:
- 仅用汉明重量模型处理高噪声数据 → 失败：相关系数 < 阈值
- 纯 DPA 不做 KS 检测 → 失败：噪声数据选择函数差异不显著
- 直接用 CNN 端到端 → 失败：数据量不够 CNN 收敛
key_observations:
- 侧信道核心是挖掘信息特征（CPA/DPA/互信息/CNN），汉明重量失效时需更复杂特征
- 降噪常用基于明文类别的均值滤波，可显著改善 SNR
- 深度学习 CNN 是侧信道特征提取新方向，但需要充足数据
- KS 检测 + 互信息 + Pearson 相关系数是三类互补的统计攻击信号
prerequisites:
- AES S 盒与第一轮 SubBytes 实现
- CPA/DPA 侧信道原理与 Pearson 相关系数
- sklearn KBinsDiscretizer 与 mutual_info_score
- 深度学习 CNN 基础（PyTorch / TensorFlow）
---
