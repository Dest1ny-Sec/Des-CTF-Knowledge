...
quality: high
full_path: 看雪2022_KCTF_春季赛_-_第五题设计思路及解析.full.md
meta_path: 看雪2022_KCTF_春季赛_-_第五题设计思路及解析.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: 看雪2022 KCTF 春季赛 | 第五题设计思路及解析。KCTF2022春季赛Q5《危机四伏》：32/64天堂之门+SEH+三段混淆算法+base64非预期。经验：天堂之门对抗大部分调试器；非预期思路：硬件断点+base64字符串+比对历史KCTF题目
category: reverse
subcategory: reverse
tools_used:
- x86/x64
...
contest_type: open
wp_url: https://www.ctfiot.com/42076.html
reasoning_chain:
- 触发：32位 exe + x86/x64 混合 + SEH 异常干扰调试
- 假设：天堂之门 CS=0x33 切换 + retf → 动作：识别 Far jump 段间调用
- 观察：算法分 3 部分 (64+32+64) → cpp SEH 异常处理干扰
- 假设：上方有 base64 字符串 → 动作：硬件断点 + 字符串观察
- 观察：KCTF2021 春季赛第 7 题对比 → 假设：历史题非预期
- 假设：XOR_NUMBER=0x8160C68FF6C4875E^0x090A0B004654434B → 动作：单字节 xor+移位
- 观察：DecM 解密 → flag
failed_attempts:
- 试图直接 F5 反编译 → 失败：天堂之门切换 + 64/32 混合反编译烂
- 试图 x64dbg 动态调试 → 失败：天堂之门对抗 + cpp 异常干扰
- 试图忽略 base64 字符串 → 失败：base64 是关键提示
key_observations:
- 天堂之门 (CS=0x33 retf) 对抗大部分调试器
- 三段 (64+32+64) 混合算法增加逆向难度
- cpp SEH 异常处理是干扰调试常见手法
- 非预期思路：硬件断点 + 字符串观察 + 历史题对比
- XOR 异或 + LEFTMOVE/RIGHTMOVE 移位是常见混淆
prerequisites:
- x86/x64 指令集 + 段间跳转 retf 原理
- 天堂之门原理（CS=0x33 切换）
- IDA/x64dbg 硬件断点使用
- base64 / XOR 单字节操作识别
---
