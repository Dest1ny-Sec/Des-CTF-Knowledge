...
quality: medium
full_path: 看雪2022_KCTF_春季赛_-_第二题设计思路及解析（含视频）.full.md
meta_path: 看雪2022_KCTF_春季赛_-_第二题设计思路及解析（含视频）.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: 看雪2022 KCTF 春季赛 | 第二题设计思路及解析（含视频）。KCTF2022春Q2《末日邀请》签到题：CRC32折叠signA+Collatz序列+KCTF固定字符+前3位异或signB。经验：签到题常用Collatz(3n+1) + 字节异或；魔改CRC32折叠算法
category: reverse
subcategory: reverse
time_required: quick
difficulty_score: 2
...
contest_type: open
wp_url: https://www.ctfiot.com/40706.html
reasoning_chain:
- 触发：题面提 Collatz(3n+1) + 序列号 421KCTF381654729
- 假设：CRC32-like arrayA 折叠 signA=0xF52E0765 → 动作：写 SN 折叠算法
- 观察：arrayB = Collatz 序列 → signB = arrayB[0xC6]|arrayB[0xC5]|arrayB[0xC4]
- 假设：SN 前 3 位字节异或 = signB → 动作：3 字节异或逆推
- 假设：4 字节 KCTF 固定 → 动作：填入 KCTF 位置
- 观察：Arglist[3]=0x14 [4]=0xC [5]=0x1D [6]=0xF → 假设：SN 第 4 位是 KCTF
failed_attempts:
- 试图用标准 CRC32 算 signA → 失败：魔改 CRC32 折叠算法不是标准表
- 试图不解 Collatz 直接硬猜 → 失败：Collatz 序列决定 arrayB
- 试图 IDA F5 不看汇编 → 失败：魔改折叠算法在汇编层
key_observations:
- 签到题常用 Collatz(3n+1) + 字节异或组合
- 魔改 CRC32 折叠算法（不是标准 CRC32 表）
- 'View→Strings + Options→Default 8 bits: GBK 解决中文乱码'
- KCTF 作为固定 4 字节是签到题彩蛋
- 序列号分 3 段 (3+4+7) 设计
prerequisites:
- Collatz 猜想（3n+1）序列生成
- CRC32 算法基础 + 魔改折叠版本理解
- IDA 字符串查看 + GBK 编码设置
- 字节异或算法（按位 XOR）
---
