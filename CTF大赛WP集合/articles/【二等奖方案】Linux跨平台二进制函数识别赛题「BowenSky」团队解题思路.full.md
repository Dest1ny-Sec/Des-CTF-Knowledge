...
quality: high
full_path: 【二等奖方案】Linux跨平台二进制函数识别赛题「BowenSky」团队解题思路.full.md
meta_path: 【二等奖方案】Linux跨平台二进制函数识别赛题「BowenSky」团队解题思路.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: CCF BDCI 2022 二等奖 - Linux跨平台二进制函数识别 BowenSky。上海交大NETSEC实验室用Bi-LSTM+GatedGCN+MS_loss在二进制函数相似性检测拿二等奖第4。。经验：二进制函数相似性检测 = 度量学习;MS miner比Norm weighted miner更关注信息量大的样本对(0....
category: reverse
subcategory: reverse
time_required: long
difficulty_score: 4
...
contest_type: open
wp_url: https://www.ctfiot.com/124838.html
reasoning_chain:
- 赛题：跨平台二进制函数相似性检测 (x86/ARM/MIPS) → 触发点：函数级别匹配
- 假设：汇编指令标准化是关键 → 动作：去掉前缀 (jmp/call/mov) + 词表化 → 观察：12532 词表
- 触发点：单条指令无法表达控制流 → 假设：基本块语义需要上下文
- 动作：Bi-LSTM 2 层 128 维编码基本块 → 观察：双向比单向准
- CFG 结构用 GatedGCN-E 提取 → 假设：基本块构成图结构 → 动作：3 层 128 维 GatedGCN
- 触发点：函数匹配 = 相似度学习 → 假设：度量学习 (metric learning) 比分类更合适
- 动作：孪生网络结构 + 余弦相似度 → 观察：收敛比 CrossEntropy 稳
- Multi-Similarity (MS) 采样 → 假设：难样本对更重要 → 动作：MS miner 选 batch 内负对
- 损失用 MS loss (ε=0.1, α=2, β=50) → 动作：比 Triplet loss +0.04 AUC
- 触发点：模型要小 → 假设：CPU 推理 123ms 才是落地标准 → 观察：模型 11MB + 2.6M 参数
- '最终 B 榜 #4 二等奖 → 触发点：MS loss 比 Norm weighted miner +0.027'
failed_attempts:
- 试图用 jTrans (Transformer) 直接特征提取 → 失败：模型太大、CPU 推理超 500ms 不达标
- 试图用 Triplet loss → 失败：负样本 mining 不够，MS loss 比它高 0.04
- 试图用 Norm-Weighted miner → 失败：忽略难正样本，AUC 比 MS 损失低 0.027
key_observations:
- 二进制函数相似性 = 度量学习 (孪生 + 余弦) 比分类学习更合适
- MS (Multi-Similarity) loss + MS miner 比 Triplet/Norm-Weighted 更关注信息量大的样本对
- 指令标准化大幅降词表 (12532 比原始助记符数万降到万级)
- Bi-LSTM + GatedGCN 双路结构只 11MB 模型即可 CPU 123ms 推理，比 Transformer 落地友好
- 跨平台函数识别本质是反汇编 + 神经网络 + 度量学习三件套
prerequisites:
- 反汇编基础 (objdump/radare2/Ghidra)
- Bi-LSTM / GatedGCN 图神经网络
- 度量学习 (siamese + cosine + MS/Triplet loss)
- PyTorch 孪生网络实现
---
