...
quality: high
full_path: Realworld_CTF_2023_Ferris_proxy.full.md
meta_path: Realworld_CTF_2023_Ferris_proxy.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: Realworld CTF 2023 - Ferris_proxy 逆向分析。Realworld CTF 2023 Ferris_proxy：Rust 协议逆向 (有符号) + pcapng 全流量 rc4 解密 + RSA 加密 AES 密钥交换 + AES-128-CBC IV 16 字节 + ZIP XOR 双方 key。。关键路径：Rust binary (有符号) server 监...
category: crypto
subcategory: oracle
tools_used:
- Python
...
contest_type: open
wp_url: https://www.ctfiot.com/106020.html
reasoning_chain:
- Rust binary (有符号) server 8888 + client → 触发点：自定义协议 + pcapng 全流量
- strace 看 server 监听 8888 → 假设：TCP 自定义协议 → 动作：开 pcapng 看 TCP data
- serde_yaml::from_str::h51d2279d81ebe645 读硬编码 RSA 密钥 → 假设：协议某处用 RSA
- client 硬编码 publickey=server publickey → 假设：公钥加密私钥解密 → key_exchange
- lib::protocol::rc4::Rc4::new::h22d438f41ed76e26 → 假设：rc4 全流量加密 → 动作：调试发现 key=explorer
- 动作：用 rc4 解所有 data，分 c2s/s2c → 观察：12 字节包头 (4 length + 4 action + 4 streamid)
- action 0=open / 1=transfer / 2=close → 每个 stream 都 key_exchange (256 字节 RSA + 0100 + SHA256(aes_key))
- AES 密钥 = 双方随机数 ZIP XOR (rust 闭包 a^b) → 解 AES-128-CBC + IV 16 字节 → 找到 HTTP/1.0 200 OK SimpleHTTP/0.6 Python/3.6.9 → flag.txt
failed_attempts:
- 试图直接 AES 解密 → 失败：需先 RSA 拿 aes_key
- 试图只看 client binary → 失败：必须分析 server + pcapng
- 试图找硬编码 key → 失败：rc4 key 藏在 rc4::new 调用
key_observations:
- Rust 协议逆向关键是闭包 (a^b) = zip+map 模式
- rc4 全流量加密是初阶 (key=explorer)
- RSA+AES key_exchange 模式在 4 stream 中重复
- SHA256(aes_key) 校验保证双方 key 一致
- AES-128-CBC IV 每次 16 字节新生成是标准做法
prerequisites:
- Rust 闭包 + zip+map 反汇编
- RC4 流密码原理
- RSA+AES key_exchange 模式
- pcapng 流量分析 (c2s/s2c 分离)
---
