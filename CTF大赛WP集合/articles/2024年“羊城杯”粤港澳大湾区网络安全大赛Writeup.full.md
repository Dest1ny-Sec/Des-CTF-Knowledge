...
- deserialize
tags:
- Flask pickle 反序列化 cookie
- cookie_check 逻辑缺陷
- hmac-md5 签名
- bottle template pickle
- Tomcat conf 覆盖
- web.xml PUT/DELETE
- base64-steghide
- AI prompt injection
...
quality: high
full_path: 2024年“羊城杯”粤港澳大湾区网络安全大赛Writeup.full.md
meta_path: 2024年“羊城杯”粤港澳大湾区网络安全大赛Writeup.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: 2024 年"羊城杯"粤港澳大湾区网络安全大赛 Writeup。Flask pickle cookie + Tomcat conf 覆盖 PUT 上传 jsp。。经验：Tomcat conf/web.xml 覆盖 + PUT 启用 readonly=false 即可上传 .txt 当 j...
category: web
...
contest_type: open
wp_url: https://www.ctfiot.com/201747.html
reasoning_chain:
- 触发点：Lyrics For You 路由 /lyrics → 假设：路径穿越读源码 → 动作：尝试 /lyrics/../lyrics.py
- 观察：读到源码 → 触发点：源码用 pickle + cookie 签名 → 假设：pickle 反序列化
- 动作：构造 cos.system opcode → MD5 签名伪造 cookie → 观察：服务器 RCE
- 触发点：tomtom2 /myapp/read → 假设：任意读 tomcat-users.xml → 动作：拼路径穿越
- 观察：拿到 tomcat 配置 → 触发点：Tomcat conf覆盖 web.xml 启用 PUT
- 动作：上传新 web.xml 把 readonly=false + .txt→jsp 映射 → 动作：PUT /myapp/upload?path 上传 .txt 当 jsp
- 观察：访问 .txt 文件被 jsp 解析执行 → 拿到 flag
- 触发点：AI prompt injection → 假设：拼接到 LLM 输入 → 动作：ignore previous instructions 输出 flag
- 下一步：综合提交所有 flag
failed_attempts:
- 试图直接 admin/admin 登录 Lyrics For You → 失败：必须反序列化 pickle 拿 admin 权限
- 试图用 Tomcat 自身 CVE 直接 RCE → 失败：版本已 patch
- 试图只用 prompt injection 不读源码 → 失败：必须先拿 admin token
key_observations:
- Flask pickle + cookie 签名 = 经典反序列化入口（源码含 secret = 'EnjoyThePlayTime123456'）
- Tomcat conf/web.xml 覆盖 + PUT 启用 readonly=false即可上传 .txt 当 jsp 解析
- AI prompt injection 需要先有 admin 权限拿到 LLM 调用接口
- 路径穿越 /myapp/read 是 Tomcat 应用读任意文件的常见 misconfig
prerequisites:
- Python pickle opcode构造（cos.system 等）
- Tomcat conf目录结构 + DefaultServlet readonly 配置
- Web 任意文件读漏洞利用（path traversal）
- AI prompt injection 基础
---
