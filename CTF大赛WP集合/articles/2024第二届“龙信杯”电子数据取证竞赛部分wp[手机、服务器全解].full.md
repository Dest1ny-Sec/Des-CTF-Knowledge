...
quality: medium
full_path: 2024第二届“龙信杯”电子数据取证竞赛部分wp[手机、服务器全解].full.md
meta_path: 2024第二届“龙信杯”电子数据取证竞赛部分wp[手机、服务器全解].meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: 2024 第二届"龙信杯"电子数据取证竞赛部分 WP（手机+服务器全解）。龙信杯计算机取证：恶意程序解密 t.wnry（WannaCry 变种勒索）后 dll 的 MD5 值；属于应急响应+样本分析综合取证。。关键路径：题：恶意程序解密了 t.wnry 后该 dll 的 md5 值（标准格式：全大写） → 用 IDA / x64dbg 动态跟踪 WannaCry 变种解密流程 → 解密 ...
category: forensic
...
contest_type: open
wp_url: https://www.ctfiot.com/207925.html
reasoning_chain:
- 触发点：恶意程序解密 t.wnry（WannaCry 变种）后 dll 的 md5 值 → 假设：动态跟踪 WannaCry 变种
- 动作：IDA / x64dbg 打开恶意程序 → 设断点解密函数 → 单步跟踪
- 观察：t.wnry 解密为 dll → 动作：导出 dll 文件
- 动作：MD5 计算 → 观察：拿到 flag（标准格式全大写）
- 触发点：手机取证 adb 连接设备数 → 假设：看 adb_temp_keys.xml → 动作：misc 下找 adbkey
- 动作：count adbkey 记录数 → flag2
- 触发点：机主参加考试时间 → 假设：日历 / 短信 → 动作：grep 2024-08-23 → flag3
- 触发点：蓝牙 MAC 地址 → 假设：系统设置 → 动作：48:87:59:76:21:0f → flag4
- 触发点：filecompress 加密文件数 → 假设：火眼特征 →动作：分析得6 → flag5
- 触发点：155 手机号码 → 假设：通讯录 + 短信 → 动作：另一个号码 → flag6
failed_attempts:
- 试图静态分析 WannaCry 变种 → 失败：必须动态跟踪解密函数
- 试图不看 adb_temp_keys.xml 数 → 失败：policies 表是 Magisk Hide 不是连接设备
- 试图 base64 解密中文字段 → 失败：取证答案严格按题目格式
key_observations:
- WannaCry 变种 t.wnry 加密扩展名是 2024 龙信杯常见考点
- MD5 答案严格全大写（取证比赛答案格式规范）
- adb_temp_keys.xml 才是 ADB 连接设备计数源，policies 表是干扰
- 龙信杯是国内电子数据取证头部赛事（苏州龙信科技主办）
prerequisites:
- WannaCry 勒索软件解密流程（动态调试）
- iOS / Android 取证基础（adb / iTunes 备份 / 雷电分析）
- MD5 算法与全大写格式
- 火眼取证 / 雷电模拟器使用
---
