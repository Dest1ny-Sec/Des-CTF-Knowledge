...
quality: high
full_path: 磐石CTF2025_Pwn赛题user解析：解密0xfbad1880背后的IO_FILE结构体利用技术.full.md
meta_path: 磐石CTF2025_Pwn赛题user解析：解密0xfbad1880背后的IO_FILE结构体利用技术.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: 磐石CTF2025 Pwn赛题user解析：解密0xfbad1880背后的IO_FILE结构体利用技术。磐石CTF2025 user：glibc 2.31菜单题无show+IO_FILE结构体伪造+stdout泄露libc。经验：堆bss数组距stdout固定0x40字节可用edit(-8)修改stdout；_flags=0xfbad1880触发写...
category: pwn
...
contest_type: open
wp_url: https://www.ctfiot.com/265310.html
reasoning_chain:
- 触发点：菜单 add/delete/edit 三件套+show 不可用+delete/edit 不判负数 idx→假设：bss 数组越界可改 stdout
- 假设：bss 上 heap[5] 与 _IO_2_1_stdout_ 距离固定 0x40 字节→动作：edit(-8) 写到 stdout 偏移
- 观察：edit 写 0x40 数据覆盖 stdout 前 0x40 字节→下一步：改 _flags=0xfbad1880 + _IO_write_base/_ptr
- _flags=0xfbad1880 含 _IO_CURRENTLY_PUTTING→假设：触发 puts 时会按 _write_base→_write_ptr 输出→动作：_write_base 设为低地址(其他 IO 结构体)
- 观察：puts 输出 libc 段地址→libc_base 已泄→下一步：用 edit 写其他结构体或打 tcache 任意写
- 假设：拿到 libc 后打 system(/bin/sh)→动作：利用 edit 错位覆盖写 GOT 或 tcache
- 观察：完成利用
failed_attempts:
- 试图用 _IO_2_1_stderr_ 触发泄漏 → 失败：edit 负数 idx 范围更倾向 stdout
- 试图只改 _IO_write_base 不动 _flags → 失败：_flags 错则不进入缓冲输出路径
- 试图走 show 路径 → 失败：show 函数是摆设
key_observations:
- bss 段全局数组与 _IO_2_1_stdout_ 距离通常固定，edit 负数 idx 是 IO_FILE 伪造入口
- _flags=0xfbad1880(含 _IO_CURRENTLY_PUTTING)是触发 stdout 输出缓冲刷新的 magic
- 无 show 函数的 2.31 堆题常用 IO_FILE 结构体伪造绕开 show 限制
- stdout 泄露 libc 后一般结合 tcache 任意写打 free_hook/system
prerequisites:
- glibc IO_FILE 结构体(_flags/_IO_write_base/_IO_write_ptr)
- bss 段布局观察(heap 数组与 stdout 偏移)
- glibc 2.31 堆管理(tcache 任意写)
- edit/delete 负数 idx 越界原理
---
