...
- 'Part 1: purchaseTest + approve 钩子 — 注册 FakeNFT，在 approve 回调里把 NFT 转给 USER，让 purchaseOrder 给 USER 转账'
- 'Part 2: Solidity 0.8.15 calldata tuple head overflow bug — abi.encode 转发 SignedCoupon 时 orderId 被覆盖'
- 攻击者构造 orderId=1 的 coupon，但 verifyCoupon 内取 order[0]（自己控制的）
- 'Part 3: 综合：airdrop 5 → purchaseTest 拿 1337 → buy #2 → mint FakeNFT 上架 → buy #1 把 own 单变 order[0] → coupon 买 #3'
key_payload: I, the issuer\
...
quality: high
full_path: 0CTF_2022_TCTF_NFT_Market.full.md
meta_path: 0CTF_2022_TCTF_NFT_Market.meta.md
images_removed: true
images_removed_count: 0
schema_version: v3.0.0-P0
summary: '0CTF/TCTF 2022 NFT Market (Solidity 0.8.15 calldata bug)。Solidity 0.8.15 calldata tuple head overflow + 数组 swap 删除 + approve 钩子。关键路径：目标：拿到 3 个 NFT（id=1, 2, 3）触发 win() emit SendFlag() → Part 1: purc...'
category: web
subcategory: logic
subcategories:
- logic
...
contest_type: open
wp_url: https://www.ctfiot.com/58223.html
reasoning_chain:
- 触发点：3 个 NFT 上架价 1/1337/13333333337，purchaseTest + purchaseWithCoupon 各只能用一次 → 假设：必须利用 one-shot 钩子
- 假设：purchaseTest() 里 approve() 是 ERC721 reentrancy hook → 动作：读 _deleteOrder() 看实现，发现 orders[id] = orders[last] + pop 做 swap-delete
- 观察：order 索引在 add/delete 后会变 → 下一步：注册 FakeNFT 在 approve()回调里把 NFT safeTransferFrom 给 USER
- 动作：FakeNFT.approve() 第 1 次调用 super.safeTransferFrom(msg.sender, USER, 1) → 观察：purchaseOrder 把 1337 token 转给 USER（ownerOf 变成 USER）
- '假设：拿到 1337 后买 NFT 2 → 动作：airdrop 5 + purchaseTest 拿 1337 + 买 #2 → 观察：剩余订单 [NFT 1, NFT 3]'
- 假设：purchaseWithCoupon 转发 SignedCoupon 用 calldata，Solidity 0.8.15 有 bug → 动作：搜 solidity 已知 bug list
- 观察：找到 'Head Overflow Bug in Calldata Tuple ABI-Reencoding'，bug 影响 0.5.8-0.8.15 → 下一个动态 tuple 头32 字节被清零
- 假设：abi.encode('...', order, ...) 里 order 是第一个 dynamic tuple 头部，orderId 被清零 → 动作：发 coupon orderId=1 让 verifyCoupon 取 order[0]
- 观察：实际 orderId 变成 0，我们给订单 0 签名 → 下一步：让订单 0 是我们控制的 NFT
- '动作：mint FakeNFT #2 + createOrder + 买 NFT 1 把 own 单变 order[0] → 观察：orders=[us, NFT 3]，coupon 验证走我们订单 → 完成'
failed_attempts:
- 试图用 Part 1 reentrancy 思路在 purchaseWithCoupon 上重入 → 失败：verifyCoupon/getOrder/ownerOf 全是 view/staticcall，无 reentrancy hook
- 试图让 transferFrom 直接发给 USER → 失败：ownerOf 在那一刻等于 market 合约，转账会转给 market 自己
- 试图绕过 coupon 签名验证 → 失败：ecrecover 验证 issuer 必须是 NFT owner，签名无法伪造
key_observations:
- Solidity 编译器历史 bug（如 calldata tuple head overflow）是 web3 CTF 高难度考点，0.8.15/0.8.16 是分水岭
- ERC721 approve() 回调是 reentrancy hook（vs ERC20 transferFrom），能插入 createOrder/cancelOrder 改变数组索引
- 变长数组用 swap-and-pop 删除是 OpenZeppelin EnumerableSet 模式，被外部可观察 id 时可被利用
- 签名前后端传同一结构体但用 calldata 转发，触发 ABI reencoding bug 是 web3 攻击经典套路
- Foundry setUp/startPrank 比 Hardhat 写多阶段 attack script 更适合复杂状态机测试
prerequisites:
- Solidity 0.8.x 编译器历史 bug 列表（重点0.8.15 前的 calldata reencoding）
- ERC721/ERC20 approve + safeTransferFrom + transferFrom 钩子顺序
- Foundry cheatcodes（vm.sign/vm.startPrank/vm.label）使用
- ABI 编码规则（dynamic tuple head + abi.encode）
- 数组 swap-and-pop 删除模式与 EnumerableSet 实现
---
