...
- post_id_crypto_randomuuid_leak
- opener_opener_xss
- same_origin_method_execution
- post_ejs
- 8bit_xsleak
...
quality: high
full_path: '[原创]Realworld_CTF_2023_The_cult_of_8_bit详解.full.md'
meta_path: '[原创]Realworld_CTF_2023_The_cult_of_8_bit详解.meta.md'
images_removed: true
images_removed_count: 6
schema_version: v3.0.0-P0
summary: '[原创] Realworld CTF 2023 The_cult_of_8_bit 详解。Realworld CTF 2023 The cult of 8 bit 详解：JSONP callback SOME 攻击 + XHR %00 报错触发 fallback + iframe name 字符 focus 事件 + opener 跨域访问，逐字符泄漏 36 字符 UUID post id ...'
category: web
...
contest_type: open
wp_url: https://www.ctfiot.com/91479.html
reasoning_chain:
- 触发点:后端 /post/:id 路由 + res.jsonp({success, name, body}) + DOMPurify 强过滤 → 假设:JSONP callback 可控 → 动作:?id={uuid}%3Fcallback=alert%23%00 切断 callback
- 观察:SOME (Same-Origin Method Execution) 攻击 + XHR fallback 触发 JSONP → 下一步:逐字符盲注
- 触发点:admin CSRF/csrf token 保护 + admin.user==='admin' 拒 create/post → 假设:必须盲注 → 动作:iframe.allow='sync-xhr 'none'' 禁用 XHR
- 动作:16 iframe name=0123456789abcdef- focus 事件 + setInterval 检测 document.activeElement.name → 观察:字符泄漏 36 字符 UUID
- 动作:opener 跨域访问 → opener.opener.document.body.children[1].childNodes[1].children[0].children[0].children[3].children[0].children[0].children[0].href[32+i].focus → 观察:36 字符 id 完整泄漏
- 动作:访问 /post/?id={uuid} → 观察:得 flag
failed_attempts:
- 试图直接创建 post → 失败:admin 权限校验
- 试图用 fetch 读 cookie → 失败:Same-Origin Policy
- 试图单字符 iframe → 失败:16 iframe 必须并行
key_observations:
- SOME (Same-Origin Method Execution) 攻击是黑帽 EU 14 论文提出的经典 XSS 替代技术
- sync-xhr 特征策略 + %00 URL 截断是绕 XHR fallback 触发 JSONP 的标配
- iframe name + document.activeElement 字符级盲注是 Realworld CTF 标准手法
- admin.user 校验 + CSRF token 是 modern web 双层防护
- 36 字符 UUID + alphabet='0123456789abcdef-' 16 iframe 是泄漏优化配置
prerequisites:
- SOME 攻击 (Same-Origin Method Execution)
- JSONP callback XSS + XHR fallback
- iframe name + focus 事件字符级盲注
- opener 跨域访问
---
