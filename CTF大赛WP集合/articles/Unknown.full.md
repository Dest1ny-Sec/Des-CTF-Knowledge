...
- tcache_poisoning
- fastbin_attack
- smallbin_consolidate
- free_hook_overwrite
- fake_chunk_in_string
- shared_ptr_vtable
...
quality: high
full_path: Unknown.full.md
meta_path: Unknown.meta.md
images_removed: true
images_removed_count: 0
schema_version: v3.0.0-P0
summary: 'Unknown (smoothie_operator)。高级 C++ shared_ptr 堆利用 OOB write + UAF 重叠字符串指针 + 双重 free_hook 覆写 + 字符串内嵌 /bin/sh，flag: Sh4r3d_ptrs_R_sm00th$。。经验：shared_ptr 引用计数被 OOB 写 0 后析构会释放 string 内存但保留 pastry 引用，是堆...'
category: pwn
...
contest_type: open
wp_url: https://www.ctfiot.com/78940.html
reasoning_chain:
- 触发点：Monster::edit_params 输入 0 → n = ARRSIZE - 1 → quantities[0xff] 越界 → OOB write
- 假设：edit_params 是逻辑漏洞，不是普通 OOB → 假设：触发 0xff 偏移可对齐 pastry 对象前 0xff*4 字节
- 动作：edit_monster(33, 0, 0, 20, 20) → 覆盖 pastry shared_ptr control block counter=0 → 触发点：shared_ptr 析构泄漏 heap
- 动作：触发 shared_ptr use_count==0 析构 → 字符串内存被释放但 pastry 引用保留 → 触发点：UAF
- 假设：泄漏 heap + libc 需要分两步 → 动作：先 leak1 = print_queue() 拿残留堆地址
- 假设：consolidate 触发 main_arena 残留 → add_complaint('B' * 0x2000) → 0x2000 chunk 触发 smallbin 残留 main_arena+0x128
- leak2 = print_queue() → glibc_addr = leak2 → free_hook = glibc_addr + 0x2248 → system = free_hook - 0x19cbb8
- 动作：7 次 fill + resolve_complaint 翻转 tcache/fastbin → 制造 UAF 重叠字符串指针
- 动作：edit_pastry(49, 1, free_hook & 0xffffffff, 0, 20) + edit_pastry(49, 2, free_hook >> 32, 0, 20) → UAF 写 free_hook
- 动作：edit_complaint(14, system.to_bytes(8, 'little')) + add_complaint('/' * 0x30 + 'bin/sh') → free complaint 15 → system('/bin/sh')
- 假设：flags Sh4r3d_ptrs_R_sm00th$ → 输出触发 flag
failed_attempts:
- 试图直接覆盖 tcache_perthread_struct → 失败：必须先 UAF 出重叠 chunk 才能改 tcache fd
- 试图爆破 free_hook 偏移 → 失败：必须靠 smallbin 残留泄漏 main_arena 推算 libc
- 直接走 system('/bin/sh') → 失败：必须先制造 UAF 重叠 string data 指针改 free_hook
key_observations:
- shared_ptr 引用计数被 OOB 写 0 后析构会释放 string 内存但保留 pastry 引用，是堆重叠+任意地址写的关键原语
- 'C++ shared_ptr control block layout: 0x00-0x08 vtable / 0x08-0x0c ref count / 0x0c-0x10 weak count'
- smallbin 残留 main_arena+0x128 是 leak libc 的经典路径（glibc 2.27+）
- tcache + fastbin 翻转 7 次翻转是堆风水构造 UAF 的标准流程
- free_hook → system 的偏移在 libc 2.31/2.35 中是 +0x2248 是固定偏移
prerequisites:
- C++ shared_ptr 内部结构（vtable / ref_count / weak_count）
- glibc heap 高级利用（tcache / fastbin / smallbin / consolidate）
- UAF 重叠指针构造（fill+resolve 翻转）
- libc 偏移计算（free_hook / system 偏移）
- pwntools + GDB 调试堆栈
---
# Unknown
