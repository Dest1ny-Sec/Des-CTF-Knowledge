...
quality: high
full_path: 香港網安奪旗賽HKCERT_CTF_2024_Write_up（上）.full.md
meta_path: 香港網安奪旗賽HKCERT_CTF_2024_Write_up（上）.meta.md
images_removed: true
images_removed_count: 10
schema_version: v3.0.0-P0
summary: 香港網安奪旗賽HKCERT CTF 2024 Write up（上）。HKCERT 2024上篇覆盖Flask+PDF生成器+自定义C服务端+JSPyaml+奇美拉五道Web+两道Misc+Forensics的混合WP，亮点在pyodide+js-yaml-js-types的客户端沙箱逃逸。。关键路径：审计GroupAPI + compute_hash构造sha256爆破admin密码7df...
category: web
...
contest_type: open
wp_url: https://www.ctfiot.com/216353.html
reasoning_chain:
- GroupAPI 审计 → 触发点：compute_hash 构造 sha256 → 假设：弱密码爆破
- 动作：admin 密码 7df71e 爆破 → 观察：登入
- flask /process wkhtmltopdf 拼接命令 → 假设：session_id 可控触发命令注入
- 动作：session_id=;cat /flag → wkhtmltopdf 执行 → 观察：flag 落地
- PDF 生成器 meta tag → 假设：pdfkit ---enable-local-file-access 开本地读
- 动作：<meta http-equiv='refresh' content='0;url=file:///etc/flag'> → 观察：读 flag
- 自定义 C socket read_file + 真实路径拼接 + ends_with 白名单 → 假设：/../../目录穿越
- 动作：GET /../../../../../../../flag.txt → 观察：flag 落地
- pyodide + js-yaml → 假设：schema 扩展支持 js/function → 动作：pyodide.code.run_js
- 假设：cookie debug=on 绕 isLoopback → 动作：JSPyaml 用 YAML 重定向到 exec
- PHP CitrusWorkspace create 软链 → 动作：symlink('/flag','/tmp/x') → 读 flag
failed_attempts:
- 试图直接读 /etc/passwd 找 flag → 失败：flag 不在标准路径
- 试图不开 debug cookie 用 pyodide → 失败：被 isLoopback 拦
key_observations:
- wkhtmltopdf 拼接 session_id 触发命令注入是 PDF 生成器 LFI 通法
- pdfkit meta tag ---enable-local-file-access 是 LFI 隐藏开关
- pyodide + js-yaml schema + js/function 组合是客户端沙箱逃逸新套路
- symlink + 任意文件写入是 PHP CitrusWorkspace 类题软链读取套路
prerequisites:
- Flask / PDF 生成器漏洞（wkhtmltopdf / pdfkit）
- 自定义 C socket read_file 目录穿越
- pyodide + js-yaml schema 扩展
- PHP symlink 软链攻击
---
